<?php 
	...
	********************************************************************/
	...
	class Billdesk_pg_module_cron_manual extends CI_Model 
	{
		public function billdesk_cron_settlement_common($responsedata='', $encData='')
		{
			//print_r($responsedata); echo '<br>'; print_r($encData); exit;
			...
			{
				sleep(2);
				$MerchantOrderNo = $responsedata['mercid']; 
				$transaction_no  = $responsedata['transactionid'];
				$payment_status = 2;
				$attachpath=$invoiceNumber=$admitcard_pdf='';
				$auth_status = $responsedata['auth_status'];
				...
				{	
					...
					if($get_user_regnum[0]['status']==2) 
					{
						...
						if($this->db->affected_rows())
						{
							...
							######### payment Transaction ############
							$update_data = array('transaction_no' => $transaction_no,'status' => 1,'member_regnumber'=>$applicationNo,'transaction_details' => $responsedata['transaction_error_type']." - ".$responsedata['transaction_error_desc'],'auth_code' => '0300', 'bankcode' => $responsedata['bankid'], 'paymode' => $responsedata['additional_info']['additional_info1'],'callback'=>'C_S2S');
							$this->db->order_by('id','DESC');
							$this->db->limit(1);	
							$this->master_model->updateRecord('payment_transaction',$update_data,array('receipt_no'=>$MerchantOrderNo));
