<?php
  /********************************************************************************************************************
  ** Description: Common Model for SUPERVISION Module
  ...
  ********************************************************************************************************************/
	class Supervision_model extends CI_Model
	{
		...
		{
			$login_user_id = $this->session->userdata('SUPERVISION_LOGIN_ID');
			$login_user_type = $this->session->userdata('SUPERVISION_USER_TYPE');
			if(isset($login_user_id) && $login_user_id != "" && isset($login_user_type) && $login_user_type != "")
			{
        if($login_user_type == 'admin') //CHECK IN ADMIN
        {
          $admin_data = $this->master_model->getRecords('supervision_admin', array('admin_id' => $login_user_id, 'is_active' => '1', 'is_deleted' => '0'), 'admin_id');

          if(count($admin_data) > 0) { redirect(site_url('supervision/admin/dashboard_admin'),'refresh'); }
				  else { redirect(site_url('supervision/login/logout'),'refresh'); }
        }
        else if($login_user_type == 'candidate') //CHECK IN AGENCY
        {
				  $candidate_data = $this->master_model->getRecords('supervision_candidates', array('id' => $login_user_id, 'is_active' => '1', 'is_deleted' => '0'), 'id');

          if(count($candidate_data) > 0) { redirect(site_url('supervision/candidate/dashboard_candidate'),'refresh'); }
				  else { 
            redirect(site_url('supervision/login/logout'),'refresh'); 
          }
        }	
    ...
    /******** START : CHECK SESSION AFTER LOGIN FOR ALL ADMIN, AGENCY, CENTER PAGES ********/
		function check_admin_session_all_pages($login_as='') //login_as = admin or candidate or centre
		{
			...
			{
        if($login_as == '') { redirect(site_url('supervision/login/logout'),'refresh'); }
        else
        {
    ...
    public function getLoggedInUserDetails($user_id, $type) /******** START : GET LOGGED IN ADMIN, AGENCY DETAILS ********/
    {
      ...
      {
        $disp_name = 'Admin';
        $admin_data = $this->master_model->getRecords('supervision_admin', array('admin_id' => $user_id), 'admin_id, admin_name');
      ...
      {
        $disp_name = 'Agency';
        $candidate_data = $this->master_model->getRecords('supervision_candidates', array('id' => $user_id), 'id, candidate_name, candidate_code');
              
        if(count($candidate_data) > 0) 
        { 
          $disp_name = $candidate_data[0]['candidate_name']." (".$candidate_data[0]['candidate_code'].")";
          $disp_sidebar_name = $candidate_data[0]['candidate_name']."<span style='font-size: 14px;display: block;margin: 5px 0 0 0;'>(".$candidate_data[0]['candidate_code'].'  '.")</span>";
        }
      }
    ...
    function insert_user_login_logs($user_id=0, $user_type=0, $type=0) /******** START : MAINTAIN LOGIN - LOGOUT LOGS ********/
      ...
      $add_log['user_type'] = $user_type;
			...
			$this->master_model->insertRecord('supervision_login_logs',$add_log);
    ...
    function insert_common_log($title='', $tbl_name='', $qry='', $pk_id='', $module_slug='', $description='', $posted_data='')
      ...
      $login_id = $this->session->userdata('SUPERVISION_LOGIN_ID');
      ...
      if($login_id == "" || $login_id == 'NULL') { $login_id = '0'; }
			...
			$add_log['ip_address'] = get_ip_address(); //general_helper.php
			$this->master_model->insertRecord('supervision_logs',$add_log);
    ...
    function upload_file($input_name, $valid_arr, $new_file_name, $upload_path, $allowed_types, $is_multiple=0, $cnt='', $height=0, $width=0, $size=0,$resize_width=0,$resize_height=0,$resize_file_name='')
		{
			...
			{
        $this->load->helper('supervision_helper'); 
				create_directories($upload_path); //supervision_helper.php
    ...
    function check_file_exist($file_name='', $file_path='', $redirect_url='', $message='')
    {
      ...
      {
        if(!isset($file_name) || $file_name == '' || !file_exists($file_path.$file_name))//file_path = "./supervision/uploads/faculty_photo/"
        {
    ...
    function extend_candidate_add_update_date_arr()
    {
      ...
      $batch_data = $this->master_model->getRecords('supervision_candidates_centre_batch acb', array('acb.is_deleted' => '0', 'acb.batch_status' => '3', 'acb.batch_end_date >='=>date('Y-m-d'), 'acb.batch_extend_date !='=>'', 'acb.batch_extend_type !=' => '0'), "acb.batch_id, acb.batch_extend_date, acb.batch_extend_type");
    ...
    function supervision_send_mail_common($mail_arg=array())
    {
    ...
    function supervision_get_fees($exam_code=0,$exam_period=0)
    {
      $fresh_fee = $rep_fee = 0;
      $fee_data = $this->master_model->getRecords('supervision_exam_fee_master',array('fee_delete'=>'0', 'member_category'=>'NM', 'exempt'=>'NE', 'exam_code'=>$exam_code, 'exam_period'=>$exam_period));
      if(count($fee_data) > 0)
      {
