<?php
  ...
  ********************************************************************************************************************/
    ...
    /******** START : CHECK SESSION AFTER LOGIN FOR CANDIDATE PAGES ********/
      ...
      if (!isset($login_candidate_id) || $login_candidate_id == "")
        ...
        $this->session->set_flashdata('error', 'Please login again to vistit this page.');
			...
			{
        $candidate_data = $this->master_model->getRecords('iibfbcbf_batch_candidates', array('candidate_id' => $login_candidate_id, 'is_deleted' => '0'), 'candidate_id, hold_release_status');
    ...
    //IF BATCH IS CANCELLED or IF 290 DAYS IS COMPLETE FROM CANDIDATE DATE OF CREATION OR CANDIDATES 3 ATTEMPT IS OVER
    function validation_check_candidate_eligibility_to_add_in_new_batch($candidate_id=0, $chk_column='', $chk_value='', $enc_batch_id='')
    {
      ...
      if($enc_batch_id != ""){ 
        ...
        if($batch_id > 0){
          $centre_batch_data = $this->master_model->getRecords('iibfbcbf_agency_centre_batch', array('is_deleted' => '0', 'batch_id' => $batch_id), 'batch_type');
          if(count($centre_batch_data) > 0){
            $batch_type = $centre_batch_data[0]['batch_type'];
          }
        }
      }
      ...
      $date_290_Days_ago = date('Y-m-d', strtotime("-290days"));
      $this->db->where(" (DATE(cand.created_on) >= '".$date_290_Days_ago."' OR btch.batch_end_date >= '".$date_290_Days_ago."') ");
      $this->db->join('iibfbcbf_agency_centre_batch btch', 'btch.batch_id = cand.batch_id','INNER');
      $candidate_data = $this->master_model->getRecords('iibfbcbf_batch_candidates cand', array('cand.is_deleted' => '0', 'cand.candidate_id !=' => $candidate_id, 'btch.batch_status !=' => '7', 'cand.re_attempt <'=>'3'), 'cand.candidate_id, cand.agency_id, cand.centre_id, cand.batch_id, cand.mobile_no, cand.email_id, cand.id_proof_number, cand.aadhar_no, cand.created_on, cand.hold_release_status, btch.batch_end_date, btch.batch_type');
      ...
      if(count($candidate_data) > 0)
      {
        ...
        if($batch_type > 0 && $batch_type != $candidate_data[0]['batch_type'])
        {
          $candidate_data = array(); 
        }
      }
    ...
    public function get_exam_activation_details($enc_exam_code='0', $agency_id='0',$apply_from='') //apply_from = individual or bulk
    {
      if($apply_from == 'bulk')
      {
        ...
        if($agency_id != '0')
        {
          $agency_data = $this->master_model->getRecords('iibfbcbf_agency_master',array('agency_id'=>$agency_id), 'agency_id, allow_exam_codes, allow_exam_types, exam_period');
          ...
          if(count($agency_data) > 0)
          {
            ...
            {  
              ...
              if (strpos($agency_data[0]['allow_exam_codes'], '1057') !== false)
              {
              ...
              else { $allow_exam_codes .= ','.$agency_data[0]['allow_exam_codes']; }
            }

            if($agency_data[0]['allow_exam_types'] == 'Hybrid') 
            {
              if(count($agency_data) > 0 && $agency_data[0]['exam_period'] != '')
              {
                $allow_exam_period = $agency_data[0]['exam_period'];
              }
    ...
    {      
      ...
      $chk_batch_type = 0;
      $chk_exam_date = date("Y-m-d");

      if(count($active_exam_data) > 0)
      {
        $chk_batch_type = $active_exam_data[0]['exam_type'];
        if($active_exam_data[0]['exam_date'] != '' && $active_exam_data[0]['exam_date'] != '0000-00-00'){
          $chk_exam_date = $active_exam_data[0]['exam_date'];
        } 
      }
          ...
          if($candidate_data[0]['id_proof_file'] != '' && file_exists('uploads/iibfbcbf/id_proof/'.$candidate_data[0]['id_proof_file']))
          {
            if($candidate_data[0]['qualification_certificate_file'] != '' && file_exists('uploads/iibfbcbf/qualification_certificate/'.$candidate_data[0]['qualification_certificate_file']))
            {
              if($candidate_data[0]['candidate_photo'] != '' && file_exists('uploads/iibfbcbf/photo/'.$candidate_data[0]['candidate_photo']))
              {
                ...
                {
                  $batch_cand_data = $this->master_model->getRecords('iibfbcbf_agency_centre_batch btch', array('btch.batch_id'=>$candidate_data[0]['batch_id'], 'btch.is_deleted'=>'0'), "btch.batch_start_date, btch.batch_end_date, btch.batch_status, btch.batch_type");
                  if(count($batch_cand_data) > 0 && date('Y-m-d', strtotime("+".$buffer_days_after_candidate_add_date."days", strtotime($batch_cand_data[0]['batch_end_date']))) >= $chk_exam_date)
                  {
                                ...
                                if(count($batch_data) > 0)
                                {
                                  if($batch_data[0]['batch_status'] == '3')
                                  {
                                    if($batch_data[0]['batch_type'] == $chk_batch_type)
                                    {
                                      ...
                                      if (!(in_array($active_exam_data[0]['exam_code'], array(1039,1040)) && $candidate_data[0]['corporate_bc_option'] === 'CSC') && !(in_array($active_exam_data[0]['exam_code'], array(1039,1040)) && $candidate_data[0]['allow_exam_types'] === 'CSC')) // Check only for candidates who selected CSC at the time of training
                                      { 
                                        $batch_end_date_chk_flag = 0;
                                        if(isset($_SESSION['IIBF_BCBF_AGENCY_CODE']) && in_array($_SESSION['IIBF_BCBF_AGENCY_CODE'], array('1019')))
                                        {
                                          $modified_batch_end_date  = date('Y-m-d', strtotime("+".$buffer_days_after_training_end_date."days", strtotime($batch_data[0]['batch_end_date'])));
                                          $one_day_before_batch_end = date('Y-m-d', strtotime("-1 days", strtotime($batch_data[0]['batch_end_date'])));
                                          if($modified_batch_end_date < $todays_date || $one_day_before_batch_end == $todays_date) 
                                          {  
                                            $batch_end_date_chk_flag = 1;
                                          }
                                        }
                                        else
                                        {
                                          $modified_batch_end_date = date('Y-m-d', strtotime("+".$buffer_days_after_training_end_date."days", strtotime($batch_data[0]['batch_end_date'])));
                                          if($modified_batch_end_date < $todays_date) 
                                          {  
                                            $batch_end_date_chk_flag = 1;
                                          }
                                        }

                                        if($batch_end_date_chk_flag == 1)
                                        { 
                                          if(date('Y-m-d', strtotime("+".$buffer_days_after_candidate_add_date."days", strtotime($batch_data[0]['batch_end_date']))) >= $chk_exam_date)
                                          {
                                            ...
                                            else 
                                            {
                                              ...
                                              foreach($exam_data as $exam_data_res)
                                              {
                                                ...
                                                {
                                                  if(in_array($exam_data_res['exam_code'], array(1037,1038,1041,1042,1057)) && $exam_data_res['exam_code'] == $active_exam_data[0]['exam_code'] && $exam_data_res['exam_period'] == $active_exam_data[0]['exam_period'])
                                                  {
                                                    $response_msg = 'The candidate has already applied for exam';
                                                    $error_exam_data_flag = 1;
                                                  }
                                                  else if(date('Y-m-d') <= date('Y-m-d', strtotime("+10days", strtotime($exam_data_res['exam_date']))))
                                                  {
                                                    $response_msg = 'The candidate has already applied for exam.';
                                                    $error_exam_data_flag = 1;
                                                  }
                                              ...
                                              if($candidate_data[0]['regnumber'] != "")
                                              {
                                                ...
                                                if($eligible_api_res['api_res_flag'] == 'success')
                                                {
                                                  if(isset($eligible_api_res['api_res_response'][0]) && count($eligible_api_res['api_res_response'][0]) > 0)
                                                  {
                                                    ...
                                                    $eligible_pas_fail_val = '';
                                                    if(isset($eligible_data['pas_fail_val'])) { $eligible_pas_fail_val = $eligible_data['pas_fail_val']; }
                                                    if($eligible_pas_fail_val == "A" || $eligible_pas_fail_val == "F" || $eligible_pas_fail_val == '')
                                                    {
                                                      //VALID MEMBER
                                                      //ALLOW MEMBER IF pas_fail_val IS A OR F OR EMPTY
                                                    }
                                                    else
                                                    {
                                                      $eligible_res_msg = '-';
                                                      if($eligible_pas_fail_val != "") { $eligible_res_msg = $eligible_pas_fail_val; }

                                                      $flag = 'error';

                                                      if($eligible_res_msg == "D")
                                                      {
                                                        $response_msg = 'The candidate is not eligible to apply for the exam as they are debarred from this exam. ('.$eligible_res_msg.')';
                                                      }
                                                      else
                                                      {
                                        ...
                                        $sub_res_msg = "(<a target='_blank' href='".base_url('iibfbcbf/apply_exam_individual?ctype=Tk0=')."'>Click Here</a>)";
                                        $response_msg = 'You are not eligible to appear at CSC center Exam. Kindly register through the IIBF website. '.$sub_res_msg;

                                        if($candidate_data[0]['allow_exam_types'] == 'CSC' && $candidate_data[0]['corporate_bc_option'] == 'CSC'){
                                          $response_msg = 'As you have undertaken training through CSC, you are not eligible to apply/appear for the BCBF exam at any CSC centre, kindly register through IIBF website. '.$sub_res_msg;
                                        }else if($candidate_data[0]['allow_exam_types'] == 'CSC' && $candidate_data[0]['corporate_bc_option'] == 'Other'){
                                          $response_msg = 'You are not eligible to appear at CSC center since you are associated with CSC Corporate BC. Kindly register through the IIBF website. '.$sub_res_msg;
                                        }
                                      }  
                  ...
                  else { $response_msg = 'The candidate is not eligible to apply for exam as the 9 month period since training completion is over'; /*$response_msg = 'The candidate is not eligible to apply for exam as its '.$buffer_days_after_candidate_add_date.' days is over';*/ }
                }
    ...
    public function iibf_bcbf_eligible_master_api($exam_code=0, $exam_period=0,$member_no=0)
    {
      ...
      if(in_array($exam_code, array(1057))) //FOR BCBF NAR EXAM - 1057
      {
        //$api_url="http://10.10.233.66:8097/getBCBFNAREligibleData/".$exam_code."/".$exam_period."/".$member_no; //UAT API
        $api_url="http://10.10.233.76:8097/getBCBFNAREligibleData/".$exam_code."/".$exam_period."/".$member_no; //PRODUCTION API ADDED BY SAGAR ON 2024-12-31       
      }
      else //FOR BCBF EXAM - 1037, 1038, 1039, 1040, 1041, 1042
      {
    ...
    function iibfbcbf_send_mail_common($mail_arg=array())
    {
      ...
      if($subject != '' && $from_email != '' && $to_email != '' && $mail_content != '')
      {
        ...
        if($to_email == 'je.mss4@iibf.org.in'){
          $to_email = 'je.mss4@iibf.org.in'; //Dharmik Modi for KYC
          //$to_email = 'sagar.matale@esds.co.in';
        }else{
          $to_email = 'sagar.matale@esds.co.in';
        }
        //$cc_email = 'sagar.matale@esds.co.in,pre_production@esds.co.in,anil.s@esds.co.in,deanjude95@gmail.com';
        $cc_email = 'sagar.matale@esds.co.in,pre_production@esds.co.in,anil.s@esds.co.in'; //Dharmik Modi: ,je.mss4@iibf.org.in
        $bcc_email = '';  
    ...
    function generate_admit_card_common($enc_pt_id='0')
    {
      ...
      if(count($payment_data) > 0 && in_array($payment_data[0]['exam_code'], $eligible_exam_code_for_admit_card_arr))
      {
        ...
        $subject_master = $this->master_model->getRecords('iibfbcbf_exam_subject_master',array('exam_code'=>$payment_data[0]['exam_code'], 'exam_period'=>$payment_data[0]['exam_period'],'subject_delete'=>'0','group_code'=>'C'),'subject_code, subject_description',array('subject_code'=>'ASC'));
    ...
    function get_capacity_bulk($exam_code = '', $exam_period = '',$exam_centre_code = '', $exam_venue_code = '', $exam_date = '', $exam_time = '', $member_exam_id=0, $chk_capacity_type='')
    {
      ...
      if($exam_code != '' && $exam_period != '' && $exam_centre_code != '' && $exam_venue_code != '' && $exam_date != '' && $exam_time != '' && in_array($exam_code, $valid_exam_code_arr))
      {
        ...
        if(count($seat_count) > 0)
        {
          ...
          $member_exam_Count = $this->master_model->getRecords('iibfbcbf_member_exam me',array('me.is_deleted'=>'0',  'exam_centre_code'=>$exam_centre_code, 'exam_venue_code'=>$exam_venue_code, 'exam_date'=>$exam_date, 'exam_time'=>$exam_time, 'exam_period'=>$exam_period, 'me.member_exam_id !='=>$member_exam_id)); //'exam_code'=>$exam_code, 'exam_period'=>$exam_period, ,
          //echo '<br>'; _pq();
    ...
    function get_capacity_csc($exam_code = '', $exam_period = '',$exam_centre_code = '', $exam_venue_code = '', $exam_date = '', $exam_time = '', $member_exam_id=0)
    {
      ...
      if($exam_code != '' && $exam_period != '' && $exam_centre_code != '' && $exam_venue_code != '' && $exam_date != '' && $exam_time != '' && in_array($exam_code, $valid_exam_code_arr))
      {
        ...
        if(count($seat_count) > 0)
        {
          ...
          $this->db->where_in('me.pay_status',$pay_status_arr);
          $this->db->where_in('me.exam_code',array(1039,1040));
    ...
    //START : SEND EMAIL NOTIFICATION TO INSPECTOR WHEN ASSIGNED TO BATCH BY ADMIN & REMINDER INSPECTION EMAIL
    function send_notification_to_inspector($inspector_id='', $batch_id='', $notification_type='Regular') //notification_type = Regular or Reminder
    { 
      $inspector_data = $this->master_model->getRecords('iibfbcbf_inspector_master', array('inspector_id'=>$inspector_id, 'is_active'=>'1', 'is_deleted'=>'0'), 'inspector_name, inspector_mobile, inspector_email, inspector_designation');
      
      $this->db->join('iibfbcbf_agency_master am', 'am.agency_id = btch.agency_id', 'INNER');
      $this->db->join('iibfbcbf_centre_master cm', 'cm.centre_id = btch.centre_id', 'INNER');
      $this->db->join('city_master cm2', 'cm2.id = cm.centre_city', 'LEFT');
      $this->db->join('state_master sm2', 'sm2.state_code = cm.centre_state', 'LEFT');
      $batch_data = $this->master_model->getRecords('iibfbcbf_agency_centre_batch btch', array('btch.batch_id'=>$batch_id, 'btch.is_deleted'=>'0', 'btch.batch_status'=>'3'), 'btch.batch_id, btch.agency_id, btch.batch_code, btch.batch_type, btch.batch_start_date, btch.batch_end_date, btch.batch_daily_start_time, btch.batch_daily_end_time, btch.training_language, btch.batch_status, btch.contact_person_name, btch.contact_person_mobile, btch.contact_person_email, btch.alt_contact_person_name, btch.alt_contact_person_mobile, btch.alt_contact_person_email, btch.batch_online_offline_flag, btch.remarks, btch.online_training_platform, btch.platform_link, btch.training_schedule_file, am.agency_name, am.agency_code, am.allow_exam_types, cm.centre_name, cm.centre_username, cm.centre_contact_person_email, cm.centre_address1, cm.centre_address2, cm.centre_address3, cm.centre_address4, cm2.city_name AS centre_city_name, cm.centre_district, sm2.state_name AS centre_state_name, cm.centre_pincode');
      
      $emailer_data = $this->master_model->getRecords('emailer', array('emailer_name'=>'IIBFBCBF_INSPECTOR_NOTIFICATION'), 'emailer_name, emailer_text, sms_text, sms_template_id, sms_sender, subject');

      if(count($inspector_data) > 0 && count($batch_data) > 0 && count($emailer_data) > 0)
      {
        $batch_type = 'Basic Batch'; if($batch_data[0]['batch_type'] == '2') { $batch_type = 'Advanced Batch'; }

        $mail_arg = array();
        $subject = str_replace("#CITY_NAME#", $batch_data[0]['centre_city_name'], $emailer_data[0]['subject']);
        $subject = str_replace("#BATCH_CODE#", $batch_data[0]['batch_code'], $subject);
        $subject = str_replace("#BATCH_START_DATE#", date("d-M-Y", strtotime($batch_data[0]['batch_start_date'])), $subject);
        $subject = str_replace("#BATCH_END_DATE#", date("d-M-Y", strtotime($batch_data[0]['batch_end_date'])), $subject);

        if($notification_type == 'Reminder') { $subject = 'REMINDER '.$subject; }
        $mail_arg['subject'] = $subject;

        $mail_arg['to_email'] =  $inspector_data[0]['inspector_email']; //'sagar.matale@esds.co.in';
        $mail_arg['to_name'] = $inspector_data[0]['inspector_name']; //'sagar';
        $mail_arg['cc_email'] = '';//sagar.matale@esds.co.in,anil.s@esds.co.in
        $mail_arg['bcc_email'] = 'iibfteam@esds.co.in';//sagar.matale@esds.co.in,anil.s@esds.co.in
        $mail_arg['is_header_footer_required'] = '0';
        $mail_arg['view_flag'] = '0';
        ...
        $mail_content = '
          <style type="text/css">            
            ...
            p.footer_regards { line-height: 20px; }
            table.inner_tbl { font-size: 14px; border-collapse: collapse; width: 100%; color:#000; }
            table.inner_tbl tbody tr td { font-size: 14px; padding: 5px 10px; border-collapse: collapse; border: 1px solid #776f6f; line-height:20px; vertical-align:top; min-width:200px; }                          
          </style>'.$emailer_data[0]['emailer_text'];        

        if($notification_type == 'Reminder')
        {
          $reminder_text = '<p style="text-align: center; line-height: 25px; margin: 0 0 20px 0; color: red; font-size: 16px"><strong>REMINDER</strong><br>"Please ignore this E-mail if you have already submitted the Inspection Report".</p>';
          $mail_content = str_replace("#REMINDER#", $reminder_text, $mail_content);
        }
        else
        {
          $mail_content = str_replace("#REMINDER#", '', $mail_content);
        }

        $mail_content = str_replace("#INSPECTOR_NAME#", $inspector_data[0]['inspector_name'], $mail_content);
        $mail_content = str_replace("#AGENCY_NAME#", $batch_data[0]['agency_name'], $mail_content);
        $mail_content = str_replace("#BATCH_CODE#", $batch_data[0]['batch_code'], $mail_content);
        $mail_content = str_replace("#TRAINING_MODE#", $batch_data[0]['batch_online_offline_flag']=='1'?'Offline':'Online', $mail_content);
        $mail_content = str_replace("#BATCH_TYPE#", $batch_type, $mail_content);
        $mail_content = str_replace("#BATCH_START_DATE#", date("d-M-Y", strtotime($batch_data[0]['batch_start_date'])), $mail_content);
        $mail_content = str_replace("#BATCH_END_DATE#", date("d-M-Y", strtotime($batch_data[0]['batch_end_date'])), $mail_content);
        $mail_content = str_replace("#BATCH_DAILY_START_TIME#", date("h:i A", strtotime($batch_data[0]['batch_daily_start_time'])), $mail_content);
        $mail_content = str_replace("#BATCH_DAILY_END_TIME#", date("h:i A", strtotime($batch_data[0]['batch_daily_end_time'])), $mail_content);
        $mail_content = str_replace("#TRAINING_LANGUAGE#", $batch_data[0]['training_language'], $mail_content);
        $mail_content = str_replace("#BATCH_COORDINATOR_NAME#", $batch_data[0]['contact_person_name'], $mail_content);
        $mail_content = str_replace("#BATCH_COORDINATOR_MOBILE#", $batch_data[0]['contact_person_mobile'], $mail_content);
        if($batch_data[0]['contact_person_email'] != "")
        {
          $mail_content = str_replace("#BATCH_COORDINATOR_EMAIL#", $batch_data[0]['contact_person_email'], $mail_content);       
        }
        else { $mail_content = str_replace("#BATCH_COORDINATOR_EMAIL#", 'NA', $mail_content); } 
        $mail_content = str_replace("#ADDITIONAL_BATCH_COORDINATOR_NAME#", $batch_data[0]['alt_contact_person_name'], $mail_content);
        $mail_content = str_replace("#ADDITIONAL_BATCH_COORDINATOR_MOBILE#", $batch_data[0]['alt_contact_person_mobile'], $mail_content);
        if($batch_data[0]['alt_contact_person_email'] != "")
        {
          $mail_content = str_replace("#ADDITIONAL_BATCH_COORDINATOR_EMAIL#", $batch_data[0]['alt_contact_person_email'], $mail_content);       
        }
        else { $mail_content = str_replace("#ADDITIONAL_BATCH_COORDINATOR_EMAIL#", 'NA', $mail_content); }   
        
        $training_centre_addr = '';
        if($batch_data[0]['centre_address1'] != '') { $training_centre_addr .= $batch_data[0]['centre_address1'].', '; }
        if($batch_data[0]['centre_address2'] != '') { $training_centre_addr .= $batch_data[0]['centre_address2'].', '; }
        if($batch_data[0]['centre_address3'] != '') { $training_centre_addr .= $batch_data[0]['centre_address3'].', '; }
        if($batch_data[0]['centre_address4'] != '') { $training_centre_addr .= $batch_data[0]['centre_address4'].', '; }
        if($batch_data[0]['centre_city_name'] != '') { $training_centre_addr .= $batch_data[0]['centre_city_name'].', '; }
        if($batch_data[0]['centre_district'] != '') { $training_centre_addr .= $batch_data[0]['centre_district'].', '; }
        if($batch_data[0]['centre_state_name'] != '') { $training_centre_addr .= $batch_data[0]['centre_state_name'].', '; }
        $training_centre_addr = rtrim($training_centre_addr,', ');
        if($batch_data[0]['centre_pincode'] != '') { $training_centre_addr .= ' - '.$batch_data[0]['centre_pincode']; }
        $training_centre_addr = str_replace(",,", ',', $training_centre_addr);
        $mail_content = str_replace("#TRAINING_CENTRE_ADDRESS#", $training_centre_addr, $mail_content);
        $mail_content = str_replace("#TRAINING_REMARK#", $batch_data[0]['remarks'], $mail_content);

        $online_training_details = '';
        if($batch_data[0]['batch_online_offline_flag'] == '2')
        {
          $online_user_details = '';
          $get_online_login_detalis = $this->master_model->getRecords('iibfbcbf_online_batch_user_details', array('agency_id'=>$batch_data[0]['agency_id'], 'batch_id'=>$batch_data[0]['batch_id'], 'is_active'=>'1', 'is_deleted'=>'0'), 'login_id, password');
          if(count($get_online_login_detalis) > 0)
          {
            foreach($get_online_login_detalis as $get_online_login_res)
            {
              $online_user_details .='<tr><td>'.$get_online_login_res['login_id'].'</td><td>'.$get_online_login_res['password'].'</td></tr>';
            }
          }          

          $online_training_details .= '<p style="margin: 20px 0 10px 0;line-height: 20px; font-weight:bold;">Please find below login details for the On-line training platform : </p>

          <table class="inner_tbl" cellspacing="0" cellpadding="0" width="100%" border="0">
            <tbody>
              <tr><td><strong>Name of the online training platform used</strong></td><td>'.$batch_data[0]['online_training_platform'].'</td></tr>
              <tr><td><strong>Online Platform Link</strong></td><td>'.$batch_data[0]['platform_link'].'</td></tr>
              <tr>
                <td><strong> Login Details</strong></td>
                <td style="padding: 15px 20px 15px 20px;">
                  <table class="inner_tbl" cellspacing="0" cellpadding="0" width="100%" border="0">
                    <tbody>
                      <tr style="text-align: center; font-weight: bold; background: #eee;"><td>Login Id</td><td>Password</td><tr>
                      '.$online_user_details.'
                    </tbody>
                  </table>
                </td>
        ...
        $mail_content = str_replace("#ONLINE_USER_DETAILS#", $online_training_details, $mail_content);
        $mail_arg['mail_content'] = $mail_content;

        if($batch_data[0]['training_schedule_file'] != '')
        {
          $mail_arg['attachment'] = './uploads/iibfbcbf/training_schedule/'.$batch_data[0]['training_schedule_file'];
        }
        $this->iibfbcbf_send_mail_common($mail_arg);
      }
    }//END : SEND EMAIL NOTIFICATION TO INSPECTOR WHEN ASSIGNED TO BATCH BY ADMIN & REMINDER INSPECTION EMAIL
    ...
    function get_candidate_exam_application_history_data_common($training_id_or_regnumber='0')
    {
      ...
      $result['candidate_data'] = $candidate_data = $this->master_model->getRecords('iibfbcbf_batch_candidates cand', array('cand.is_deleted' => '0'), "cand.candidate_id, cand.agency_id, cand.regnumber, cand.training_id, cand.salutation, cand.first_name, cand.middle_name, cand.last_name, cand.dob, IF(cand.gender=1,'Male','Female') AS DispGender, cand.mobile_no, cand.alt_mobile_no, cand.email_id, cand.alt_email_id, IF(cand.hold_release_status=1,'Auto Hold', IF(cand.hold_release_status=2,'Manual Hold','Release')) AS Disphold_release_status, cand.re_attempt, cand.created_on, cand.id_proof_file, cand.qualification_certificate_file, cand.candidate_photo, cand.candidate_sign", array('cand.candidate_id'=>'DESC'));
      
      if(count($candidate_data) > 0)
      {
        $this->db->where('me.candidate_id',$candidate_data[0]['candidate_id']);
        $this->db->join('iibfbcbf_admit_card_details ac', 'ac.mem_mem_no = me.regnumber AND ac.exm_cd = me.exam_code AND ac.mem_exam_id = me.member_exam_id AND ac.remark = 1', 'LEFT');
        $this->db->join('iibfbcbf_payment_transaction pt', 'pt.id = me.pt_id', 'LEFT');
        $result['exam_application_data'] = $exam_application_data = $this->master_model->getRecords('iibfbcbf_member_exam me', array('me.is_deleted' => '0'), "me.*, ac.admitcard_id, pt.receipt_no, pt.description, pt.transaction_details", array('me.member_exam_id'=>'DESC'));
        //_pq(1);
      }
    ...
    function get_exam_details_common_data(){
        ...
        $join_qry = " INNER JOIN iibfbcbf_agency_master am1 ON am1.agency_id = cand.agency_id"; 
        $join_qry .= " INNER JOIN iibfbcbf_centre_master cm ON cm.centre_id = cand.centre_id";
        $join_qry .= " INNER JOIN iibfbcbf_agency_centre_batch acb ON acb.batch_id = cand.batch_id";
        $join_qry .= " LEFT JOIN iibfbcbf_batch_candidates cand2 ON cand2.parent_id = cand.candidate_id AND cand2.parent_table_name = 'iibfbcbf_batch_candidates'";
        $join_qry .= " LEFT JOIN iibfbcbf_agency_master am2 ON am2.agency_id = cand2.agency_id";
        $join_qry .= " LEFT JOIN iibfbcbf_centre_master cm2 ON cm2.centre_id = cand2.centre_id";
        $join_qry .= " LEFT JOIN iibfbcbf_agency_centre_batch btch2 ON btch2.batch_id = cand2.batch_id";
        ...
        foreach ($Rows as $Res) 
        {
          ...
          $member_exam_data = $this->master_model->getRecords('iibfbcbf_member_exam me', array('me.candidate_id' => $Res['candidate_id'], 'me.pay_status' => '1'), 'me.candidate_id, me.exam_code, me.exam_period, me.pay_status, me.exam_date, bm.status AS result',array('me.member_exam_id','ASC'));
          ...
          $member_exam_data2 = $this->master_model->getRecords('iibfbcbf_member_exam me', array('me.candidate_id' => $Res['new_candidate_id'], 'me.pay_status' => '1'), 'me.candidate_id, me.exam_code, me.exam_period, me.pay_status, me.exam_date, bm.status AS result',array('me.member_exam_id','ASC'));
          //echo $this->db->last_query();
    ...
    function get_eligible_candidate_for_examination_common_data(){
        ...
        $column_order = array('cand.candidate_id', 'am1.agency_name', 'CONCAT(cm.centre_name," (",cm1.city_name,")") AS DispCentreName', 'acb.batch_code', 'cand.training_id', 'cand.regnumber', 'CONCAT(cand.salutation, " ", cand.first_name, IF(cand.middle_name != "", CONCAT(" ", cand.middle_name), ""), IF(cand.last_name != "", CONCAT(" ", cand.last_name), "")) AS DispName', 'acb.batch_start_date', 'acb.batch_end_date', 'IF(cand.hold_release_status = "3", "Y", "N") AS candidate_release_status', '"" AS exam_date1', '"" AS exam_date2', '"" AS exam_date3', '"" AS result1', '"" AS result2', '"" AS result3', 'GREATEST((3 - cand.re_attempt) ,0) AS attempt_remaining', 'GREATEST(290 - (DATEDIFF(CURDATE(), acb.batch_end_date )) ,0) AS validity_remaining', 'cand.parent_id'); //SET COLUMNS FOR SORT
        
        $column_search = array('am1.agency_name', 'CONCAT(cm.centre_name," (",cm1.city_name,")")', 'acb.batch_code', 'cand.training_id', 'cand.regnumber', 'CONCAT(cand.salutation, " ", cand.first_name, IF(cand.middle_name != "", CONCAT(" ", cand.middle_name), ""), IF(cand.last_name != "", CONCAT(" ", cand.last_name), ""))', 'acb.batch_start_date', 'acb.batch_end_date', 'IF(cand.hold_release_status = "3", "Y", "N")', 'GREATEST((3 - cand.re_attempt) ,0)', 'GREATEST(290 - (DATEDIFF(CURDATE(), acb.batch_end_date )) ,0)', 'cand.parent_id'); //SET COLUMN FOR SEARCH 
        ...
        {
          // Excel file name for download 
          $fileName = "Candidate_Eligible_For_Examination_".date('Y-m-d').".xls";  
          // Column names 
          $fields = array('Sr. No.', 'Agency Name', 'Centre Name', 'Batch Code', 'Training Id', 'Registration No.', 'Candidate Full Name', 'Training From', 'Training To', 'Release Status (Y/N)', 'Exam Date1', 'Exam Date2', 'Exam Date3', 'Result1', 'Result2', 'Result3', 'Attempt Remaining', 'Validity Remaining (in days)');  
          // Display column names as first row 
          $excelData = implode("\t", array_values($fields)) . "\n";  
        }
    ...
    function get_training_batch_details_common_data(){
        $form_action = trim($this->security->xss_clean($this->input->post('form_action')));
        ...
        //DATE_FORMAT(DATE_ADD(acb.batch_end_date, INTERVAL $this->buffer_days_after_training_end_date DAY), '%Y-%m-%d') AS adjusted_date
        $column_order = array('""', 'acb.batch_id', 'am1.agency_name', 'CONCAT(cm.centre_name," (",cm1.city_name,")") AS DispCentreName', 'acb.batch_code', 'IF(acb.batch_type=1, "Basic", IF(acb.batch_type=2, "Advanced", "")) AS DispBatchType', 'COUNT(DISTINCT CASE WHEN bc.is_deleted = "0" AND acb.batch_status = "3" THEN bc.candidate_id END) AS tot_candiadtes_registered_for_training', 'COUNT(DISTINCT CASE WHEN bc.re_attempt = "0" AND bc.hold_release_status = "3" AND DATE_FORMAT(DATE_ADD(acb.batch_end_date, INTERVAL '.$this->buffer_days_after_training_end_date.' DAY), "%Y-%m-%d") < CURDATE() AND CURDATE() <= DATE_FORMAT(DATE_ADD(acb.batch_end_date, INTERVAL '.$this->buffer_days_after_candidate_add_date.' DAY), "%Y-%m-%d") THEN bc.candidate_id END) AS no_of_candidates_eligible_for_exam', 'COUNT(DISTINCT CASE WHEN bc.re_attempt > "0" THEN bc.candidate_id END) AS no_of_candidates_applied_for_exam', 'acb.batch_status', 'acb.centre_id', 'acb.batch_type'); //SET COLUMNS FOR SORT
        
        $column_search = array('am1.agency_name', 'CONCAT(cm.centre_name," (",cm1.city_name,")")', 'acb.batch_code', 'IF(acb.batch_type=1, "Basic", IF(acb.batch_type=2, "Advanced", ""))', 'COUNT(DISTINCT CASE WHEN bc.is_deleted = "0" AND acb.batch_status = "3" THEN bc.candidate_id END)'); //SET COLUMN FOR SEARCH
        ...
        if ($form_action == 'export')
        {
          ...
          // Column names 
            ...
            $fields = array('Sr. No.', 'Agency Name', 'Centre Name', 'Batch Code', 'Batch Type', 'Total Candiadtes registered for Training','No. of Candidates Eligible for Exam', 'No. of Candidates Applied for Exam'); 

          if($selected_batch_ids != ""){
            $fields = array('Sr. No.', 'Agency Name', 'Centre Name', 'Batch Code', 'Batch Type', 'Total Candiadtes registered for Training','No. of Candidates Eligible for Exam', 'No. of Candidates Applied for Exam', 'No. of Candidates Passed'); 
          }
    ...
    function get_batch_communication_common_data(){

        $form_action = trim($this->security->xss_clean($this->input->post('form_action')));

        $table = 'iibfbcbf_logs bcbflog'; 
        
        $column_order = array('bcbflog.log_id', 'am1.agency_name', 'CONCAT(cm.centre_name," (",cm1.city_name,")") AS DispCentreName', 'acb.batch_code', 'DATE_FORMAT(bcbflog.created_on,"%Y-%m-%d %h:%i %p") AS log_date_time', 'IF(bcbflog.login_type="centre", bcbflog.posted_data, "") AS remark_by_centre', 'IF(bcbflog.login_type="agency", bcbflog.posted_data, "") AS remark_by_agency', 'IF(bcbflog.login_type="admin", bcbflog.posted_data, "") AS remark_by_admin', 'IF(acb.batch_type=1, "Basic", IF(acb.batch_type=2, "Advance", "")) AS DispBatchType'); //SET COLUMNS FOR SORT
        ...
        $order = array('bcbflog.log_id' => 'DESC'); // DEFAULT ORDER    
         
        $WhereForTotal = "WHERE bcbflog.module_slug = 'batch_action' AND (bcbflog.title LIKE '%Centre : Batch Communication%' OR bcbflog.title LIKE '%Admin : Batch Communication%') AND acb.is_deleted = 0 "; //DEFAULT WHERE CONDITION FOR ALL RECORDS 
        $Where = "WHERE bcbflog.module_slug = 'batch_action' AND (bcbflog.title LIKE '%Centre : Batch Communication%' OR bcbflog.title LIKE '%Admin : Batch Communication%') AND acb.is_deleted = 0  ";
        ...
        $join_qry = " LEFT JOIN iibfbcbf_agency_centre_batch acb ON acb.batch_id = bcbflog.pk_id";
        $join_qry .= " LEFT JOIN iibfbcbf_centre_master cm ON cm.centre_id = acb.centre_id";
        $join_qry .= " LEFT JOIN city_master cm1 ON cm1.id = cm.centre_city";
        $join_qry .= " LEFT JOIN iibfbcbf_agency_master am1 ON am1.agency_id = acb.agency_id"; 
        ...
        {
          // Excel file name for download 
          $fileName = "Batch_Communication_".date('Y-m-d').".xls";  
          // Column names 
          $fields = array('Sr. No.', 'Agency Name', 'Centre Name', 'Batch Code', 'Date and Time', 'Communication Shared By Center', 'Remarks By Agency', 'Remarks By Admin');  
          // Display column names as first row 
          $excelData = implode("\t", array_values($fields)) . "\n";  
        }
        ...
        foreach ($Rows as $Res) 
        {
          ...
          $row[] = $Res['batch_code'];
          //$row[] = $Res['DispBatchType'];
          $row[] = $Res['log_date_time'];

          $data_json_centre_arr = json_decode($Res['remark_by_centre'], true);  
          $remark_by_centre = $data_json_centre_arr['batch_communication'];
          $data_json_agency_arr = json_decode($Res['remark_by_agency'], true);  
          $remark_by_agency = $data_json_agency_arr['batch_communication'];
          $data_json_admin_arr = json_decode($Res['remark_by_admin'], true);  
          $remark_by_admin = $data_json_admin_arr['batch_communication'];

          $row[] = $remark_by_centre;
          $row[] = $remark_by_agency;
          $row[] = $remark_by_admin;
    ...
    function get_candidates_to_re_enroll_for_training_common_data(){
        ...
        //, 'IF(cand.hold_release_status = "3", "Y", "N")', 'GREATEST((3 - cand.re_attempt) ,0)', 'GREATEST(290 - (DATEDIFF(CURDATE(), acb.batch_end_date )) ,0)', 'cand.parent_id'
    ...
    //IIBFBCBF No. of Pass/Fail/Absent Count API
    public function get_iibfbcbf_no_of_pass_fail_absent_cnt_api($batch_code = '')
    { 
      //echo $type.' / '.$exam_code.' / '.$exam_period.' / '.$part_no.' / 1 / '.$member_no; 
      $final_arr = $response_msg = array();
      $response = '';  
      ...
      if($batch_code != ""){ 
        ...
        $url="http://10.10.233.66:8099/resultcountapi/getResultPassFailAbsentCount/".$batch_code; // UAT API
        $string = preg_replace('/\s+/', '+', $url);
        $x = curl_init($url);
        curl_setopt($x, CURLOPT_HEADER, 0);
        curl_setopt($x, CURLOPT_FOLLOWLOCATION, 0);
    ...
    function send_approve_reject_kyc_email_sms($candidate_data,$module_name,$kyc_action)
    {
      ...
      $emailer_data = $this->master_model->getRecords('emailer', array('emailer_name'=>'IIBFBCBF_KYC_APPROVE_REJECT'), 'emailer_name, emailer_text, sms_text, sms_template_id, sms_sender, subject'); 
