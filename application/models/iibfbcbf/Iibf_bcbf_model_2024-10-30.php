  ...
  ********************************************************************************************************************/
    ...
    function iibfbcbf_send_mail_common($mail_arg=array())
    {
      ...
      if($subject != '' && $from_email != '' && $to_email != '' && $mail_content != '')
      {
        ...
        $cc_email = 'sagar.matale@esds.co.in,pre_production@esds.co.in,anil.s@esds.co.in';
        $bcc_email = '';  
    ...
    function get_candidate_exam_application_history_data_common($training_id_or_regnumber='0')
    {
      ...
      if(count($candidate_data) > 0)
      {
        ...
        $result['exam_application_data'] = $exam_application_data = $this->master_model->getRecords('iibfbcbf_member_exam me', array('me.is_deleted' => '0'), "me.*, ac.admitcard_id", array('me.member_exam_id'=>'DESC'));
        //_pq(1);
      }
    ...
    function get_eligible_candidate_for_examination_common_data(){
        ...
        $column_order = array('cand.candidate_id', 'am1.agency_name', 'CONCAT(cm.centre_name," (",cm1.city_name,")") AS DispCentreName', 'acb.batch_code', 'cand.training_id', 'cand.regnumber', 'CONCAT(cand.salutation, " ", cand.first_name, IF(cand.middle_name != "", CONCAT(" ", cand.middle_name), ""), IF(cand.last_name != "", CONCAT(" ", cand.last_name), "")) AS DispName', 'acb.batch_start_date', 'acb.batch_end_date', 'IF(cand.hold_release_status = "3", "Y", "N") AS candidate_release_status', '"" AS exam_date1', '"" AS exam_date2', '"" AS exam_date3', '"" AS result1', '"" AS result2', '"" AS result3', 'GREATEST((3 - cand.re_attempt) ,0) AS attempt_remaining', 'GREATEST(270 - (DATEDIFF(CURDATE(), acb.batch_end_date )) ,0) AS validity_remaining', 'cand.parent_id'); //SET COLUMNS FOR SORT
        
        $column_search = array('am1.agency_name', 'CONCAT(cm.centre_name," (",cm1.city_name,")")', 'acb.batch_code', 'cand.training_id', 'cand.regnumber', 'CONCAT(cand.salutation, " ", cand.first_name, IF(cand.middle_name != "", CONCAT(" ", cand.middle_name), ""), IF(cand.last_name != "", CONCAT(" ", cand.last_name), ""))', 'acb.batch_start_date', 'acb.batch_end_date', 'IF(cand.hold_release_status = "3", "Y", "N")', 'GREATEST((3 - cand.re_attempt) ,0)', 'GREATEST(270 - (DATEDIFF(CURDATE(), acb.batch_end_date )) ,0)', 'cand.parent_id'); //SET COLUMN FOR SEARCH 
    ...
    function get_training_batch_details_common_data(){
        ...
        $column_search = array('am1.agency_name', 'CONCAT(cm.centre_name," (",cm1.city_name,")")', 'acb.batch_code', 'IF(acb.batch_type=1, "Basic", IF(acb.batch_type=2, "Advanced", ""))'); //SET COLUMN FOR SEARCH
    ...
    function get_candidates_to_re_enroll_for_training_common_data(){
        ...
        //, 'GREATEST((3 - cand.re_attempt) ,0) AS attempt_remaining', 'GREATEST(270 - (DATEDIFF(CURDATE(), acb.batch_end_date )) ,0) AS validity_remaining'
        ...
        //, 'IF(cand.hold_release_status = "3", "Y", "N")', 'GREATEST((3 - cand.re_attempt) ,0)', 'GREATEST(270 - (DATEDIFF(CURDATE(), acb.batch_end_date )) ,0)', 'cand.parent_id'
