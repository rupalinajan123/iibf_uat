...
class LoginModel extends CI_Model {	
	...
	public function isVendorAdminExist($data) {
		$this->db->select('*');
		$this->db->from('vendor_admin');
		$this->db->where($data);
		$q = $this->db->get();
		//echo $this->db->last_query();exit;
		$res['result'] = $q->result_array();
	...
	public function isCaseWritingCompetitionAdminExist($data) {
		$this->db->select('*');
		$this->db->from('case_study_comp_admin');
		$this->db->where($data);
		$q = $this->db->get();
		//echo $this->db->last_query();exit;
		$res['result'] = $q->result_array();
	...
	public function isCSCUserExist($data)	
	{
		$this->db->select('*');
		$this->db->from('administrators u');
		$this->db->join('role_master r', 'u.roleid = r.roleid','left');	
		$this->db->where('u.roleid',15);
