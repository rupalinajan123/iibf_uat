...
{
	public function getDepartmentCount($field='',$value='')
	{
		$this->db->select('*');
		if(strpos($field,','))
		{
		...
		$this->db->where('dept_delete',0);
		$num = $this->db->count_all_results('department_master');
		return $num;
	}
	
	public function getDepartments($field='', $value='', $sortkey='', $sortval='', $per_page='', $start='')
	{
		$this->db->select('*');
	...
	*/
	...
	public function getRecordCount1($tbl_name,$condition=FALSE)
	{
	...
	*/
	...
	public function getRecords1($tbl_name,$condition=FALSE,$select=FALSE,$order_by=FALSE,$start=FALSE,$limit=FALSE)
	{
		if($select!="")
