  ...
  ********************************************************************/
  ...
  class Billdesk_pg_module_cron extends CI_Model 
  {
    public function billdesk_cron_settlement_common($responsedata='', $encData='', $receiptNo='')
    {
      ...
      if($additional_info3 == 'IIBF_BULK_DRA') { $pay_type = 'iibfbulkdra'; }  //FOR IIBF DRA EXAM CODE 1036        

      if ($pay_type == '') {
        $get_pg_flag=$this->master_model->getRecords('dra_payment_transaction',array('receipt_no'=>$receiptNo),'pg_flag');
        $pay_type = $get_pg_flag[0]['pg_flag'];
      }
      ...
      else if($pay_type == "BC") //FOR IIBF BCBF MODULE. ADDED BY SAGAR & ANIL ON 15-04-2024
      {
        ...
        else if($payment_status==0)
        {
          if($payment_data[0]['status'] == '2')//IF payment status is PENDING
          {
            ...
            $update_data['transaction_details'] = $responsedata['transaction_error_type']. " - " .$responsedata['transaction_error_desc'];
            $update_data['auth_code'] = $auth_status;
            $update_data['bankcode'] = $responsedata['bankid'];
            $update_data['paymode'] = $responsedata['txn_process_type'];
            $update_data['callback'] = 'C_S2S';           
      ...
      else if($pay_type == "iibfbulkdra") //FOR IIBF DRA MODULE. ADDED BY GAURAV SHEWALE ON 31-01-2025
      {
        sleep(8);
        ...
        if(isset($responsedata) && $responsedata['orderid'] != '' && $receiptNo != "")
        {   
          $MerchantOrderNo = $responsedata['orderid'];  
          $transaction_no  = $responsedata['transactionid'];
          $auth_status     = $responsedata['auth_status'];
          ...
          // echo '<pre>'; print_r($get_user_regnum_info); exit;
          if (count($get_user_regnum_info) > 0 && ($get_user_regnum_info[0]['status'] == 3 || $get_user_regnum_info[0]['status'] == 5) )
          {
            ...
            if(isset($MerchantOrderNo) && $MerchantOrderNo != '')
            {
              ...
              {
                log_dra_admin($log_title = "DRA Institute Online Payment Failed Due to RPE mode is not Physical : C_S2S", $log_message = "mode not defined. Qry : ".$error_qry);
                exit;
              }     
              ...
              $update_data = array(
                ...
                'callback'            => 'C_S2S',
                'description'         => 'Online Payment Success - Approved by Institute : C_S2S',
              ...
              if( $chk_exam_mode == 'PHYSICAL')
              {
                ...
                {
                  log_dra_admin($log_title = "DRA C_S2S when Online Payment is success Transaction status updated.", $log_message = serialize($update_data));
          
                  // Generate registration number for members if payment transaction is success
                  if( $payment_status == 1 ) //WHEN PAYMENT IS APPROVED
                  {
                  ...
                  $this->db->query("UPDATE dra_member_exam, dra_member_payment_transaction SET dra_member_exam.pay_status = '".$payment_status."' WHERE dra_member_exam.id = dra_member_payment_transaction.memexamid AND ptid = ".$id);
                    
                  $update_data =  array( 'pay_status' => $payment_status,'ptid' => $id );

                  log_dra_admin($log_title = "DRA Member Exam C_S2S Payment Transaction Status updated after Online Payment Success.", $log_message = serialize($update_data));
                  
                  // Manage Log
                  $pg_response = "encData=" . $encData . "&merchIdVal=" . $merchIdVal . "&Bank_Code=" . $bankid;
      ...
      if($pay_type == "iibfbulkdra")
      {
        ...
        if(count($responsedata) > 0)
        {
          ...
          { 
            $get_user_regnum = $this->master_model->getRecords('dra_payment_transaction', array('receipt_no' => $receiptNo) , 'id, date, status');
            // echo date("Y-m-d H:i:s",strtotime($get_user_regnum[0]['date'])).'----'.date('Y-m-d H:i:s', strtotime("-30 minutes")); exit;
            if(count($get_user_regnum) > 0 && ($get_user_regnum[0]['status'] == '3' || $get_user_regnum[0]['status'] == '5') && date("Y-m-d H:i:s",strtotime($get_user_regnum[0]['date'])) < date('Y-m-d H:i:s', strtotime("-30 minutes")))
            { 
              $update_data = array();
              ...
              $this->db->group_start();
              $this->db->where('receipt_no',$receiptNo);
              $this->db->group_end();
              ...
              // QUERY TO GET UPDATED PAYMENT DETAILS AFTER PAYMENT SUCCESS
              $iibfdra_payment_data = $this->master_model->getRecords('dra_payment_transaction', array('receipt_no' => $receiptNo, 'gateway'=>'2'), 'transaction_no, date, amount, id, description, status, exam_code, exam_period, receipt_no');
          ...
          $this->log_model->logdratransaction("billdesk", $pg_response, $responsedata['error_type']);
        }
      }
      else if($pay_type == "iibfamp")
      {
