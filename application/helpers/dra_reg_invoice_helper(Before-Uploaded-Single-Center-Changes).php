...
{ 
	...
	/*if($center_record[0]['invoice_flag'] == 'AS')
	{*/
		$CI->db->join('agency_center','agency_center.agency_id = dra_inst_registration.id');
		$ag_add = $CI->master_model->getRecords('dra_inst_registration',array('id'=>$record[0]['ref_id']),'inst_name,main_address1,main_address2,main_address3,main_address4,main_state,main_district');
		// gaurav commented $name_of_center = $record[0]['main_city'];
		$name_of_agency = $ag_add[0]['inst_name'];
		
		$address = $ag_add[0]['main_address1']." ".$ag_add[0]['main_address2'];
		$address1 = $ag_add[0]['main_address3']." ".$ag_add[0]['main_address4']." ".$ag_add[0]['main_district'];
		
		$state_code = $ag_add[0]['main_state'];
		$state_data = $CI->master_model->getRecords('state_master', array(
                'state_code' => $state_code
            ));
		$state  = $state_data[0]['state_name'];
		
		/*$dra_inst_reg = $CI->master_model->getRecords('dra_inst_registration',array('id'=>$record[0]['ref_id']),'main_city');
	...
	/*}
	elseif($center_record[0]['invoice_flag'] == 'CS')
	{
		$CI->db->join('agency_center','agency_center.agency_id = dra_inst_registration.id');
		$ag_add = $CI->master_model->getRecords('dra_inst_registration',array('id'=>$record[0]['ref_id']),'inst_name,main_address1,main_address2,main_address3,main_address4');
		// gaurav commented $name_of_center = $record[0]['city'];
		$name_of_agency = $ag_add[0]['inst_name']; 	
	...
	// gaurav commented imagestring($im, 3, 118,  626,$city_name, $black); 
	imagestring($im, 3, 118,  626,"", $black);
	imagestring($im, 3, 260,  626, "CGST ", $black);
	imagestring($im, 3, 260,  646, "SGST ", $black);
	imagestring($im, 3, 260,  666, "IGST ", $black);
