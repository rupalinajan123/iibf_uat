...
function amtinword_renewal($amt){
   $number = $amt;
   $no = round($number);
...
function genarate_renewal_invoice($invoice_no){
	$CI = & get_instance();
	...
	/*if($invoice_info[0]['state_of_center'] == 'JAM'){
		return genarate_renewal_invoice_jk($invoice_no);
		exit;
	}*/
	
	if($invoice_info[0]['state_of_center'] == 'MAH'){
		$wordamt = amtinword_renewal($invoice_info[0]['cs_total']);
	}elseif($invoice_info[0]['state_of_center'] != 'MAH'){
		$wordamt = amtinword_renewal($invoice_info[0]['igst_total']);
	}
	...
	imagestring($im, 3, 40,  280, "Name of service recipient: ".$member_name, $black);
	imagestring($im, 3, 40,  300, "Address: ".$mem_info[0]['address1_pr'], $black);
	imagestring($im, 3, 40,  320, $mem_info[0]['address2_pr'], $black);
	imagestring($im, 3, 40,  340, $mem_info[0]['city_pr']."-".$mem_info[0]['pincode_pr'], $black);
	...
	imagestring($im, 3, 40,  400, "GSTIN / Unique Id: -", $black);
	...
	imagestring($im, 3, 118,  600, "Charges paid towards renewal of ordinary membership", $black);
	...
	$savepath = base_url()."uploads/renewal_invoice/user/";
	...
	imagepng($im,"uploads/renewal_invoice/user/".$imagename);
	$png = @imagecreatefromjpeg('assets/images/sign.jpg');
	$png2 = @imagecreatefromjpeg('assets/images/iibf_logo_short.jpg');
	$jpeg = @imagecreatefromjpeg("uploads/renewal_invoice/user/".$imagename);
	...
	imagestring($im, 3, 40,  280, "Name of the service recipient: ".$member_name, $black);
	imagestring($im, 3, 40,  300, "Address: ".$mem_info[0]['address1_pr'], $black);
	imagestring($im, 3, 40,  320, $mem_info[0]['address2_pr'], $black);
	imagestring($im, 3, 40,  340, $mem_info[0]['city_pr']."-".$mem_info[0]['pincode_pr'], $black);
	imagestring($im, 3, 40,  360, "State: ".$invoice_info[0]['state_name'], $black);
	...
	$savepath = base_url()."uploads/renewal_invoice/supplier/";
	$ino = str_replace("/","_",$invoice_info[0]['invoice_no']);
	$imagename = $invoice_info[0]['member_no']."_".$ino.".jpg";
	imagepng($im,"uploads/renewal_invoice/supplier/".$imagename);
	$png = @imagecreatefromjpeg('assets/images/sign.jpg');
	$png2 = @imagecreatefromjpeg('assets/images/iibf_logo_short.jpg');
	$jpeg = @imagecreatefromjpeg("uploads/renewal_invoice/supplier/".$imagename);
	...
	return $attachpath = "uploads/renewal_invoice/user/".$imagename;
	
	
}

function genarate_renewal_invoice_jk($invoice_no)
{
	$CI = & get_instance();
	$invoice_info = $CI->master_model->getRecords('exam_invoice',array('invoice_id'=>$invoice_no));
	$mem_info = $CI->master_model->getRecords('member_registration',array('regnumber'=>$invoice_info[0]['member_no']),'firstname,middlename,lastname,address1_pr,address2_pr,address3_pr,address4_pr,city_pr,state_pr,pincode_pr');
	...
	imagestring($im, 3, 100,  560, "Charges paid towards renewal of ordinary membership", $black);
...
function generate_renewal_invoice_number($invoice_id= NULL){
	$last_id='';
	$CI = & get_instance();
	if($invoice_id  !=NULL){
		$insert_info = array('invoice_id'=>$invoice_id);
		$last_id = str_pad($CI->master_model->insertRecord('config_renewal_member_invoice',$insert_info,true), 6, "0", STR_PAD_LEFT);;
	}
	return $last_id;
}
function generate_renewal_invoice_number_jammu($invoice_id= NULL)
{
	$last_id='';
	$CI = & get_instance();
	if($invoice_id  !=NULL){
		$insert_info = array('invoice_id'=>$invoice_id);
		$last_id = str_pad($CI->master_model->insertRecord('config_renewal_member_invoice_jammu',$insert_info,true), 4, "0", STR_PAD_LEFT);;
	}
	return $last_id;
}

function custom_genarate_renewal_invoice($invoice_no)
{ 
	$CI = & get_instance();
	$invoice_info = $CI->master_model->getRecords('exam_invoice',array('invoice_id'=>$invoice_no));
	$mem_info = $CI->master_model->getRecords('member_registration',array('regnumber'=>$invoice_info[0]['member_no']),'firstname,middlename,lastname,address1_pr,address2_pr,address3_pr,address4_pr,city_pr,state_pr,pincode_pr');
