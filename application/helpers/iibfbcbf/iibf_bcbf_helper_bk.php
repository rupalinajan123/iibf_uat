...
/********************************************************************************************************************
 ** Description: Common Helper for BCBF Module
...
/*START - Generate IIBF BCBF Exam Invoice Number for NEFT Payment*/
function generate_iibfbcbf_exam_invoice_number($invoice_id = NULL)
{
  $last_id = '';
  $CI = &get_instance();
  ...
  if ($invoice_id  != NULL)
  {
    ...
    $last_id = str_pad($CI->master_model->insertRecord('iibfbcbf_config_exam_invoice', $insert_info, true), 6, "0", STR_PAD_LEFT);;
  }
  return $last_id;
}
/*END - Generate IIBF BCBF Exam Invoice Number for NEFT Payment*/
...
/*START - Generate IIBF BCBF INVOICE IMAGE*/
function genarate_iibf_bcbf_exam_invoice($invoice_no)
{
  $CI = &get_instance();
  ...
  if (!$invoice_info)
  {
    ...
    $CI->Iibf_bcbf_model->insert_common_log('Transaction : IIBF BCBF exam invoice generation', 'exam_invoice', $CI->db->last_query(), $invoice_no, 'transaction_action', 'The Exam Invoice ID not found', json_encode($invoice_info));
    return '';
  }
  ...
  $inst_details = $CI->master_model->getRecords('iibfbcbf_agency_master am', array('am.agency_code' => $invoice_info[0]['institute_code']), 'am.agency_address1, am.agency_address2, am.agency_address3, am.agency_address4, am.agency_pincode, sm.state_name, sm.state_no, ct.city_name');

  if (!$inst_details)
  {
    //log_dra_user($log_title = "DRA exam invoice generation", $log_message = 'Institute code not found in dra_accerdited_master');
    $CI->Iibf_bcbf_model->insert_common_log('Transaction : IIBF BCBF exam invoice generation', 'iibfbcbf_agency_master', $CI->db->last_query(), $invoice_info[0]['institute_code'], 'transaction_action', 'The Agency code not found', json_encode($inst_details));
    return '';
  }

  // convert invoice amount in words
  $amt_in_words = '';
  if ($invoice_info[0]['state_code'] == '27') //if ($invoice_info[0]['state_of_center'] == 'MAH')
  {
  ...
  /* if($invoice_info[0]['center_code'] != 0)
  {
    ...
    $center = $CI->master_model->getRecords('iibfbcbf_centre_master',array('centre_id'=>$invoice_info[0]['center_code']));
    
    $state_desc = $CI->master_model->getRecords('state_master',array('state_code'=>$center[0]['centre_state']));
    $city_desc = $CI->master_model->getRecords('city_master',array('id'=>$center[0]['centre_city']));
    
    
    
    $address1 = $center[0]['centre_address1'];
    $address2 = $center[0]['centre_address2'];
    $pincode = $center[0]['centre_pincode'];
  ...
  else
  {
    ...
    if($inst_details[0]['agency_city']!=''){
      if(is_int($inst_details[0]['agency_city'])){
        $city_desc = $CI->master_model->getRecords('city_master',array('id'=>$inst_details[0]['agency_city']));
        $city = $city_desc[0]['city_name'];
      }else{
        $city = $inst_details[0]['agency_district'];
      }
    }else{
      $city  = '-';
    }
  ...
  $payment_mode = 'Bulk';
  $payment_info = $CI->master_model->getRecords('iibfbcbf_payment_transaction', array('id' => $invoice_info[0]['pay_txn_id']), 'payment_mode, exam_ids, centre_id');
  if ($payment_info > 0)
  {
  ...
  if ($payment_mode == 'Bulk')
  {
    ...
    $center_data = $CI->master_model->getRecords('iibfbcbf_centre_master cm', array('cm.centre_id' => $payment_info[0]['centre_id']), 'cm.invoice_address, cm.centre_address1, cm.centre_address2, cm.centre_address3, cm.centre_address4, cm.centre_pincode, sm.state_name, sm.state_no, ct.city_name');
  ...
  {
    $CI->db->join('iibfbcbf_batch_candidates bc', 'bc.candidate_id = me.candidate_id', 'INNER');
    $CI->db->join('state_master sm', 'sm.state_code = bc.state', 'LEFT');
    $CI->db->join('city_master ct', 'ct.id = bc.city', 'LEFT');
    $candidate_data = $CI->master_model->getRecords('iibfbcbf_member_exam me', array('me.member_exam_id' => $payment_info[0]['exam_ids']), 'bc.salutation, bc.first_name, bc.middle_name, bc.last_name, bc.address1, bc.address2, bc.address3, bc.address4, bc.pincode, sm.state_name, sm.state_no, ct.city_name');
  ...
  if ($invoice_info[0]['state_code'] != '27' && $invoice_info[0]['state_code'] != '01')
  {
  ...
  if ($invoice_info[0]['state_code'] == '01') //if ($invoice_info[0]['state_of_center'] == 'JAM')
  {
  ...
  $savepath = base_url() . "uploads/iibfbcbf/iibf_bcbf_examinvoice/user/";
  ...
  imagepng($im, "uploads/iibfbcbf/iibf_bcbf_examinvoice/user/" . $imagename);
  $png = @imagecreatefromjpeg('assets/images/sign.jpg');
  $png2 = @imagecreatefromjpeg('assets/images/iibf_logo_short.jpg');
  $jpeg = @imagecreatefromjpeg("uploads/iibfbcbf/iibf_bcbf_examinvoice/user/" . $imagename);
  ...
  else //elseif ($invoice_info[0]['state_of_center'] != 'MAH')
  {
    imagestring($im, 3, 900,  830, $invoice_info[0]['igst_total'], $black);
  }
  ...
  $savepath = base_url() . "uploads/iibfbcbf/iibf_bcbf_examinvoice/supplier/";
  $ino = str_replace("/", "_", $invoice_info[0]['invoice_no']);
  $imagename = $invoice_info[0]['institute_code'] . "_" . $ino . ".jpg";

  imagepng($im, "uploads/iibfbcbf/iibf_bcbf_examinvoice/supplier/" . $imagename);
  $png = @imagecreatefromjpeg('assets/images/sign.jpg');
  $png2 = @imagecreatefromjpeg('assets/images/iibf_logo_short.jpg');
  $jpeg = @imagecreatefromjpeg("uploads/iibfbcbf/iibf_bcbf_examinvoice/supplier/" . $imagename);
  ...
  return $attachpath = "uploads/iibfbcbf/iibf_bcbf_examinvoice/user/" . $imagename;
}
/*END - Generate IIBF BCBF INVOICE IMAGE*/
...
function iibfbcbf_img_p($member_id = NULL, $type = '')
{
  $CI = &get_instance();
  $mem_info = $CI->master_model->getRecords('iibfbcbf_batch_candidates', array('regnumber' => $member_id), 'regnumber,candidate_photo,candidate_sign');
  if ($mem_info)
  {
    $scannedphoto = '';
    if ($mem_info[0]['candidate_photo'] != '' && $type == 'photo')
    {
      $new_image_path  = 'uploads/iibfbcbf/photo/';
    ...
    {
      $new_image_path  = 'uploads/iibfbcbf/sign/';
...
function display_exam_name($description = '', $exam_code = '', $exam_type = '')
{
  ...
  if ($description != '')
  {
    $final_exam_name .= ucwords(strtolower($description));
  }
  ...
  else if (in_array($exam_code, array(1041, 1042)))
  {
