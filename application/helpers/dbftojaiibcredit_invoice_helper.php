...
function genarate_dbftojaiibcredit_invoice($invoice_no,$dbf_regnumber){
	$CI = & get_instance();
	...
	imagestring($im, 3, 118,  600, "Application for DBF To JAIIB Credit Transfer", $black);
	...
	imagepng($im, 'uploads/dbftojaiibcredit/user/'.$imagename);
	imagedestroy($im);
	...
	imagestring($im, 3, 40,  600, "1", $black);
	imagestring($im, 3, 118,  600, "Charges paid towards DBF to JAIIB Credit Transfer", $black);
	imagestring($im, 3, 550,  600, $invoice_info[0]['service_code'], $black);
	imagestring($im, 3, 690,  600, $invoice_info[0]['fee_amt'], $black);
	imagestring($im, 3, 820,  600, 1, $black);
	imagestring($im, 3, 900,  600, $invoice_info[0]['fee_amt'], $black);
	...
	$savepath = base_url()."uploads/dbftojaiibcredit/supplier/";
	$imagename = $invoice_info[0]['member_no']."_".$ino.".jpg";
	
	
	imagepng($im,"uploads/dbftojaiibcredit/supplier/".$imagename);
	$png = @imagecreatefromjpeg('assets/images/sign.jpg');
	$png2 = @imagecreatefromjpeg('assets/images/iibf_logo_short.jpg');
	$jpeg = @imagecreatefromjpeg("uploads/dbftojaiibcredit/supplier/".$imagename);
	...
	return $attachpath = "uploads/dbftojaiibcredit/user/".$imagename;
