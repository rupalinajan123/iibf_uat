...
function genarate_finquest_invoice_jk($invoice_no,$id){
	
	$CI = & get_instance();
	$invoice_info = $CI->master_model->getRecords('exam_invoice',array('invoice_id'=>$invoice_no));
	$mem_info = $CI->master_model->getRecords('fin_quest',array('id'=>$id));
	//middle name
	if(isset($mem_info[0]['mname']) )
	{
		$mname=$mem_info[0]['mname'];
	}else
	{
		$mname='';
	}
	//last name	
	if(isset($mem_info[0]['lname']) )
	{
		$lname=$mem_info[0]['lname'];
	}else
	{
		$lname='';
	}
		$member_name = $mem_info[0]['fname'].' '.$mname.' '.$lname;
	
	$wordamt = finamtinword($invoice_info[0]['igst_total']);
	...
	imagestring($im, 3, 22,  274, "Address: ".$mem_info[0]['address_1'], $black);
	imagestring($im, 3, 22,  286, $mem_info[0]['address_2'], $black);
	...
	$savepath = base_url()."uploads/finquestinvoice/user/";
	...
	imagepng($im,"uploads/finquestinvoice/user/".$imagename);
	$png = @imagecreatefromjpeg('assets/images/sign.jpg');
	$jpeg = @imagecreatefromjpeg("uploads/finquestinvoice/user/".$imagename);
	...
	$savepath = base_url()."uploads/finquestinvoice/supplier/";
	$ino = str_replace("/","_",$invoice_info[0]['invoice_no']);
	$imagename = $ino.".jpg";
	
	imagepng($im,"uploads/finquestinvoice/supplier/".$imagename);
	$png = @imagecreatefromjpeg('assets/images/sign.jpg');
	$jpeg = @imagecreatefromjpeg("uploads/finquestinvoice/supplier/".$imagename);
	@imagecopyresampled($im, $png, 760, 850, 0, 0, 50, 50, 170, 124);
	imagepng($im, 'uploads/finquestinvoice/supplier/'.$imagename);
	
	imagedestroy($im);
	
	return $attachpath = "uploads/finquestinvoice/user/".$imagename;
}
function genarate_finquest_invoice($invoice_no,$id)
{ 
	$CI = & get_instance();
	...
	//address
	$address_1=$address_2='';
	if(isset($mem_info[0]['address_1']) )
	{
		$address_1=$mem_info[0]['address_1'].$mem_info[0]['address_2'];
	}if(isset($mem_info[0]['address_2']) )
	{
		$address_2=$mem_info[0]['address_3'].$mem_info[0]['address_4'];
	}
	
		
	if($invoice_info[0]['state_of_center'] == 'MAH'){
		$wordamt = finamtinword($invoice_info[0]['cs_total']);
	...
	imagestring($im, 3, 118,  596, "Charges for  Subscribe to Finquest", $black);
...
function generate_finquest_invoice_number($invoice_id){
	$last_id='';
	$CI = & get_instance();
	...
	if($invoice_id  !=NULL)
	{
		...
		$last_id = str_pad($CI->master_model->insertRecord('config_finquest_invoice',$insert_info,true), 5, "0", STR_PAD_LEFT);
	}
	return $last_id;
}

function generate_finquest_invoice_number_jammu123($invoice_id){
	$last_id='';
	$CI = & get_instance();
	//$CI->load->model('my_model');
	if($invoice_id  !=NULL)
	{
		$insert_info = array('invoice_id'=>$invoice_id);
		//$last_id = str_pad($CI->master_model->insertRecord('config_finquest_invoice_jammu',$insert_info,true), 4, "0", STR_PAD_LEFT);
	}
	return $last_id;
}

function genarate_finquest_subscription_number($id,$categoty){
	$last_id='';
	$CI = & get_instance();
	if($id  !=NULL || $id !=''){
		$insert_info = array('id'=>$id,'created_date'=>date("Y-m-d H:i:s"),'category_type'=>$categoty);
		$last_insert_id = $CI->master_model->insertRecord('config_subscription_number_finquest',$insert_info,true);
	}
