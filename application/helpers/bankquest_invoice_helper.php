...
function log_banquest_invoice($log_title, $log_message = "",$invoice_id='',$bv_id='')
{
    $CI = & get_instance();
	$rId = $invoice_id;
	$regNo = $bv_id;
...
function genarate_bankquest_invoice($invoice_id,$bv_id){
	$CI = & get_instance();
	...
	log_banquest_invoice($log_title = "genarate_bankquest_invoice function call.", $log_message = serialize($invoice_id.'|'.$bv_id),$invoice_id = $invoice_id, $bv_id = $bv_id);
...
function generate_bankquest_invoice_number($invoice_id){
	$last_id='';
	$CI = & get_instance();
	...
	if($invoice_id  !=NULL)
	{
		...
		$last_id = str_pad($CI->master_model->insertRecord('config_bankquest_invoice',$insert_info,true), 5, "0", STR_PAD_LEFT);;
	}
	return $last_id;
}

function storedUserActivity_bv_number($log_title, $log_message, $rId, $regNo){
	$CI = & get_instance();
    $CI->load->model('Log_model');
    $CI->Log_model->userActivity_log($log_title, $log_message, $rId, $regNo);
}

function genarate_subscription_number($bv_id,$categoty){
	$last_id='';
	$CI = & get_instance();
	if($bv_id  !=NULL || $bv_id !=''){
		$insert_info = array('bv_id'=>$bv_id,'created_date'=>date("Y-m-d H:i:s"),'category_type'=>$categoty);
		$last_insert_id = $CI->master_model->insertRecord('config_subscription_number_bv',$insert_info,true);
		
		$log_title ="BV subscription number generate>>".$last_insert_id;
		$log_message = $CI->db->last_query();
		$rId = '1';
		$regNo = '1';
		storedUserActivity_bv_number($log_title, $log_message, $rId, $regNo); 
