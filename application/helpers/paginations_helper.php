...
if ( ! function_exists('check_session'))
{
	function check_session()
	{
		...
		/*print_r($CI->input->post('from_date'));exit;*/
		...
		/*Add date range filter POOJA MANE : 14/7/22*/
		if($CI->session->userdata('from_date')=='')
		{
			if($CI->input->post('from_date')!='')
				$from_date = $CI->input->post('from_date');
			else
				$from_date = $limit;
			$CI->session->set_userdata('from_date',$from_date);
		}
		else
		{
			if(isset($_POST['from_date']) && $CI->input->post('from_date') != $CI->session->userdata('from_date'))
			{
				$from_date = $CI->input->post('from_date');	
				$CI->session->set_userdata('from_date',$from_date);
			}
		...
		$per_page = $CI->session->userdata('to_date');

		if($CI->session->userdata('to_date')=='')
		{
			if($CI->input->post('to_date')!='')
				$to_date = $CI->input->post('to_date');
			else
				$to_date = $limit;
			$CI->session->set_userdata('to_date',$to_date);
		}
		else
		{
			if(isset($_POST['to_date']) && $CI->input->post('to_date') != $CI->session->userdata('to_date'))
			{
				$to_date = $CI->input->post('to_date');	
				$CI->session->set_userdata('to_date',$to_date);
			}
		...
		/*Add date range filter POOJA MANE : 14/7/22*/
		$session_arr['from_date'] = $from_date;
		$session_arr['to_date'] = $to_date;
		/*Add date range filter POOJA MANE : 14/7/22*/
		return $session_arr;
	}
}