...
function amtinwordcpd($amt){
 ...
 function genarate_cpd_invoice($invoice_no){
	//$invoice_no =   ;
	...
	if($invoice_info[0]['state_of_center'] == 'MAH'){
		$wordamt = amtinwordcpd($invoice_info[0]['cs_total']);
	...
	imagestring($im, 3, 118,  596, "Charges for  CPD", $black);
	...
	function generate_cpd_invoice_number($invoice_id= NULL)
	{
		$last_id='';
		$CI = & get_instance();
		...
		if($invoice_id  !=NULL)
		{
			...
			$last_id = str_pad($CI->master_model->insertRecord('config_cpd_invoice',$insert_info,true), 5, "0", STR_PAD_LEFT);;
		}
		return $last_id;
	}
	...
	//custom CPD Invoice generation
	function custom_genarate_cpd_invoice($invoice_no){
	//$invoice_no =   ;
	...
	$savepath = base_url()."uploads/custom_cpdinvoice/user/";
	$ino = str_replace("/","_",$invoice_info[0]['invoice_no']);
	$imagename = $invoice_info[0]['member_no']."_".$ino.".jpg";
	...
	imagepng($im,"uploads/custom_cpdinvoice/user/".$imagename);
	$png = @imagecreatefromjpeg('assets/images/sign.jpg');
	$jpeg = @imagecreatefromjpeg("uploads/custom_cpdinvoice/user/".$imagename);
	@imagecopyresampled($im, $png, 760, 850, 0, 0, 50, 50, 170, 124);
	imagepng($im, 'uploads/custom_cpdinvoice/user/'.$imagename);
	...
	$savepath = base_url()."uploads/custom_cpdinvoice/supplier/";
	$ino = str_replace("/","_",$invoice_info[0]['invoice_no']);
	$imagename = $invoice_info[0]['member_no']."_".$ino.".jpg";
	
	imagepng($im,"uploads/custom_cpdinvoice/supplier/".$imagename);
	$png = @imagecreatefromjpeg('assets/images/sign.jpg');
	$jpeg = @imagecreatefromjpeg("uploads/custom_cpdinvoice/supplier/".$imagename);
	@imagecopyresampled($im, $png, 760, 850, 0, 0, 50, 50, 170, 124);
	imagepng($im, 'uploads/custom_cpdinvoice/supplier/'.$imagename);
	
	imagedestroy($im);
	
	return $attachpath = "uploads/custom_cpdinvoice/user/".$imagename;
}
