...
function genarate_duplicateicard_invoice($invoice_no){
	//$invoice_no =   ;
	...
	/*if($invoice_info[0]['state_of_center'] == 'JAM'){
		return genarate_duplicateicard_invoice_jk($invoice_no);
		exit;
	}*/
	
	if($invoice_info[0]['state_of_center'] == 'MAH'){
		$wordamt = amtinworddup($invoice_info[0]['cs_total']);
	}elseif($invoice_info[0]['state_of_center'] != 'MAH'){
		$wordamt = amtinworddup($invoice_info[0]['igst_total']);
	}
	...
	imagestring($im, 3, 118,  596, "Charges of Duplicate I Card", $black);
	...
	$savepath = base_url()."uploads/dupicardinvoice/user/";
	...
	imagepng($im,"uploads/dupicardinvoice/user/".$imagename);
	$png = @imagecreatefromjpeg('assets/images/sign.jpg');
	$png2 = @imagecreatefromjpeg('assets/images/iibf_logo_short.jpg');
	$jpeg = @imagecreatefromjpeg("uploads/dupicardinvoice/user/".$imagename);
	...
	$savepath = base_url()."uploads/dupicardinvoice/supplier/";
	//$imagename = 'new_dra.jpg';
	$ino = str_replace("/","_",$invoice_info[0]['invoice_no']);
	$imagename = $invoice_info[0]['member_no']."_".$ino.".jpg";
	$update_data = array('invoice_image' => $imagename);
	...
	imagepng($im,"uploads/dupicardinvoice/supplier/".$imagename);
	$png = @imagecreatefromjpeg('assets/images/sign.jpg');
	$png2 = @imagecreatefromjpeg('assets/images/iibf_logo_short.jpg');
	$jpeg = @imagecreatefromjpeg("uploads/dupicardinvoice/supplier/".$imagename);
	...
	return $attachpath = "uploads/dupicardinvoice/user/".$imagename;
}


function genarate_duplicateicard_invoice_jk($invoice_no){
	...
	imagestring($im, 3, 22,  148, "2nd & 3rd Floor, Kirol Road, Off-L.B.B Marg", $black);
...
function generate_duplicate_id_invoice_number($invoice_id= NULL)
	{
		$last_id='';
		$CI = & get_instance();
		...
		if($invoice_id  !=NULL)
		{
			...
			$last_id = str_pad($CI->master_model->insertRecord('config_dup_icard_invoice',$insert_info,true), 5, "0", STR_PAD_LEFT);;
		}
		return $last_id;
	}
...
function generate_duplicate_id_invoice_number_jammu($invoice_id= NULL)
	{
		...
		{
			$insert_info = array('invoice_id'=>$invoice_id);
			$last_id = str_pad($CI->master_model->insertRecord('config_dup_icard_invoice_jammu',$insert_info,true), 4, "0", STR_PAD_LEFT);;
		}
		return $last_id;
	}
