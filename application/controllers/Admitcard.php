...
class Admitcard extends CI_Controller
{
      ...
      if (isset($_POST['submit']))
      {
        ...
        if ($this->form_validation->run() == TRUE)
        {
          ...
          if (count($user_info) > 0)
          {
            ...
            $this->db->where_in('exm_cd', array(1046, 1047));
            $chkexam = $this->master_model->getRecords('admitcard_info', $exmarr);

            if (count($chkexam) > 0)
            {
              $mysqltime = date("H:i:s");
              $seprate_user_data = array(
          ...
          else
          {
            ...
            if (count($user_info) > 0)
            {
              ...
              $this->db->where_in('exm_cd', array(1037, 1038));
              $chkexam = $this->master_model->getRecords('admitcard_info', $exmarr);
              //   print_r($chkexam);exit;

              if (count($chkexam) > 0)
              {
                $mysqltime = date("H:i:s");
                $seprate_user_data = array(
                  'regid' => $user_info[0]['candidate_id'],
                  'spregnumber' => $user_info[0]['regnumber'],
  ...
  public function getadmitdashboard()
  {
    try
    {
      ...
      $query_1 = $this->db->query("select exm_cd,exm_prd from admit_card_details where mem_mem_no = '" . $member_id . "' AND exam_date >='" . $todayDate . "' order by admitcard_id desc");
      $exm_arr_1 = $query_1->result();

      // echo "<pre>"; print_r($exm_arr_1); exit;
      ...
      $examCdArr = array('1001', '1002', '1003', '1004', '1005', '1006', '1007', '1008', '1009', '1010', '1011', '1012', '1013', '1014', '1017', '1019', '1020', '2027', '1058'); //,'11'
      ...
      if (sizeof($exm_arr_1) > 0)
      {
        ...
        if(in_array($exm_arr_1[0]->exm_cd,$this->config->item('skippedAdmitCardForExams'))) {

          /*START: Hide Admit Card Link during exam activation period Added on 09 Dec 2025 by Anil S*/
          ...
          $exam_activation_data = $this->master_model->getRecords('exam_activation_master', array('exam_activation_master.exam_code' => $exm_arr_1[0]->exm_cd, 'exam_activation_master.exam_period' => $exm_arr_1[0]->exm_prd), 'exam_code,exam_period');
          if(count($exam_activation_data) > 0)
          {
            if(in_array($exam_activation_data[0]['exam_code'],$this->config->item('skippedAdmitCardForExams'))) {
              return 1;
            }
            //return 1;
          }
          /*END: Hide Admit Card Link during exam activation period Added on 09 Dec 2025 by Anil S*/
  ...
  public function getadmitcardsp($enc_exam_code = '')
  {
    try
    {
      ...
      if (in_array($exam_code, array(1037, 1038)))
      {
        ...
        {
          $disp_unique_no = date("mdYHis", strtotime($payment_data[0]['created_on'])) . ' ' . $payment_data[0]['ref_utr_no'];
        }
      }
      else
      {
      ...
      {
        $memberDetails = $this->master_model->getRecords('iibfbcbf_batch_candidates', array('regnumber' => $member_id), array('dob AS dateofbirth', ' "" AS associatedinstitute'));
      }
  ...
  public function getadmitcardpdfsp($enc_exam_code = '')
  {
    try
    {
      ...
      // Added by priyanka D >>23apr25
      $this->db->select('center_name');
      $this->db->order_by('id', 'desc');
      $centerRow = $this->db->get_where('center_master', array('center_code' => $result->center_code, 'exam_name' => $exam_code));
      $centerRow = $centerRow->row();

      $center = $centerRow->center_name;
  ...
  public function getadmitcardjd()
  {
    try
    {
      ...
      $dipcert_arr = array(8, 11, 18, 19, 119, 24, 25, 26, 78, 79, 149, 151, 153, 154, 156, 157, 158, 162, 163, 165, 166);
      ...
      if ($exam_code == $this->config->item('examCodeJaiib') || $exam_code == $this->config->item('examCodeDBF'))
      {
        // $drr = array('2022-04-09','2022-04-23','2022-04-24');
        $drr = array('2025-11-02', '2025-11-09', '2025-11-08','2025-11-16');
      }
      else if (in_array($exam_code, $dipcert_arr)  || $exam_code == $this->config->item('examCodeSOB'))
      {
        $drr = array('2026-01-04','2026-01-11','2026-01-18');
      }
      else if (in_array($exam_code, $caiib_arr))
      {
        $drr = array('2025-11-30', '2025-12-07', '2025-12-13', '2025-12-14', '2025-12-21');
      }
      else
      {
      ...
      if ($exam_code == $this->config->item('examCodeJaiib') || $exam_code == $this->config->item('examCodeDBF'))
      { //echo $member_id;exit;
        /*$subject=$this->db->query("SELECT subject_description,date,time,venueid,seat_identification FROM admit_subject_master JOIN admitcard_info ON admit_subject_master.subject_code = LEFT(admitcard_info.sub_cd,3) WHERE admit_subject_master.exam_code = ".$exam_code." AND admitcard_info.mem_mem_no = '".$member_id."' AND subject_delete = '0' AND date >='".date('Y-m-d')."' ORDER BY STR_TO_DATE(date, '%Y-%m-%d') ASC ");*/

        $subject = $this->db->query("SELECT subject_description,date,time,venueid,seat_identification FROM admit_subject_master JOIN admitcard_info ON admit_subject_master.subject_code = LEFT(admitcard_info.sub_cd,3) WHERE admit_subject_master.exam_code = " . $exam_code . " AND admitcard_info.mem_mem_no = '" . $member_id . "' AND subject_delete = '0' AND date >='" . date('Y-m-d') . "' ORDER BY STR_TO_DATE(date, '%Y-%m-%d') ASC "); // Remove today date condition by Anil S on 16 Oct 2023.

        //	echo $this->db->last_query();
      }
      else if (in_array($exam_code, $dipcert_arr) || $exam_code == $this->config->item('examCodeSOB'))
      {
        $subject = $this->db->query("SELECT subject_description,date,time,venueid,seat_identification FROM admit_subject_master JOIN admitcard_info ON admit_subject_master.subject_code = LEFT(admitcard_info.sub_cd,3) WHERE admit_subject_master.exam_code = " . $exam_code . " AND admitcard_info.mem_mem_no = '" . $member_id . "' AND subject_delete = '0' AND date IN('2026-01-04','2026-01-11','2026-01-18') GROUP BY admitcard_info.date,admitcard_info.time ORDER BY STR_TO_DATE(date, '%Y-%m-%d') ASC ");

        // In above query gaurav add the "GROUP BY admitcard_info.date, admitcard_info.time" condition

      }
      else if (in_array($exam_code, $caiib_arr))
      {
        $subject = $this->db->query("SELECT subject_description,date,time,venueid,seat_identification FROM admit_subject_master JOIN admitcard_info ON admit_subject_master.subject_code = LEFT(admitcard_info.sub_cd,3) WHERE admit_subject_master.exam_code = " . $exam_code . " AND admitcard_info.mem_mem_no = '" . $member_id . "' AND subject_delete = '0' AND date IN('2025-11-30', '2025-12-07', '2025-12-13', '2025-12-14', '2025-12-21') ORDER BY STR_TO_DATE(date, '%Y-%m-%d') ASC ");
      }
      else
      {
      ...
      $this->db->select('admit_card_details.*, DATE_FORMAT(admit_card_details.created_on, "%d%m%Y%H%i%s") as created_on');
      $this->db->from('admit_card_details');
      $this->db->where(array('admit_card_details.mem_mem_no' => $member_id, 'exm_cd' => $exam_code, 'remark' => 1, 'exam_date = ' => $result->date));
      $record = $this->db->get();
      $admit_result = $record->row();
      $result->created_on = $admit_result->created_on;

      $this->db->select('transaction_no');
      $payment = $this->db->get_where('payment_transaction', array('member_regnumber' => $admit_result->mem_mem_no, 'ref_id' => $admit_result->mem_exam_id));
      $payment_result = $payment->row();
      // Added by Priyanka W end for Transaction no dissplay on admit card
      ...
      //echo 'hhhhhh';
      if ($exam_code == $this->config->item('examCodeJaiib') || $exam_code == $this->config->item('examCodeDBF') || $exam_code == $this->config->item('examCodeSOB') || in_array($exam_code, $caiib_arr))
      {
        $this->load->view('admitcard_jaiib_caiib', $data);
      }
      else
      $this->load->view('admitcard_jaiib_caiib', $data);
  ...
  // function run for old table  from website link without exam codde 101
  public function getadmitcardjd_gaurav()
  {
    try
    {
      if ($this->session->userdata('spregnumber') != '')
      ...
      {
        // $drr = array('2022-04-09','2022-04-23','2022-04-24');
        $drr = array('2023-11-05', '2023-11-19', '2023-11-25');
      }
      else if (in_array($exam_code, $dipcert_arr))
      {
        $drr = array('2024-01-07', '2024-01-28', '2024-01-21');
      }
      else if (in_array($exam_code, $caiib_arr))
      {
        $drr = array('2023-11-26', '2023-12-03', '2023-12-09', '2023-12-10', '2023-12-17');
      }
      else
      {
        $drr = array('2022-01-30', '2022-02-06', '2022-02-12');
      ...
      {
        $subject = $this->db->query("SELECT subject_description,date,time,venueid,seat_identification FROM admit_subject_master JOIN admitcard_info ON admit_subject_master.subject_code = LEFT(admitcard_info.sub_cd,3) WHERE admit_subject_master.exam_code = " . $exam_code . " AND admitcard_info.mem_mem_no = '" . $member_id . "' AND subject_delete = '0' AND date IN('2024-01-07','2024-01-28','2024-01-21') GROUP BY admitcard_info.date,admitcard_info.time  ORDER BY STR_TO_DATE(date, '%Y-%m-%d') ASC ");
        // echo $this->db->last_query(); exit;
      }
      else if (in_array($exam_code, $caiib_arr))
      {
        $subject = $this->db->query("SELECT subject_description,date,time,venueid,seat_identification FROM admit_subject_master JOIN admitcard_info ON admit_subject_master.subject_code = LEFT(admitcard_info.sub_cd,3) WHERE admit_subject_master.exam_code = " . $exam_code . " AND admitcard_info.mem_mem_no = '" . $member_id . "' AND subject_delete = '0' AND date IN('2023-11-26','2023-12-03','2023-12-09','2023-12-10','2023-12-17') ORDER BY STR_TO_DATE(date, '%Y-%m-%d') ASC ");
      }
      else
      {
        $subject = $this->db->query("SELECT subject_description,date,time,venueid,seat_identification FROM admit_subject_master JOIN admitcard_info ON admit_subject_master.subject_code = LEFT(admitcard_info.sub_cd,3) WHERE admit_subject_master.exam_code = " . $exam_code . " AND admitcard_info.mem_mem_no = '" . $member_id . "' AND subject_delete = '0' AND date IN('2022-01-30','2022-02-06','2022-02-12') ORDER BY STR_TO_DATE(date, '%Y-%m-%d') ASC ");
      ...
      $data = array("exam_name" => $exam_result->description, "subject" => $subject_result, "record" => $result, "medium" => $medium_result->medium_description, "img_path" => $img_path, "sig_path" => $sig_path, "subjectprint" => $subject_result, "examdate" => $printdate, "exam_code" => $exam_code, "vcenter" => $vcenter, "records" => $results, "recordsp" => $results, 'frm' => 'dwn', 'mid' => $member_id, 'idate' => $exdate, "member_result" => $result, "venue_result" => $results, 'member_id' => $member_id, 'memberDetails' => $memberDetails, 'instituteDetails' => $instituteDetails, 'optFlgRecord' => $optFlgRecord, 'selected_vendor' => $selected_vendor, 'transaction_no' => $payment_result->transaction_no);
      //echo 'hhhhhh';
  ...
  public function getadmitcardpdfjd()
  {
    try
    {
      ...
      $dipcert_arr = array(8, 11, 18, 19, 24, 25, 26, 78, 79, 149, 151, 153, 154, 156, 158, 162, 163, 165, 166, 220);
      $caiib_arr = array($this->config->item('examCodeCaiib'), 61, $this->config->item('examCodeCaiibElective63'), 65, $this->config->item('examCodeCaiibElective68'), $this->config->item('examCodeCaiibElective69'), $this->config->item('examCodeCaiibElective70'), $this->config->item('examCodeCaiibElective71'));

      //priyanka d >>27-dec-24 >> by default selecting venue for jaiib/caiiib as we don't have to create admitcard from filled form now >> exam_cd
      /*if(in_array($exam_code,$this->config->item('skippedAdmitCardForExams'))) {
        return 1;
      }*/

      $caiib_dip_arr = array(24,25,26,149);
      //priyanka d >>27-dec-24 >> by default selecting venue for jaiib/caiiib as we don't have to create admitcard from filled form now >> exam_cd
      if(in_array($exam_code,$caiib_dip_arr)) {
        return 1;
      }
      ...
      if ($exam_code == $this->config->item('examCodeJaiib') || $exam_code == $this->config->item('examCodeDBF'))
      {
        $subject = $this->db->query("SELECT subject_description,date,time,venueid,seat_identification FROM admit_subject_master JOIN admitcard_info ON admit_subject_master.subject_code = LEFT(admitcard_info.sub_cd,3) WHERE admit_subject_master.exam_code = " . $exam_code . " AND admitcard_info.mem_mem_no = '" . $member_id . "' AND subject_delete = '0'  AND date IN('2025-11-02', '2025-11-09', '2025-11-08', '2025-11-16') ORDER BY STR_TO_DATE(date, '%Y-%m-%d') ASC ");
      }
      else if (in_array($exam_code, $dipcert_arr))
      {
        $subject = $this->db->query("SELECT subject_description,date,time,venueid,seat_identification FROM admit_subject_master JOIN admitcard_info ON admit_subject_master.subject_code = LEFT(admitcard_info.sub_cd,3) WHERE admit_subject_master.exam_code = " . $exam_code . " AND admitcard_info.mem_mem_no = '" . $member_id . "' AND subject_delete = '0'  AND date IN('2026-01-04','2026-01-11','2026-01-18') GROUP BY admitcard_info.date, admitcard_info.time ORDER BY STR_TO_DATE(date, '%Y-%m-%d') ASC ");
      ...
      // Added by Pooja M for Transaction no dissplay on admit card
      $this->db->select('admit_card_details.*, DATE_FORMAT(admit_card_details.created_on, "%d%m%Y%H%i%s") as created_on');
      $this->db->from('admit_card_details');
      $this->db->where(array('admit_card_details.mem_mem_no' => $member_id, 'exm_cd' => $exam_code, 'remark' => 1, 'exam_date = ' => $result->date));
      $record = $this->db->get();
      $admit_result = $record->row();
      if (isset($admit_result) && count($admit_result) > 0)
      {
      $result->created_on = $admit_result->created_on;
      }
      ...
      $payment = $this->db->get_where('payment_transaction', array('member_regnumber' => $admit_result->mem_mem_no, 'ref_id' => $admit_result->mem_exam_id));
      $payment_result = $payment->row();
      // Added by Pooja M end for Transaction no dissplay on admit card
      ...
      if ($exam_code == $this->config->item('examCodeJaiib') || $exam_code == $this->config->item('examCodeDBF') || $exam_code == $this->config->item('examCodeSOB') || in_array($exam_code, $caiib_arr))
      {
        $html = $this->load->view('admitcard_jaiib_caiib', $data,true);
      }
      else
  ...
  public function getadmitcardsp_new()
  {
    ...
    try
    {
      ...
      $query_1 = $this->db->query("select exm_cd,exm_prd from admit_card_details where mem_mem_no = '" . $member_id . "' AND mem_exam_id = '" . $mem_exam_id . "' AND exm_cd = '" . $exam_code . "' AND remark = 1 order by admitcard_id desc");
      $exm_arr_1 = $query_1->result();
      //echo $this->db->last_query(); die;
      if(in_array($exm_arr_1[0]->exm_cd,$this->config->item('skippedAdmitCardForExams'))) 
      {
      ...
      if (in_array($exam_code, $caiib) || in_array($exam_code, $dipcert))
      {
        ...
        if (empty($results))
        { // priyanka d- 23-march-23 >> added this query because for caaiib admitcard not showing venue details
          //echo '2nd if';
          ...
          if (in_array($exam_code, $caiib))
          {
            $this->db->where(array('admit_card_details.exm_prd' => '224'));
          }

          //Added by Priyanka W for Dipcert Admit card
          if (in_array($exam_code, $dipcert))
          {
            $this->db->where(array('admit_card_details.exam_date>' => date('Y-m-d')));
          }
          //End
      ...
      {
        if ( $exam_code == 1048 || $exam_code == 1049 || $exam_code == 1050) //$exam_code == 1051 || $exam_code == 1034 || 
        {
          $this->load->view('remote_admitcardpdf_attach', $data);
          ## added by pooja mane to display 1051 admit card in user profile 2024-10-15
        }
        else
        {
  ...
  public function getadmitcardpdfsp_new()
  {
    try
    {
      ...
      else if (in_array($exam_code, $caiib))
      {
        $subject = $this->db->query("SELECT subject_description,admit_card_details.exam_date,time,venueid,seat_identification FROM admit_subject_master JOIN admit_card_details ON admit_subject_master.subject_code = LEFT(admit_card_details.sub_cd,3) WHERE admit_subject_master.exam_code = " . $exam_code . " AND admit_card_details.mem_mem_no = '" . $member_id . "' AND subject_delete = '0' and remark = 1 AND admit_card_details.exam_date >= '" . date("Y-m-d") . "' AND admit_card_details.mem_exam_id = " . $mem_exam_id . "   group by admit_card_details.exam_date  ORDER BY (admit_card_details.exam_date) ASC ");
        $subject_result = $subject->result();
      }
      else
      {
      ...
      if ($exam_code == 1001 || $exam_code == 1002 || $exam_code == 1003 || $exam_code == 1004  || $exam_code == 1005 || $exam_code == 1006 || $exam_code == 1007 || $exam_code == 1008 || $exam_code == 1009 || $exam_code == 1010 || $exam_code == 1011 || $exam_code == 1012 || $exam_code == 1013 || $exam_code == 1014  || $exam_code == 2027 || $exam_code == 1019 || $exam_code == 1020 || $exam_code == 1031 || $exam_code == 1032 || $exam_code == 1033 || $exam_code == 1058 || $exam_code == 1059 || $exam_code == 1051 || $exam_code == 1034 || $exam_code == 1062 || $exam_code == 1063 || $exam_code == 1064 || $exam_code == 1065 || $exam_code == 1066 || $exam_code == 1067 || $exam_code == 1068 || $exam_code == 1069)
      {
  ...
  public function jaiibdashboard()
  {
    try
    {
      ...
      // Comment today date condition by Anil S, Priyanka D on 16 Oct 2023
  ...
  public function jaiibdashboard_anil()
  {
    try
    {
      $result = array();
      ...
      if (sizeof($exm_arr) > 0)
      {
        if (count($exm_arr) > 1)
        {
          $record = $this->db->query("SELECT admitcard_info.exm_cd, admitcard_info.m_1, admitcard_info.sub_cd, admit_exam_master.description, admitcard_info.mode, admitcard_info.date as exam_date, admitcard_info.admitcard_id
					FROM admit_exam_master
					JOIN admitcard_info ON admitcard_info.exm_cd = admit_exam_master.exam_code
					WHERE admitcard_info.mem_mem_no = '" . $member_id . "' AND date IN ('2026-01-04','2026-01-11','2026-01-18')
					GROUP BY admitcard_info.exm_cd
					ORDER BY admitcard_info.admitcard_id DESC 
					 ");
          $result = $record->result();
