<?php
	...
	class Cron1 extends CI_Controller 
	{
		...
		{
			$log_title ="sbicallback Function call Cron1";
			$log_message = 'sbicallback Function call Cron1';
			$rId = '';
			$regNo = '';
			$this->member_regn_log($log_title, $log_message, $rId, $regNo);
			
			$filehandle = fopen("cs2s_log/cron1/lock.txt", "c+");
			if (flock($filehandle, LOCK_EX | LOCK_NB)) {
				...
				$query='SELECT receipt_no FROM payment_transaction WHERE date <= NOW() - INTERVAL 20 MINUTE AND date > NOW() - INTERVAL 40 MINUTE AND gateway = "sbiepay" AND status = 2';
				...
				if ($crnt_day_txn_qry->num_rows())
				{
					...
					$dir = 'cs2s_log/cron1/'.$todays_date;
					if(!is_dir($dir)){
						mkdir($dir, 0755);
					}
					$cell = 1;
					...
					foreach ($crnt_day_txn_qry->result_array() as $c_row)
					{
						...
						if (isset($responsedata) && count($responsedata) > 0)
						{
							...
							*/
							...
							/* else if($responsedata[12] == "iibfcc") ## iibfcc code added on 16 Mar 2021 commented on 2022-04-11 by chaitali
							{
								sleep(8);
								$MerchantOrderNo = $responsedata[6]; 
								$transaction_no  = $responsedata[1];
