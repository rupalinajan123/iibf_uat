<?php
	...
	class Cron1_billdesk_custom extends CI_Controller 
	{
		...
		public function billdesk_callback()
		{
			...
			$filehandle = fopen("cs2s_log/billdesk_cron1/lock_custom.txt", "c+");
			if (flock($filehandle, LOCK_EX | LOCK_NB)) 
			{
				...
				$query='SELECT receipt_no FROM payment_transaction WHERE date <= NOW() - INTERVAL 20 MINUTE AND date > NOW() - INTERVAL 40 MINUTE AND gateway = "billdesk" AND status = 2';
				
				// SELECT receipt_no,date FROM payment_transaction WHERE date <= NOW() - INTERVAL 60 MINUTE AND date > NOW() - INTERVAL 360 MINUTE AND gateway = "billdesk" AND status = 2 
				...
				//echo  "Total Count =>". $crnt_day_txn_qry->num_rows();
						...
						//	echo $c_row['receipt_no'].'<pre>transaction status='; print_r($responsedata); echo '</pre>'; 
							...
							$refundInitiated=0;
							if(isset($refundStatusData['refundid']) && $refundStatusData['refundid']!='')
							{
								echo'in refund status<br>';
								$refundInitiated=1;
							...
							if($refundInitiated==0 && isset($responsedata) && count($responsedata) > 0) {
								//echo'<pre>processing this record='.$c_row['receipt_no'].'<br>';
