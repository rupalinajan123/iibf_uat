...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Disa extends CI_Controller 
	{		
	...
	//exit;
	  ...
	  //accedd denied due to GST
		...
		$cutoffTime = new DateTime('2025-07-29 23:55'); // Fixed cutoff datetime

		// if ($currentTime >= $cutoffTime) {
		//     // Registration is closed
		//     echo "<br>DISA registration is closed."; exit;
		// } else {
		//     // Registration is open
		//     echo "<br>DISA registration is still open.";
		// } 
	}

  function index()
  {
    redirect(site_url('disa/login'));
  }

	 ##---------default userlogin (prafull)-----------##
	public function login()
	{
		//echo 'Registration of DISA has been temporarily stopped. We will start registration shortly';
	   // exit;
		...
		$this->form_validation->set_rules($config);
			$dataarr=array(
				'regnumber'=> $this->security->xss_clean($this->input->post('Username')),
				'isactive'=>'1',
				'isdeleted'=>'0'
			);
			if ($this->form_validation->run() == TRUE)
			{
				$where="(registrationtype='O' OR registrationtype='A' OR registrationtype='F' OR registrationtype='DB' OR registrationtype='NM')";
				$this->db->where($where);
				...
				{ 
					 if($user_info[0]['isactive']==1)
					 {
						...
						$flag=$this->checkusers('990');
						
						$check=$this->examapplied($this->security->xss_clean($this->input->post('Username')),base64_encode('990'));
						
						if($flag==0) 
						{
							redirect(base_url().'Disa/accessdenied/');
						}
						else if($check)
						{
							redirect(base_url().'Disa/alreadyapplied/');
						}
						else
						{
							//redirect(base_url().'Disa/sbi_make_payment/');
							redirect(base_url().'Disa/dashboard/');
						}
						//redirect(base_url().'Disa/examdetails/');
						 }
		...
		$this->load->view('DISA/DISA_login',$data);

	}

  function logout()
  {
    $user_data=array('mregid_applyexam'=>'', 'mregnumber_applyexam'=>'', 'mfirstname_applyexam'=>'', 'mmiddlename_applyexam'=>'', 'mlastname_applyexam'=>'', 'mtimer_applyexam'=>'', 'memtype'=>'', 'mpassword_applyexam'=>'');
    $this->session->set_userdata($user_data);
    redirect(site_url('disa/login'));
  }
      ...
      $this->check_admin_session_all_pages();
		...
		####DisaExamintion has been closed#####
		//$user_info=array();
		
		$data = array("user_info"=>$user_info);
		
		$this->load->view('DISA/DISA_DASHBOARD',$data);
	...
	{
    $this->check_admin_session_all_pages();
		$message='<div style="color:#F00">You are not eligible to register for Diploma in   Information System Audit (DISA)</div>';
	...
	{
    $this->check_admin_session_all_pages();
		 $message='<div style="color:#F00">You have already applied for Diploma in Information System Audit (DISA) </div>';
		 $data=array('middle_content'=>'DISA/not_eligible','check_eligibility'=>$message);
		 $this->load->view('DISA/mem_apply_exam_common_view',$data);
	}
...
requestparameter=IIBF|2138759|NA|500.00|NA|NA|NA|INR|NA|R|iibf|NA|NA|F|iibfexam|500081141|148201701|NA|NA|NA|NA|http://abc.somedomain.com|2387462372
	...
	public function sbi_make_payment()
	{
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			//$amount = $this->config->item('DISA_apply_fee');
			$amount =$this->config->item('DISA_cs_total');
			$inser_array=array(	'regnumber'=>$this->session->userdata('mregnumber_applyexam'),
			 								'exam_code'=>'990',
											'exam_mode'=>'',
											'exam_medium'=>'E',
			...
			$insert_data = array(
									...
									'pay_type'         => '2',
									'ref_id'           => $inser_id,
									'description'      =>'Diploma in  Information System Audit (DISA)',//"Duplicate ID card request. Reason:".$this->session->userdata('desc'),
									'status'           => '2',
									'exam_code'    	   => '990',
			...
			else
			{
				...
				$invoice_insert_array=array('pay_txn_id'=>$pt_id,
											...
											'app_type'=>'O',
											'exam_period'=>'904',
											'service_code'=>$this->config->item('DISA_service_code'),
											'qty'=>'1',
											'state_code'=>'27',
											'state_name'=>'MAHARASHTRA',
											'tax_type'=>'Intra',
											'fee_amt'=>$this->config->item('DISA_apply_fee'),
											'cgst_rate'=>$this->config->item('DISA_cgst_rate'),
											'cgst_amt'=>$this->config->item('DISA_cgst_amt'),
											'sgst_rate'=>$this->config->item('DISA_sgst_rate'),
											'sgst_amt'=>$this->config->item('DISA_sgst_amt'),
											'igst_rate'=>'0.00',
											'igst_amt'=>'0.00',
											'cs_total'=>$this->config->item('DISA_cs_total'),
											'igst_total'=>'0.00',
											'exempt'=>'NE',
											'created_on'=>date('Y-m-d H:i:s'));
				...
				$this->load->view('pg_sbi_form',$data);
					...
					elseif ($pg_name == 'billdesk')
					{
						...
						$billdesk_res = $this->billdesk_pg_model->init_payment_request($MerchantOrderNo, $amount, $member_exam_id, $member_exam_id, '', 'Disa/handle_billdesk_response', '', '', '', $custom_field_billdesk);
			...
			if (isset($_REQUEST['transaction_response'])) 
			{
				...
				if($auth_status == "0300" && $qry_api_response['auth_status'] == '0300')
				{
					...
					if($get_user_regnum[0]['status']==2)
					{
						...
						if($exam_info[0]['exam_code']==990) { $sms_final_str = $emailerstr[0]['sms_text']; }
						else
						{
				...
				elseif ($auth_status=='0002') {
					...
					$pg_response = "encData=".$encData."&merchIdVal=".$merchIdVal."&Bank_Code=".$Bank_Code;
					$this->log_model->logtransaction("sbiepay", $pg_response,$responsedata['transaction_error_type']);
					redirect(base_url().'Disa/pending/'.base64_encode($MerchantOrderNo));
				}
				else
				{
	...
	public function details($order_no=NULL,$excd=NULL)
	{
		...
		$data=array('middle_content'=>'DISA/exam_applied_success');
		$this->load->view('DISA/mem_apply_exam_common_view',$data);
	...
	public function fail($order_no=NULL)
	{
		...
		$data=array('middle_content'=>'DISA/exam_applied_fail');
		$this->load->view('DISA/mem_apply_exam_common_view',$data);
		}
		else
		{
		...
		public function pending($order_no=NULL)
	{
		...
		$data=array('middle_content'=>'DISA/exam_applied_pending');
		$this->load->view('DISA/mem_apply_exam_common_view',$data);
		}
		else
		{
      ...
      if(in_array($examcode,$exam_code))
			{
				 ...
				 $valid_member_list=$this->master_model->getRecords('eligible_master',array('eligible_period'=>'904'),'member_no');
         //echo $this->db->last_query(); exit;
  ...
  function check_admin_session_all_pages($login_as='') //login_as = admin or agency or centre
  {
    ...
    {
      redirect(site_url('disa/logout'),'refresh');
    }
