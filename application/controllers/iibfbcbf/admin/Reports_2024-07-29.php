...
class Reports extends CI_Controller
{
  ...
  public function get_batch_mis_data_ajax()
  {
    ...
    $column_order = array('acb.batch_id', 'am1.agency_name', 'CONCAT(cm.centre_name," (",cm1.city_name,")") AS DispCentreName', 'acb.batch_code', 'IF(acb.batch_type=1, "Basic", IF(acb.batch_type=2, "Advanced", "")) AS DispBatchType', 'acb.batch_start_date', 'acb.batch_end_date', 'CONCAT(TIME_FORMAT(acb.batch_daily_start_time, "%h:%i %p"), " To ", TIME_FORMAT(acb.batch_daily_end_time, "%h:%i %p")) AS BatchTime', 'acb.total_candidates', 'IF(acb.batch_status = 0, "In Review", IF(acb.batch_status = 1, "Final Review", IF(acb.batch_status = 2, "Batch Error", IF(acb.batch_status = 3, "Go Ahead", IF(acb.batch_status = 4, "Hold", IF(acb.batch_status = 5, "Rejected", IF(acb.batch_status = 6, "Re-Submitted", "Cancelled"))))))) AS DispBatchStatus', 'COUNT(DISTINCT CASE WHEN bc.hold_release_status = "3" THEN bc.candidate_id END) AS total_release_candidate', 'COUNT(DISTINCT CASE WHEN bc.hold_release_status = "2" OR bc.hold_release_status = "1" THEN bc.candidate_id END) AS total_hold_candidate', 'GROUP_CONCAT( DISTINCT im.inspector_name SEPARATOR ", ") AS all_assign_inspectors', 'COUNT(bi.inspection_id) AS inspection_count', 'IF(bi.inspection_start_time != "0000-00-00 00:00:00", SUM(TIMESTAMPDIFF(MINUTE, bi.inspection_start_time, bi.created_on)),"0") AS total_inspection_time_minutes', '(
      SUM(CASE WHEN bi.teaching_quality_interaction_with_candidates = "Poor" THEN 1 ELSE 0 END) +
      SUM(CASE WHEN bi.teaching_quality_softskill_session = "Poor" THEN 1 ELSE 0 END) +
      SUM(CASE WHEN bi.candidates_attentiveness = "Poor" THEN 1 ELSE 0 END) +
      SUM(CASE WHEN bi.attitude_behaviour = "Poor" THEN 1 ELSE 0 END) +
  ...
  public function get_batch_summary_data_ajax()
  {
    ...
    $column_search = array('am1.agency_code', 'am1.agency_name', 'CONCAT(cm.centre_name," (",cm1.city_name,")")', 'acb.batch_code', 'IF(acb.batch_online_offline_flag = "1", "Offline", "Online")', 'IF(acb.batch_type = "1", "Basic", "Advanced")', 'acb.training_language', 'acb.batch_start_date', 'acb.batch_end_date', 'acb.total_candidates', 'IF(acb.batch_status = 0, "In Review", IF(acb.batch_status = 1, "Final Review", IF(acb.batch_status = 2, "Batch Error", IF(acb.batch_status = 3, "Go Ahead", IF(acb.batch_status = 4, "Hold", IF(acb.batch_status = 5, "Rejected", IF(acb.batch_status = 6, "Re-Submitted", IF(acb.batch_status = 7, "Cancelled", "Drafted"))))))))', 'GROUP_CONCAT(DISTINCT im.inspector_name)', 'COUNT(bi.inspection_id)', 'GROUP_CONCAT(DISTINCT DATE_FORMAT(bi.created_on, "%Y-%m-%d"))', 'COUNT(DISTINCT CASE WHEN bc.hold_release_status = "2" OR bc.hold_release_status = "1" THEN bc.candidate_id END)'); //SET COLUMN FOR SEARCH
