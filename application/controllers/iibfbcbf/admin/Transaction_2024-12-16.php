  ...
  ********************************************************************************************************************/
  ...
  class Transaction extends CI_Controller 
  {
    public function __construct()
    {
      ...
      $this->venue_master_eligible_exam_codes_arr = array(1041,1042,1057);//VENUE MASTER IS ONLY APPLICABLE FOR EXAM CODE 1041, 1042, 1057. HENCE THE CAPACITY CHECK IS APPLY FOR ONLY 1041, 1042, 1057. 1041 & 1042 ARE HYBRID MODE EXAMS. 1057 IS BCBF NAR HYBRID EXAM

      $this->eligible_exam_code_for_admit_card_arr = array(1039,1040,1041,1042,1057); //ADMITCARD IS AVAILABLE ONLY FOR 1039,1040,1041,1042,1057 EXAM CODES. 1039 & 1040 ARE CSC MODE EXAMS. 1041 & 1042 ARE HYBRID MODE EXAMS. 1057 IS BCBF NAR HYBRID EXAM
    }
    ...
    public function get_transaction_data_ajax()
    {
      ...
      $column_order = array('pt.id', 'CONCAT(am.agency_name, " (", am.agency_code, " - ", IF(am.allow_exam_types="Bulk/Individual", "Regular", am.allow_exam_types),")") AS agency_name', 'CONCAT(cm.centre_name," (", cm.centre_username, " - ", cm1.city_name, ")") AS DispCentre', 'IF(pt.payment_mode="Bulk", pt.UTR_no, pt.transaction_no) AS DispTransactionNo', 'pt.receipt_no', 'CONCAT(em.description," (", em.exam_code, ")") AS description', 'pt.amount', 'pt.pay_count', 'IF(pt.payment_mode="Bulk", DATE_FORMAT(pt.date, "%Y-%m-%d"), DATE_FORMAT(pt.date, "%Y-%m-%d %H:%i")) AS PaymentDate', 'pt.exam_code', 'pt.exam_period', '"" AS DispMemberNumbers', '"" AS DispMemberTrainingIds', 'pt.payment_mode', 'IF(pt.status=0, "Fail", IF(pt.status=1, "Success", IF(pt.status=2, "Pending", IF(pt.status=3, "Applied", IF(pt.status=4, "Cancelled", ""))))) AS DispPayStatus', 'pt.status', 'pt.pg_flag'); //SET COLUMNS FOR SORT
      
      $column_search = array('CONCAT(am.agency_name, " (", am.agency_code, " - ", IF(am.allow_exam_types="Bulk/Individual", "Regular", am.allow_exam_types),")")', 'CONCAT(cm.centre_name," (", cm.centre_username, " - ", cm1.city_name, ")")', 'IF(pt.payment_mode="Bulk", pt.UTR_no, pt.transaction_no)', 'pt.receipt_no', 'CONCAT(em.description," (", em.exam_code, ")")', 'pt.amount', 'pt.pay_count', 'IF(pt.payment_mode="Bulk", DATE_FORMAT(pt.date, "%Y-%m-%d"), DATE_FORMAT(pt.date, "%Y-%m-%d %H:%i"))', 'pt.exam_code', 'pt.exam_period', 'pt.payment_mode', 'IF(pt.status=0, "Fail", IF(pt.status=1, "Success", IF(pt.status=2, "Pending", IF(pt.status=3, "Applied", IF(pt.status=4, "Cancelled", "")))))'); //SET COLUMN FOR SEARCH
      ...
      foreach ($Rows as $Res) 
      {
        ...
        $this->db->limit(1);
        $invoice_data = $this->master_model->getRecords('exam_invoice ei', array('ei.receipt_no' => $Res['receipt_no'], 'ei.exam_code'=>$Res['exam_code'], 'ei.exam_period'=>$Res['exam_period'], 'ei.app_type'=>$Res['pg_flag'], 'ei.invoice_image !='=>''), "invoice_id");
        
        if(count($invoice_data) > 0 && !empty($invoice_data[0]['invoice_id']) && $invoice_data[0]['invoice_id'] != "")
        {
          $btn_str .= ' <a target="_blank" href="'.site_url('iibfbcbf/download_file_common/index/'.url_encode($invoice_data[0]['invoice_id']).'/invoice_image').'" class="btn btn-info btn-xs" title="View Invoice"><i class="fa fa-file-text"></i></a> ';
        }
