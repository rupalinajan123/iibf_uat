  ...
  ********************************************************************************************************************/
  ...
  class Batch_candidates extends CI_Controller
  {
    ...
    public function get_batch_candidates_data_ajax()
    {
      ...
      if($s_full_name != "") { $Where .= " AND CONCAT(bc.salutation,' ', bc.first_name, IF(bc.middle_name != '', CONCAT(' ', bc.middle_name), ''), IF(bc.last_name != '', CONCAT(' ', bc.last_name), '')) LIKE '%".custom_safe_string($s_full_name)."%'"; }
      
      $s_status = trim($this->security->xss_clean($this->input->post('s_status')));
      if($s_status != "") { $Where .= " AND bc.hold_release_status = '".$s_status."'"; }     
    ...
    public function add_candidates($enc_batch_id=0, $enc_candidate_id=0)
    {
      ...
      if($enc_batch_id == '0') { redirect(site_url('iibfbcbf/admin/training_batches')); }
      else
      {
        ...
        if(count($batch_data) == 0) { redirect(site_url('iibfbcbf/admin/training_batches')); }  
      ...
      else
      {
        ...
        if(count($form_data) == 0) { redirect(site_url('iibfbcbf/admin/batch_candidates/add_candidates')); }
      ...
      if (isset($_POST) && count($_POST) > 0)
      {
        ...
        if($mode == 'Add') { $id_proof_file_req_flg = $qualification_certificate_file_req_flg = $candidate_photo_req_flg = $candidate_sign_req_flg = 'y'; }
        else
        {
          if($form_data[0]['id_proof_file'] == "") { $id_proof_file_req_flg = 'y'; } 
          if($form_data[0]['qualification_certificate_file'] == "") { $qualification_certificate_file_req_flg = 'y'; } 
          if($form_data[0]['candidate_photo'] == "") { $candidate_photo_req_flg = 'y'; } 
          if($form_data[0]['candidate_sign'] == "") { $candidate_sign_req_flg = 'y'; } 
        }
        ...
        if ($validate_form_type == 'full') //THIS FIELDS VALIDATE ONLY IF USER CLICK ON SUBMIT II BUTTON
        {
          ...
          $this->form_validation->set_rules('qualification_certificate_file', 'qualification certificate', 'callback_fun_validate_file_upload[qualification_certificate_file|' . $qualification_certificate_file_req_flg . '|jpg,jpeg,png|100|qualification certificate]'); //callback parameter separated by pipe 'input name|required|allowed extension|max size in kb|display name|min size in kb'  
          $this->form_validation->set_rules('candidate_photo', 'passport-size photo of the candidate', 'callback_fun_validate_file_upload[candidate_photo|' . $candidate_photo_req_flg . '|jpg,jpeg,png|20|passport photo of the candidate]'); //callback parameter separated by pipe 'input name|required|allowed extension|max size in kb|display name|min size in kb'  
          $this->form_validation->set_rules('candidate_sign', 'signature of the candidate', 'callback_fun_validate_file_upload[candidate_sign|' . $candidate_sign_req_flg . '|jpg,jpeg,png|20|signature of the candidate]'); //callback parameter separated by pipe 'input name|required|allowed extension|max size in kb|display name|min size in kb'        
