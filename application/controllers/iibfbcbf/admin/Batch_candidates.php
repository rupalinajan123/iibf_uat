...
class Batch_candidates extends CI_Controller
{
  ...
  public function index($enc_batch_id = 0)
  {
    ...
    if ($enc_batch_id == '0') //FOR ALL CANDIDATE LISTING
    {
      ...
      $data['page_title'] = 'IIBF - BCBF Admin Training Batches : All Candidates List';
    }
    else  //FOR SPECIFIC BATCH CANDIDATE LISTING
    {
      $data['act_id'] = "Training Batches";
      $data['sub_act_id'] = "Training Batches";
      $data['page_title'] = 'IIBF - BCBF Admin Training Batches : Batch Candidate List';
    ...
    $this->load->view('iibfbcbf/admin/candidate_list_admin', $data);
  }

  /******** START : SERVER SIDE DATATABLE CALL FOR GET THE CANDIDATES DATA ********/
  public function get_batch_candidates_data_ajax()
  {
    $table = 'iibfbcbf_batch_candidates bc';

    $column_order = array('bc.candidate_id', 'CONCAT(am1.agency_name, " (", am1.agency_code, " - ", IF(am1.allow_exam_types="Bulk/Individual", "Regular", am1.allow_exam_types),")") AS agency_name', 'CONCAT(cm.centre_name," (", cm.centre_username, " - ", cm1.city_name,")") AS DispCentreName', 'acb.batch_code', 'bc.training_id', 'bc.regnumber', 'CONCAT(bc.salutation, " ", bc.first_name, IF(bc.middle_name != "", CONCAT(" ", bc.middle_name), ""), IF(bc.last_name != "", CONCAT(" ", bc.last_name), "")) AS DispName', 'bc.dob', 'bc.mobile_no', 'bc.email_id', 'IF(bc.hold_release_status=1, "Auto Hold", IF(bc.hold_release_status=2, "Manual Hold", "Release")) AS HoldReleaseStatus', 'bc.batch_id', 'acb.batch_status', 'acb.batch_start_date', 'acb.batch_end_date', 'bc.hold_release_status'); //SET COLUMNS FOR SORT

    $column_search = array('acb.batch_code', 'CONCAT(am1.agency_name, " (", am1.agency_code, " - ", IF(am1.allow_exam_types="Bulk/Individual", "Regular", am1.allow_exam_types),")")', 'CONCAT(cm.centre_name," (", cm.centre_username, " - ", cm1.city_name,")")', 'bc.training_id', 'bc.regnumber', 'CONCAT(bc.salutation, " ", bc.first_name, IF(bc.middle_name != "", CONCAT(" ", bc.middle_name), ""), IF(bc.last_name != "", CONCAT(" ", bc.last_name), ""))', 'bc.dob', 'bc.mobile_no', 'bc.email_id', 'IF(bc.hold_release_status=1, "Auto Hold", IF(bc.hold_release_status=2, "Manual Hold", "Release"))'); //SET COLUMN FOR SEARCH
    $order = array('bc.candidate_id' => 'ASC', 'bc.batch_id' => 'DESC',); // DEFAULT ORDER

    $WhereForTotal = "WHERE bc.is_deleted = 0 ";
    $Where = "WHERE bc.is_deleted = 0  ";
    ...
    if ($s_batch_code != "")
    {
      $Where .= " AND acb.batch_code = '" . custom_safe_string($s_batch_code) . "'";
    ...
    $join_qry = " LEFT JOIN iibfbcbf_agency_centre_batch acb ON acb.batch_id = bc.batch_id";
    ...
    foreach ($Rows as $Res)
    {
      ...
      $btn_str .= ' <a href="' . site_url('iibfbcbf/admin/batch_candidates/candidates_details/' . $enc_batch_id . '/' . url_encode($Res['candidate_id'])) . '" class="btn btn-success btn-xs" title="View Details"><i class="fa fa-eye" aria-hidden="true"></i></a> ';

      if ($Res['batch_status'] == '3' /* && date('Y-m-d') <= $Res['batch_end_date']  && $enc_batch_id != "" && $enc_batch_id != "0"*/)
      {
        $btn_str .= '<a href="' . site_url('iibfbcbf/admin/batch_candidates/add_candidates/' . url_encode($Res['batch_id']) . '/' . url_encode($Res['candidate_id'])) . '" class="btn btn-primary btn-xs" title="Edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a> ';
      }
  ...
  public function add_candidates($enc_batch_id = 0, $enc_candidate_id = 0)
  {
    ...
    else
    {
      ...
      $this->db->join('city_master cm1', 'cm1.id = cm.centre_city', 'LEFT');
      $data['batch_data'] = $batch_data = $this->master_model->getRecords('iibfbcbf_agency_centre_batch acb', array('acb.batch_id' => $batch_id, 'acb.batch_status' => '3', 'acb.is_deleted' => '0', 'am.is_active' => '1', 'am.is_deleted' => '0', 'cm.status' => '1', 'cm.is_deleted' => '0'), "acb.*, am.agency_id, am.agency_code, am.agency_name, sm.state_name AS CentreState, cm.centre_district, cm1.city_name AS CentreCity");
    ...
    if ($enc_candidate_id == '0')
    {
      ...
      if ($batch_candidate_count >= $batch_data[0]['total_candidates'])
      {
        ...
        redirect(site_url('iibfbcbf/admin/batch_candidates/index/' . $enc_batch_id));
    ...
    else
    {
      ...
      $data['form_data'] = $form_data = $this->master_model->getRecords('iibfbcbf_batch_candidates bc', array('bc.centre_id' => $centre_id, 'bc.candidate_id' => $candidate_id, 'bc.batch_id' => $batch_data[0]['batch_id'], 'bc.is_deleted' => '0'), "bc.*, IF(bc.gender=1,'Male','Female') AS DispGender, IF(bc.qualification=1, 'Under Graduate', IF(bc.qualification=2,'Graduate','Post Graduate')) AS DispQualification");
      if (count($form_data) == 0)
      {
        redirect(site_url('iibfbcbf/admin/batch_candidates/add_candidates'));
      }
    ...
    if (isset($_POST) && count($_POST) > 0)
    {
      ...
      if ($this->form_validation->run())
      {
        ...
        else if ($error_flag == 0)
        {
          ...
          $adminName = $this->Iibf_bcbf_model->getLoggedInUserDetails($this->login_agency_or_centre_id, 'admin');
          ...
          if ($validate_form_type == 'full') //THIS CONDITION CHECK ONLY IF USER CLICK ON SUBMIT II BUTTON
          {
            ...
            $this->Iibf_bcbf_model->check_file_exist($chk_id_proof_file, "./" . $id_proof_file_path . "/", 'iibfbcbf/admin/batch_candidates/index/' . $enc_batch_id, 'Candidate record not added due to missing Proof of Identity'); //IF FILE NOT EXIST WHILE ADDING THE RECORD, THEN SHOW ERROR MESSAGE

            $this->Iibf_bcbf_model->check_file_exist($chk_qualification_certificate_file, "./" . $qualification_certificate_file_path . "/", 'iibfbcbf/admin/batch_candidates/index/' . $enc_batch_id, 'Candidate record not added due to missing Qualification Certificate'); //IF FILE NOT EXIST WHILE ADDING THE RECORD, THEN SHOW ERROR MESSAGE

            $this->Iibf_bcbf_model->check_file_exist($chk_candidate_photo, "./" . $candidate_photo_path . "/", 'iibfbcbf/admin/batch_candidates/index/' . $enc_batch_id, 'Candidate record not added due to missing Passport Photograph of the Candidate'); //IF FILE NOT EXIST WHILE ADDING THE RECORD, THEN SHOW ERROR MESSAGE

            $this->Iibf_bcbf_model->check_file_exist($chk_candidate_sign, "./" . $candidate_sign_path . "/", 'iibfbcbf/admin/batch_candidates/index/' . $enc_batch_id, 'Candidate record not added due to missing Signature of the Candidate'); //IF FILE NOT EXIST WHILE ADDING THE RECORD, THEN SHOW ERROR MESSAGE
          ...
          if ($mode == "Add")
          {
            ...
            if ($candidate_id > 0)
            {
              $this->Iibf_bcbf_model->insert_common_log('Admin : Candidate Added', 'iibfbcbf_batch_candidates', $this->db->last_query(), $candidate_id, 'candidate_action', 'The candidate has successfully added by the admin ' . $adminName['disp_name'], $posted_arr);

              $this->session->set_flashdata('success', 'Candidate record added successfully');
            }
            else
            {
              $this->Iibf_bcbf_model->insert_common_log('Admin : Candidate Added Error', 'iibfbcbf_batch_candidates', $this->db->last_query(), $candidate_id, 'candidate_action', 'The candidate has not added by the admin ' . $adminName['disp_name'], $posted_arr);
          ...
          else if ($mode == "Update")
          {
            ...
            $this->Iibf_bcbf_model->insert_common_log('Admin : Candidate Updated', 'iibfbcbf_batch_candidates', $this->db->last_query(), $candidate_id, 'candidate_action', 'The candidate has successfully updated by the admin ' . $adminName['disp_name'], $posted_arr);
    ...
    $this->load->view('iibfbcbf/admin/add_candidates_admin', $data);
  }
  ...
  function change_hold_release_status()
  {
    ...
    {
      echo $this->Iibf_bcbf_model->change_candidate_hold_release_status_common($_POST, 'admin');
    }
    else
    {
      echo "error";
  ...
  public function validation_check_mobile_exist($str = '', $type = 0) //$type : 0=>Server side, 1=>Ajax
  {
    ...
    if (isset($_POST) && $_POST['mobile_no'] != "")
    {
      ...
      $candidate_data = $this->Iibf_bcbf_model->validation_check_candidate_eligibility_to_add_in_new_batch($candidate_id, 'mobile', $mobile_no,$enc_batch_id);
  ...
  public function validation_check_email_exist($str = '', $type = 0) //$type : 0=>Server side, 1=>Ajax
  {
    ...
    if (isset($_POST) && $_POST['email_id'] != "")
    {
      ...
      $candidate_data = $this->Iibf_bcbf_model->validation_check_candidate_eligibility_to_add_in_new_batch($candidate_id, 'email', $email_id,$enc_batch_id);
      if (count($candidate_data) == 0)
      {
        $return_val_ajax = 'true';
      }
  ...
  public function validation_check_id_proof_number_exist($str = '', $type = 0) //$type : 0=>Server side, 1=>Ajax
  {
    ...
    if (isset($_POST) && $_POST['id_proof_number'] != "")
    {
      ...
      $candidate_data = $this->Iibf_bcbf_model->validation_check_candidate_eligibility_to_add_in_new_batch($candidate_id, 'id_proof', $id_proof_number,$enc_batch_id);
      if (count($candidate_data) == 0)
      {
        $return_val_ajax = 'true';
      }
  ...
  public function validation_check_qualification_candidates($str = '', $type = 0) //$type : 0=>Server side, 1=>Ajax
  {
    ...
    if (isset($_POST) && $_POST['qualification'] != "")
    {
      ...
      if ($selected_qualification == '1')
      {
        $chk_batch_candidate_count = $batch_data[0]['under_graduate_candidates'];
      }
      else if ($selected_qualification == '2')
      {
        $chk_batch_candidate_count = $batch_data[0]['graduate_candidates'];
      }
      else if ($selected_qualification == '3')
      {
        $chk_batch_candidate_count = $batch_data[0]['post_graduate_candidates'];
      }

      if (count($candidate_data) < $chk_batch_candidate_count)
      {
        $flag = 'success';
      }
      else
  ...
  public function validation_check_aadhar_no_exist($str = '', $type = 0) //$type : 0=>Server side, 1=>Ajax
  {
    ...
    if (isset($_POST) && $_POST['aadhar_no'] != "")
    {
      ...
      $candidate_data = $this->Iibf_bcbf_model->validation_check_candidate_eligibility_to_add_in_new_batch($candidate_id, 'aadhar', $aadhar_no,$enc_batch_id);
      if (count($candidate_data) == 0)
      {
        $return_val_ajax = 'true';
      }
  ...
  public function candidates_details($enc_batch_id = '0', $enc_candidate_id = '0')
  {
    if ($enc_batch_id == '0') //FOR ALL CANDIDATE LISTING
    {
      $data['act_id'] = "All Candidates";
    ...
    else
    {
      ...
      if ($enc_batch_id != '0')
      {
        $this->db->where('bc.batch_id', url_decode($enc_batch_id));
      }
      if ($this->login_user_type == 'centre')
      {
        $this->db->where('bc.centre_id', $centre_id);
      }
      ...
      $this->db->join('iibfbcbf_agency_master am', 'am.agency_id = acb.agency_id', 'LEFT');
      $this->db->join('iibfbcbf_centre_master cm', 'cm.centre_id = acb.centre_id', 'LEFT');
      $data['batch_data'] = $batch_data = $this->master_model->getRecords('iibfbcbf_agency_centre_batch acb', array('acb.centre_id' => $form_data[0]['centre_id'], 'acb.batch_id' => $batch_id, 'acb.batch_status' => '3', 'acb.is_deleted' => '0', 'am.is_active' => '1', 'am.is_deleted' => '0', 'cm.is_deleted' => '0'), "acb.*");

      if (count($batch_data) == 0)
      {
        redirect(site_url('iibfbcbf/admin/training_batches_agency'));
      }
      //END : CHECK IF BATCH EXIST OR NOT. ALSO CHECK BATCH STATUS AS GO AHEAD
    } //END : FIND OUT THE CURRENT MODE AND GET THE CANDIDATES DATA
    ...
    $this->load->view('iibfbcbf/admin/candidates_details_admin', $data);
  }
