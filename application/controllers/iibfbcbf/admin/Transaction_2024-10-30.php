  ...
  ********************************************************************************************************************/
  ...
  class Transaction extends CI_Controller 
  {
    ...
    public function get_transaction_data_ajax()
    {
      ...
      $column_order = array('pt.id', 'CONCAT(am.agency_name, " (", am.agency_code, " - ", IF(am.allow_exam_types="Bulk/Individual", "Regular", am.allow_exam_types),")") AS agency_name', 'CONCAT(cm.centre_name," (", cm.centre_username, " - ", cm1.city_name, ")") AS DispCentre', 'IF(pt.payment_mode="Bulk", pt.UTR_no, pt.transaction_no) AS DispTransactionNo', 'pt.receipt_no', 'CONCAT(em.description," (", em.exam_code, ")") AS description', 'pt.amount', 'pt.pay_count', 'IF(pt.payment_mode="Bulk", DATE_FORMAT(pt.date, "%Y-%m-%d"), DATE_FORMAT(pt.date, "%Y-%m-%d %H:%i")) AS PaymentDate', 'pt.exam_code', 'pt.exam_period', '"" AS DispMemberNumbers', '"" AS DispMemberTrainingIds', 'pt.payment_mode', 'IF(pt.status=0, "Fail", IF(pt.status=1, "Success", IF(pt.status=2, "Pending", IF(pt.status=3, "Applied", IF(pt.status=4, "Cancelled", ""))))) AS DispPayStatus', 'pt.status', 'me.regnumber', 'cand.training_id', '(SELECT invoice_id FROM exam_invoice ei WHERE ei.receipt_no = pt.receipt_no AND ei.exam_code = pt.exam_code AND ei.exam_period = pt.exam_period AND ei.app_type = pt.pg_flag AND ei.invoice_image != "" LIMIT 1) AS invoice_id'); //SET COLUMNS FOR SORT
      
      $column_search = array('CONCAT(am.agency_name, " (", am.agency_code, " - ", IF(am.allow_exam_types="Bulk/Individual", "Regular", am.allow_exam_types),")")', 'CONCAT(cm.centre_name," (", cm.centre_username, " - ", cm1.city_name, ")")', 'IF(pt.payment_mode="Bulk", pt.UTR_no, pt.transaction_no)', 'pt.receipt_no', 'CONCAT(em.description," (", em.exam_code, ")")', 'pt.amount', 'pt.pay_count', 'IF(pt.payment_mode="Bulk", DATE_FORMAT(pt.date, "%Y-%m-%d"), DATE_FORMAT(pt.date, "%Y-%m-%d %H:%i"))', 'pt.exam_code', 'pt.exam_period', 'pt.payment_mode', 'IF(pt.status=0, "Fail", IF(pt.status=1, "Success", IF(pt.status=2, "Pending", IF(pt.status=3, "Applied", IF(pt.status=4, "Cancelled", "")))))', 'me.regnumber', 'cand.training_id'); //SET COLUMN FOR SEARCH
      ...
      foreach ($Rows as $Res) 
      {
        ...
        if(!empty($Res['invoice_id']) && $Res['invoice_id'] != "")
        {
