<?php 
  /********************************************************************************************************************
  ** Description: Controller for BCBF CSC CENTRE login page 
  ** Created BY: Sagar Matale On 12-03-2023
  ********************************************************************************************************************/
  defined('BASEPATH') OR exit('No direct script access allowed');
  
  class Login_csc extends CI_Controller 
  {
    ...
    {   
      ...
      $this->Iibf_bcbf_model->check_session_login_csc_centre(); // If session is already started then login page is directly redirect to dashboard
      $data['error'] = "";
      ...
      if(isset($_POST) && count($_POST) > 0)
      {
        ...
        {
          generate_captcha('IIBF_BCBF_LOGIN_CSC_CAPTCHA',6);
          ...
          if($remember_me == '1')
          {            
            setcookie('COOKIE_IIBF_BCBF_CSC_USERNAME',$username, time()+60*60*24*365,'/');
            setcookie('COOKIE_IIBF_BCBF_CSC_PASSWORD',$password, time()+60*60*24*365,'/');
          }
          else
          {            
            setcookie('COOKIE_IIBF_BCBF_CSC_USERNAME',"", time()+60*60*24*365,'/');
            setcookie('COOKIE_IIBF_BCBF_CSC_PASSWORD',"", time()+60*60*24*365,'/');
          }
          ...
          $centre_data = $this->master_model->getRecords('iibfbcbf_centre_master cm', array('cm.centre_username' => $username, 'cm.centre_password' => $enc_password, 'cm.status' => '1', 'cm.is_deleted' => '0', 'am.allow_exam_types' => 'CSC'), 'cm.centre_id, cm.status, cm.is_deleted, am.is_active as AgencyStatus, am.is_deleted AgencyIsDeleted, am.allow_exam_types');
          ...
          if(count($centre_data) > 0)
          {
            ...
            {		
              ...
              $session_data = array('IIBF_BCBF_LOGIN_ID' => $centre_data[0]['centre_id'], 'IIBF_BCBF_USER_TYPE' => 'centre');
              $this->session->set_userdata($session_data);						
              redirect(site_url('iibfbcbf/agency/dashboard_agency'),'refresh');						
            }
          } 
          else { $data['error'] = "Please enter valid centre login details"; }       						
        }			
      }     
      
      $data['COOKIE_IIBF_BCBF_CSC_USERNAME'] = $data['COOKIE_IIBF_BCBF_CSC_PASSWORD'] = '';      
      if(isset($_COOKIE['COOKIE_IIBF_BCBF_CSC_USERNAME']) && $_COOKIE['COOKIE_IIBF_BCBF_CSC_USERNAME'] != "") { $data['COOKIE_IIBF_BCBF_CSC_USERNAME'] = $_COOKIE['COOKIE_IIBF_BCBF_CSC_USERNAME']; }
      if(isset($_COOKIE['COOKIE_IIBF_BCBF_CSC_PASSWORD']) && $_COOKIE['COOKIE_IIBF_BCBF_CSC_PASSWORD'] != "") { $data['COOKIE_IIBF_BCBF_CSC_PASSWORD'] = $_COOKIE['COOKIE_IIBF_BCBF_CSC_PASSWORD']; }

      $data['captcha_img'] = generate_captcha('IIBF_BCBF_LOGIN_CSC_CAPTCHA',6); //iibfbcbf/iibf_bcbf_helper.php
      $data['page_title'] = 'IIBF - BCBF CSC Centre Login';
      $this->load->view('iibfbcbf/login_csc',$data);
    }
    
    public function refresh_captcha() /******** START : REFRESH CAPTCHA ********/
    { 
      $this->load->helper('captcha');
      echo generate_captcha('IIBF_BCBF_LOGIN_CSC_CAPTCHA',6); //iibfbcbf/iibf_bcbf_helper.php
    }  /******** END : REFRESH CAPTCHA ********/
    ...
    public function validation_check_captcha($str='',$type=0)//$type : 0=>Server side, 1=>Ajax
    {
      ...
      if(isset($_POST) && $_POST['iibf_bcbf_captcha'] != "")
      {
        ...
        $session_captcha = $this->session->userdata('IIBF_BCBF_LOGIN_CSC_CAPTCHA');
    ...
    public function validation_check_username($str='',$type=0)//$type : 0=>Server side, 1=>Ajax
    {
      ...
      if(isset($_POST) && $_POST['iibf_bcbf_username'] != "")
      {
        ...
        { 
          ...
          {
            $this->form_validation->set_message('validation_check_username','Please enter the valid center code');
            return false;
          }
        }
      }
    ...
    public function logout()
    {
      ...
      $admin_session_data = array('IIBF_BCBF_LOGIN_ID' => "", 'IIBF_BCBF_USER_TYPE' => "");	 //IIBF_BCBF_USER_TYPE = admin, agency, inspector
      $this->session->set_userdata($admin_session_data);
      redirect(site_url('iibfbcbf/login_csc'),'refresh');
    }	
  }