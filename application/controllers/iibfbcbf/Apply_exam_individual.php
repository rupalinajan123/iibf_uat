            ...
            if($form_total_fees == $exam_fees)
            {
              ...
              if($pt_id > 0)
              {
                ...
                if($exam_fees > 0)
                {
                  ...
                  $customer_name_arr = explode(" ",trim($candidate_data[0]['first_name']));
                  $billdesk_res = $this->billdesk_pg_model->init_payment_request($receipt_no, $exam_fees, $invoice_id, $invoice_id, $customer_name_arr[0], 'iibfbcbf/apply_exam_individual/handle_billdesk_response/'.url_encode($pt_id), '', '', '', $custom_field_billdesk);                  				
