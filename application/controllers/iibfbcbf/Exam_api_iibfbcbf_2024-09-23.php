  ...
  ********************************************************************************************************************/
  ...
  class Exam_api_iibfbcbf extends CI_Controller 
  {
    public function __construct()
    {
      ...
      //1=>Basic, 2=>Advance
      $this->exam_type_arr = array('1037'=>'2', '1038'=>'1', '1041'=>'2', '1042'=>'1');
    }
    ...
    {   
      ...
      { 
        ...
        if(count($exam_code_arr) > 0)
        {
          ...
          {
            if(!in_array($exam_code_res, array(1037,1038,1041,1042)))
            {
        ...
        if($is_valid_exam_code_flag == '0') { echo 'Invalid exam code entered. Valid exam codes are 1037,1038,1041,1042'; exit; } ?>
        <html>
          <head>
            <style>
                      ...
                      <tr class="final_result <?php if($error_cnt > 0) { echo "row_error"; } ?>">
                        <td colspan="8">
                          ...
                          { 
                            ...
                            if($exam_code_val == '1041' || $exam_code_val == '1042')
                            {
                              /* $exam_activation_data = $this->master_model->getRecords('iibfbcbf_exam_activation_master',array('exam_code'=>$exam_code_val),'exam_code, exam_period, exam_from_date, exam_to_date');

                              $this->master_model->updateRecord('iibfbcbf_agency_master', array('allow_exam_codes'=>'', 'exam_period'=>''),  array('allow_exam_types'=>'Hybrid'));  */                             
                            }
                          } 
          ...
          if(is_array($decoded_res) && count($decoded_res) > 0)
          {
            ...
            foreach($decoded_res as $res)
            {
              ...
              if($response_exam_code ==  $exam_code)
              {
                ...
                $add_data = array();
                $add_data['exam_code'] = $response_exam_code;
                $add_data['description'] = $res['exam_desc'];
          ...
          if(is_array($decoded_res) && count($decoded_res) > 0)
          {
            ...
            foreach($decoded_res as $res)
            {
              ...
              if($response_exam_code == $exam_code && $response_exam_period == $exam_period)
              {
                ...
                $add_data = array();
                $add_data['exam_code'] = $response_exam_code;
                $add_data['exam_period'] = $response_exam_period;
                $add_data['part_no'] = $res['prt_NO'];
                $add_data['syllabus_code'] = $res['syl_CD'];
                $add_data['subject_code'] = $res['sub_CD'];
                $add_data['subject_description'] = $res['sub_DSC'];
                $add_data['group_code'] = $res['grp_CD'];
                $add_data['exam_date'] = $res['exm_DT'];
                $add_data['exam_time'] = $res['exm_TIME']."00";
                $insert_id = $this->master_model->insertRecord($tbl_name,$add_data, true);
                if($insert_id > 0)
                {
                  $success_cnt++;                  	
          ...
          if(is_array($decoded_res) && isset($decoded_res[0]) && count($decoded_res[0]) > 0)
          {
            ...
            foreach($decoded_res as $res)
            {
              ...
              if($response_exam_code == $exam_code && $response_exam_period == $exam_period)
              {
                ...
                $add_data['exam_code'] = $response_exam_code;
                $add_data['exam_period'] = $response_exam_period;
                $add_data['exam_month'] = $res['exam_mth'];
                $add_data['trg_value'] = $res['trg_val_upto'];
                $insert_id = $this->master_model->insertRecord($tbl_name,$add_data, true);
                if($insert_id > 0)
                {
                  $success_cnt++;                  	
          ...
          if(is_array($decoded_res) && count($decoded_res) > 0)
          {
            ...
            foreach($decoded_res as $res)
            {
              ...
              if($response_exam_code == $exam_code && $response_exam_period == $exam_period)
              {
                ...
                $add_data['exam_code'] = $response_exam_code;
                $add_data['exam_period'] = $response_exam_period;
                $add_data['part_no'] = $res['part_no'];
                $add_data['syllabus_code'] = $res['syl_cd'];
                $add_data['member_category'] = $res['mem_category'];
                $add_data['group_code'] = $res['grp_cd'];
                $add_data['fee_amount'] = $res['fee_amt'];
                $add_data['sgst_amt'] = $res['sgst_amt'];
                $add_data['cgst_amt'] = $res['cgst_amt'];
                $add_data['igst_amt'] = $res['igst_amt'];
                $add_data['cs_tot'] = $res['cs_tot'];
                $add_data['igst_tot'] = $res['igst_tot'];
                $add_data['fr_date'] = $res['frm_date'];
                $add_data['to_date'] = $res['to_date'];
                $add_data['exempt'] = $res['exempt'];
                $insert_id = $this->master_model->insertRecord($tbl_name,$add_data, true);
                if($insert_id > 0)
                {
                  $success_cnt++;                  	
          ...
          if(is_array($decoded_res) && count($decoded_res) > 0)
          {
            ...
            foreach($decoded_res as $res)
            {
              ...
              if($response_exam_code == $exam_code && $response_exam_period == $exam_period)
              {
                ...
                $add_data = array();
                $add_data['exam_name'] = $response_exam_code;
                $add_data['exam_period'] = $response_exam_period;
                $add_data['centre_code'] = $res['ctr_CD'];
                $add_data['centre_name'] = $res['ctr_NAM'];
                $add_data['state_code'] = $res['ste_CD'];
                $add_data['state_description'] = $res['ste_DSC'];
                $add_data['exammode'] = $res['mode_OF_EXAM'];
                $insert_id = $this->master_model->insertRecord($tbl_name,$add_data, true);
                if($insert_id > 0)
                {
                  $success_cnt++;                  	
          ...
          if(is_array($decoded_res) && count($decoded_res) > 0)
          {
            ...
            foreach($decoded_res as $res)
            {
              ...
              if($response_exam_code == $exam_code && $response_exam_period == $exam_period)
              {
                ...
                $add_data['exam_code'] = $response_exam_code;
                $add_data['exam_period'] = $response_exam_period;
                $add_data['medium_code'] = $res['med_CD'];
                $add_data['medium_description'] = $res['med_DESC'];
                $insert_id = $this->master_model->insertRecord($tbl_name,$add_data, true);
                if($insert_id > 0)
                {
                  $success_cnt++;                  	
          ...
          if(is_array($decoded_res) && count($decoded_res) > 0)
          {
            ...
            foreach($decoded_res as $res)
            {
              ...
              if($response_exam_code == $exam_code && $response_exam_period == $exam_period)
              {
                ...
                $add_data['exam_code'] = $response_exam_code;
                $add_data['exam_period'] = $response_exam_period;
                $add_data['exam_from_date'] = $res['activation_from_date'];
                $add_data['exam_from_time'] = '10:00:00';
                $add_data['exam_to_date'] = $res['activation_to_date'];
    ...
    function inc_delete_existing_master_records($tbl_name='', $field_name='', $response_exam_code='', $exam_period='')
    {
      if($tbl_name != '' && $field_name != '' && $response_exam_code != '' && $exam_period != '')
      {
        ...
        else if(in_array($response_exam_code, array(1041,1042)))
        {
          ...
          if(count($exam_activation_data) > 0)
          {
            ...
            foreach($exam_activation_data as $exam_activation_res)
            {
              ...
              else if($exam_activation_res['exam_to_date'] < date('Y-m-d'))//DELETE EXISTING RECORD IF EXAM ACTIVATION'S 'EXAM TO DATE' IS LESS THAN CURRENT DATE
              {
                $delete_flag = 1;
              }
