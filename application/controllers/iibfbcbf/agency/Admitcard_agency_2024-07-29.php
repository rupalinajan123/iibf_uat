...
/********************************************************************************************************************
 ** Description: Controller for BCBF Agency Admitcard
 ** Created BY: Sagar Matale On 11-12-2023
 ********************************************************************************************************************/
  defined('BASEPATH') OR exit('No direct script access allowed');

class Admitcard_agency extends CI_Controller
{
  ...
  public function index($center_code_enc = '', $exam_code_enc = '', $exam_period_enc = '')
  {
    $data['act_id'] = "Download Admitcard";
    $data['sub_act_id'] = "IIBF BCBF Admitcard";
    ...
    if ($this->login_user_type == 'agency')
    {
      ...
      $this->db->order_by('bm.center_code');
      $data['agency_centre_data'] = $this->master_model->getRecords('iibfbcbf_admit_card_info bm', array('bm.inscd' => $agency_data[0]["agency_code"]), 'bm.center_code, cm.centre_name');
    }


    $inst_code = $this->login_agency_or_centre_id;

    $this->load->view('iibfbcbf/agency/admitcard_agency', $data);
  }
  ...
  public function get_admitcard_data_ajax()
  {
    ...
    $GroupBy = " Group By bc.center_code,bc.exm_cd";
    ...
    {
      $center_data = $this->master_model->getRecords('iibfbcbf_centre_master', array('centre_id' => $this->login_agency_or_centre_id), 'centre_username,agency_id,centre_id');

      $agency_data = $this->master_model->getRecords('iibfbcbf_agency_master', array('agency_id' => $center_data[0]["agency_id"]), 'agency_code');

      $WhereForTotal = "WHERE bc.inscd = '" . $agency_data[0]["agency_code"] . "' AND ibc.centre_id = '" . $center_data[0]["centre_id"] . "' "; //DEFAULT WHERE CONDITION FOR ALL RECORDS 
      $Where = "WHERE bc.inscd = '" . $agency_data[0]["agency_code"] . "' AND ibc.centre_id = '" . $center_data[0]["centre_id"] . "' "; // AND bm.exam_code = '1038'
    }
    ...
    $join_qry .= " LEFT JOIN iibfbcbf_exam_centre_master cm ON cm.centre_code = bc.center_code";
      ...
      $join_qry .= " LEFT JOIN iibfbcbf_batch_candidates ibc ON ibc.regnumber = bc.mem_mem_no";
    }
    ...
    foreach ($Rows as $Res)
    {
      ...
      if ($center_code_enc == "" && $exam_code_enc == "" && $exam_period_enc == "")
      {
        $center_name .= '<a href="' . site_url('iibfbcbf/agency/admitcard_agency/centerwise_admitcard_pdf/' . url_encode($Res['center_code']) . '/' . url_encode($Res['exm_cd']) . '/' . url_encode($Res['exam_period'])) . '" title="Centerwise Admitcard">' . $Res['centre_name'] . '</a>';

        $btn_str .= ' <a target="_blank" href="' . site_url('iibfbcbf/agency/admitcard_agency/download_admit_card_pdf/' . url_encode($Res['center_code']) . '/' . url_encode($Res['exm_cd']) . '/' . url_encode($Res['exam_period'])) . '" class="btn btn-success btn-xs" title="Download"><i class="fa fa-download"></i></a> ';

        }else{
        $center_name .= $Res['centre_name'];

        $btn_str .= ' <a target="_blank" href="' . site_url('iibfbcbf/agency/admitcard_agency/download_admit_card_pdf/' . url_encode($Res['center_code']) . '/' . url_encode($Res['exm_cd']) . '/' . url_encode($Res['exam_period']) . '/' . url_encode($Res['admitcard_id'])) . '" class="btn btn-success btn-xs" title="Download"><i class="fa fa-download"></i></a> ';
      }
      $row[] = $center_name;
      $row[] = $Res['exm_cd'];
      $row[] = $Res['exam_period'];
    ...
    /*if($this->login_user_type == 'agency') 
      { 
        ...
        $this->db->join('iibfbcbf_centre_master cm', 'cm.agency_id = am.agency_id', 'LEFT');
        $this->db->group_by('bm.center_code');
        $this->db->order_by('bm.center_name');
        $data['agency_centre_data'] = $this->master_model->getRecords('iibfbcbf_admitcard_info bm', array('am.agency_id'=>$agency_id), 'bm.center_code, bm.center_name');
      }*/
    $inst_code = $this->login_agency_or_centre_id;
      ...
      foreach ($admit_card_data as $k => $member_result)
      {
        ...
        if ($admitcard_id == "0")
        {
          ...
          $data['exam_result'] = $this->master_model->getRecords('iibfbcbf_exam_master', array('exam_code' => $data['admit_card_data'][0]['exm_cd']), 'description, exam_type, exam_code', '', '0', '1');
                   ...
                   if($cnt > 7) { redirect(site_url('iibfbcbf/agency/admitcard_agency/download_admit_card_pdf/'.$center_code_enc."/".$exam_code_enc."/".$exam_period_enc)); } 

          $pdfFilePath = $data['admit_card_data'][0]['exm_cd'] . "_" . $data['admit_card_data'][0]['exm_prd'] . "_" . $data['admit_card_data'][0]['mem_mem_no'] . ".pdf";
        ...
        else
        {
          ...
          if ($admitcard_id != "0")
          {
            $pdfFilePath_Individual = $data['admit_card_data'][0]['exm_cd'] . "_" . $data['admit_card_data'][0]['exm_prd'] . "_" . $data['admit_card_data'][0]['mem_mem_no'] . ".pdf";
    ...
    else
    {
      ...
      foreach ($member_record as $member_result)
      {
        ...
        //$this->Iibf_bcbf_model->insert_common_log('IIBF BCBF : Agency Generate admit card End', 'iibfbcbf_admitcard_info', $this->db->last_query(), $this->login_agency_or_centre_id,'agency_admit_card_action','The agency Generate admit card End ', json_encode($data));
          ...
          if($cnt > 7) { redirect(site_url('iibfbcbf/agency/admitcard_agency/download_centerwise_pdf/'.$center_code_enc."/".$exam_code_enc."/".$exam_period_enc)); }
