  ...
  ********************************************************************************************************************/
  ...
  class Training_batches_agency extends CI_Controller 
  {
    ...
    {   
      ...
      if($search_batch_current_state == '1') { $data['search_batch_status'] = '3'; }
    ...
    public function get_training_batches_agency_data_ajax()
    {
      ...
      foreach ($Rows as $Res) 
      {
        ...
        if($this->login_user_type == 'agency' && isset($_SESSION['IIBF_BCBF_AGENCY_CODE']) && in_array($_SESSION['IIBF_BCBF_AGENCY_CODE'], array('1019')) && date("Y-m-d") <= date('Y-m-d', strtotime("+2days", strtotime($Res['batch_start_date']))) && $Res['batch_status'] == '3')
        {
          $function_update_candidates = "get_modal_update_batch_number_of_candidates_data('".url_encode($Res['batch_id'])."')";
          $btn_str .= '<a href="javascript:void(0)" onclick="'.$function_update_candidates.'" class="btn btn-primary btn-xs" title="Update Batch Candidate Details"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a> ';
        }
    ...
    {   
      ...
      if(isset($_POST) && count($_POST) > 0)
      {
        ...
        $training_schedule_file_req_validation = '';
        if($mode == 'Add') { $training_schedule_file_flg = 'y'; $training_schedule_file_req_validation = 'required|'; }
        else { if($form_data[0]['training_schedule_file'] == "") { $training_schedule_file_flg = 'y'; $training_schedule_file_req_validation = 'required|'; } }
        ...
        $this->form_validation->set_rules('training_schedule_file', 'training schedule', 'trim|'.$training_schedule_file_req_validation.'callback_fun_validate_file_upload[training_schedule_file|'.$training_schedule_file_flg.'|txt,doc,docx,pdf|5000|training schedule]'); //callback parameter separated by pipe 'input name|required|allowed extension|size in kb|display name'  
        ...
        $this->form_validation->set_rules('contact_person_email', 'batch coordinator email', 'trim|required|max_length[80]|valid_email|xss_clean', array('required' => "Please enter the %s"));
        ...
        $this->form_validation->set_rules('alt_contact_person_email', '', 'trim|max_length[80]|valid_email|xss_clean');     
        ...
        if($this->form_validation->run())
        {
          ...
          else if($error_flag == 0)
          {
            ...
            $add_data['contact_person_email'] = $this->input->post('contact_person_email');
            ...
            $add_data['alt_contact_person_email'] = $this->input->post('alt_contact_person_email');
      ...
      $data['medium_master'] = $this->master_model->getRecords('iibfbcbf_exam_medium_master', array('medium_delete'=>'0'));
    ...
    {      
      ...
      if($chk_faculty_id != "")
      {
        ...
        {
          $this->db->where_not_in('batch_status', array(5,7));
          ...
          $faculty_data = $this->master_model->getRecords('iibfbcbf_agency_centre_batch', array('is_deleted' => '0', 'batch_id !=' => $batch_id), 'batch_id, agency_id, batch_code, first_faculty, second_faculty, third_faculty, fourth_faculty');
    ...
    {   
      ...
      else if(isset($_POST) && count($_POST) > 0 && isset($_POST["form_action"]) && $_POST['form_action'] == 'batch_status_action' && $this->login_user_type == 'agency') //START : SUBMIT BATCH STATUS
      {
        if($form_data[0]['batch_end_date'] < date('Y-m-d'))
        {
          ...
          redirect(site_url('iibfbcbf/agency/training_batches_agency/training_batch_details_agency/'.$enc_batch_id)); exit;
        }
      ...
      else if(isset($_POST) && count($_POST) > 0 && isset($_POST['form_action']) && $_POST['form_action'] == 'extend_date_action' && $this->login_user_type == 'agency' && date('Y-m-d') <= $form_data[0]['batch_end_date'] && $form_data[0]['batch_status'] == '3')//START : EXTEND DATE FOR ADD/UPDATE CANDIDATES
      {
        if(in_array($form_data[0]['agency_code'], array(1001,1002,1003,1010,1019)))
        {
          ...
          {          
            ...
            $AgencyName = $this->Iibf_bcbf_model->getLoggedInUserDetails($form_data[0]['agency_id'], 'agency'); 
            ...
            $up_data['updated_by'] = $form_data[0]['agency_id'];
            ...
            $this->Iibf_bcbf_model->insert_common_log('Agency : Extend date for add/update candidate', 'iibfbcbf_agency_centre_batch', $this->db->last_query(), $batch_id,'batch_action','Batch '.$log_text.' date successfully extended to '.$batch_extend_date.' by the agency '.$AgencyName['disp_name'], $posted_arr);
    ...
    function fun_check_calculated_readonly_values($str, $input_id='') // Custom callback function for check valid pan number
    {
      if($str != '')
      {
        ...
        if($input_id == 'batch_start_date')
        {
          ...
          $batch_chk_date_end = date('Y-m-d', strtotime("+90day", strtotime($batch_chk_date)));
    ...
    /******** START : GET MODAL FORM TO UPDATE THE BATCH NUMBER OF CANDIDATES ********/
    function get_modal_update_batch_number_of_candidates_data()
		{
			$data['enc_batch_id'] = $enc_batch_id = $this->input->post('enc_batch_id');

      if($enc_batch_id == "0")
			{
				echo "error";
			}
			else if($this->login_user_type == 'agency' && isset($_SESSION['IIBF_BCBF_AGENCY_CODE']) && in_array($_SESSION['IIBF_BCBF_AGENCY_CODE'], array('1019')))
			{
				$batch_id = url_decode($enc_batch_id);
        $agency_id = $this->login_agency_or_centre_id;
        $result_data = $this->master_model->getRecords('iibfbcbf_agency_centre_batch btch', array('btch.agency_id'=>$agency_id, 'btch.batch_id'=>$batch_id, 'btch.is_deleted'=>'0', 'btch.batch_status' => '3', 'btch.batch_end_date >=' => date("Y-m-d")));
        ...
        if(count($result_data) == 0)
				...
				{
          $this->db->group_by('cand.qualification');
          $candidate_data = $this->master_model->getRecords('iibfbcbf_batch_candidates cand', array('cand.agency_id'=>$agency_id, 'cand.batch_id'=>$batch_id, 'cand.is_deleted'=>'0'), 'count(cand.qualification) AS cnt, cand.qualification');

          $batch_total_ug_candidates = $batch_total_g_candidates = $batch_total_pg_candidates = 0;
          if(count($candidate_data) > 0)
          {
            if(isset($candidate_data[0]['qualification']) && $candidate_data[0]['qualification'] == '1')//Under Graduate
            {
              $batch_total_ug_candidates = $candidate_data[0]['cnt'];
            }

            if(isset($candidate_data[0]['qualification']) && $candidate_data[0]['qualification'] == '2')//Graduate
            {
              $batch_total_g_candidates = $candidate_data[0]['cnt'];
            }

            if(isset($candidate_data[0]['qualification']) && $candidate_data[0]['qualification'] == '3')//Post Graduate
            {
              $batch_total_pg_candidates = $candidate_data[0]['cnt'];
            }
          }
          
					$data['form_data'] = $result_data;
          $data['chk_total_batch_candidates'] = $this->chk_total_batch_candidates;
          $data['batch_total_ug_candidates'] = $batch_total_ug_candidates;
          $data['batch_total_g_candidates'] = $batch_total_g_candidates;
          $data['batch_total_pg_candidates'] = $batch_total_pg_candidates;
          $this->load->view('iibfbcbf/agency/modal_update_batch_number_of_candidates_agency', $data);
				}
			}
      else
      {
		...
		}/******** END : GET MODAL FORM TO UPDATE THE BATCH NUMBER OF CANDIDATES ********/

    /******** START : UPDATE THE BATCH NUMBER OF CANDIDATES ********/
    public function update_batch_number_of_candidates($enc_batch_id=0)
    {   
      if($this->login_user_type == 'agency' && isset($_SESSION['IIBF_BCBF_AGENCY_CODE']) && in_array($_SESSION['IIBF_BCBF_AGENCY_CODE'], array('1019')))
      {
        $agency_id = $this->login_agency_or_centre_id;           
        $data['chk_total_batch_candidates'] = $this->chk_total_batch_candidates;           
        ...
        $batch_data = $this->master_model->getRecords('iibfbcbf_agency_centre_batch acb', array('acb.agency_id'=>$agency_id, 'acb.batch_id'=>$batch_id, 'acb.is_deleted'=>'0', 'acb.batch_status' => '3', 'acb.batch_end_date >=' => date("Y-m-d")), "acb.*");        
        if(count($batch_data) == 0) 
        { 
          $this->session->set_flashdata('error','You can not update the number of candidates in this Training Batch');
          redirect(site_url('iibfbcbf/agency/training_batches_agency'));
        }
        ...
        if(isset($_POST) && count($_POST) > 0)
        {
          ...
          if($this->form_validation->run())
          {
            ...
            $posted_arr = json_encode($_POST);
            $AgencyName = $this->Iibf_bcbf_model->getLoggedInUserDetails($agency_id, 'agency');
            ...
            $this->Iibf_bcbf_model->insert_common_log('Centre : Batch Updated', 'iibfbcbf_agency_centre_batch', $this->db->last_query(), $batch_id,'batch_action','The number of candidates in Training Batch successfully updated by the agency '.$AgencyName['disp_name'], $posted_arr);

            $this->session->set_flashdata('success','The number of candidates in Training Batch '.$batch_data[0]['batch_code'].' successfully updated.');              
            redirect(site_url('iibfbcbf/agency/training_batches_agency'));
          }
          else
          {
      ...
      {
        $this->session->set_flashdata('error','You do not have permission to update the number of candidates in Training Batch');
        redirect(site_url('iibfbcbf/agency/training_batches_agency'));
      }
    }/******** END : UPDATE THE BATCH NUMBER OF CANDIDATES ********/
    ...
    function clear_extended_date($enc_batch_id=0)
    {
      ...
      $posted_arr = json_encode($_POST);
      $centreName = $this->Iibf_bcbf_model->getLoggedInUserDetails($this->login_agency_or_centre_id, 'agency'); 
      ...
      $this->Iibf_bcbf_model->insert_common_log('Agency : Cleared extended date for add/update candidate', 'iibfbcbf_agency_centre_batch', $this->db->last_query(), $batch_id,'batch_action','Batch Add/Update candidate date successfully cleared by the agency '.$centreName['disp_name'], $posted_arr);
