  ...
  class Transaction_details_agency extends CI_Controller 
  {
    ...
    {   
      ...
      if(isset($_POST) && count($_POST) > 0 && $this->login_user_type == 'centre')
        ...
        $this->form_validation->set_rules('utr_slip', 'payment (UTR) slip', 'callback_fun_validate_file_upload[utr_slip|y|jpg,jpeg,png|100|payment (UTR) slip]'); //callback parameter separated by pipe 'input name|required|allowed extension|size in kb' 
        ...
        { 
          if(in_array($active_exam_data[0]['exam_code'],$this->venue_master_eligible_exam_codes_arr))//VENUE MASTER IS ONLY APPLICABLE FOR EXAM CODE 1041 & 1042. HENCE THE CAPACITY CHECK IS APPLY FOR ONLY 1041 & 1042. 1041 & 1042 ARE HYBRID MODE EXAMS
          {
            foreach ($candidate_data as $candidate_res)
            {
              ...
              $chk_capacity = $this->Iibf_bcbf_model->get_capacity_bulk($active_exam_data[0]['exam_code'], $active_exam_data[0]['exam_period'], $candidate_res['exam_centre_code'], $candidate_res['exam_venue_code'], $candidate_res['exam_date'], $candidate_res['exam_time'], $candidate_res['member_exam_id']);
