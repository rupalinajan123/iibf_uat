  ...
  ********************************************************************************************************************/
  ...
  class Batch_candidates_agency extends CI_Controller 
  {
    ...
    {   
      ...
      echo $data['chk_batch_start_date'] = $chk_batch_start_date = get_add_candidate_date($this->Iibf_bcbf_model->extend_candidate_add_update_date_arr(), $batch_id, $batch_data[0]['batch_start_date']);//ADDED BY SAGAR TO ALLOW AGENCY TO ADD/EDIT CANDIDATE AFTER START DATE IS OVER  //iibfbcbf/iibf_bcbf_helper.php
      ...
      if(isset($_POST) && count($_POST) > 0)
      {
        ...
        { 
          ...
          {
            if(isset($_POST['id_proof_file_old']) && $_POST['id_proof_file_old'] != "") { $id_proof_file_req_flg = 'n'; }
            if(isset($_POST['qualification_certificate_file_old']) && $_POST['qualification_certificate_file_old'] != "") { $qualification_certificate_file_req_flg = 'n'; }
            if(isset($_POST['candidate_photo_old']) && $_POST['candidate_photo_old'] != "") { $candidate_photo_req_flg = 'n'; }
            if(isset($_POST['candidate_sign_old']) && $_POST['candidate_sign_old'] != "") { $candidate_sign_req_flg = 'n'; }
          }
        }
        else 
        { 
        ...
        if($chk_batch_start_date >= date("Y-m-d"))
        {
          ...
          $this->form_validation->set_rules('first_name', 'first name', 'trim|required|callback_fun_restrict_input[allow_only_alphabets]|max_length[20]|xss_clean', array('required'=>"Please enter the %s"));     
          $this->form_validation->set_rules('middle_name', 'middle name', 'trim|callback_fun_restrict_input[allow_only_alphabets]|max_length[20]|xss_clean');     
          $this->form_validation->set_rules('last_name', 'last name', 'trim|callback_fun_restrict_input[allow_only_alphabets]|max_length[20]|xss_clean');
        ...
        if($validate_form_type == 'full') //THIS FIELDS VALIDATE ONLY IF USER CLICK ON SUBMIT II BUTTON
        {
          ...
          $this->form_validation->set_rules('district', 'district', 'trim|required|callback_fun_restrict_input[allow_only_alphabets_and_space]|max_length[30]|xss_clean', array('required'=>"Please enter the %s"));
          ...
          $this->form_validation->set_rules('id_proof_file', 'proof of identity', 'callback_fun_validate_file_upload[id_proof_file|'.$id_proof_file_req_flg.'|jpg,jpeg,png|25|proof of identity]'); //callback parameter separated by pipe 'input name|required|allowed extension|size in kb|display name'
          ...
          $this->form_validation->set_rules('qualification_certificate_file', 'qualification certificate', 'callback_fun_validate_file_upload[qualification_certificate_file|'.$qualification_certificate_file_req_flg.'|jpg,jpeg,png|100|qualification certificate]'); //callback parameter separated by pipe 'input name|required|allowed extension|size in kb|display name'  
          $this->form_validation->set_rules('candidate_photo', 'passport-size photo of the candidate', 'callback_fun_validate_file_upload[candidate_photo|'.$candidate_photo_req_flg.'|jpg,jpeg,png|20|passport photo of the candidate]'); //callback parameter separated by pipe 'input name|required|allowed extension|size in kb|display name'  
          $this->form_validation->set_rules('candidate_sign', 'signature of the candidate', 'callback_fun_validate_file_upload[candidate_sign|'.$candidate_sign_req_flg.'|jpg,jpeg,png|20|signature of the candidate]'); //callback parameter separated by pipe 'input name|required|allowed extension|size in kb|display name'        
        ...
        if($this->form_validation->run())
        {
          ...
          if($validate_form_type == 'full') //THIS FIELDS VALIDATE ONLY IF USER CLICK ON SUBMIT II BUTTON
          {
            if($_FILES['id_proof_file']['name'] != "")
            {
              ...
              $upload_data1 = $this->Iibf_bcbf_model->upload_file("id_proof_file", array('jpg','jpeg','png'), $new_file_name1, "./".$id_proof_file_path, "jpg|jpeg|png", '', '', '', '', '25','1000','500',$new_file_name1);
            ...
            {
              $add_data['qualification_certificate_file'] = $qualification_certificate_file = $new_qualification_certificate_file = basename($this->security->xss_clean($this->input->post('qualification_certificate_file_old')));
            }
            ...
            {
              $add_data['candidate_sign'] = $candidate_sign = $new_candidate_sign = basename($this->security->xss_clean($this->input->post('candidate_sign_old')));
          ...
          else if($error_flag == 0)
          {
            ...
            else if($mode == "Update")
            {
              ...
              if($new_id_proof_file != '') { @unlink("./".$id_proof_file_path."/".$form_data[0]['id_proof_file']); }
              if($new_qualification_certificate_file != '') { @unlink("./".$qualification_certificate_file_path."/".$form_data[0]['qualification_certificate_file']); }
              if($new_candidate_photo != '') { @unlink("./".$candidate_photo_path."/".$form_data[0]['candidate_photo']); }
              if($new_candidate_sign != '') { @unlink("./".$candidate_sign_path."/".$form_data[0]['candidate_sign']); }
