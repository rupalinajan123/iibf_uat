  ...
  ********************************************************************************************************************/
  ...
  class Apply_exam_agency extends CI_Controller 
  {
    ...
    public function get_exam_candidates_agency_data_ajax()
    {
      ...
      $todays_date = date("Y-m-d"); //'2024-10-16';//
      
      $inc_where_cond = "DATE_ADD(btch.batch_end_date, INTERVAL ".$this->buffer_days_after_training_end_date." DAY) < '".$todays_date."'";
      if(isset($_SESSION['IIBF_BCBF_AGENCY_CODE']) && in_array($_SESSION['IIBF_BCBF_AGENCY_CODE'], array('1019')))
      {
        $inc_where_cond = " (".$inc_where_cond." OR DATE_ADD(btch.batch_end_date, INTERVAL -1 DAY) = '".$todays_date."') ";
      }
      ...
      cand.candidate_id NOT IN (SELECT candidate_id FROM iibfbcbf_member_exam WHERE exam_date >= '".date("Y-m-d")."' AND exam_period IN (1, ".$active_exam_period.") AND CASE WHEN exam_code = '".$exam_code."' THEN pay_status IN (1) WHEN exam_code > 0 AND exam_code != '".$exam_code."' THEN pay_status IN (1,2) END)"; //DEFAULT WHERE CONDITION FOR ALL RECORDS
      ...
      $join_qry .= " LEFT JOIN iibfbcbf_exam_centre_master ex_cm ON ex_cm.centre_code = mem_ex.exam_centre_code AND ex_cm.exam_name = '".$exam_code."' AND ex_cm.exam_period = '".$active_exam_period."'";    
      $join_qry .= " LEFT JOIN iibfbcbf_exam_medium_master ex_mm ON ex_mm.medium_code = mem_ex.exam_medium AND ex_mm.exam_code = '".$exam_code."' AND ex_mm.exam_period = '".$active_exam_period."'";    
    ...
    {      
      ...
      $data['subject_master'] = $this->master_model->getRecords('iibfbcbf_exam_subject_master',array('exam_code'=>$active_exam_data[0]['exam_code'], 'exam_period'=> $active_exam_data[0]['exam_period'], 'subject_delete'=>'0','group_code'=>'C'),'',array('subject_code'=>'ASC'));

      $this->db->join('iibfbcbf_exam_activation_master eam','eam.exam_code = ecm.exam_name AND eam.exam_period = ecm.exam_period');
      $data['centre_master'] = $this->master_model->getRecords('iibfbcbf_exam_centre_master ecm',array('ecm.exam_name'=>$active_exam_data[0]['exam_code'], 'ecm.exam_period'=>$active_exam_data[0]['exam_period'], 'eam.exam_period'=>$active_exam_data[0]['exam_period'], 'eam.exam_activation_delete'=>'0', 'ecm.centre_delete'=>'0'),'',array('ecm.centre_name'=>'ASC'));
            
      $this->db->join('iibfbcbf_exam_activation_master eam','eam.exam_code = emm.exam_code AND eam.exam_period = emm.exam_period');
      $data['medium_master'] = $this->master_model->getRecords('iibfbcbf_exam_medium_master emm',array('emm.exam_code'=>$active_exam_data[0]['exam_code'], 'emm.exam_period'=>$active_exam_data[0]['exam_period'], 'eam.exam_period'=>$active_exam_data[0]['exam_period'], 'eam.exam_activation_delete'=>'0', 'emm.medium_delete'=>'0'),'',array('emm.medium_description'=>'ASC'));
      ...
      ////echo '<div style="display:none">'; _pq(); echo '</div>'; 
      ...
      if(isset($_POST) && count($_POST) > 0)
        ...
        if(in_array($exam_code, $this->venue_master_eligible_exam_codes_arr)) //VENUE MASTER IS ONLY APPLICABLE FOR EXAM CODE 1041, 1042, 1057. HENCE THE CAPACITY CHECK IS APPLY FOR ONLY 1041, 1042, 1057. 1041 & 1042 ARE HYBRID MODE EXAMS. 1057 IS BCBF NAR HYBRID EXAM
        {
          ...
          if(count($get_exam_fee_data) > 0)
          {
           ...
           /*  _pa($_POST);
            ...
            if(count($applied_exam_data) == 0) //FOR ADD MODE
            {
              ...
              $this->Iibf_bcbf_model->insert_common_log('Centre : Candidate Applied for exam', 'iibfbcbf_batch_candidates', $this->db->last_query(), $candidate_data[0]['candidate_id'],'candidate_action','The candidate is applying for the exam ('.$exam_code.' & '.$active_exam_data[0]['exam_period'].') by centre '.$centreName['disp_name'], $posted_arr);
    ...
    function make_payment($enc_exam_code='0')
    {
      ...
      if(isset($_POST) && count($_POST) > 0)
        ...
        {
          if(in_array($active_exam_data[0]['exam_code'], $this->venue_master_eligible_exam_codes_arr)) //VENUE MASTER IS ONLY APPLICABLE FOR EXAM CODE 1041, 1042, 1057. HENCE THE CAPACITY CHECK IS APPLY FOR ONLY 1041, 1042, 1057. 1041 & 1042 ARE HYBRID MODE EXAMS. 1057 IS BCBF NAR HYBRID EXAM
          {
            ...
            if(!array_key_exists($array_key, $capacity_chk_arr))
            {
              ...
              $capacity_chk_arr[$array_key]['available_capacity'] = $this->Iibf_bcbf_model->get_capacity_bulk($active_exam_data[0]['exam_code'], $active_exam_data[0]['exam_period'], $candidate_res['exam_centre_code'], $candidate_res['exam_venue_code'], $candidate_res['exam_date'], $candidate_res['exam_time'], '', 'make_proforma_invoice');     
        ...
        if($this->form_validation->run() == TRUE)
        {
          ...
          else if($chk_form_type == 'make_payment')
          {
            ...
            if($agency_centre_data[0]['invoice_address'] == '1')
            {
              $state_code = $agency_centre_data[0]['agency_state']; //agency state code
            }
            else
            {
            ...
            if($form_selcted_member_exam_ids_str == $selcted_member_exam_ids_str && $form_total_fees == $calculated_total_fee && $form_exam_code == $active_exam_data[0]['exam_code'] && $form_exam_period == $active_exam_data[0]['exam_period'] && $form_exam_from_date == $active_exam_data[0]['exam_from_date'] && $form_exam_to_date == $active_exam_data[0]['exam_to_date'])
            {
              ...
              if($pt_id > 0)
              {
                if(count($selcted_member_exam_ids_arr) > 0)
                {
                  foreach($selcted_member_exam_ids_arr as $selcted_member_exam_id_res)
                  {
                    $this->master_model->updateRecord('iibfbcbf_member_exam ', array('pt_id'=>$pt_id), array('member_exam_id' => $selcted_member_exam_id_res));//UPDATE PT ID IN iibfbcbf_member_exam
                  }
                }
                ...
                foreach ($candidate_data as $candidate_res)
                {
                  ...
                  $up_exam_arr['regnumber'] = $candidate_res['regnumber'];
                  ...
                  $this->Iibf_bcbf_model->insert_common_log('Candidate : Candidate Proforma Invoice Generated', 'iibfbcbf_batch_candidates', '', $candidate_res['candidate_id'],'candidate_action','The '.$centreName['disp_name'].' successfully generated the proforma invoice for the exam ('.$active_exam_data[0]['exam_code'].' & '.$active_exam_data[0]['exam_period'].').', '');
                ...
                if($calculated_total_fee > 0) 
                {
                  ...
                  //FOR INDIVIDUAL & CSC : center_code, center_name, state_of_center : THESE VALUE SHOULD BE APPLIED EXAM CENTER CODE, EXAM CENTER NAME & EXAM CENTER STATE
                  //FOR BULK : center_code, center_name, state_of_center : THESE VALUE SHOULD BE LOGGED IN CENTER CODE, CENTER NAME & CENTER STATE
                  $add_invoice['center_code'] = $agency_centre_data[0]['centre_id'];
                  $add_invoice['center_name'] = $agency_centre_data[0]['centre_name'];
                  $add_invoice['state_of_center'] = $agency_centre_data[0]['centre_state'];
                  ...
                  if($agency_centre_data[0]['invoice_address'] == '1')//Agency state code & state name 
                  {
                    $add_invoice['state_code'] = $agency_centre_data[0]['AgencyStateCode'];
                    $add_invoice['state_name'] = $agency_centre_data[0]['AgencyStateName'];
                  }
                  else
                  {
    ...
    /******** START : TO GET THE VENUE DETAILS FOR SELECTED CENTRE CODE AND VALID FOR EXAM CODE 1041,1042,1057 ********/
    function get_venue_details_ajax()
    {
        ...
        if(count($venue_data) > 0 && in_array($exam_code, $this->venue_master_eligible_exam_codes_arr)) //VENUE MASTER IS ONLY APPLICABLE FOR EXAM CODE 1041, 1042, 1057. HENCE THE CAPACITY CHECK IS APPLY FOR ONLY 1041, 1042, 1057. 1041 & 1042 ARE HYBRID MODE EXAMS. 1057 IS BCBF NAR HYBRID EXAM
				{
    ...
    }/******** END : TO GET THE VENUE DETAILS FOR SELECTED CENTRE CODE AND VALID FOR EXAM CODE 1041,1042,1057 ********/

    /******** START : TO GET THE VENUE DATE DETAILS FOR SELECTED VENUE CODE AND VALID FOR EXAM CODE 1041,1042,1057 ********/
    function get_venue_date_details_ajax()
    {
      ...
      echo json_encode($result);
    }/******** END : TO GET THE VENUE DATE DETAILS FOR SELECTED VENUE CODE AND VALID FOR EXAM CODE 1041,1042,1057 ********/

    /******** START : TO GET THE VENUE TIME DETAILS FOR SELECTED VENUE CODE AND VALID FOR EXAM CODE 1041,1042,1057 ********/
    function get_venue_time_details_ajax()
    {
        ...
        $this->db->order_by('STR_TO_DATE(session_time, "%l:%i %p")', 'ASC', FALSE);
        $this->db->where(" FIND_IN_SET('".$exam_code."',exam_codes) > 0 ");
      ...
      echo json_encode($result);
    }/******** END : TO GET THE VENUE TIME DETAILS FOR SELECTED VENUE CODE AND VALID FOR EXAM CODE 1041,1042,1057 ********/
