  ...
  ********************************************************************************************************************/
  ...
  class Apply_exam_csc_agency extends CI_Controller 
  {
    ...
    {      
      ...
      if(isset($_POST) && count($_POST) > 0)
			{	 
        if(count($resData) > 0 && date('Y-m-d', strtotime("+".$this->buffer_days_after_candidate_add_date."days", strtotime($resData['result_data'][0]['batch_end_date']))) >= $_POST['exam_date'])
        {

        }
        else {  
        //$this->session->set_flashdata('error','The candidate is not eligible to apply for selected exam date as its '.$this->buffer_days_after_candidate_add_date.' days is over');
        $this->session->set_flashdata('error','The candidate is not eligible to apply for exam as the 9 month period since training completion is over');
          redirect(site_url('iibfbcbf/agency/apply_exam_csc_agency/apply_exam_candidate/'.$enc_exam_code.'/'.$enc_candidate_id));
        }
        /*_pa($_POST);
        _pa($resData,1);*/
        ...
        if($this->form_validation->run() == TRUE)  
          ...
          $venue_code = $this->security->xss_clean($this->input->post('venue_name'));
            ...
            if(count($get_exam_fee_data) > 0)
            {
              ...
              if($form_total_fees == $exam_fees)
              {
                ...
                if($pt_id > 0)
                {
                  ...
                  $receipt_no = $this->master_model->generate_csc_receipt_number(); //rand(22222222, 99999999);
          ...
          {
            $data['error'] = 'The venue is closed on selected exam date '.$exam_date.'. Kindy select the different exam date.';
          }            
				}
			}
    ...
    function get_csc_venue_date_details_ajax()
    {
        ...
        if(count($venue_data) > 0 && in_array($exam_code, $this->csc_venue_master_eligible_exam_codes_arr)) //VENUE MASTER IS ONLY APPLICABLE FOR EXAM CODE 1041, 1042, 1057. HENCE THE CAPACITY CHECK IS APPLY FOR ONLY 1041, 1042, 1057. 1041 & 1042 ARE HYBRID MODE EXAMS. 1057 IS BCBF NAR HYBRID EXAM
				{
					$html .= '	<option value="">Select Exam Date</option>';
					foreach($venue_data as $venue_res)
					{
            $disabled_flag = 0;
            if(isset($disabledDates['disabledDates']) && count($disabledDates['disabledDates']) > 0 &&  in_array($venue_res['exam_date'], $disabledDates['disabledDates'])) { $disabled_flag = 1; }

            if(isset($disabledDates['weeklyOff']) && $disabledDates['weeklyOff'] != '' &&  strtolower(date('l', strtotime($venue_res['exam_date']))) == strtolower($disabledDates['weeklyOff'])) { $disabled_flag = 1; }            
            ...
            {
              $html .= '	<option value="" '.$selected.' disabled>'.$venue_res['exam_date'].' (Closed)</option>';
            }
          }
        }
				else
    ...
    function get_csc_venue_time_details_ajax()
    {
        ...
        $venue_data = $this->master_model->getRecords('iibfbcbf_exam_csc_exam_dates', array('exam_date' => $exam_date,'exam_date >'=>date('Y-m-d', strtotime(' + 6 days'))), 'session_time');
