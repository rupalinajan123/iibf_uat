  ...
  class Batch_candidates_agency extends CI_Controller
  {
    ...
    function save_image()
    {
      ...
      if (isset($_POST) && count($_POST) > 0)
      {
        ...
        //$db_col_name = trim($this->security->xss_clean($this->input->post('db_col_name')));
        ...
        else
        {
          ...
          $form_data = $this->master_model->getRecords('iibfbcbf_batch_candidates bc', array('bc.centre_id' => $this->login_agency_or_centre_id, 'bc.candidate_id' => $candidate_id, 'bc.batch_id' => $batch_id, 'bc.is_deleted' => '0'), "bc.regnumber");
        }
        /* _pa($_POST);
        _pa($_FILES); */
        ...
        if ($_FILES['selected_image']['name'] != "")
        {
          ...
          $id_proof_file_path = $this->id_proof_file_path;
          $qualification_certificate_file_path = $this->qualification_certificate_file_path;
          $candidate_photo_path = $this->candidate_photo_path;
          $candidate_sign_path = $this->candidate_sign_path;
          ...
          //$input_name, $valid_arr, $new_file_name, $upload_path, $allowed_types, $is_multiple=0, $cnt='', $height=0, $width=0, $size=0,$resize_width=0,$resize_height=0,$resize_file_name
          $upload_data = $this->Iibf_bcbf_model->upload_file("selected_image", array('jpg', 'jpeg', 'png'), $new_file_name, "./" . $upload_path, "jpg|jpeg|png", '', '', '', '', '', $resize_width, $resize_height, $new_file_name);
          ...
          {
            $flag = 'success';
            $final_image = $upload_path . '/' . $upload_data['message'];
            ...
            $this->compressImage($sourceImagePath, $destinationImagePath, $maxFileSize);
            
            /* if ($this->compressImage($sourceImagePath, $destinationImagePath)) {
              echo 'Image compressed and saved as: ' . $destinationImagePath;
              } else {
              echo 'Failed to compress the image within the size limit.';
            } */
            
            $response_msg = base_url($upload_path . '/' . $upload_data['message']);
          }
        }
      }
    ...
    function compressImage($source, $destination, $maxFileSize = 20480)
    {
      ...
      {
        ob_start();
        // Save the image to the output buffer with the current quality setting
        switch ($type)
        {
          case IMAGETYPE_JPEG:
          ...
          break;
          case IMAGETYPE_PNG:
          imagepng($sourceImage, null, round($quality / 10));
          break;
          case IMAGETYPE_GIF:
          imagegif($sourceImage, null);
          break;
        }
