...
class CronJBIMS extends CI_Controller {
	public function __construct()
	{
		parent::__construct();
		$this->load->library('upload');	
		$this->load->helper('upload_helper');
		$this->load->helper('amp_invoice_helper');
	...
	{
	die;
		$filehandle = fopen("cs2s_log/CronJBIMS/lock.txt", "c+");
		if (flock($filehandle, LOCK_EX | LOCK_NB)) {
			...
			$query='SELECT * FROM JBIMS_payment_transaction WHERE receipt_no = "4000572" AND gateway = "sbiepay" AND status = 2';
			...
			if ($crnt_day_txn_qry->num_rows())
			{
				...
				$dir = 'cs2s_log/CronJBIMS/'.$todays_date;
				if(!is_dir($dir)){
					mkdir($dir, 0755);
				}
				$cell = 1;
						...
						if ($responsedata[12] == "iibfJBIMS")
						{
							...
							if($payment_status==1)
							{
								...
								if($get_user_regnum_info[0]['status']==2)
								{
									...
									if($get_user_regnum_info[0]['payment_option']==4)
									{
            							...
            							$update_mem_data = array('isactive' =>'1','regnumber'=>$applicationNo,'payment' =>$payment_option);
										...
										$log_title ="JBIMS candidates";
										$update_info['membershipno'] = $reg_id;
										...
										//$this->send_sms($applicationNo);
											...
											if($attachpath!='')
											{
												...
												if($this->Emailsending->mailsend_attch($info_arr,$attachpath))
												{
													...
													{
														$emailerSelfStr=$this->master_model->getRecords('emailer',array('emailer_name'=>'JBIMS_emailer_self'));
														if(count($emailerSelfStr) > 0){
															...
															$selfstr31 = str_replace("#gender#", "".$user_info[0]['gender']."",  $selfstr30);
															$final_selfstr = str_replace("#sponsor#", "".$sponsor."",  $selfstr31);
														...
														//'to'=>'skdatta@iibf.org.in,kavan@iibf.org.in,prabhakara@iibf.org.in',
															...
															$this->Emailsending->mailsend_attch($self_mail_arr,$attachment);
														}
													}
												...
												$contact_mail_id = $user_info[0]['sponsor_contact_email'];
														...
														if(count($emailerBankStr) > 0){
															...
															$final_bankstr = str_replace("#sponsor_contact_email#", "".$user_info[0]['sponsor_contact_email']."",  $bankstr40);
															
															$bank_mail_arr = array(
															//'to'=>'skdatta@iibf.org.in,kavan@iibf.org.in',
															'to'=>'dd.trg1@iibf.org.in ,iibfdevp@esds.co.in,'.$contact_mail_id.'',
				...
				echo "CronJBIMS is already running";
			}
			fclose($filehandle);
	...
	{	
		...
		if(empty($user_info_details)){
			redirect(base_url().'CronJBIMS/sbicallback');
		}
