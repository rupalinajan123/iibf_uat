...
class Dwnletterayusha extends CI_Controller {
	...
	public function custom_chkinvoice(){  
		...
		$arr = array(901326925,901342333,901342370,901342712,901348079,901348607,901350610,901352819,901361478,901368793,901368797,901370687,901370700,901371043,901371053);   
		for($i=0;$i<=14;$i++){
			echo $path = custom_genarate_exam_invoice($arr[$i]);
			echo "<br/>"; 
		}
		//echo $path = custom_genarate_exam_invoice(901304130); 
	}
	...
	public function custom_bcbf_invoice_pawan(){  
		$arr = array(901330107,901330130);    
		for($i=0;$i<=1;$i++){ 
	...
	public function custom_bcbf_invoice_pooja(){
		$arr = array(901331079,901331095,901331105,901331124,901331129,901331139,901331161,901331185,901331186,901331192,901331206,901331216,901331234,901331235,901331247,);    
		for($i=0;$i<=14;$i++){ 
			echo $path = custom_genarate_bcbfexam_invoice($arr[$i]);
			echo "<br/>"; 
		}
	...
	public function custom_bcbf_invoice_bhushan(){
		$arr = array(901330107,901330130);    
		for($i=0;$i<=1;$i++){ 
			echo $path = custom_genarate_bcbfexam_invoice($arr[$i]);
			echo "<br/>"; 
	...
	public function custom_bcbf_invoice_ayusha(){

		$arr = array(901337838,901337870,901337896,901337902,901337951,901337987,901337997,901338038,901338039,901338052,901338053);       
		for($i=0;$i<=9;$i++){ 
			echo $path = custom_genarate_bcbfexam_invoice($arr[$i]);
			echo "<br/>"; 
		}
	...
	public function admitcard(){
		echo $path = custom_genarate_admitcard_test();
	}  
	...
	public function insert_admitcard_jaiib(){
		...
		$jaiib_member = $this->master_model->getRecords('jaiib_previous_new');
	...
	// function to add offline data in database 
	public function insert_admitcard_caiib(){
		//$member_array = array("500051343");
		//$this->db->where_in('member_number', $member_array);
		//$caiib_member = $this->master_model->getRecords('offline_data_caiib');
		
		$caiib_member = $this->master_model->getRecords('offline_33');
		
		
		
		foreach($caiib_member as $jaiib_member_res){
			...
			$admitcard_insert_array = array(
											...
											'mode'=>'Online',
											'scribe_flag'=>'',
											'vendor_code'=>$jaiib_member_res['vendor_code'],
											'remark'=>2,
											'created_on'=>date('Y-m-d H:i:s'),
											'record_source'=>'Offline'
											);
			$inser_id=$this->master_model->insertRecord('admit_card_details',$admitcard_insert_array);
		}
	}
	...
	public function custom_contact_class_invoice(){
		echo $path = custom_genarate_custom_contact_classes_invoice(125110,4);
	}
...
");
	...
	public function capacity_incrase(){
		...
		$venue_details = $this->master_model->getRecords('quater_817',array('is_done'=>'no'));
		
		foreach($venue_details as $venue_details_rec){
			
			$old_venue_detail = $this->master_model->getRecords('venue_master',array('center_code'=>$venue_details_rec['center_code'],'venue_code'=>$venue_details_rec['venue_code'],'exam_date'=>'2018-02-24'));
			
			foreach($old_venue_detail as $old_venue_detail_rec){
				
				$new_capacity = $venue_details_rec['capacity'];
			...
			$this->master_model->updateRecord('quater_817',$update_data_new,array('center_code'=>$venue_details_rec['center_code'],'venue_code'=>$venue_details_rec['venue_code']));	
								...
								if($chk_two['flag'] == 0){
									...
									if($chk_three['flag'] == 0){ 
										...
										if($chk_four['flag'] == 0){
											...
											if($exam_code == 21 || $exam_code == 60 || $exam_code == 62 || $exam_code == 63 || $exam_code == 64 || $exam_code == 65 || $exam_code == 66 || $exam_code == 67 || $exam_code == 68 || $exam_code == 69 || $exam_code == 70 || $exam_code == 71 || $exam_code == 72){
	...
	public function custome_bulk_invoice(){
		echo ">>". $attach = custome_generate_bulk_examinvoice(2);	
	}
	...
	public function custom_examinvoice_send_mail(){  
	...
	public function admitcard_info_sendmail(){
		$member_array = array(500115960);
		
		
		$sql = $this->master_model->getRecords('admitcard_info','','mem_mem_no,admitcard_image,exm_cd'); 
	...
	public function custom_admitcardpdf_send_mail(){ 
		//$member_array = array(510344764,400021020,801206571,510041861,510317024,510021104,510222382,500013097,510203893,801249027,801228582,510212778,510208953,510386852,500001352,801233660,510058825,510162261,510147065,510358244,510397446,510025309,801249561,510155903,801249604,801249653,510394303,7278374,500083405,510316934,510231883,510286782,510312976,510146479,510369678,510180931,510300624,510185834,510244255,500072798,510318890,510028946,500048683,500063557,510191303,510372251,510329264,510289884,510183645,510021007,801231922,500059317,500098723,500163981,510240711,510044239,510166187,510341252,801251312,500107560,510300447,801251704,510399079,510394395,510398354,801251928,510259182,510282047,510142057,510350544,510223636,510071328,801212931,500184945,510123320,510071282,510229473,500076770,510138824,6986997,510135292,510138934,510222072,510264472,510227128,510225161,510113774,510015282,510280633,510167016,510337220,400109165,510348838,510052638,510315402,510388248,510306686,510181467,510383146,510169187,500035715,500096530,510351765,510143129,500199486,510387853,510056347,5754595,510076964,510363960,510040007,500156979,7527943,500031249,510346159,801209006,500113924,500177734,510027970,5653322,7429325,6589306,7071631,510331562,510218964,510289612,510362366,500208796,510194426,510052368,510361253,510258451,510383830,500209669,500183155,7065562,510217848,510148460,510283226,510292063,510322647,510107269,510163152,510176863,510053766,510389408,500134747,510012076,510163153,510202208,510201915,510029735,500183650,510218456,510332048,510366808,510364557,510172230,500191807,500189860,510128583,500088087,510330088,510380439,510396778,510100191,500132617,500009772,500141394,510046322,510220181,510211114,510141195,510315303,510170094,510203070,510069107,510279758,510146727,510078291,510103782,500189790,510349749,7650320,510013232,510398425,510305058,5653206,510019688,510152395,510217314,510339036);      
		$member_array = array(510376625);   
		$date_array = array('2019-02-23');
		//$center_array = array(306);  
		
		$this->db->distinct('mem_mem_no');  
		$this->db->where('remark',1);
		$this->db->where('exm_prd','809');
		$this->db->where('admitcard_image !=','');
	...
	public function wrong_venue(){ 
		...
		$member_array = array();
		$exam_array = array('60','62','63','64','65','66','67','68','69','70','71','72');
	...
	public function custom_draexaminvoice_newdesign(){ 
		$invoice_id = 1681712;
		//echo $path = custom_genarate_draexam_invoice_new($invoice_id);
		echo $path = custom_genarate_draexam_invoice_new_design_196($invoice_id);
	}
	
	/*public function blended_email()
	{
	
		//$attchpath_pdf = 'https://iibf.esdsconnect.com/uploads/Jammu_and_Kashmir_GST_Payment.pdf';
		$cand_arr = array('7368284','7282930','510098631','510141728','7372222','6404519','510021881','510166683','500149756','510041361','500086515','510137113','510135715','510145968','510055688','510155619','510285622','500037361','510115397','801186360','510032930','7124270','510337132','510100529','500148504');
		if(count($cand_arr) > 0)
		{
			$cnt=1;
			$emailerstr=$this->master_model->getRecords('refund_email',array('emailer_name'=>'blended_email'));
			foreach($cand_arr as $reg_num)
			{
				$email_res = $this->master_model->getRecords('member_registration',array("regnumber"=>$reg_num),'email');
				if(count($email_res)>0)
				{
					//$email_res[0]['email'] anishrivastava@iibf.org.in
					//$files_pdf=array($attchpath_pdf);
					//'to'=>$email_res[0]['email'],
				$info_arr = array('to'=>$email_res[0]['email'],
					//$info_arr = array('to'=>'tpynadath@gmail.com',
					  'from'=>$emailerstr[0]['from'],
					  'subject'=>$emailerstr[0]['subject'],
					  'message'=>$emailerstr[0]['emailer_text']
					);
