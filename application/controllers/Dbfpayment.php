...
class Dbfpayment extends CI_Controller {
	...
	public function make_payment()
	{
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			$pg_return_url = base_url()."Dbfpayment/pg_response";
			...
			if ($res_checksum == $checksum)
			{
				...
				if($pg_res[16] == "iibfexam")
				{
					...
					if($payment_status==1)
					{
						...
						$result=$this->master_model->getRecords('member_registration',array('regid'=>$this->session->userdata('dbregid'),'regnumber'=>$this->session->userdata('dbregnumber')),'firstname,middlename,lastname,address1,address2,address3,address4,district,city,email,mobile,office,pincode,state_master.state_name');
					...
					$exam_info=$this->master_model->getRecords('member_exam',array('regnumber'=>$this->session->userdata('dbregnumber'),'member_exam.id'=>$get_user_regnum[0]['ref_id']),'member_exam.exam_mode,member_exam.exam_medium,member_exam.exam_period,center_master.center_name,member_exam.exam_center_code,exam_master.description,misc_master.exam_month');
						...
						$payment_info=$this->master_model->getRecords('payment_transaction',array('receipt_no'=>$MerchantOrderNo,'member_regnumber'=>$this->session->userdata('dbregnumber')),'transaction_no,date,amount');
						...
						{
							redirect(base_url().'Dbf/details/'.base64_encode($MerchantOrderNo).'/'.$this->session->userdata['examinfo']['excd']);
						}
						}
						else if($payment_status==0)
						{
							...
							$result=$this->master_model->getRecords('member_registration',array('regid'=>$this->session->userdata('dbregid'),'regnumber'=>$this->session->userdata('dbregnumber')),'firstname,middlename,lastname,email,mobile');
