      ...
      {     
        ...
        {          
          ...
          if(count($subject_data) > 0)  
          {
            ...
            if($availeble_subjects_cnt == 0) { $response .= '<label>You have already purchased all the elearning subjects</label>'; }
          }
    ...
    function get_all_subject_fee_arr($subject_code='')
    {
      ...
      $exam_code = $this->session->userdata['session_array_'.$login_type]['exam_name']; 
        ...
        /* $log_title ="E-learning member UPDATE array :".$regid;
			...
			if(count($ChkExist) == 0) { $this->session->set_flashdata('error','Invalid credential'); redirect(site_url('ApplyElearning/logout')); }
      ...
      {				
		...
		public function sbi_make_payment()
		{
			...
			if(count($ChkExist) == 0) 
			{ 
			 	$this->session->set_flashdata('error','Invalid credential'); redirect(site_url('ApplyElearning/logout')); 
			}
        ...
        //echo '<pre>'; print_r($all_subject_fee_arr); echo '</pre>';
