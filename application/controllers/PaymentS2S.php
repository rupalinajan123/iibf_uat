...
class PaymentS2S extends CI_Controller {
...
echo "MerchantOrderNo = ".$MerchantOrderNo; print_r($update_data);
...

	{
		...
		if (flock($filehandle, LOCK_EX | LOCK_NB)) {
			...
			$query='SELECT receipt_no FROM payment_transaction WHERE date <= NOW() - INTERVAL 60 MINUTE 

			AND date > NOW() - INTERVAL 180 MINUTE AND gateway = "sbiepay" AND status = 2';
	...
	$this->Emailsending->mailsend_attch($info_arr,$attachpath);
			...
			//exit;
				...
				else if($responsedata[12] == "iibfmisc")
