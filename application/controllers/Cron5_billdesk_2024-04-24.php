	...
	class Cron5_billdesk extends CI_Controller 
	{
		...
		public function billdesk_callback()
		{
			...
			if (flock($filehandle, LOCK_EX | LOCK_NB)) 
			{
				...
				AND date > NOW() - INTERVAL 120 MINUTE AND gateway = "billdesk" AND status = 2';


				// SELECT receipt_no,date FROM payment_transaction WHERE date <= NOW() - INTERVAL 60 MINUTE AND date > NOW() - INTERVAL 1500 MINUTE AND gateway = "billdesk" AND status = 2 
