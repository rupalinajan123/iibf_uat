<?php
	...
	class Login extends CI_Controller 
	{
		...
		{
			$data['error'] = "";
			if($this->session->userdata('username')) { redirect(site_url('scribe_dashboard/Scribe_list')); }
			...
			{ 
				...
				{
					if($this->input->post('Username') == 'scribeadmin' && $this->input->post('Password') == 'scribeadmin')
					{
						$sessionData['username'] = 'scribeadmin';
						$sessionData['password'] = 'scribeadmin';
						$sessionData['active'] = '1';
						$sessionData['deleted'] = '0';
						$sessionData['user_type'] = 'admin';
						$this->session->set_userdata('sessionData',$sessionData);					
						redirect(site_url('scribe_dashboard/Scribe_list'));
					}	
					else
					{
						...
						$this->session->set_userdata('sessionData',$sessionData);			
						redirect(site_url('scribe_dashboard/Scribe_list/SearchQry'));
			...
			$this->load->view('scribe_dashboard/login',$data);		
		}
		...
		public function Logout()
		{
			...
			redirect(site_url('scribe_dashboard/login'));
		} 
	}	