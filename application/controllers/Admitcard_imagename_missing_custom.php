...
class Admitcard_imagename_missing_custom extends CI_Controller {
	...
	public function settle(){
		...
		$this->db->where("module_type","Admitcard_imagename_missing_custom");
		...
		$activation = $this->master_model->getRecords('exam_activation_master','','exam_period');
	...
	public function settle_image_name(){  
		...
		foreach($admit_card_image_name_missing as $res){
			...
			$this->master_model->updateRecord('admit_card_image_name_missing',$update_admit_settle,array('admitcard_id'=>$res['admitcard_id'])); 
