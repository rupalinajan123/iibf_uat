	...
	class Payment extends CI_Controller 
	{
		...
		public function billdesk_callback() //billdesk_callback : This function is provided to billdesk team for s2s callback
		{
			...
			{	
        		...
        		if (isset($responsedata['additional_info']['additional_info3'])) 
        		{ 
        			$additional_info3 = $responsedata['additional_info']['additional_info3']; 
        		}
		        ...
		        if(count($get_receipt) == 0)
		        {
		          $get_receipt=$this->master_model->getRecords('dra_payment_transaction',array('receipt_no'=>$MerchantOrderNo,'gateway'=>'2'),'pg_flag,receipt_no,date,status');
		        }
		        ...
		        if(count($get_pg_flag) == 0)
		        {
		          $get_pg_flag = $this->master_model->getRecords('dra_payment_transaction',array('receipt_no'=>$MerchantOrderNo),'pg_flag');
		        }
				...
				switch ($auth_status)
				{
					...
					case "0002": $payment_status = 5; break; // pending
				}	

				if ($pay_type == "iibfbulkdra") 
				{
					$payment_status = 5; 
					...
					switch ($auth_status)
					{
						case "0300": $payment_status = 1; $payment_action = "Approved"; break; // success
						case "0399": $payment_status = 0; $payment_action = "Rejected"; break; // failed
						case "0002": $payment_status = 0; $payment_action = "Pending"; break; // pending
					}	
				}
				...
				if($payment_status==1)
				{
					if($pay_type == "iibfregn")
					{
			            ...
			            else if($additional_info3 == 'iibffq')
			            {
							...
							if($get_user_regnum[0]['status']==2)
							{
								...
								if($this->db->affected_rows())
								{
									...
									if($get_payment_status[0]['status']==1)
									{
										...
										$this->db->join('state_master','state_master.state_code=member_registration.state');
										//$this->db->join('institution_master','institution_master.institude_id=member_registration.associatedinstitute');
										$result=$this->master_model->getRecords('member_registration',array('regid'=>$reg_id),'firstname,middlename,lastname,address1,address2,address3,address4,district,city,email,mobile,office,pincode,state_master.state_name,usrpassword,email,scannedphoto,scannedsignaturephoto,idproofphoto,bank_bc_id_card');	
										########get Old image Name############
										$log_title ="Non member OLD Image :".$reg_id;
										$log_message = serialize($result);
										$rId = $reg_id;
				                  	...
				                  	if(count($member_data) > 0)
				                  	{
				                    	...
				                    	if($member_data[0]['regnumber'] == '')
				                    	{
											...
											if(!empty($current_id_proof_file)) 
											{
												...
												{ 
													$up_cand_data['id_proof_file'] = $new_id_proof_file; 
												}
											}
											...
											if(!empty( $current_qualification_certificate_file)) 
											{
						                        ...
						                        { 
						                        	$up_cand_data['qualification_certificate_file'] = $new_qualification_certificate_file; 
						                        }
                      						}
						                    ...
						                    if(!empty($current_candidate_photo)) 
					                        {
						                        ...
						                        { 
						                        	$up_cand_data['candidate_photo'] = $new_candidate_photo; 
						                        }
						                    }
					                    ...
					                    if(isset($new_regnumber) && $new_regnumber != '') 
				                    	{ 
				                    		$up_exam_data['regnumber'] = $new_regnumber; 
				                    	}
					...
					else if($pay_type == "iibfbulkdra") //FOR IIBF DRA MODULE. ADDED BY GAURAV SHEWALE ON 29-01-2025
					{
						sleep(8);
						...
						$desc   = 'Online Payment Success - Approved by Institute : S2S';
						$status = 1;	
						
			        	$get_user_regnum_info = $this->master_model->getRecords('dra_payment_transaction', array('receipt_no' => $MerchantOrderNo,'gateway'=>2), 'id,registration_id,exam_code,exam_period,gateway,amount,date,inst_code,pay_count,status,created_date');

			          	if (count($get_user_regnum_info) > 0 && ($get_user_regnum_info[0]['status'] == 3 || $get_user_regnum_info[0]['status'] == 5) )
			          	{
			          		$id 		 = $get_user_regnum_info[0]['id'];
				          	$examcode    = $get_user_regnum_info[0]['exam_code'];
				          	$exam_period = $get_user_regnum_info[0]['exam_period'];

				          	//START APPROVED
				          	if(isset($MerchantOrderNo) && $MerchantOrderNo != '')
							{
								...
								{
									log_dra_admin($log_title = "DRA Institute Online Payment Failed Due to RPE mode is not Physical : S2S", $log_message = "mode not defined. Qry : ".$error_qry);
									exit;
								}			
								...
								if( $chk_exam_mode == 'PHYSICAL')
								{
									...
									{
										log_dra_admin($log_title = "DRA S2S when Online Payment is success Transaction status updated.", $log_message = serialize($update_data));
										...
										if( $status == 1 ) //WHEN PAYMENT IS APPROVED
										{
												...
												foreach( $memexamidlst as $memexamids ) 
												{
													...
													if( $memregid ) 
													{
														...
														if( empty( $regnum ) ) 
														{
															...
															$update_data_admit  =  array('mem_mem_no' => $memregnumber );	
															...
															log_dra_admin($log_title = "DRA Reg No. generated successfully after Online Payment Approval : S2S", $log_message = serialize($log_update_data));
																			
															//update uploaded file names which will include generated registration number
															//get cuurent saved file names from DB
															$currentpics = $this->master_model->getRecords('dra_members', array('regid'=>$memregid), 'scannedphoto, scannedsignaturephoto, idproofphoto, training_certificate, quali_certificate'); 							

															$scannedphoto_file = $scannedsignaturephoto_file = $idproofphoto_file = $trainingphoto_file = $qualiphoto_file = '';
															...
															if(count($upd_files)>0) {
																log_dra_admin($log_title = "DRA Member Images Updated successfully after Online Payment Approval : S2S", $log_message = serialize($upd_files));																
											...
											if(count($exam_invoice) > 0) 
											{
												...
												log_dra_admin($log_title = "DRA Exam Invoice updated after Online Payment Approval : S2S", $log_message = serialize($invoice_update_data));
												...
												foreach( $memexamidlst as $memexamids ) 
												{
													...
													if( $memregid ) 
													{
														...
														if($reattempt >= 1)
														{
															...
															$update_datas = array('re_attempt'	=> $re_attempt, 'new_reg' => 0);
													  	}
													  	else
													  	{
															$re_attempt = $reattempt + 1;
														...
														$log_update_datas = array('regid' => $memregid,'re_attempt' => $re_attempt);
															
														log_dra_admin($log_title = "DRA Re-attempt successfully after Online Payment Approval : S2S", $log_message = serialize($log_update_datas));		
													}
												}
											}							
										}
										...
										$update_data =  array( 'pay_status' => $status,'ptid' => $id );

										log_dra_admin($log_title = "DRA Member Exam S2S Payment Transaction Status updated after Online Payment Success.", $log_message = serialize($update_data));
				...
				else if($payment_status==0)
				{
		          	...
		          	{
			            sleep(8);
						$desc   = 'Payment Failed - Rejected by Institute : S2S';
						$status = 0;	
			        	...
			        	$get_user_regnum_info = $this->master_model->getRecords('dra_payment_transaction', array('receipt_no' => $MerchantOrderNo,'gateway'=>2), 'id,registration_id,exam_code,exam_period,gateway,amount,date,inst_code,pay_count,status,created_date');
			          	...
			          	if ( count($get_user_regnum_info) > 0 && ($get_user_regnum_info[0]['status'] == 3 || $get_user_regnum_info[0]['status'] == 5) )
			          	{
				          	...
				          	if(isset($MerchantOrderNo) && $MerchantOrderNo != '')
							{
								...
								{
									log_dra_admin($log_title = "DRA Institute Online Payment Failed : S2S", $log_message = "mode not defined. Qry : ".$error_qry);
									exit;
								}			
								...
								if( ($chk_exam_mode == 'RPE' && count($sub_arr) == count($member_array) && $flag == 0 && (count($member_array) > 0 || $payment_action == "Rejected")) || $chk_exam_mode == 'PHYSICAL')
								{
									if($this->master_model->updateRecord('dra_payment_transaction',$update_data,  array('id' => $id)))
									{
										log_dra_admin($log_title = "DRA S2S when Online Payment is failed Transaction status updated.", $log_message = serialize($update_data));
										...
										log_dra_admin($log_title = "DRA Member Exam S2S Transaction Status updated after when Online Payment is Failed.", $log_message = serialize($update_data));
									}
								}
							}
						}
