...
class Admitcardtest extends CI_Controller {
	...
	// Common login function for BCBF Admitcardtest link
	public function bcbf(){ 
		try{ 
		    ...
		    if(isset($_POST['submit'])){
				...
				if ($this->form_validation->run() == TRUE){
					...
					if(count($user_info) > 0){ 
						...
						$chkexam = $this->master_model->getRecords('Admitcardtest_info',$exmarr);
						if(count($chkexam) > 0){
							...
							redirect(base_url().'Admitcardtest/feedback');
						}else{
			...
			$data['code']=$cap['word'];
			$this->session->set_userdata('userAdmitcardtestlogincaptcha', $cap['word']);
	...
	public function getadmitdashboard(){
		try{
			...
			$query = $this->db->query("select exm_cd from Admitcardtest_info where mem_mem_no = '".$member_id."' ");
			...
			if(sizeof($exm_arr) > 0){
				if(count($exm_arr) > 1){
					$record=$this->db->query("SELECT Admitcardtest_info.exm_cd, Admitcardtest_info.m_1, Admitcardtest_info.sub_cd, admit_exam_master.description, Admitcardtest_info.mode, Admitcardtest_info.date as exam_date, Admitcardtest_info.Admitcardtest_id
					FROM admit_exam_master
					JOIN Admitcardtest_info ON Admitcardtest_info.exm_cd = admit_exam_master.exam_code
					WHERE Admitcardtest_info.mem_mem_no = '".$member_id."' 
					GROUP BY Admitcardtest_info.exm_cd
					ORDER BY Admitcardtest_info.Admitcardtest_id DESC 
					 ");
			...
			if(sizeof($exm_arr_1) > 0){ 
				$record=$this->db->query("SELECT admit_card_details.exm_cd, admit_card_details.m_1, admit_card_details.sub_cd, admit_exam_master.description, admit_card_details.mode, admit_card_details.exam_date as exam_date, admit_card_details.Admitcardtest_id
				FROM admit_exam_master
				JOIN admit_card_details ON admit_card_details.exm_cd = admit_exam_master.exam_code
				WHERE admit_card_details.mem_mem_no = '".$member_id."' AND admit_card_details.remark = 1 AND admit_card_details.Admitcardtest_image != '' AND admit_card_details.exam_date >= '".date("Y-m-d")."' AND exm_prd NOT IN(808,901,905,906,907)  AND exam_date != '2019-03-10' AND admit_card_details.exm_cd NOT IN (21,42,992) 
				GROUP BY admit_card_details.exm_cd
				ORDER BY admit_card_details.Admitcardtest_id DESC 
				 ");
	...
	public function getAdmitcardtestsp(){
		try{
			if($this->session->userdata('spregnumber') != ''){
				$member_id = $this->session->userdata('spregnumber');
			}
			...
			$this->db->select('Admitcardtest_info.*,member_registration.scannedphoto,member_registration.scannedsignaturephoto');
			$this->db->from('Admitcardtest_info');
			$this->db->join('member_registration', 'Admitcardtest_info.mem_mem_no = member_registration.regnumber');
			$this->db->where(array('Admitcardtest_info.mem_mem_no' => $member_id,'exm_cd'=>$exam_code));
			$record = $this->db->get();
			$result = $record->row();
			...
			$subject=$this->db->query("SELECT subject_description,date,time,venueid,seat_identification FROM admit_subject_master JOIN Admitcardtest_info ON admit_subject_master.subject_code = LEFT(Admitcardtest_info.sub_cd,3) WHERE admit_subject_master.exam_code = ".$exam_code." AND Admitcardtest_info.mem_mem_no = '".$member_id."' AND subject_delete = '0' ORDER BY STR_TO_DATE(date, '%e-%b-%y') ASC ");
			...
			$this->load->view('Admitcardtestsp', $data);
	...
	// function run for old table for exam code 101
	public function getAdmitcardtestpdfsp(){
		try{
			if($this->session->userdata('spregnumber') != ''){
				$member_id = $this->session->userdata('spregnumber');
			}
			...
			$html=$this->load->view('Admitcardtestpdf', $data, true);
			$this->load->library('m_pdf');
			$pdf = $this->m_pdf->load();
	...
	public function check_captcha_userlogin($code){
		try{
			if(!isset($this->session->userAdmitcardtestlogincaptcha) && empty($this->session->userAdmitcardtestlogincaptcha)){
				redirect(base_url().'index/');
			}
			if($code == '' || $this->session->userAdmitcardtestlogincaptcha != $code ){
			...
			if($this->session->userAdmitcardtestlogincaptcha == $code){
				$this->session->set_userdata('userAdmitcardtestlogincaptcha','');
				$this->session->unset_userdata("userAdmitcardtestlogincaptcha");
				return true;
			}
		}catch(Exception $e){
			echo "Message : ".$e->getMessage();	
	...
	public function generatecaptchaajax(){
		try{
			$this->load->helper('captcha');
			$this->session->unset_userdata("userAdmitcardtestlogincaptcha");
			$this->session->set_userdata("userAdmitcardtestlogincaptcha", rand(1, 100000));
			$vals = array('img_path' => './uploads/applications/','img_url' => base_url().'uploads/applications/',);
			$cap = create_captcha($vals);
			$data = $cap['image'];
			$_SESSION["userAdmitcardtestlogincaptcha"] = $cap['word'];
			echo $data;
		}catch(Exception $e){
			echo "Message : ".$e->getMessage();	
		}
	...
	public function getAdmitcardtestjd(){
		try{
			if($this->session->userdata('spregnumber') != ''){
				$member_id = $this->session->userdata('spregnumber');
			}
			...
			$this->load->view('Admitcardtestjd', $data);
		}catch(Exception $e){
			echo $e->getMessage();
		}
	}
	// function run for old table  from website link without exam codde 101
	public function getAdmitcardtestpdfjd(){
		try{
			if($this->session->userdata('spregnumber') != ''){
				$member_id = $this->session->userdata('spregnumber');
			}
			...
			$html=$this->load->view('Admitcardtestpdfjd', $data, true);
			$this->load->library('m_pdf');
			$pdf = $this->m_pdf->load();
			$pdfFilePath = "IIBF_ADMIT_CARD_".$member_id.".pdf";
			$pdf->WriteHTML($html);
	...
	public function getAdmitcardtestsp_new(){
		try{
			if($this->session->userdata('spregnumber') != ''){
				$member_id = $this->session->userdata('spregnumber');
			}
			...
			$this->db->where(array('admit_card_details.mem_mem_no' => $member_id,'exm_cd'=>$exam_code,'remark'=>1,'exam_date >= '=> date("Y-m-d")));
			$record = $this->db->get();
			$result = $record->row(); 
			...
			$this->load->view('jdAdmitcardtestsp', $data);
		}catch(Exception $e){
			echo $e->getMessage();
		}
	}
	// function run for new table  from user portal
	public function getAdmitcardtestpdfsp_new(){
		try{
			if($this->session->userdata('spregnumber') != ''){
				$member_id = $this->session->userdata('spregnumber');
			}
			...
			$html=$this->load->view('jdAdmitcardtestpdf', $data, true);
			$this->load->library('m_pdf');
			$pdf = $this->m_pdf->load();
	...
	public function caiib119(){
		try{
			$data=array();
			$data['error']='';
			
			$feedback_type = 'new';
			$feedback_exam_name = 'caiib119';
		    ...
		    if(isset($_POST['submit'])){
				...
				if ($this->form_validation->run() == TRUE){
					...
					if(count($user_info) > 0){ 
						...
						$chkexam = $this->master_model->getRecords('Admitcardtest_info',$exmarr,'exm_cd');
						if(count($chkexam) > 0){
	...
	public function caiib119dashboard(){
		$result = array();
		$tbl = '';
		try{
			if($this->session->userdata('spregnumber') != ''){
			...
			if(!isset($member_id)){
				redirect(base_url('Admitcardtest/caiib119/'));
			}
		    ...
		    if(isset($_POST['submit'])){
				...
				if ($this->form_validation->run() == TRUE){
					...
					if(count($user_info) > 0){ 
						...
						$chkexam = $this->master_model->getRecords('Admitcardtest_caiib_rescheduled',$exmarr,'exm_cd');
						if(count($chkexam) > 0){
							$ex_arr = explode(",",$this->input->post('examcode'));
							if(in_array($chkexam[0]['exm_cd'],$ex_arr)){
								$mysqltime=date("H:i:s");
	...
	public function caiibdashboardres(){
		...
		try{
			...
			if(!isset($member_id)){
				redirect(base_url('Admitcardtest/caiib_rescheduled/'));
			}
		
			$query = $this->db->query("select exm_cd from Admitcardtest_caiib_rescheduled where mem_mem_no = '".$member_id."' ");
			$exm_arr = $query->result();
		
			if(sizeof($exm_arr) > 0){
				if(count($exm_arr) > 1){
					 $record=$this->db->query("SELECT Admitcardtest_caiib_rescheduled.exm_cd, Admitcardtest_caiib_rescheduled.m_1, Admitcardtest_caiib_rescheduled.sub_cd, admit_exam_master.description, Admitcardtest_caiib_rescheduled.mode, Admitcardtest_caiib_rescheduled.date as exam_date, Admitcardtest_caiib_rescheduled.Admitcardtest_id
					FROM admit_exam_master
					JOIN Admitcardtest_caiib_rescheduled ON Admitcardtest_caiib_rescheduled.exm_cd = admit_exam_master.exam_code
					WHERE Admitcardtest_caiib_rescheduled.mem_mem_no = '".$member_id."' 
					GROUP BY Admitcardtest_caiib_rescheduled.exm_cd
					ORDER BY Admitcardtest_caiib_rescheduled.Admitcardtest_id DESC 
					 ");
					$result = $record->result();
				}elseif(count($exm_arr) == 1){
					$record=$this->db->query("SELECT Admitcardtest_caiib_rescheduled.exm_cd, Admitcardtest_caiib_rescheduled.m_1, Admitcardtest_caiib_rescheduled.sub_cd, admit_exam_master.description, Admitcardtest_caiib_rescheduled.mode, Admitcardtest_caiib_rescheduled.date as exam_date, Admitcardtest_caiib_rescheduled.Admitcardtest_id
	...
	public function getAdmitcardtestcdres(){
		try{
			if($this->session->userdata('spregnumber') != ''){
				$member_id = $this->session->userdata('spregnumber');
			}
			...
			$this->db->select('Admitcardtest_caiib_rescheduled.*,member_registration.scannedphoto,member_registration.scannedsignaturephoto');
			$this->db->from('Admitcardtest_caiib_rescheduled');
			$this->db->join('member_registration', 'Admitcardtest_caiib_rescheduled.mem_mem_no = member_registration.regnumber');
			$this->db->where(array('Admitcardtest_caiib_rescheduled.mem_mem_no' => $member_id,'exm_cd'=>$exam_code));
			$record = $this->db->get();
			$result = $record->row(); 
			...
			$subject=$this->db->query("SELECT subject_description,date,time,venueid,seat_identification FROM admit_subject_master JOIN Admitcardtest_caiib_rescheduled ON admit_subject_master.subject_code = LEFT(Admitcardtest_caiib_rescheduled.sub_cd,3) WHERE admit_subject_master.exam_code = ".$exam_code." AND Admitcardtest_caiib_rescheduled.mem_mem_no = '".$member_id."' AND subject_delete = '0' ORDER BY STR_TO_DATE(date, '%e-%b-%y') ASC ");
			$subject_result = $subject->result();
			$pdate = $subject->result();
			...
			$this->load->view('Admitcardtestcdres', $data);
		}catch(Exception $e){
			echo $e->getMessage();
		}
	}
	public function getAdmitcardtestpdfcdres(){
		try{
			if($this->session->userdata('spregnumber') != ''){
				$member_id = $this->session->userdata('spregnumber');
			}
	...
	public function jaiibdashboard(){
		try{
			...
			if(!isset($member_id)){
				redirect(base_url('Admitcardtest/jaiib/'));
			}
			$query = $this->db->query("select exm_cd from Admitcardtest_info where mem_mem_no = '".$member_id."' ");
			$exm_arr = $query->result();
...
");
	...
	public function jaiibdashboardres(){
		try{ 
			...
			$query = $this->db->query("select exm_cd from Admitcardtest_info_190 where mem_mem_no = '".$member_id."' ");
			
			$exm_arr = $query->result();
			
			if(sizeof($exm_arr) > 0 && count($exm_arr) > 1){
				$record=$this->db->query("SELECT Admitcardtest_info_190.exm_cd, Admitcardtest_info_190.m_1, Admitcardtest_info_190.sub_cd, admit_exam_master.description, Admitcardtest_info_190.mode, Admitcardtest_info_190.date as exam_date, Admitcardtest_info_190.Admitcardtest_id
FROM admit_exam_master
JOIN Admitcardtest_info_190 ON Admitcardtest_info_190.exm_cd = admit_exam_master.exam_code
WHERE Admitcardtest_info_190.mem_mem_no = '".$member_id."' 
GROUP BY Admitcardtest_info_190.exm_cd
ORDER BY Admitcardtest_info_190.Admitcardtest_id DESC 
");
				$result = $record->result();
			}elseif(count($exm_arr) == 1){
				$record=$this->db->query("SELECT Admitcardtest_info_190.exm_cd, Admitcardtest_info_190.m_1, Admitcardtest_info_190.sub_cd, admit_exam_master.description, Admitcardtest_info_190.mode, Admitcardtest_info_190.date as exam_date, Admitcardtest_info_190.Admitcardtest_id
...
");
	...
	// function run for old table  from website link without exam codde 101
	public function getAdmitcardtestjdres(){
		try{
			if($this->session->userdata('spregnumber') != ''){
				$member_id = $this->session->userdata('spregnumber');
			}
			...
			$this->db->select('Admitcardtest_info_190.*,member_registration.scannedphoto,member_registration.scannedsignaturephoto');
			$this->db->from('Admitcardtest_info_190');
			$this->db->join('member_registration', 'Admitcardtest_info_190.mem_mem_no = member_registration.regnumber');
			$this->db->where(array('Admitcardtest_info_190.mem_mem_no' => $member_id,'exm_cd'=>$exam_code));
			$record = $this->db->get();
			$result = $record->row(); 
			...
			$subject=$this->db->query("SELECT subject_description,date,time,venueid,seat_identification FROM admit_subject_master JOIN Admitcardtest_info_190 ON admit_subject_master.subject_code = LEFT(Admitcardtest_info_190.sub_cd,3) WHERE admit_subject_master.exam_code = ".$exam_code." AND Admitcardtest_info_190.mem_mem_no = '".$member_id."' AND subject_delete = '0' ORDER BY STR_TO_DATE(date, '%e-%b-%y') ASC ");
			$subject_result = $subject->result();
			$pdate = $subject->result();
			...
			$this->load->view('Admitcardtestjdres', $data);
		}catch(Exception $e){
			echo $e->getMessage();
		}
	}
	// function run for old table  from website link without exam codde 101
	public function getAdmitcardtestpdfjdres(){
		try{
			if($this->session->userdata('spregnumber') != ''){
				$member_id = $this->session->userdata('spregnumber');
			}
	...
	public function dipcertdashboard808(){
		...
		try{
			...
			if(!isset($member_id)){
				redirect(base_url('Admitcardtest/dipcert808'));
			}
		...
		try{
			...
			if(sizeof($exm_arr) > 0){
				if(count($exm_arr) > 1){
					...
					FROM admit_exam_master
					JOIN Admitcardtest_info ON Admitcardtest_info.exm_cd = admit_exam_master.exam_code
					WHERE Admitcardtest_info.mem_mem_no = '".$member_id."' AND date = '27-Apr-19' 
					GROUP BY Admitcardtest_info.exm_cd
					ORDER BY Admitcardtest_info.Admitcardtest_id DESC 
					 ");
					$result = $record->result();
		    ...
		    if(isset($_POST['submit'])){
				...
				if ($this->form_validation->run() == TRUE){
					...
					if(count($user_info) > 0){ 
						...
						$this->db->order_by('Admitcardtest_id','desc');
						$chkexam = $this->master_model->getRecords('Admitcardtest_info',$exmarr,'exm_cd');
						if(count($chkexam) > 0){
							$ex_arr = explode(",",$this->input->post('examcode'));
							if(in_array($chkexam[0]['exm_cd'],$ex_arr)){
		...
		try{
			...
			if(sizeof($exm_arr) > 0){ 
				if(count($exm_arr) > 1){
					...
					FROM admit_exam_master
					JOIN Admitcardtest_info ON Admitcardtest_info.exm_cd = admit_exam_master.exam_code
					WHERE Admitcardtest_info.mem_mem_no = '".$member_id."' AND date = '26-Oct-19'
					GROUP BY Admitcardtest_info.exm_cd
					ORDER BY Admitcardtest_info.Admitcardtest_id DESC 
					 ");
					$result = $record->result();
