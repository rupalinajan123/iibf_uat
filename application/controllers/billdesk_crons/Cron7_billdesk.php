<?php
	...
	class Cron7_billdesk extends CI_Controller 
	{
      ...
      $this->load->helper('iibfbcbf/iibf_bcbf_helper');
			...
			$log_title ="billdesk_callback Function call Cron7";
			$log_message = 'billdesk_callback Function call Cron7';
			$rId = $regNo = '';
			$this->member_regn_log($log_title, $log_message, $rId, $regNo);
			
			$filehandle = fopen("cs2s_log/billdesk_cron7/lock.txt", "c+");
        ...
        //START : PAYMENT TRANSACTION QUERY
				$query='SELECT receipt_no, pg_flag FROM payment_transaction WHERE date <= NOW() - INTERVAL 120 MINUTE AND date > NOW() - INTERVAL 1560 MINUTE AND gateway = "billdesk" AND status = 2';
        ...
        $query_bcbf = 'SELECT receipt_no, pg_flag FROM iibfbcbf_payment_transaction WHERE date <= NOW() - INTERVAL 120 MINUTE AND date > NOW() - INTERVAL 1560 MINUTE AND gateway = "2" AND status = "2" AND payment_mode = "Individual"';
        ...
        $query_amp = 'SELECT receipt_no, pg_flag FROM amp_payment_transaction WHERE date <= NOW() - INTERVAL 120 MINUTE AND date > NOW() - INTERVAL 1560 MINUTE AND gateway = "billdesk" AND status = "2"';
        ...
        //echo '<pre>'; print_r($receipt_no_arr); echo '</pre>';
				...
				if (count($receipt_no_arr) > 0)
				{
					...
					$dir = 'cs2s_log/billdesk_cron7/'.$todays_date;
						...
						if($refundInitiated==0 && isset($responsedata) && count($responsedata) > 0) 
            {
              ...
              else
              {
                  ...
                  else
                  {
							...
							'txn_data' 		=> $encData.'&CALLBACK=C_S2S7',
