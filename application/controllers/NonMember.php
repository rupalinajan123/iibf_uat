...
class NonMember extends CI_Controller
{
    public function __construct()
    {
        ...
        {*/
        if ($this->router->fetch_method() != 'comApplication' && $this->router->fetch_method() != 'preview' && $this->router->fetch_method() != 'Msuccess' && $this->router->fetch_method() != 'editmobile' && $this->router->fetch_method() != 'editemailduplication' && $this->router->fetch_method() != 'setExamSession' && $this->router->fetch_method() != 'details' && $this->router->fetch_method() != 'applydetails' && $this->router->fetch_method() != 'saveexam' && $this->router->fetch_method() != 'exampdf' && $this->router->fetch_method() != 'printexamdetails' && $this->router->fetch_method() != 'sbi_make_payment' && $this->router->fetch_method() != 'sbitranssuccess' && $this->router->fetch_method() != 'sbitransfail' && $this->router->fetch_method() != 'accessdenied' && $this->router->fetch_method() != 'check_emailduplication' && $this->router->fetch_method() != 'check_mobileduplication' && $this->router->fetch_method() != 'check_checkpin' && $this->router->fetch_method() != 'refund' && $this->router->fetch_method() != 'info' &&  $this->router->fetch_method() != 'set_nonmem_elsub_cnt' && $this->router->fetch_method() != 'handle_billdesk_response' && $this->router->fetch_method() != 'getsetAsFresherOrOld') // priyanka d- 02-feb-23  // priyanka d- 10-feb-23 >> added handle_billdesk_response 
        {
    ...
    public function profile()
    {
        ...
        $user_info = $this->master_model->getRecords('member_registration', array(
			...
			///echo'<pre>';print_r($user_info[0]);exit;
        ...
        if (isset($_POST['btnSubmit'])) {
           // exit; // priyanka d - 28-dec-22 as per asked by client
            ...
            // $this->form_validation->set_rules('email','Email','trim|required|valid_email|is_unique[member_registration.email.regid.'.$this->session->userdata('nmregid').']|xss_clean');
				//priyanka d -12-july-23 >> added excode condition so candidate will not update email/mob
				if($prevData['excode']!='997') {
					...
					// Commented by anil on 22 Oct 2024 so candidate will not update email/mob 
            }
            ...
            if ($this->form_validation->run() == true) {  
                ...
                $prev_edited_on_qry = $this->master_model->getRecords('member_registration', array(
                    'regid' => $this->session->userdata('nmregid'),
                ), 'DATE(images_editedon) images_editedon,photo_flg,signature_flg,id_flg,empid_flg,declaration_flg');
                ...
                if ($flag == 1) { 
                    ...
                    $update_data_image['editedon']              = date('Y-m-d H:i:s');//ADDED BY POOJA MANE (26-2-24)
                    ...
                    if (count($prevData)) {
                        ...
                        if ($prevData['specify_qualification'] != $specify_qualification) {
							...
							/*if ($prevData['email'] != $email && $prevData['excode']!='997') { //priyanka d -12-july-23 >> added excode condition so candidate will not update email/mob
                            $update_data['email'] = $email;
                        }*/

                        if ($prevData['excode'] == '997' || $prevData['excode'] == '1053' || $prevData['bank_bc_id_no'] != '' || ($prevData['ippb_emp_id'] != '0' && $prevData['ippb_emp_id'] != '')) {         
                            //print_r($prevData['email']);                   
                             //The candidate will not be able to update their email or mobile number for the IPPB exam application.
                        }else if ($email != "" && $prevData['email'] != $email) {
                            $update_data['email'] = $email;   
                        }
                        ...
                        if ($prevData['office_phone'] != $phone) {
							...
							/*if ($prevData['mobile'] != $mobile  && $prevData['excode']!='997') { //priyanka d -12-july-23 >> added excode condition so candidate will not update email/mob
                            $update_data['mobile'] = $mobile;
                        }*/

                        if ($prevData['excode'] == '997' || $prevData['excode'] == '1053' || $prevData['bank_bc_id_no'] != '' || ($prevData['ippb_emp_id'] != '0' && $prevData['ippb_emp_id'] != '')) {         
                            //print_r($prevData['mobile']);                   
                             //The candidate will not be able to update their email or mobile number for the IPPB exam application.
                        }else if ($mobile != "" && $prevData['mobile'] != $mobile) {
                            $update_data['mobile'] = $mobile;
                        }

                        //print_r($prevData['mobile']);
                        //echo "ippb_emp_id: ".$prevData['ippb_emp_id']."<br>mobile: "; print_r($update_data['mobile']);die;
...
//echo '**';exit;
    ...
    public function benchmark_disability_check()
    {
        ...
        if ($user_images[0]['benchmark_disability'] == '') {
            ...
            if($this->session->userdata('nm_without_pass') && ($this->session->userdata('nm_without_pass') == 1)){
                ...
                //redirect(base_url() . 'NonMember/examlist');
                $msg .= '<li>
                Kindly go to login page <a href="' . base_url() . 'Nonreg/logout/">click here</a> and click on Edit Profile to update the, "Benchmark Disability" and then apply for exam. For any queries contact zonal office.</li>';
            }else{
                $msg .= '<li>
    ...
    // ##---------Edit Images(Prafull)-----------##
    //RENAMED BY SAGAR M ON 2024-11-11, AS WE NEED TO REPLACE BROWSE IMAGE FUNCTIONALITY WITH CROPPER IMAGE (CROPPER JS) FUNCTIONALITY
    public function editimages_bk_without_cropper()
    {
      $kyc_update_data = array();
      $kyc_edit_flag   = 0;
      $flag            = 1;
        $member_info     = $this->master_model->getRecords('member_registration', array('regid' => $this->session->userdata('nmregid')), 'scannedphoto,scannedsignaturephoto,idproofphoto,empidproofphoto,declaration,excode');//added empidproofphoto,declaration,excode for Fedai  2024-08-08
      $applicationNo   = $this->session->userdata('nmregnumber');
        $fedai_array = array(1009);

        if (isset($_POST['btnSubmit'])) {
            // echo '<pre>';print_r($_POST);die;
            // echo $member_info[0]['excode'];die;
            if ($_FILES['scannedphoto']['name'] == '' && $_FILES['scannedsignaturephoto']['name'] == '' && $_FILES['idproofphoto']['name'] == ''  && $_FILES['empidproofphoto']['name'] == '' && $_FILES['declaration']['name'] == '') //added empproof & declaration by pooja mane 2024-08-08
        {
          ...
          $this->form_validation->set_rules('idproofphoto', 'id proof', 'file_required|file_allowed_type[jpg,jpeg]|file_size_max[300]');
            ...
            // Code added by pooja mane for fedai idproof 2024-06-08
            if(in_array($member_info[0]['excode'],$fedai_array))
            {   
                if ($_FILES['empidproofphoto']['name'] != '')
                {
                    $this->form_validation->set_rules('empidproofphoto', 'Employment proof', 'file_required|file_allowed_type[jpg,jpeg]|file_size_max[300]');
                }
            ...
            // Code added by pooja mane for fedai idproof end 2024-06-08
          ...
          $prev_signature_flg = "N";
          $prev_id_flg        = "N";
                $prev_empid_flg       = "N";//added by pooja mane for fedai
                $prev_declaration_flg = "N";//added by pooja mane for fedai
                $prev_edited_on_qry = $this->master_model->getRecords('member_registration', array('regid' => $this->session->userdata('nmregid')), 'DATE(images_editedon) images_editedon,photo_flg,signature_flg,id_flg,empid_flg,declaration_flg');
                if (count($prev_edited_on_qry)) {
            $prev_edited_on     = $prev_edited_on_qry[0]['images_editedon'];
            $prev_photo_flg     = $prev_edited_on_qry[0]['photo_flg'];
            $prev_signature_flg = $prev_edited_on_qry[0]['signature_flg'];
            $prev_id_flg        = $prev_edited_on_qry[0]['id_flg'];
                    $prev_empid_flg        = $prev_edited_on_qry[0]['empid_flg'];//added by pooja mane for fedai
                    $prev_declaration_flg        = $prev_edited_on_qry[0]['declaration_flg'];//added by pooja mane for fedai
                    if ($prev_edited_on != date('Y-m-d')) {
                        $this->master_model->updateRecord('member_registration', array('photo_flg' => 'N', 'signature_flg' => 'N', 'id_flg' => 'N','empid_flg' => 'N','declaration_flg' => 'N'), array('regid' => $this->session->userdata('nmregid')));
            }
          }
          ...
          $idproofphoto_file = '';
            ...
            $idproofphoto_file = $this->input->post('idproofphoto1_hidd');
                ...
                if ($prev_edited_on != '' && $prev_edited_on != date('Y-m-d')) {$empid_flg = 'N';} else { $empid_flg = $prev_empid_flg;}
                if ($_FILES['empidproofphoto']['name'] != '') {
                    @unlink('uploads/photograph/' . $member_info[0]['empidproofphoto']);
                    $path = "./uploads/empidproof";
                    //$new_filename = 'idproof_'.strtotime($date).rand(1,99999);
                    $new_filename = 'empr_' . $applicationNo;
                    $uploadData   = upload_file('empidproofphoto', $path, $new_filename, '', '', true);
                    if ($uploadData) {
                        $kyc_edit_flag                       = 1;
                        $kyc_update_data['edited_employee_proof'] = '1';
                        $empidproofphoto_file                   = $uploadData['file_name'];
                        $empid_flg                              = 'Y';
                        $edited .= 'EMPPROOF || ';
                    } else {
                        $flag              = 0;
                        $empidproofphoto_file = $this->input->post('empidproofphoto1_hidd');
                    }
                } else {
                    $empidproofphoto_file = $this->input->post('empidproofphoto1_hidd');
                }
                ...
                // Code added by pooja mane for fedai idproof end 2024-06-08
                $declaration_file = '';
                if ($prev_edited_on != '' && $prev_edited_on != date('Y-m-d')) {$declaration_flg = 'N';} else { $declaration_flg = $prev_declaration_flg;}
                if ($_FILES['declaration']['name'] != '') {
                    @unlink('uploads/photograph/' . $member_info[0]['declaration']);
                    ...
                    if ($uploadData) {
                        ...
                        $flag              = 0;
                        $declaration_file = $this->input->post('declaration1_hidd');
                    }
                } else {
                    $declaration_file = $this->input->post('declaration1_hidd');
                }
            ...
            $update_info = array(
              ...
              'scannedsignaturephoto' => $scannedsignaturephoto_file,
              'idproofphoto'          => $idproofphoto_file,
                        'empidproofphoto'       => $empidproofphoto_file,//added by pooja mane for fedai id proof 2024-06-08
                        'declaration'           => $declaration_file,//added by pooja mane for fedai id proof 2024-06-08
              'images_editedon'       => date('Y-m-d H:i:s'),
              'images_editedby'       => 'Candidate',
              'photo_flg'             => $photo_flg,
              'signature_flg'         => $signature_flg,
              'id_flg'                => $id_flg,
                        'empid_flg'             => $empid_flg,//added by pooja mane for fedai id proof 2024-06-08
                        'declaration_flg'       => $declaration_flg,//added by pooja mane for fedai declaration 2024-06-14
            ...
            //$personalInfo = filter($personal_info);
              ...
              log_profile_user($log_title = "Profile updated successfully", $finalStr, 'image', $this->session->userdata('nmregid'), $this->session->userdata('nmregnumber'));
                        if ((in_array($member_info[0]['excode'],$fedai_array)) && !is_file(get_img_name($this->session->userdata('nmregnumber'), 'declaration')) && !is_file(get_img_name($this->session->userdata('nmregnumber'), 'empr')) ||!is_file(get_img_name($this->session->userdata('nmregnumber'), 'pr')) || !is_file(get_img_name($this->session->userdata('nmregnumber'), 's')) || !is_file(get_img_name($this->session->userdata('nmregnumber'), 'p')) || validate_edit_nonmemdata($this->session->userdata('nmregnumber'))) {
                            $this->session->set_flashdata('error', 'Please update your profile!!');
                            redirect(base_url() . 'NonMember/profile/');
                        }elseif(!is_file(get_img_name($this->session->userdata('nmregnumber'), 'empr')) ||!is_file(get_img_name($this->session->userdata('nmregnumber'), 'pr')) || !is_file(get_img_name($this->session->userdata('nmregnumber'), 's')) || !is_file(get_img_name($this->session->userdata('nmregnumber'), 'p')) || validate_edit_nonmemdata($this->session->userdata('nmregnumber'))) {
                $this->session->set_flashdata('error', 'Please update your profile!!');
                redirect(base_url() . 'NonMember/profile/');
              }
  ...
  public function editimagesX()
    ...
    // ADDED BY SAGAR M ON 2024-11-11, AS WE NEED TO REPLACE BROWSE IMAGE FUNCTIONALITY WITH CROPPER IMAGE (CROPPER JS) FUNCTIONALITY
    public function editimages()
    {
      ...
      $member_info = $this->master_model->getRecords('member_registration', array('regid' => $this->session->userdata('nmregid')), 'namesub, firstname, middlename, lastname, scannedphoto, scannedsignaturephoto, idproofphoto, empidproofphoto, declaration, excode, date_of_commenc_bc, bank_bc_id_card');//added empidproofphoto,declaration,excode for Fedai  2024-08-08
      
      if (isset($_POST) && count($_POST) > 0)
      {
        $scannedphoto_req_flg = $scannedsignaturephoto_req_flg = $idproofphoto_req_flg = 'required|';
        $empidproofphoto_req_flg = $declaration_req_flg = $bank_bc_id_card_req_flg = '';
        ...
        else if (isset($_POST['scannedphoto1_hidd']) && $_POST['scannedphoto1_hidd'] != "") { $scannedphoto_req_flg = ''; }
        ...
        else if (isset($_POST['scannedsignaturephoto1_hidd']) && $_POST['scannedsignaturephoto1_hidd'] != "") { $scannedsignaturephoto_req_flg = ''; }
        ...
        else if (isset($_POST['idproofphoto1_hidd']) && $_POST['idproofphoto1_hidd'] != "") { $idproofphoto_req_flg = ''; }
        
        if (in_array($member_info[0]['excode'], $fedai_array))
        {
          $empidproofphoto_req_flg = $declaration_req_flg = 'required|';

          if (isset($_POST['empidproofphoto_cropper']) && $_POST['empidproofphoto_cropper'] != "") { $empidproofphoto_req_flg = ''; }
          else if ($member_info[0]['empidproofphoto'] != "") { $empidproofphoto_req_flg = ''; }
          else if (isset($_POST['empidproofphoto1_hidd']) && $_POST['empidproofphoto1_hidd'] != "") { $empidproofphoto_req_flg = ''; }

          if (isset($_POST['declaration_cropper']) && $_POST['declaration_cropper'] != "") { $declaration_req_flg = ''; }
          else if ($member_info[0]['declaration'] != "") { $declaration_req_flg = ''; }
          else if (isset($_POST['declaration1_hidd']) && $_POST['declaration1_hidd'] != "") { $declaration_req_flg = ''; }          
        }

        if($member_info[0]['date_of_commenc_bc'] != '' && $member_info[0]['date_of_commenc_bc'] != '0000-00-00')
        {
          $bank_bc_id_card_req_flg = 'required|';

          if (isset($_POST['bank_bc_id_card_cropper']) && $_POST['bank_bc_id_card_cropper'] != "") { $bank_bc_id_card_req_flg = ''; }
          else if ($member_info[0]['bank_bc_id_card'] != "") { $bank_bc_id_card_req_flg = ''; }
          else if (isset($_POST['bank_bc_id_card1_hidd']) && $_POST['bank_bc_id_card1_hidd'] != "") { $bank_bc_id_card_req_flg = ''; }
        }
        ...
        $this->form_validation->set_rules('empidproofphoto', 'Employment Proof', 'trim|'.$empidproofphoto_req_flg.'xss_clean');
        $this->form_validation->set_rules('declaration', 'Declaration', 'trim|'.$declaration_req_flg.'xss_clean');
        $this->form_validation->set_rules('bank_bc_id_card', 'Bank BC ID Card', 'trim|'.$bank_bc_id_card_req_flg.'xss_clean');
        ...
        if ($this->form_validation->run() == true)
        {
          ...
          $prev_edited_on = '';
          $prev_photo_flg = $prev_signature_flg = $prev_id_flg = $prev_empid_flg = $prev_declaration_flg = $prev_bank_bc_id_card_flg = "N";

          $prev_edited_on_qry = $this->master_model->getRecords('member_registration', array('regid' => $this->session->userdata('nmregid')), 'DATE(images_editedon) images_editedon, photo_flg, signature_flg, id_flg, empid_flg, declaration_flg, bank_bc_id_card_flg');
          if (count($prev_edited_on_qry))
          {
            $prev_edited_on = $prev_edited_on_qry[0]['images_editedon'];
            $prev_photo_flg = $prev_edited_on_qry[0]['photo_flg'];
            ...
            $prev_empid_flg = $prev_edited_on_qry[0]['empid_flg']; //added by pooja mane for fedai
            $prev_declaration_flg = $prev_edited_on_qry[0]['declaration_flg']; //added by pooja mane for fedai
            $prev_bank_bc_id_card_flg = $prev_edited_on_qry[0]['bank_bc_id_card_flg'];
            if ($prev_edited_on != date('Y-m-d'))
            {
              $this->master_model->updateRecord('member_registration', array('photo_flg' => 'N', 'signature_flg' => 'N', 'id_flg' => 'N', 'empid_flg' => 'N', 'declaration_flg' => 'N', 'bank_bc_id_card_flg'=>'N'), array('regid' => $this->session->userdata('nmregid')));
            }
          }
          ...
          //EMPLOYMENT PROOF
          $empidproofphoto_file = '';
          if ($prev_edited_on != '' && $prev_edited_on != date('Y-m-d')) { $empid_flg = 'N'; }
          else { $empid_flg = $prev_empid_flg; }
          if (isset($_POST['empidproofphoto_cropper']) && $_POST['empidproofphoto_cropper'] != "")
          {
            ...
            $new_file_name4 = "empr_" . $file_name_str . '.' . strtolower(pathinfo($empidproofphoto_cropper, PATHINFO_EXTENSION));
            if (copy(str_replace(base_url(), '', $empidproofphoto_cropper), $empidproofphoto_path . '/' . $new_file_name4))
            {
              $kyc_edit_flag = 1;
              $kyc_update_data['edited_employee_proof'] = 1;
              $empidproofphoto_file = $new_file_name4;
              $empid_flg = 'Y';
              $edited .= 'EMPPROOF || ';
            }
            else { $flag = 0; }
          ...
          if (isset($_POST['declaration_cropper']) && $_POST['declaration_cropper'] != "")
          {
            ...
            $new_file_name5 = "declaration_" . $file_name_str . '.' . strtolower(pathinfo($declaration_cropper, PATHINFO_EXTENSION));
            if (copy(str_replace(base_url(), '', $declaration_cropper), $declaration_path . '/' . $new_file_name5))
            {
              $kyc_edit_flag = 1;
              $kyc_update_data['edited_mem_declaration'] = 1;
              $declaration_file = $new_file_name5;
              $declaration_flg = 'Y';
              $edited .= 'DECLARATION || ';
            }
            else { $flag = 0; }
          ...
          if ($prev_edited_on != '' && $prev_edited_on != date('Y-m-d')) { $bank_bc_id_card_flg = 'N'; }
          else { $bank_bc_id_card_flg = $prev_bank_bc_id_card_flg; }          
          if (isset($_POST['bank_bc_id_card_cropper']) && $_POST['bank_bc_id_card_cropper'] != "")
          {
            ...
            $new_file_name6 = "bank_bc_id_card_" . $file_name_str . '.' . strtolower(pathinfo($bank_bc_id_card_cropper, PATHINFO_EXTENSION));
            if (copy(str_replace(base_url(), '', $bank_bc_id_card_cropper), $bank_bc_id_card_path . '/' . $new_file_name6))
            {
              $kyc_edit_flag = 1;
              $kyc_update_data['edited_bank_bc_id_card'] = 1;
              $bank_bc_id_card_file = $new_file_name6;
              $bank_bc_id_card_flg = 'Y';
              $edited .= 'BANK BC ID CARD || ';
            }
            else { $flag = 0; }
          }
          ...
          if ($flag == 1)
          {
            ...
            if($empidproofphoto_file != '') 
            { 
              $update_info['empidproofphoto'] = $empidproofphoto_file; 
              $update_info['empid_flg'] = $empid_flg; 
            }
            ...
            if($bank_bc_id_card_file != '') 
            { 
              $update_info['bank_bc_id_card'] = $bank_bc_id_card_file; 
              $update_info['bank_bc_id_card_flg'] = $bank_bc_id_card_flg; 
            }
            ...
            if(count($update_info) > 0)
            {
              ...
              $this->db->where('regid', $this->session->userdata('nmregid'));
              ...
              if ($this->master_model->updateRecord('member_registration', $update_info, array('regid' => $this->session->userdata('nmregid'), 'regnumber' => $this->session->userdata('nmregnumber'))))
              {
                ...
                if($scannedphoto_file != '')
                {
                  $scannedphoto_file_new = 'p_' . $this->session->userdata('nmregnumber') . '.' . pathinfo($scannedphoto_file, PATHINFO_EXTENSION);
                ...
                if($scannedsignaturephoto_file != '')
                {
                  $scannedsignaturephoto_file_new = 's_' . $this->session->userdata('nmregnumber') . '.' . pathinfo($scannedsignaturephoto_file, PATHINFO_EXTENSION);
                ...
                if($idproofphoto_file != '')
                {
                  $idproofphoto_file_new = 'pr_' . $this->session->userdata('nmregnumber') . '.' . pathinfo($idproofphoto_file, PATHINFO_EXTENSION);
                ...
                if($empidproofphoto_file != '')
                {
                  $empidproofphoto_file_new = 'empr_' . $this->session->userdata('nmregnumber') . '.' . pathinfo($empidproofphoto_file, PATHINFO_EXTENSION);

                  if ($empidproofphoto_file != $empidproofphoto_file_new)
                  {
                    if (rename($empidproofphoto_path . "/" . $empidproofphoto_file, $empidproofphoto_path . "/" . $empidproofphoto_file_new))
                    {
                      $up_img_data['empidproofphoto'] = $empidproofphoto_file_new;
                    }
                  }
                }
                ...
                if($declaration_file != '')
                {
                  $declaration_file_new = 'declaration_' . $this->session->userdata('nmregnumber') . '.' . pathinfo($declaration_file, PATHINFO_EXTENSION);
                ...
                if($bank_bc_id_card_file != '')
                {
                  $bank_bc_id_card_file_new = 'bank_bc_id_card_' . $this->session->userdata('nmregnumber') . '.' . pathinfo($bank_bc_id_card_file, PATHINFO_EXTENSION);

                  if ($bank_bc_id_card_file != $bank_bc_id_card_file_new)
                  {
                    if (rename($bank_bc_id_card_path . "/" . $bank_bc_id_card_file, $bank_bc_id_card_path . "/" . $bank_bc_id_card_file_new))
                    {
                      $up_img_data['bank_bc_id_card'] = $bank_bc_id_card_file_new;
                    }
                  }
                }
                ...
                {
                  $this->master_model->updateRecord('member_registration', $up_img_data, array('regid' => $this->session->userdata('nmregid'), 'regnumber' => $this->session->userdata('nmregnumber')));
                }
      ...
      $data = array('middle_content' => 'nonmember/edit_images', 'member_info' => $member_info, 'scannedphoto_path' => $scannedphoto_path, 'scannedsignaturephoto_path' => $scannedsignaturephoto_path, 'idproofphoto_path' => $idproofphoto_path, 'declaration_path' => $declaration_path, 'empidproofphoto_path' => $empidproofphoto_path, 'bank_bc_id_card_path' => $bank_bc_id_card_path);
      $this->load->view('nonmember/nm_common_view', $data);
      //print_r($data);die;
    }
    ...
    public function editemailduplication()
    {
        ...
        if ($email != "" && $regid != "") {
				...
				$checkExCode = $this->master_model->getRecords('member_registration', array('regid' => $regid));
				if($checkExCode[0]['excode']=='997' || $checkExCode[0]['excode'] == '1053') {
					echo 'ok'; return 1;
				}
            $where = "( registrationtype='NM')";
            $this->db->where($where);
            $prev_count = $this->master_model->getRecordCount('member_registration', array('email' => $email, 'regid !=' => $regid, 'isactive' => '1'));
				if ($prev_count == 0) {echo 'ok';} 
				else {
    ...
    {
        if($this->get_client_ip_nonmem()!='115.124.115.69'  && $this->get_client_ip_nonmem()!='182.73.101.70') {
        ...
        $flag       = 1;
        $exam_list  = array();
        //$examcodes  = array('528', '529', '530', '531', '534', '991', '997', '1031', '1032', '1052', '1054', '1053', '1062', '1063', '1002', '1003', '1004', '1005', '1009', '1013', '1014');
        $examcodes  = array('528', '529', '530', '531', '534', '991', '997', '1031', '1032', '1052', '1054', '1053', '1062', '1063', '1064', '1065', '1066', '1067', '1068', '1069','157');
    ...
    //Added by Priyanka W on 26/10/202 for Dipcert
    ...
    public function examdetails()
    {
			// if($this->get_client_ip() !='115.124.115.72' && $this->get_client_ip() !='182.73.101.70') {        
			//     echo 'Under maintenance'; exit;
			// }

		/*if (!in_array($this->session->userdata('nmregnumber'), $this->rpe_reschedule_arr)){
            $this->session->set_flashdata('error_nm_without_pass', 'Exam is not active, registration is closed.');
            redirect(base_url() . 'NonMember/examlist'); 
        }*/
        ...
        if ($examcode == 19 || $examcode == 156 || $examcode == 153) {
            $check_eligible_fresh_old_member = $this->master_model->getRecords('eligible_master', array('
                exam_code' => $examcode,'member_no' => $this->session->userdata('nmregnumber')));
            ...
            if (count($check_eligible_fresh_old_member) < 1) {
                ...
                $this->session->set_flashdata('error_nm_without_pass', $message);
                redirect(base_url() . 'NonMember/examlist'); 
            }   
            else if(count($check_eligible_fresh_old_member)>0) {
                if ($check_eligible_fresh_old_member[0]['app_category']=='R') {
                $message = 'The exam has been discontinued, and no new registrations will be accepted.';
                $this->session->set_flashdata('error_invalide_exam_selection', $message);
        ...
        if ($chk_exm_code_jaiib == $this->config->item('examCodeJaiib') || $chk_exm_code_jaiib == $this->config->item('examCodeDBF')) {
            exit;
        }

        /*For OLD BCBF Exam check in New BCBF Module*/
        if(base64_decode($this->input->get('excode2')) == "101" || base64_decode($this->input->get('excode2')) == "1046" || base64_decode($this->input->get('excode2')) == "1047"){
            ...
            if(isset($user_details_iibfbcbf) && count($user_details_iibfbcbf) > 0){
                ...
                redirect(base_url() . 'NonMember/accessdenied_not_old_bcbf_mem/');
            }
        }
        /*For OLD BCBF Exam check in New BCBF Module*/
        ...
        $get_ip_address = '';
	        $set_client_ip_address = array('182.73.101.70','115.124.115.75','106.216.246.171','106.216.243.125'); 
        ...
        if($check_valid_exam_flag == 1){
            ...
            //$this->session->set_flashdata('error_invalide_exam_selection', "This certificate course is applicable for <strong>Ordinary SBI staff only</strong>.");
            ...
            if(in_array($decode_exam_code,array(1031,1032))){
                ...
                $this->session->set_flashdata('error_invalide_exam_selection', "This certificate course is applicable for <strong>Ordinary Karur Vysya Bank staff only</strong>.");
            }

            redirect(base_url() . 'NonMember/accessdenied_not_sbi_mem/');
        } 
        //END CODE TO CHECK VALIDE EXAM CODE ACCORDING TO INSTITUTE CODE ADDED BY ANIL ON 11 Sep 2023
         ...
         //Added bby Priyanka W on 26/10/202 for Dipcert
            ...
            if(in_array($examCd,$this->config->item('examCodeDIPCERT'))){ 
                $memNo =  $this->session->userdata('nmregnumber');
				...
				if(count($res)>0)
        {
                    if(($res[0]['exam_status'] == 'F' || $res[0]['exam_status'] == 'A') && $res[0]['app_category'] == 'R' || $res[0]['app_category'] == 'B1_1')
                    {
                        if ($examCd == 8 || $examCd == 158) 
                        {
                            ...
                            {
                                redirect(base_url() . 'Applyexam/accessdenied_dipcert/');
                            }
                        }
                        else
                        {
    ...
    {
        /*if (!in_array($this->session->userdata('nmregnumber'), $this->rpe_reschedule_arr)){ 
            $this->session->set_flashdata('error','Exam is not active, registration is closed.');
            redirect(base_url() . 'NonMember/examlist');
        }*/
        ...
        $get_ip_address = '';
        $dipcert_set_client_ip_address = array('182.73.101.70','115.124.115.75'); 
        $dipcert_exam_codes_chk = array(8,11,19,78,79,119,151,153,154,156,157,158,163,165,166,220);
        
        if( in_array($this->session->userdata('examcode'),$dipcert_exam_codes_chk) && $this->get_client_ip_nonmem()!='115.124.115.75' && $this->get_client_ip_nonmem()!='182.73.101.70' )
        {
            // echo "<br>".$get_ip_address;
        ...
        if (in_array($this->session->userdata('examcode'), $excodes))
        {
          echo'This Exam is not active'; exit;
			...
			if($this->get_client_ip_nonmem()!='115.124.115.72'    && $this->get_client_ip_nonmem()!='115.124.115.75'    && $this->get_client_ip_nonmem()!='182.73.101.70'&& $this->get_client_ip_nonmem()!='192.168.11.253') 
			{
        ...
        if (isset($_POST['btnPreviewSubmit'])) {
            //exit;
            $scribe_flag       = 'N';
            $scannedphoto_file = $scannedsignaturephoto_file = $idproofphoto_file = $state = $password = $empidproofphoto_file = $declarationphoto_file = $var_errors = '';

            $is_fedai_fiels_eligible = $_POST['is_fedai_emp_declaration_valid']; 
            $empidproof_file = $declaration_file  = $declaration_form = '';
            ...
            if ($this->session->userdata('examcode') == 1009 && $is_fedai_fiels_eligible == 'yes') 
            {
            ...
            if ( $this->form_validation->run() == true ) {
                ...
                $selinstitute = $selinstitutionname = '';
                ...
                {      
                    ...
                    $this->db->where('fedai_institution_master.institude_id',$selinstitute);
                    $sel_nonmem_institute_data = $this->master_model->getRecords('fedai_institution_master', '', '', array('name' => 'asc'));

                    $selinstitutionname = $sel_nonmem_institute_data[0]['name'];
                }
                ...
                //echo $date_of_commenc_bc;die;
                  
                $bank_bc_id_card_file_path = $outputempidproof1 = $bank_bc_id_card_filename = $empidproof_file = '';
                if ($this->session->userdata('examcode') == 101 || $this->session->userdata('examcode') == 1046 || $this->session->userdata('examcode') == 1047) 
                {
                    ...
                    if (isset($_FILES['empidproofphoto']['name']) && ($_FILES['empidproofphoto']['name'] != '')) 
                    {
                        ...
                        if($this->session->userdata('nmregnumber') != ""){
                            $new_filename = 'bank_bc_id_card_'.$this->session->userdata('nmregnumber');
                        }else{
                    ...
                    if (isset($_POST['bank_bc_id_card_cropper']) && $_POST['bank_bc_id_card_cropper'] != "")
                    {
                        ...
                        if($this->session->userdata('nmregnumber') != ""){
                            $bank_bc_id_card_new_file_name = "bank_bc_id_card_" . $this->session->userdata('nmregnumber') . '.' . strtolower(pathinfo($bank_bc_id_card_cropper, PATHINFO_EXTENSION));
                        }else{
                    ...
                    if(!file_exists('./uploads/empidproof/' . $bank_bc_id_card_filename)){
                ...
                if(isset($_POST['el_subject'])){ //priyanka d - 06-feb-23 >> added if else both to get proper fee for nomember after select e-learning
                ...
                $user_data = array('email' => $_POST["email"],
                    ...
                    'el_subject'=>$el_subject, //priyanka d - 06-feb-23 >> added it to get proper fee for nomember after select e-learning
                    ...
                    'is_fedai_fiels_eligible' => $is_fedai_fiels_eligible, 
        ...
        //START CODE TO CHECK VALIDE EXAM CODE ACCORDING TO INSTITUTE CODE ADDED BY ANIL ON 11 Sep 2023
        //print_r($this->session->userdata('examcode'));
			$decode_exam_code = $this->session->userdata('examcode');//base64_decode($this->input->get('excode2'));
        $check_valid_exam_flag = $this->check_valid_exam_for_member($decode_exam_code);
        if($check_valid_exam_flag == 1){
            //$this->session->set_flashdata('error_invalide_exam_selection', "This certificate course is applicable for <strong>Ordinary SBI staff only</strong>.");
            if(in_array($decode_exam_code,array(1031,1032))){
        ...
        $this->db->where("eligible_master.app_category !=", 'R');
			...
			// $this->db->where("eligible_master.institute_id",0); // this '"eligible_master.institute_id",0' codition added by gaurav refered by pooja mane 
			
        $this->db->where('exam_master.exam_code', $this->session->userdata('examcode'));
			// $this->db->group_by('exam_master.exam_code');
        $examinfo = $this->master_model->getRecords('exam_master');
		...
		$checkJaiibFlag = $this->master_model->getRecords('eligible_master', array('exam_code' => $this->session->userdata('examcode'),  'eligible_period' => $examinfo[0]['exam_period'],'member_no' => $this->session->userdata('nmregnumber')),'',$order_by,0,1);
		$showOptForJaiib=0;
	 //   echo $this->db->last_query();
	//	echo'<pre>';print_r($checkJaiibFlag);exit;
	 ...
	 //   echo $showOptForJaiib;exit;
        ...
        if(!isset($_GET['optval']) && $this->session->userdata('selectedoptVal')!=0) {
            redirect(base_url().'/NonMember/comApplication/?optval='.$this->session->userdata('selectedoptVal'));
        }
        ...
        $arr_fedai_eligible = [];
        $is_fedai_emp_declaration_valid = 'no';
        $AssociateInstituteId = '';

			$ButtonDisable = true;

        if ($decode_exam_code != '' && $decode_exam_code == 1009 && $regnumber != '' && $exam_period != '') {
            $arr_fedai_eligible = $this->master_model->check_fedai_eligible($decode_exam_code,$exam_period,$regnumber);
            $arr_response = [];
                ...
                if (count($arr_response)>0) {
                    ...
                    if($examstatus !='F' && $examstatus!='') {
                        ...
                        if($examstatus=='P') {
							...
							if($examstatus=='D') {
								$msg = 'You have debarred selected exam';
							} 

							if($examstatus=='V') {
								$msg = 'Valid application exist, Attempt remaining.';
							}                        
        ...
        if ( $ExamCode == 1009  && ($AssociateInstituteId == '' || $AssociateInstituteId == null  )) 
        {                
            $fedai_institute_data = [];
            $this->db->where('fedai_institution_master.institution_delete', '0');
            $fedai_institute_data = $this->master_model->getRecords('fedai_institution_master', '', '', array('name' => 'asc'));
            $is_fedai_emp_declaration_valid = 'yes';
            // $exam_period = $examinfo[0]['exam_period'];
            // $arr_insdata = $this->iibf_fedai_institute_master_api($ExamCode,$exam_period,1);
            // if ( $arr_insdata['api_res_flag'] == 'success' ) {
            //     $fedai_institute_data = json_decode($arr_insdata['api_res_response'],true);
            // }
        }
        else
        {
        ...
        $exemptionres = $this->master_model->exemption_api_call_func($this->session->userdata('examcode'),$examinfo[0]['exam_period'],$this->session->userdata('nmregnumber'));
        if($exemptionres=='true' && $this->session->userdata('examcode') == '1007') {
                $showExemptionOption =1;
        }
					if($this->get_client_ip_nonmem()!='115.124.115.75'  && $this->get_client_ip_nonmem()!='182.73.101.70') {
						$showExemptionOption = 0; 
					}
        //echo $showExemptionOption;exit;
        $chk_exemption_application_exist = $this->master_model->chk_exemption_application($this->session->userdata('examcode'),$this->session->userdata('nmregnumber'));
        ...
        if ($cookieflag == 0 && $this->session->flashdata('error')=='') {
            $data = array('middle_content' => 'nonmember/exam_apply_cms_msg');
				...
				$data = array('benchmark_disability_info' => $benchmark_disability_info, 'middle_content' => 'nonmember/comApplication', 'states' => $states, 'undergraduate' => $undergraduate, 'fedai_institute_data' => $fedai_institute_data,'is_fedai_emp_declaration_valid'=>$is_fedai_emp_declaration_valid, 'AssociateInstituteId' => $AssociateInstituteId, 'sel_institute_data' => $sel_institute_data, 'graduate' => $graduate,'ButtonDisable' => $ButtonDisable, 'postgraduate' => $postgraduate, 'institution_master' => $institution_master, 'designation' => $designation, 'user_info' => $user_info, 'idtype_master' => $idtype_master, 'examinfo' => $examinfo, 'medium' => $medium, 'center' => $center, 'caiib_subjects' => $caiib_subjects, 'compulsory_subjects' => $compulsory_subjects,'showOptForJaiib'=>$showOptForJaiib,'selectedoptVal'=>$this->session->userdata('selectedoptVal'), 'old_bcbf_institute_data'=>$old_bcbf_institute_data,'showExemptionOption'=>$showExemptionOption); //priyanka d -,'showOptForJaiib'=>$showOptForJaiib -17-feb-23
        }
    ...
    public function iibf_fedai_eligible_api($exam_code=0, $exam_period=0,$member_no=0)
    {
        ...
        // $api_url= "http://10.10.233.66:8093/fedaieligibleapi/getFedaiEligible/1009/811/510280156";
        
			// $api_url= "http://10.10.233.76:8093/fedaieligibleapi/getFedaiEligible/1009/811/".$member_no;

        //NEW API ADDED BY GAURAV ON 2024-05-27          
			$api_url =  "http://10.10.233.76:8093/fedaieligibleapi/getFedaiEligible/".$exam_code."/".$exam_period."/".$member_no;  
    ...
    public function preview()
    {
        ...
        $decode_exam_code = $this->session->userdata('examcode');
			...
			if (($decode_exam_code == 101 || $decode_exam_code == 1046 || $decode_exam_code == 1047 || $decode_exam_code == 991 || $decode_exam_code == 997)) 
        {
            $images_flag = 0;
        ...
        if ($decode_exam_code == 1009) 
        {
            $images_flag = 0;
            if (!file_exists("uploads/empidproof/" . $this->session->userdata['examinfo']['empidproofphotoname'])) {
                $images_flag = 1;
            }
        }
        if ($decode_exam_code == 1009) 
        {    
            if (!file_exists("uploads/declaration/" . $this->session->userdata['examinfo']['declaration'])) {
                $images_flag = 1;
            } 
        ...
        //print_r($this->session->userdata['examinfo']['subject_arr']);die;
        ...
        //print_r($this->session->userdata['examinfo']['subject_arr']);
        //END : ADDED BY SAGAR M 2024-09-12
        /*echo $this->session->userdata('nmregnumber')."==";
        print_r($exam_date_arr);die;*/
         
        $check = $this->examapplied($this->session->userdata('nmregnumber'), $this->session->userdata['examinfo']['excd'],$exam_date_arr); 
        if (!$check) {
            //get medium
            $this->db->join('exam_activation_master', 'exam_activation_master.exam_code=medium_master.exam_code AND exam_activation_master.exam_period=medium_master.exam_period');
    ...
    public function details($order_no = null, $excd = null)
    {
        ...
        $applied_candidate = array();
        $applied_candidate = $this->master_model->getRecords('member_registration', array('regnumber' => $applied_exam_info[0]['regnumber']), 'firstname,middlename,lastname,regnumber');

        $data = array('middle_content' => 'nonmember/exam_applied_success', 'medium' => $medium, 'center' => $center, 'applied_exam_info' => $applied_exam_info, 'payment_info' => $payment_info, 'applied_candidate' => $applied_candidate);
        $this->load->view('nonmember/nm_common_view', $data);
    }
    ...
    public function examapplied($nmregnumber = null, $exam_code = null, $exam_date_arr=array())
    {
        //check where exam alredy apply or not
        $cnt        = 0;
        $today_date = date('Y-m-d');
        ...
        if (count($applied_exam_info) <= 0 && count($exam_date_arr) > 0)
        {
          ...
          $applied_exam_info = $this->master_model->getRecords('member_exam', array('regnumber' => $nmregnumber));
          ...
          if(count($applied_exam_info) > 0)//START : LOG ADDED BY SAGAR M ON 2024-10-02
          {
            ...
            $add_duplicate_log['regnumber'] = $nmregnumber;
            ...
            $email_send_arr['subject'] = 'Duplicate exam application prevented : '.$nmregnumber;
            $email_send_arr['message'] = 'The member '.$nmregnumber.' tried to apply for the duplicate exam date but redirected due to the check added by Anil & Sagar.';
        ...
        if(in_array($exCode, $examCd_Arr)){
            ...
            $applied_exam_info_old_bcbf = array();
            ...
            $applied_exam_info = $applied_exam_info_old_bcbf = $this->master_model->getRecords('member_exam', array('member_exam.exam_period' => $get_exam_period[0]['exam_period'], 'regnumber' => $nmregnumber));

            if(count($applied_exam_info_old_bcbf)){
                $cnt_old_bcbf = 1;
                return $cnt_old_bcbf;
            }
            //echo $this->db->last_query(); die;
    ...
    public function Msuccess()
    {
        ...
        if($this->session->userdata('examinfo')){
            ...
            if(count($exam_date_arr) > 0){
                ...
                $insert_dup_application_data['member_no'] = $this->session->userdata('nmregnumber');
                ...
                if(isset($applied_exam_info) && count($applied_exam_info) > 0){
                    ...
                    redirect(base_url() . 'NonMember/accessdenied_already_apply');
                }
            } 
        }
        ...
        if(in_array($exCode, $examCd_Arr)){
            ...
            $duplicate_applied_exam_chk = $this->master_model->getRecords('member_exam', array('member_exam.exam_period' => $this->session->userdata['examinfo']['eprid'], 'member_exam.regnumber' => $this->session->userdata('nmregnumber')));
            ...
            $insert_dup_oldbcbf_application_data['member_no'] = $this->session->userdata('nmregnumber');
        ...
        // END: OLD BCBF EXTRA CHECK ADDED BY ANIL
			...
			if(isset($this->session->userdata['examinfo']['el_subject']) &&(base64_decode($this->session->userdata['examinfo']['excd']) == $this->config->item('examCodeJaiib') || base64_decode($this->session->userdata['examinfo']['excd']) == $this->config->item('examCodeDBF') || base64_decode($this->session->userdata['examinfo']['excd']) ==$this->config->item('examCodeSOB')  || base64_decode($this->session->userdata['examinfo']['excd']) == 11)){
           ...
           //priyanka d =22-feb-23
            ...
            if ($insert_id = $this->master_model->insertRecord('member_exam', $inser_array, true)) 
            {
                ...
                $this->master_model->updateRecord('member_registration', array('associatedinstitute' => $AssociateInstituteId), array('regnumber' => $regnumber));
					...
					$log_title   = "Non Member member_exam table details";
					$log_message = serialize($inser_array);
					$rId         = $this->session->userdata('nmregnumber');
					$regNo       = $this->session->userdata('nmregnumber');
					storedUserActivity($log_title, $log_message, $rId, $regNo); //priyanka d - added log to cross check amount for SOB
                ...
                if($sess_exam_code == 101 || $sess_exam_code == 1046 || $sess_exam_code == 1047){
					...
					if(base64_decode($this->session->userdata['examinfo']['excd']) == 1009)
                {  
                    if ($this->session->userdata['examinfo']['empidproofphotoname'] != '') {
                        $update_array = array_merge($update_array, array("empidproofphoto" => $this->session->userdata['examinfo']['empidproofphotoname']));
                        $photo_name   = $this->master_model->getRecords('member_registration', array('regid' => $this->session->userdata('nmregid'), 'regnumber' => $this->session->userdata('nmregnumber')), 'empidproofphoto');
                        $empidproofphotoname = $photo_name[0]['empidproofphotoname'];
                    }   
                }
                if($sess_exam_code == 1009)
                {    
                    if ($this->session->userdata['examinfo']['declaration'] != '') {
                        $update_array = array_merge($update_array, array("declaration" => $this->session->userdata['examinfo']['declaration']));
                        $declaration   = $this->master_model->getRecords('member_registration', array('regid' => $this->session->userdata('nmregid'), 'regnumber' => $this->session->userdata('nmregnumber')), 'declaration');
                        $declaration    = $declaration[0]['declaration'];
                    }
                }    
    ...
    public function sbi_make_payment()
    {
        ...
        if (isset($_POST['processPayment']) && $_POST['processPayment']) {
            ...
            $checkpayment = $this->master_model->getRecords('payment_transaction', array('pay_type' => 2,'ref_id' => $this->session->userdata['examinfo']['insert_id']));
            //echo $this->db->last_query();exit;
                if (count($checkpayment) > 0) {
            ...
            if ($getcenter[0]['state_code'] == 'MAH') {
                ...
                if ($this->session->userdata['examinfo']['elearning_flag'] == 'Y') {
						if(base64_decode($this->session->userdata['examinfo']['excd']) == $this->config->item('examCodeSOB') || base64_decode($this->session->userdata['examinfo']['excd']) == 11) {  //priyanka d-added if else to get proper amount after select e-learning  - 27-apr-23
                        $cs_total        = $amount;
                    ...
                    //set an amount as per rate
                        ...
                        $total_el_amount = 0;$total_el_base_amount = 0;
                        $total_el_gst_amount  = 0;
                    }
            ...
            else 
            {
                ...
                if ($this->session->userdata['examinfo']['elearning_flag'] == 'Y') {
                        ...
                        $amount_base = $getfees[0]['elearning_fee_amt'];
                        $total_el_base_amount = 0;
                        $total_el_gst_amount  = 0;$total_el_amount      = 0;
                    }
    ...
    public function handle_billdesk_response()
    {
        ...
        if (isset($_REQUEST['transaction_response'])) {
            ...
            if ($auth_status == '0300' && $qry_api_response['auth_status'] == '0300' && $get_user_regnum[0]['status'] == 2) {
                ...
                //Main Code
                $get_user_regnum_new = $this->master_model->getRecords('payment_transaction', array('receipt_no' => $MerchantOrderNo), 'member_regnumber,ref_id');
                $this->db->join('center_master', 'center_master.center_code=member_exam.exam_center_code AND center_master.exam_name=member_exam.exam_code AND center_master.exam_period=member_exam.exam_period');
                $this->db->join('exam_master', 'exam_master.exam_code=member_exam.exam_code');
                $this->db->join('misc_master', 'misc_master.exam_code=member_exam.exam_code AND misc_master.exam_period=member_exam.exam_period');
                $this->db->join('exam_activation_master', 'exam_activation_master.exam_code=member_exam.exam_code AND exam_activation_master.exam_period=member_exam.exam_period');
                $exam_info_new = $this->master_model->getRecords('member_exam', array('regnumber' => $get_user_regnum_new[0]['member_regnumber'], 'member_exam.id' => $get_user_regnum_new[0]['ref_id']), 'member_exam.exam_code,member_exam.exam_mode,member_exam.exam_medium,member_exam.exam_period,center_master.center_name,member_exam.exam_center_code,exam_master.description,misc_master.exam_month');
                ...
                redirect(base_url() . 'NonMember/details/' . base64_encode($MerchantOrderNo) . '/' . base64_encode($exam_info_new[0]['exam_code']));
                exit();
             ...
             if($get_user_regnum[0]['status']!=0 && $get_user_regnum[0]['status']==2) {
                ...
                redirect(base_url() . 'NonMember/pending/' . base64_encode($MerchantOrderNo));
               }
            }
    ...
    public function pending($order_no = null)
    {
        ...
        $data = array('middle_content' => 'nonmember/exam_applied_pending', 'payment_info' => $payment_info);
        $this->load->view('nonmember/nm_common_view', $data);
    }
    ...
    public function notification()
    {
        ...
        if ((!file_exists('./uploads/photograph/' . $user_images[0]['scannedphoto']) || !file_exists('./uploads/scansignature/' . $user_images[0]['scannedsignaturephoto']) || !file_exists('./uploads/idproof/' . $user_images[0]['idproofphoto']) || $user_images[0]['scannedphoto'] == '' || $user_images[0]['scannedsignaturephoto'] == '' || $user_images[0]['idproofphoto'] == '') && (!is_file(get_img_name($this->session->userdata('nmregnumber'), 's')) || !is_file(get_img_name($this->session->userdata('nmregnumber'), 'p')))) {  
            ...
            if($this->session->userdata('nm_without_pass') && ($this->session->userdata('nm_without_pass') == 1)){
                $msg .= '<li>Kindly go to login page <a href="' . base_url() . 'Nonreg/logout/">click here</a> and click on Edit Profile to update the, "Photo/signature or ID proof" and then apply for exam. For any queries contact zonal office.</li>';
            }else{
        ...
        if ($user_images[0]['mobile'] == '' || $user_images[0]['email'] == '') {
            ...
            $flag = 0;
            if($this->session->userdata('nm_without_pass') && ($this->session->userdata('nm_without_pass') == 1)){
                $msg .= '<li>Kindly go to login page <a href="' . base_url() . 'Nonreg/logout/">click here</a> and click on Edit Profile to update the, "email id or mobile number" and then apply for exam. For any queries contact zonal office.</li>';
            }else{
                $msg .= '<li>
        ...
        if (validate_nonmemdata($this->session->userdata('nmregnumber'))) { 
            $flag = 0;
            if($this->session->userdata('nm_without_pass') && ($this->session->userdata('nm_without_pass') == 1)){
                $msg .= '<li>Kindly go to login page <a href="' . base_url() . 'Nonreg/logout/">click here</a> and click on Edit Profile to check all mandatory fields and update the, profile. For any queries contact zonal office.</li>';
            }else{
                $msg .= '<li>
    ...
    public function set_nonmem_elsub_cnt(){ // priyanka d - 01-feb-23 >> function added because fee wat not getting properly for e-learning
    ...
    public function accessdenied_not_sbi_mem()
    {
        $message = '<div style="color:#F00">This certificate course is applicable for <strong>Ordinary SBI staff only</strong>.</div>';
        $data    = array('middle_content' => 'nonmember/not_eligible', 'check_eligibility' => $message);
        $this->load->view('nonmember/nm_common_view', $data);
    }
    ...
    /* --------------------- EMAIL VERIFCATION CODE BY POOJA MANE ON : 2024-11-18---------------------*/
