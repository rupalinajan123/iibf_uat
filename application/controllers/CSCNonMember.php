...
class CSCNonMember extends CI_Controller {
	public function __construct()
	{ 
		parent::__construct();

				// echo '<div align="center"><h3>SITE IS UNDER MAINTENANCE WILL BE BACK SOON</h3>';
				// echo '<br/>';
	...
	public function memlogin(){
		...
		exit;*//*<p>
  Please be advised that the registration for BCBF examinations is temporarily put on hold till further notice.
</p>
<p><strong><span style="font-size: 24px;">.</span> IIBF</strong></p>*/
		/*<marquee width='auto' direction='right' height='80px' scrollamount='10'></marquee>*/
		
		echo '<h3 style="color:red; text-align: center;">Please be advised that the registration for BCBF examinations is temporarily put on hold till further notice.</h3><p style="text-align: right; margin-right: 316px;"><strong><span style="font-size: 24px;">.</span> <span style="font-size:20px;">IIBF</span></strong></p>';
		...
		if($check_exam_activation==0 || !in_array(base64_decode($this->input->get('ExId')),$exam_code_array)){
			redirect(base_url().'CSCNonMember/accessdenied/');
		}
				...
				if(count($user_info)){ 
					if($user_info[0]['isactive']==1){
						...
						$user_data=array('cscnmregid'=>$user_info[0]['regid'],
										 ...
										 'csctype'=>'exm',
										 'cscnmpassword'=>base64_encode($decpass)
										 );
						$this->session->set_userdata($user_data);
						$sess = $this->session->userdata();
						redirect(base_url().'CSCNonMember/showexam/?ExId='.$this->input->get('ExId').'&Extype='.$Extype);
					}elseif($user_info[0]['isactive']==0){
		...
		// $data['code']=$cap['word'];
		// $this->session->set_userdata('cscnonmemlogincaptcha', $cap['word']);
		$this->load->model('Captcha_model');                 
		$captcha_img = $this->Captcha_model->generate_captcha_img('cscnonmemlogincaptcha');
		$data['image'] = $captcha_img;
		$this->load->view('cscnonmember/nonmember_login',$data);
	}
	...
	public function showexam(){
		...
		if($flag==1 && $checkqualifyflag==0){
			redirect(base_url().'CSCNonMember/examdetails/?excode2='.$examcode.'&Extype='.$Extype); 
		}else{
  ...
  // ##---------End user profile (prafull)-----------##
	public function profile(){
		...
		$user_info = $this->master_model->getRecords('member_registration', array(
			'regid' => $this->session->userdata('cscnmregid') ,
			'regnumber' => $this->session->userdata('cscnmregnumber')
		));
		...
		$applicationNo = $this->session->userdata('cscnmregnumber');
		if (count($user_info)){
		...
		if (isset($_POST['btnSubmit'])){
			...
			if ($this->input->post('aadhar_card') != ''){
				$this->form_validation->set_rules('aadhar_card', 'Aadhar Card Number', 'trim|xss_clean|max_length[12]|min_length[12]|numeric|is_unique[member_registration.aadhar_card.isactive.1.regid.' . $this->session->userdata('cscnmregid') . '.registrationtype.' . $this->session->userdata('memtype') . ']');
			}
			$this->form_validation->set_rules('gender', 'Gender', 'trim|required');
			$this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email|is_validunique[member_registration.email.regid.' . $this->session->userdata('cscnmregid') . '.isactive.1.registrationtype.' . $this->session->userdata('memtype') . ']|xss_clean');
			...
			$oldfilepath_photo = get_img_name($this->session->userdata('cscnmregnumber') , 'p');
			if ($oldfilepath_photo != ''){
				$p = '';
				$p = strpos($oldfilepath_photo, $this->session->userdata('cscnmregnumber'));
				if ($p == ''){
					$oldfilepath_photo = '';
				}
				if ($oldfilepath_photo == ''){
		...
		$oldfilepath_s = get_img_name($this->session->userdata('cscnmregnumber') , 's');
		if ($oldfilepath_s != ''){
			$s = '';
			$s = strpos($oldfilepath_s, $this->session->userdata('cscnmregnumber'));
			if ($s == ''){
				$oldfilepath_s = '';
			}
			if ($oldfilepath_s == ''){
		...
		// get idproof path
		$oldfilepath_pr = get_img_name($this->session->userdata('cscnmregnumber') , 'pr');
		if ($oldfilepath_pr != ''){
			$pr = '';
			$pr = strpos($oldfilepath_pr, $this->session->userdata('cscnmregnumber'));
			if ($pr == ''){
				$oldfilepath_pr = '';
			}
		...
		if (!file_exists($oldfilepath_photo))
		{
			if (isset($_FILES['scannedphoto']['name']))
			{
				...
				{
					$this->form_validation->set_rules('scannedphoto', 'scanned Photograph', 'file_required|file_allowed_type[jpg,jpeg]|file_size_min[20]|file_size_max[50]');
				}
			}
		}
		if (!file_exists($oldfilepath_s))
		{
			if (isset($_FILES['scannedsignaturephoto']['name']))
			{
				...
				{
					$this->form_validation->set_rules('scannedsignaturephoto', 'Scanned Signature Specimen', 'file_required|file_allowed_type[jpg,jpeg]|file_size_min[20]|file_size_max[50]');
				}
			}
		}
		if (!file_exists($oldfilepath_pr))
		...
		if($kyc_edit_flag==1)
		{
				if ($this->master_model->updateRecord('member_registration', $update_data_image, array(
					...
					{	$desc['updated_data'] = $update_data_image;
						...
						log_profile_user($log_title = "Profile updated successfully", $finalStr, 'image', $this->session->userdata('cscnmregid') , $this->session->userdata('cscnmregnumber'));
						if ($kyc_edit_flag == 1)
						{
							...
							if (count($kycmemdetails) > 0)
							{
								...
								$this->db->like('allotted_member_id', $this->session->userdata('cscnmregnumber'));
								$this->db->or_like('original_allotted_member_id', $this->session->userdata('cscnmregnumber'));
								...
								if (count($check_duplicate_entry) > 0)
								{
									...
									{
										$allotted_member_id = $this->removeFromString($row['allotted_member_id'], $this->session->userdata('cscnmregnumber'));
										$original_allotted_member_id = $this->removeFromString($row['original_allotted_member_id'], $this->session->userdata('cscnmregnumber'));
								...
								if ($kycmemdetails[0]['kyc_status'] == '0')
								{
									...
									$this->KYC_Log_model->create_log('kyc member edited images', '', '', $this->session->userdata('cscnmregnumber') , serialize($desc));
								}
								// check membership count
								$check_membership_cnt = $this->master_model->getRecords('member_idcard_cnt', array(
									'member_number' => $this->session->userdata('cscnmregnumber')
								));
	...
	//echo $kyc_edit_flag ;exit;
				...
				{//echo '**';exit;
					...
					redirect(base_url('CSCNonMember/profile/'));	
				}
				$edited = array();
				$edited = '';
				if (count($update_data))
				{
					...
					)))
					{
						...
						if (count($emailerstr) > 0)
						{
							...
							$newstring = str_replace("#application_num#", "" . $this->session->userdata('cscnmregnumber') . "", $emailerstr[0]['emailer_text']);
							...
							{
								redirect(base_url('CSCNonMember/acknowledge/'));
							}
							else
							{
					...
					else
					{
						...
						/* User Log Activities : Bhushan */
						$log_title = "Profile update error id:" . $this->session->userdata('cscnmregid');
						$log_message = serialize($desc);
						$rId = $this->session->userdata('cscnmregid');
						$regNo = $this->session->userdata('cscnmregnumber');
						storedUserActivity($log_title, $log_message, $rId, $regNo);
	...
	$data = array(
		'middle_content' => 'cscnonmember/userprofile',
		'states' => $states,
   ...
   { 
		...
		$data=array('middle_content'=>'cscnonmember/edif_profile_thankyou','application_number'=>$this->session->userdata('cscnmregnumber'),'password'=>base64_decode($this->session->userdata('cscnmpassword')));
		$this->load->view('cscnonmember/nm_common_view',$data);
	}
	// ##---------Edit Images(Prafull)-----------##
	public function editimages()
	{
			...
			$kyc_edit_flag=0;
			$flag=1;
			$member_info=$this->master_model->getRecords('member_registration',array('regid'=>$this->session->userdata('cscnmregid')),'scannedphoto,scannedsignaturephoto,idproofphoto');
			$applicationNo = $this->session->userdata('cscnmregnumber');
			if(isset($_POST['btnSubmit']))  	
			 {
				...
				if($this->form_validation->run()==TRUE)
				{
					...
					$prev_signature_flg = "N";
					$prev_id_flg = "N";
					$prev_edited_on_qry = $this->master_model->getRecords('member_registration',array('regid'=>$this->session->userdata('cscnmregid')),'DATE(images_editedon) images_editedon,photo_flg,signature_flg,id_flg');
					if(count($prev_edited_on_qry))
					{
						$prev_edited_on = $prev_edited_on_qry[0]['images_editedon'];
						$prev_photo_flg = $prev_edited_on_qry[0]['photo_flg'];
						...
						if($prev_edited_on != date('Y-m-d'))
						{
							$this->master_model->updateRecord('member_registration', array('photo_flg'=>'N', 'signature_flg'=>'N', 'id_flg'=>'N'), array('regid'=>$this->session->userdata('cscnmregid')));
						}
					}
					$date=date('Y-m-d h:i:s');
					$scannedphoto_file = '';
					...
					if($_FILES['scannedsignaturephoto']['name']!='')
					{
						...
						if($uploadData)
						{
							$kyc_edit_flag=1;$kyc_update_data['edited_mem_sign']=1;
							$scannedsignaturephoto_file = $uploadData['file_name'];
							$signature_flg = 'Y';
							$edited .= 'SIGNATURE || ';
						}
					...
					if($_FILES['idproofphoto']['name']!='')
					{
						...
						if($uploadData)
						{
							$kyc_edit_flag=1;$kyc_update_data['edited_mem_proof']=1;
							$idproofphoto_file = $uploadData['file_name'];
							$id_flg = 'Y';
							$edited .= 'PROOF || ';
						}
					...
					if($flag==1)
					{
						...
						if($this->master_model->updateRecord('member_registration',$update_info,array('regid'=>$this->session->userdata('cscnmregid'),'regnumber'=>$this->session->userdata('cscnmregnumber'))))
						{
							$desc['updated_data'] = $update_info; 
							$desc['old_data'] = $member_info[0];
							/* User Log Activities : Bhushan */
						...
						if($kyc_edit_flag==1)
						{
							$kycmemdetails=$this->master_model->getRecords('member_kyc',array('regnumber'=>$this->session->userdata('cscnmregnumber')),'',array('kyc_id'=>'DESC'),'0','1');
							if(count($kycmemdetails) > 0)
							{
								$kyc_update_data['user_edited_date']=date('Y-m-d H:i:s');
								$kyc_update_data['kyc_state']='2';
							...
							//check membership count
							$check_membership_cnt=$this->master_model->getRecords('member_idcard_cnt',array('member_number'=>$this->session->userdata('cscnmregnumber')));
							if(count($check_membership_cnt) > 0)
							{
							$this->master_model->updateRecord('member_idcard_cnt',array('card_cnt'=>'0'),array('member_number'=>$this->session->userdata('cscnmregnumber')));
							/* Close update dowanload count */
							}
						/* User Log Activities : Bhushan */
						$log_title ="kyc member edited images id : ".$this->session->userdata('cscnmregid');
						$log_message = serialize($desc);
						$rId = $this->session->userdata('cscnmregid');
						$regNo = $this->session->userdata('cscnmregnumber');
						storedUserActivity($log_title, $log_message, $rId, $regNo);
						/* Close User Log Actitives */
							...
							log_profile_user($log_title = "Profile updated successfully", $finalStr,'image',$this->session->userdata('cscnmregid'),$this->session->userdata('cscnmregnumber'));
							if(!is_file(get_img_name($this->session->userdata('cscnmregnumber'),'pr')) || !is_file(get_img_name($this->session->userdata('cscnmregnumber'),'s')) || !is_file(get_img_name($this->session->userdata('cscnmregnumber'),'p')) || validate_edit_nonmemdata($this->session->userdata('cscnmregnumber')))
							{
							...
							$sms_template_id = 'IZAU5IQGR';
							if(count($emailerstr) > 0)
							{
								$member_info=$this->master_model->getRecords('member_registration',array('regid'=>$this->session->userdata('cscnmregid')),'email');
								$newstring = str_replace("#application_num#", "".$this->session->userdata('cscnmregnumber')."",  $emailerstr[0]['emailer_text']);
								$final_str= str_replace("#password#", "".base64_decode($this->session->userdata('cscnmpassword'))."",  $newstring);
								$info_arr=array(
															'to'=>$member_info[0]['email'],
															'from'=>$emailerstr[0]['from'],
															'subject'=>$emailerstr[0]['subject'],
								...
								{
									$this->session->set_flashdata('error','Error while sending email !!');
									redirect(base_url('CSCNonMember/editimages/'));
								}
							}
							else
							{
		...
		$data=array('middle_content'=>'cscnonmember/edit_images','member_info'=>$member_info);
		$this->load->view('cscnonmember/nm_common_view',$data);
	}
	...
	public function changepass()
	{
		...
		if(isset($_POST['btn_password']))
		{
			...
			if($this->form_validation->run())
			{
				...
				$row=$this->master_model->getRecordCount('member_registration',array('usrpassword'=>$curr_encrypass,'regid'=>$this->session->userdata('cscnmregid')));
				if($row==0)
				{
					$this->session->set_flashdata('error','Current Password is Wrong.'); 
					redirect(base_url().'CSCNonMember/changepass/');
				}
				else
				{
					...
					$this->master_model->updateRecord('member_registration',$input_array,array('regid'=>$this->session->userdata('cscnmregid'),'regnumber'=>$this->session->userdata('cscnmregnumber')));
					/* User Log Activities : Bhushan */
					...
					storedUserActivity($log_title, $log_message, $rId, $regNo);
					/* Close User Log Actitives */
					$this->session->unset_userdata('cscnmpassword');
					$this->session->set_userdata("cscnmpassword",base64_encode($new_pass));
		...
		$data=array('middle_content'=>'cscnonmember/change_pass',$data);
		$this->load->view('cscnonmember/nm_common_view',$data);
	}
	##GST Message
	public function GST()
	{
		...
		$data=array('middle_content'=>'cscnonmember/nmmember_notification','msg'=>$msg);
		$this->load->view('cscnonmember/nm_common_view',$data);
	}
	...
	public function examlist()
	{
			...
			if($GST_val==2)
			{
				redirect(base_url() . 'CSCNonMember/GST');
			}
			...
			if(!is_file(get_img_name($this->session->userdata('cscnmregnumber'),'s')) || !is_file(get_img_name($this->session->userdata('cscnmregnumber'),'p'))  || !is_file(get_img_name($this->session->userdata('cscnmregnumber'),'pr')) || validate_nonmemdata($this->session->userdata('cscnmregnumber')))
			{
				 // priyanka d- added id-proof condition to restrict user to apply for exam before upload id-proof and reset conditions are added previously- 29-dec-22
				redirect(base_url().'CSCNonMember/notification');
			}
		 ...
		 $data=array('middle_content'=>'cscnonmember/nm_exam_list','exam_list'=>$exam_list);
		 $this->load->view('cscnonmember/nm_common_view',$data);
	}
	...
	{		$this->chk_session->chk_cscnon_member_session();
			...
			{
				redirect(base_url().'CSCNonMember/examdetails/?excode2='.$this->input->get('excode2').'&'.'Extype='.$this->input->get('Extype'));
			}
			 ...
			 if($is_transaction_doone >0)
			 {
				...
				{	
					...
					if(count($eligible_info) > 0){
						redirect(base_url().'Remote_CSCNonMember/comApplication/?excode='.base64_encode($examcode).'&Exprd='.base64_encode($eligible_info[0]['eligible_period']));
					}
					$exam_info=$this->master_model->getRecords('exam_master',array('exam_code'=>$examcode));
					$data=array('middle_content'=>'cscnonmember/cms_page','exam_info'=>$exam_info);
					$this->load->view('cscnonmember/nm_common_view',$data);
				}
		}
	...
	public function comApplication()
	{
		...
		if(isset($_POST['btnPreviewSubmit']))  	
		{
			...
			{
			$this->form_validation->set_rules('scannedsignaturephoto','Scanned Signature Specimen','file_required|file_allowed_type[jpg,jpeg]|file_size_min[20]|file_size_max[50]|callback_scannedsignaturephoto_upload');
			}

			$scannedphoto_req_flg = $scannedsignaturephoto_req_flg = $bank_bc_id_card_req_flg  = $empidproofphoto_req_flg = 'required|';
			...
			if($this->session->userdata('examcode') == 1052 || $this->session->userdata('examcode') == 1054){ 
    			...
    			if($corporate_bc_option == "CSC"){
    				...
    				redirect(base_url().'CSCNonMember/comApplication');
    			}
			...
			/*1052*/
                ...
                if ($this->session->userdata('examcode') == 1052 || $this->session->userdata('examcode') == 1054) 
                {
                    ...
                    if (isset($_FILES['empidproofphoto']['name']) && ($_FILES['empidproofphoto']['name'] != '')) 
                    {
                        ...
                        if($this->session->userdata('cscnmregnumber') != ""){
                        	$new_filename = 'bank_bc_id_card_'.$this->session->userdata('cscnmregnumber');
                        }else{
                    ...
                    if (isset($_POST['bank_bc_id_card_cropper']) && $_POST['bank_bc_id_card_cropper'] != "")
					{
						...
						if($this->session->userdata('cscnmregnumber') != ""){
                        	//$new_filename = 'bank_bc_id_card_'.$this->session->userdata('cscnmregnumber');
                        	$bank_bc_id_card_new_file_name = "bank_bc_id_card_" . $this->session->userdata('cscnmregnumber') . '.' . strtolower(pathinfo($bank_bc_id_card_cropper, PATHINFO_EXTENSION));
                        }else{
                        	//$new_filename = 'bank_bc_id_card_'.$tmp_inputempidproof;
                ...
                if($corporate_bc_option == "CSC"){
					...
					/* User Log Activities : Bhushan */
					$log_title ="CSC Non Member exam apply details";
					$log_message = serialize($user_data);
					$rId = $this->session->userdata('cscnmregid');
					$regNo = $this->session->userdata('cscnmregnumber');
					storedUserActivity($log_title, $log_message, $rId, $regNo);
					/* Close User Log Actitives */
					redirect(base_url().'CSCNonMember/preview');
				}
				else
				{
	...
	public function preview()
	{
		...
		{		
			...
			{
				$data=array('middle_content'=>'cscnonmember/exam_preview','user_info'=>$user_info,'medium'=>$medium,'center'=>$center,'misc'=>$misc,'compulsory_subjects'=>$this->session->userdata['examinfo']['subject_arr']);
			}
			$this->load->view('cscnonmember/nm_common_view',$data);
		}
		else
	...
	//payment detail
		...
		$data=array('middle_content'=>'cscnonmember/exam_applied_success','medium'=>$medium,'center'=>$center,'applied_exam_info'=>$applied_exam_info,'payment_info'=>$payment_info);
		$this->load->view('cscnonmember/nm_common_view',$data);
	}
	...
	public function applydetails()
	{
		...
		$data=array('middle_content'=>'cscnonmember/exam_applied_success_withoutpay','medium'=>$medium,'center'=>$center,'applied_exam_info'=>$applied_exam_info);
		$this->load->view('cscnonmember/nm_common_view',$data);
	}
		...
		if(isset($_POST['btnPreview']))
		{
	        ...
	        if(in_array($exCode, $examCd_Arr)){
	            ...
	            if(isset($duplicate_applied_exam_chk) && count($duplicate_applied_exam_chk) > 0)
	            {
	                ...
	                redirect(base_url() . 'CSCNonMember/accessdenied_already_apply');
	            }
			...
			if($insert_id=$this->master_model->insertRecord('member_exam',$inser_array,true))
			{
				...
				/* Close User Log Actitives */
				 ...
				 //redirect(base_url().'CSCNonMember/connect');
	...
	public function wallet_make_payment()
	{
      ...
      {        
        require_once $_SERVER['DOCUMENT_ROOT'] . '/BridgePG/PHP_BridgePG/BridgePGUtil.php';//PRODUCTION URL        
			...
			$csc_success_url = base_url() . "CSCNonMember/csc_transsuccess";
			$csc_fail_url = base_url() . "CSCNonMember/csc_transfail";
			...
			if(count($getcenter) > 0){
				...
				#########get message if capacity is full##########
				$log_title ="CSC exam invoice CSCNonmember  session value:".$this->session->userdata['examinfo']['excd'];
			...
			#########get message if capacity is full##########
			$log_title ="CSC exam invoice CSCNonmember  get fee:".$querygetfees.'='.$querygetcenter;
		...
		$invoice_insert_array=array('pay_txn_id'=>$pt_id,
			...
			#########get message if capacity is full##########
			$log_title ="CSC exam invoice CSCNonMember:".$query;
    ...
    {        
      require_once $_SERVER['DOCUMENT_ROOT'] . '/BridgePG/PHP_BridgePG/BridgePGUtil.php';//PRODUCTION URL        
		...
		if ($params['txn_status'] == '100')
		{
			...
			redirect(base_url().'CSCNonMember/acknowledge/'.base64_encode($MerchantOrderNo));
			}
		}else{
		...
		$this->db->join('exam_activation_master','exam_activation_master.exam_code=member_exam.exam_code AND exam_activation_master.exam_period=member_exam.exam_period');
		$exam_info=$this->master_model->getRecords('member_exam',array('regnumber'=>$get_user_regnum[0]['member_regnumber'],'member_exam.id'=>$get_user_regnum[0]['ref_id']),'member_exam.exam_code,member_exam.exam_mode,member_exam.exam_medium,member_exam.exam_period,center_master.center_name,member_exam.exam_center_code,exam_master.description,misc_master.exam_month');
		redirect(base_url().'CSCNonMember/details/'.base64_encode($MerchantOrderNo).'/'.base64_encode($exam_info[0]['exam_code']));
	}
    ...
    {        
      require_once $_SERVER['DOCUMENT_ROOT'] . '/BridgePG/PHP_BridgePG/BridgePGUtil.php';//PRODUCTION URL        
		...
		$customer_id=$params['merchant_id']; 
			...
			//END OF SBI CALLBACK
			//Main Code
			redirect(base_url().'CSCNonMember/fail/'.base64_encode($MerchantOrderNo));
	}
	...
	public function fail($order_no=NULL)
	{
		...
		$data=array('middle_content'=>'cscnonmember/exam_applied_fail','payment_info'=>$payment_info);
		$this->load->view('cscnonmember/nm_common_view',$data);
	}
