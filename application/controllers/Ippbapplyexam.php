...
class Ippbapplyexam extends CI_Controller
{
        ...
        if ($this->input->post('form_type') == 'search_form') {
            ...
            if ($this->form_validation->run() == TRUE) {
				...
				if (count($aCandidate_ar) > 0) {
					...
					$emp_id=$aCandidate['emp_id'];
					//priyanka d- 20-july-23 >> update ippb_emp_id based on mob no
					$this->db->where_in('excode', array(997,1053));
					$checkEmpIdExist = $this->master_model->getRecords('member_registration', array('mobile' => $aCandidate['mobile'],'isactive' => '1'));

					

					if(!empty($checkEmpIdExist)) {

						$log_title ="IPPB ippb_emp_id check:".$emp_id;
						$log_message = serialize($checkEmpIdExist);
						$rId = $emp_id;
						$regNo = $emp_id;
						storedUserActivity($log_title, $log_message, $rId, $regNo);
						if($checkEmpIdExist[0]['ippb_emp_id']=='0') {

							$updateData=array('ippb_emp_id'=>$emp_id);

							$this->master_model->updateRecord('member_registration',$updateData,array('regnumber'=>$checkEmpIdExist[0]['regnumber']));

							$log_title ="IPPB ippb_emp_id update:".$emp_id;
					...
					$member_candidate = $this->master_model->getRecords('member_registration', array('ippb_emp_id' => $aCandidate['emp_id'],'isactive' => '1','registrationtype' => 'NM'));
					//Remove 'email' => $aCandidate['email'],condition in order to fetch previous unsuccessful application only based on mobile number Pooja Mane 2023-7-6
    ...
    public function candidate_details($emp_id)
    {
		...
		if ($emp_id) {
			...
			$member_candidate = $this->master_model->getRecords('member_registration', array('ippb_emp_id' => $searchStr,'isactive' => '1','registrationtype' => 'NM'));
			 //Remove 'email' => $aCandidate['email'],condition in order to fetch previous unsuccessful application only based on mobile number Pooja Mane 2023-7-6
				...
				if (count($member_candidate) > 0) {
					...
					if( isset($aCandidate['regnumber'])){
						...
						if($cookieflag==0)
						{
							...
							return $this->load->view('ippbapplyexam/nm_common_view',$data);
						}
		...
		if(isset($_POST['btnSubmit']))  	
		{
				...
				$this->db->limit(1);
				$check_user = $this->master_model->getRecords('member_registration',array('ippb_emp_id'=>$this->input->post('emp_id')),'regid,createdon');
				...
				if(! isset($aCandidate['regnumber'])){
					  ...
					  if(!isset($_POST['drafthiddenphoto']) || !file_exists('./uploads/saveddraft/photograph/'.$_POST['drafthiddenphoto'])) { //priyanka d - added this if condition for save as draft >> 21-07-23
					  ...
					  if(!isset($_POST['drafthiddenscansignature']) || !file_exists('./uploads/saveddraft/scansignature/'.$_POST['drafthiddenscansignature'])) { //priyanka d - added this if condition for save as draft >> 21-07-23
					...
					if(!isset($_POST['drafthiddenidproofphoto']) || !file_exists('./uploads/saveddraft/idproof/'.$_POST['drafthiddenidproofphoto'])) { //priyanka d - added this if condition for save as draft >> 21-07-23
				...
				*/ // commented this by priyanka D >> 11-july-23 >> we dont need for this check as we just checking if entered adhar number is exist or not in callback func
				...
				if($this->input->post('aadhar_card') != '' )
				{
					...
					//priyanka d >> 11-july-23 added below condition
				...
				//priyanka d >> 12-july-23 added below condition
				$this->form_validation->set_rules('emp_id','Employee ID','trim|required|xss_clean|callback_check_candidate_details');	
				...
				if($examcode == 1053){
	                ...
	                $this->form_validation->set_rules('bank_emp_id', 'Bank Employee Id', 'alpha_numeric_spaces|max_length[20]|xss_clean'); // Added By Anil S on 25 July 2025
				...
				if($this->form_validation->run()==TRUE)
				{
					...
					if(! isset($aCandidate['regnumber'])){
						...
						//priyanka d - added this elseif condition for save as draft >> 19-06-23
						else if(isset($_POST['drafthiddenphoto']) && file_exists('./uploads/saveddraft/photograph/'.$_POST['drafthiddenphoto'])) {
							
							copy('./uploads/saveddraft/photograph/'.$_POST['drafthiddenphoto'], './uploads/photograph/'.$_POST['drafthiddenphoto']);

							$scannedphoto_file = $_POST['drafthiddenphoto'];
							$outputphoto1 = base_url()."uploads/photograph/".$scannedphoto_file;
						...
						//priyanka d - added this elseif condition for save as draft >> 19-06-23
						else if(isset($_POST['drafthiddenscansignature']) && file_exists('./uploads/saveddraft/scansignature/'.$_POST['drafthiddenscansignature'])) {
							
							copy('./uploads/saveddraft/scansignature/'.$_POST['drafthiddenscansignature'], './uploads/scansignature/'.$_POST['drafthiddenscansignature']);

							$scannedsignaturephoto_file = $_POST['drafthiddenscansignature'];
							$outputsign1 = base_url()."uploads/scansignature/".$scannedsignaturephoto_file;
						...
						//priyanka d - added this elseif condition for save as draft >> 19-06-23
						else if(isset($_POST['drafthiddenidproofphoto']) && file_exists('./uploads/saveddraft/idproof/'.$_POST['drafthiddenidproofphoto'])) {
							
							copy('./uploads/saveddraft/idproof/'.$_POST['drafthiddenidproofphoto'], './uploads/idproof/'.$_POST['drafthiddenidproofphoto']);

							$idproof_file = $_POST['drafthiddenidproofphoto'];
							$outputidproof1 = base_url()."uploads/idproof/".$idproof_file;
						...
						/* >> priyanka d >>commented this and added above code >> 21-july-23
						$scannedphoto_file = $aCandidate['scannedphoto'];
						$outputphoto1 = base_url()."uploads/photograph/".$scannedphoto_file;
						$scannedsignaturephoto_file=$aCandidate['scannedsignaturephoto'];
						$outputsign1 = base_url()."uploads/scansignature/".$scannedsignaturephoto_file;
                ...
                $date_of_commenc_bc = $bank_bc_id_card_file_path = $bank_bc_id_card_filename = '';
                if ($examcode == 1053) 
                {
                    ...
                    if (isset($_FILES['empidproofphoto']['name']) && ($_FILES['empidproofphoto']['name'] != '')) 
                    {
                        ...
                        if(isset($aCandidate['regnumber']) && $aCandidate['regnumber'] != ""){
                        	$new_filename = 'bank_bc_id_card_'.$aCandidate['regnumber'];
                        }else{
                        ...
                        if ($size) 
                        {
                            ...
                            {
                                $this->session->set_flashdata('error', 'Bank BC Card ID :' . $this->upload->display_errors());
                                redirect(base_url().'Ippbapplyexam/candidate_details/'.base64_encode($emp_id));
                            }
                        } 
                        else 
					...
					if (isset($_POST['bank_bc_id_card_cropper']) && $_POST['bank_bc_id_card_cropper'] != "")
					{
						...
						if(isset($aCandidate['regnumber']) && $aCandidate['regnumber'] != ""){
                        	//$new_filename = 'bank_bc_id_card_'.$aCandidate['regnumber'];
                        	$bank_bc_id_card_new_file_name = "bank_bc_id_card_" . $aCandidate['regnumber'] . '.' . strtolower(pathinfo($bank_bc_id_card_cropper, PATHINFO_EXTENSION));
                        }else{
                        	//$new_filename = 'bank_bc_id_card_'.$tmp_inputempidproof;
					...
					//echo $scannedphoto_file.'='.$scannedsignaturephoto_file.'='.$idproof_file;exit;
					if($scannedphoto_file!='' && $idproof_file!='' && $scannedsignaturephoto_file!='')
					{
						$user_data=array(	'firstname'				=>$_POST["firstname"],
						                    ...
						                    'bank_emp_id' => (isset($_POST['bank_emp_id']) ? $_POST['bank_emp_id']:''),
						                    'bank_bc_id_no' => (isset($_POST['ippb_emp_id']) ? $_POST['ippb_emp_id']:''),
											...
											'selection_csc_nseit'	=>$_POST['selection_csc_nseit'] // priyanka d- 01-jun-23 >> for ippb csc/nseit
                                        );
        ...
        if (count($aCandidate) > 0) {
			...
			if( isset($aCandidate['regnumber'])){
	            ...
	            $data = array(
	                'middle_content' => 'ippbapplyexam/ippb_exam_form', 'states' => $states, 'undergraduate' => $undergraduate, 'graduate' => $graduate, 'postgraduate' => $postgraduate,'examinfo' => $examinfo, 'medium' => $medium, 'center' => $center, 'idtype_master' => $idtype_master, 'compulsory_subjects' => $compulsory_subjects, 'aCandidate' => $aCandidate,'old_bcbf_institute_data' => $old_bcbf_institute_data
	            );
	        }
        }
	...
	public function check_duplicate_aadhar() //priyanka d >> 11-july-23 added below condition
	{
		if($this->input->post('aadhar_card') =='') {
			echo'ok';
			return true;
		}
		//$this->db->where('registrationtype', 'NM');
		$this->db->where('isactive', '1');
		$this->db->where('aadhar_card', $this->input->post('aadhar_card') );
		$this->db->where('ippb_emp_id!=', $this->input->post('emp_id') );
		$checkDuplicateAdharNo = $this->master_model->getRecordCount('member_registration');
		//echo $this->db->last_query();
		if($checkDuplicateAdharNo==0) {
			echo'ok';
			return true;
		}
		else
    ...
    public function addmember()
    {
        ...
        //check email,mobile duplication on the same time from different browser!!
        $check_duplication = $this->master_model->getRecords('member_registration', array('ippb_emp_id' => $this->session->userdata['enduserinfo']['emp_id'],'isactive' => '1','registrationtype' => 'NM','regnumber !=' => '' ));
        ...
        // echo $this->db->last_query();exit;
        {
            ...
            $bank_emp_id = $name_of_bank_bc = $bank_bc_id_no = $date_of_commenc_bc = $bank_bc_id_card = $are_you_corporate_bc = $corporate_bc_option = $corporate_bc_associated = ''; 
		      if ($this->session->userdata['enduserinfo']['excd'] == 1053)
		      {
		        ...
		        $bank_bc_id_no = $this->session->userdata['enduserinfo']['bank_bc_id_no'];
            ...
            if(count($check_duplication) > 0)
            {
                ...
                {                    
                    ...
                    $inser_exam_array=array(	'regnumber'=>$regnumber,
												...
												'selected_vendor'=>$this->session->userdata['enduserinfo']['selection_csc_nseit'], //priyanka d >>01-jun-23 
                                            );
                ...
                $insert_info = array(
			        ...
			        'corporate_bc_option' => $corporate_bc_option,
			        'corporate_bc_associated' => $corporate_bc_associated,
					'ippb_emp_id'=>$this->session->userdata['enduserinfo']['emp_id'], // priyanka d -- 11-july- >> added ippb emp id
                );
	...
	//cross check candidate details
	public function check_candidate_details() {
		...
		$candidateDetails = $this->master_model->getRecordCount('member_registration_ippb');
		//echo $this->db->last_query();
		if($candidateDetails==0) {
			$str='Please Entered correct mobile no, email id';
			$this->form_validation->set_message('check_candidate_details', $str); 
			return false;
		}
		else
		{
	...
	public function check_aadhar()
	{
		...
		//above code added >> priyanka d >>11-july-23
		///end 
		if($aadhar_card!="")
		{
			$where="registrationtype='NM'";
	...
	public function sbi_make_payment()
	{
...

		{
			...
			$this->db->where_in('exam_code', array(997,1053));
			$checkRecordAlExist=$this->master_model->getRecords('payment_transaction',array('ref_id'=>$member_exam_id));
        ...
        if (isset($_REQUEST['transaction_response'])) 
			...
			{				
				...
				if($this->db->affected_rows())
				{
					...
					//$this->db->where('excode','997');
					$this->db->where_in('excode', array(997,1053));
					$result=$this->master_model->getRecords('member_registration',array(),'firstname,middlename,lastname,address1,address2,address3,address4,district,city,email,mobile,office,pincode,state_master.state_name,usrpassword,email,scannedphoto,scannedsignaturephoto,idproofphoto,bank_bc_id_card');	
					...
					$bank_bc_id_card_file = 'bank_bc_id_card_'.$applicationNoIppb.'.jpg';
					...
					if(count($upd_files)>0)
					{
						...
						//	$this->db->or_where('regnumber',$reg_id);
							...
							*/
							...
							//$this->db->where('excode','997');
							$this->db->where_in('excode', array(997,1053));
							$newresult=$this->master_model->getRecords('member_registration',array(),'scannedphoto,scannedsignaturephoto,idproofphoto,usrpassword');	
					
							########get Old image Name############
							$log_title ="IPPB NONreg new Image :".$reg_id;
							$log_message = serialize($newresult);
							$rId = $get_user_regnum[0]['ref_id'];
							$regNo = $get_user_regnum[0]['member_regnumber'];
							storedUserActivity($log_title, $log_message, $rId, $regNo);	
					...
					$cc_array = array('iibfdevp@esds.co.in');
	...
	public function bulk_mail($value='')
	{
		...
		$this->db->where('status','1');
		$this->db->where_in('exm_cd', array(997,1053));
		$candidates = $this->master_model->getRecords('admit_card_details',array('remark'=>'1','exam_date >= '=> '2023-04-29'),'admitcard_image,mem_mem_no'); 
