<?php 
	...
	class Kyc_dashboard extends CI_Controller 
	{
      ...
      $this->login_user_type = $this->session->userdata('KYC_ADMIN_TYPE');
    ...
    public function index()
		{   
			...
			$data['sub_act_id'] = "";
      $data['page_title'] = 'IIBF - KYC Dashboard';
      
      $this->load->view('kyc/kyc_dashboard', $data);
    }
    ...
    function change_password()
      ...
      { 
        $data['page_title'] = 'IIBF - KYC RECOMMENDER Change Password';
        $log_slug = 'kyc_recommender_self_password_action';
        $logged_in_type = 'Recommender';
      }   
      else if($this->login_user_type == "2") //APPROVER
      { 
        $data['page_title'] = 'IIBF - KYC APPROVER Change Password';
        $log_slug = 'kyc_approver_self_password_action';
        $logged_in_type = 'Approver';
      }   
      
      $data['form_data'] = $form_data = $this->master_model->getRecords('kyc_admin', array('kyc_admin_id' => $this->login_admin_id, 'kyc_admin_type' => $this->login_user_type), 'kyc_admin_id, is_active, is_deleted');
          ...
          $this->master_model->updateRecord('kyc_admin', $up_data, array('kyc_admin_id' => $this->login_admin_id, 'kyc_admin_type' => $this->login_user_type));
          
          $this->Kyc_model->insert_common_log($logged_in_type.' : Profile password updated', 'kyc_admin', $this->db->last_query(), $this->login_admin_id, $log_slug, 'The '.$logged_in_type.' '.$admin_name['disp_name'].' has successfully updated the password', $posted_arr); 
					$this->session->set_flashdata('success','Password successfully updated');
					
          redirect(site_url('kyc/kyc_dashboard/change_password'));
				}
			}
      ...
      $data["log_slug"] = $log_slug;      
			$this->load->view('kyc/kyc_change_password', $data);
    ...
    /******** START : VALIDATION FUNCTION TO CHECK THE OLD PASSWORD ********/
      ...
      $return_val_ajax = 'false';
        ...
        if(count($this->master_model->getRecords('kyc_admin', array('kyc_admin_password' => $enc_password, 'kyc_admin_id' => $this->login_admin_id, 'kyc_admin_type' => $this->login_user_type, 'is_active' => '1', 'is_deleted' => '0'), 'kyc_admin_id, is_active, is_deleted')) > 0)
        {
