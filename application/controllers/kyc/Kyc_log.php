<?php 
    ...
    public function index()
      ...
      $dispName = $this->Kyc_model->getLoggedInUserDetails($this->login_admin_id, $this->login_user_type); 
			...
			$data['act_id'] = "kyc_log";
			$data['sub_act_id'] = "";
      $data['page_title'] = 'IIBF - KYC '.$dispName['disp_sidebar_name'].' Logs';
      $data['dispName'] = $dispName;
      
      $this->load->view('kyc/kyc_logs_common', $data);
    }

    /******** START : SERVER SIDE DATATABLE CALL FOR KYC LOG DATA DATA ********/
    function get_kyc_log_data_ajax()
    {
      $table = 'kyc_log_data ld';
      
      $column_order = array('ld.log_id', 'ld.training_id', 'ld.regnumber', 'ld.exam_code', 'ld.description'); //SET COLUMNS FOR SORT
      
      $column_search = array('ld.training_id', 'ld.regnumber', 'ld.exam_code', 'ld.description'); //SET COLUMN FOR SEARCH
