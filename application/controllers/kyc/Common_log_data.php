  ...
  ********************************************************************************************************************/
  ...
  class Common_log_data extends CI_Controller 
  {
    ...
    function get_logs_common_ajax()
    {
        ...
        $data['log_data'] = $this->master_model->getRecords('kyc_log_data', array('pk_id' => $pk_id), 'log_id, module_slug, description, created_on', array('created_on'=>'DESC'));   
        //echo $this->db->last_query();
        $data['log_title'] = $log_title;      
        
        $result['response'] =  $this->load->view('kyc/common/get_logs_common',$data,TRUE);
        $result['flag'] = "success";
      }
      else
      {      
