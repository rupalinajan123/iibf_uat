  ...
  class Login extends CI_Controller 
  {
    ...
    {   
      ...
      if(isset($_POST) && count($_POST) > 0)
      {
        ...
        {
          generate_captcha('KYC_LOGIN_CAPTCHA',6);
          ...
          $admin_data = $this->master_model->getRecords('kyc_admin', array('kyc_admin_username' => $username, 'kyc_admin_password' => $enc_password, 'is_active' => '1', 'is_deleted' => '0'), 'kyc_admin_id, kyc_admin_type, related_id, is_active, is_deleted');
          ...
          if(count($admin_data) > 0)
          {
            ...
            {						
              ...
              $session_data = array('KYC_LOGIN_ID' => $admin_data[0]['kyc_admin_id'], 'KYC_ADMIN_TYPE' => $admin_data[0]['kyc_admin_type'], 'KYC_RELATED_ID' => $admin_data[0]['related_id']);
              $this->session->set_userdata($session_data);						
              redirect(site_url('kyc/kyc_dashboard'),'refresh');						
            }
          }
          else
          {
      ...
      $data['captcha_img'] = generate_captcha('KYC_LOGIN_CAPTCHA',6); //iibfbcbf/iibf_bcbf_helper.php
      $data['page_title'] = 'IIBF - KYC Login';
      $this->load->view('kyc/kyc_login',$data);
    }
    
    public function refresh_captcha() /******** START : REFRESH CAPTCHA ********/
    { 
      $this->load->helper('captcha');
      echo generate_captcha('KYC_LOGIN_CAPTCHA',6); //iibfbcbf/iibf_bcbf_helper.php
    }  /******** END : REFRESH CAPTCHA ********/
    ...
    public function validation_check_captcha($str='',$type=0)//$type : 0=>Server side, 1=>Ajax
    {
      ...
      if(isset($_POST) && $_POST['kyc_captcha'] != "")
      {
        ...
        $session_captcha = $this->session->userdata('KYC_LOGIN_CAPTCHA');
    ...
    public function validation_check_username($str='',$type=0)//$type : 0=>Server side, 1=>Ajax
    {
      ...
      if(isset($_POST) && $_POST['kyc_username'] != "")
      {
        ...
        $result_data = $this->master_model->getRecords('kyc_admin', array('kyc_admin_username' => $kyc_username, 'is_active' => '1', 'is_deleted' => '0'), 'kyc_admin_id, is_active, is_deleted');
    ...
    public function validation_check_login_details($str='',$type=0)//$type : 0=>Server side, 1=>Ajax
    {
      ...
      if(isset($_POST) && $_POST['kyc_username'] != "" && $_POST['kyc_password'] != "")
      {
        ...
        $result_data = $this->master_model->getRecords('kyc_admin', array('kyc_admin_username' => $kyc_username, 'kyc_admin_password' => $enc_password, 'is_active' => '1', 'is_deleted' => '0'), 'kyc_admin_id, is_active, is_deleted');
    ...
    public function logout()
    {
      ...
      $user_id = $this->session->userdata('KYC_LOGIN_ID');
      $user_type = $this->session->userdata('KYC_ADMIN_TYPE');
      ...
      $admin_session_data = array('KYC_LOGIN_ID' => "", 'KYC_ADMIN_TYPE' => "", 'KYC_RELATED_ID'=>"");
      $this->session->set_userdata($admin_session_data);
      redirect(site_url('kyc/login'),'refresh');
    }	
  }