            ...
            if ($this->form_validation->run() == true) {  
                ...
                if ($flag == 1) { 
                    ...
                    if (count($prevData)) {
                        ...
                        if ($prevData['email'] != $email && $prevData['ippb_emp_id']==0) { //priyanka d -12-july-23 >> added ippb_emp_id condition so candidate will not update email/mob
                        ...
                        if ($prevData['mobile'] != $mobile  && $prevData['ippb_emp_id']==0) { //priyanka d -12-july-23 >> added ippb_emp_id condition so candidate will not update email/mob
...
//echo '**';exit;
    ...
    public function comApplication()
    {
        ...
        //base64_decode($this->input->get('excode2'));
        // echo $decode_exam_code; exit;
        $check_valid_exam_flag = $this->check_valid_exam_for_member($decode_exam_code);
        if($check_valid_exam_flag == 1){
            $this->session->set_flashdata('error_invalide_exam_selection', "This certificate course is applicable for <strong>Ordinary SBI staff only</strong>.");
            redirect(base_url() . 'NonMember/accessdenied_not_sbi_mem/');
    ...
    public function iibf_fedai_eligible_api($exam_code=0, $exam_period=0,$member_no=0)
    {
        ...
        // $api_url =  "http://10.10.233.66:8093/fedaieligibleapi/getFedaiEligible/".$exam_code."/".$exam_period."/".$member_no;  
        ...
        if (isset($_POST['processPayment']) && $_POST['processPayment']) {
            ...
            if ($getcenter[0]['state_code'] == 'MAH') {
                ...
                if ($this->session->userdata['examinfo']['elearning_flag'] == 'Y') {
                    if(base64_decode($this->session->userdata['examinfo']['excd']) == $this->config->item('examCodeSOB')) {  //priyanka d-added if else to get proper amount after select e-learning  - 27-apr-23
                        $cs_total        = $amount;
