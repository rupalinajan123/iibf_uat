<?php
	...
	class Cron_billdesk_custom extends CI_Controller 
	{
		...
		public function billdesk_callback()
		{
			...
			$log_title ="billdesk_callback Function call Cron custom";
			$log_message = 'billdesk_callback Function call custom';
			$rId = $regNo = '';
			$this->member_regn_log($log_title, $log_message, $rId, $regNo);
			
			$filehandle = fopen("cs2s_log/billdesk_cron_custom/lock.txt", "c+");
			if (flock($filehandle, LOCK_EX | LOCK_NB)) 
			{
				...
				$query='SELECT receipt_no FROM payment_transaction WHERE receipt_no = "903492383" AND gateway = "billdesk" AND status = 2';
				...
				if ($crnt_day_txn_qry->num_rows())
				{
					...
					$dir = 'cs2s_log/billdesk_cron_custom/'.$todays_date;
					...
					foreach ($crnt_day_txn_qry->result_array() as $c_row)
					{
						...
						if($data_count == 0)
						{
							...
							## Update counts
							if($responsedata['transaction_error_type'] == 'success')
							{
								$succ_cnt++;
								array_push($succ_recp_arr,$receipt_no);
							}
							else if($responsedata['transaction_error_type'] == 'fail' || $responsedata['transaction_error_type'] == 'abort' || $responsedata['transaction_error_type'] == 'payment_authorization_error')
							{
								$fail_cnt++;
							...
							else if($responsedata['transaction_error_type'] == 'pending')
							{
								$pending_cnt++;
			...
			else 
			{
				...
				echo "Cron custom is already running";
			}
			fclose($filehandle);			
		}		
	}	