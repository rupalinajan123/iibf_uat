...
class NonMemberone extends CI_Controller {
	 ...
	 */
...

	 {
		...
		redirect(base_url().'NonMemberone/dashboard');	
...

	{
...

		{
			...
			redirect(base_url().'NonMemberone/examdetails/?excode2='.$examcode); 
...
/* Benchmark Disability Code - Bhushan */
...

					{
...

						{
...

							{
								...
								redirect(base_url('NonMemberone/acknowledge/'));
...

							{
								...
								redirect(base_url('NonMemberone/profile/'));
	...
	$data = array(
		...
		'middle_content' => 'NonMemberone/userprofile',
...

{
 ...
 Kindly go to Edit Profile and <a href="' . base_url() . 'NonMemberone/profile/">click here</a> to update the, "Benchmark Disability" and then apply for exam. For any queries contact zonal office.</li>';
        ...
        $data = array(
            ...
            'middle_content' => 'NonMemberone/nmmember_notification',
...

   {
						...
						/* Close User Log Actitives */
...

							{
...

								{
									...
									redirect(base_url('NonMemberone/editimages/'));
...

		{
...

			{
...

				{
					...
					redirect(base_url().'NonMemberone/changepass/');
...

	{
...

			{
				...
				redirect(base_url() . 'NonMemberone/GST');
		 ...
		 ||$user_images[0]['mobile']=='' ||$user_images[0]['email']=='')*/
...

			{
				...
				redirect(base_url().'NonMemberone/notification');
...

			{
            	...
            	redirect(base_url() . 'NonMemberone/benchmark_disability_check');
		 ...
		 $exam_list=array();$examcodes=array('526','527','991');
	...
	{		
...

			{
				...
				redirect(base_url().'NonMemberone/accessdenied/');
...

			 {
...

				{
...

					{
						...
						$data = array('middle_content' => 'NonMemberone/nmmember_notification','msg' => $msg);
	...
	{ 
		...
		{ //exit;
...

			{
...

				{
						...
						{	
...

							{
...

								{
									...
									redirect(base_url().'NonMemberone/comApplication');
					...
					redirect(base_url().'NonMemberone/preview');
...

		{
			...
			redirect(base_url().'NonMemberone/examlist/');	
		...
		$flag=allowed_examdate($this->session->userdata('nmregnumber'),$this->session->userdata('examcode'),$v['date']);
		 ...
		 if ($flag == 1)

			  {
					...
					redirect(base_url().'NonMemberone/info');
...

	{
...

		{
...

			{
				...
				$emailerstr=$this->master_model->getRecords('emailer',array('emailer_name'=>'NonMemberone_exam_enrollment_nofee '));
				...
				redirect(base_url().'NonMemberone/applydetails/');
...

		{
...

			{
...

				{
					...
					redirect(base_url().'NonMemberone/sbi_make_payment/');
...

	{
...

		{
			...
			$pg_success_url = base_url()."NonMemberone/sbitranssuccess";

			$pg_fail_url    = base_url()."NonMemberone/sbitransfail";
		...
		$invoice_insert_array=array('pay_txn_id'=>$pt_id,
			...
			$log_title = "Exam invoice data from NonMemberone cntrlr inser_id = '".$inser_id."'";
...
=1000003|DOM|IN|INR|2|Other|https://test.sbiepay.coom/secure/fail.jsp|SBIEPAY|2|2|NB|ONLINE|ONLINE
...

	{
						...
						{		
...

							{
...

								{
									...
									redirect(base_url().'NonMemberone/refund/'.base64_encode($MerchantOrderNo));
		...
		redirect(base_url().'NonMemberone/details/'.base64_encode($MerchantOrderNo).'/'.base64_encode($exam_info[0]['exam_code']));
...

	{
...

		{
			...
			redirect(base_url().'NonMemberone/fail/'.base64_encode($MerchantOrderNo));
...

	{
		...
		if((!file_exists('./uploads/photograph/'.$user_images[0]['scannedphoto']) || !file_exists('./uploads/scansignature/'.$user_images[0]['scannedsignaturephoto']) || !file_exists('./uploads/idproof/'.$user_images[0]['idproofphoto']) ||$user_images[0]['scannedphoto']=='' ||$user_images[0]['scannedsignaturephoto']=='' || $user_images[0]['idproofphoto']=='') && (!is_file(get_img_name($this->session->userdata('nmregnumber'),'s')) || !is_file(get_img_name($this->session->userdata('nmregnumber'),'p'))))
...

		 {
			...
			$msg.='<li>Your Photo/signature or ID proof are not available kindly go to Edit Profile and <a href="'.base_url().'NonMemberone/profile/">click here</a> to upload the Photo/Signature and then apply for exam. For any queries contact zonal office.</li>';
...
Your email id or mobile number are not available kindly go to Edit Profile and <a href="'.base_url().'NonMemberone/profile/">click here</a> to update the, email id or mobile number and then apply for exam. For any queries contact zonal office.</li>';
...
Please check all mandatory fields in profile <a href="'.base_url().'NonMemberone/profile/">click here</a> to update the, profile. For any queries contact zonal office.</li>';
...
/*		if((!file_exists('./uploads/photograph/'.$user_images[0]['scannedphoto']) || !file_exists('./uploads/scansignature/'.$user_images[0]['scannedsignaturephoto']) || !file_exists('./uploads/idproof/'.$user_images[0]['idproofphoto']) ||$user_images[0]['scannedphoto']=='' ||$user_images[0]['scannedsignaturephoto']=='' || $user_images[0]['idproofphoto']) && ($user_images[0]['mobile']=='' ||$user_images[0]['email']==''))

		{
			...
			$msg='<li>Your Photo/signature are not available kindly go to Edit Profile and <a href="'.base_url().'NonMemberone/profile/">click here</a> to upload the Photo/Signature and then apply for exam. For any queries contact zonal office.</li>

<li>

Your email id or mobile number are not available kindly go to Edit Profile and <a href="'.base_url().'NonMemberone/profile/">click here</a> to update the, then email id or mobile number and then apply for exam. For any queries contact zonal office.</li>';
