<?php
	...
	{	
		...
		{
			$res_institute_no = $res_invoice_no = $res_institute_name = $res_amount = $res_subscription_year = $res_invoice_pdf = '';
      ...
      $response_res = json_decode($response,true);
				...
				if($response_flag == 'success')
				{
					...
					if($response_msg_str != "")
					{
						...
						if(count($response_msg_arr) > 0)
						{
							...
							if(isset($response_msg_arr[3])) { $res_amount = $response_msg_arr[3]; }								
							if(isset($response_msg_arr[4])) { $res_subscription_year = $response_msg_arr[4]; }								
							if(isset($response_msg_arr[5])) { $res_invoice_pdf = $response_msg_arr[5]; }								
							
							//echo $res_institute_no.' == '.$institute_no.' && '.$res_invoice_no.' == '.$invoice_no.' && '.$res_amount.' > 0';
							
							if($res_institute_no == $institute_no && $res_invoice_no == $invoice_no && $res_amount > 0)
							{
								$flag = 'success';
								$error_msg = "";
							}
		...
		public function dashboard()//START : DASHBOARD
		{
			...
			{	
				$this->form_validation->set_rules('amount', 'Amount', 'trim|required|xss_clean',array('required' => 'Please enter the %s'));
								
				if($this->form_validation->run() == TRUE)
				{			
					//if need to check member validation in esds database, then code goes here
					
					$posted_fee_amount = $this->security->xss_clean(trim($this->input->post('amount')));
					
					if($amount > 0 && $amount == $posted_fee_amount)
					{
		...
		function after_login_institute_data()
		{
			...
			if(count($institute_data) > 0 && isset($institute_data['flag']) && $institute_data['flag'] == 'success' && isset($institute_data['institute_no']) && $institute_data['institute_no'] == $login_institute_no && isset($institute_data['invoice_no']) && $institute_data['invoice_no'] == $login_invoice_no && isset($institute_data['amount']) && $institute_data['amount'] > 0)
			{
		...
		function after_login_check_payment_data($enc_receipt_no=0)
		{
			...
			$last_payment_data = $this->master_model->getRecords('institute_subscription is',array('pt.pay_type' => '21'), 'is.subscription_id, is.institute_no, is.invoice_no, is.institute_name, is.subscription_year, is.invoice_pdf, pt.gateway, pt.amount, pt.date, pt.transaction_no, pt.receipt_no, pt.status AS PaymentStatus');
		...
		public function make_payment()
		{
			...
			if(isset($_POST['processPayment']) && $_POST['processPayment'])
			{
				...
				$institute_subscription_pay_type = "21";
        ...
        $this->check_last_payment($institute_no);				
				...
				$custom_field = $MerchantOrderNo . "^IIBFINSTSUB^" . $institute_no . "^" . $ref4;
				$custom_field_billdesk = $MerchantOrderNo . "-IIBFINSTSUB-" . $institute_no . "-" . $ref4;
				...
				{ 
					...
					if (count($billdesk_res) > 0 && $billdesk_res['status'] == 'ACTIVE') 
					{
						...
						$data['ins_subscription_pg_flag'] = 'IIBFINSTSUB';
		...
		public function handle_billdesk_response()
    {
		...
		function payment_history()
		{
			...
			$this->db->order_by('is.subscription_id','DESC');
			$this->db->join('payment_transaction pt', 'pt.member_regnumber = is.institute_no AND pt.ref_id = is.subscription_id');
			$payment_history_data = $this->master_model->getRecords('institute_subscription is',array('is.institute_no'=> $institute_no, 'is.invoice_no'=> $invoice_no, 'pt.pay_type' => '21'), 'is.subscription_id, is.institute_no, is.invoice_no, is.subscription_year, pt.gateway, pt.amount, pt.date AS PaymentDate, pt.transaction_no, pt.receipt_no, pt.status AS PaymentStatus');
    ...
    public function admin_dashboard()//START : DASHBOARD
      ...
      $this->db->order_by('is.subscription_id','DESC');
			$this->db->join('payment_transaction pt', 'pt.member_regnumber = is.institute_no AND pt.ref_id = is.subscription_id');
			$payment_history_data = $this->master_model->getRecords('institute_subscription is',array('pt.pay_type' => '21'), 'is.subscription_id, is.institute_name, is.institute_no, is.invoice_no, is.subscription_year, pt.gateway, pt.amount, pt.date AS PaymentDate, pt.transaction_no, pt.receipt_no, pt.status AS PaymentStatus');
    ...
    function admin_invoice($enc_receipt_no=0)
      ...
      if($enc_receipt_no == '0')
        ...
        $institute_data = $this->master_model->getRecords('payment_transaction pt',array('pt.receipt_no' => $receipt_no, 'pt.pay_type' => '21'), 'pt.id, pt.amount, pt.date, pt.transaction_no, pt.receipt_no, pt.status, is.subscription_id, is.institute_no, is.invoice_no, is.institute_name, is.subscription_year, is.invoice_pdf, pt.gateway, pt.amount');
    ...
    function admin_receipt($enc_receipt_no=0)
      ...
      if($enc_receipt_no == '0')
        ...
        $this->db->join('institute_subscription is', 'is.institute_no = pt.member_regnumber AND pt.ref_id = is.subscription_id');
        $institute_data = $this->master_model->getRecords('payment_transaction pt',array('pt.receipt_no' => $receipt_no, 'pt.pay_type' => '21'), 'pt.id, pt.amount, pt.date, pt.transaction_no, pt.receipt_no, pt.status AS PaymentStatus, is.subscription_id, is.institute_no, is.invoice_no, is.institute_name, is.subscription_year, is.invoice_pdf, pt.gateway, pt.amount');
