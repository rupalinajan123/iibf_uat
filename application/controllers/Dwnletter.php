...
class Dwnletter extends CI_Controller {
	 ...
	 *sagar custom_examinvoice_send_mail * So any other public methods not prefixed with an underscore will
	...
	/////ADMIT CARD IMAGE GENERATE
	public function custom_admitcard_pdf_image(){     
		//echo 'hi';exit;
		$exam_code = 997;     
		$exam_period = 851;
		$member_array = array(802281220,802281407); 
		$arr_size = sizeof($member_array);  
		for($i=0;$i<=$arr_size;$i++){
			echo $path = genarate_admitcard210($member_array[$i],$exam_code,$exam_period);  
			echo "<br/>"; 
		}
	}

	///elearning EXAM INVOICE IMAGE GENERATE
	//https://iibf.esdsconnect.com/dwnletter/custom_elearning_invoice
	public function custom_elearning_invoice(){
		$invoice_no = array(5838003);//invoice Id 

		for($i=0;$i<count($invoice_no);$i++)
		{
			$inv_no = $this->master_model->getRecords('spm_elearning_config_invoice',array('invoice_id'=>$invoice_no[$i]),'exam_invoice_no');

			if( !empty($inv_no))
			{
				$invoice_no= 'EL/24-25/'.$inv_no[0]['exam_invoice_no'];
				$update_data = { 'invoice_no' => $invoice_no }
			}else
			{
				$invoice_insert_array = array( 'invoice_id' => $invoice_no );
				if($this->master_model->insertRecord('spm_elearning_config_invoice', $invoice_insert_array)
				{
					$inv_no = $this->master_model->getRecords('spm_elearning_config_invoice',array('invoice_id'=>$invoice_no[$i]),'exam_invoice_no');
			...
			if($this->master_model->updateRecord('exam_invoice',$update_data,array('invoice_id'=>$invoice_no[$i])))
			{
				echo 'Updated -'.$invoice_no;
			}

			$invoice = $this->master_model->getRecords('exam_invoice',array('invoice_id'=>$invoice_no[$i]));

			echo $path =  custom_genarate_elearning_exam_invoice($invoice_no[$i]);echo '<br/>';
			$invoice_img = trim($path,"uploads/examinvoice/supplier/");
			$update_data = array('invoice_image'=>$invoice_img, 
								 'date_of_invoice'=>$invoice[$i]['created_on']);

			if($this->master_model->updateRecord('exam_invoice',$update_data,array('invoice_id'=>$invoice_no[$i])))
			{
				echo 'image name & invoice date updated of invoice_id '.$invoice_no[$i];
			}
		}
	}


	///EXAM INVOICE IMAGE GENERATE
	public function custom_exam_invoice_image()
	{       
		//url https://iibf.esdsconnect.com/dwnletter/custom_exam_invoice_image
		 $arr = array(4741358,4741408); // add invoice id  
	...
	///EXAM INVOICE SETTLEMENT
	public function custom_exam_invoice_settlement()
	{    		
		//url https://iibf.esdsconnect.com/dwnletter/custom_exam_invoice_settlement
		$arr = array(86350126,30166485,55283192,58715416,50363697,28684490,45420084,);  
		// add receipt number  
		for($i=0;$i<sizeof($arr);$i++){ 
			...
			if($config_info[0]['exam_invoice_no']!=''){
				$last_id = $config_info[0]['exam_invoice_no'];
			...
			echo $this->db->last_query();
			echo '<br/>';
			echo $invoice_number = 'EX/22-23/'.$last_id;
			echo '<br/>';
			echo $invoice_img_name = $member_info[0]['regnumber'].'_EX_22-23_'.$last_id.'.jpg';
			echo '<br/>';

			$arr = array(4779002,4787743,4787751,4795708,4795713,4795717,4795721); // add invoice id  

			//for($i=0;$i<sizeof($arr);$i++){
				echo 'check_invoice_image'.$path = custom_genarate_exam_invoice_newdesign($arr[$i]);
				echo "<br/>"; 
			//} 
	...
	{   
		//URL https://iibf.esdsconnect.com/Dwnletter/member_settlement_new  		
		$member_no =802206420;   	
		$mem_exam_id =7571555; // member exam table primary key OR admit_card_detail mem_exam_id
		$exam_code = 991;  
		$exam_prd = 998; 
		
		//echo'password '.$password = $this->random_password(); 
		...
		{
			echo 'Password is:';
			echo $password = random_password();echo '<br>';
		}
		/********End of Password Code********/
		
		if(!empty($admit_card_details))
		{
			echo '<br>';echo 'Total recode found in admit card table :';
			echo count($admit_card_details);echo '<br>';
	...
	public function testfun(){
		$admitcard_id = ('8722005');
				$where_primary_key = array('admitcard_id'=>$admitcard_id,
								'remark'=>'1');
	...
	// /usr/local/bin/php /home/supp0rttest/public_html/index.php dwnletter admit_update_912
	// /usr/local/bin/php /home/iibfgrow/public_html/index.php dwnletter admit_update_912
	public function admit_update_912()
	{
		// Get Admit Card Data from TEMP
		
		$select = '*';
		//$this->db->limit(1);
		$temp_admit_card_data = $this->Master_model->getRecords('admit_card_details_jaiib',array('remark'=>'0','exm_prd'=>912),$select);
		//echo $this->db->last_query();
		//exit;
		if(count($temp_admit_card_data))
		{
			foreach($temp_admit_card_data as $admit_row)
			{
				...
				$admit_update_data = array('center_code'=>$admit_row['center_code'],
								...
								'time'=>$admit_row['time'],
								'created_on'=>'0000-00-00 00:00:00',
								'vendor_code' => $admit_row['vendor_code']
								);
	...
	// /usr/local/bin/php /home/supp0rttest/public_html/index.php dwnletter admit_update_913 
	// /usr/local/bin/php /home/iibfgrow/public_html/index.php dwnletter admit_update_913
	
	public function admit_update_913()
	{
		// Get Admit Card Data from TEMP
		
		$select = '*'; 
		//$this->db->limit(1);
		$temp_admit_card_data = $this->Master_model->getRecords('admit_card_details_913',array('remark'=>'0','exm_prd'=>913),$select);
		//echo $this->db->last_query();
		//exit;
		if(count($temp_admit_card_data))
		{
			foreach($temp_admit_card_data as $admit_row)
			{
				...
				$where_admit_remark = array('mem_mem_no'=>$admit_row['mem_mem_no'],
								...
								'sub_cd'=>$admit_row['sub_cd'],
								'remark'=>'0');
				$this->master_model->updateRecord('admit_card_details_913',$admit_update_remark,$where_admit_remark);
				echo '-|-'. $admit_row['mem_mem_no'].'-|-';
			}
		}
	} 
	...
	//invoice_settlement_function_use_this
	...
	public function exam_invoice_settlement_bulk(){    		
	$arr = array('537','543','556','568');   // add receipt number  
		for($i=0;$i<sizeof($arr);$i++){ 
			$this->db->where('receipt_no',$arr[$i]);
			$invoice_info=$this->master_model->getRecords('exam_invoice','','invoice_id,member_no');
			echo $this->db->last_query();
			...
			$this->db->where('receipt_no',$arr[$i]);
			$payment_info=$this->master_model->getRecords('bulk_payment_transaction','','inst_code,UTR_no,date');
			echo $this->db->last_query();
			echo '<br/>';
			
			$invoice_mem_no = $payment_info[0]['inst_code'];
			...
			echo $invoice_img_name = 'bulk_'.$payment_info[0]['inst_code'].'_EX_21-22_'.$last_id.'.jpg';
			echo '<br/>';
			
			
			$update_arr = array(
									'transaction_no'=>$payment_info[0]['UTR_no'],
									'invoice_no' => $invoice_number,
									'invoice_image' => $invoice_img_name,
									'date_of_invoice' =>$invoice_date_of_invoice,
									'modified_on' => $invoice_modified_on
	...
	//exit;  
		...
		for($i=0;$i<sizeof($arr);$i++){ 
			...
			// echo 'in'; echo '<br/><pre>'; print_r($payment_info) exit;
	...
	public function custom_renewal_invoice_settled()
	{
		echo $attachpath = genarate_renewal_invoice('4462414');
	}
	...
	public function custom_spm_settlement()
	{ 
		$invoice_id = array(4643273); 


		for($i=0;$i<sizeof($invoice_id);$i++){

			echo $invoiceNumber = generate_el_invoice_number($invoice_id[$i]);
			if($invoiceNumber)
			{
				/* $cyear = date("y");
				$nyear = $cyear+1;
        ...
        //END : THIS CODE WAS UPDATED BY SAGAR M ON 2024-04-16. EARLIER THE INVOICE YEAR FOLLOW THE CALENDER YEAR INSTEAD OF FINANCIAL YEAR
			...
			$this->master_model->updateRecord('exam_invoice',$update_data,array('invoice_id'=>$invoice_id[$i],'app_type'=>'EL'));
			echo $this->db->last_query();

			echo '<br>'.$attachpath=genarate_el_invoice($invoice_id[$i]);
			
			echo '<br>'.$last_qry_exam_invoice = $this->db->last_query();
	...
	public function centerstat(){ 
		...
		$delimiter = ",";
		$newline = "\r\n";
		$filename = "BCBF_centerwise_counts_".$exam_code."_".$exam_period."_.csv";
	 ...
	 public function centerstat_new()
    {
    		$data = array();
    		if (count($_POST) > 0) {
	...
	public function custom_chkinvoice(){  
		...
		echo $path = custom_genarate_exam_invoice(903200362); 
	}
	...
	public function custom_dra_exam_invoice(){
		...
		//echo $path = custom_genarate_draexam_invoice('21132,21142,21145');  
	}
	...
	{
		$exam_invoice_id = 4641786;
	echo 	$genarate_draexam_invoice = genarate_draexam_invoice_custom($exam_invoice_id);
	}
	...
	public function custom_bankquest_invoice(){
		echo $a = custom_genarate_bankquest_invoice(2763596,3221);
	}
	
	// to check custom bankquest invoice
	public function custom_vision_invoice(){
		echo $a = custom_genarate_vision_invoice(2762426,4471);
	}
	
	public function custom_examinvoice_send_mail(){  
		$final_str = "Hello Sir/Madam"; 
		$final_str.= "<br/><br/>";
		$final_str.= 'Your subscription number is 7437 and validity of your subscription is for 1
year i.e. from 01-01-2021.
<br/>
Please note down your subscription no and valid period of subscription
for future reference.'; 
		...
		$attachpath = "uploads/admitcardpdf/991_998_802229692.pdf";
		$info_arr=array(//'to'=>$email[0]['email'],
						'to'=>'iibfdevp@esds.co.in',//'bidyapati1988nimi@gmail.com',
						'from'=>'noreply@iibf.org.in',
						'subject'=>'IIBF Bankquest Subscription',
						'message'=>$final_str
					);
		$files=array($attachpath);
		
		if($this->Emailsending->mailsend_attch($info_arr,$files)){
			echo 'mail senttttttt';
			echo "<br/>"; 
		}
	...
	public function custom_contact_class_invoice(){
		echo $path = genarate_contact_classes_invoice_cs2s(3345266,3578,$this->config->item('examCodeCaiib'),array(34),'NZ');
	}
	...
	public function close_venue_mail(){
		...
		$exm_array = array($this->config->item('examCodeCaiib'),62,$this->config->item('examCodeCaiibElective63'),64,65,66,67,$this->config->item('examCodeCaiibElective68'),$this->config->item('examCodeCaiibElective69'),$this->config->item('examCodeCaiibElective70'),$this->config->item('examCodeCaiibElective71'),72);
...
");
	...
	public function custome_bulk_invoice(){
		echo ">>##". $attach = custome_generate_bulk_examinvoice(3116);	 
	}
...
'; 
	...
	public function caiib_eligible118(){ 
		...
		foreach($sql as $rec){
			...
			$this->master_model->updateRecord('eligible_master',$update_array,array('member_no'=>$rec['member_no'],'exam_code'=>$this->config->item('examCodeJaiib'),'eligible_period'=>118,'part_no'=>$rec['part_no']));
	...
	public function custom_admitcard_pdf_single210(){     
		//url https://iibf.esdsconnect.com/dwnletter/custom_admitcard_pdf_single
		$exam_code = 210;     
		$exam_period = 123;  

	

		$array=array(510384087,510443700,510457876,510450417,510512270,510411166,510226636,510414755,510270788,510508860,510574430,510561150,510461966,510523170,500187044,510068578,500137371,510517751,510539008,510553842,510238807,510256484,510577821,510385014,510527393,500183127,510550223,510509960,510489975,510490961,510154663,510482373,510557850,510142443,510170817,510097290,510468002,510496504,510576858,510547678,510171800,510418350,510525058,510478491,510530639,510544447,510229397,510559352,510354849,510250709,510421886,510385707,510542379,510460183,510541399,510080886,510035426,510577096,510538770,510078960,510577722,510577585,510386818,510523359,510540306,510448396,510484933,510534424,510505106,510084798,510555004,500177296,510271926,510411689,510526094,510380892,510567327,510336229,510092643,510412929,510519945,510519725,510498520,510203845,510327137,510140491,510306721,510554936,510536719,510342097,510538171,510562193,510246053,500092954,510479391,510504702,510224498,510551260,510518226,510573087,510464423,500156860,510491466,510554276,510215417,510577000,510438212,510578035,510577858,510577830,510516179,510383770,510576846,510491206,510563228,510528025,510560512,510482646,510249259,510361348,510558617,510371065,510477391,500049190,510447457,510572484,510476079,510294428,510216179,510178935,510064403,510515619,510543551,510184694,510558477,510534572,510506017,510534604,510516887,510531485,500164313,510411268,500147549,510406619,510314265,510576055,510032188,510556238,510478151,510279244,510392623,510107833,510419224,510487488,510314543,510165812,510515706,500110196,500061465,510531365,510498714,510349722,500061584,510137198,510398575,510570980,510523952,510577124,510304211,510557005,510538073,510453131,510571150,510531830,510494280,510560968,510515429,510577934,510538544,510446358,510548882,510556611,510357871,510139901,510407255,510464681,510563330,510164343,510541475,510549748,510474915,510570560,510492165,510408962,510542801,510551721,510311077,510484156,510454299,510479266,510328126,510501260,510481021,500213825,510513852,510263219,510534857,500063214,510476791,510436816,510578075,510264537,510155363,510577031,510533683,510470632,510219247,510578091,510558074,510538926,510158025,500208342,510232978,510516420,510468931,510272788,510388785,510280970,510578088,510111713,510143319,510493189,510301234,510419812,510012718,510498622,510452966,510577321,510391819,510430042,510539067,500113340,510428362,510396455,510323064,510318538,510518991,510302778,510300831,500162332,510578086,510248241,500053378,510238392,510565131,510064626,510578089,510539360,510577331,510553060,510290823,510056793,510578092,510576807,510565722,510417233,510517117,510534045,510487382,510533160,510536217,510271801,510238709,510301754,510400944,510556691,510484670,510577889,510502041,510472944,510415354,500050534,510380512,510410258,510388444,510536370,510512899,510517243,510542327,510536661,510535255,510553158,510578082,500089166,510551297,510163558,510549101,510522336,510141801,510340928,510578081,510543617,510556519,510330611,510571991,510354225,510563657,510487282,510065244,510095559,500069180,510577708,510577805,510401577,510183147,510577582,510466392,510166558,510505860,510569440,500200426,510317252,510211104,510352483,510578006,510550802,510514720,510253674,510330018,510349440,510473060,510075436,510497284,500045283,510236751,510562856,510201622,510434965,500146860,510136886,510303159,510391072,510504505,510577565,510405362,510490541,510464815,510556900,510551227,510296160,510485727,510349946,510397730,510330770,510577980,510567853,510519453,510038260,510502902,510279364,510467393,510510875,510209699,510265033,510418243,510414151,510544268,510561217,510391013,510499004,510488651,510468836,510454785,510577942,510476770,510577717,510476224,510249430,510543320,510542358,510091534,510556518,510536982,510537005,510539847,510435476,510049603,510367160,510577688,510545879,510403403,510171764,510307433,510483393,510554828,510248034,510476761,510415898,510018764,510459154,510555492,510503726,510562879,510527570,510557001,510494587,510482335,510284660,400128803,510328474,510307795);
		foreach($array as $rec){ 
			
			echo genarate_admitcard210($rec,$exam_code,$exam_period).'<br>';  
		}
		return 1;
		
		
		//$center_array = array(306);  
		$this->db->limit(400,3000);        
		$this->db->distinct('mem_mem_no');    
		$this->db->where('remark',1);
		$this->db->where('created_on < ','2023-03-02 13:00:00');
		$this->db->where('exm_cd',210);
		$this->db->where('exm_prd','123');
		$this->db->where('admitcard_image !=','');
		$sql = $this->master_model->getRecords('admit_card_details','','mem_mem_no'); 
		//echo $this->db->last_query();exit;
		echo'<pre>';print_r($sql);
		foreach($sql as $rec){ 
			
			echo genarate_admitcard210($rec['mem_mem_no'],$exam_code,$exam_period).'<br>';  
		}
	}
	...
	public function custom_admitcard_pdf_single(){     
		//url https://iibf.esdsconnect.com/dwnletter/custom_admitcard_pdf_single
		echo genarate_admitcard210(802237051,997,851).'<br>';  
		return 1;
		$exam_code = 210;     
		$exam_period = 123;   
		$array=array(510127739,510315781,510440083,510045316,510416492,510578150,510248267,510566468,510577423,510400725,510487715,500202030);//array(510532859);
	...
	public function custom_admitcard_pdf_multiple(){     
		//url https://iibf.esdsconnect.com/dwnletter/custom_admitcard_pdf_single
		...
		$this->db->where('date(date) =  ','2023-03-01');
		//$this->db->where('date > ','2023-03-01 23:59:59');
		$this->db->where('payment_transaction.exam_code',210);
		
		//$this->db->where('admit_card_details.remark',3);
		$this->db->group_by('payment_transaction.receipt_no');
		//$this->db->where('admitcard_image !=','');
		$sql = $this->master_model->getRecords('payment_transaction','','payment_transaction.receipt_no,payment_transaction.member_regnumber'); 
		echo $this->db->last_query();
	//	echo'<pre>';print_r($sql);exit; 
		//echo'<pre>';print_r($sql);
		
		foreach($sql as $rec){ 
			$responsedata = $this->billdesk_pg_model->billdeskqueryapi($rec['receipt_no']);
			//echo'<pre>';print_r($responsedata);exit;
			if(isset($responsedata['auth_status']) && $responsedata['auth_status']=='0300') {
				...
				$this->db->where('payment_transaction.receipt_no !=',$rec['receipt_no']);
				$this->db->where('admit_card_details.remark',1);
				$this->db->where('payment_transaction.member_regnumber',$rec['member_regnumber']);
				$this->db->group_by('admit_card_details.mem_exam_id');

				//$this->db->where('admitcard_image !=','');
				$checkOtherTransactionExist = $this->master_model->getRecords('payment_transaction','','payment_transaction.receipt_no'); 
				//echo $this->db->last_query();
				if(!empty($checkOtherTransactionExist)) {
					echo'<pre> already paid amount again=';print_r($rec);
					echo'<pre>';print_r($checkOtherTransactionExist);
				}
				else {
					echo'<pre> no made payment again=';print_r($rec);
					echo'<pre>';print_r($checkOtherTransactionExist);
				}
			}
			else
			echo $rec['receipt_no'].'=No Payment<br>';
			//echo genarate_admitcard210($rec['mem_mem_no'],$exam_code,$exam_period).'<br>';  
		}
	} 
	public function admitcard_pdf_single(){     
		//url https://iibf.esdsconnect.com/dwnletter/admitcard_pdf_single
		$exam_code = 991;     
		return 1;
		$exam_period = 998;    
		$member_array = array(802206272);//510455634
		$arr_size = sizeof($member_array);  
		for($i=0;$i<=$arr_size;$i++){
	...
	public function remote_custom_admitcard_pdf_single(){    
		...
		for($i=0;$i<=$arr_size;$i++){
			...
			echo "<br/>"; 
			$update_member_app = array('admitcard_image' => $path);
			$where_member_app = array('exm_cd' => 1020, 'exm_prd' => 833, 'mem_mem_no' => $member_array[$i]);
			$this->master_model->updateRecord('admit_card_details',$update_member_app,$where_member_app);
				
		}
	}
	
	public function naar_custom_admitcard_pdf_single(){  
	/*/usr/local/bin/php /home/supp0rttest/public_html/index.php dwnletter naar_custom_admitcard_pdf_single*/  
		//echo 'hi';exit;
		$exam_code = 1015;       
		$exam_period = 43;    
		$member_array = array(801757740);                
		$arr_size = sizeof($member_array);  
		for($i=0;$i<=$arr_size;$i++){
	...
	public function naar_institute_profile_admitcard_pdf_single(){  
	/*/usr/local/bin/php /home/supp0rttest/public_html/index.php dwnletter naar_institute_profile_admitcard_pdf_single 801547856*/  
		//echo 'hi';exit;
		$exam_code = 1015;       
		$exam_period = 9;    
		$member_array = $this->uri->segment(3);
		              
		$arr_size = sizeof($member_array);  
		for($i=0;$i<=$arr_size;$i++){
			echo $path = naar_institute_admitcard_bulk($member_array[$i],$exam_code,$exam_period);  
			echo "<br/>"; 
		}
	}
	...
	public function wrong_venue(){ 
		...
		$member_array = array();
		$exam_array = array($this->config->item('examCodeCaiib'),'62',$this->config->item('examCodeCaiibElective63'),'64','65','66','67',$this->config->item('examCodeCaiibElective68'),$this->config->item('examCodeCaiibElective69'),$this->config->item('examCodeCaiibElective70'),$this->config->item('examCodeCaiibElective71'),'72');
	...
	public function getpassword(){ 
		...
		//echo '<br>'.$res[0]['usrpassword'];
		echo $decpass = $aes->decrypt(trim($res[0]['usrpassword']));
		//echo '<br>'.$aes->encrypt($decpass);
		//echo '<br>'.$aes->decrypt('etOrQlKbSM3kUGAceFk4XQ==');
		//
		//		nPG8IXg8el5pQOVs2BhX4Q==
		//echo '<br><br><br>Pranita$21';
		//echo '<br>'.$aes->encrypt('Pranita$21');
		//echo '<br>'.$aes->decrypt('cX30/C0NTtIufa3DQi6wgw==');
	...
	public function subject_missing_R(){
		...
		$member_array = array();  
		// $exarr = array(60,63,65,68,69,70,71);  
		...
		$this->db->where('exm_prd',122);
		$this->db->where('remark','1');
		$this->db->where('app_update','0');
		//$this->db->where('mem_mem_no',500114590);
		$this->db->where('created_on >= ','2022-04-22 00:00:01');   
		$this->db->where('created_on <= ','2022-04-24 23:59:59');         
		...
		foreach($admit_card as $member_no){ 
			...
			$this->db->where_in('exam_code',$member_no['exm_cd']);
			$this->db->where('eligible_period',122);
			...
			if($member_rec_cnt != 0){
				...
				$this->db->where_in('exm_cd',$member_no['exm_cd']);
				$this->db->where('exm_prd',122);
				$this->db->where('remark','1');
				$this->db->where('app_update','0');
				...
				//echo '>>'. $admit_card_cnt;
				if($member_no['exm_cd'] != $this->config->item('examCodeSOB')){
					if($admit_card_cnt != 3){
						$member_array[] = $member_no['mem_exam_id'];
					}
				}elseif($member_no['exm_cd'] == $this->config->item('examCodeSOB')){
					if($admit_card_cnt != 2){
						$member_array[] = $member_no['mem_exam_id'];
					}
				}
		...
		foreach($admit_card as $member_no){
			...
			if($member_rec_cnt != 0){ 
				...
				if($member_no['exm_cd'] != $this->config->item('examCodeSOB')){
					...
					/*if($admit_card_cnt != 2){
						$member_array[] = $member_no['mem_exam_id'];
					}else */if($member_rec_cnt != $admit_card_cnt){
						$member_array[] = $member_no['mem_exam_id'];
					}
				}
	...
	public function subject_missing_fresh(){             
		$member_array = array();
		// $elective_subjects = array(63,65,68,69,70,71);
		// $exarr = array(60,63,65,68,69,70,71);     
		$exarr = array($this->config->item('examCodeJaiib'),$this->config->item('examCodeDBF'),$this->config->item('examCodeSOB')); 
		$this->db->select('mem_mem_no'); 
		$this->db->distinct('mem_mem_no');
		...
		foreach($admit_card as $member_no){
			...
			if($member_rec_cnt <= 0){ 
				...
				if($member_no['exm_cd'] != $this->config->item('examCodeSOB')){
					if(in_array($member_no['exm_cd'],$elective_subjects))
					{
						if($admit_card_cnt != $member_rec_cnt)
						{
							$member_array[] = $member_no['mem_exam_id'];
						}
					}else if($admit_card_cnt != 3){
						$member_array[] = $member_no['mem_exam_id'];
					} 
				}elseif($member_no['exm_cd'] == $this->config->item('examCodeSOB')){
					if($admit_card_cnt != 2){
...
public function subject_missing_R_916(){  

		$exam_period = 916;                    
		$member_array = array();  
		$exarr = array(8,11,153,18,19,151,156,158);  
		//$exarr = array(21); 
		$this->db->select('mem_mem_no');  
		$this->db->distinct('mem_mem_no');  
		$this->db->where_in('exm_cd',$exarr); 
		...
		$this->db->where('created_on >= ','2021-05-18 00:00:01');   
		$this->db->where('created_on <= ','2021-05-18 23:59:59');         
		$admit_card = $this->master_model->getRecords('admit_card_details','','mem_mem_no,exm_cd,mem_exam_id'); 
		...
		foreach($admit_card as $member_no){ 
			...
			if($member_rec_cnt != 0){
				...
				$admit_card_cnt = count($admit_card);
				//echo '>>'. $admit_card_cnt;
				if($member_no['exm_cd'] == 8 || $member_no['exm_cd'] == 11 || $member_no['exm_cd'] == 153){
					if($admit_card_cnt != 3){
						$member_array[] = $member_no['mem_exam_id'];
					}
				}elseif($member_no['exm_cd'] == 18 || $member_no['exm_cd'] == 19 || $member_no['exm_cd'] == 151 || $member_no['exm_cd'] == 156 || $member_no['exm_cd'] == 158){
					if($admit_card_cnt != 2){
						$member_array[] = $member_no['mem_exam_id'];
					}
				}
...
public function subject_missing_F_916(){ 
		$exam_period = 916;           
		$member_array = array();   
		$exarr = array(8,11,153,18,19,151,156,158);    
		$this->db->select('mem_mem_no');  
...
public function subject_missing_fresh_916(){  
		$exam_period = 916;           
		$member_array = array();
		$exarr = array(8,11,153,18,19,151,156,158);     
		 //$exarr = array(21);   
	...
	public function fee_mismatch(){
		$exmcd = array($this->config->item('examCodeJaiib'),$this->config->item('examCodeDBF'));
		$this->db->where('remark',1);
		$this->db->where('exm_prd',119);
...
/*		$this->db->where('gen_flag',0);
	...
	public function custom_exam_invoice_new_design_swati(){       
		//url https://iibf.esdsconnect.com/dwnletter/custom_exam_invoice_new_design_swati
		 $arr = array(4779006,4779025,4779031,4779035,4779051,4779071,4779073,4779079,4779080,4779087,4779089,4795704,4795706); // add invoice id  
	...
	// added by chaitali on 2022-06-20
	public function custom_exam_invoice_pb(){       
		 $arr = array(4180601); // add invoice id  
		 for($i=0;$i<sizeof($arr);$i++){
		 	echo $path = genarate_PB_invoice($arr[$i]);
		 	echo "<br/>"; 
		 } 
	}
...
public function swati_data_insert_gst_invocie(){
	...
	public function custom_contactclass_invoice_new_design(){  
		
		
		echo $path = genarate_contact_classes_invoice_custome_new(3033371,3376);
		 	echo "<br/>"; 
	...
	public function custom_cpd_invoice(){   
		...
		echo $attachpath=genarate_cpd_invoice(4683207);
		
	}
	//dra invoice generate swati
	public function custom_draexaminvoice_newdesign(){ 
		
		/*$arr = array(4641468,4641786,4641938,4643208,4645315,4647712,4649259,4649322,4649386,4651100,4651141,4651635,4651739,4652185,4652673,4654142,4654382,4654462,4655500,4655567,4655576,4655616,4655622,4655626,4655628,4655677,4655733,4655738,4655757,4655880,4655904); 
		for($i=0;$i<=0;$i++){
			echo $path = custom_genarate_draexam_invoice_new_design($arr[$i]);
			echo "<br/>"; 
		}*/
		$invoice_id = '4641786'; //2611791;
	...
	public function custom_draexaminvoice_newdesign_swati(){ 
		...
		$invoice_id = 4656005;
		echo $path = genarate_draexam_invoice($invoice_id);
	}
	public function custome_bulk_invoice_new_design(){  
		$rec_no = array('4811');
		for($i=0;$i<=0;$i++){
		echo $attach = custome_generate_bulk_examinvoice_new_design($rec_no[$i]); 	
        echo "<br/>"; 
		}
	}
	...
	public function custom_mem_reg_invoice_new(){   
		...
		$arr = array(4994137,4994136,4994135,4994134,4994133,4994046,4894381,4779084,4779009,4778620,4777586);       
		 for($i=0;$i<sizeof($arr);$i++){
	...
	public function custom_exam_invoice_new(){   
		// $invoice_id = 1835819;
		
		$arr = array(4208618,4208842,4208934);     // invoice id   
		 for($i=0;$i<sizeof($arr);$i++){
		 	echo $path = genarate_exam_invoice($arr[$i]);
		 	echo "<br/>"; 
		 }
	}
	...
	public function fu_genarate_amp_invoice_custom(){ 
		$invoice_id = 4254116;
		echo $path = genarate_amp_invoice_custom($invoice_id);
	}
...
public function send_mail(){
	...
	public function GenerateDraMemberAdmitCard()
	{
		//exit;
		error_reporting(E_ALL);
		$mem_mem_no = array(801549389,801549408,801549404,801549405,801549403,801549395,801549396,801549400,801549399,801549398,801549401,801549402,801464057,801464055,801464056,801464046,801464040,801464043,801464054,801549397,801549387,801549386,801549385,801549384,801549383,801549382,801549381,801549380,801549379,801549378,801549377,801549376,801549388);
		$exm_cd = '45'; 
		$exm_prd = '777';	
	...
	public function GenerateDraPaymentInvoice()
	{
		error_reporting(E_ALL);
		//$invoice_id = "3291587";
		//echo "<br>".$path = genarate_dra_exam_invoice_custom($invoice_id);
		...
		{
			echo "<br>";
			echo "<br>".$path = genarate_dra_exam_invoice_custom($arr[$i]);
		}
		
		
	}
	
		/** FUNCTION ADDED  TO GENERATE charterd INVOICE using invoice id with manual updation in table***/
	public function GenerateCharteredPaymentInvoice()
	{
		error_reporting(E_ALL);
        //exam invoice id in array
		$arr = array(2846128);
		for($i=0;$i<sizeof($arr);$i++)
		{
			echo "<br>";
			echo "<br>".$path = genarate_chartered_invoice_custom($arr[$i]);
		}
	
	}
	
		/** FUNCTION ADDED  TO GENERATE charterd INVOICE using payment trans id***/
...
public function dra_mem_period_update(){
 die;
    
	$pay_trans = $this->master_model->getRecords('dra_payment_transaction', array( 'exam_period' => 0), 'id,exam_period');

$q=1;
foreach ($pay_trans as $key ) {

	
	$dra_mem_pay = $this->master_model->getRecords('dra_member_payment_transaction', array( 'ptid ' => $key['id']), 'memexamid');
      //print_r($dra_mem_pay);die;
      
	foreach ($dra_mem_pay as $dra_mem_pay ) {
    
		$update_member_app=array(
         'exam_period' => $key['exam_period']
		);
		$this->master_model->updateRecord('dra_member_exam',$update_member_app,array('id'=>$dra_mem_pay['memexamid']));
...
## Code added by pratibha on 31 MAr 2021
	public function subject_missing_RR(){
		
		$member_array = array(); 
		//$exarr = array(992);  
		$exarr = array($this->config->item('examCodeJaiib')); 
		$this->db->select('mem_exam_id');  
		$this->db->distinct('mem_exam_id');  
		$this->db->where_in('exm_cd',$exarr); 
		$this->db->where('exm_prd',121);
		$this->db->where('remark','1');
		$this->db->where('app_update','0');
		$this->db->where('created_on >= ','2021-03-2 00:00:01');   
		$this->db->where('created_on <= ','2021-03-2 23:59:59');         
	...
	public function subject_missing_freshpp(){         
		$member_array = array();
		//$exarr = array(992);    
		$exarr = array($this->config->item('examCodeJaiib'));   
		$this->db->select('mem_exam_id'); 
		...
		$this->db->where('remark','1');
		$this->db->where('app_update','0');
		$this->db->where('created_on >= ','2021-03-09 00:00:01');        
		$this->db->where('created_on <= ','2021-03-09 23:59:59');    
		$admit_card = $this->master_model->getRecords('admit_card_details','','mem_mem_no,exm_cd');
	...
	function getExamFee($centerCode = 751, $eprid = 810, $excd = 'MjAyNw==', $grp_code = 'S1', $memcategory = 'O', $elearning_flag = 'N')
	{
		$fee=0;
		$CI = & get_instance();
		...
		if($centerCode !=NULL  && $eprid !=NULL  && $excd !=NULL  && $grp_code !=NULL  && $memcategory !=NULL )
		{
			...
			echo '</br>Q1:'.$CI->db->last_query();//exit;
			if(count($getstate) <= 0)
			{
			...
			if(count($getstate) > 0)
			{
				...
				else
				{
				 ...
				 if(in_array($CI->session->userdata('regnumber'), array(510362692))) 
				 { $today_date = '2022-06-08'; }
	...
	public function send_RPE_mail_Pooja(){
		...
		/*$member_candidate_qry= $this->db->query("SELECT exm_cd, exm_prd, mem_type, g_1, mem_mem_no, mam_nam_1, email, mobile, center_code, center_name, sub_cd, sub_dsc, venueid, venue_name, CONCAT(`venueadd1`,', ',`venueadd2`,', ',`venueadd3`,', ',`venueadd4`,', ',`venueadd5`) AS VENUE_ADDRESS, venpin, seat_identification, pwd, exam_date, time, mode, m_1, scribe_flag, vendor_code, admitcard_image FROM `admit_card_details`, `member_registration` 
                                                WHERE `exm_cd` IN (1002,1003,1004,1005,1009) 
                                                AND `remark` = 1 
                                                AND member_registration.regnumber = admit_card_details.mem_mem_no 
                                                AND isactive = '1' 
                                                and exm_prd IN (818) 
                                                and exam_date = '2023-02-25'
                                                AND admitcard_image != ''");*/
		...
		{
			$email = $member['email'];
			//$email = array('pooja.mane@esds.co.in','priyanka.wadnere@esds.co.in');
			$exam_name = $member['sub_dsc'];
			
			$admitcard_image = 'https://iibf.esdsconnect.com//uploads/admitcardpdf/'.$member['admitcard_image'];

			$final_str = 'Hello Sir/Madam <br/><br/>';
			$final_str.= 'Please visit IIBF Portal to download your admit card letter for '.$exam_name.' examination.';   
			$final_str.= 'or Please refer a link to download your Admit card using '.$admitcard_image.' the link.';  
			$final_str.= '<br/><br/>';
			$final_str.= 'Regards,';
			$final_str.= '<br/>';
			$final_str.= 'IIBF TEAM'; 
