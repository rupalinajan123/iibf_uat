<?php
	...
	class Cron5 extends CI_Controller {
		...
		public function getResponse()
		{
			...
			$aes->require_pkcs5();
			$query='SELECT * FROM payment_transaction WHERE date(date) BETWEEN "2018-04-01" AND "2019-03-31" and status = 1 and amount<2000 and gateway = "sbiepay" AND `paymode` IS NULL';
		...
		{
			$log_title ="sbicallback Function call Cron5";
			$log_message = 'sbicallback Function call Cron5';
			$rId = '';
			$regNo = '';
			$this->member_regn_log($log_title, $log_message, $rId, $regNo);
			$filehandle = fopen("cs2s_log/cron5/lock.txt", "c+");
			if (flock($filehandle, LOCK_EX | LOCK_NB)) {
				...
				AND date > NOW() - INTERVAL 120 MINUTE AND gateway = "sbiepay" AND status = 2';
				$crnt_day_txn_qry = $this->db->query($query);
				...
				if ($crnt_day_txn_qry->num_rows())
				{
					...
					$dir = 'cs2s_log/cron5/'.$todays_date;
					if(!is_dir($dir)){
						mkdir($dir, 0755);
					}
					$cell = 1;
					...
					foreach ($crnt_day_txn_qry->result_array() as $c_row)
					{
						...
						if (isset($responsedata) && count($responsedata) > 0)
						{
							...
							/*if($responsedata[12]=='iibfregn')
								...
								## Add query log
								$log_title ="Payment status update in payment transaction for member registration Cron5 C_S2S  :".$reg_id;
								...
								########get Old image Name############
								$log_title ="Ordinory member OLD Image Cron5 C_S2S :".$reg_id;
								...
								$log_title ="Ordinory member PIC update Cron5 C_S2S :".$reg_id;
								...
								$upd_files['idproofphoto'] = $proof_file;
								$this->master_model->updateRecord('member_registration',$upd_files,array('regid'=>$reg_id));
								$log_title ="Member MANUAL PICS Update Cron5 C_S2S :".$reg_id;
								$log_message = serialize($upd_files);
								$rId = $reg_id;
								$regNo = $reg_id;
								//storedUserActivity($log_title, $log_message, $rId, $regNo);	
								...
								$log_title ="Ordinory member invoice update C_S2S Cron5 Invoice number:".$invoiceNumber;
							...
							*/
							if ($responsedata[12] == "iibfren")
							{
								...
								if($payment_status==1)
								{
									...
									if ($get_user_regnum_info[0]['status'] == 2) {
										...
										$update_data = array('member_regnumber' => $applicationNo, 'transaction_no' => $transaction_no, 'status' => 1, 'transaction_details' => $responsedata[2] . " - " . $responsedata[7], 'auth_code' => '0300', 'bankcode' => $responsedata[9], 'paymode' =>  $responsedata[16], 'callback' => 'c_S2S');
										$this->master_model->updateRecord('payment_transaction', $update_data, array('receipt_no' => $MerchantOrderNo));
								...
								else if($payment_status==0)
								{
									...
									if($get_user_regnum_info[0]['status']==2)
									{
										$update_data = array('transaction_no' => $transaction_no, 'status' => 0, 'transaction_details' => $responsedata[2] . " - " . $responsedata[7], 'auth_code' => '0399', 'bankcode' => $responsedata[9], 'paymode' =>  $responsedata[16], 'callback' => 'c_S2S');
										$this->master_model->updateRecord('payment_transaction', $update_data, array('receipt_no' => $MerchantOrderNo));
									}
								}
							}
							
							else if($responsedata[12] == "iibfdup")
							{
								sleep(8);
								$MerchantOrderNo = $responsedata[6];  
								$transaction_no  = $responsedata[1];
							...
							else if($responsedata[12] == "IIBF_EXAM_O")
							{
								sleep(8);
								$MerchantOrderNo = $responsedata[6];  
								$transaction_no  = $responsedata[1];
								...
								{	
									...
									if($get_user_regnum[0]['status']==2)
									{
										...
										if($get_user_regnum[0]['exam_code']=='1016')
										{
											...
											/* Transaction Log */
											$pg_response = "encData=" . $encData . "&merchIdVal=" . $merchIdVal . "&Bank_Code=" . $Bank_Code;
											$this->log_model->logcharteredtransaction("sbiepay", $pg_response, $responsedata[2]);
											//echo $reg_id; die;
											$newlog=array(
							...
							else if($responsedata[6] == "IPPB_EXAM_REG")
							{
								...
								{	
									...
									if($get_user_regnum[0]['status']==2) 
									{
										...
										$update_data = array('transaction_no' => $transaction_no,'status' => 1, 'transaction_details' => $responsedata[2]." - ".$responsedata[7],'auth_code' => '0300', 'bankcode' => $responsedata[8], 'paymode' => $responsedata[5],'callback'=>'C_S2S');
										...
										if($this->db->affected_rows())
										{
											...
											######### payment Transaction ############
											$update_data = array('transaction_no' => $transaction_no,'status' => 1,'member_regnumber'=>$applicationNo,'transaction_details' => $responsedata[2]." - ".$responsedata[7],'auth_code' => '0300', 'bankcode' => $responsedata[8], 'paymode' => $responsedata[5],'callback'=>'C_S2S');
											$this->db->order_by('id','DESC');
											$this->db->limit(1);	
											$this->master_model->updateRecord('payment_transaction',$update_data,array('receipt_no'=>$MerchantOrderNo));
							...
							else if ($responsedata[12] == "iibfXLRI")
							{
								sleep(8);
								$MerchantOrderNo = $responsedata[6];  
								$transaction_no  = $responsedata[1];
							...
							else if($responsedata[12] == "iibfmisc") ## Code added on 14 May 2021
							{
								sleep(2);
								$MerchantOrderNo = $responsedata[6]; 
								$transaction_no  = $responsedata[1];
							...
							/* else if($responsedata[12] == "iibfcc") ## iibfcc code added on 16 Mar 2021
							{
								sleep(8);
								$MerchantOrderNo = $responsedata[6]; 
								$transaction_no  = $responsedata[1];
								...
								if($payment_status==1)
								{
									...
									if ($get_user_regnum_info[0]['status'] == 2) {
									  ...
									  $update_bank_data = array(
										...
										'contact_classes_id' => $ref_id));
										
										//subject remark update
										...
										FROM contact_classes_registration
										GROUP BY member_no
										) and pay_status = 1 AND member_no=" . $mem_no);
										...
										'member_no' => $mem_no,'center_code'=>$memtype[0]['center_code'],'contact_classes_regid'=>$memtype[0]['contact_classes_id']
										));                   
										
										
										// email to user
										$emailerstr = $this->master_model->getRecords('emailer', array('emailer_name' => 'contactclasses'));
										$sms_template_id = 'IO0yFSwGR';
									  ...
									  $info_arr = array('to'=>$memtype[0]['email'],
										...
										if ($user_info[0]['zone_code'] == 'NZ') {
											$client_arr = array(
											'to'=>'sanjay@iibf.org.in,mkbhatia@iibf.org.in',
							...
							else if($responsedata[12] == "IIBFELS" && $responsedata[2] == 'SUCCESS')
							{
								$MerchantOrderNo = $responsedata[6];  
								$transaction_no  = $responsedata[1];
								$payment_status = 2;
								...
								if($payment_status==1)
								{
									...
									{
										######### payment Transaction ############
										$update_data = array('transaction_no' => $transaction_no,'status' => 1, 'transaction_details' => $responsedata[2]." - ".$responsedata[7],'auth_code' => '0300', 'bankcode' => $responsedata[8], 'paymode' => $responsedata[16],'callback'=>'C_S2S');
										$update_query=$this->master_model->updateRecord('payment_transaction',$update_data,array('receipt_no'=>$MerchantOrderNo,'status'=>2));
								...
								else if($payment_status==0)
								{
									...
									{
										######### payment Transaction ############
										$update_data = array('transaction_no' => $transaction_no,'status' => 0, 'transaction_details' => $responsedata[2]." - ".$responsedata[7],'auth_code' => '0300', 'bankcode' => $responsedata[8], 'paymode' => $responsedata[5],'callback'=>'C_S2S');
										$update_query=$this->master_model->updateRecord('payment_transaction',$update_data,array('receipt_no'=>$MerchantOrderNo,'status'=>2));
							...
							else if($responsedata[12] == "IIBF_INST_SUB")
							{
								$MerchantOrderNo = $responsedata[6];  
								$transaction_no  = $responsedata[1];
								$payment_status = 2;
