<?php 
    ...
    public function index()
		{   
			...
			$data['sub_act_id'] = "";
      $data['page_title'] = 'IIBF - Exam Macroresearch Admin Dashboard';

    
      $this->load->view('macroresearch/admin/dashboard_admin', $data);
    }
    ...
    function change_password()
      ...
      { 
        $data['page_title'] = 'IIBF - Macroresearch Admin Change Password'; 

        $data['form_data'] = $form_data = $this->master_model->getRecords('macroresearch_admin', array('admin_id' => $this->login_admin_id), 'admin_id, is_active, is_deleted');
        ...
        $log_slug = 'admin_self_password_action';
          ...
          $admin_name = $this->macroresearch_model->getLoggedInUserDetails($this->login_admin_id, $this->login_user_type);

          if($this->login_user_type == "admin")
          {
            $up_data['admin_password'] = $this->macroresearch_model->password_encryption($this->input->post('new_pass_admin'));
            $up_data['updated_on'] = date("Y-m-d H:i:s");
            $up_data['updated_by'] = $this->login_admin_id;
            $this->master_model->updateRecord('macroresearch_admin', $up_data, array('admin_id' => $this->login_admin_id));
            
            $this->macroresearch_model->insert_common_log('Admin : Profile password updated', 'macroresearch_admin', $this->db->last_query(), $this->login_admin_id,'admin_self_password_action','The admin '.$admin_name['disp_name'].' has successfully updated the password', $posted_arr); 
          }  
					$this->session->set_flashdata('success','Password successfully updated');
					
          redirect(site_url('macroresearch/admin/dashboard_admin/change_password'));
				}
			}
			
      $data["enc_login_admin_id"] = url_encode($this->login_admin_id);
      
			$this->load->view('macroresearch/admin/change_password_admin', $data);
    ...
    /******** START : VALIDATION FUNCTION TO CHECK THE OLD PASSWORD ********/
      ...
      $return_val_ajax = 'false';
        ...
        if($type == '1') { $current_pass_admin = $this->security->xss_clean($this->input->post('current_pass_admin')); }
				...
				$enc_password = $this->macroresearch_model->password_encryption($current_pass_admin);

        if($this->login_user_type == "admin") 
        { 
          if(count($this->master_model->getRecords('macroresearch_admin', array('admin_password' => $enc_password, 'admin_id' => $this->login_admin_id, 'is_active' => '1', 'is_deleted' => '0'), 'admin_id, is_active, is_deleted')) > 0)
          {
