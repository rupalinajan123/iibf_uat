...
class Cron_csv_ftp extends CI_Controller
{
    ...
    public function sendmail($current_date = "", $msg = "")
    {
        ...
        // New Email List : Updated on 18 June 2018 - Bhushan
				//'Kanhiya.agrawal@mpsinteractive.com', 'Sandeep.agarwal@mpsinteractive.com', 'barun.yadav@mpsinteractive.com', 'iibflmssupport@mpsinteractive.com',
	...
	// MPS FTP Code : 07/July/2020 : Bhushan 
	// /usr/local/bin/php /home/supp0rttest/public_html/index.php admin Cron_csv_ftp mps
	public function mps($current_date = "")
    {
        $start_time = date("Y-m-d H:i:s");
        $success    = array();
        $error      = array();
        ...
        $desc   = json_encode($result);
        $this->log_model->cronlog("MPS File Transfer Cron - Start", $desc);
        ...
        $CurrentDate = date('Y-m-d', strtotime($current_date));
        $this->db->where('CurrentDate', $CurrentDate);
        $array         = $this->master_model->getRecords('cron_csv_mps', '', 'new_file_name,record_count');
        $new_file_name = $array[0]['new_file_name'];
        $record_count  = $array[0]['record_count'];
        ...
        $local_server_dir_path = '/home/supp0rttest/public_html/uploads/cronCSV_mps/' . $current_date . '/';
        ...
        $desc   = json_encode($result);
        $this->log_model->cronlog("MPS CSV File Transfer Cron - Ends", $desc);
        ...
        // send email notification -
        $result = $this->mpssendmail($current_date, $msg);
    }
    public function mpssendmail($current_date = "", $msg = "")
    {
        if ($current_date == "") {
            $current_date = date("Ymd");
        }
        ...
        //$from_email = 'noreply@iibf.org.in';
        $from_email = 'logs@iibf.esdsconnect.com';
        $subject    = 'MPS : Exam Applications CSV File Transfered';
        ...
        $attachment_filename = 'logs_' . $current_date . '.txt';
        $attachment_path     = './uploads/cronCSV_mps/' . $current_date . '/' . $attachment_filename;
	...
	public function mps_caiib($current_date = "")
	{
        ...
        $desc   = json_encode($result);
        $this->log_model->cronlog("MPS CAIIB File Transfer Cron - Start", $desc);
        ...
        $CurrentDate = date('Y-m-d', strtotime($current_date));
        $this->db->where('CurrentDate', $CurrentDate);
        $array         = $this->master_model->getRecords('caiib_csv_mps', '', 'new_file_name,record_count');
        $new_file_name = $array[0]['new_file_name'];
        $record_count  = $array[0]['record_count'];
        ...
        $local_server_dir_path = '/home/supp0rttest/public_html/uploads/caiib_mps/' . $current_date . '/';
        ...
        $desc   = json_encode($result);
        $this->log_model->cronlog("MPS CAIIB CSV File Transfer Cron - Ends", $desc);
        ...
        // send email notification -
        $result = $this->mpssendmail_caiib($current_date, $msg);
    }
		
    public function mpssendmail_caiib($current_date = "", $msg = "")
    {
        if ($current_date == "") {
            $current_date = date("Ymd");
        }
        ...
        //$from_email = 'noreply@iibf.org.in';
        $from_email = 'logs@iibf.esdsconnect.com';
        $subject    = 'MPS CAIIB: Exam Applications CSV File Transfered';
        ...
        $attachment_filename = 'logs_' . $current_date . '.txt';
        $attachment_path     = './uploads/caiib_mps/' . $current_date . '/' . $attachment_filename;
	...
	// KESDEE CAIIB FTP Code : CODE ADDED BY SAGAR ON 07-04-2021 TO SEND DATA THROUGH FTP TO KESDEE VENDOR
	// /usr/local/bin/php /home/supp0rttest/public_html/index.php admin Cron_csv_ftp kesdee_caiib
	public function kesdee_caiib()
	{
		...
		$desc   = json_encode($result);
		$this->log_model->cronlog("KESDEE CAIIB File Transfer Cron - Start", $desc);
		...
		$CurrentDate = date('Y-m-d', strtotime($current_date));
		$this->db->where('CurrentDate', $CurrentDate);
		$array = $this->master_model->getRecords('caiib_csv_kesdee', '', 'new_file_name,record_count');
		$new_file_name = $array[0]['new_file_name'];
		$record_count  = $array[0]['record_count'];
		...
		$local_server_dir_path = '/home/supp0rttest/public_html/uploads/caiib_kesdee/' . $current_date . '/';
		...
		$desc   = json_encode($result);
		$this->log_model->cronlog("KESDEE CAIIB CSV File Transfer Cron - Ends", $desc);
		...
		if ($record_count != 0) 
		{
			...
			$msg = '';
			$msg .= 'Dear IIBF LMS Support Team,<br><br>';
			$msg .= 'The file named ' . $new_file_name . ' with a total of ' . $record_count . ' records has been copied to the SFTP location on ' . date("Y/m/d", strtotime($today_date)).date(" H:i:s") . ' for further processing.  Please inform the IIBF team once the file has been processed.<br><br>';
			$msg .= 'If the file gets corrupted or there is mismatch in the number of records, please inform iibfdevp@esds.co.in.<br><br>';
			$msg .= 'Regards,<br>';
			$msg .= 'The ESDS Team.';
		}
		...
		// send email notification -
		$result = $this->kesdee_sendmail_caiib($current_date, $msg);
	} 
		
	public function kesdee_sendmail_caiib($current_date = "", $msg = "")
	{
		...
		//$from_email = 'noreply@iibf.org.in';
		$from_email = 'logs@iibf.esdsconnect.com';
		$subject    = 'KESDEE CAIIB: Exam Applications CSV File Transferred';
    
		$recipient_list = array('manoj@kesdee.com', 'venuk@kesdee.com', 'info@kesdee.com', 'kalpanashetty@iibf.org.in', 'priti@iibf.org.in', 'dd.aca4@iibf.org.in', 'elearning@iibf.org.in', 'smuralidaran@iibf.org.in', 'iibfdevp@esds.co.in','dd.aca2@iibf.org.in','Je.aca7@iibf.org.in');
		
		$attachment_filename = 'logs_' . $current_date . '.txt';
		$attachment_path     = './uploads/caiib_kesdee/' . $current_date . '/' . $attachment_filename;
		...
		$this->email->subject($subject);
		$this->email->message($message);
		if ($attachment_path != '') { $this->email->attach($attachment_path); }
