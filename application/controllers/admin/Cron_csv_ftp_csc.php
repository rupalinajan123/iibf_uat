<?php
/*
 * Controller Name	:	Cron File Transfer Windows Server : CSC Vendor
 * Created By		:	Bhushan Amrutkar
 * Created Date		:	29 May 2019
 *
*/
defined('BASEPATH') OR exit('No direct script access allowed');
class Cron_csv_ftp_csc extends CI_Controller {
	...
	public function index($current_date = "")
	{
        ...
        $array = $this->master_model->getRecords('cron_csv_csc','','new_file_name,record_count');
		...
		//$current_date = date("Ymd");
		//$path_list = array("/shezartech/".$current_date."/");
		...
		//$local_server_dir_path = '/home/iibfgrow/public_html/uploads/cronCSV_csc/'.$current_date.'/';
		$local_server_dir_path = '/home/supp0rttest/public_html/uploads/cronCSV_csc/'.$current_date.'/';
		...
		if($login_result){
			...
			if (ftp_mkdir($conn, $cron_file_dir.$current_date)){
				echo "Directory Successfully created";
		  		...
		  		echo "Error while creating Directory or it is already exist";
		  	}
			
			$path_list = array("/csc/".$current_date."/");
	...
	public function sendmail($current_date = "", $msg = ""){
		...
		$subject = 'Member Exam Applications CSV File Transfer To CSC Vendor';
	
		$recipient_list = array('kalpanashetty@iibf.org.in', 'elearning@iibf.org.in',' smuralidaran@iibf.org.in','iibfdevp@esds.co.in','omveer@csc.gov.in','neeraj.kr@nic.in','rishikesh@csc.gov.in','iibfexam@cscacademy.org');
		
		//$recipient_list = array('bhushan.amrutkar@esds.co.in');
		
		$attachment_filename = 'logs_'.$current_date.'.txt';
		$attachment_path = './uploads/cronCSV_csc/'.$current_date.'/' . $attachment_filename;
	...
	// Cron_csv_ftp_csc/csc_free
	public function csc_free($current_date = "")
	{
		$start_time = date("Y-m-d H:i:s");
		$success = array();
		$error = array();
		...
		$CurrentDate = date('Y-m-d',strtotime($current_date));
		$this->db->where('CurrentDate', $CurrentDate);
        $array = $this->master_model->getRecords('cron_csv_csc_free','','new_file_name,record_count');
		$new_file_name = $array[0]['new_file_name'];
		$record_count = $array[0]['record_count'];
		...
		if($login_result){
			...
			$path_list = array("/CSC_FREE_APP/".$current_date."/");
		...
		// send email notification -
		$result = $this->sendmail_free($current_date, $msg);
	}
	
	public function sendmail_free($current_date = "", $msg = ""){
		...
		//$from_email = 'noreply@iibf.org.in';
		$from_email = 'logs@iibf.esdsconnect.com';
		$subject = 'Free CSC : Member Exam Applications CSV File Transfer To CSC Vendor';
