...
class Cron extends CI_Controller {
	...
	public function admit_update_caiib_220()
	{
		...
		$select = '*';
		$this->db->limit(3000); 
		$temp_admit_card_data = $this->Master_model->getRecords('admit_card_details_jaiib',array('exm_cd'=>$this->config->item('examCodeCaiibElective71'),'exm_prd'=>'220','remark'=>'0'),$select);
		//echo $this->db->last_query();
		if(count($temp_admit_card_data))
		{
			foreach($temp_admit_card_data as $admit_row)
			{
				...
				$where_admit_card_arr = array('mem_mem_no'=>$admit_row['mem_mem_no'],
								'exm_cd'=>$this->config->item('examCodeCaiibElective71'),
								'exm_prd'=>'219',
	...
	public function exam()
	{
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			'status'=>1,'isactive'=>'1','isdeleted'=>0,'pay_status'=>1,'pay_type'=>2),$select);
...
//EXM_CD|EXM_PRD|MEMBER_NO|CTR_CD|CTR_NAM|SUB_CD|SUB_DSC|VENUE_CD|VENUE_ADDR1|VENUE_ADDR2|VENUE_ADDR3|VENUE_ADDR4|VENUE_ADDR5|VENUE_PINCODE|EXAM_SEAT_NO|EXAM_PASSWORD|EXAM_DATE|EXAM_TIME|EXAM_MODE(Online/Offline)| EXAM_MEDIUM|SCRIBE_FLG(Y/N)|VENDOR_CODE(1/3)|TRN_DATE|
  ...
  /* Only for Free CSC Vendor : 30-09-2020 */
	...
	/* Institute Subscription Cron */
	// https://iibf.esdsconnect.com/staging/admin/Cron/institute_subscription
	public function institute_subscription()
	{
		...
		$result = array("success" => "", "error" => "", "Start Time" => $start_time, "End Time" => "");  
		$desc = json_encode($result);
		$this->log_model->cronlog("Institute Subscription Details Cron Execution Start", $desc);
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			$file = "iibf_institute_subscription_details_".$current_date.".txt";
			$fp = fopen($cron_file_path.'/'.$file, 'w');
			
			$file1 = "logs_institute_subscription_".$current_date.".txt";
			$fp1 = fopen($cron_file_path.'/'.$file1, 'a');
			fwrite($fp1, "\n***** IIBF Institute Subscription Details Cron Start - ".$start_time." ***** \n");
			
			$yesterday = date('Y-m-d', strtotime("- 1 day"));
			
			$yesterday = '2024-04-01';
			// $mem = array(802491425,802493061);//
			// $this->db->where_in('regnumber', $mem);
			
			$this->db->join('institute_subscription i','i.subscription_id = a.ref_id');
			$new_inst_sub = $this->Master_model->getRecords('payment_transaction a',array(' DATE(date)'=>$yesterday,'status'=>'1','pg_flag'=> 'IIBF_INST_SUB'),'a.member_regnumber,a.amount,a.date,a.transaction_no,a.receipt_no,i.institute_no,i.invoice_no,i.institute_name,i.subscription_base_amount,i.subscription_gst_amount,i.it_tds_percentage_rate,i.it_tds_percentage_amount,i.gst_tds_percentage_rate,i.gst_tds_percentage_amount,i.subscription_year,a.ref_id,a.pg_flag');	
			// echo $this->db->last_query();
			// echo '<br>data_count is - '.count($new_inst_sub);
			// exit;
			$data = '';
			if(count($new_inst_sub))
			{
				
				$i = 0;
				$data = 'institute_no|invoice_no|institute_name|subscription_year|date|subscription_base_amount|subscription_gst_amount|it_tds_percentage_rate|it_tds_percentage_amount|gst_tds_percentage_rate|gst_tds_percentage_amount|amount|transaction_no|receipt_no|ref_id|pg_flag'."\n";

				$file_w_flg = fwrite($fp, $data);

				foreach($new_inst_sub as $new_inst)
				{
					...
					// echo '<pre>';print_r($new_inst);die;
					//institute_no|invoice_no|institute_name|subscription_year|date|amount|transaction_no|receipt_no|ref_id|pg_flag


					if($new_inst['date']!='0000-00-00')
					{	$transaction_date = date('d-M-y',strtotime($new_inst['date']));	}
					
					$data = ''.$new_inst['institute_no'].'|'.$new_inst['invoice_no'].'|'.$new_inst['institute_name'].'|'.$new_inst['subscription_year'].'|'.$transaction_date.'|'.$new_inst['subscription_base_amount'].'|'.$new_inst['subscription_gst_amount'].'|'.$new_inst['it_tds_percentage_rate'].'|'.$new_inst['it_tds_percentage_amount'].'|'.$new_inst['gst_tds_percentage_rate'].'|'.$new_inst['gst_tds_percentage_amount'].'|'.$new_inst['amount'].'|'.$new_inst['transaction_no'].'|'.$new_inst['receipt_no'].'|'.$new_inst['ref_id'].'|'.$new_inst['pg_flag']."\n";
					...
					if($file_w_flg)
					{
						$success['file'] = "Institute Subscription Member Details File Generated Successfully. ";
					}
					else
					{
						$error['file'] = "Error While Generating Institute Subscription Member Details File.";
					}
				}
				// echo'<pre>';print_r($data);die;
				
				fwrite($fp1, "Total Institute Subscription Members Added = ".$i."\n");
			}
			else
			{
				$success[] = "No data found for the date";
			...
			fclose($fp);
			echo "Data Generated for Institute Subscription Members -".$i;

			// Cron End Logs
			$end_time = date("Y-m-d H:i:s");
			$result = array("success" => $success, "error" => $error, "Start Time" => $start_time, "End Time" => $end_time);
			$desc = json_encode($result);
			$this->log_model->cronlog("Institute Subscription Member Details Cron End", $desc);
			
			fwrite($fp1, "\n"."******* IIBF Institute Subscription Member Details Cron End ".$end_time." ********"."\n");
			fclose($fp1);
		}
	}

	public function institute_subscription_ex()
	{
		ini_set("memory_limit", "-1");
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			$yesterday = '2024-01-24';
			// $mem = array(802491425,802493061);//
			// $this->db->where_in('regnumber', $mem);
			
			// $this->db->where_in('a.excode', $excode);
			$new_inst_sub = $this->Master_model->getRecords('payment_transaction a',array(' DATE(date)'=>$yesterday,'status'=>'1','pg_flag'=> 'IIBF_INST_SUB'));	
			echo $this->db->last_query();
			echo '<br>data_count is - '.count($new_inst_sub);
			exit;
			...
			if(count($new_inst_sub))
			{
				...
				$sign_cnt = 0;
				$idproof_cnt = 0;
				foreach($new_inst_sub as $reg_data)
				{
					$data = '';
					...
					if($dir_flg)
					{
						...
						if($photo_zip_flg || $sign_zip_flg || $idproof_zip_flg)
						{
							$success['zip'] = "Digital Subscription Member Images Zip Generated Successfully";
						}
						else
						{
							$error['zip'] = "Error While Generating Institute Subscription Member Images Zip";
						}
					}
					...
					if($file_w_flg)
					{
						$success['file'] = "Digital Subscription Member Details File Generated Successfully. ";
					}
					else
					{
						$error['file'] = "Error While Generating Institute Subscription Member Details File.";
				...
				fwrite($fp1, "\n"."Total Institute Subscription Members Added = ".$mem_cnt."\n");
				fwrite($fp1, "\n"."Total Institute Subscription Photographs Added = ".$photo_cnt."\n");
				fwrite($fp1, "\n"."Total Institute Subscription Signatures Added = ".$sign_cnt."\n");
				fwrite($fp1, "\n"."Total Institute Subscription ID Proofs Added = ".$idproof_cnt."\n");
			...
			fclose($fp);
			echo "Data Generated for Institute Subscription Members -".$mem_cnt;
			echo "<br>Total Photographs Added = ".$photo_cnt;
			echo "<br>Total Signatures Added = ".$sign_cnt;
			echo "<br>Total ID Proofs Added = ".$idproof_cnt;
			// Cron End Logs
			$end_time = date("Y-m-d H:i:s");
			$result = array("success" => $success, "error" => $error, "Start Time" => $start_time, "End Time" => $end_time);
			$desc = json_encode($result);
			$this->log_model->cronlog("Digital Subscription Member Details Cron End", $desc);
