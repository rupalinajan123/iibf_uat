...
class Cron_payment_custom extends CI_Controller {
	...
	*/
	public function index()
	{
		...
		//$pt_query = $this->db->query("SELECT * FROM payment_transaction WHERE gateway = 'sbiepay' AND status = 2 AND (pay_type = '1') AND receipt_no IN (811879362,811879363,811879364,811879367,811879369,811879371)");
		$pt_query = $this->db->query("SELECT * FROM payment_transaction WHERE gateway = 'sbiepay' AND status = 2 AND (pay_type = '3') AND receipt_no IN (19830)");
	...
	private function update_id_card_transaction($MerchantOrderNo, $reg_no, $q_details)
	{
		if ($q_details[2] == "SUCCESS")
		{
			...
			if($get_user_regnum[0]['status']==2)
			{
				...
				$update_data = array('transaction_no' => $transaction_no,'status' => 1, 'transaction_details' => $responsedata[2]." - ".$responsedata[7],'auth_code' => '0300', 'bankcode' => $responsedata[8], 'paymode' => $responsedata[5],'callback'=>'DV');
				$this->master_model->updateRecord('payment_transaction',$update_data,array('receipt_no'=>$MerchantOrderNo));
		...
		else if ($q_details[2] == "FAIL")
		{
			...
			$update_data = array('transaction_no' => $transaction_no,'status' => 0,'transaction_details' => $responsedata[2]." - ".$responsedata[7],'auth_code' => '0399', 'bankcode' => $responsedata[8], 'paymode' => $responsedata[5],'callback'=>'DV');
			$this->master_model->updateRecord('payment_transaction',$update_data,array('receipt_no'=>$MerchantOrderNo));
		}
	}
