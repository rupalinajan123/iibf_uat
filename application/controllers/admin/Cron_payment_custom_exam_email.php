...
class Cron_payment_custom_exam_email extends CI_Controller {
	...
	*/
	public function index() 
	{
		...
		*/    
	    ...
	    //for exam
	    $pt_query = $this->db->query("SELECT * FROM payment_transaction WHERE gateway = 'sbiepay' AND status = 1 AND (pay_type = '2') AND receipt_no IN (901012778)");//
		//exit;
		//for registration
		//$pt_query = $this->db->query("SELECT * FROM payment_transaction WHERE gateway = 'sbiepay' AND status = 1 AND (pay_type = '1') AND receipt_no IN (900339264)"); 
...
//				$MerchantOrderNo = $responsedata[0]; 
			...
			{	
				...
				echo 'email : ',print_r($result[0]['email']);  
		...
		$attachpath='https://iibf.esdsconnect.com/uploads/examinvoice/user/510238660_EX_18-19_123113.jpg'; 
		$admitcard_pdf='https://iibf.esdsconnect.com/uploads/admitcardpdf/59_803_510238660.pdf';	
			...
			{ 		     
				...
				//$this->master_model->send_sms_trustsignal(intval($result[0]['mobile']),$sms_final_str,'template_id');
			...
			$attachpath='https://iibf.esdsconnect.com/uploads/examinvoice/user/801187490_EX_17-18_208983.jpg';
			//$admitcard_pdf='https://iibf.esdsconnect.com/uploads/admitcardpdf/1600_417_801176274.pdf';  
				...
				if($payment_status==1)
				{
					 ...
					 if(count($emailerstr) > 0)
					{
						...
						{	
							...
							//$this->master_model->send_sms_trustsignal(intval($user_info[0]['mobile']),$sms_final_str,'template_id');
...
function jaiib_exam()
{
	$emailerstr=$this->master_model->getRecords('emailer',array('emailer_name'=>'jaiib_Exam'));
	$info_arr=array(//'to'=>$result[0]['email'],
											//'to'=>'mukesh.bohat@yahoo.co.in',
											'from'=>$emailerstr[0]['from'],
											'subject'=>$emailerstr[0]['subject'],
