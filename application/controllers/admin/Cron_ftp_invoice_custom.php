...
class Cron_ftp_invoice_custom extends CI_Controller {
	...
	public function index($current_date = "")
	{
		...
		//xxx$this->log_model->cronlog("Invoice File Transfer Cron - Start", $desc);
		...
		$local_server_dir_path = '/home/supp0rttest/public_html/uploads/rahultest/'.$current_date.'/';
		//$local_server_dir_path = '/home/supp0rttest/public_html/uploads/cronFilesCustom/'.$current_date.'/';
		echo '<br>local_server_dir_path - '.$local_server_dir_path;
		...
		if($login_result)
		{
			...
			foreach($file_list as $key => $file_name)
			{
				...
				echo '<br>remote_server_dir_path - '.$remote_server_dir_path;
				...
				echo '<br>remote_file_path - '.$remote_file_path;
				echo '<br>';
		...
		$desc = json_encode($result);
		//xxx$this->log_model->cronlog("Invoice File Transfer Cron - Ends", $desc);
	...
	public function sendmail($current_date = "", $msg = "")
	{
		...
		$from_name = 'IIBF';
		$from_email = 'sagar.matale@esds.co.in';
		$subject = 'Invoice Data Sync to IIBF FTP completed';
		
		// email receipient list -
		//xxx$recipient_list = array('iibfdevp@esds.co.in','dattatreya@iibf.org.in', 'sgbhatia@iibf.org.in');
