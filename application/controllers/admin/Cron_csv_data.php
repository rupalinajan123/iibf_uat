 ...
 * Last Update 		:   21-04-2020sc

*/
defined('BASEPATH') OR exit('No direct script access allowed');
class Cron_csv_data extends CI_Controller {
	public $UserID;
	...
	/* jaiib_csv_kesdee - elearning_flag = Y : 13/Oct/2020 : Bhushan*/
	...
	///usr/local/bin/php /home/supp0rttest/public_html/index.php admin Cron_csv_data jaiib_csv_kesdee
	public function jaiib_csv_kesdee()
	{
		ini_set("memory_limit", "-1");
		$dir_flg = 0;
	...
	/* caiib_csv_mps : 13/Oct/2020 : Bhushan */  
	/*public function caiib_csv_mps()
	{
		...
		$start_time = date("Y-m-d H:i:s");
		$current_date = date("Ymd");	
		$cron_file_dir = "./uploads/cron_csv_JAIIB/";
		//Cron start Logs
		$result = array("success" => "", "error" => "", "Start Time" => $start_time, "End Time" => "");
		$desc = json_encode($result);
		$this->log_model->cronlog("Examination CSV Details Cron Execution Start", $desc);
				...
				foreach($can_exam_data as $exam)
				{
					...
					{	
						foreach($admit_card_details_arr as $admit_card_data)
						{
							...
							$exam_arr = array('60'=>'CAIIB','62'=>'CORPORATE BANKING','63'=>'RURAL BANKING','64'=>'INTERNATIONAL BANKING','65'=>'RETAIL BANKING','66'=>'CO-OPERATIVE BANKING','67'=>'FINANCIAL ADVISING','68'=>'HUMAN RESOURCES MANAGEMENT','69'=>'INFORMATION TECHNOLOGY','70'=>'RISK MANAGEMENT','71'=>'CENTRAL BANKING','72'=>'TREASURY MANAGEMENT');
							foreach($exam_arr as $k => $val){
								if($exam_code == $k) { $exam_name = $val; }
							}
							if($exam_code == '62' || $exam_code == '63' || $exam_code == '64' || $exam_code == '65' || $exam_code == '66' || $exam_code == '67' || $exam_code == '68' || $exam_code == '69' || $exam_code == '70' || $exam_code == '71' || $exam_code == '72'){
								$subject_code = '';
								$subject_description = '';
							}
							//First Name|Middle name|Last Name|Mem. Number|Date of Birth|Gender|Email ID|Mobile|Address|State|Pin Code|Country|Profession	|Organization|Designation|Exam Code|Course|Elective Sub Code|Elective Sub Desc|Attempt
	...
	///usr/local/bin/php /home/supp0rttest/public_html/index.php admin Cron_csv_data caiib_csv_mps
	public function caiib_csv_mps()
	{
		ini_set("memory_limit", "-1");
		$dir_flg = 0;
	...
	public function exam_csv_kesdee_old()
	{
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			$this->db->where_in('a.regnumber' , $mem);
			$can_exam_data3 = $this->Master_model->getRecords('member_exam a',array('pay_type'=>2,'status'=>1,'isactive'=>'1','isdeleted'=>0,'pay_status'=>1,'a.elearning_flag'=>'Y'),$select);
			//'DATE(a.created_on)'=>$yesterday,
	...
	public function exam_csv_kesdee()
	{
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			//print_r($can_exam_data);
			$yesterday = date('Y-m-d', strtotime("- 1 day"));
			$id = array(5912652,5912839,5912860,5912908,5912936,5912954,5912994,5913008,5913011,5913099,5913102,5913141,5913159,5913199,5913209,5913214,5913225,5913502,5913526,5913532,5913550,5913577,5913579,5913584,5913610,5913639,5913713,5913724,5913738,5913795,5913919,5913970,5913972,5914068,5914072,5914100,5914172,5914196,5914199,5914238,5914264,5914282,5914329,5914344,5914349,5914371,5914374,5914380,5914426,5914427,5914430,5914463,5914473,5914490,5914511,5914523,5914533,5914538,5914567,5914720);
			$exam_code_rpe = array('1002','1014');
			$select = 'DISTINCT(b.transaction_no),a.exam_code,c.firstname,c.middlename,c.lastname,a.regnumber,c.dateofbirth,c.gender,c.email,c.stdcode,c.office_phone,c.mobile,c.address1,c.address2,c.address3,c.address4,c.district,c.city,c.state,c.pincode,c.associatedinstitute,c.designation,a.exam_period,a.elected_sub_code,c.editedon,a.examination_date,a.id AS mem_exam_id,a.elearning_flag';
			$this->db->join('payment_transaction b','b.ref_id = a.id','LEFT');
			$this->db->join('member_registration c','a.regnumber=c.regnumber','LEFT'); 
    ...
    public function exam_csv_shezartech_old()
    {
        ini_set("memory_limit", "-1");
        $dir_flg        = 0;
        $parent_dir_flg = 0;
        ...
        if (file_exists($cron_file_dir . $current_date)) {
            ...
            $exam_code = array('527');
            $mem = array('510146557','510139353','510444828','801783976');
...
801564273);
    ...
    public function SIFFY_MSME_old()
    {
        ini_set("memory_limit", "-1");
        $dir_flg        = 0;
        $parent_dir_flg = 0;
    ...
    public function SIFFY_MSME()
    {
        ...
        if (file_exists($cron_file_dir . $current_date)) {
			...
			//$this->db->where('DATE(a.created_on) <=', '2020-06-01');
            $this->db->where_in('a.exam_code', $exam_code_arr);
            $this->db->where_in('a.regnumber', $id);
			$this->db->join('payment_transaction b', 'b.ref_id = a.id', 'LEFT');
            $this->db->join('member_registration c', 'a.regnumber=c.regnumber', 'LEFT');
            $can_exam_data = $this->Master_model->getRecords('member_exam a', array(
				'pay_type' => 2,
   ...
   public function exam_csv_NSEIT()
    {
        ...
        if (file_exists($cron_file_dir . $current_date)) {
			...
			$id = array(500176542,510319027,500049421,500037957,100086278,500193417,500091402,500076253,500176542,500079719,500079023,510319027,100086278,500193417);
    ...
    public function exam_csv_csc_vendor_old()
    { 
        ini_set("memory_limit", "-1");
        $dir_flg        = 0;
        $parent_dir_flg = 0;
        ...
        if (file_exists($cron_file_dir . $current_date)) {
           ...
           // $yesterday = '2020-05-29';
			...
			$select    = 'a.exam_code,c.firstname,c.middlename,c.lastname,a.regnumber,c.dateofbirth,c.gender,c.email,c.stdcode,c.office_phone,c.mobile,c.address1,c.address2,c.address3,c.address4,c.district,c.city,c.state,c.pincode,c.associatedinstitute,c.designation,a.exam_period,a.elected_sub_code,c.editedon,a.examination_date,a.id AS mem_exam_id,a.created_on AS registration_date,a.exam_medium,a.exam_center_code,d.venueid,d.pwd,d.exam_date,d.time';
            //$this->db->join('payment_transaction b', 'b.ref_id = a.id', 'LEFT');
			//$this->db->where('DATE(a.created_on) >=', $from_date);
			//$this->db->where('DATE(a.created_on) <=', $to_date);
			
            $this->db->join('member_registration c', 'a.regnumber=c.regnumber', 'LEFT');
			$this->db->join('admit_card_details d', 'a.id=d.mem_exam_id', 'LEFT');
            $this->db->where_in('a.exam_code', $exam_code);
			//$this->db->where('DATE(d.exam_date) =', '2021-06-26');
    ...
    { 
        ...
        if (file_exists($cron_file_dir . $current_date)) {
			...
			$exam_code = array('1003','1004','1005','1006','1007','1008','1009'); // Send Free and Paid Both Applications
			// DISTINCT(b.transaction_no),
			$mem = array(801811203);
            ...
            $this->db->where_in('a.regnumber' , $mem);
			$this->db->where_in('a.exam_code', $exam_code);
			//$this->db->where('d.mem_exam_id !=', '5900719');
            $can_exam_data = $this->Master_model->getRecords('member_exam a', array(
				'remark' => 1,
                /*'pay_type' => 2,
                'status' => 1,*/
    ...
    { 	     
        ...
        if (file_exists($cron_file_dir . $current_date)) {
			...
			//$exm_cd = array('996','1015');
			$exm_cd = array('1015');
			$members = array('801764172','801764171');
			//$exm_prd = array('45');
			...
			//$this->db->where_in('a.mem_mem_no',$members);
	 ...
	 public function bcbf_nar_csv_flipick_chaitali()
    { 	     
		ini_set("memory_limit", "-1");
        $dir_flg        = 0;
        $parent_dir_flg = 0;
    ...
    { 	      
        ...
        if (file_exists($cron_file_dir . $current_date)) {
		   ...
		   // $yesterday = date('Y-m-d', strtotime("- 1 day"));
            ...
            $this->db->where_in('a.exm_cd', $exm_cd);
			 $this->db->where_in('a.mem_mem_no', $member);
