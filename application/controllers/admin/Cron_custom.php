	...
	*/
	...
	class Cron_custom extends CI_Controller {
		...
		public function admit_update_220()
		{
			...
			$select = '*';
			$this->db->limit(3000);
			$temp_admit_card_data = $this->Master_model->getRecords('admit_card_details_jaiib',array('exm_cd'=>$this->config->item('examCodeJaiib'),'exm_prd'=>'220','remark'=>'0'),$select);
			//echo $this->db->last_query();
			if(count($temp_admit_card_data))
			{
				foreach($temp_admit_card_data as $admit_row)
		...
		public function member_csc()
		{
			...
			if(file_exists($cron_file_dir.$current_date))
			{
				...
				$new_mem_reg = $this->Master_model->getRecords('member_registration a',array(' DATE(createdon)'=>$yesterday,'isactive'=>'1','isdeleted'=>0, 'is_renewal' => 0,'bankcode' => 'csc'));
		...
		public function edit_data()
		{
			...
			$error = array();
			$start_time = date("Y-m-d H:i:s");
			$current_date = '20220428'; //date("Ymd");
			//$cron_file_dir = "./uploads/cronfiles_pg/";
			$cron_file_dir = "./uploads/cronFilesCustom/";
			$result = array("success" => "", "error" => "", "Start Time" => $start_time, "End Time" => "");
			$desc = json_encode($result);
		...
		public function exam()
		{
			...
			if(file_exists($cron_file_dir.$current_date))
			{
				...
				$can_exam_data = $this->Master_model->getRecords('member_exam a',array(' DATE(a.created_on)'=>$yesterday,'status'=>1,'isactive'=>'1','isdeleted'=>0,'pay_status'=>1,'pay_type'=>2),$select);
				echo $this->db->last_query();
		...
		public function csc_exam()
		{
			...
			if(file_exists($cron_file_dir.$current_date))
			{
				...
				$can_exam_data = $this->Master_model->getRecords('member_exam a',array(' DATE(a.created_on)'=>$yesterday,'pay_type'=>2,'status'=>1,'isactive'=>'1','isdeleted'=>0,'pay_status'=>1,'a.exam_code'=>'991','a.free_paid_flg'=>'P'),$select);
	...
	// https://iibf.esdsconnect.com/admin/Custom_cron/remote_exam
	public function remote_exam()
	{
	    ...
	    if (file_exists($cron_file_dir . $current_date))
	    {
	      ...
	      $receipt_no = array('ZHD51946463505','ZHD51948582252','ZHD51947893867');
	      $exam_codes_arr = array('1009');

	      // $this->db->where_in('b.transaction_no', $receipt_no);
	      $select = 'DISTINCT(b.transaction_no),a.exam_code,a.exam_mode,a.exam_medium,a.exam_period,a.exam_center_code,a.exam_fee,a.examination_date,b.member_regnumber,b.member_exam_id,b.amount,DATE_FORMAT(b.date,"%Y-%m-%d")date,b.receipt_no,b.ref_id,b.status,c.regnumber,c.registrationtype,a.elected_sub_code,a.place_of_work,a.state_place_of_work,a.pin_code_place_of_work,c.editedon,c.branch,c.office,c.city,c.state,c.pincode,a.scribe_flag,e.exam_date,a.elearning_flag,a.free_paid_flg,a.sub_el_count';
	      $this->db->join('payment_transaction b', 'b.ref_id = a.id', 'LEFT');
	      $this->db->join('member_registration c', 'a.regnumber = c.regnumber', 'LEFT');
	      $this->db->join('admit_card_details e', 'a.id = e.mem_exam_id', 'LEFT');
	      // $this->db->where_in('a.exam_period', '845');
	      $this->db->where_in('a.exam_code', $exam_codes_arr);
	      ...
	      // exit;
	      //' DATE(a.created_on) >=' => '2024-05-01',' DATE(a.created_on) <=' => $yesterday,
	      ...
	      if (count($can_exam_data))
	      {
	        ...
	        foreach ($can_exam_data as $exam)
	        {
	          ...
	          $exam_mode = 'O';
	          if ($exam['exam_mode'] == 'ON')
	          {
	            $exam_mode = 'O';
	          }
	          else if ($exam['exam_mode'] == 'OFF')
	          {
	            $exam_mode = 'F';
	          }
	          else if ($exam['exam_mode'] == 'OF')
	          {
	            $exam_mode = 'F';
	          }
