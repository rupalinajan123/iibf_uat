...
class Cron_monitor_payment extends CI_Controller {
	...
	*/
	public function index()
	{
		...
		// sql query 
		$sql = "SELECT receipt_no FROM payment_transaction WHERE date >= NOW() - INTERVAL ".$interval2." AND date < NOW() - INTERVAL ".$interval1." AND gateway = 'sbiepay' AND status = 2 ORDER BY id DESC";
		
		//echo $sql . "<br>";
		
		$pt_query = $this->db->query($sql);
		
		//$pt_query = $this->db->query("SELECT receipt_no FROM payment_transaction WHERE date >= '2017-02-25 16:32:02' AND date < '2017-02-25 18:23:05' AND gateway = 'sbiepay' AND status = 2 ORDER BY id DESC");
		...
		if ($pt_query->num_rows())
		{
			...
			foreach ($pt_query->result_array() as $row)
			{
				...
				else
				{
					...
					$subject = 'Alert !!! SBI ePay Status Query API Down [Payment Monitoring Cron]';
			...
			if(!empty($success_txn))
			{
				...
				$from_name = 'IIBF';
				$from_email = 'noreply@iibf.org.in';
				$subject = 'Payment Monitoring Cron';
			...
			$log_file = "payment_monitoring_log_".$current_date.".txt";
