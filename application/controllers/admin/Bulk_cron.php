<?php
/*
 	* Controller Name	:	Bulk Cron File Generation
 	* Created By		:	Bhushan
 	* Created Date		:	21-08-2019
*/
defined('BASEPATH') OR exit('No direct script access allowed');
class Bulk_cron extends CI_Controller {
	...
	public function bulk_member($exam_period,$inst_code)
	{   
		/* https://iibf.esdsconnect.com/admin/Bulk_cron/bulk_member/622/1343 */
		...
		$cron_file_dir = "./uploads/bulk_data/";

		$result = array("success" => "", "error" => "", "Start Time" => $start_time, "End Time" => "");  
		$desc = json_encode($result);
		$this->log_model->cronlog("IIBF Bulk Member Details Cron Execution Start", $desc);
		...
		if(file_exists($cron_file_dir.$current_date)){
			...
			$file = "iibf_bulk_mem_details_".$current_date.".txt";
			$fp = fopen($cron_file_path.'/'.$file, 'w');
			
			$file1 = "logs_".$current_date.".txt";
			$fp1 = fopen($cron_file_path.'/'.$file1, 'a');
			fwrite($fp1, "\n********** IIBF Bulk Member Details Cron Execution Started - ".$start_time." ********* \n");
			
			//$yesterday = date('Y-m-d', strtotime("- 1 day"));
			
			$new_mem_reg = $this->Master_model->getRecords('member_registration a',array('isactive'=>'1','isdeleted'=>0, 'is_renewal' => 0,'excode !='=>'991','exam_period' => $exam_period));	
			
			// ' DATE(createdon)'=>$yesterday,
			
			if(count($new_mem_reg)){
				$dirname = "bulk_regd_image_".$current_date;
				...
				foreach($new_mem_reg as $reg_data){
					...
					$this->db->where("inst_code = '".$inst_code."' AND a.status = 1 AND d.regnumber = '".$reg_data['regnumber']."'");
								
					//(DATE(a.updated_date) = '".$yesterday."') 
					...
					if($dir_flg){
						...
						if($photo_zip_flg || $sign_zip_flg || $idproof_zip_flg){
							$success['zip'] = "Bulk Member Images Zip Generated Successfully";
						}
						else{
							$error['zip'] = "Error While Generating Bulk Member Images Zip";
						}
					}
					...
					if($file_w_flg){
						$success['file'] = "Bulk Member Details File Generated Successfully. ";
					}
					else{
						$error['file'] = "Error While Generating Bulk Member Details File.";
					}
				}
			...
			$desc = json_encode($result);
			$this->log_model->cronlog("IIBF Bulk Member Details Cron Execution End", $desc);
			
			fwrite($fp1, "\n"."********* IIBF Bulk Member Details Cron Execution End ".$end_time." **********"."\n");
			fclose($fp1);
		}
	}
	
	public function bulk_admit_card_temp($exm_cd,$exm_prd,$inst_code)
	{
		/* https://iibf.esdsconnect.com/admin/Bulk_cron/bulk_member/10100/620/1343 */
		ini_set("memory_limit", "-1");
        $dir_flg        = 0;
        $parent_dir_flg = 0;
        $exam_file_flg  = 0;
		...
		if (file_exists($cron_file_dir . $current_date)) 
		{
            ...
            $file           = "bulk_".$current_date.".csv";
            $fp             = fopen($cron_file_path.'/'.$file, 'w');
            $file1          = "logs_".$current_date.".txt";
            $fp1            = fopen($cron_file_path.'/'.$file1, 'a');
            fwrite($fp1, "\n***** BULK CSV Cron Execution Started - " . $start_time . " ***** \n");
		
			$select = 'm.regnumber,mr.firstname,mr.middlename,mr.lastname,mr.email,mr.gender,mr.address1,mr.address2,mr.address3,mr.address4,mr.city,mr.state,mr.pincode,s.state_name,mr.registrationtype,mr.mobile,exam_code,m.exam_period,exam_center_code,m.exam_medium,exmode,aadhar_card,c.center_name,m.app_category,m.exam_fee,m.base_fee,m.institute_id';
			$this->db->join('member_registration mr','mr.regnumber = m.regnumber','LEFT');
			$this->db->join('state_master s','s.state_code = mr.state','LEFT');
			$this->db->join('center_master c','c.center_code = mr.centercode','LEFT');
			$bulk_csv = $this->Master_model->getRecords('member_exam m',array('m.pay_status'=>'1','m.exam_period' => $exm_prd,'m.exam_code' => $exm_cd,'c.exam_period' => $exm_prd,'m.institute_id' => $inst_code),$select);
			//echo "SQL > <br>".$this->db->last_query(); exit;
				...
				if (count($bulk_csv)) 
				{
				   ...
				   $data1 = "Member_No,Firstname,Middlename,Lastname,Email,Gender,Address1,Address2,Address3,Address4,City,State,Pincode,State_Name,Registrationtype,Mobile,Exam_Code,Exam_Period,Exam_Center_Code,Exam_Medium,Ex_mode,Aadhar_Card,Center_Name,App_Category,Exam_Fee,Base_Fee,Institute_Id\n";
					$exam_file_flg = fwrite($fp, $data1);
					$data = '';
					foreach ($bulk_csv as $csv) 
					{
					
						if ($csv['exam_code'] == 1010 || $csv['exam_code'] == 10100 || $csv['exam_code'] == 101000 || $csv['exam_code'] == 1010000 || $csv['exam_code'] == 1010000 || $csv['exam_code'] == 10100000){
							$exam_code = 101;
						}else{
							$exam_code = $csv['exam_code'];
						}
						
						$data .= ''.$csv['regnumber'] . ',' . $csv['firstname'] . ',' . $csv['middlename'] . ',' . $csv['lastname'] . ',' . $csv['email'] . ',' . $csv['gender'] . ',' . $csv['address1'] . ',' . $csv['address2'] . ',' . $csv['address3'] . ',' . $csv['address4'] . ',' . $csv['city'] . ',' . $csv['state'] . ',' . $csv['pincode'] . ',' . $csv['state_name'] . ',' . $csv['registrationtype'] . ',' . $csv['mobile'] . ',' . $exam_code . ',' . $csv['exam_period'] . ',' . $csv['exam_center_code'] . ',' . $csv['exam_medium'] . ',' . $csv['exmode'] . ',' . $csv['aadhar_card'] . ',' . $csv['center_name'] . ',' . $csv['app_category'] . ',' . $csv['exam_fee'] . ',' . $csv['base_fee'] . ',' . $csv['institute_id']."\n";
						$i++;
						$exam_cnt++;
					}
					...
					if ($exam_file_flg){
						$success['cand_exam'] = "BULK CSV File Generated Successfully.";
					}
					else{
						$error['cand_exam'] = "Error While Generating BULK CSV File.";
					}
					...
					$desc     = json_encode($result);
					$this->log_model->cronlog("BULK CSV Cron Execution End", $desc);
				
					fwrite($fp1, "\n" . "***** BULK CSV Cron Execution End " . $end_time . " *****" . "\n");
					fclose($fp1);
				}		
			}
	}
