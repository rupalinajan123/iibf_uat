...
class Cron_csv_ftp_cscvendor extends CI_Controller {
	...
	{	exit; 
		...
		$array = $this->master_model->getRecords('cron_csv_cscvendor','','new_file_name,record_count');
		...
		$local_server_dir_path = '/home/supp0rttest/public_html/uploads/cronCSV_cscvendor/'.$current_date.'/';
		...
		if($login_result){
			...
			$path_list = array("/cscvendor/".$current_date."/");
	...
	public function sendmail($current_date = "", $msg = ""){
		...
		$subject = 'Member Exam Applications CSV File Transfer To CSC Vendor - 1003,1004';
	
		$recipient_list = array('kalpanashetty@iibf.org.in','iibfdevp@esds.co.in','omveer@csc.gov.in','iibfexam@cscacademy.org','sgbhatia@iibf.org.in','amit@iibf.org.in','sajan@iibf.org.in');
		
		//$recipient_list = array('bhushan.amrutkar@esds.co.in');
		
		$attachment_filename = 'logs_'.$current_date.'.txt';
		$attachment_path = './uploads/cronCSV_cscvendor/'.$current_date.'/' . $attachment_filename;
