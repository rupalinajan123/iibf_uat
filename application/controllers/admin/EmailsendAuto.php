...
class EmailsendAuto extends CI_Controller {
	...
	public function settlement_mail_send(){  
		...
		$attachpath='';
		$record_details = $this->master_model->getRecords('exam_invoice_settlement',array('refund_case'=> '0','email_send'=>'0' ),'id,exam_code,exam_period,member_regnumber,receipt_no,record_type');
		...
		foreach($sql as $rec){ 
			...
			if($record_details['record_type'] == 'member_invoice'){
				$attachpath = "uploads/reginvoice/user/".$rec['invoice_image'];
			}else{
        		...
        		if($record_details['app_type'] =='K' || $record_details['app_type'] =='L' || $record_details['app_type'] =='O'){
        					...
        					$arr = array($record_details['invoice_id']);    
						     ...
						     $sql= $this->master_model->updateRecord('exam_invoice_images',$update_data,array('id' => $record_details['id']));
							 		...
							 		for($i=0;$i<=0;$i++){
							 			 $path = custom_genarate_reg_invoice_new($arr[$i]);
	            	 	...
	            	 	$attach = custome_generate_bulk_examinvoice_new_design($record_details['receipt_no']);
	            	 	$files=array($oldfilepath_user);
		            	  	...
		            	  	$invoice_id = $record_details['invoice_id'];
				            $path = genarate_amp_invoice_custom1($invoice_id);
		                    ...
		                    $examinv_det = $this->master_model->getRecords('exam_invoice',array('invoice_id'=>$record_details['invoice_id']),'member_no'); 
		                    $mem_det = $this->master_model->getRecords('member_registration',array('regnumber'=>$examinv_det['member_no']),'email'); 
		                     $invoice_no = $record_details['invoice_id'];
							 $zone_code = $zonel_code;
							 $program_name = 'CERTIFIED CREDIT OFFICER';
							 //$mem_gstin_no = '36AABCT5589K1ZQ';
							 $path = genarate_blended_invoice_custom_new($invoice_no,$zone_code,$program_name,$mem_gstin_no);
