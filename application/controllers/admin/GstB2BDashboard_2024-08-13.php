			...
			{ 
				...
				if ($this->form_validation->run() == TRUE)
				{
					...
					if($exam_code == '01')
					{
						...
						$approved_bulk_count = $res->result()[0]->numrows;
						//echo $this->db->last_query(); die;
					}
					else if($exam_code == '02') // dra count
					{
						...
						$pending_dra_count = $this->master_model->getRecordCount('dra_payment_transaction'); */
						 ...
						 $res = $this->db->query("SELECT count(dra_payment_transaction.receipt_no) AS `numrows` FROM`exam_invoice`,dra_payment_transaction WHERE exam_invoice.`exam_period` != 0 AND dra_payment_transaction.exam_code IN (45,57) AND exam_invoice.pay_txn_id =dra_payment_transaction.id   and dra_payment_transaction.status = 3 AND Date(`date`) BETWEEN '".$from_date."' and '".$to_date."' AND app_type='I'");
						...
						//echo $this->db->last_query(); die;
						$res = $this->db->query("SELECT count(exam_invoice.receipt_no) AS `numrows` FROM`exam_invoice`,dra_payment_transaction WHERE exam_invoice.`exam_period` != 0 AND exam_invoice.exam_code IN (45,57) AND exam_invoice.pay_txn_id =dra_payment_transaction.id AND`invoice_no` != '' and exam_invoice.gstin_no != '-' AND `exam_invoice`.`gstin_no` != '' and dra_payment_transaction.status = 1 AND Date(`date_of_invoice`) BETWEEN '".$from_date."' and '".$to_date."' AND app_type='I'");
