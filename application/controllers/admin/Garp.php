...
class Garp extends CI_Controller
{
	public $UserID;
	...
	public function examReg()
	{
		...
		if ($this->session->userdata('roleid') != 1 && $this->session->userdata('roleid') != 16) {
		...
		$this->load->view('admin/garp_exam_details', $data);
	}
	public function exportToCSV()
	{
		...
		$filename = FCPATH.'/uploads/garpcsv/garp_data.xlsx';
		$newfilename = FCPATH.'/uploads/garpcsv/garp-records-'.date('ymdhis').'.xlsx';
		//$filename = 'garp_data.xlsx';
		...
		foreach ($result as $key=>$c_row){
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$cell,$i++)
								->setCellValue('B'.$cell, 'Financial Risk and Regulation')
								...
								->setCellValue('G'.$cell, $c_row['registrationtype'])
								->setCellValue('H'.$cell, '$300.00')
								->setCellValue('I'.$cell, 'Included')
								->setCellValue('J'.$cell, '$300.00')
	...
	function getRecords($from_date,$end_date) {
		...
		$select = 'd.description,b.transaction_no,a.regid,a.regnumber,g.namesub,g.firstname,g.lastname,g.dateofbirth,g.createdon,b.status,b.date,c.exam_medium,c.exam_fee,gender,d.description,c.exam_center_code,b.transaction_details,g.email,a.registrationtype';
		...
		$this->db->join('garp_exam_registration g', 'g.mem_exam_id=c.id', 'LEFT');
		//$this->db->join('member_registration a','a.regnumber=b.member_regnumber','RIGHT');
		$this->db->group_by('b.transaction_no');
		$this->db->order_by('c.id', 'DESC');
		$res = $this->UserModel->getRecords("member_exam c", $select,'c.exam_code', 1018);
