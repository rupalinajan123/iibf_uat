					...
					<div class="row">
						<div class="col-lg-12">
							<div class="form-group">
								<select class="form-control" id="exam_code" name="exam_code" required autofocus onchange="get_date_range(this.value)">
									<?php
										if(!empty($active_exam_data) && count($active_exam_data) > 0)
										{
											...
											{ ?>
											<option value="<?php echo $active_exam_res['exam_code']; ?>" <?php if(set_value('exam_code') == $active_exam_res['exam_code']) { echo "selected"; } else { if($exam_code == $active_exam_res['exam_code']) { echo "selected"; } } ?>><?php if($active_exam_res['description'] != "") { echo $active_exam_res['description']." - "; } echo $active_exam_res['exam_code']; ?></option>
											<?php	}
										}
										else
										{
											echo '<option value="">No Exam Available</option>';
										} ?>
								</select>
				...
				{	?>
					<section class="content" style="min-height:auto;">
						<div class="box box-info">
							<h4 class="text-center"><?php echo "Exam Name : ".$exam_details[0]['description']; ?></h4>
								...
								<table class="table table-hover table-bordered" cellspacing="0" width="100%">
									...
									<tbody>  
										...
										{	
											...
											<tr>
												<?php if($i == 0) { ?> <td class="text-center" rowspan="<?php echo count($result_data); ?>" style="vertical-align:middle"><?php if($exam_code == '2027'){ echo 1017;} else {echo $exam_code;} ?></td><?php } ?>
							...
							<h4 class="text-center" style="margin: 0 0 15px 0 !important; position: relative; padding: 0;">
								...
								<?php if($final_tot > 0) { if($start_date == '') { $start_date = 0; } if($end_date == '') { $end_date = 0; } if($exam_date == '') { $exam_date = 0; }   ?> <a target="_blank" href="<?php echo site_url('admin/rpe_exam_dashboard/download_admit_card/'.$exam_code.'/'.$start_date.'/'.$end_date.'/'.$exam_date); ?>" class="btn btn-primary" style="padding: 2px 10px 3px;border-radius: 0; font-size: 13px;line-height: 15px;float: right;	margin: 0 0 5px 0;">Download Admit Cards</a> <?php } ?>
							</h4>
						</div>
					</section>
				<?php } ?>
...
<script type="text/javascript">
	...
	function get_date_range(exam_code)
	{
		$("#page_loader").show();
