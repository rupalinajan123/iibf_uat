<?php
/*
 * Controller Name	:	Cron File Transfer : NSEIT Vendor
 * Created By		:	Bhushan
 * Created Date		:	29 May 2020
 *
*/
defined('BASEPATH') OR exit('No direct script access allowed');
class Cron_csv_ftp_NSEIT extends CI_Controller {
	...
	public function index($current_date = "")
	{
        ...
        $array = $this->master_model->getRecords('cron_csv_NSEIT','','new_file_name,record_count');
		...
		$local_server_dir_path = '/home/supp0rttest/public_html/uploads/cronCSV_NSEIT/'.$current_date.'/';
	...
	public function sendmail($current_date = "", $msg = "")
	{
		...
		$subject = 'NSEIT Vendor: Candidate Registrations Data Transfer';
		...
		$recipient_list = array(
            ...
            'iibfdevp@esds.co.in',
            'corebusiness_support@nseit.com',
		...
		$attachment_path = './uploads/cronCSV_NSEIT/'.$current_date.'/' . $attachment_filename;
	...
	// /usr/local/bin/php /home/supp0rttest/public_html/index.php admin Cron_csv_ftp_NSEIT dra_nseit
	public function dra_nseit($current_date = "")
	{
		$start_time = date("Y-m-d H:i:s");
		$success = array();
		$error = array();
		...
		$CurrentDate = date('Y-m-d',strtotime($current_date));
		$this->db->where('CurrentDate', $CurrentDate);
        $array = $this->master_model->getRecords('cron_csv_dra','','new_file_name,record_count');
		$new_file_name = $array[0]['new_file_name'];
		$record_count = $array[0]['record_count'];
		...
		$result = array("success" => $success, "error" => $error, "Start Time" => $start_time, "End Time" => $end_time);
		$desc = json_encode($result);
		$this->log_model->cronlog("DRA CSV File Transfer Cron - Ends", $desc);
	...
	public function sendmail_dra_nseit($current_date = "", $msg = "")
	{
		...
		//$from_email = 'noreply@iibf.org.in';
		$from_email = 'logs@iibf.esdsconnect.com';
		$subject = 'NSEIT Vendor: DRA Candidate Registrations Data Transfer';
