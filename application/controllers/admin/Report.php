...
/* 
  Last editedon : 2025-03-17 
  Last editedby : Pooja Mane
  Changes : Commented $update_data['kyc_status'] = '0'; from Reg_edit function to avoid updating kyc_status on change of txt data only.  
 */
...
class Report extends CI_Controller
{
	...
	public function getList()
	{
		...
		if ($res) {
			...
			foreach ($result as $row) {
				...
				// $decpass = preg_replace('/[-?]/', '', $decpass);
				// $decpass = preg_replace('/[\x00-\x1F\x7F-\xFF]/', '', $decpass);
	...
	public function reg_edit()
	{
		...
		if (isset($_POST['btnSubmit'])) {
			...
			} else if ($prevData['excode'] != '997')  //priyanka d -12-july-23 >> added $prevData['excode']!='997'  condition so candidate will not update email/mob
			{
			...
			//die;
			/* $this->form_validation->set_rules('email','Email','trim|required|valid_email|is_validuniqueO[member_registration.email.regid.'.$regid.'.isactive.1.registrationtype.'.$memtype.']|xss_clean'); */
			//priyanka d -12-july-23 >> added ippb_emp_id condition so candidate will not update email/mob
			...
			/*START: Extra Fields for OLD BCBF Exam added by anil on 01 Oct 2024*/
			$name_of_bank_bc = isset($_POST['name_of_bank_bc']) ? $_POST['name_of_bank_bc'] : '';
			$ippb_emp_id = isset($_POST['ippb_emp_id']) ? $_POST['ippb_emp_id'] : '';
			$doj12 = isset($_POST['doj12']) ? $_POST['doj12'] : '';

			$date_of_commenc_bc = '';
			if (isset($_POST["doj12"]) && $_POST["doj12"] != "") {
				$doj12       = $_POST["doj12"];
				$doj        = str_replace('/', '-', $doj12);
				$date_of_commenc_bc = date('Y-m-d', strtotime($doj));
			}

			if ($name_of_bank_bc != "" && $ippb_emp_id != "" && $doj12 != "") {

				$this->form_validation->set_rules('doj12', 'Date of commencement of operations/joining as BC', 'required|xss_clean');
				...
				if ($this->input->post('name_of_bank_bc') != '') {
					...
					$this->form_validation->set_rules('ippb_emp_id', 'Bank BC ID No', 'trim|required|alpha_numeric_spaces|xss_clean|callback_check_bank_bc_id_no_duplication[' . $name_of_bank_bc . ',' . $regid . ']');
				}

				if ($this->input->post('doj12') != '') {
					//echo "Inn".$this->input->post('doj12')."==".$this->input->post('exam_date_exist');die;
					$exam_date_exist = '';
					$this->form_validation->set_rules('doj12', 'Date of Joining', 'trim|required|xss_clean|callback_check_date_of_joining_bc_validation[' . $exam_date_exist . ']');
				}
			}
			/*END: Extra Fields for OLD BCBF Exam added by anil on 01 Oct 2024*/
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if ($regData[0]['registrationtype'] != 'NM' && $regData[0]['registrationtype'] != 'DB') {
					$ins_nameoncard = $firstname;
					if ($middlename != "") {
						$ins_nameoncard .= ' ' . $middlename;
					}
					if ($lastname != "") {
						$ins_nameoncard .= ' ' . $lastname;
					}
					//$nameoncard = strtoupper($_POST["nameoncard"]);
					$nameoncard = strtoupper($ins_nameoncard);
				...
				if (count($prevData)) {
					...
					if ($prevData['email'] != $email && $prevData['excode'] != '997' && $email != "") {  //priyanka d -12-july-23 >> added $prevData['excode']!='997'  condition so candidate will not update email/mob
						//$update_data['email'] = $email; // Commented by anil on 01 Oct 2024 so admin will not update email/mob 
					}
					...
					if ($prevData['mobile'] != $mobile && $prevData['excode'] != '997' && $mobile != "") {  //priyanka d -12-july-23 >> added $prevData['excode']!='997'  condition so candidate will not update email/mob
						//$update_data['mobile'] = $mobile; // Commented by anil on 01 Oct 2024 so admin will not update email/mob 
					}
					...
					/*START: Extra Fields for OLD BCBF Added By Anil on 01 Oct 2024*/
					if ($name_of_bank_bc != "" && $prevData['name_of_bank_bc'] != $name_of_bank_bc) {
						$update_data['name_of_bank_bc'] = $name_of_bank_bc;
					}
					if ($ippb_emp_id != "" && $prevData['ippb_emp_id'] != $ippb_emp_id) {
						$update_data['ippb_emp_id'] = $ippb_emp_id;
					}
					if ($date_of_commenc_bc != "" && $prevData['date_of_commenc_bc'] != $date_of_commenc_bc) {
						$update_data['date_of_commenc_bc'] = $date_of_commenc_bc;
					}
					/*END: Extra Fields for OLD BCBF Added By Anil on 01 Oct 2024*/
				}
		...
		// 1444
		$instiarray = array(764, 793, 845, 939, 946, 1238, 1449, 1456, 1458, 1459, 1460, 1464, 1465, 1469, 1470, 1471, 1472, 1473, 1474, 1476, 1487, 1490, 1491, 1497, 14999, 1506, 1511, 1522, 1525, 1526, 1527, 1528, 1530, 1531, 1533, 1538, 1539, 1540, 1541, 1549, 1559, 1567, 1570, 1571, 1573, 1574, 1575, 1576, 1581, 1584, 1587, 1589, 1591, 1592, 1593, 1594, 1598, 1602, 1603, 1607, 1608, 1609, 1612, 1616, 1617, 1620, 1625, 1626, 1627, 1628, 1629, 1630, 1635, 1643, 1644, 1646, 1648, 1652, 1654, 1655, 1656, 1657, 1658, 1660, 1661, 1663, 1669, 1678, 1679, 1680, 1687, 1688, 1690, 1691, 1692, 1699, 1707, 1708, 1709, 1714, 1720, 1721, 1723, 1724, 1725, 1727, 1730, 1731, 1740, 1742, 1743, 1755, 1758, 1760, 1767, 1769, 1773, 1774, 1775, 1776, 1780, 1781, 1782, 1783, 1785, 1786, 1788, 1790, 1795, 1802, 1803, 1804, 1806, 1813, 1815, 1817, 1818, 1820, 1824, 1825, 1828, 1844, 1845, 1846, 1848, 1850, 1851, 1852, 1853, 1855, 1861, 1862, 1863, 1864, 1868, 1869, 1870, 1876, 1884, 1885, 1886, 1890, 1894, 1896, 1897, 1898, 1905, 1908, 1909, 1911, 1912, 1913, 1914, 1915, 1921, 1925, 1926, 1930, 1931, 1936, 1947, 1951, 1952, 1961, 1971, 1976, 1982, 1986, 1988, 1991, 1992, 1994, 1995, 1996, 1997, 2000, 2002, 2012, 2025, 2028, 2029, 2034, 2041, 2043, 2044, 2046, 2050, 2052, 2053, 2054, 2056, 2058, 2059, 2060, 2061, 2062, 2063, 2064, 2065, 2066, 2067, 2068, 2069, 2070, 2071, 2072, 2073, 2076, 2077, 2078, 2079, 2080, 2081, 2082, 2083, 2084, 2086, 2087, 2090, 2093, 2096, 2097, 2098, 2100, 2101, 2102, 2105, 2107, 2108, 2109, 2110, 2111, 2112, 2113, 2115, 2116, 2117, 2119, 2120, 2122, 2123, 2125, 2126, 2129, 2130, 2131, 2132, 2133, 2134, 2135, 2136, 2137, 2138, 2139, 2140, 2141, 2145, 2146, 2147, 2152, 2153, 2154, 2155, 2156, 2180, 2183, 2184, 2185, 2186, 2187, 2683, 2698, 1389, 2751);
		$this->db->where_not_in('institude_id', $instiarray);
		$data['institution_master'] = $this->master_model->getRecords('institution_master', '', '', array('name' => 'ASC', 'institution_delete' => '0'));
		...
		$data['old_bcbf_institute_data'] = $old_bcbf_institute_data = $this->master_model->getRecords('bcbf_old_exam_institute_master', '', '', array('institute_name' => 'asc'));
		/*OLD BCBF Inst Mater Dropdown*/
	...
	public function editimages()
	{
		...
		if ($reg_id != '' && $reg_no != '') {
			if (is_numeric($reg_id)) {
				...
				$member_info = $this->master_model->getRecords('member_registration', array('regid' => $reg_id), 'scannedphoto,scannedsignaturephoto,idproofphoto, declaration, regnumber,registrationtype,bank_bc_id_card,empidproofphoto,excode');

				if (isset($_POST['btnSubmit'])) {
					if ($_FILES['scannedphoto']['name'] == '' && $_FILES['scannedsignaturephoto']['name'] == '' && $_FILES['idproofphoto']['name'] == '' && $_FILES['declaration']['name'] == '' && $_FILES['empidproofphoto']['name'] == '' && $_FILES['bank_bc_id_card']['name'] == '') {
					...
					if ($_FILES['empidproofphoto']['name'] != '') {
						$this->form_validation->set_rules('empidproofphoto', 'Upload Employment Proof', 'file_required|file_allowed_type[jpg,jpeg]|file_size_max[300]');
					}
					if ($_FILES['bank_bc_id_card']['name'] != '') {
						$this->form_validation->set_rules('bank_bc_id_card', 'Upload Bank BC ID Card', 'file_required|file_allowed_type[jpg,jpeg]|file_size_min[100]|file_size_max[300]');
					}
					...
					if ($this->form_validation->run() == TRUE) {
						...
						$prev_bank_bc_id_card_flg = "N";
						$empidproofphoto_file = '';
						$bank_bc_id_card_file = '';
						$prev_edited_on_qry = $this->master_model->getRecords('member_registration', array('regid' => $reg_id), 'DATE(images_editedon) images_editedon,photo_flg,signature_flg,id_flg,declaration_flg,bank_bc_id_card,empid_flg,bank_bc_id_card_flg,empidproofphoto');
						if (count($prev_edited_on_qry)) {
							...
							$empidproofphoto_file = $prev_edited_on_qry[0]['empidproofphoto'];
							$prev_empid_flg = $prev_edited_on_qry[0]['empid_flg'];
							$bank_bc_id_card_file = $prev_edited_on_qry[0]['bank_bc_id_card'];
						...
						if (isset($_FILES['scannedphoto']['name']) && $_FILES['scannedphoto']['name'] != '') {
							...
							$new_filename = strtolower('p_' . $applicationNo);
							...
							if ($uploadData) {
								...
								//@unlink('uploads/photograph/'.$member_info[0]['scannedphoto']);
								$scannedphoto_file = strtolower($uploadData['file_name']);
								$photo_flg = 'Y';
								$edited .= 'PHOTO || ';
							} else {
						...
						if ($_FILES['declaration']['name'] != '') {
							...
							$date = date_create();
							$new_filename = strtolower('declaration_' . $applicationNo);
							$uploadData = upload_file('declaration', $path, $new_filename, '', '', TRUE);
							if ($uploadData) {
								$kyc_edit_flag = 1;
								$kyc_update_data['edited_mem_declaration'] = 1;
								$declaration_file = strtolower($uploadData['file_name']);
								$declaration_flg = 'Y';
								$edited .= 'DECLARATION || ';
							} else {
						...
						/*START: Added By Anil on 09 Oct 2024*/
						// Upload employment proof
						if ($prev_edited_on != '' && $prev_edited_on != date('Y-m-d')) {
							$empidproofphoto_flg = 'N';
						} else {
							$empidproofphoto_flg = $prev_empid_flg;
						}
						...
						if (isset($_FILES['empidproofphoto']['name']) && $_FILES['empidproofphoto']['name'] != '') {
							...
							$date = date_create();
							//$timestamp = date_timestamp_get($date); 
							$new_filename = strtolower('empr_' . $applicationNo);
							$uploadData = upload_file('empidproofphoto', $path, $new_filename, '', '', TRUE);
							if ($uploadData) {
								$kyc_edit_flag = 1;
								$kyc_update_data['edited_employee_proof'] = '1';
								$empidproofphoto_file = strtolower($uploadData['file_name']);
								$empidproofphoto_flg = 'Y';
								$edited .= 'EMPLOYMENTPROOF || ';
							} else {
								//$empidproofphoto_file = $this->input->post('empidproofphoto1_hidd');
							}
						}
						/*END: Added By Anil on 09 Oct 2024*/

						/*START: Added By Anil on 01 Oct 2024*/
						// Upload Bank BC ID Card

						if ($prev_edited_on != '' && $prev_edited_on != date('Y-m-d')) {
							$bank_bc_id_card_flg = 'N';
						} else {
							$bank_bc_id_card_flg = $prev_bank_bc_id_card_flg;
						}
						...
						if ($_FILES['bank_bc_id_card']['name'] != '') {
							...
							$uploadData = upload_file('bank_bc_id_card', $path, $new_filename, '', '', TRUE);
							if ($uploadData) {
								$kyc_edit_flag = 1;
								$kyc_update_data['edited_bank_bc_id_card'] = '1';
								$bank_bc_id_card_file = strtolower($uploadData['file_name']);
								$bank_bc_id_card_flg = 'Y';
								$edited .= 'BANKBCIDCARD || ';
							}
						}
						/*END: Added By Anil on 01 Oct 2024*/
						...
						$this->db->where('regnumber', $reg_no);
						...
						$update_info['images_editedby'] = $this->session->userdata('username');
						$update_info['images_editedbyadmin'] = $this->UserID;
						...
						if ($empidproofphoto_file != "") {
							...
							$update_info["empid_flg"] = $empidproofphoto_flg;
						}
	...
	// Function to Reset all images from candidate profile by pooja mane - 2024-12-31
	public function deleteimages()
	{
		$last = $this->uri->total_segments();
		$reg_id = base64_decode($this->uri->segment($last - 2));
		$reg_no = base64_decode($this->uri->segment($last - 1));
		$applicationNo = $reg_no;
		$field = $this->uri->segment($last);

		if ($reg_id != '' && $reg_no != '') {
			$update_data = array();
			$uerlog = $this->master_model->getRecords('member_registration', array('regnumber' => $reg_no));
			if ($field === 'scannedphoto') {
				$update_data['scannedphoto'] = "";
				$field = 'Photo';
			}
			if ($field === 'scannedsignaturephoto') {
				$update_data['scannedsignaturephoto'] = "";
				$field = 'Signature';
			}
			if ($field === 'idproofphoto') {
				$update_data['idproofphoto'] = "";
				$field = 'Id Proof';
			}
			if ($field === 'empidproofphoto') {
				$update_data['empidproofphoto'] = "";
				$field = 'Employee Proof';
			}
			if ($field === 'declaration') {
				$update_data['declaration'] = "";
				$field = 'Declaration';
			}
			if ($field === 'bank_bc_id_card') {
				$update_data['bank_bc_id_card'] = "";
				$field = 'Bank Bc Id';
			}
			...
			if (count($uerlog) > 0 && $uerlog[0]['image_path'] != '') {
				$update_data['image_path'] = "old" . $uerlog[0]['image_path'];
			}

			$update_result = $this->master_model->updateRecord('member_registration', $update_data, array('regid' => $reg_id));
			$user_info = $this->db->last_query();

			if ($update_result) {

				$final_str = 'Dear Candidate,<br/><br/>';
				$final_str .= 'You are requested to Upload the soft copy of your missing documents in your profile after login at the earliest. ';
				$final_str .= 'Pl read the guidelines (Annexure I and II) pertaining to upload of documents image. <a target="_blank "href="https://iibf.esdsconnect.com/uploads/Guideline_for_photo_image_loading_final.pdf "><b>Click here</b></a> to view the guidelines';
				$final_str .= '<br/><br/>';
				$final_str .= 'Regards,';
				$final_str .= '<br/>';
				$final_str .= 'IIBF TEAM';
				...
				if ($this->Emailsending->mailsend($info_arr)) {
					$log_title = "Images Reset Mail Sent to Member";
				} else {
					$log_title = "Failed to Send Image Reset Mail";
				}

				/* Mail sent Log Actitives : Pooja Mane 2025-02-14*/
				$log_message = serialize($user_info);
				$rId = $uerlog[0]['regid'];
				$reg_no = $applicationNo;
				storedUserActivity($log_title, $log_message, $reg_id, $reg_no);
				/* Mail sent Log Actitives End : Pooja Mane 2025-02-14*/
			} else {
				$this->session->set_flashdata('error', $field . ' Failed to Update Record for Member No - ' . $reg_no);
			}

			/* Update all images to blank User Log Activities : Pooja Mane 2025-01-10 */
			// $log_title = "Member Images Reset to blank by admin id : ".$this->UserID." for regid " . $uerlog[0]['regid'];
			$log_title = "Member Images Reset to blank by admin id : " . $this->UserID;
			$log_message = serialize($user_info);
			$rId = $uerlog[0]['regid'];
			$reg_no = $applicationNo;
			storedUserActivity($log_title, $log_message, $reg_id, $reg_no);
			/* Close User Log Actitives End */
			$this->session->set_flashdata('success', $field . ' Image Reset Successfully for Member No - ' . $reg_no);
		}
		redirect(base_url() . 'admin/Report/reg_edit/' . base64_encode($reg_id));
	}
	// Function to Reset all images from candidate profile ends by pooja mane - 2024-12-31
	...
	public function preview()
	{
		...
		if ($regid) {
			...
			$select = 'regid,regnumber,usrpassword,namesub,firstname,middlename,lastname,displayname,contactdetails,address1,address2,address3,address4,
				...
				idproofphoto,declaration,optnletter,,excode,fee,exam_period,centercode,exmode,registration_status,createdon,qid,c.name q_name,b.id,b.state_code,b.state_name,institude_id,d.name institute_name,dcode,dname,f.id,f.name id_name,name_of_bank_bc,date_of_commenc_bc,ippb_emp_id,bank_bc_id_card,bank_emp_id';
			//paymode
			...
			if ($searchBy != '' && $searchOn != '' && $SearchVal != '') {
				switch ($searchBy) {
					...
					case 'transaction_no': 	/*$where .= ' AND b.transaction_no = "'.$SearchVal.'"';*/
						...
						$contactclassWhr =  ' b.transaction_no = "' . $SearchVal . '"';
						$nonmembercscWhr =  ' b.transaction_no = "' . $SearchVal . '"';
						break;
					/* ADDED BY - POOJA MANE 29/06/2022 */
					case 'receipt_no':
						$examWhr = ' b.receipt_no = "' . $SearchVal . '" AND a.isdeleted = 0';
						$icardWhr = ' b.receipt_no = "' . $SearchVal . '"';
						$certiWhr = ' b.receipt_no = "' . $SearchVal . '"';
						$renewalWhr =  ' p.receipt_no = "' . $SearchVal . '"';
						$contactclassWhr =  ' b.receipt_no = "' . $SearchVal . '"';
						$nonmembercscWhr =  ' b.receipt_no = "' . $SearchVal . '"';
						break;
					case 'email':
				...
				//below if condition added by vishal tfor bcbc bulk 
				if (count($type_result) > 0 &&  $type_result->excode == '101') {

					$this->db->order_by('id', 'desc');
					$this->db->limit(0, 1);
					$is_bulk_entry_bcbf = $this->master_model->getRecords('member_exam', array('regnumber' => $SearchVal, 'institute_id != ' => ''), 'regnumber,institute_id');
					if (count($is_bulk_entry_bcbf) > 0) {
						$bulk_entry = 'Bulk';
					}
				}
				...
				/* ADDED BY - POOJA MANE 29/06/2022 */ elseif ($searchBy == 'receipt_no') {

					$reg_num_res =  $this->master_model->getRecords("payment_transaction", array('receipt_no' => $SearchVal), 'member_regnumber,exam_code,pay_type,status');
					//echo $this->db->last_query();die();
					//print_r($reg_num_res);die;
					$where .= " AND receipt_no = '" . $SearchVal . "' ";
					...
					if (count($reg_num_res)) {
						...
						/*FOR CHECKING FAILED CSC TRANSACTIONS*/
						if ($reg_num_res[0]['exam_code'] == '991' && $reg_num_res[0]['status'] != '1') {
							$where .= " AND a.regid = '" . $mem_regnum . "'";
							//echo $this->db->last_query();die();
						}
					}
				}
				...
				// MEMBER DETAILS
				$select1 = 'DISTINCT(a.regnumber),b.transaction_no, a.regid,namesub, a.firstname, a.middlename, a.lastname,usrpassword,  a.registrationtype,DATE_FORMAT( a.dateofbirth,"%d-%m-%Y") dateofbirth, a.mobile, a.createdon,b.status,b.amount,b.transaction_details,b.auth_code,b.date, a.bank_emp_id,p.emp_id';
				$this->db->join('payment_transaction b', 'b.member_regnumber=a.regnumber', 'LEFT');
				$this->db->join('member_registration_ippb p', 'p.mobile=a.mobile', 'LEFT'); //for employee id added by Pooja mane on :2023-05-18
				$this->db->where($where1, '', false);
				$member_details = $this->master_model->getRecords("member_registration a", "", $select1, array('date' => 'ASC'));
				//echo $this->db->last_query();

				//Transaction Details
				$this->db->select('DISTINCT(b.transaction_no), a.regid, a.regnumber, namesub, a.firstname, a.middlename, a.lastname, usrpassword, a.registrationtype, DATE_FORMAT(a.dateofbirth,"%d-%m-%Y")dateofbirth,a.mobile ,a.createdon, status, amount, transaction_details, auth_code,date,a.bank_emp_id,p.emp_id');
				$this->db->from('payment_transaction b');
				$this->db->join('member_registration a', 'b.member_regnumber = a.regnumber');
				$this->db->join('member_registration_ippb p', 'p.mobile=a.mobile', 'LEFT'); //for employee id added by Pooja mane on :2023-05-18
				$this->db->where(array('b.transaction_no' => $SearchVal));
				...
				if (count($member_details)) {
					...
					WHERE `isactive` = '1' AND `isdeleted` = 0 AND a.regnumber = "510000742" AND b.status = 1 AND CASE WHEN a.registrationtype = "NM" THEN b.pay_type = 2 AND b.ref_id = c.id WHEN a.registrationtype = "DB" THEN b.pay_type = 2 AND b.ref_id = c.id ELSE b.pay_type = 1 AND b.ref_id = a.regid END ORDER BY `date` ASC*/
						...
						//priyanka d - 28-march-23 >> added this to get status email was sent or not
						$this->db->select('subject');
						$this->db->where(array('subject' => 'Exam Enrollment Acknowledgement'));
						$this->db->where("message like '%" . $SearchVal . "%'");
						$this->db->from('email_send_logs');
						$email_send_logs = $this->db->get();
						$emailLogs = $email_send_logs->result_array();
						//$data['member'] = $members;
						//echo 'member</br>';
						//echo $this->db->last_query(); exit;
				...
				if ($searchOn == '01') // 01 for Exam Details
				{
					...
					$this->db->join('payment_transaction b', 'b.member_regnumber=c.regnumber AND b.ref_id = c.id', 'LEFT');
					$this->db->join('admit_card_details ad', 'ad.mem_exam_id=b.ref_id', 'LEFT');
					...
					if (count($exams1)) {
						...
						for ($j = 0; $j < count($exams1); $j++) {
							...
							/*ADDED BY POOJA MANE 29-6-2023*/
							$exm_cd = $exams1[$j]['exam_code'];
							$exm_prd = $exams1[$j]['exam_period'];
							if ($exm_cd == '101') {
								$exm_dt = $this->master_model->getRecords("subject_master", array('exam_code' => 101, 'exam_period' => $exm_prd), 'exam_date');
								$exam_date = $exm_dt[0]['exam_date'];
							}
							/*ADDED BY POOJA MANE END 29-6-2023*/ //echo $this->db->last_query(); die;
							...
							$exams1[$j]['exam_date'] = $exam_date;

							$exams1[$j]['real_transaction_status'] = array();
							$billdesk_pg_model = $this->billdesk_pg_model->billdeskqueryapi($exams1[$j]['receipt_no']);
							if (isset($billdesk_pg_model['refundInfo']) && !empty($billdesk_pg_model['refundInfo'])) {
								$exams1[$j]['status'] = 3;
							}
						}
					}
				...
				$data['member'] = array_merge($members1, $members2, $emailLogs);
		...
		if (isset($_POST['btnSearch'])) {
			...
			} else	// for "Noraml Exams" 
			{
				...
				if ($res_flg == 0) {
					...
					$select = 'a.regnumber,a.exam_code,a.exam_period,COUNT(DISTINCT (a.id)) mem_cnt ,b.description,b.exam_type';
					if ($exam_period != '' && $search_for != '') {
						if ($search_for == "01") {
							...
							$where .= " AND  d.exam_code = " . $exam_code . "";
							...
							if ($exam_code != '')
								$where .= " AND  a.exam_code = " . $exam_code . "";
							$where .= " AND  a.exam_period = " . $exam_period . "";
							$this->db->join('payment_transaction d', 'd.ref_id=a.id', 'LEFT');
							$this->db->join('exam_master b', 'b.exam_code=a.exam_code', 'INNER');
							//$this->db->join('member_registration c','c.regnumber=a.regnumber','LEFT');
						...
						if (!empty($exm_date)) {
							$this->db->join('admit_card_details ad', 'ad.mem_mem_no=a.regnumber', 'LEFT');
							$array = array(
								'ad.exam_date=' => $exm_date,
	...
	//Get Exam date function added by Pooja Mane:06-12-2022
	public function GetExamDate()
	{
		...
		$exm_cd = $_POST['exm_cd'];
		$exm_prd = $_POST['exm_prd'];

		if (!empty($exm_cd && $exm_prd)) {
			//$this->db->distinct();
			$this->db->select('exam_date');
			//$this->db->join('admit_card_details' , 'admit_card_details.exm_cd = misc_master.exm_cd');
			$array = array('exm_cd =' => $exm_cd, 'exm_prd ' => $exm_prd, 'exam_date >=' => $today);
			$this->db->where($array);
			$this->db->group_by('exam_date');
			$exam_date = $this->Master_model->getRecords("admit_card_details");


			if (count($exam_date)) {
				$str .= '<option value="">Select</option>';
				foreach ($exam_date as $row) {
					$str .= '<option value="' . $row['exam_date'] . '">' . $row['exam_date'] . '</option>';
				}
			}
		}
		//echo $this->db->last_query();
	...
	//Get Exam date function end by Pooja Mane:06-12-2022
	...
	public function sendAcknowledgeMail()
	{

		if (isset($_GET['transaction_no'])) {

			$payment_info  = $this->master_model->getRecords('payment_transaction', array('transaction_no' => $_GET['transaction_no']));
			//echo $this->db->last_query().'<br>';
			$this->db->where('invoice_image !=""');
			$invoice_info     = $this->master_model->getRecords('exam_invoice', array('receipt_no' => $payment_info[0]['receipt_no']));
			//echo $this->db->last_query().'<br>';
			//echo'<pre>';print_r($invoice_info);
			$result     = $this->master_model->getRecords('member_registration', array('regnumber' => $invoice_info[0]['member_no']));
			//echo $this->db->last_query().'<br>';
			$admit_card_info  = $this->master_model->getRecords('admit_card_details', array('mem_exam_id' => $payment_info[0]['ref_id']), 'admitcard_image,mode');
			//echo $this->db->last_query().'<br>';
			...
			$exam_info = $this->master_model->getRecords('member_exam', array('member_exam.id' => $payment_info[0]['ref_id']), 'member_exam.exam_mode,member_exam.exam_medium,member_exam.exam_period,center_master.center_name,member_exam.exam_center_code,exam_master.description');
			//echo $this->db->last_query().'<br>';


			$this->db->where('medium_master.exam_code', $invoice_info[0]['exam_code']);
			...
			$admit_card_pdf = base_url() . '/uploads/admitcardpdf/' . $admit_card_info[0]['admitcard_image'];
			$invoice_pdf = base_url() . 'uploads/examinvoice/supplier/' . $invoice_info[0]['invoice_image'];
			...
			$newstring2 = str_replace("#REG_NUM#", "" . $invoice_info[0]['member_no'] . "", $newstring1);
			...
			$newstring18 = str_replace("#MODE#", "" . $admit_card_info[0]['mode'] . "", $newstring17);
			...
			//echo $result[0]['email'].'=========='.$final_str;
			$files = array($invoice_pdf, $admit_card_pdf);
	...
	public function check_bank_bc_id_no_duplication($ippb_emp_id, $params)
	{
		list($name_of_bank_bc, $regid) = explode(',', $params);
	...
	{
		$data["exam_code"] = $data["exam_period"] = array();

		$data["breadcrumb"] = '<ol class="breadcrumb"><li><a href="' . base_url() . 'admin/MainController"><i class="fa fa-dashboard"></i>Home</a></li><li>Exam Registration List</li></ol>';
		$current_date = date('Y-m-d');
		/*$d = date('Y-m-d', strtotime("- 15 day", strtotime($current_date)));
		$this->db->where("(exam_to_date >='" . $d . "' OR exam_to_date='0000-00-00')");
		//$this->db->or_where('exam_to_date','0000-00-00');
		...
		$this->db->select('exm_cd AS exam_code');
		$this->db->where("exam_date >= NOW()", NULL, FALSE);
		$data["exam_code"] = $this->Master_model->getRecords("admit_card_details", '', '', array('exm_cd' => 'ASC'));
		/*SELECT exm_cd,exm_prd,exam_date FROM admit_card_details WHERE exam_date >= NOW() GROUP BY exm_cd,exm_prd ORDER BY exm_cd ASC;  */
		//echo $this->db->last_query();exit;

		$this->db->group_by('exm_prd');
		$this->db->select('exm_prd AS exam_period');
		$this->db->where("exam_date >= NOW()", NULL, FALSE);
		$data["exam_period"] = $this->Master_model->getRecords("admit_card_details", '', '', array('exm_prd' => 'ASC'));
		/*SELECT exm_cd,exm_prd,exam_date FROM admit_card_details WHERE exam_date >= NOW() GROUP BY exm_cd,exm_prd ORDER BY exm_cd ASC;  */
		//echo $this->db->last_query();exit;  

		$this->load->view('admin/exam_dashboard_report', $data);
	}

	public function getExamDashboardList()
	{
		$this->load->model('iibfbcbf/Iibf_bcbf_model');

		$form_action = trim($this->security->xss_clean($this->input->post('form_action')));

		$table = 'member_exam me'; //

		$GroupBy = "";
		//$GroupBy = " Group By pt.transaction_no"; 

		$column_order = array('mr.regid', 'mr.regnumber', 'CONCAT(mr.namesub, " ", mr.firstname, IF(mr.lastname != "", CONCAT(" ", mr.middlename), ""), IF(mr.lastname != "", CONCAT(" ", mr.lastname), "")) AS DispName', 'gender', 'd.description', 'me.exam_fee', 'me.exam_medium', 'me.exam_center_code', 'pt.transaction_no', 'pt.status', 'pt.date', 'ac.exam_date', 'pt.transaction_details'); //SET COLUMNS FOR SORT

		$column_search = array('mr.regnumber', 'CONCAT(mr.namesub, " ", mr.firstname, IF(mr.lastname != "", CONCAT(" ", mr.middlename), ""), IF(mr.lastname != "", CONCAT(" ", mr.lastname), ""))', 'gender', 'd.description', 'me.exam_fee', 'me.exam_medium', 'me.exam_center_code', 'pt.transaction_no', 'pt.status', 'pt.date', 'ac.exam_date', 'pt.transaction_details'); //SET COLUMN FOR SEARCH
		$order = array('mr.regid' => 'DESC'); // DEFAULT ORDER    


		//mr.isactive = 1 AND 
		$WhereForTotal = "WHERE mr.isdeleted = 0 AND pt.pay_type = '2' AND me.exam_code IN (SELECT exam_code FROM subject_master WHERE exam_date >= NOW() GROUP BY exam_code) AND me.exam_period IN (SELECT exam_period FROM subject_master WHERE exam_date >= NOW() GROUP BY exam_period)"; //DEFAULT WHERE CONDITION FOR ALL RECORDS 
		$Where = "WHERE mr.isdeleted = 0 AND pt.pay_type = '2' AND me.exam_code IN (SELECT exam_code FROM subject_master WHERE exam_date >= NOW() GROUP BY exam_code) AND me.exam_period IN (SELECT exam_period FROM subject_master WHERE exam_date >= NOW() GROUP BY exam_period)";
		...
		//CUSTOM SEARCH
		$exam_code = trim($this->security->xss_clean($this->input->post('exam_code')));
		if ($exam_code != "") {
			$Where .= " AND me.exam_code = '" . $exam_code . "'";
		}

		$exam_period = trim($this->security->xss_clean($this->input->post('exam_period')));
		if ($exam_period != "") {
			$Where .= " AND me.exam_period = '" . $exam_period . "'";
		}

		$search_for = trim($this->security->xss_clean($this->input->post('search_for')));
		$exam_date = trim($this->security->xss_clean($this->input->post('exam_date')));
		if ($search_for != '') {
			if ($search_for == "NM") {
				$Where .= " AND mr.registrationtype = 'NM'";
			} else if ($search_for == "O") {
				$Where .= " AND mr.registrationtype = 'O'";
			} else if ($search_for == "DB") {
				$Where .= " AND mr.registrationtype = 'DB'";
			}
		}
		if ($exam_date != "") {
			$Where .= " AND ac.exam_date = '" . $exam_date . "'";
		}
		...
		$join_qry = " LEFT JOIN member_registration mr ON mr.regnumber=me.regnumber";
		$join_qry .= " LEFT JOIN admit_card_details ac ON ac.mem_exam_id=me.id AND ac.exam_date >= NOW()";
		$join_qry .= " LEFT JOIN exam_master d ON d.exam_code=me.exam_code";
		$join_qry .= " LEFT JOIN payment_transaction pt ON pt.ref_id=me.id";
		...
		if ($form_action == 'export') {
			...
			$fileName = "Exam_Registration_List_" . date('Y-m-d') . ".xls";
			// Column names 
			$fields = array('Sr. No.', 'Registration No.', 'First Name', 'Gender', 'Exam Name', 'Exam Fee', 'Exam Medium', 'Center Name', 'Transaction No.', 'Payment Status', 'Transaction Date', 'Exam Date');
		...
		if ($Rows && count($Rows) > 0) {
			foreach ($Rows as $Res) {
				...
				$row[] = $Res['exam_fee'];
				$row[] = $Res['exam_medium'];
				$row[] = $Res['exam_center_code'];
				$row[] = $Res['transaction_no'];
				$row[] = $Res['status'];
				$row[] = $Res['date'];
				$row[] = $Res['exam_date'];
