<?php
/*
 * Controller Name	:	Cron File Transfer : Siffy Vendor
 * Created By		:	Bhushan
 * Created Date		:	21 April 2020
 *
*/
defined('BASEPATH') OR exit('No direct script access allowed');
class Cron_csv_ftp_siffy extends CI_Controller {
	...
	{ exit;
        ...
        $array = $this->master_model->getRecords('cron_csv_SIFFY','','new_file_name,record_count');
		...
		$local_server_dir_path = '/home/supp0rttest/public_html/uploads/cronCSV_SIFFY/'.$current_date.'/';
	...
	public function sendmail($current_date = "", $msg = "")
	{
		...
		$subject = 'SIFY Vendor: E_Learning Candidate Registrations Data Transfer';
		...
		$recipient_list = array(
            ...
            'iibfdevp@esds.co.in',
            'rajiv.gopal@sifycorp.com',
            'karthick.krishnakumar@sifycorp.com',
            'sameer.deshmukh@sifycorp.com',
			'ponradhika.murugesan@sifycorp.com',
			'livewire.helpdesk@sifycorp.com'
        );
		
		//$recipient_list = array('bhushan.amrutkar@esds.co.in');
		
		$attachment_filename = 'logs_'.$current_date.'.txt';
		$attachment_path = './uploads/cronCSV_SIFFY/'.$current_date.'/' . $attachment_filename;
	...
	public function SIFFY_MSME_FTP($current_date = "")
	{
		$start_time = date("Y-m-d H:i:s");
		$success = array();
		$error = array();
		...
		$CurrentDate = date('Y-m-d',strtotime($current_date));
		$this->db->where('CurrentDate', $CurrentDate);
        $array = $this->master_model->getRecords('cron_csv_SIFFY_MSME','','new_file_name,record_count');
		$new_file_name = $array[0]['new_file_name'];
		$record_count = $array[0]['record_count'];
		...
		$local_server_dir_path = '/home/supp0rttest/public_html/uploads/cronCSV_SIFFY_MSME/'.$current_date.'/';
		...
		// send email notification -
		$result = $this->SIFFY_MSMEsendmail($current_date, $msg);
	}
	
	public function SIFFY_MSMEsendmail($current_date = "", $msg = "")
	{
		if($current_date == "")
		{
			$current_date = date("Ymd");	
		...
		//$from_email = 'noreply@iibf.org.in';
		$from_email = 'logs@iibf.esdsconnect.com';
		$subject = 'SIFFY_MSME Vendor: E_Learning Candidate Registrations Data Transfer';
		...
		$attachment_filename = 'logs_'.$current_date.'.txt';
		$attachment_path = './uploads/cronCSV_SIFFY_MSME/'.$current_date.'/' . $attachment_filename;
