...
class TrainingDashboard extends CI_Controller
{
	...
	if (isset($_POST['btnSearch'])) 
		...
		$batch_code = $this->input->post('batch_code'); 
		 $select = 'batch_code, zone_code,training_type, program_code,COUNT(*) AS Counts,center_name,start_date,end_date';
		 $this->db->select($select);
		 $this->db->where('pay_status',1);
		 $this->db->where('batch_code',$batch_code);
		 $this->db->group_by("batch_code");
		 $this->db->order_by("batch_code", "DESC");
		 $countQry = $this->master_model->getRecords('blended_registration');
	
		//check traning is activated or not 
		$this->db->where('isdelete',0);
		$batch_code = $this->master_model->getRecords('blended_dates');
		//echo $this->db->last_query(); die;
		$data['batch_code'] = $batch_code;
		$traninginfo=array();
		$traninginfo = $this->master_model->getRecords('blended_program_activation_master');
		
		$data['counts'] = $countQry;
		$data['traninginfo'] = $traninginfo;
		$this->load->view('admin/trainingbatch/counts',$data);
	}
	else{
		$this->db->where('isdelete',0);
		$batch_code = $this->master_model->getRecords('blended_dates');
	...
	{ 
		...
		if (isset($_POST['btnSearch'])) 
		{
			...
			$this->load->view('admin/trainingbatch/member_list',$data);
		}
		else
		{
			$this->db->where('blended_registration.batch_code',$batch_code);
	...
	public function unpaid_count()
	{
		if (isset($_POST['btnSearch'])) 
		{
			...
			//echo $this->db->last_query(); die;
			$data['mem_info'] = $mem_info;
			$this->load->view('admin/trainingbatch/unpaid_list',$data);
		}
		else
		{
			$this->db->where('blended_registration.pay_status',1);
	...
	{
		$regnumber = $this->input->post('member_no');
		//$batchcode = $this->uri->segment(5);
		$batchcode = $this->input->post('batch_code'); 
	...
	$attachpath="";
		 ...
		 $email = 'chaitali.jadhav@esds.co.in';//$reg_info[0]['email'];
									...
									//'to'=>'nirmala.menezes@yahoo.co.in',
										...
										'message'=>$final_selfstr);	
											if($this->Emailsending->mailsend_attch($self_mail_arr,$attachpath))
											{
											 ...
											 $resp = array('status'  => 'success','userMsg' => 'Acknowledgment mail is sent successfully.');
        									echo json_encode($resp);
												//redirect('admin/trainingbatch/TrainingDashboard/member_count');
												//echo $email. ' '.$regnumber; 
												}
												else{
												 
											 $resp = array('status'  => 'error','userMsg' => 'Acknowledgment mail is not sent.');
        									echo json_encode($resp);
												}
					...
					if($reg_info[0]['member_no'] == $regnumber)
					{
						...
						if(count($emailerSelfStr) > 0)
						{
							...
							if($this->Emailsending->mailsend_attch($self_mail_arr,$attachpath))
										...
										redirect('admin/trainingbatch/TrainingDashboard/unpaid_count');
												//echo $regnumber;
												//echo '**ghjgfhj'; 
												//echo  $emails;
												}
...
");
	...
	#-------------------end -contact classes -------------------------#
		...
		//	$data['cc_sub_info'] = $cc_sub_info;
			...
			$this->load->view('admin/trainingbatch/cc_member_list',$data);
	...
	#-------------------end -contact classes -------------------------#
		...
		//	$data['cc_sub_info'] = $cc_sub_info;
			...
			$this->load->view('admin/trainingbatch/cc_subject_count',$data);
