...
class Cron_custom_sm extends CI_Controller {
			//exit;
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			if(count($edited_mem_data))
			{
				...
				{	
					...
					$benchmark_data = $data.$img_edited_by.'|'.$optnletter.'|'.$photoflag.'|'.$signflag.'|'.$idprofflag.'|'.date('d-M-y H:i:s',strtotime($imgdata['benchmark_edit_date'])).'|'.$imgdata['aadhar_card'].'|'.$imgdata['bank_emp_id'].'|'.$photo.'|'.$signature.'|'.$idproofimg.'|'.$benchmark_disability.'|'.$visually_impaired.'|'.$vis_imp_cert_img.'|'.$orthopedically_handicapped.'|'.$orth_han_cert_img.'|'.$cerebral_palsy.'|'.$cer_palsy_cert_img."\n";
					$benchmark_file_flg = fwrite($benchmark_fp, $benchmark_data);
					if($benchmark_file_flg)
						$success['benchmark_file'] = "Edited Candidate Details Photo File Generated Successfully. ";
					else
	...
	public function dra_member_edited()
	{
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			$this->db->select("d.*");
			$this->db->where('regnumber', '845135596');
