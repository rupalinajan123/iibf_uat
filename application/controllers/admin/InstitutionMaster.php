...
class InstitutionMaster extends CI_Controller {
	...
	public function index()
	{
		...
		$data["breadcrumb"] = '<ol class="breadcrumb"><li><a href="'.base_url().'admin/MainController"><i class="fa fa-dashboard"></i> Home</a></li>
									<li><a href="'.base_url().'admin/'.$this->router->fetch_class().'">Manage Institution</a></li>
							   </ol>';
		
		$this->load->view('admin/masters/institution_list',$data);
	}
	...
	public function getList()
	{
		...
		$total_row = $this->UserModel->getRecordCount("institution_master a",$field,$value);
		
		$url = base_url()."admin/InstitutionMaster/getList/";
		
		$config = pagination_init($url,$total_row, $per_page, 2);
		$this->pagination->initialize($config);
		
		$this->db->where('institution_delete',0);
		$res = $this->UserModel->getRecords("institution_master a", '', $field, $value, $sortkey, $sortval, $per_page, $start);
		...
		if($res)
		{
			...
			foreach($result as $row)
			{
				...
				$action = '<a href="'.base_url().'admin/InstitutionMaster/edit/'.$row['id'].'">Edit | </a> <a href="'.base_url().'admin/InstitutionMaster/delete/'.$row['id'].'" onclick="'.$confirm.'">Delete</a>';
				$data['action'][] = $action;
	...
	public function add(){
		...
		if(isset($_POST['btnSubmit']))
		{
			...
			$this->form_validation->set_rules('institude_id','Institute Code','trim|required');
			if($this->form_validation->run()==TRUE)
			{
				...
				$insert_data = array(	
								...
								'institude_id'			=>$this->input->post('institude_id'),
								'created_by'			=>$this->UserID,
								'created_on'			=>date('Y-m-d H:i:s'),
							);
				
				if($this->master_model->insertRecord('institution_master',$insert_data))
				{
					
					$logs_data = array(
										'date'=>date('Y-m-d H:i:s'),
										'title'=>'Add Accredited Institution Successful',
					...
					redirect(base_url().'admin/InstitutionMaster');
				}
				else
				{
					$logs_data = array(
										'date'=>date('Y-m-d H:i:s'),
										'title'=>'Add Accredited Institution Unsuccessful',
										'description'=>serialize($insert_data),
										'userid'=>$this->UserID,
										'ip'=>$this->input->ip_address()
									);
					...
					redirect(base_url().'admin/InstitutionMaster/add');
				}
			}
			else
			{
		...
		$data['breadcrumb'] = '<ol class="breadcrumb"><li><a href="'.base_url().'admin/MainController"><i class="fa fa-dashboard"></i> Home</a></li>
					<li><a href="'.base_url().'admin/'.$this->router->fetch_class().'">Manage Accredited Institution</a></li>
		...
		$this->load->view('admin/masters/institution_add',$data);
	}
	...
	public function edit(){
		...
		if(is_numeric($id))
		{
			$institutionRes = $this->master_model->getRecords('institution_master',array('id'=>$id));
			if(count($institutionRes))
			{
				$institutionRes[0]['add_form'] = 0;
		...
		if(isset($_POST['btnSubmit']))
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				if($this->master_model->updateRecord('institution_master',$update_data,array('id'=>$id)))
				{
					...
					$logs_data = array(
										'date'=>date('Y-m-d H:i:s'),
										'title'=>'Update Accredited Institution Successful',
				...
				else
				{
					$logs_data = array(
										'date'=>date('Y-m-d H:i:s'),
										'title'=>'Update Accredited Institution Unsuccessful',
										'description'=>serialize($desc),
										'userid'=>$this->UserID,
										'ip'=>$this->input->ip_address()
									);
					...
					$this->session->set_flashdata('error','Error occured while adding record');
					redirect(base_url().'admin/InstitutionMaster/edit/'.$id);
				}
			}
			else
			{
	...
	public function delete()
	{
		...
		if(is_numeric($id))
		{
			$update_data = array('institution_delete'=>1);
			
			if($this->master_model->updateRecord('institution_master', $update_data, array('id'=>$id)))
			{
				$logs_data = array(
									'date'=>date('Y-m-d H:i:s'),
									'title'=>'Delete Institution Successful',
			...
			else
			{
				$logs_data = array(
									'date'=>date('Y-m-d H:i:s'),
									'title'=>'Delete Institution Unsuccessful',
									'description'=>serialize(array('id'=>$id)),
									'userid'=>$this->UserID,
									'ip'=>$this->input->ip_address()
								);
	...
	public function import(){
		...
		{			
			if (isset($_FILES["masterfile"]))
			{
				if ($_FILES["masterfile"]["type"] == "text/plain")
				{
					...
					{
						$target_file = "uploads/admin/masters_imported_files/"."INSTITUTE_MASTER_".$this->UserID."_".date('Y-m-d_his').".txt";
						
						// move uploaded file in file logs dir
						$flg = move_uploaded_file($_FILES["masterfile"]["tmp_name"], $target_file);
							...
							while (($line = fgets($handle)) !== false) {
								...
								if($firstlineflag == 1)
								{
									...
									$insert_data = array(	
													...
													'name'	=>$raw_array[1],
													'institude_id'		=>$raw_array[2],
													'created_by'		=>$this->UserID,
													'created_on'		=>date('Y-m-d H:i:s'),
												);
				
									$this->master_model->insertRecord('institution_master',$insert_data);
								}
								else
								{
									$raw_array = explode("|" ,$line);
									...
									{	$firstlineflag = 1;  // Skip first header line
										master_db_backup($basetable = "institution_master"); // keep old and new master in DB
									}	
									else
									{
										//unlink
										...
										redirect(base_url().'admin/InstitutionMaster/import');
									}
								}
							}
			...
			if ($data['success'] != "")
			{
				logadminactivity($log_title = "Accredited Institution master import sucessfull", $log_message = "New Accredited Institution master file import sucessfull file=".$target_file);
			}
			else
			{
				logadminactivity($log_title = "Accredited Institution master import failed", $log_message = "Error = ".$data['error']."  file = ".$target_file);
			}
		}
		...
		$data['sample_file'] = 'ACCREDITED_INSTUTION_ALL_212.TXT';
		$this->load->view('admin/masters/master_import', $data);
	}
	
	public function download(){
		
		$data = "INS_INS_CD|INS_INS_NAM|institude_id\n";
		
		$this->db->where('institution_delete',0);
		$inst_list = $this->Master_model->getRecords("institution_master a");

		foreach($inst_list as $id => $row_details) {
		
	$name=	htmlspecialchars_decode ($row_details['name']);
		  $data .= $row_details['id']."|".$name."|".$row_details['id']."\n";
		}
		
		logadminactivity($log_title = "Accredited Institution master downloaded", $log_message = "");
		
		header("Content-type: application/x-gzip");
		header('Content-Disposition: attachement; filename="accerdited_institution_master.txt.gz"');
