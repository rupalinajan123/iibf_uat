...
class Cron_csv_bk extends CI_Controller {
	public $UserID;
	...
	//Cron_csv_bk/exam_csv_21
	// // /usr/local/bin/php /home/supp0rttest/public_html/index.php admin Cron_csv_bk exam_csv_21
	public function exam_csv_21()
	{
	//exit;
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			$this->db->where('a.exam_period','220');
			//$this->db->limit(10);
			...
			if(count($can_exam_data))
			{
				...
				foreach($can_exam_data as $exam)
				{
					...
					{	
						...
						{
							$subject_data = $this->Master_model->getRecords('subject_master',array('exam_code'=>'21','exam_period'=>'220','group_code'=>'E','subject_code' =>$exam['elected_sub_code']),'subject_code,subject_description',array('id'=>'DESC'),0,1);
							...
							$firstname = $middlename = $lastname = $city = $state = $pincode = $dateofbirth = $address = $gender = $exam_name = $elected_sub_code = '';
							...
							$designation_name = '';
							$designation  = $this->master_model->getRecords('designation_master',array('dcode'=>$exam['designation']),'dname');
							//echo "<br>SQL => ".$this->db->last_query(); 
							$designation_name = $designation[0]['dname'];
							$designation_name = preg_replace('/[^A-Za-z0-9\-\/ ]/', '', $designation_name);
							
							$institution_name = '';
							$institution_master = $this->master_model->getRecords('institution_master',array('institude_id'=>$exam['associatedinstitute']),'name');	
							//echo "<br>SQL => ".$this->db->last_query(); 
							$institution_name ='';
							...
							if($exam_file_flg)
									$success['cand_exam'] = "File Generated Successfully.";
							else
								$error['cand_exam'] = "Error While Generating File.";
			...
			$desc = json_encode($result);
			$this->log_model->cronlog("CSV END", $desc);
			fwrite($fp1, "\n"."***** CSV END ".$end_time." ******"."\n");
			fclose($fp1);
		}
	}
	...
	exit;
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			$exam_code = array('21','60','62','63','64','65','66','67','68','69','70','71','72','42','58','580','81','5800','810','58000','992');
				...
				foreach($can_exam_data as $exam)
				{
					...
					{	
						foreach($admit_card_details_arr as $admit_card_data)
						{
							...
							$elected_sub_code = '';
							if($exam_code == 60){ $elected_sub_code = $exam['elected_sub_code'];	}
							...
							$exam_name = '';
							$exam_arr = array('21'=>'JAIIB','60'=>'CAIIB','62'=>'CORPORATE BANKING','63'=>'RURAL BANKING','64'=>'INTERNATIONAL BANKING','65'=>'RETAIL BANKING','66'=>'CO-OPERATIVE BANKING','67'=>'FINANCIAL ADVISING','68'=>'HUMAN RESOURCES MANAGEMENT','69'=>'INFORMATION TECHNOLOGY','70'=>'RISK MANAGEMENT','71'=>'CENTRAL BANKING','72'=>'TREASURY MANAGEMENT','42'=>'Diploma in Banking & Finance','58'=>'CUST. SER & BKG CODES AND STAN','580'=>'CUST. SER & BKG CODES AND STAN','81'=>'CERTIFICATE IN RISK IN FINANCIAL SERVICES','5800'=>'CUST. SER & BKG CODES AND STAN','810'=>'CERTIFICATE IN RISK IN FINANCIAL SERVICES','58000'=>'CUST. SER & BKG CODES AND STAN','992'=>'SPECIALIST OFFICERS OF BANKS');
							foreach($exam_arr as $k => $val){
								if($exam_code == $k) { $exam_name = $val; }
							}
							
							if($exam_code == '21' || $exam_code == '62' || $exam_code == '63' || $exam_code == '64' || $exam_code == '65' || $exam_code == '66' || $exam_code == '67' || $exam_code == '68' || $exam_code == '69' || $exam_code == '70' || $exam_code == '71' || $exam_code == '72' || $exam_code == '42' || $exam_code == '58' || $exam_code == '580' || $exam_code == '81' || $exam_code == '5800' || $exam_code == '810' || $exam_code == '58000' || $exam_code == '992')
							{
								$subject_code = '';
								$subject_description = '';
							}
        ...
        if (file_exists($cron_file_dir . $current_date)) {
            ...
            if (count($can_exam_data)) {
                ...
                foreach ($can_exam_data as $exam) {
					...
					$exam_arr         = array('527' => 'ETHICS IN BANKING (E-LEARNING)');
        ...
        if (file_exists($cron_file_dir . $current_date)) {
            ...
            if (count($can_exam_data)) {
                ...
                foreach ($can_exam_data as $exam) {
                    ...
                    $pincode          = preg_replace('/[^0-9]/', '', $exam['pincode']);
                    $exam_arr         = array(
                        '526' => 'DIGITAL BANKING (E-LEARNING)'
                    );
                    foreach ($exam_arr as $k => $val) {
                        if ($exam_code == $k) {
                            $exam_name = $val;
        ...
        if (file_exists($cron_file_dir . $current_date)) {
            ...
            if (count($can_exam_data)) {
                ...
                foreach ($can_exam_data as $exam) {
					...
					$exam_arr = array('1003' => 'CERTIFICATE COURSE ON MSME','1005' => 'CERTIFIED CREDIT PROFESSIONAL');
    ...
    public function exam_csv_NSEIT()
    {
        ...
        if (file_exists($cron_file_dir . $current_date)) {
			...
			$exam_code = array('1002','1010','1011','1012','1013','1014'); // Send Free and Paid Both Applications
	...
	public function jaiib_csv_kesdee()
	{
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			if(count($can_exam_data))
			{
				...
				// Column headers			
				$data1 = "First Name,Middle name,Last Name,Mem. Number,Date of Birth,Gender,Email ID,Mobile,Address,State,Pin Code,Country,Profession,Organization,Designation,Exam Code,Course,Subject Code,Subject Desc,Attempt \n";
				$exam_file_flg = fwrite($fp, $data1);
	...
	public function caiib_csv_mps()
	{
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			$exam_code = array('60','62','63','64','65','66','67','68','69','70','71','72');
			$select = 'DISTINCT(b.transaction_no),a.exam_code,c.firstname,c.middlename,c.lastname,a.regnumber,c.dateofbirth,c.gender,c.email,c.stdcode,c.office_phone,c.mobile,c.address1,c.address2,c.address3,c.address4,c.district,c.city,c.state,c.pincode,c.associatedinstitute,c.designation,a.exam_period,a.elected_sub_code,c.editedon,a.examination_date,a.id AS mem_exam_id';
			$this->db->join('payment_transaction b','b.ref_id = a.id','LEFT');
			$this->db->join('member_registration c','a.regnumber=c.regnumber','LEFT'); 
			$this->db->where_in('a.exam_code', $exam_code);
				...
				foreach($can_exam_data as $exam)
				{
					...
					{	
						foreach($admit_card_details_arr as $admit_card_data)
						{
							...
							$exam_arr = array($this->config->item('examCodeCaiib')=>'CAIIB','62'=>'CORPORATE BANKING',$this->config->item('examCodeCaiibElective63')=>'RURAL BANKING','64'=>'INTERNATIONAL BANKING','65'=>'RETAIL BANKING','66'=>'CO-OPERATIVE BANKING','67'=>'FINANCIAL ADVISING','68'=>'HUMAN RESOURCES MANAGEMENT','69'=>'INFORMATION TECHNOLOGY','70'=>'RISK MANAGEMENT','71'=>'CENTRAL BANKING','72'=>'TREASURY MANAGEMENT');
							foreach($exam_arr as $k => $val){
								if($exam_code == $k) { $exam_name = $val; }
							}
							
							if($exam_code == '62' || $exam_code == $this->config->item('examCodeCaiibElective63') || $exam_code == '64' || $exam_code == '65' || $exam_code == '66' || $exam_code == '67' || $exam_code == '68' || $exam_code == '69' || $exam_code == '70' || $exam_code == '71' || $exam_code == '72'){
								$subject_code = '';
								$subject_description = '';
							}
