  ...
  class Kyc extends CI_Controller
  {
    ...
    public function allocation_type()
    {
      ...
      else 
      {
        ...
        $this->db->where(" mr.regnumber NOT IN (SELECT regnumber FROM member_kyc WHERE field_count = '0' GROUP BY regnumber) ");
    ...
    public function edited_allocation_type()
    {
      ...
      else 
      {
        ...
        $this->db->join('exam_master e','e.exam_code = me.exam_code', 'INNER');
        $this->db->group_by('me.exam_code');
        $mem_list = $this->master_model->getRecords("member_exam me", array('me.pay_status' => '1', 'mr.kyc_edit' => '1', 'mr.isactive' => '1', 'mr.kyc_status' => '0', 'mr.registrationtype' => 'NM', 'mr.editedon !=' => '0000-00-00 00:00:00', 'DATE(mr.editedon) >=' => $kyc_start_date, 'DATE(mr.editedon) <=' => $three_days_back, 'mr.scannedphoto !=' => '','mr.scannedsignaturephoto !=' => '','mr.idproofphoto !=' => ''), "e.exam_code, e.description, count(DISTINCT(me.regnumber)) as count");
        // echo $this->db->last_query();exit;
    ...
    public function edited_list()
    {
      ...
      if (isset($_POST['selectby'])) {
       ...
       $kyc_data = $this->master_model->getRecords("admin_kyc_users", array('DATE(date)' => date('Y-m-d'), 'list_type' => 'Edit','user_type' => 'recommender'), 'original_allotted_member_id');
          ...
          $this->db->where(" mr.regnumber NOT IN (SELECT regnumber FROM member_kyc WHERE field_count = '0' GROUP BY regnumber) ");
          $members = $this->master_model->getRecords("member_registration mr", array( 'mr.kyc_edit' => '1', 'mr.isactive' => '1','mr.editedon !=' => '0000-00-00 00:00:00','mr.registrationtype'=>$type,'DATE(mr.editedon) >=' => $kyc_start_date, 'mr.kyc_status' => '0','DATE(mr.editedon) <=' => $three_days_back, 'DATE(mr.editedon) <=' => $three_days_back, 'mr.scannedphoto !=' => '','mr.scannedsignaturephoto !=' => '','mr.idproofphoto !=' => ''), 'mr.*', array('mr.regid' => 'DESC'), $start, $per_page);
    ...
    public function edited_list_LIVE()
    {
      $type = '';
      $tilte = '';
      $description = $emptylistmsg = '';
      ...
      if (isset($_POST['selectby'])) {
        ...
        /* to show list  for  3 days back dated data */
        $this->db->where("((DATE(editedon) <= '" . $three_days_back . "' AND DATE(editedon) != '0000-00-00') OR (DATE(images_editedon) <= '" . $three_days_back . "' AND DATE(images_editedon) != '0000-00-00'))");
        ...
        if ($row_count == 0) {
          ...
          foreach ($members as $row) {
            ...
            //  $sqltoday=date('Y-m-d');
            //$sql=' select regnumber from member_kyc where  kyc_state = 2 GROUP BY member_kyc.regnumber ORDER BY kyc_id DESC';
            //$this->db->where_not_in('regnumber',stripcslashes($sql),false);
            //$this->db->group_by('member_kyc.regnumber');
            //  $kyc_data = $this->master_model->getRecords("member_kyc",array('regnumber' => $regnumber,'kyc_state'=>'2'),'MAX(kyc_id),regnumber,DATE(recommended_date)');
            //echo $this->db->last_query();exit;
            //  print_r($kyc_data);
            //  die();
            // check if record exist
    ...
    public function	next_edited_allocation_type()
    {
      $kyc_start_date = $this->config->item('kyc_start_date');
        ...
        $this->db->join('exam_master e','e.exam_code = me.exam_code', 'INNER');
        $this->db->group_by('me.exam_code');
        $mem_list = $this->master_model->getRecords("member_exam me", array('me.pay_status' => '1', 'mr.kyc_edit' => '1', 'mr.isactive' => '1', 'mr.kyc_status' => '0', 'mr.registrationtype' => 'NM', 'mr.editedon !=' => '0000-00-00 00:00:00', 'DATE(mr.editedon) >=' => $kyc_start_date, 'DATE(mr.editedon) <=' => $three_days_back), "e.exam_code, e.description, count(me.regnumber) as count");
        // echo $this->db->last_query();exit;
      ...
      if (isset($_POST['selectby'])) {
        ...
        if (count($check)) 
        {
          if ($check[0]['allotted_member_id'] == '') 
          {
            ...
            $this->db->where(" mr.regnumber NOT IN (SELECT regnumber FROM member_kyc WHERE field_count = '0' GROUP BY regnumber) ");
            $members = $this->master_model->getRecords("member_registration mr", array( 'mr.kyc_edit' => '1', 'mr.isactive' => '1','mr.editedon !=' => '0000-00-00 00:00:00','mr.registrationtype'=>$type,'DATE(mr.editedon) >=' => $kyc_start_date, 'mr.kyc_status' => '0','DATE(mr.editedon) <=' => $three_days_back), 'mr.*', array('mr.regid' => 'DESC'), $start, $per_page);
    ...
    */
    ...
    //To show the Edited member Recommender screen  	
    public function edited_member_old($regnumber = NULL, $srno = NULL, $totalRecCount = NULL)
    {
      $success = $error = $description = '';
      $noarray = array();
      $oldfilepath = $file_path = $photo_file = '';
      if ($regnumber) {
        ...
        if ($regnumber) {
          $select = 'regnumber,namesub,firstname,middlename,lastname,DATE_FORMAT(dateofbirth,"%d-%m-%Y") dateofbirth,registrationtype,associatedinstitute,scannedphoto,scannedsignaturephoto,idproofphoto,registrationtype,email';
          $members = $this->master_model->getRecords("member_registration", array(
					'regnumber' => $regnumber,
					'isactive' => '1'
          ), $select, "", '0', '1');
    ...
    //to get next 100 allocation ...for  same day
    public function next_allocated_list_OLD()
    {
      if (isset($_POST['selectby'])) {
        $kyc_start_date = $this->config->item('kyc_start_date');
        $type = $_POST['selectby'];
        ...
        if (count($check)) {
          if ($check[0]['allotted_member_id'] == '') {
            ...
            //	print_r($allocatedmemberarr);exit;
          ...
          $emptylistmsg = ' No records available...!!<br />
          <a href=' . base_url() . 'admin/kyc/Kyc/next_allocation_type/>Back</a>';
    ...
    ////to show the new member list & allocate 100 member  	
    public function allocated_list_OLD()
    {
      //get the todays edited profile candidates
      //$edited_user=array();
      //$this->db->where('kyc_status','0');
      //$this->db->where('kyc_edit','1');
      //	$this->db->where('isactive','1');
      //$this->db->where('DATE(images_editedon)',date('Y-m-d'));		
      //$this->db->or_where('DATE(editedon)',date('Y-m-d'));		
      //$edited_user = $this->master_model->getRecords('member_registration',"",'regnumber'); 
      //print_r($edited_user);exit;
      
      
      $total_id = $recommendedmemberarr = array();
      $kyc_start_date = $this->config->item('kyc_start_date');
      //$kyc_start_date=strtotime('-3 day' ,strtotime($kyc_start_date1));
      //	echo $kyc_start_date1; exit;
      $data['count'] = 0;
      $tilte = $allocated_count = '';
      $description = $emptylistmsg = '';
      $allocates_arr = $members_arr = $result = $array = $allocated_member_list = $data_array = array();
      ...
      if (isset($_POST['selectby'])) {
        ...
        // get all recommended members to todays date
        $member_kyc = $this->db->query("SELECT regnumber
        FROM member_kyc
        WHERE kyc_id IN (
        SELECT MAX(kyc_id)
        FROM member_kyc
        ...
        $this->db->where('kyc_status', '0');
        $this->db->where('registrationtype', $type);
        //	$this->db->where('DATE(createdon)>=',$kyc_start_date && 'DATE(createdon)!=','00-00-0000'  && 'DATE(createdon)<',date('Y-m-d'));	
        $this->db->where('DATE(createdon) !=', '00-00-0000');
        $this->db->where('DATE(createdon)>=', $kyc_start_date);
        $this->db->where('DATE(createdon)<=', $three_days_back);
        ...
        if (count($arraid) > 0) {
          if ($searchBy != '' || $searchBy_regtype != '') {
            ...
            */
            ...
            $this->db->where('DATE(createdon)>=', $kyc_start_date);
            $this->db->where('DATE(createdon)<=', $three_days_back);
            $this->db->where_in('regnumber', array_map('stripslashes', $arraid));
            $members = $this->master_model->getRecords("member_registration");
            /* code end - sagar walzade */
      ...
      //print_r(array_count_values($no));exit;
      /*$emptylistmsg=' No records available-click on the below link to get next 10 Records.........!!<br />
    ...
    /// By VSU : Function to fetch list of members to initiate KYC
    public function member_old($regnumber = NULL, $srno = NULL, $totalRecCount = NULL)
    {
      if ($regnumber) {
    ...
    public function benchmark_allocated_list_old()
    {
      $total_id = $recommendedmemberarr = array();
      //$kyc_start_date=$this->config->item('kyc_start_date');
      $data['count'] = 0;
      ...
      if (isset($form_start_date) && isset($form_end_date)) {
        ...
        $this->db->where('((benchmark_disability = "Y" AND DATE(createdon)>="' . $form_start_date . '" 
				AND DATE(createdon)<="' . $form_end_date . '") OR 
				(benchmark_edit_flg = "Y" AND benchmark_disability != "N" 
				AND DATE(benchmark_edit_date)>="' . $form_start_date . '" 
				AND DATE(benchmark_edit_date)<="' . $form_end_date . '"))');
