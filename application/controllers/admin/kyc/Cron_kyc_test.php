...
class Cron_kyc_test extends CI_Controller {
	...
	public function recommended()
    {
        ...
        if(file_exists($cron_file_dir.$current_date))
        {
			...
			$select = 'mk.kyc_id, mk.regnumber, mk.mem_type, mk.old_data, mk.mem_name, mk.mem_dob, mk.mem_associate_inst, mk.mem_photo, mk.mem_sign, mk.mem_proof,mk.mem_declaration, mr.createdon, mk.user_edited_date, mk.recommended_date, mk.recommended_by, mk.user_type, mk.record_source';
			$this->db->where_in('user_type','recommender');
			$this->db->join('member_registration mr', 'mk.regnumber = mr.regnumber', 'LEFT');
			$member_kyc = $this->Master_model->getRecords('member_kyc mk', array(' DATE(mk.recommended_date)' => $yesterday,' DATE(mk.approved_date)' => $yesterday, 'mk.kyc_status' => '0', 'mk.kyc_state' => 1, 'mr.isactive' => '1', 'mr.isactive' => '1', 'mr.isdeleted' => 0), $select);
            
             // echo '<pre>'.count($member_kyc).$this->db->last_query();die;
            ...
            if(count($member_kyc))
            {
                ...
                foreach($member_kyc as $kyc_data)
                {
                    ...
                    $REC_MEM_INS_CD_FLG = "RA";
                    if($kyc_data['mem_associate_inst'] == '0')
                    {
                        $REC_MEM_INS_CD_FLG = "RR";
                    }
                    ...
                    $REC_SIGNATURE_FLG = "RA";
                    if($kyc_data['mem_sign'] == '0')
                    {
                        $REC_SIGNATURE_FLG = "RR";
                    }
                    
                    $REC_IDPROOFIMG_FLG = "RA";
                    if($kyc_data['mem_proof'] == '0')
                    {
                        $REC_IDPROOFIMG_FLG = "RR";
                    }

                    $REC_DECLARATION_FLG = "RA";
                    if($kyc_data['mem_declaration'] == '0')
                    {
                        $REC_DECLARATION_FLG = "RR";
                    }
                    ...
                    $data .= ''.$kyc_data['kyc_id'].'|'.$kyc_data['regnumber'].'|'.$kyc_data['mem_type'].'|'.$namesub.'|'.$firstname.'|'.$middlename.'|'.$lastname.'|'.$mem_dob.'|'.$associatedinstitute.'|'.$REC_MEM_NAM_FLG.'|'.$REC_MEM_DOB_FLG.'|'.$REC_MEM_INS_CD_FLG.'|'.$REC_PHOTO_FLG.'|'.$REC_SIGNATURE_FLG.'|'.$REC_IDPROOFIMG_FLG.'|'.$REC_DECLARATION_FLG.'|'.$CREATED_ON.'|'.$UPDATED_ON.'|'.$REC_ON.'|'.$REC_BY_USER_ID.'|'.$REC_BY_USER_NAME.'|'.$REC_BY_USER_TYPE.'|'.$LIST_TYPE."\n";
                ...
                //echo"Total KYC Recommended Member Added = ".$mem_cnt.' for date '.$yesterday;
	...
	public function recommended_old()
	{
		//MEM_KYC_ID|MEM_MEM_NO|MEM_MEM_TYP|MEM_TLE|MEM_NAM_1|MEM_NAM_2|MEM_NAM_3|MEM_DOB|MEM_INS_CD|REC_MEM_NAM_FLG(Y/N)|REC_MEM_DOB_FLG(Y/N)|REC_MEM_INS_CD_FLG(Y/N)|REC_PHOTO_FLG(Y/N)|REC_SIGNATURE_FLG(Y/N)|REC_IDPROOFIMG_FLG(Y/N)|CREATED_ON TIMESTAMP 'DD/MON/YY HH24:MI:SS'|UPDATED_ON TIMESTAMP 'DD/MON/YY HH24:MI:SS'|REC_ON TIMESTAMP 'DD/MON/YY HH24:MI:SS'|REC_BY_USER_ID|REC_BY_USER_NAME|REC_BY_USER_TYPE(R/A)|LIST_TYPE(NEW/EDIT)
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			if(count($member_kyc))
			{
				...
				foreach($member_kyc as $kyc_data)
				{
					...
					$REC_DECLARATION_FLG = "N";
					if($kyc_data['mem_declaration'] == '0')
					{
						$REC_DECLARATION_FLG = "Y";
					}
	...
	public function approved()
	{
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			$this->db->join('member_registration mr', 'mk.regnumber = mr.regnumber', 'LEFT');
			$member_kyc = $this->Master_model->getRecords('member_kyc mk', array(' DATE(mk.approved_date)' => $yesterday,' DATE(mk.recommended_date)' => $yesterday, 'mk.kyc_status' => '1', 'mk.kyc_state' => 3, 'mr.isactive' => '1', 'mr.isdeleted' => 0), $select);
			...
			if(count($member_kyc))
			{
				...
				foreach($member_kyc as $kyc_data)
				{
					...
					$REC_MEM_NAM_FLG = "AA";
					$REC_MEM_DOB_FLG = "AA";
					$REC_MEM_INS_CD_FLG = "AA";
					$REC_PHOTO_FLG = "AA";
					$REC_SIGNATURE_FLG = "AA";
					$REC_IDPROOFIMG_FLG = "AA";
					$REC_DECLARATION_FLG = "AA";
                    ...
                    // if($kyc_data['mem_associate_inst'] == '0')
                    // {
                    //     $REC_MEM_INS_CD_FLG = "AR";
                    // }
                    // // echo '<br>inst_flg'.$kyc_data['mem_associate_inst'];
                    // // echo '<br>rec_id'.$kyc_data['recommended_by'];

                    // // echo $REC_MEM_INS_CD_FLG;DIE;
                    ...
                    // if($kyc_data['mem_sign'] == '0')
                    // {
                    //     $REC_SIGNATURE_FLG = "AR";
                    // }
                    
                    // if($kyc_data['mem_proof'] == '0')
                    // {
                    //     $REC_IDPROOFIMG_FLG = "AR";
                    // }

                    // if($kyc_data['mem_declaration'] == '0')
                    // {
                    //     $REC_DECLARATION_FLG = "AR";
                    // }
					...
					$data .= ''.$kyc_data['kyc_id'].'|'.$kyc_data['regnumber'].'|'.$kyc_data['mem_type'].'|'.$namesub.'|'.$firstname.'|'.$middlename.'|'.$lastname.'|'.$mem_dob.'|'.$associatedinstitute.'|'.$REC_MEM_NAM_FLG.'|'.$REC_MEM_DOB_FLG.'|'.$REC_MEM_INS_CD_FLG.'|'.$REC_PHOTO_FLG.'|'.$REC_SIGNATURE_FLG.'|'.$REC_IDPROOFIMG_FLG.'|'.$REC_DECLARATION_FLG.'|'.$CREATED_ON.'|'.$UPDATED_ON.'|'.$REC_ON.'|'.$REC_BY_USER_ID.'|'.$REC_BY_USER_NAME.'|'.$REC_BY_USER_TYPE.'|'.$APPROVED_ON.'|'.$APPROVED_BY_USER_ID.'|'.$APPROVED_BY_USER_NAME.'|'.$LIST_TYPE."\n";
					//echo'<br>';print_r(count($data));//die;
					$i++;
					$mem_cnt++;
				...
				echo"Total KYC Approved Member Added = ".$mem_cnt.' for date '.$yesterday;
	...
	public function approver_recommended()
	{
		//MEM_KYC_ID|MEM_MEM_NO|MEM_MEM_TYP|MEM_TLE|MEM_NAM_1|MEM_NAM_2|MEM_NAM_3|MEM_DOB|MEM_INS_CD|REC_MEM_NAM_FLG(Y)|REC_MEM_DOB_FLG(Y)|REC_MEM_INS_CD_FLG(Y)|REC_PHOTO_FLG(Y)|REC_SIGNATURE_FLG(Y)|REC_IDPROOFIMG_FLG(Y)|CREATED_ON TIMESTAMP 'DD/MON/YY HH24:MI:SS'|UPDATED_ON TIMESTAMP 'DD/MON/YY HH24:MI:SS'|REC_ON TIMESTAMP 'DD/MON/YY HH24:MI:SS'|REC_BY_USER_ID|REC_BY_USER_NAME|REC_BY_USER_TYPE(R/A)|APPROVED_ON TIMESTAMP 'DD/MON/YY HH24:MI:SS'|APPROVED_BY_USER_ID|APPROVED_BY_USER_NAME|LIST_TYPE(NEW/EDIT)
		...
		$result = array("success" => "", "error" => "", "Start Time" => $start_time, "End Time" => "");
		$desc = json_encode($result);
		$this->log_model->cronlog("IIBF KYC Approver Recommended Member Details Cron Execution Start", $desc);
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			// cron file name
			$file = "member_kyc_approver_recommended_".$current_date.".txt";
			...
			$file1 = "logs_".$current_date.".txt";
			$fp1 = fopen($cron_file_path.'/'.$file1, 'a');
			fwrite($fp1, "\n************************* IIBF KYC Approver Recommended Member Details Cron Execution Started - ".$start_time." ********************* \n");
			...
			$this->db->where_in('user_type','approver');
			$this->db->join('member_registration mr', 'mk.regnumber = mr.regnumber', 'LEFT');
			$member_kyc = $this->Master_model->getRecords('member_kyc mk', array(' DATE(mk.recommended_date)' => $yesterday, 'mk.kyc_status' => '0', 'mk.kyc_state' => 1, 'mr.isactive' => '1', 'mr.isactive' => '1', 'mr.isdeleted' => 0), $select);
			...
			if(count($member_kyc))
			{
				...
				foreach($member_kyc as $kyc_data)
				{
					...
					if($file_w_flg)
					{
						$success['file'] = "KYC Approver Recommended Member Details File Generated Successfully. ";
					}
					else
					{
						$error['file'] = "Error While Generating KYC Approver Recommended Member Details File.";
					}
				}
				//echo"Total KYC Approver Recommended Member Added = ".$mem_cnt.' for date '.$yesterday;
				fwrite($fp1, "\n"."Total KYC Approver Recommended Member Added = ".$mem_cnt."\n");
			}
			else
			{
				$success[] = "No data found for the date";
			...
			$result = array("success" => $success, "error" => $error, "Start Time" => $start_time, "End Time" => $end_time);
			$desc = json_encode($result);
			$this->log_model->cronlog("IIBF KYC Approver Recommended Member Details Cron Execution End", $desc);
			
			fwrite($fp1, "\n"."************************* IIBF KYC Approver Recommended Member Details Cron Execution End ".$end_time." **************************"."\n");
			fclose($fp1);
		}
	}

	public function approved_OLD()
	{
		//MEM_KYC_ID|MEM_MEM_NO|MEM_MEM_TYP|MEM_TLE|MEM_NAM_1|MEM_NAM_2|MEM_NAM_3|MEM_DOB|MEM_INS_CD|REC_MEM_NAM_FLG(Y)|REC_MEM_DOB_FLG(Y)|REC_MEM_INS_CD_FLG(Y)|REC_PHOTO_FLG(Y)|REC_SIGNATURE_FLG(Y)|REC_IDPROOFIMG_FLG(Y)|CREATED_ON TIMESTAMP 'DD/MON/YY HH24:MI:SS'|UPDATED_ON TIMESTAMP 'DD/MON/YY HH24:MI:SS'|REC_ON TIMESTAMP 'DD/MON/YY HH24:MI:SS'|REC_BY_USER_ID|REC_BY_USER_NAME|REC_BY_USER_TYPE(R/A)|APPROVED_ON TIMESTAMP 'DD/MON/YY HH24:MI:SS'|APPROVED_BY_USER_ID|APPROVED_BY_USER_NAME|LIST_TYPE(NEW/EDIT)
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			$yesterday = date('Y-m-d', strtotime("- 2 day"));
			$yesterday ='2023-09-27';
			$select = 'mk.kyc_id, mk.regnumber, mk.mem_type, mk.old_data, mk.mem_name, mk.mem_dob, mk.mem_associate_inst, mk.mem_photo, mk.mem_sign, mk.mem_proof,mk.mem_declaration, mr.createdon, mk.user_edited_date, mk.recommended_date, mk.recommended_by, mk.user_type, mk.approved_by, mk.approved_date, mk.record_source';
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			if(count($member_kyc))
			{
				...
				foreach($member_kyc as $kyc_data)
				{
					...
					//echo $kyc_data['user_type'].'idproof--';print_r($REC_IDPROOFIMG_FLG);die;
					...
					$EDIT_DECLARATION_FLG = "N";
					if($kyc_data['edited_mem_declaration'] == '1')
					{
						$EDIT_DECLARATION_FLG = "Y";
					}
					// edited flag end
					...
					$data .= ''.$kyc_data['kyc_id'].'|'.$kyc_data['regnumber'].'|'.$kyc_data['mem_type'].'|'.$namesub.'|'.$firstname.'|'.$middlename.'|'.$lastname.'|'.$mem_dob.'|'.$associatedinstitute.'|'.$REC_MEM_NAM_FLG.'|'.$REC_MEM_DOB_FLG.'|'.$REC_MEM_INS_CD_FLG.'|'.$REC_PHOTO_FLG.'|'.$REC_SIGNATURE_FLG.'|'.$REC_IDPROOFIMG_FLG.'|'.$REC_DECLARATION_FLG.'|'.$EDIT_MEM_NAM_FLG.'|'.$EDIT_MEM_DOB_FLG.'|'.$EDIT_MEM_INS_CD_FLG.'|'.$EDIT_PHOTO_FLG.'|'.$EDIT_SIGNATURE_FLG.'|'.$EDIT_IDPROOFIMG_FLG.'|'.$EDIT_DECLARATION_FLG.'|'.$CREATED_ON.'|'.$UPDATED_ON.'|'.$REC_ON.'|'.$REC_BY_USER_ID.'|'.$REC_BY_USER_NAME.'|'.$REC_BY_USER_TYPE.'|'.$LIST_TYPE."\n";
					//print_r($data);die;
					$i++;
					$mem_cnt++;
	...
	public function benchmark_recommended()
	{
		...
		if(file_exists($cron_file_dir.$current_date))
		{
			...
			$this->db->join('member_registration mr', 'mk.regnumber = mr.regnumber', 'LEFT');
			//$member_kyc = $this->Master_model->getRecords('benchmark_member_kyc mk', array(' DATE(mk.recommended_date)' => $yesterday, 'mr.isactive' => '1', 'mr.isactive' => '1', 'mr.isdeleted' => 0), $select);
