...
/** SAGAR WALZADE : CUSTOM FUNCTIONS START LIVE*/
function _pa($a)
{
  echo '<pre>';
  echo print_r($a, true);
...
class Kyc extends CI_Controller
{
  ...
  public function pending_member_list()
  {
    ...
    $this->db->where('DATE(editedon) >=', $kyc_start_date);
  ...
  public function pending_allocation_type()
  {
    ...
    if (count($allocated_member_list)) {
      ...
      foreach ($arraid as $row) {
        ...
        if ($type == 'NM' && $exam_code != '') {   //echo $type;die;
  ...
  public function pending_allocated_list()
  {
    ...
    if (isset($_POST['selectby'])) {
      ...
      if ($process_flag == '1') {
        ...
        $members = $this->master_model->getRecords("member_registration mr", array('mr.isactive' => '1'), 'mr.*', array('mr.regid' => 'DESC'), $start, $per_page);
        // echo $this->db->last_query();die;
    ...
    if (isset($_POST['selectby'])) {
      ...
      if (count($check)) {
        if ($check[0]['allotted_member_id'] == '') {
          ...
          $members = $this->master_model->getRecords("member_registration as mr", "", 'mr.*', array('mr.regid' => 'DESC'), $start, $per_page);
  ...
  public function allocation_type()
  {
    ...
    if (count($allocated_member_list)) {
      ...
      foreach ($arraid as $row) {
        ...
        // Added exam code condition End Pooja Mane 2023-10-13
        $this->db->group_by('member_registration.regnumber'); //POOJA MANE 2024-08-12
        $members = $this->master_model->getRecords("member_registration", array('member_registration.regnumber' => $row));
        $members_arr[] = $members;
      }
      $emptylistmsg = ' ';
      ...
      //echo '<br>1 : '.date("Y-m-d H:i:s"); //exit;        

      $three_days_back = date('Y-m-d', strtotime("- 3 days")); /* to show list  for  3 days back dated data */
      $data_array = array();
      if (count($allocatedmemberarr) > 0) // get the column data in a single array
      {
        $allocatedmemberarr = iterator_to_array(new RecursiveIteratorIterator(new RecursiveArrayIterator($allocatedmemberarr)), 0);
        $this->db->where_not_in('mr.regnumber', array_map('stripslashes', $allocatedmemberarr));
      }

      $this->db->where(" mr.regnumber NOT IN (SELECT mk.regnumber FROM member_kyc mk JOIN member_registration mr ON mr.regnumber = mk.regnumber  WHERE field_count = '0' AND approved_date >= editedon GROUP BY regnumber) ");

      $this->db->join('member_registration mr', 'mr.regnumber = me.regnumber', 'INNER');
      $this->db->join('exam_master e', 'e.exam_code = me.exam_code', 'INNER');
      $this->db->join('member_kyc as mk', 'mr.regnumber = mk.regnumber', 'left');
      $this->db->where('mk.regnumber IS NULL');
      $this->db->group_by('me.exam_code');
      $this->db->group_by('YEAR(createdon)');
      $this->db->order_by('exam_code');
      $mem_list = $this->master_model->getRecords("member_exam me", array('me.pay_status' => '1', 'mr.kyc_edit' => '0', 'mr.isactive' => '1', 'mr.kyc_status' => '0', 'mr.registrationtype' => 'NM', 'mr.createdon !=' => '0000-00-00 00:00:00', 'DATE(mr.createdon) >=' => $kyc_start_date, 'DATE(mr.createdon) <=' => $three_days_back), "e.exam_code, e.description, count(DISTINCT(me.regnumber)) as count,YEAR(createdon) as year");
      // echo '<br>'.$this->db->last_query();DIE;

      // echo '<br>2 : '.date("Y-m-d H:i:s"); exit;
  ...
  {
    // (to fetch data exam code and year wise)
    if (isset($_POST['select_exm_cd'])) {
      $str = $_POST['select_exm_cd'];
      $str1 = (explode("-", $str));

      $_POST['select_exm_cd'] = $str1[0];
      $year = $str1[1];
    }
    ...
    if (isset($_POST['selectby'])) {
      ...
      if ($process_flag == '1') {
        ...
        if ($type == 'NM') {
          ...
          $this->db->where('me.exam_code', $exam_code);
          $this->db->where('me.pay_status', '1');
          $this->db->like('mr.createdon', $year);
          $this->db->group_by('me.regnumber');
          $this->db->order_by('me.id', 'DESC');
        }
        ...
        $this->db->join('member_kyc as mk', 'mr.regnumber = mk.regnumber', 'left');
        $this->db->where('mk.regnumber IS NULL');
        $members = $this->master_model->getRecords("member_registration mr", array('mr.kyc_edit' => '0', 'mr.isactive' => '1', 'mr.kyc_status' => '0', 'mr.createdon !=' => '0000-00-00 00:00:00', 'mr.registrationtype' => $type, 'DATE(mr.createdon) >=' => $kyc_start_date, 'DATE(mr.createdon) <=' => $three_days_back), 'mr.*', array('mr.regid' => 'DESC'), $start, $per_page);
        // echo "<pre>";
    ...
    $allocated_member_list = $this->master_model->getRecords("admin_kyc_users ", array('DATE(date)' => date('Y-m-d'), 'user_id' => $this->session->userdata('kyc_id'), 'list_type' => 'New', 'allotted_member_id !=' => ''));
    //  print_r($allocated_member_list);exit;
    //allocated_count
    if (count($allocated_member_list) > 0) {
      $data['count'] = $allocated_member_list[0]['allocated_count'];
      $arraid = explode(',', $allocated_member_list[0]['allotted_member_id']);
      ...
      if (count($arraid) > 0) {
        if ($searchBy != '' || $searchBy_regtype != '') {
          ...
          /* 
              Change by : Sagar Walzade (12-5-2022) - code start
              below loop removed due to page speed issue and we have used where_in condition.
            */
    ...
    /* by sagar walzade - below line commented and passed direct $members to $data['result'] */
    // $data['result'] = call_user_func_array('array_merge', $members_arr);
    $data['result'] = $members;
  ...
  public function edited_allocation_type()
  {
    ...
    if (count($edited_member_list)) {
      ...
      foreach ($arraid as $row) {
        ...
        $this->db->where(" member_registration.regnumber IN (SELECT mk.regnumber FROM member_kyc mk JOIN member_registration ON member_registration.regnumber = mk.regnumber  WHERE field_count != '0' and kyc_state = '2' GROUP BY regnumber) ");
        // Added exam code condition End Pooja Mane 2023-10-13
        $this->db->where_in('member_registration.regnumber', array_map('stripslashes', $arraid));
        $members = $this->master_model->getRecords("member_registration", array('member_registration.regnumber' => $row));
        $members_arr[] = $members;
      ...
      // echo count($edited_member_list);die;
      /* Start Code To Get Recent Allotted Member Total Count */
      ...
      $this->db->where(" mr.regnumber IN (SELECT mk.regnumber FROM member_kyc mk JOIN member_registration mr ON mr.regnumber = mk.regnumber  WHERE field_count != '0' and kyc_state = '2' GROUP BY regnumber) ");

      $this->db->where("(SELECT id FROM member_exam WHERE regnumber = me.regnumber order by id DESC LIMIT 1) = me.id");

      $this->db->join('member_registration mr', 'mr.regnumber = me.regnumber', 'INNER');
      $this->db->join('exam_master e', 'e.exam_code = me.exam_code', 'INNER');
      $this->db->group_by('me.exam_code');
      $mem_list = $this->master_model->getRecords("member_exam me", array('me.pay_status' => '1', 'mr.kyc_edit' => '1', 'mr.isactive' => '1', 'mr.kyc_status' => '0', 'mr.registrationtype' => 'NM', 'mr.images_editedon !=' => '0000-00-00 00:00:00', 'DATE(mr.images_editedon) >=' => $kyc_start_date, 'DATE(mr.images_editedon) <=' => $three_days_back, 'mr.scannedphoto !=' => '', 'mr.scannedsignaturephoto !=' => '', 'mr.idproofphoto !=' => ''), "e.exam_code, e.description, count(DISTINCT(me.regnumber)) as count");
      // echo '<pre>'.$this->db->last_query();exit;

      // print_r($edited_mem_exm_arr);die();
  ...
  public function edited_list()
  {
    ...
    if (isset($_POST['selectby'])) {
      ...
      $kyc_data = $this->master_model->getRecords("admin_kyc_users", array('DATE(date)' => date('Y-m-d'), 'list_type' => 'Edit', 'user_type' => 'recommender'), 'original_allotted_member_id');
      ...
      if ($type == 'NM') {
        ...
        $this->db->where(" mr.regnumber NOT IN (SELECT regnumber FROM member_registration  WHERE declaration = '' AND createdon >= '2022-04-01' AND registrationtype = 'O' GROUP BY regnumber) ");
      } //ADDED TO FILTER EMPTY DECLARATION RECORDS 2024-06-19

      $this->db->where(" mr.regnumber NOT IN (SELECT mk.regnumber FROM member_kyc mk JOIN member_registration mr ON mr.regnumber = mk.regnumber  WHERE field_count = '0' AND approved_date >= images_editedon GROUP BY regnumber) ");

      $this->db->where(" mr.regnumber IN (SELECT mk.regnumber FROM member_kyc mk JOIN member_registration mr ON mr.regnumber = mk.regnumber  WHERE field_count != '0' and kyc_state = '2' GROUP BY regnumber) ");

      $this->db->where(" mr.regnumber NOT IN (SELECT mk.regnumber FROM member_kyc mk JOIN member_registration mr ON mr.regnumber = mk.regnumber  WHERE field_count != '0' AND kyc_state = '1' AND user_edited_date = '0000-00-00 00:00:00' ) ");

      $members = $this->master_model->getRecords("member_registration mr", array('mr.kyc_edit' => '1', 'mr.isactive' => '1', 'mr.images_editedon !=' => '0000-00-00 00:00:00', 'mr.registrationtype' => $type, 'DATE(mr.images_editedon) >=' => $kyc_start_date, 'mr.kyc_status' => '0', 'DATE(mr.images_editedon) <=' => $three_days_back,  'mr.scannedphoto !=' => '', 'mr.scannedsignaturephoto !=' => '', 'mr.idproofphoto !=' => ''), 'mr.*', array('mr.regid' => 'DESC'), $start, $per_page);
      ...
      if ($row_count == 0) {
        ...
        //  print_r($regstr);exit;
        // if ($regstr != '')
        if (($regstr != '' && $regstr > '0' && $list_type == 'Edit') || $user_id == '138') {  // insert the allocated array list in table
          $insert_data = array(
            'user_type'      => $this->session->userdata('role'),
            'user_id'        => $this->session->userdata('kyc_id'),
            'allotted_member_id'  => $regstr,
      ...
      if (count($arraid) > 0) {
        if ($searchBy != '' || $searchBy_regtype != '') {
          ...
          if (isset($_POST['btnSearch'])) {
            if (count($members) == 0) {
              ...
              if (count($kyc_done)) {
                ...
                //Update searched member in alloted list- pooja mane : 02-02-2023
                $arr = $this->master_model->updateRecord('admin_kyc_users', $update_data, array('DATE(date)' => date('Y-m-d'), 'list_type' => 'Edit', 'user_type' => 'recommender', 'user_id' => $this->session->userdata('kyc_id')));
          ...
          foreach ($members as $member) {
            ...
            $this->db->where('mr.kyc_status', '0');
            $this->db->where('mr.registrationtype', $type);
            $this->db->where('DATE(mr.images_editedon) !=', '00-00-0000');
            $this->db->where_in('mr.regnumber', array_map('stripslashes', $arraid));
            $members = $this->master_model->getRecords("member_registration mr", array());
            $members_arr[] = $members;
          }
        }
      }
        ...
        if (count($allocated_member_list) > 0) {
          ...
          if (count($arraid) > 0) {
            if ($searchBy != '' || $searchBy_regtype != '') {
              ...
              //   $this->db->where('kyc_status', '0');
              //   $this->db->where('registrationtype', $type);
              //   $this->db->where('DATE(images_editedon) !=', '00-00-0000');
              //   $members = $this->master_model->getRecords("member_registration", array('regnumber' => $row));
              //   $members_arr[] = $members;
              // }
            }
  ...
  public function edited_member($regnumber = NULL, $srno = NULL, $totalRecCount = NULL)
  {
    ...
    if ($regnumber) {
      ...
      $today = date("Y-m-d H:i:s");
      $date = date("Y-m-d H:i:s");
      $fedai_array = array(1009); //Added by pooja mane

      if (isset($_POST['btnSubmit'])) {
        $select = 'regnumber,registrationtype,email,createdon,excode,bank_bc_id_card,date_of_commenc_bc'; //excode added by pooja mane for Fedai 2024-08-06 
        ...
        if (count($check_arr) > 0) {
          ...
          ### ---- added by pooja mane for bcbc empid card 2024-10-17 ---- ###
          if ($data[0]['date_of_commenc_bc'] != '') {
            if (in_array('bcempidprf_checkbox', $check_arr)) {
              $bcempidprf_checkbox = '1';
            } else {
              $bcempidprf_checkbox = '0';
              $field_count++;
              $update_data[] = 'Bcbf-Employment-proof';
              $msg .= 'Bcbf-Employment-proof';
            }
          }
          ### ---- added by pooja mane for bcbc empid card 2024-10-17 ---- ###
          ...
          $bcempidprf_checkbox = '0'; #### added for bc id card 2024-10-17 ###
          if ($data[0]['date_of_commenc_bc'] != '') {
            $msg .= 'Employment-proof,';
            $field_count++;
            $update_data[] = 'Bcbf-Employment-proof';
        ...
        $select = 'namesub,firstname,middlename,lastname,dateofbirth,associatedinstitute,scannedphoto,scannedsignaturephoto,idproofphoto,declaration,empidproofphoto,excode,bank_bc_id_card,date_of_commenc_bc';
        ...
        $insert_data = array(
          ...
          'employee_proof' => isset($empidprf_checkbox) ? $empidprf_checkbox : '0', // Store 0 if NULL
          'mem_declaration' => $declaration_checkbox, //added by pooja mane
          'mem_bank_bc_id_card' => $bcempidprf_checkbox,
        ...
        if ($data[0]['registrationtype'] == 'F' || $data[0]['registrationtype'] == 'A') {
          if ($name_checkbox == '1' && $dob_checkbox == '1' && $emp_checkbox == '1' && $photo_checkbox == '1' && $sign_checkbox == '1' && $idprf_checkbox == '1') {
            ...
            if ($this->Emailsending->mailsend($info_arr)) {
              ...
              // ------- check for employment proof added by pooja mane 2024-08-06-----------#
              ...
              // ------- check for employment proof added by pooja ends 2024-08-06-----------#

              // ------- check for declaration by pooja mane -----------#
              ...
              // -------End heck for declaration by pooja mane -----------#
          ...
          if ($data[0]['registrationtype'] == 'NM' && $data[0]['date_of_commenc_bc'] != '' && $name_checkbox == '1' && $dob_checkbox == '1' && $photo_checkbox == '1' && $sign_checkbox == '1' && $idprf_checkbox == '1' && $bcempidprf_checkbox == '1') {
            ...
            $description = serialize($log_desc);
            $this->KYC_Log_model->create_log('Member recommend', $this->session->userdata('kyc_id'), $last_insterid, $regnumber, $description);
          } //condition added by pooja mane for employment proof for NM bcbf members 2024-10-17

          elseif ($data[0]['date_of_commenc_bc'] == '' && $data[0]['registrationtype'] == 'NM' && in_array($data[0]['excode'], $fedai_array) && $name_checkbox == '1' && $dob_checkbox == '1' && $photo_checkbox == '1' && $sign_checkbox == '1' && $idprf_checkbox == '1' && $empidprf_checkbox == '1' && $declaration_checkbox == '1') { //echo 'in nm in else';die;
            $this->session->set_flashdata('success', 'KYC recommended for' . $data[0]['regnumber'] . '(Previous record) sent to approver !!');
          ...
          //condition added by pooja mane for employyment proof for NM fedai members 2024-08-07
          elseif (($data[0]['registrationtype'] == 'NM' || $data[0]['registrationtype'] == 'DB') && $data[0]['date_of_commenc_bc'] == '' && !in_array($data[0]['excode'], $fedai_array) && $name_checkbox == '1' && $dob_checkbox == '1' && $photo_checkbox == '1' && $sign_checkbox == '1' && $idprf_checkbox == '1') { //echo 'in db in else';die;
            $this->session->set_flashdata('success', 'KYC recommended for' . $data[0]['regnumber'] . '(Previous record) sent to approver !!');
            ...
            $description = serialize($log_desc);
            $this->KYC_Log_model->create_log('Member recommend', $this->session->userdata('kyc_id'), $last_insterid, $regnumber, $description);
            // ECHO $empidprf_checkbox;DIE;
          } //condition added by pooja mane for employyment proof for NM fedai members end 2024-08-07 
          else {
            include_once APPPATH . 'third_party/SBI_ePay/CryptAES.php';
            // echo 'in else in else';die;
            $key = $this->config->item('pass_key');
            $aes = new CryptAES();
            $aes->set_key(base64_decode($key));
            $aes->require_pkcs5();
            ...
            if ($this->Emailsending->mailsend($info_arr)) {
              ...
              // ------- check for BCBF employment proof added by pooja mane 2024-10-17-----------#
              if (in_array('Bcbf-Employment-proof', $update_data)) {
                $updatedata['bank_bc_id_card'] = '';
                $oldfilepath = get_img_name($regnumber, 'bank_bc_id_card');
                $noarray = explode('/bank_bc_id_card_', $oldfilepath);
                // print_r($noarray);//die;
                $description = 'oldpath:' . $oldfilepath . ' || ' . 'file path: ' . $file_path . '/' . $photo_file . '';
                if (isset($noarray[1])) {
                  $file_path = implode('/', explode('/', $oldfilepath, -1));
                  $photo_file = 'k_bank_bc_id_card_' . $noarray[1];
                  if (@rename($oldfilepath, $file_path . '/' . $photo_file)) {
                    $this->KYC_Log_model->create_log('Recommended Employment proof Rename ', $this->session->userdata('kyc_id'), $last_insterid, $regnumber, $description);
                    // echo '<br>Employment proof Rename';
                  } else {
                    $file_path = implode('/', explode('/', $oldfilepath, -1));
                    $photo_file = 'k_bank_bc_id_card_' . $regnumber;
                    @rename($oldfilepath, $file_path . '/' . $photo_file);
                    $description = 'oldpath:' . $oldfilepath . ' || ' . 'file path: ' . $file_path . '/' . $photo_file . '';
                    $this->KYC_Log_model->create_log('fail to delete Employment proof ', $this->session->userdata('kyc_id'), 0, $regnumber, $description);
                    //echo '<br>fail to delete Employment';
                    //$this->KYC_Log_model->create_log('fail to deleted idproof ',$this->session->userdata('id'),0, $reg_no, 'idproof');
                  }
                } else {
                  $file_path = implode('/', explode('/', $oldfilepath, -1));
                  ...
                  $description = 'oldpath:' . $oldfilepath . ' || ' . 'file path: ' . $file_path . '/' . $photo_file . '';
                  $this->KYC_Log_model->create_log('member Employment proof  not found ', $this->session->userdata('kyc_id'), 0, $regnumber, $description);
                  //echo '<br>Employment proof  not found';
                  // $this->KYC_Log_model->create_log('member idproof  not found',$this->session->userdata('id'),0, $reg_no, 'idproof');
                }
              }
              // ------- check for BCBF employment proof added by pooja ends 2024-10-17-----------#
          ...
          if ($data[0]['createdon'] >= '2022-04-01') {
            if ($data[0]['date_of_commenc_bc'] != '' && $name_checkbox == '1' && $dob_checkbox == '1' && $photo_checkbox == '1' && $sign_checkbox == '1' && $idprf_checkbox == '1' && $bcempidprf_checkbox == '1' && $declaration_checkbox == '1') {
              $this->session->set_flashdata('success', 'KYC recommended for ' . $data[0]['regnumber'] . '(Previous record) sent to approver !!');
              $select = 'namesub,firstname,middlename,lastname,dateofbirth,associatedinstitute,scannedphoto,scannedsignaturephoto,idproofphoto,declaration,empidproofphoto,excode,bank_bc_id_card,date_of_commenc_bc';
              $old_data = $this->master_model->getRecords("member_registration", array(
                'regnumber' => $regnumber,
              ...
              $description = serialize($log_desc);
              $this->KYC_Log_model->create_log('Member recommend', $this->session->userdata('kyc_id'), $last_insterid, $regnumber, $description);
            } elseif ($data[0]['date_of_commenc_bc'] == '' && $name_checkbox == '1' && $dob_checkbox == '1' && $emp_checkbox == '1' && $photo_checkbox == '1' && $sign_checkbox == '1' && $idprf_checkbox == '1' && $declaration_checkbox == '1') {
              $this->session->set_flashdata('success', 'KYC recommended for' . $data[0]['regnumber'] . '(Previous record) sent to approver !!');
            ...
            if ($data[0]['date_of_commenc_bc'] != '' && $name_checkbox == '1' && $dob_checkbox == '1' && $photo_checkbox == '1' && $sign_checkbox == '1' && $idprf_checkbox == '1' && $bcempidprf_checkbox == '1') {
              $this->session->set_flashdata('success', 'KYC recommended for ' . $data[0]['regnumber'] . '(Previous record) sent to approver !!');
              ...
              $description = serialize($log_desc);
              $this->KYC_Log_model->create_log('Member recommend', $this->session->userdata('kyc_id'), $last_insterid, $regnumber, $description);
            } elseif ($name_checkbox == '1' && $dob_checkbox == '1' && $emp_checkbox == '1' && $photo_checkbox == '1' && $sign_checkbox == '1' && $idprf_checkbox == '1') {
              $this->session->set_flashdata('success', 'KYC recommended for' . $data[0]['regnumber'] . '(Previous record) sent to approver !!');
      ...
      if ($regnumber) {
        $select = 'regnumber,namesub,firstname,middlename,lastname,DATE_FORMAT(dateofbirth,"%d-%m-%Y") dateofbirth,registrationtype,associatedinstitute,scannedphoto,scannedsignaturephoto,idproofphoto,declaration,empidproofphoto,excode,bank_bc_id_card,date_of_commenc_bc,registrationtype,email,createdon';
  ...
  public function pending_member($regnumber = NULL, $srno = NULL, $totalRecCount = NULL)
  {
    if ($regnumber) {
      ...
      $date = date("Y-m-d H:i:s");
      if (isset($_POST['btnSubmit'])) {
        $select = 'regnumber,registrationtype,email,createdon,excode,bank_bc_id_card,date_of_commenc_bc';
        ...
        $select = 'namesub,firstname,middlename,lastname,dateofbirth,associatedinstitute,scannedphoto,scannedsignaturephoto,idproofphoto,declaration,empidproofphoto,excode,bank_bc_id_card,date_of_commenc_bc,kyc_edit';
        $old_user_data = $this->master_model->getRecords('member_registration', array(
          'regnumber' => $regnumber,
          'isactive' => '1'
        ), $select);
        ...
        if ($data[0]['registrationtype'] == 'F' || $data[0]['registrationtype'] == 'A') {
          if ($name_checkbox == '1' && $dob_checkbox == '1' && $emp_checkbox == '1' && $photo_checkbox == '1' && $sign_checkbox == '1' && $idprf_checkbox == '1') {
            ...
            $info_arr = array(
              ...
              //'to'=> "kyciibf@gmail.com",
              'to' => 'iibfdevp@esds.co.in,Je.mss3@iibf.org.in,Je.mss4@iibf.org.in', //'to' => $userdata[0]['email'],
              'from' => $emailerstr[0]['from'],
              'subject' => $emailerstr[0]['subject'],
              'message' => $final_str
            );
  ...
  public function member($regnumber = NULL, $srno = NULL, $totalRecCount = NULL)
  {
    if ($regnumber) {
      ...
      if (isset($_POST['btnSubmit'])) {
        $select = 'regnumber,registrationtype,email,createdon,excode,date_of_commenc_bc'; //excode,date_of_commenc_bc added by pooja mane for Fedai & old bcbf 2024-08-05 & 2024-10-17
        $data = $this->master_model->getRecords('member_registration', array(
          'regnumber' => $regnumber,
          'isactive' => '1'
        ), $select);
        ...
        if ($data[0]['registrationtype'] == 'F' || $data[0]['registrationtype'] == 'A') {
          if ($name_checkbox == '1' && $dob_checkbox == '1' && $emp_checkbox == '1' && $photo_checkbox == '1' && $sign_checkbox == '1' && $idprf_checkbox == '1') {
            ...
            if ($this->Emailsending->mailsend($info_arr)) {
              ...
              // ------- check for employment proof added by pooja mane 2024-08-05-----------#
              ...
              // ------- check for employment proof added by pooja ends 2024-08-05-----------#
          ...
          elseif ($data[0]['registrationtype'] == 'NM' && $data[0]['date_of_commenc_bc'] == '' && in_array($data[0]['excode'], $fedai_array) && $name_checkbox == '1' && $dob_checkbox == '1' && $photo_checkbox == '1' && $sign_checkbox == '1' && $idprf_checkbox == '1' && $empidprf_checkbox == '1' && $declaration_checkbox == '1') {
            $this->session->set_flashdata('success', 'KYC recommended for ' . $data[0]['regnumber'] . '(Previous record) sent to approver !!');
          }
          //condition added by pooja mane for employyment proof for NM fedai members 2024-08-07

          elseif (($data[0]['registrationtype'] == 'NM' || $data[0]['registrationtype'] == 'DB') && $data[0]['date_of_commenc_bc'] == '' && !in_array($data[0]['excode'], $fedai_array) && $name_checkbox == '1' && $dob_checkbox == '1' && $photo_checkbox == '1' && $sign_checkbox == '1' && $idprf_checkbox == '1') {
            $this->session->set_flashdata('success', 'KYC recommended for ' . $data[0]['regnumber'] . '(Previous record) sent to approver !!');
            // ECHO $empidprf_checkbox;DIE;
          } //condition added by pooja mane for employyment proof for NM fedai members end 2024-08-07 
          else {
            // echo 'in else else';die;
            include_once APPPATH . 'third_party/SBI_ePay/CryptAES.php';
            ...
            if ($this->Emailsending->mailsend($info_arr)) {
              ...
              // ---------- check for declaration by pooja mane 2024-08-12-------------#
              ...
              if (in_array('Declaration', $update_data)) {
                ...
                // -------End  check for declaration added by pooja mane on 2024-08-12 -----------#
              }
              // 
          ...
          if ($data[0]['createdon'] >= '2022-04-01') {
            ...
            // echo '<br>'.$name_checkbox;
            // echo $dob_checkbox;
            // echo $photo_checkbox;
            // echo $sign_checkbox;
            // echo $idprf_checkbox;
            // echo $bcempidprf_checkbox;
            // echo $declaration_checkbox;
            // die;
      ...
      if ($regnumber) {
        $select = 'regnumber,namesub,firstname,middlename,lastname,DATE_FORMAT(dateofbirth,"%d-%m-%Y") dateofbirth,registrationtype,associatedinstitute,scannedphoto,scannedsignaturephoto,idproofphoto,declaration,empidproofphoto,excode,bank_bc_id_card,date_of_commenc_bc,registrationtype,email,createdon,date_of_commenc_bc';
        $members = $this->master_model->getRecords("member_registration a", array(
          'regnumber' => $regnumber,
          'isactive' => '1'
        ), $select, "", '0', '1');
  ...
  public function scribe_member($regnumber = NULL, $srno = NULL, $totalRecCount = NULL)
  {
    ...
    if ($regnumber) {
      ...
      if (isset($_POST['btnSubmit'])) {
        ...
        $this->db->distinct('regnumber');
        $this->db->group_by('regnumber');
        $select = 'namesub,firstname,middlename,lastname,vis_imp_cert_img,orth_han_cert_img,cer_palsy_cert_img,email,idproofphoto,declaration,empidproofphoto,excode,bank_bc_id_card_img,date_of_commenc_bc';
        $this->db->group_by('regnumber');
        ...
        // log activity
        // get recommended fields data from member registration -
        $select = 'email,namesub,firstname,middlename,lastname,vis_imp_cert_img,orth_han_cert_img,cer_palsy_cert_img,idproofphoto,declaration,empidproofphoto,excode,bank_bc_id_card_img,email,date_of_commenc_bc';
        $this->db->group_by('regnumber');
        ...
        if (($declaration_checkbox == '1' || $declaration_checkbox == '3')
          ...
          if ($this->Emailsending->mailsend($info_arr)) {
            ...
            // log activity
            // get recommended fields data from member registration 
            $select = 'namesub,firstname,middlename,lastname,vis_imp_cert_img,orth_han_cert_img,cer_palsy_cert_img,idproofphoto,declaration,empidproofphoto,excode,bank_bc_id_card_img,email,date_of_commenc_bc';
            $this->db->group_by('regnumber');
            $old_data = $this->master_model->getRecords("scribe_registration", array(
              'regnumber' => $regnumber,
              //'scribe_uid' => $scribe_uid,
