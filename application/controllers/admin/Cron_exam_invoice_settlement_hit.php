...
class Cron_exam_invoice_settlement_hit  extends CI_Controller {
	...
	*/
	public function index()
	{
		...
		if(!empty($result))
 		{
 			...
 			{	
				...
				///print_r($arr_member_exam); echo "<br>";
				//exit;
				...
				if(!empty($arr_exam_invoice) && !empty($arr_member_exam) && $arr_member_exam['pay_status'] == 1 && $arr_exam_invoice['is_new_record']==0) 
 				{
					$is_insert_in_settlement = 1; 
 					$str_reason .= " pay_status = 1 from member_exam tbl";
 				}
		...
		else
		{
			...
			$this->master_model->updateRecord('cron_limit',$arr_update,array('created_at' => $current_date,'module_type'=>'exam_invoice'));
 	...
 	public function settle_exam_invoice()
 	{
		...
		$isExists = $this->master_model->getRecords('exam_invoice_settlement_bk');
 	...
 	public function exam_settlement($res,$arr_member_exam,$is_new_record,$str_reason='')
 	{
		...
		$isExists = $this->master_model->getRecords('exam_invoice_settlement_bk',$where);
		
		if(count($isExists) == 0)
		{
				/* INSERT into exam invoice settlement tbl */
				if($this->master_model->insertRecord('exam_invoice_settlement_bk',$insert_data))
				{
					...
					if(!empty($isExistsCancelation) && count($isExistsCancelation) > 0)
					{
						...
						$this->master_model->updateRecord('exam_invoice_settlement_bk',$arr_update,$update_where);
						/* update the cancelation status */
 	...
 	public function check_exam_invoice($receipt_no)
 	{
		...
		*/
		...
		if(count($is_exists_exam_invoice) > 0 && !empty($is_exists_exam_invoice) )
		{
			...
			if(($is_exists_exam_invoice[0]['transaction_no'] != '' ) && ($is_exists_exam_invoice[0]['invoice_no'] != '') && 		($is_exists_exam_invoice[0]['invoice_image'] != '')	
				)
			{
				echo 'in';exit;
				$is_new_record = 2;
 	...
 	public function update_exam_settlement($id,$arr_update)
 	{
 		...
 		{
 			$this->master_model->updateRecord('exam_invoice_settlement_bk',$arr_update,array('id'=>$id));
 		}
 	}
