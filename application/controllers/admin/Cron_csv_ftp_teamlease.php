<?php
/*
 * Controller Name	:	Cron File Transfer : Teamlease Vendor
 * Created By		:	Vishal
 * Created Date		:	30 DEC 2022
 *
*/
defined('BASEPATH') OR exit('No direct script access allowed');
class Cron_csv_ftp_teamlease extends CI_Controller {
	...
	public function index($current_date = "")
	{
        ...
        $array = $this->master_model->getRecords('cron_csv_teamlease','','new_file_name,record_count');
		...
		$local_server_dir_path = '/home/supp0rttest/public_html/uploads/cronCSV_teamlease/'.$current_date.'/';
	...
	public function rpe_teamlease($current_date = "")
	{
		...
		$CurrentDate = date('Y-m-d',strtotime($current_date));
		$this->db->where('CurrentDate', $CurrentDate);
        $array = $this->master_model->getRecords('cron_rpe_elearning_teamlease','','new_file_name,record_count');
		$new_file_name = $array[0]['new_file_name'];
		$record_count = $array[0]['record_count'];
		...
		// file wise remote server path -
		$path_list = array("/teamlease/elearning/");
			//$path_list = array("/public_html/uploads/Save_CSV/");
		...
		$local_server_dir_path = '/home/supp0rttest/public_html/uploads/cronCSV_rpe_elearning_teamlease/'.$current_date.'/';
	...
	public function sendmail($current_date = "", $msg = "")
	{
		...
		$subject = 'Member Exam Applications CSV File Transfer To Teamlease Vendor';
	  ...
	  //           'iibfdevp@esds.co.in',
	  //           'lmssupport@shezartech.com',
	  //           'vahitha@iibf.org.in'
	  //       );
		
		
		$recipient_list = array('vishal.phadol@esds.co.in');
		
		$attachment_filename = 'logs_'.$current_date.'.txt';
		$attachment_path = './uploads/cronCSV_teamlease/'.$current_date.'/' . $attachment_filename;
