<?php
	...
	*/
	...
	class Cron_csv_custom extends CI_Controller {
		public $UserID; 
					...
					foreach($can_exam_data as $exam)
					{
						...
						{	
							foreach($admit_card_details_arr as $admit_card_data)
							{
								...
								if($exam_code == $this->config->item('examCodeJaiib') || $exam_code == '62' || $exam_code == $this->config->item('examCodeCaiibElective63') || $exam_code == '64' || $exam_code == '65' || $exam_code == '66' || $exam_code == '67' || $exam_code == '68' || $exam_code == '69' || $exam_code == $this->config->item('examCodeCaiibElective70') || $exam_code == $this->config->item('examCodeCaiibElective71') || $exam_code == '72' || $exam_code == $this->config->item('examCodeDBF') || $exam_code == '58' || $exam_code == '580' || $exam_code == '81' || $exam_code == '5800' || $exam_code == '810' || $exam_code == '58000' || $exam_code == $this->config->item('examCodeSOB'))
								{
									$subject_code = '';
									$subject_description = '';
								}
    ...
    public function exam_csv_csc()
    {
			...
			if (file_exists($cron_file_dir . $current_date)) {
				...
				'isdeleted' => 0,
				'pay_status' => 1,
				'DATE(d.created_on)' => $yesterday
				), $select);
				//'bankcode' => 'csc',
				//'DATE(d.exam_date)' => '2022-08-22'
				//echo $this->db->last_query();
				if (count($can_exam_data)) {
					$i             = 1;
					$exam_cnt      = 0;
    ...
    public function SIFFY_MSME()
    {
			...
			$error          = array();
			$start_time     = date("Y-m-d H:i:s");
			$current_date = '20220427'; //date("Ymd");
	...
	public function exam_csv_teamlease_RPE()
    {
		...
		if (file_exists($cron_file_dir . $current_date)) {
			...
			$exam_code = array('1002'); // Send Free and Paid Both Applications
			...
			if (count($can_exam_data)) {
				...
				foreach ($can_exam_data as $exam) 
				{
					...
					$exam_arr         = array('1002' => 'Anti-Money Laundering And Know Your Customer');
    ...
    { 	     
		    ...
		    $yesterday = '2023-01-23'; 
				...
				'm.isactive' => '1',
				'm.isdeleted' => 0,
				// 'a.exam_date' => '2022-12-27',
				'DATE(a.modified_on)' => $yesterday
				), $select);
				//  echo $this->db->last_query(); 
				// echo count($can_exam_data);
				// exit;
				if (count($can_exam_data)) 
				{
					$i             = 1;
    ...
    { 	      
			...
			$error          = array();
			$start_time     = date("Y-m-d H:i:s");
			$current_date = '20220424'; //date("Ymd");
			//$current_date   = '20200924';
		...
		public function jaiib_csv_kesdee()
		{
			...
			if(file_exists($cron_file_dir.$current_date))
			{
				...
				$this->db->join('member_registration c','a.regnumber=c.regnumber','LEFT'); 
				$this->db->where_in('a.exam_code', $exam_code);
				$can_exam_data = $this->Master_model->getRecords('member_exam a',array('pay_type'=>2,'status'=>1,'isactive'=>'1','isdeleted'=>0,'pay_status'=>1,'sub_el_count >' => 0),$select);
				//'DATE(a.created_on)'=>$yesterday,
			//	echo ">>".$this->db->last_query();
			//	exit;
				if(count($can_exam_data))
					...
					foreach($can_exam_data as $exam)
					{
						...
						{	
							foreach($admit_card_details_arr as $admit_card_data)
							{
								...
								if($exam_code == '62' || $exam_code == $this->config->item('examCodeCaiibElective63') || $exam_code == '64' || $exam_code == '65' || $exam_code == '66' || $exam_code == '67' || $exam_code == '68' || $exam_code == '69' || $exam_code == $this->config->item('examCodeCaiibElective70') || $exam_code == $this->config->item('examCodeCaiibElective71') || $exam_code == '72'){
									$subject_code = '';
									$subject_description = '';
								}
		...
		{ 
			...
			if(file_exists($cron_file_dir.$current_date))
			{
				...
				$this->db->where_in('a.exam_code', $exam_code);
				$can_exam_data = $this->Master_model->getRecords('member_exam a',array('pay_type'=>2,'status'=>1,'isactive'=>'1','isdeleted'=>0,'pay_status'=>1,
				//'DATE(a.created_on)'=>$yesterday, 
				'a.elearning_flag'=>'Y'),$select);
				...
				if(count($can_exam_data))
				{
					...
					$exam_cnt = 0;
					// Column headers			
					$data1 = "First Name,Middle name,Last Name,Mem. Number,Date of Birth,Gender,Email ID,Mobile,Address,State,Pin Code,Country,Profession,Organization,Designation,Exam Code,Course,Elective Sub Code,Elective Sub Desc,Attempt,E-learning Subject Code \n";
					$exam_file_flg = fwrite($fp, $data1);
					...
					foreach($can_exam_data as $exam)
					{
						...
						{	
							foreach($admit_card_details_arr as $admit_card_data)
							{
								...
								//First Name|Middle name|Last Name|Mem. Number|Date of Birth|Gender|Email ID|Mobile|Address|State|Pin Code|Country|Profession	|Organization|Designation|Exam Code|Course|Elective Sub Code|Elective Sub Desc|Attempt
								$data = ''.$firstname.','.$middlename.','.$lastname.','.$exam['regnumber'].','.$dateofbirth.','.$gender.','.$exam['email'].','.$mobile.','.$address.','.$exam['state'].','.$pincode.','.'INDIA'.','.''.','.$institution_name.','.$designation_name.','.$exam_code.','.$exam_name.','.$subject_code.','.$subject_description.','.''.','.$elearning_sub_code."\n";
    ...
    public function exam_csv_csc_old()
    {
		...
		public function exam_csv_shezartech()
		{
				...
				if (file_exists($cron_file_dir . $current_date)) {
					...
					$yesterday = date('Y-m-d', strtotime("- 1 day", strtotime($current_date)));
					$yesterday = '2022-12-11';
					$exam_code = array('529');
					//$member = array(802091415,802091426,802091418,802091416);
					...
					'isactive' => '1',
					'isdeleted' => 0,
					'pay_status' => 1,'DATE(a.created_on)' => $yesterday
		...
		public function exam_csv_teamlease()
		{
              ...
              '530' => 'Certificate Course in Climate Risk & Sustainable Finance (Foundation) E-Learning Mode',
		...
		public function exam_csv_teamlease_temp()
		{
				ini_set("memory_limit", "-1");
				$dir_flg        = 0;
				$parent_dir_flg = 0;
				...
				if (file_exists($cron_file_dir . $current_date)) {
					...
					$exam_code = array('528', '529', '530', '531', '534','1005','1003','77','11','1011','1006','33');
					$member_no = array(802067633);
					$select    = 'DISTINCT(b.transaction_no),a.exam_code,c.firstname,c.middlename,c.lastname,a.regnumber,c.dateofbirth,c.gender,c.email,c.stdcode,c.office_phone,c.mobile,c.address1,c.address2,c.address3,c.address4,c.district,c.city,c.state,c.pincode,c.associatedinstitute,c.designation,a.exam_period,a.elected_sub_code,c.editedon,a.examination_date,a.id AS mem_exam_id,a.created_on AS registration_date';
					$this->db->join('payment_transaction b', 'b.ref_id = a.id', 'LEFT');
					$this->db->join('member_registration c', 'a.regnumber=c.regnumber', 'LEFT');
