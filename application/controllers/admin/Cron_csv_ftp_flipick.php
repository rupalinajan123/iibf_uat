<?php
/*
 * Controller Name	:	Cron File Transfer : Flipick Vendor
 * Created By		:	Bhushan
 * Created Date		:	08 April 2019
 *
*/
defined('BASEPATH') OR exit('No direct script access allowed');
class Cron_csv_ftp_flipick extends CI_Controller {
	...
	public function index($current_date = "")
	{
		...
		$array = $this->master_model->getRecords('cron_csv_flipick','','new_file_name,record_count');
		...
		$local_server_dir_path = '/home/supp0rttest/public_html/uploads/cronCSV_flipick/'.$current_date.'/';
	...
	public function sendmail($current_date = "", $msg = "")
	{
		...
		$subject = 'Member Exam Applications CSV File Transfer To Flipick Vendor';
		...
		$recipient_list = array(
            ...
            'iibfdevp@esds.co.in',
			'iibfsupport@flipick.com',
			'vahitha@iibf.org.in'
        ); 
				
			/* $recipient_list = array('sagar.matale@esds.co.in','khambekar.rucha@gmail.com','iibfsupport@flipick.com'); */
		
		/* ,'suhas@iibf.org.in' */
/*amol.chandsarkar@flipick.com
vishal.marathe@flipick.com
ratish.ramachandran@flipick.com*/

		
		//$recipient_list = array('bhushan.amrutkar@esds.co.in');
		
		$attachment_filename = 'logs_'.$current_date.'.txt';
		$attachment_path = './uploads/cronCSV_flipick/'.$current_date.'/' . $attachment_filename;
	...
	public function bcbf_nar_csv($current_date = "")
	{
		$start_time = date("Y-m-d H:i:s");
		$success = array();
		$error = array();
		...
		$CurrentDate = date('Y-m-d',strtotime($current_date));
		$this->db->where('CurrentDate', $CurrentDate);
        $array = $this->master_model->getRecords('cron_bsbf_nar_flipick','','new_file_name,record_count');
		$new_file_name = $array[0]['new_file_name'];
		$record_count = $array[0]['record_count'];
		...
		// file wise remote server path -
		$path_list = array("/BCBF-NAR-Flipick/");
			//$path_list = array("/public_html/uploads/Save_CSV/");
		...
		$local_server_dir_path = '/home/supp0rttest/public_html/uploads/BCBF-NAR-Flipick/'.$current_date.'/';
		...
		// send email notification -
		$result = $this->sendmail_bcbf_nar_csv($current_date, $msg);
	}
	
	public function sendmail_bcbf_nar_csv($current_date = "", $msg = "")
	{
		if($current_date == "")
		{
			$current_date = date("Ymd");	
		...
		//$from_email = 'noreply@iibf.org.in';
		$from_email = 'logs@iibf.esdsconnect.com';
		$subject = 'BCBF NAR CSV File Transfer To Flipick Vendor';
	
		$recipient_list = array(
            'iibfsupport@flipick.com','sgbhatia@iibf.org.in','amit@iibf.org.in','dattatreya@iibf.org.in','iibfdevp@esds.co.in','dd.exm1@iibf.org.in'
        ); 
		//     // sneha.salvi@flipick.com
		
		$attachment_filename = 'logs_'.$current_date.'.txt';
		$attachment_path = './uploads/BCBF-NAR-Flipick/'.$current_date.'/' . $attachment_filename;
