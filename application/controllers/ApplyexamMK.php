...
class ApplyexamMK extends CI_Controller {
	...
	{	
		...
		{
			redirect(base_url().'ApplyexamMK/accessdenied/');
		}*/
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				{ 
					 if($user_info[0]['isactive']==1)
					 {
						...
						redirect(base_url().'ApplyexamMK/examdetails/?ExId='.$this->input->get('ExId').'&Extype='.$Extype);
						 }
	...
	{		
			...
			if($this->session->userdata('mregnumber_applyexam')!='510226859' && $this->session->userdata('mregnumber_applyexam')!='510067565')
			{
					echo 'in123';exit;
					redirect(base_url().'ApplyexamMK/accessdenied/');
			}
			...
			//{
			 ...
			 if($is_transaction_doone >0)
			 {
				...
				else if($cookieflag==1 && $profile_flag==1)
				{
					...
					$data=array('middle_content'=>'memapplyexam/cms_pageMK','exam_info'=>$exam_info);
					$this->load->view('memapplyexam/mem_apply_exam_common_viewMK',$data);
				}
	...
	public function comApplication()
	{
		...
		if(isset($_POST['btnPreviewSubmit']))  	
		{
			...
			if($this->session->userdata('examcode')==$this->config->item('examCodeCaiib'))
			{
				...
				if($this->session->userdata('mregnumber_applyexam') == '510226859'){
					$fee_tmp = '1298';
					$center_tmp = '322';
					$state_tmp = 'MAH';
					$pincode_place_of_work_tmp = '415004';
					$grp_code_tmp = 'B1_1';
				}elseif($this->session->userdata('mregnumber_applyexam') == '510067565'){
					$fee_tmp = '1416';
					$center_tmp = '87' ;
					$state_tmp = 'JHA';
					$pincode_place_of_work_tmp = '814112';
					$grp_code_tmp = 'S1';
				}	
					...
					$user_data=array('email'=>$_POST["email"],	
												...
												'optmode'=>'ON',
												'extype'=>2,
												'exname'=>'CERTIFICATE EXAMINATION IN MICROFINANCE - JAN 2020',
												'excd'=>79,
												'eprid'=>910,
												'fee'=>$fee_tmp,
					...
					redirect(base_url().'ApplyexamMK/Msuccess');
				}
				else
				{
		...
		if($this->session->userdata('examcode')=='')
		{
			redirect(base_url().'ApplyexamMK/examlist/');	
		}
		...
		{		
			...
			foreach($subject_arr as $k=>$v)
			{
				...
				if($msg!='')
				{
					...
					redirect(base_url().'ApplyexamMK/comApplication');
				}
			}
		}
		if($sub_flag==0)
   ...
   public function checkpin()
	...
	public function Msuccess()
	{
		...
		if(($this->session->userdata('examinfo')==''))
		{
			...
			if($inser_id=$this->master_model->insertRecord('member_exam',$inser_array,true))
			{
				...
				{
					redirect(base_url().'ApplyexamMK/sbi_make_payment/');
				}
				else
				{
					redirect(base_url().'ApplyexamMK/make_payment/');
				}
			}
	...
	public function make_payment()
	{
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			$pg_return_url = base_url()."ApplyexamMK/pg_response";
...
requestparameter=IIBF|2138759|NA|500.00|NA|NA|NA|INR|NA|R|iibf|NA|NA|F|iibfexam|500081141|148201701|NA|NA|NA|NA|http://abc.somedomain.com|2387462372
			...
			if ($res_checksum == $checksum)
			{
				if($pg_res[16] == "iibfexam")
				{
					...
					$exam_info=$this->master_model->getRecords('member_exam',array('regnumber'=>$this->session->userdata('mregnumber_applyexam'),'member_exam.id'=>$get_user_regnum[0]['ref_id']),'member_exam.exam_mode,member_exam.exam_medium,member_exam.exam_period,spl_center_master.center_name,member_exam.exam_center_code,exam_master.description,spl_misc_master.exam_month');
						...
						{
							redirect(base_url().'ApplyexamMK/details/'.base64_encode($MerchantOrderNo).'/'.$this->session->userdata['examinfo']['excd']);
						}
						}
						else if($payment_status==0)
						{
							...
							if($this->Emailsending->mailsend($info_arr))
							{
								redirect(base_url().'ApplyexamMK/fail/'.base64_encode($MerchantOrderNo));
							}
	...
	public function sbi_make_payment()
	{
		...
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			$pg_success_url = base_url()."ApplyexamMK/sbitranssuccess";
			$pg_fail_url    = base_url()."ApplyexamMK/sbitransfail";
		...
		//	{
			...
			$insert_data = array(
				...
				'pay_type'         => '2',
				'ref_id'           => $this->session->userdata['examinfo']['insdet_id'],
				'description'      => ' CERTIFICATE EXAMINATION IN MICROFINANCE - JAN 2020',
				'status'           => '2',
				'exam_code'    	   => 79,
			...
			if($this->session->userdata('mregnumber_applyexam') == '510226859'){
					...
					$grp_code_tmp = 'B1_1';
					$amount=1298;
					$center_name_tmp = 'PUNE';
					$state_name_tmp = 'MAHARASHTRA';
					$base_amount = '1100';
					$state_code_tmp = '27';
				}elseif($this->session->userdata('mregnumber_applyexam') == '510067565'){
					$fee_tmp = '1416';
					...
					$grp_code_tmp = 'S1';
					$amount=1416;
					$center_name_tmp = 'RANCHI';
					$state_name_tmp = 'JHARKHAND';
					$base_amount = '1200';
					$state_code_tmp = '20';   
				}
			...
			$log_title = "Exam invoice data from applyexamMK cntrlr inser_id = '".$inser_id."'";
...
=1000003|DOM|IN|INR|2|Other|https://test.sbiepay.coom/secure/fail.jsp|SBIEPAY|2|2|NB|ONLINE|ONLINE
				...
				/*if($exam_info[0]['exam_code']!=101)
				{
					...
					foreach($exam_admicard_details as $row)
					{
						...
						if($capacity==0)
						{
							...
							storedUserActivity($log_title, $log_message, $rId, $regNo);
							redirect(base_url().'ApplyexamMK/refund/'.base64_encode($MerchantOrderNo));
						}
					}
				}
				if(count($exam_admicard_details) > 0)
	...
	///End of SBICALL Back	
		...
		$user_data=array(	'regid'=>$this->session->userdata('mregid_applyexam'),
										...
										'password'=>$this->session->userdata('mpassword_applyexam'));
		$this->session->set_userdata($user_data);		
		redirect(base_url().'ApplyexamMK/details/'.base64_encode($MerchantOrderNo).'/'.base64_encode($exam_info[0]['exam_code']));
