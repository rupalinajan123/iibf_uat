<?php
  ...
  {    
    ...
    */
    ...
    public function pdc_dashboard_old(){
      $this->load->view('pdc_dashboard');	
    }
    
    public function pdc_adviceresult_old(){
      try{ 
        if($this->session->userdata('result_mem_no')=='')
        {
          redirect(base_url().'marksheet/advice');
        }
        $member_id = $this->session->userdata('result_mem_no');
        $exam_code = $this->session->userdata('result_examcode');
        $exam_period = $this->session->userdata('result_period');
    ...
    public function dipcertresult(){
      
      $exam_period = base64_decode($this->uri->segment(3));
      // echo $exam_period; exit;
      $exam_code = $this->input->post('exam');
      ...
      if(isset($_POST['submit'])){ 
        ...
        if ($this->form_validation->run() == TRUE){
          ...
          if(count($user_info1) > 0){ 
            ...
            $this->db->where('exam_code', $exam_code);
            $debarred_member_chk = $this->master_model->getRecords('result_debarred_member', array('exam_period' => $exam_period, 'member_number' => $this->input->post('Username')));

            if(isset($debarred_member_chk) && count($debarred_member_chk) > 0)
            {
              $data['error']='<span style="">You Are Debarred For this Exam.</span>';
            } 
            else
            {
              redirect(base_url().'marksheet/result_feedback');
    ...
    public function spel_dashboard(){
      $this->load->view('spel_dashboard');	
    }
    ...
    public function dipcert_adviceresult(){
      try{ 
        ...
        $this->db->join('result_exam', 'result_exam'.'.exam_code = exam_master.exam_code');
        $result=$this->master_model->getRecords('exam_master',array('exam_master.exam_code'=>$exam_code,'exam_period'=>$this->session->userdata('result_period')),'exam_id, description,result_date,exam_conduct,exam_name_full');
        $exam_name = $result[0]['exam_name_full']; // get exam name
        ...
        if($exam_period == 777){
          $this->db->order_by('dipcert_mark.id','desc');
          $this->db->limit(1);
        }
        
        $record=$this->master_model->getRecords($marktable,'','marks,status,subject_name,result_date');
        
        
        
        if($record[0]['result_date'] == ''){
          $result_date = $result[0]['result_date'];
          }else{
          $result_date = date('d-M-Y',strtotime($record[0]['result_date']));
        }
    ...
    public function spel_adviceresult(){
      try{ 
        ...
        $this->db->select('member_number, firstname, middlename, lastname, address1, address2, address3, address4, address5, address6, pincode,no_of_attempt');
        $this->db->from('spel_el_member');
        if(is_numeric($member_id)){
          $this->db->where('member_number' , $member_id, false);
          }else{
          $this->db->where('member_number' , $member_id,'', false);
        }
        $this->db->where('spel_el_member.exam_period' , $exam_period, false);
        $this->db->where('exam_code' , $exam_code);
        $record = $this->db->get();
        $user_info = $record->row(); // get all member detail
        ...
        $this->db->where('spel_el_marks.exam_period',$exam_period);
        $this->db->where('result_subject.exam_code',$exam_code);
        $this->db->where('result_subject.exam_period',$this->session->userdata('result_period'));
        
        if($exam_period == 777){
          $this->db->order_by('spel_el_marks.id','desc');
          $this->db->limit(1);
        }
        ...
        $data = array("user_info"=>$user_info, "exam_name" => $exam_name,"result_date"=>$result_date,"record"=>$record,"printrecord"=>$record,"exam_code"=>$exam_code,"exam_conduct"=>$exam_conduct,"exam_period"=>$exam_period);
        $this->load->view('spel_adviceresult',$data);
    ...
    public function spel_adviceresult_old(){
      try{ 
    ...
    public function spel_result(){
      ...
      if(isset($_POST['submit'])){ 
        ...
        if ($this->form_validation->run() == TRUE){
          ...
          $this->db->from('spel_el_member');
          //$exam_code=explode('_',$this->input->post('exam'));
          // $this->db->join('member_registration', 'member_registration'.'.regnumber = spel_el_member.member_number');
          //$this->db->where(array('exam_code' => $exam_code,"member_number"=>$this->input->post('Username'),'usrpassword'=>$encpass));
          $this->db->where('spel_el_member.exam_period',$exam_period);
          $this->db->where(array('exam_code' => $exam_code,"member_number"=>$this->input->post('Username')));
      ...
      $this->load->view('spel_result_login',$data);
    }
    
    public function dipcert_adviceresulttest(){
      try{ 
    ...
    public function dipcert_consolidated(){
      if ($_SERVER["SERVER_PORT"] != 443) {
        ...
        $redir = "Location: https://" . $_SERVER['HTTP_HOST'] . $_SERVER['REDIRECT_SCRIPT_URL'];
        //echo $redir;exit;
        header($redir);
        exit();
      }
      ...
      try{
        ...
        $member_id = $this->session->userdata('result_mem_no');
        $exam_code = $this->session->userdata('result_examcode');
        $exam_p = trim(preg_replace('/\s\s+/', ' ', $this->session->userdata('result_period')));
        
        if($this->db->table_exists('CONS_MRK_DIPCERT')){ 
          ...
          $exam = $this->db->get_where('result_exam', array('exam_code' => $exam_code)); // ,"exam_period"=>$exam_p
          ...
          $this->db->where('exam_code',$this->session->userdata('result_examcode'));
          $this->db->from('CONS_MRK_DIPCERT');
    ...
    /*public function dipcert_consolidated(){
      ...
      $this->db->select('exam_name_full');
      $exam = $this->db->get_where('result_exam', array('exam_code' => $exam_code));
      $exam_result = $exam->row();
      $exam_name = $exam_result->exam_name_full;
      ...
      $data = array("user_info"=>$user_info,"exam_name" => $exam_name,"result"=>$result,"exam_hold_on"=>$exam_hold_on,"printresult"=>$result,"exam_result_date"=>$exam_result_date,"is_conso"=>$is_conso,"exam_code"=>$exam_code,"datearr"=>$datearr,"exam_period"=>$exam_p,"exam_p"=>$exam_p,'exam_conduct'=>$exam_conduct);
    ...
    public function naarresult(){
      ...
      $this->load->view('naarlogin',$data);
    }
    
    public function naarresult_dashboard(){
      $this->load->view('naar_dashboard_new.php');	
    }
    
    public function naarresult_adviceresult(){
      try{ 
        ...
        //$this->db->group_by('dipcert_mark.subject_id');
        if($exam_period == 777){
          $this->db->order_by('dipcert_mark.id','desc');
          $this->db->limit(1);
        } 
    ...
    public function jaiibresult(){
		try{
          ...
          //Added on 31-07-2025 by Rupali...

					if (
    $exam_code == 210 &&
    $exam_period == 125 &&
    in_array($meminfo[0]['regnumber'], [
        510546167, 510537107, 510607331, 510530716, 510325232, 510637461,
        510316191, 510639510, 510634252, 510533738, 510438239, 510058360,
        510553865, 510550101, 510408248, 510550013, 510584612, 510615345,
        510612816, 510544190, 510534085, 510532341, 510349400, 510650280,
        510650733, 510611668, 510651278, 510401452, 510479641, 510161401,
        510652912, 510540943, 510348600
    ])
) {
    echo "Your result is under review";
    exit;
}
...
//end
					...
					if(count($meminfo) > 0){ 
            			...
            			//$this->db->where(array('exam_code' => $exam_code,"member_number"=>$this->input->post('Username'),'exam_period'=>$exam_period),'',false);
						$this->db->where('exam_code', $exam_code);
						$this->db->where('member_number', $this->input->post('Username'));
						...
						if(count($user_info1) > 0){  
							...
							// START : code added by pooja mane to display message to debar members - 2025-01-07*/
							$this->db->where('exam_code', $exam_code);
		            		$debarred_member_chk = $this->master_model->getRecords('result_debarred_member', array('exam_period' => $exam_period, 'member_number' => $this->input->post('Username')));
							...
							/* END : code added by pooja mane to display message to debar members - 2025-01-07*/
						}
					}else
					{
						...
						// START : code added by pooja mane to display message to debar members */
						$this->db->where('exam_code', $exam_code);
						$debarred_member_chk = $this->master_model->getRecords('result_debarred_member', array('exam_period' => $exam_period, 'member_number' => $this->input->post('Username')));
          //  print_r($debarred_member_chk);exit;
						if(isset($debarred_member_chk) && count($debarred_member_chk) > 0)
						{
							$data['error']='<span style="">You Are Debarred For this Exam.</span>';
						} 
						...
						/* END : code added by pooja mane to display message to debar members */ 
    ...
    public function jaiibdashboard(){
      ...
      $data = array('is_conso'=>$is_conso);
      $this->load->view('jaiibdashboard',$data);
    }
    ...
    public function jaiibadvice(){
      try{
        ...
        $this->db->join('result_exam', 'result_exam'.'.exam_code = exam_master.exam_code');
        $result=$this->master_model->getRecords('exam_master',array('exam_master.exam_code'=>$exam_code,'exam_period'=>$exam_period),'description,result_date,exam_conduct,exam_name_full');
        $exam_name = $result[0]['exam_name_full'];
        ...
        $this->db->where('jaiib_marks.exam_id',$exam_code);
        $this->db->where('result_subject.exam_code',$exam_code);
        $this->db->where('result_subject.exam_period',$this->session->userdata('result_period'));
        $this->db->group_by('jaiib_marks.subject_id');
        $record=$this->master_model->getRecords($marktable,'','marks,status,subject_name');
        //echo $this->db->last_query();
        //exit;
    ...
    public function bcbf($enc_exam_period='')
    {
      try
      {        
        //$exam_period = base64_decode($this->uri->segment(3));
        $exam_period = base64_decode($enc_exam_period);
        $exam_code = $this->input->post('exam');
          ...
          {            
            ...
            { 
              ...
              { 
                ...
                //echo '<pre>'; print_r($result__data); exit;
                $this->session->set_userdata($result__data);
                redirect(base_url().'marksheet/result_feedback');
              } 
            } 
            
            // START : code added by anil s to display message to debar members - 2025-02-25 */
              $this->db->where('exam_code', $exam_code);
              $debarred_member_chk = $this->master_model->getRecords('result_debarred_member', array('exam_period' => $exam_period, 'member_number' => $this->input->post('Username')));
            ...
            /* END : code added by anil s to display message to debar members - 2025-02-25*/
    ...
    //23-04-2024 by nikita
    
    public function canara(){
      try{
        ...
        $exam_result = $exam->result();
        $data['exam']=$exam_result;
        $this->load->view('bcbf_result_canara_login',$data);
    ...
    public function can_fin_home(){
      try{
        ...
        $exam_result = $exam->result();
        $data['exam']=$exam_result;
        $this->load->view('bcbf_result_can_fin_home_login',$data);
    ...
    public function bcbf_advice_result()
    {
      ...
      {        
        ...
        $this->db->join('result_exam', 'result_exam'.'.exam_code = exam_master.exam_code');
        $result=$this->master_model->getRecords('exam_master',array('exam_master.exam_code'=>$exam_code,'exam_period'=>trim($this->session->userdata('result_period'),"\n")),'description,result_date,exam_conduct');
        // echo $this->db->last_query();die;
        $exam_name = $result[0]['description'];
        $result_date = $result[0]['result_date'];
        $exam_conduct = $result[0]['exam_conduct'];
        ...
        $this->db->where('bcbf_marks.exam_period',trim($exam_period,"\n"));
        $this->db->where('result_subject.exam_code',$exam_code);
        $this->db->where('result_subject.exam_period',trim($exam_period,"\n"));
        $record=$this->master_model->getRecords($marktable,'','marks,status,subject_name');
        // $record = str_replace("\n", "", $record);
        // $record = trim($record);
        // echo $this->db->last_query();
    ...
    // common function for CAIIB result
    ...
    public function caiibresultdemo(){
      try{
        $flag=0;
        $data=array();
        $data['error']='';
    ...
    public function caiibresulttest(){
      try{ 
        $flag=0;
        $data=array();
        $data['error']='';
    ...
    public function panjab_sindh(){
      try{
        ...
        $exam_result = $exam->result();
        $data['exam']=$exam_result;
        $this->load->view('panjab_sindh_result_login',$data);
