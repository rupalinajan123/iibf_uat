<?php
    ...
    public function validation_check_captcha($str='',$type=0)//$type : 0=>Server side, 1=>Ajax
    {
      ...
      {
        if($type == '1') { $captcha = $this->input->post('code'); }
      ...
      { 
		...
		public function check_captcha_userlogin($code) 
		{
			...
			if($captcha_code == $session_captcha) { return TRUE; } else { $this->form_validation->set_message('check_captcha_userlogin', 'Please enter correct code');	 return FALSE; }
		}		
    ...
    public function validation_check_member_no_forgot_password($str='',$type=0)//$type : 0=>Server side, 1=>Ajax
    {
      ...
      {
        if($type == '1') { $memno = $this->input->post('memno'); }
        else if($type == '0') { $memno = $str; }
      ...
      { 
        ...
        {
          $this->form_validation->set_message('validation_check_member_no_forgot_password','Membership details not found for the provided Membership Number');
          return false;
        }
      }
    }
    ...
    function member_data_forgot_membership_no($email_mobile='',$reg_id='', $regnumber='')
    {
      ...
      {
        if($email_mobile != '') { $this->db->where("(email='".$email_mobile."' OR mobile='".$email_mobile."')");	}
        else if($reg_id != '' && $regnumber != '') { $this->db->where("regid='".$reg_id."' AND regnumber='".$regnumber."'");	}
    ...
    public function validation_email_mobile_forgot_membership_no($str='',$type=0)//$type : 0=>Server side, 1=>Ajax
    {
      ...
      {
        if($type == '1') { $email_mobile = $this->input->post('email_mobile'); }
        else if($type == '0') { $email_mobile = $str; }
      ...
      { 
        ...
        {
          $this->form_validation->set_message('validation_email_mobile_forgot_membership_no','Membership details not found for the provided Email Id / Mobile Number');
          return false;
        }
      }
    }
    ...
    function fun_send_otp_sms($member_data=array())
    {
      if(count($member_data) > 0)
      {
        ...
        $otp_mail_arr['to'] = 'anil.s@esds.co.in';//$email_id;
    ...
    public function login_with_otp()
    {
      ...
      {       
        $action_flag = '';
        if(isset($_POST['send_otp']) && $_POST['send_otp'] == 'Send OTP') { $action_flag = 'send_otp'; }
        else if(isset($_POST['verify_otp']) && $_POST['verify_otp'] == 'Verify OTP') { $action_flag = 'verify_otp'; }
        ...
        if ($this->form_validation->run() == TRUE)
        {
          ...
          if(count($member_data) > 0)
          {
            if($action_flag == 'send_otp')
            {
              ...
              if($send_otp)
              {
                $this->session->set_flashdata('success','OTP successfully sent on '.$this->mask_email_mobile('mobile', $member_data[0]['mobile']).' & '.$this->mask_email_mobile('email', $member_data[0]['email']).'. OTP is valid for 5 minutes.');
              }
              else
              {
            ...
            {
              $otp_data = $this->master_model->getRecords('member_login_otp',array('regnumber'=>$regnumber, 'is_validate'=>'0'), 'otp_id, otp, is_validate, otp_expired_on', array('otp_id'=>'DESC'), '', 1);
      ...
      if(count($member_data) > 0)
      {
        ...
        $get_otp_record = $this->master_model->getRecords('member_login_otp',array('regnumber'=>$regnumber), 'otp_id, otp_expired_on, created_on', array('otp_id'=>'DESC'), '', 1);
    ...
    public function validation_validate_membership_no($str='',$type=0)//$type : 0=>Server side, 1=>Ajax
    {
      ...
      {
        if($type == '1') { $regnumber = $this->input->post('regnumber'); }
        else if($type == '0') { $regnumber = $str; }
      ...
      { 
        ...
        {
          $this->form_validation->set_message('validation_validate_membership_no','Please enter the valid membership number');
          return false;
        }
      }
    } 
    ...
    public function validation_validate_otp($str='',$type=0)//$type : 0=>Server side, 1=>Ajax
    {
      ...
      {
        if($type == '1') { $input_otp = $this->input->post('input_otp'); }
        else if($type == '0') { $input_otp = $str; }
      ...
      if($type == '1') {/*  echo $return_val_ajax; */ echo json_encode($result); }
      else if($type == '0') 
      { 
