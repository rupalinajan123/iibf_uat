...
class Login extends CI_Controller {
	...
	public function index()
	{
			...
			$this->form_validation->set_rules($config);
				...
				else
				{
					...
					$res=$this->LoginModel->isUserExist($dataarr);
						...
						redirect('admin/blended/BlendedDashboard');	
						}
						else
						{
		...
		// $data['image'] = $this->Captcha_model->generate_captcha_img('adminlogincaptcha');
			$this->load->view('admin/blended_dashboard/admin/login/login',$data);
		}
		else
		{
			redirect(base_url().'blended_login/admin/MainController');
		}
	}
	...
	'rules' => 'trim|required',
		...
		if ($this->form_validation->run() == FALSE){
			$this->load->view('admin/blended_dashboard/login/login',$data);
		}else{
