...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	public function get_table_data_ajax()
	{
		...
		foreach ($Rows as $Res) 
		{
			...
			$row[] = '<a class="btn btn-info btn-xs vbtn" href="'.base_url().'iibfdra/batch/batch_detail/'.base64_encode($Res['id']).'">View '.$cls.'</a><a target="blank_" class="btn btn-xs" href="'.base_url().'iibfdra/batch/candidates_list/'.base64_encode($Res['id']).'">All Candidates</a>';
	...
	public function get_candidates_data_ajax()
	{
		...
		//$batch_id='13688';
		$table = 'dra_members m';
		$column_order = array('m.regid', 'cm.city_name', 'batch_code', 'ab.batch_name', 'm.regnumber', 'REPLACE(CONCAT(TRIM(m.firstname), " ", TRIM(m.middlename), " ", TRIM(m.lastname)), "  "," ") AS DispName', 'm.dateofbirth', 'm.email_id', 'ab.batch_from_date', 'ab.batch_to_date', 'm.hold_release'); //SET COLUMNS FOR SORT 			
		
		$column_search = array('cm.city_name', 'batch_code', 'ab.batch_name', 'm.regnumber', 'REPLACE(CONCAT(TRIM(m.firstname), " ", TRIM(m.middlename), " ", TRIM(m.lastname)), "  "," ")', 'm.dateofbirth', 'm.email_id', 'ab.batch_from_date', 'ab.batch_to_date', 'm.hold_release'); //SET COLUMN FOR SEARCH			
		...
		foreach ($Rows as $Res) 
		{
			...
			$candidate_icon = $this->master_model->getRecords("dra_candidate_logs",array('candidate_id'=>$Res['regid'], 'is_read' => '0'));
      ...
      $cls = '';
			...
			$bell_icon_url = base_url()."iibfdra/batch/editApplicant/".$Res['regid'].'/'.$s_batch_id;
      $action_str = '<a href="'.$bell_icon_url.'"> '.$cls.'</a>';

			$action_str .= '&nbsp; &nbsp; &nbsp; <a  href="'.base_url().'iibfdra/batch/editApplicant/'.$Res['regid'].'/'.$s_batch_id.'" target="blank_">Edit</a>';
			$row[] = $action_str;
			$data[] = $row; 
		}      
