<?php 
	...
	class Dashboard_candidate extends CI_Controller 
	{
      ...
      $this->login_candidate_id = $this->session->userdata('IIBF_DRA_CANDIDATE_LOGIN_ID');
      $this->Iibf_dra_model->check_candidate_session_all_pages(); // If admin session is not started then redirect to logout

      $this->id_proof_file_path = 'uploads/iibfdra';
      $this->qualification_certificate_file_path = 'uploads/iibfdra';
      $this->candidate_photo_path = 'uploads/iibfdra';
      $this->candidate_sign_path = 'uploads/iibfdra';
    }
    ...
    public function index()
		{   
			...
			$data['sub_act_id'] = "";
      $data['page_title'] = 'IIBF - DRA Candidate Dashboard';

      $this->load->view('iibfdra/candidate/dashboard_candidate', $data);
    }
    ...
    public function update_profile()
    {
      ...
      $data['form_data'] = $form_data = $this->master_model->getRecords('dra_members bc', array('bc.regid' => $candidate_id, 'bc.isdeleted' => '0'), "bc.*, acb.batch_from_date, acb.batch_to_date");        
      
      if(count($form_data) == 0) { redirect(site_url('iibfdra/candidate/dashboard_candidate')); }
      
      if($form_data[0]['batch_to_date'] >= date('Y-m-d')) 
      { 
        $this->session->set_flashdata('error','You can update your profile information after ('.date('d M Y', strtotime($form_data[0]['batch_to_date'])).') completing the training.');
      ...
      if (isset($_POST) && count($_POST) > 0)
      {
        ...
        $id_proof_file_req_flg = $candidate_photo_req_flg = $candidate_sign_req_flg = '';
        
        if ($form_data[0]['idproofphoto'] == "") { $id_proof_file_req_flg = 'required|'; }
        if(isset($_POST['id_proof_file_cropper']) && $_POST['id_proof_file_cropper'] != "") { $id_proof_file_req_flg = ''; }

        if ($form_data[0]['scannedphoto'] == "") { $candidate_photo_req_flg = 'required|'; }
        if(isset($_POST['candidate_photo_cropper']) && $_POST['candidate_photo_cropper'] != "") { $candidate_photo_req_flg = ''; }

        if ($form_data[0]['scannedsignaturephoto'] == "") { $candidate_sign_req_flg = 'required|'; }
        ...
        if ($this->form_validation->run())
        {
          ...
          if (isset($_POST['id_proof_file_cropper']) && $_POST['id_proof_file_cropper'] != "")
          {
            ...
            {
              $add_data['idproofphoto'] = $id_proof_file = $new_id_proof_file = basename($new_file_name1);
            }
            else
            {
          ...
          if (isset($_POST['candidate_photo_cropper']) && $_POST['candidate_photo_cropper'] != "")
          {
            ...
            {
              $add_data['scannedphoto'] = $candidate_photo = $new_candidate_photo = basename($new_file_name3);
            }
            else
            {
          ...
          if (isset($_POST['candidate_sign_cropper']) && $_POST['candidate_sign_cropper'] != "")
          {
            ...
            {
              $add_data['scannedsignaturephoto'] = $candidate_sign = $new_candidate_sign = basename($new_file_name4);
            }
            else
            {
          ...
          if ($error_flag == 0)
          {
            ...
            $candidateName = $this->Iibf_dra_model->getLoggedInUserDetails($candidate_id, 'candidate');
            ...
            {
              $chk_id_proof_file = $form_data[0]['idproofphoto'];
            }
            ...
            {
              $chk_candidate_photo = $form_data[0]['scannedphoto'];
            }
            ...
            {
              $chk_candidate_sign = $form_data[0]['scannedsignaturephoto'];
            }
            ...
            $this->Iibf_dra_model->check_file_exist($chk_id_proof_file, "./" . $id_proof_file_path . "/", 'iibfdra/candidate/dashboard_candidate/update_profile', 'Candidate profile not updated due to missing Proof of Identity'); //IF FILE NOT EXIST WHILE UPDATING THE PROFILE, THEN SHOW ERROR MESSAGE

            $this->Iibf_dra_model->check_file_exist($chk_candidate_photo, "./" . $candidate_photo_path . "/", 'iibfdra/candidate/dashboard_candidate/update_profile', 'Candidate profile not updated due to missing Passport Photograph of the Candidate'); //IF FILE NOT EXIST WHILE UPDATING THE PROFILE, THEN SHOW ERROR MESSAGE

            $this->Iibf_dra_model->check_file_exist($chk_candidate_sign, "./" . $candidate_sign_path . "/", 'iibfdra/candidate/dashboard_candidate/update_profile', 'Candidate profile not updated due to missing Signature of the Candidate'); //IF FILE NOT EXIST WHILE UPDATING THE PROFILE, THEN SHOW ERROR MESSAGE
            //END : IF FILE NOT EXIST WHILE UPDATING THE PROFILE, THEN REDIRECT & SHOW ERROR MESSAGE            

            $add_data['editedon']     = date("Y-m-d H:i:s");
            $add_data['edited_by_id'] = $candidate_id;


            $this->master_model->updateRecord('dra_members', $add_data, array('regid' => $candidate_id));
            
            // $this->Iibf_dra_model->insert_common_log('Candidate : Candidate Profile Updated', 'dra_members', $this->db->last_query(), $candidate_id, 'candidate_action', 'The candidate has successfully updated the profile', $posted_arr);

            $add_cand_log['action'] = 'Add';                  
            $add_cand_log['candidate_id'] = $candidate_id;
            $add_cand_log['log_title'] = 'Candidate : Candidate Profile Updated';
            $add_cand_log['log_decription'] = $posted_arr;
            $add_cand_log['status'] = 'success';
            $add_cand_log['created_by_type'] = 'candidate_action';
            $add_cand_log['created_by'] = $candidate_id;
            ...
            if ($candidate_id > 0)
            {
              $candidate_data = $this->master_model->getRecords('dra_members bc', array('bc.regid' => $candidate_id, 'bc.regnumber !=' => ''), "bc.regid, bc.regnumber, bc.idproofphoto, bc.quali_certificate, bc.scannedphoto, bc.scannedsignaturephoto");

              if (count($candidate_data) > 0)
              {
                $id_proof_file = $candidate_data[0]['idproofphoto'];
                $candidate_photo = $candidate_data[0]['scannedphoto'];
                $candidate_sign = $candidate_data[0]['scannedsignaturephoto'];
                ...
                if ($id_proof_file != $id_proof_file_new)
                {
                  ...
                  {
                    $up_img_data['idproofphoto'] = $id_proof_file_new;
                  }
                }
                ...
                if ($candidate_photo != $candidate_photo_new)
                {
                  ...
                  {
                    $up_img_data['scannedphoto'] = $candidate_photo_new;
                  }
                }
                ...
                if ($candidate_sign != $candidate_sign_new)
                {
                  ...
                  {
                    $up_img_data['scannedsignaturephoto'] = $candidate_sign_new;
                  }
                }
                
                if (count($up_img_data) > 0)
                {
                  $this->master_model->updateRecord('dra_members', $up_img_data, array('regid' => $candidate_data[0]['regid']));

                  $add_cand_log['action'] = 'Add';                  
                  $add_cand_log['candidate_id'] = $candidate_id;
                  $add_cand_log['log_title'] = 'Candidate : Candidate Profile Updated - image log';
                  $add_cand_log['log_decription'] = serialize($up_img_data);
                  $add_cand_log['status'] = 'success';
                  $add_cand_log['created_by_type'] = 'candidate_action_image_log';
                  $add_cand_log['created_by'] = $candidate_id;
                  $add_cand_log['created_on'] = date("Y-m-d H:i:s");
                  ...
                  // $this->Iibf_dra_model->insert_common_log('Candidate : Candidate Profile Updated - image log', 'dra_members', $this->db->last_query(), $candidate_data[0]['regid'], 'candidate_action_image_log', 'The candidate has successfully updated the profile images', '');
                }
              }
            }
            // END : Rename the images and update into database table name
            redirect(site_url('iibfdra/candidate/dashboard_candidate/update_profile'));
          }
        }
      }	
      
      $data['page_title'] = 'IIBF - DRA Candidate Update Profile'; 
      ...
      $this->load->view('iibfdra/candidate/update_profile_candidate', $data);
    ...
    public function validation_check_mobile_exist($str = '', $type = 0) //$type : 0=>Server side, 1=>Ajax
    {
      ...
      if (isset($_POST) && $_POST['mobile_no'] != "")
      {
        ...
        $candidate_data = $this->Iibf_dra_model->validation_check_candidate_eligibility_to_add_in_new_batch($candidate_id, 'mobile', $mobile_no);
        if (count($candidate_data) == 0)
        {
    ...
    public function validation_check_email_exist($str = '', $type = 0) //$type : 0=>Server side, 1=>Ajax
    {
      ...
      if (isset($_POST) && $_POST['email_id'] != "")
      {
        ...
        $candidate_data = $this->Iibf_dra_model->validation_check_candidate_eligibility_to_add_in_new_batch($candidate_id, 'email', $email_id);
        if (count($candidate_data) == 0)
        {
          $return_val_ajax = 'true';
        }
    ...
    { 
      if($str != '')
      {
        $result = $this->Iibf_dra_model->fun_restrict_input($str, $type); 
    ...
    {
      $result = $this->Iibf_dra_model->fun_validate_file_upload($parameter); 
      if($result['flag'] == 'success') { return true; }
      else
      {
    ...
    function candidate_logs()
    {
      if(isset($_POST) && count($_POST) > 0)
      {
        ...
        $data['log_data'] = $this->master_model->getRecords('dra_candidate_logs',array('candidate_id'=>$pk_id, 'status'=>'success'),'log_id, action, form_type, candidate_id, log_title,reason, status, created_by_type, created_by, created_on', array('log_id'=>'DESC'));

        $data['log_title'] = $log_title;      
        
        $result['response'] =  $this->load->view('iibfdra/candidate/get_logs_common',$data,TRUE);
        $result['flag'] = "success";
      }
      else
      {      
