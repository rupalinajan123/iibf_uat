...
class InspectorHome extends CI_Controller {
	...
	{	
		...
		$where = array('b.batch_status' => 'Approved', 'b.is_deleted' => 0);
		...
		$where1 = array('b.batch_status' => 'Approved', 'b.is_deleted' => 0, 'b.inspector_id' => $inspector_id);
    ...
    $result['message'] = "";
		...
		if (!empty($_FILES['attachment']['name']))
        {
        	if($_FILES['attachment']['size'] == 0){
        		echo 'File is empty.. Please upload valid file.';
        		exit(0);
        	}
        	else{
	        	//echo 'if';
    ...
    if(count($get_last_record) > 0)
    {
      ...
      {
        $result['message'] = "Wait for 2 min and submit the form again";
        echo json_encode($result); 
        exit;
      }
    }
    ...
    if($inserted_id){
      ...
      if(count($attendanceArr) > 0){
        foreach ($attendanceArr as $key => $attendance) {
          ...
          $where = array( 'batch_id' => $this->input->post('batch_id'),
                  'batch_inspection_id' => $batch_inspection_id, 
                  'candidate_id' => $key);
    ...
    foreach ($res as $key => $value) {
      ...
      if($value['hours'] == 100 && $value['absent_cnt'] >= 5){
	...
	/*public function inspection_summary()
	{
		...
		//$this->db->join('faculty_master f3','b.additional_first_faculty=f3.faculty_id','left');
		//$this->db->join('faculty_master f4','b.additional_sec_faculty=f4.faculty_id','left');
		//$data['batch'] = $batch = $this->master_model->getRecords('agency_batch b', $where);
