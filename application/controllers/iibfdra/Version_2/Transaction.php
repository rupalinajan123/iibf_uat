...
class Transaction extends CI_Controller {
	...
	public function getTransactions()
	{
		...
		// $select = 'dra_payment_transaction.id,gateway,dra_payment_transaction.inst_code,receipt_no,dra_payment_transaction.status,transaction_no,UTR_no,DATE_FORMAT(date,"%Y-%m-%d") As pay_date,bankcode,pay_count AS member_count,amount,tds_amount,dra_accerdited_master.institute_name AS inst_name';	

	 	$select = 'dra_payment_transaction.id,gateway,dra_payment_transaction.proformo_invoice_no,dra_payment_transaction.exam_code,dra_payment_transaction.inst_code,receipt_no,dra_payment_transaction.status,transaction_no,UTR_no,DATE_FORMAT(date,"%Y-%m-%d") As pay_date,bankcode,pay_count AS member_count,amount,tds_amount,dra_accerdited_master.institute_name AS inst_name';	
		...
		if($res)
		{
			...
			foreach($result as $row)
			{
				foreach($row as $key => $value)
				{
					...
					else if($key == "status" && $value == 3) // status = 3 for Pending
					{
						$row['status'] = "Pending";	
					}
					...
					else if($key == "status" && $value == 7) // status = 7 for Proforma Invoice Invoked
					{
					...
					else if($key == "gateway" && $value == "2") // gateway = 2 for Online Payment
					{
				...
				// action -
				$action = '<a href="'.base_url().'iibfdra/Version_2/transaction/view_inst_receipt/'.base64_encode($row['id']).'" target="_blank">Receipt</a>';
				...
				$exam_invoice = $this->master_model->getRecords('exam_invoice',array('pay_txn_id' => $row['id'],'exam_code'=>$row['exam_code']),'invoice_image,date_of_invoice,gstin_no,invoice_no');
				...
				if($invoice_img_path != '')
				{
					...
					if($date_invoice < '2021-01-01' && $date_invoice !=''){
						...
						if($exam_invoice[0]['gstin_no'] == ''){
							...
							$action .= ' <br> | <a href="'.base_url().'iibfdra/Version_2/transaction/getInvoice/'.base64_encode($str_invoice_no).'">E-Invoice</a>';
						}
					}
				}
	...
	public function getInvoice($invoice_no)
	{
		...
		$inv_no = base64_decode($invoice_no);
		## Test invoice no
	...
	public function view_inst_receipt($txn_id)
	{
		...
		$this->load->view('iibfdra/Version_2/transaction/view_inst_receipt',$data);
	}
	...
	public function mem_receipt_list($txn_id)
	{
		...
		$this->load->view('iibfdra/Version_2/transaction/mem_receipt_list',$data);
	}
	...
	public function mem_receipt($txn_id, $mem_id)
	{
		...
		// get list of all members for this payment transaction -
		$select = 'dra_members.regid,dra_members.regnumber,dra_members.firstname,dra_members.middlename,dra_members.lastname,email_id,dra_member_exam.exam_fee, dra_member_exam.exam_center_code, dra_member_exam.exam_medium';	
		$this->db->join('dra_member_exam','dra_member_exam.id = dra_member_payment_transaction.memexamid','LEFT');
		$this->db->join('dra_members','dra_members.regid = dra_member_exam.regid','LEFT');
		...
		$this->load->view('iibfdra/Version_2/transaction/mem_receipt',$data);
	}
} ?>