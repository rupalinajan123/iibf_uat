...
class InspectorLogin extends CI_Controller {
	...
	public function index()
	{
				...
				if($this->form_validation->run() == TRUE)
        {
						...
						if(!empty($aUser[0]))
            {
							...
							redirect('iibfdra/Version_2/inspectorHome/dashboard');	
						} 
            else
            {
			...
			$this->load->view('iibfdra/Version_2/inspector_login',$data);
		/*}
		else
		{
			redirect(base_url().'iibfdra/Version_2/inspectorHome/dashboard');
		}*/
	}
	...
	public function check_captcha_dralogin($code) 
	{
		...
		{
			redirect(base_url().'iibfdra/Version_2/InspectorLogin/');
		}
