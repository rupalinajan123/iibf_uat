...
class CenterMaster extends CI_Controller {
	...
	public function index()
	{
		...
		$data["breadcrumb"] = '<ol class="breadcrumb"><li><a href="'.base_url().'iibfdra/Version_2/admin/MainController"><i class="fa fa-dashboard"></i> Home</a></li>
									<li><a href="'.base_url().'iibfdra/Version_2/admin/'.$this->router->fetch_class().'">Manage Center</a></li>
							   </ol>';
		
		$this->load->view('iibfdra/Version_2/admin/masters/center_list',$data);
	}
	...
	public function getList()
	{
		...
		$url = base_url()."iibfdra/Version_2/admin/CenterMaster/getList/";
		...
		if($res)
		{
			...
			foreach($result as $row)
			{
				...
				$action = '<a href="'.base_url().'iibfdra/Version_2/admin/CenterMaster/edit/'.$row['center_id'].'">Edit | </a> <a href="'.base_url().'iibfdra/Version_2/admin/CenterMaster/delete/'.$row['center_id'].'" onclick="'.$confirm.'">Delete</a>';
				$data['action'][] = $action;
	...
	public function add(){
		...
		if(isset($_POST['btnSubmit']))
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				if($this->master_model->insertRecord('dra_center_master',$insert_data))
				{
					...
					redirect(base_url().'iibfdra/Version_2/admin/CenterMaster');
				}
				else
				{
					...
					redirect(base_url().'iibfdra/Version_2/admin/CenterMaster/add');
				}
			}
			else
			{
		...
		$this->load->view('iibfdra/Version_2/admin/masters/center_add',$data);
	}
		...
		if(isset($_POST['btnSubmit']))
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				else
				{
					...
					redirect(base_url().'iibfdra/Version_2/admin/CenterMaster/edit/'.$id);
				}
			}
			else
			{
							...
							while (($line = fgets($handle)) !== false) {
								...
								else
								{
									...
									else
									{
										...
										redirect(base_url().'iibfdra/Version_2/admin/CenterMaster/import');
									}
								}
							}
