...
class Login extends CI_Controller {
	...
	public function index()
	{
		...
		if( !$this->session->userdata('dra_admin') ) {
			if(isset($_POST['submit'])) {
				...
				//	{
					...
					{					
						...
						redirect('iibfdra/Version_2/admin/MainController');	
					} 
					else 
					{
			...
			$this->load->view('iibfdra/Version_2/admin/login/login',$data);
		}
		else
		{
			redirect(base_url().'iibfdra/Version_2/admin/MainController');
		}
	}
	...
	public function check_captcha_adminlogin($code) 
	{
		...
		{
			redirect(base_url().'iibfdra/Version_2/admin/login/');
		}
	...
	public function changepassword()
	{
		...
		if(isset($_POST['btn_password']))
		{
			...
			if($this->form_validation->run())
			{
				...
				if($current_pass == $new_pass)
				{
					...
					redirect(base_url().'iibfdra/Version_2/admin/login/changepassword');
				}
		...
		$this->load->view('iibfdra/Version_2/admin/login/change_password', $data);
	}
	
	public function Logout(){
		$sessionData = $this->session->userdata('dra_admin');
		$this->session->unset_userdata('dra_admin');
		redirect('iibfdra/Version_2/admin/Welcome');
	}
}