...
class ExamActiveMaster extends CI_Controller {
	...
	public function index()
	{
		...
		$data["breadcrumb"] = '<ol class="breadcrumb"><li><a href="'.base_url().'iibfdra/Version_2/admin/MainController"><i class="fa fa-dashboard"></i> Home</a></li>
									<li><a href="'.base_url().'iibfdra/Version_2/admin/'.$this->router->fetch_class().'">Manage Exam Activation Master</a></li>
							   </ol>';
		
		$this->load->view('iibfdra/Version_2/admin/masters/exam_active_list',$data);
	}
	...
	public function getList()
	{
		...
		$total_row = $this->UserModel->getRecordCount("dra_exam_activation_master a",$field,$value);
		$url = base_url()."iibfdra/Version_2/admin/ExamActiveMaster/getList/";
		$config = pagination_init($url,$total_row, $per_page, 2);
		$this->pagination->initialize($config);
		
		$select = 'a.id exam_active_id,a.exam_period,a.exam_code,exam_from_date,exam_from_time,exam_to_date,exam_to_time,b.id exam_id,b.description';
		$this->db->where('exam_activation_delete',0);
		$this->db->where('exam_delete',0);
		$this->db->join('dra_exam_master b','b.exam_code=a.exam_code','LEFT');	
		$res = $this->UserModel->getRecords("dra_exam_activation_master a", $select, $field, $value, $sortkey, $sortval, $per_page, $start);
		if($res)
		{
			...
			foreach($result as $row)
			{
				...
				$action = '<a href="'.base_url().'iibfdra/Version_2/admin/ExamActiveMaster/edit/'.$row['exam_active_id'].'">Edit | </a> <a href="'.base_url().'iibfdra/Version_2/admin/ExamActiveMaster/delete/'.$row['exam_active_id'].'" onclick="'.$confirm.'">Delete</a>';
				$data['action'][] = $action;
	...
	public function add(){
		...
		if(isset($_POST['btnSubmit']))
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				if($this->master_model->insertRecord('dra_exam_activation_master',$insert_data))
				{
					log_dra_admin($log_title = "Add DRA Exam Activation Successfull", $log_message = serialize($insert_data));
					$this->session->set_flashdata('success','Record added successfully');
					redirect(base_url().'iibfdra/Version_2/admin/ExamActiveMaster');
				}
				else
				{
					$this->master_model->insertRecord('adminlogs',$logs_data);
					log_dra_admin($log_title = "Add DRA Exam Activation Unsuccessfull", $log_message = serialize($insert_data));
					$this->session->set_flashdata('error','Error occured while adding record');
					redirect(base_url().'iibfdra/Version_2/admin/ExamActiveMaster/add');
				}
			}
			else
			{
		...
		$data['examActiveRes'] = array(	'exam_code'			=>'',
										'exam_period'		=>'',
										'exam_from_date'	=>'',
										'exam_from_time'	=>'',
										'exam_to_date'		=>'',
		...
		$this->load->view('iibfdra/Version_2/admin/masters/exam_active_add',$data);
	}
	...
	public function edit(){
		...
		if(is_numeric($id))
		{
			$examActiveRes = $this->master_model->getRecords('dra_exam_activation_master',array('id'=>$id));
		...
		if(isset($_POST['btnSubmit']))
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				if($from_time < $to_time)
				{
					...
					if($this->master_model->updateRecord('dra_exam_activation_master',$update_data, array('id'=>$id)))
					{
						$desc['updated_data'] = $update_data;
						$desc['old_data'] = $examActiveRes[0];
						log_dra_admin($log_title = "Edit DRA Exam Activation Successfull", $log_message = serialize($desc));
					...
					else
					{
						$desc['updated_data'] = $update_data;
						$desc['old_data'] = $examActiveRes[0];
						log_dra_admin($log_title = "Edit DRA Exam Activation Unsuccessfull", $log_message = serialize($desc));
						$this->session->set_flashdata('error','Error occured while updating record');
						redirect(base_url().'iibfdra/Version_2/admin/ExamActiveMaster/edit/'.$id);
					}
				}
				else
				{
	...
	public function delete()
	{
		...
		if(is_numeric($id))
		{
			...
			if($this->master_model->updateRecord('dra_exam_activation_master', $update_data, array('id'=>$id)))
			{
				log_dra_admin($log_title = "Delete DRA Exam Activation Successfull", $log_message = serialize(array('id'=>$id)));
				$this->session->set_flashdata('success','Record deleted successfully');
				redirect(base_url().'iibfdra/Version_2/admin/ExamActiveMaster');
			}
			else
			{
				log_dra_admin($log_title = "Delete DRA Exam Activation Unsuccessfull", $log_message = serialize(array('id'=>$id)));
	...
	public function import(){
		...
		{			
						...
						if ($handle) {
							while (($line = fgets($handle)) !== false) {
								...
								if($firstlineflag == 1)
								{
									...
									$this->master_model->insertRecord('dra_exam_activation_master',$insert_data);
								}
								else
								{
									$raw_array = explode("|" ,$line);
									...
									{	
										...
										//master_db_backup($basetable = "dra_exam_activation_master"); // keep old and new master in DB
									}	
									else
									{
										@unlink($target_file);
										$this->session->set_flashdata('error','Please upload valid file');
										redirect(base_url().'iibfdra/Version_2/admin/ExamActiveMaster/import');
									}
								}
							}
			...
			if ($data['success'] != "")
			{
				log_dra_admin($log_title = "DRA Exam Activation master import sucessfull", $log_message = "New DRA Exam Activation master file import sucessfull file=".$target_file);
			}
			else
			{
				log_dra_admin($log_title = "DRA Exam Activation master import failed", $log_message = "Error = ".$data['error']."  file = ".$target_file);
			}	
		}
		...
		$data['sample_file'] = 'DRA_EXAM_ACTIVATE.TXT';
	...
	public function download(){
		...
		$this->db->where('exam_activation_delete',0);
		$exam_active_list = $this->Master_model->getRecords("dra_exam_activation_master a");
		...
		log_dra_admin($log_title = "DRA Exam Activation master downloaded", $log_message = "");
		header("Content-type: application/x-gzip");
		header('Content-Disposition: attachement; filename="dra_exam_activation_master.txt.gz"');
