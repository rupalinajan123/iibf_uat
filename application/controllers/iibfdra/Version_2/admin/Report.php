...
class Report extends CI_Controller {
	...
	public function billdesk_success()
	{
		...
		$this->load->view('iibfdra/Version_2/admin/report/billdesk_success',$data);
	}
	...
	public function getBDSuccess()
	{
		...
		if($res)
		{
			...
			$url = base_url()."iibfdra/Version_2/admin/report/getBDSuccess/";
	...
	public function billdesk_failure()
	{
		...
		$this->load->view('iibfdra/Version_2/admin/report/billdesk_failure',$data);
	}
	...
	public function getBDFailure()
	{
		...
		if($res)
		{
			...
			$url = base_url()."iibfdra/Version_2/admin/report/getBDFailure/";
			//$total_row = count($result);
			$config = pagination_init($url,$total_row, $per_page, 2);
			$this->pagination->initialize($config);
	...
	public function failure_reason()
	{
		...
		$this->load->view('iibfdra/Version_2/admin/report/failure_reason',$data);
	}
	...
	public function getFailureReason()
	{
		...
		if($res)
		{
			...
			$url = base_url()."iibfdra/Version_2/admin/report/getFailureReason/";
			//$total_row = count($result);
			$config = pagination_init($url,$total_row, $per_page, 2);
			$this->pagination->initialize($config);
	...
	public function billdesk_neft_report()
	{
		...
		$this->load->view('iibfdra/Version_2/admin/report/billdesk_neft_report',$data);
	}

//Quarterly_Report of dra batch and candidates
	public function Quarterly_Report(){
       if($this->input->post('submit'))
		...
		$query = "
		SELECT dra_accerdited_master.institute_name as 'Institute Name',dra_accerdited_master.institute_code as 'Institute Code',`city_master`.`city_name` as 'City Name',`agency_batch`.`batch_code` as 'Batch Code',`agency_batch`.`batch_from_date` as 'Batch From Date',`agency_batch`.`batch_to_date` as 'Batch To Date',`agency_batch`.`batch_status` as 'Batch Status(A-Approved)',`agency_batch`.`created_on` as 'Created Date',COUNT(dra_members.regid) as 'No Of Members' FROM `agency_batch` LEFT JOIN `agency_center` ON `agency_batch`.`center_id`=`agency_center`.`center_id` LEFT JOIN `city_master` ON `city_master`.`id`=`agency_center`.`location_name` LEFT JOIN `dra_members` ON `dra_members`.`batch_id`=`agency_batch`.`id` AND  batch_id != 0 LEFT JOIN `dra_accerdited_master` ON `dra_accerdited_master`.`dra_inst_registration_id`=`agency_batch`.`agency_id` WHERE `agency_center`.`center_display_status` = '1' AND `agency_batch`.`batch_status` = 'A' AND DATE(batch_from_date) BETWEEN '".$from_date."' AND '".$end_date."' GROUP BY agency_batch.id
		";
		...
		$this->load->view('iibfdra/Version_2/admin/report/quarterly_report');
 }
