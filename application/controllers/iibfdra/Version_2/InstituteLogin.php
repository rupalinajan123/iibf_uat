	...
	public function index()
	{
		...
		if( !$this->session->userdata('FGet') ) {
			if(isset($_POST['submit'])) {
				...
				$instCode = $this->input->post('Username');
				$DRA_Version_2_instCode = $this->config->item('DRA_Version_2_instCode');

				//print_r($DRA_Version_2_instCode); die;
				if(!in_array($instCode, $DRA_Version_2_instCode)){
					die($instCode.' Institute Code Do not have Access to DRA Version 2. Please Login to Link : https://iibf.esdsconnect.com/iibfdra/InstituteLogin');
				}
