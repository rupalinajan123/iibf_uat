...
class Transaction extends CI_Controller {
	...
	public function transactions()
	{
		...
		$data["institute_list"] = $this->Master_model->getRecords("dra_accerdited_master","accerdited_delete = '0'","institute_code,institute_name", array('institute_name'=>'ASC'));
		$data["exam_list"] = $this->Master_model->getRecords("dra_exam_master","exam_delete = '0'","exam_code,description", array('description'=>'ASC'));
	...
	public function getTransactions()
	{
		...
		if(count($_POST) > 0)
		{
			...
			if(isset($_POST['value'])) { $value = $_POST['value']; }
 			if(isset($_POST['sortkey']) && $sortkey == "") { $sortkey = $_POST['sortkey']; }
 			if(isset($_POST['sortval']) && $sortval == "") { $sortval = $_POST['sortval']; }
			if(isset($_POST['per_page']) && $per_page == "") { $per_page = $_POST['per_page']; }
			if(isset($_POST['start']) && $start == "") { $start = $_POST['start']; }
		}
		...
		else
		{
			...
			//$where .= '1=1';	// status = success or fail
		}
		
	 	$select = 'dra_payment_transaction.id,gateway,dra_payment_transaction.inst_code,receipt_no,status,transaction_no,UTR_no,DATE_FORMAT(date,"%Y-%m-%d") As pay_date,bankcode,pay_count AS member_count,amount,tds_amount,dra_accerdited_master.institute_name AS inst_name';	
		...
		if($sortkey == '' || $sortkey == 'paid_reg_nos') { $sortkey = 'date'; }
		if($sortval == '') { $sortval = 'DESC'; }
		else if($sortval == 'descending') { $sortval = 'DESC'; }
		else if($sortval == 'ascending') { $sortval = 'ASC'; }
	...
	public function neft_transactions()
	{
		...
		$this->load->view('iibfdra/admin/transaction/neft_transactions',$data);
	}
	...
	public function getNeftTransactions()
	{
		...
		if($res)
		{
			...
			$url = base_url()."iibfdra/admin/transaction/getNeftTransactions/";
