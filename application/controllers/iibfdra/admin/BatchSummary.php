...
class BatchSummary extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $agencyQry = $this->db->query("SELECT a.id, ac.institute_code, ac.institute_name
    FROM agency_batch a 
    LEFT JOIN dra_accerdited_master ac ON a.agency_id = ac.dra_inst_registration_id 
    ...
    $data['inspectors']= $this->Master_model->getRecords('agency_inspector_master',array('is_delete'=>'0'),"", array('inspector_name'=>'ASC'));
    $this->load->view('iibfdra/admin/batch_summary/batch_summary', $data);
  }
  ...
  public function get_batch_mis()
  {
    ...
    if ($agencyvalue != '') { $searchQuery .= " AND (ac.id = ".$agencyvalue." ) "; }
    if ($inspectorvalue != '') { $searchQueryHaving = " HAVING FIND_IN_SET('".$inspectorvalue."',inspected_by_id)"; }
    if ($status != '') { $searchQuery .= " AND (a.batch_status = '".$status."' ) "; }   
    ...
    // echo "Filter :".$total_records;
    // COUNT(dm.regid) AS total_registered_candidate
    
    ## Total number of records without filtering
    $select2 = "SELECT a.id, a.batch_code, a.created_on, a.updated_on, a.contact_person_name, ac.institute_code, ac.institute_name, a.batch_from_date, a.batch_to_date, CONCAT_WS(', ', fm1.faculty_name, fm2.faculty_name, fm3.faculty_name, fm4.faculty_name) AS faculty_names, 
    COUNT(dbi.id) AS inspection_count, GROUP_CONCAT(DISTINCT DATE_FORMAT(dbi.created_on, '%Y-%m-%d')) AS inspection_dates, 
    ...
    GROUP_CONCAT(DISTINCT aim.id) AS inspected_by_id,
    a.total_candidates, a.batch_status
            FROM agency_batch a 
            LEFT JOIN dra_accerdited_master ac ON a.agency_id = ac.dra_inst_registration_id 
            LEFT JOIN agency_inspector_master i ON a.inspector_id = i.id
            LEFT JOIN faculty_master fm1 ON a.first_faculty = fm1.faculty_id
            LEFT JOIN faculty_master fm2 ON a.sec_faculty = fm2.faculty_id
            LEFT JOIN faculty_master fm3 ON a.additional_first_faculty = fm3.faculty_id
            LEFT JOIN faculty_master fm4 ON a.additional_sec_faculty = fm4.faculty_id
            ...
            WHERE a.is_deleted = 0 " . $searchQuery." ".
            "GROUP BY a.id ".$searchQueryHaving;
    ...
    // $totalRecordwithFilter = count($agency_list);
    // echo "Filter :".$totalRecordwithFilter; exit;
