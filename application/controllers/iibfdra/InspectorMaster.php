...
class InspectorMaster extends CI_Controller 
{
	...
	{	
		...
		$data["inspector_list"] = $this->Master_model->getRecords("agency_inspector_master","","inspector_name,inspector_mobile,inspector_email,inspector_designation");
		$data["breadcrumb"] = '<ol class="breadcrumb"><li><a href="'.base_url().'iibfdra/admin/MainController"><i class="fa fa-dashboard"></i> Home</a></li>
								<li><a href="'.base_url().'iibfdra/admin/'.$this->router->fetch_class().'">Manage Inspector</a></li>
							</ol>';
		$this->load->view('iibfdra/admin/masters/inspector_list',$data);
	}
	...
	public function getList()
	{
		...
		$url = base_url()."iibfdra/admin/InspectorMaster/getList";
	...
	public function add()
	{
		...
		if(isset($_POST['btnSubmit']))
		{
			...
			{		//echo "swads"; die;		
					...
					if($last_id)
					{
						...
						redirect(base_url().'iibfdra/admin/InspectorMaster');
					}
					else
					{
						...
						redirect(base_url().'iibfdra/admin/InspectorMaster/add');
					}
			}
			else
			{
		...
		$this->load->view('iibfdra/admin/masters/inspector_add',$data);
	}
	...
	{		
		...
		$data["inspector_list"] = $this->Master_model->getRecords("agency_inspector_master",array('is_delete'=>0),"id,inspector_name,inspector_mobile,inspector_email,inspector_designation");
...
//print_r($data['cities']); die;
		...
		$this->load->view('iibfdra/admin/masters/inspector_edit',$data);
		//redirect(base_url().'iibfdra/admin/InspectorMaster');
	}
