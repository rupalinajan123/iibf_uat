...
class Automation extends CI_Controller {
					...
					if (($handle = fopen($file, "r")) !== FALSE) {
			        	...
			        	foreach ($arr as $key => $value) {
					    	if(!empty($value)){
								...
								// add gaurav for 1036 exam code condition
								$updated_id8 = $this->master_model->updateRecord('dra_result_exam',$uploadData,array('exam_code' => 1036));
								// end 
								...
								// Add gaurav for 1036 exam code condition
								$updated_id9 = $this->master_model->updateRecord('dra_result_subject',$uploadData1,array('exam_code' => 1036));
								// end 
						    }
					    }
	...
	public function insertRPE_exam(){
		...
		foreach($files as $file){
		    if(is_file($file)){
			        	...
			        	if(!empty($arr)){
						    foreach ($arr as $key => $value) {
						    	if(!empty($value)){
								    ...
								    //	$center_sql.= "('".$final_value."'),";
							    		$center_sql.= "('".$final_value."'),";
									/*$center_sql .= "('".$counter."', '".$final_value."'),";
								    	echo '</br>'.$center_sql;*/
								    }
							    }
						    }
				...
				if(strpos($file,"ELG")!=false){
					...
					$elg_insert = "INSERT INTO eligible_master(exam_code, eligible_period, part_no, member_no, member_type, exam_status, app_category, fees, subject, med_cd, remark, subject_code, institute_id) VALUES ";
					$counter++;
					if (($eligible_handle = fopen($file, "r")) !== FALSE) {
						$readFile =  fread($eligible_handle,filesize($file));
						$arr = explode("\n",
		...
		if($elg_sql != '') {
			...
			$insId2 = $this->db->query($elg_insert);
			//echo 'eligible_insert2---'.$this->db->last_query();
		}
		...
		echo "Total records inserted: " . $counter;
	}
		    ...
		    if(is_file($file)){
		    	...
		    	if(strpos($file, 'EXAM_LIST') !== FALSE){
	    			...
	    			if(count($check_exam_exists) > 0){
	    				$this->db->query('DELETE FROM result_exam WHERE exam_code ='.$exp1[2].' AND exam_period = '.$exp1[3]);
	    			}
	...
	public function insertDipcert_exam(){
		...
		foreach($files as $file){
		    if(is_file($file)){
				...
				if(strpos($file,"CENTER_MASTER")!=false){
					...
					$center_insert = "INSERT INTO center_master(exam_name, exam_period, center_code, center_name, state_code, state_description, exammode,vendor_code,vendor) VALUES";
					...
					if (($center_handle = fopen($file, "r")) !== FALSE) {
			        	...
			        	if(!empty($arr)){
						    foreach ($arr as $key => $value) {
						    	if(!empty($value)){
						    		if(strpos($value, 'EXM_CD|EXM_PRD|CTR_CD|CTR_NAM|STE_CD|STE_DSC|MODE_OF_EXAM|VENDOR_CD|VENDOR_NAME') === false){
				...
				if(strpos($file,"Venue")!==false){
					...
					foreach($venueData as $key => $venue) {
				    	if($key > 1){
				    		$filtered_venue = array_filter($venue); 
				    		//print_r($filtered_venue); die;
				    		if(count($filtered_venue)>0){
				    			// echo $filtered_venue['B'].'<br>';

				    			// Create a DateTime object from the original date
								$dateTime = DateTime::createFromFormat('d/m/Y', $filtered_venue['B']);

								// Format the date to Y-m-d
								$exam_date = $dateTime->format('Y-m-d');
				    			...
				    			// echo count($check_venue_exists);

				    			if(count($check_venue_exists) > 0) {
				    				foreach ($check_venue_exists as $key => $value) {
				    					
				    					$this->db->query('DELETE FROM venue_master WHERE venue_master_id ='.$value['venue_master_id']);
				...
				if(strpos($file,"Additional")!==false){
					$inputFileType = PHPExcel_IOFactory::identify($file);
					$objReader = PHPExcel_IOFactory::createReader($inputFileType);
					$objPHPExcel = $objReader->load($file);
					$venueData = $objPHPExcel->getActiveSheet()->toArray(null,true,true,true);
					...
					foreach($venueData as $key => $venue) {
				    	if($key > 1){
				    		$filtered_venue = array_filter($venue); 
				    		//print_r($filtered_venue); die;
				    		//echo count($filtered_venue);
				    		if(count($filtered_venue)>0){
				    			//echo $filtered_venue['B'];
				    			//$date = new Date($filtered_venue['B']);
				    			//$exam_date = $date->format('Y-m-d');
				    			$date = explode('/', $filtered_venue['B']);
								$exam_date = $date[2].'-'.$date[1].'-'.$date[0];
				    			...
				    			$this->db->select('venue_master_id');
								$where = array('exam_date' => $exam_date, 'center_code' => $filtered_venue['C'], 'session_time' => $filtered_venue['D'], 'venue_code' => $filtered_venue['E'], 'vendor_code' => $filtered_venue['A'], 'institute_code' => 0);
								//print_r($where); die;
				    			$check_venue_exists = $this->master_model->getRecords('venue_master', $where);
		...
		if($fee_sql1 != ''){
			...
			// echo "<pre>This feeArr1 ". $this->db->last_query();
		}
		...
		if($fee_sql2 != ''){
			...
			$fee_insert2.=rtrim($fee_sql2,",".PHP_EOL).';';
			$insId4 = $this->db->query($fee_insert2);
			// echo "<pre>This feeArr2 ". $this->db->last_query(); exit;
		}
		...
		if($insId1 > 0 || $insId2 > 0 || $insId3 > 0 || $insId4 > 0 || $insId5 > 0 || $insId6 > 0 || $insId7 > 0 || $insId10 > 0 || $insId11 > 0) {
			        	...
			        	if(!empty($arr)){
						    foreach ($arr as $key => $value) {
						    	if(!empty($value)){
						    		...
						    		unset($final_valueArr1[5]);
						    		$final_valueArr1[1] = $final_valueArr1[1].' (Dipcert)';
						    		$final_value1 = implode('|', $final_valueArr1);
						    		$final_value1 = str_replace('|', "','", $final_value1);
						    		$final_value1 = $final_value1."','".$exp1[3];
						    		$exam_period = $exp1[3];
		...
		foreach($files as $file){
		    if(is_file($file)){
				...
				if(strpos($file,"CENTER_MASTER")!=false){
					...
					$center_insert = "INSERT INTO center_master(exam_name, exam_period, center_code, center_name, state_code, state_description, exammode, vendor, vendor_code) VALUES";
	...
	public function insertBCBF_exam(){
						    ...
						    foreach ($arr as $key => $value) {
						    	if(!empty($value)){
						    		...
						    		if($key > 0){
							    		...
							    		//echo $value;
							    		$final_valueArr1 = explode('|', $value);
										//echo $final_valueArr1[1]."==".$final_valueArr1[2];die;
							    		/*unset($final_valueArr1[12]);
							    		unset($final_valueArr1[14]);*/
							    		unset($final_valueArr1[13]);
							    		//unset($final_valueArr1[13]);
		...
		if($elg_sql != ''){
			$elg_insert.=rtrim($elg_sql,",").';';
			$insId2 = $this->db->query($elg_insert);
			//echo 'elg_insert---'.$this->db->last_query();exit;
		}
	...
	public function insertBCBF_result(){
		...
		foreach($allfiles as $file){
			if(is_file($file)){
				...
				if(strpos($file,"MEMBER_LIST")!=false){
					...
					/*('100002920','O','SYED ABDUL KHADER','AMBALAM','','FLAT 415 SURYA ENCLAVE APTS','12/129 SECTOR 12 INDIRA NAGAR','MUNSHIPULIA','LUCKNOW','LUCKNOW','LUCKNOW','226016','1034','1','R','0','202412','1','17-03-1973'), 
					100002920|O|SYED ABDUL KHADER|AMBALAM||FLAT 415 SURYA ENCLAVE APTS|12/129 SECTOR 12 INDIRA NAGAR|MUNSHIPULIA|LUCKNOW|LUCKNOW|LUCKNOW|226016|1034|1|R|0|202412|1|17-03-1973|
					100004399|O|VIJAYA KUMAR P J|||15-77, PADMAVATHI NAGAR|TIRUPATHI|CHITTOOR DIST.||CHITTOOR|CHITTOOR|517501|1034|1|R|0|202412|1|11-05-1973|
					100005921|O|S RAGHOTHAMAN|||INDIAN OVERSEAS BANK|D402 IOB OFFICER QUARTERS|EA BLOCK SEC 1 SALT LAKE CITY|BIDHAN NAGAR|NORTH PARAGNAS|KOLKATA|700064|1034|1|R|0|0|1|13-08-1966|*/
			        	...
			        	if(!empty($arr1)){
						    ...
						    foreach ($arr1 as $key => $value) {
						    	if(!empty($value)){
							    	...
							    	$final_valueArr1[19] = $exam_period;
							    	//echo '</br>'.count($final_valueArr1);
		...
		if($member_sql != ''){
			//$member_insert.=rtrim($member_sql,",".PHP_EOL).';';
			$member_insert.=rtrim($member_sql,",").';';
			$insId2 = $this->db->query($member_insert);
			echo $this->db->last_query();
		}
		...
		if($insId1 > 0 || $insId2 > 0 || $updated_id1 > 0 || $updated_id1 > 0 || $insId6 > 0){
			$url = base_url().'marksheet/bcbf/'.base64_encode($exam_period);
		...
		foreach($allfiles as $file){
			if(is_file($file)){
				...
				if(strpos($file,"MARK_OBT")!=false){
					...
					$table_name = 'blended_marks_'.$exam_code;
					echo '<pre>';
					echo $marks_insert = "INSERT INTO $table_name(regnumber, exam_id, exam_period, part_no, subject_id, marks, status, result_date) VALUES";
					...
					if (($handle = fopen($file, "r")) !== FALSE) {
			        	...
			        	if(!empty($arr)){
						    ...
						    foreach ($arr as $key => $value) {
						    	if(!empty($value)){
							    	$final_value = str_replace('|', "','", $value);
							    	$final_Arr = explode(',', $final_value);
							    	$final_valueArr = str_replace("'", '', $final_Arr);
							    	$regnumber = $final_valueArr[0];
							    	$exam_id = $final_valueArr[1];
							    	$exam_period = $final_valueArr[2];
							    	$result_date = $final_valueArr[7];
									//print_r($final_value);//die;
							    	$table_name = 'blended_marks_'.$exam_id;
							    	
							    	$this->db->select('regnumber');
					    			$check_mark_exists = $this->master_model->getRecords($table_name, array('regnumber' => $regnumber,'exam_id' => $exam_id, 'exam_period' => $exam_period));
					    			//echo count($check_mark_exists);die;
					    			if(count($check_mark_exists) > 0){
					    				//$this->db->query('DELETE FROM '.$table_name.' WHERE regnumber ='.$regnumber.' AND result_date = "'.$result_date.'" AND  exam_id = '.$exam_id.' AND exam_period = '.$exam_period);
					    				//echo $this->db->last_query(); //die;
					    			}
					    			//echo 'OUT';die;
									//echo $this->db->last_query(); //die;
				...
				if(strpos($file,"MEMBER_LIST")!=false){
					...
					$table_name = 'blended_memdtl_'.$exam_code;

					$member_insert = "INSERT INTO $table_name(member_number, member_type, firstname, middlename, lastname, address1, address2, address3, address4, address5, address6, pincode, exam_code, part_no, syllabus_code, dummy, dummy_one, no_of_attempt, dob, exam_period) VALUES";
			        	...
			        	if(!empty($arr1)){
						    ...
						    foreach ($arr1 as $key => $value) {
						    	if(!empty($value)){
						    		...
						    		$final_value1 = str_replace('|', "','", $final_value1);
						    		$final_valueArr1 = explode(',', $final_value1);
						    		$final_valueArr = str_replace("'", '', $final_valueArr1);
						    		//echo '</br>'.count($final_valueArr1);
							    	unset($final_valueArr1[19]);
							    	//print_r($final_valueArr1);die;

							    	 $member_number = $final_valueArr[0]; 
							    	 $exam_code = $final_valueArr[12]; 
							    	 $exam_period = $exp1[3];

					    			$this->db->select('member_number');
					    			$check_mem_exists = $this->master_model->getRecords($table_name, array('member_number' => $member_number,'exam_code' => $exam_code,'exam_period' => $exam_period));

					    			if(count($check_mem_exists) > 0){
					    				$this->db->query('DELETE FROM '.$table_name.' WHERE member_number = "'.$member_number.'" AND  exam_code = "'.$exam_code.'" AND exam_period = '.$exam_period);
					    				//echo $this->db->last_query();// die;
					    			}
				...
				if(strpos($file, 'EXAM_LIST') !== FALSE){
		    		...
		    		$result_exam_insert = "INSERT INTO blended_result_exam(exam_code, exam_period,part_no, exam_name_full, exam_name_short, exam_frequency, exam_conduct, exam_date, result_date) VALUES";
			        	...
			        	if(!empty($arr)){
						    foreach ($arr as $key => $value) {
						    	if(!empty($value)){
						    		...
						    		$final_valueArr1 = explode('|', $value);
						    		//print_r($final_valueArr1);die;
						    		//$period=array("9" => $exam_period); 
									//$final_valueArr1=array_merge($final_valueArr1,$period);
						    		...
						    		$this->db->select('exam_id');
					    			$check_exam_exists = $this->master_model->getRecords('blended_result_exam', array('exam_code' => $final_valueArr1[0], 'exam_period' => $final_valueArr1[1]));
					    			
					    			if(count($check_exam_exists) > 0){
					    				$this->db->query('DELETE FROM blended_result_exam WHERE exam_code ='.$final_valueArr1[0].' AND exam_period = '.$final_valueArr1[1]);
					    			}
					    			//echo $this->db->last_query(); die;
			        	...
			        	if(!empty($arr)){
						    foreach ($arr as $key => $value) {
						    	if(!empty($value)){
					    			...
					    			$final_value1 = str_replace('|', "','", $final_value1);
					    			$final_valueArr1 = explode(',', $final_value1);
					    			$final_valueArr1 = str_replace("'", '', $final_valueArr1);
					    			
					    			$mem_no = $final_valueArr1[1];
					    			$exam_code = $final_valueArr1[0];
					    			$exam_period = $final_valueArr1[13];
					    			$subject_code = $final_valueArr1[11];//die;

					    			$query = $this->db->query("select * from blended_cons_mrk where mem_no = '$mem_no' and exam_code = '$exam_code' and exam_period = '$exam_period' and subject_code = '$subject_code'");
					    			$row = $query->row_array();
					    			//print_r($row);die;
					    			
					    			if(count($row) > 0){
					    				$this->db->query("DELETE from blended_cons_mrk where exam_code = '$exam_code' and exam_period = '$exam_period' and subject_code = '$subject_code'");
					    			}
					    			...
					    			$this->db->query("DELETE from blended_cons_mrk where mem_no = 'MEMBER NO' and exam_period = 'EXM PRD' and subject_code = 'SUB CD' and exam_code = 'EXM CD'");
	...
	public function insertSOB_exam(){
		...
		if($fee_sql2 != ''){
			...
			$fee_insert2.=rtrim($fee_sql2,",".PHP_EOL).';';
			$insId4 = $this->db->query($fee_insert2);
			// echo 'fee_insert2---'.$this->db->last_query(); exit;
		}
	...
	function upload_admit_card_bcbf()
	{
		try
		{
		  error_reporting(E_ERROR); 

		  $csv_file = "uploads/Automation/admit_card_bcbf/1046_1047_02_Jan_2025.csv"; //change csv file name
		  //$exam_period = '6'; // change period given in mail
		  //1037,1038 = 10
		  //1046,1047 = 6
		  ...
		  if (($handle = fopen($csv_file, "r")) !== FALSE)
		  {
		    ...
		    while (($data = fgetcsv($handle, 2000, ",")) !== FALSE)
		    {
		      ...
		      //admitcard_info //bk_adm_info
		      $query = "INSERT INTO bk_adm_info(center_code, mem_type, mem_mem_no, g_1, mam_nam_1, mem_adr_1, mem_adr_2, mem_adr_3, mem_adr_4, mem_adr_5, mem_adr_6, mem_pin_cd, zo, state, exm_cd, sub_cd, m_1, inscd, insname, venueid, venueadd1, venueadd2, venueadd3, venueadd4, venueadd5, venpin, stat, pwd, date, time, mode, seat_identification) VALUES ('" . $center_code . "','" . $mem_type . "','" . $mem_mem_no . "','" . $g_1 . "','" . $mam_nam_1 . "','" . $mem_adr_1 . "','" . $mem_adr_2 . "','" . $mem_adr_3 . "','" . $mem_adr_4 . "','" . $mem_adr_5 . "','" . $mem_adr_6 . "','" . $mem_pin_cd . "','" . $zo . "','" . $state . "','" . $exm_cd . "','" . $sub_cd . "','" . $m_1 . "','" . $inscd . "','" . $insname . "','" . $venueid . "','" . $venueadd1 . "','" . $venueadd2 . "','" . $venueadd3 . "','" . $venueadd4 . "','" . $venueadd5 . "','" . $venpin . "','" . $stat . "','" . $pwd . "','" . $date . "','" . $time . "','" . $mode . "','" . $seat_identification . "')";
		    ...
		    echo "File data successfully imported to database. Total imported count is ".$cnt." and total fail count is ".$err_cnt;
		  }
		  else
		  {
		    echo "Error";
	...
	public function upload_admit_card_dra()
	{
		try{
			error_reporting(E_ERROR);
			
			$csv_file    = "uploads/Automation/admit_card_dra/DRA Admit Letter_Dec 2025(20).csv"; // change csv file name
			$exam_period = '20';// change period given in mail
	...
	function upload_demo_csv_file_with_img(){
		try
		{
			error_reporting(E_ERROR); 

			//require 'PHPExcel/PHPExcel.php'; // Include PHPExcel library

			$this->load->library('excel'); // Include PHPExcel library 

			$filePath = 'uploads/Automation/test_demo_csv_file/demo_reg_data.xls'; // Path to the Excel file
			...
			$excel_data =
		          '<style>
		            table { border: 1px solid #000; margin: 20px 0px; border-collapse: collapse; }
		            table th { border: 1px solid #000; padding: 5px 10px; text-align: center; white-space: nowrap; } 
		            table td { border: 1px solid #000; padding: 5px 10px; text-align: center; }
		          </style>

		          <table style="border: 1px solid #000; margin: 20px 0px; border-collapse: collapse;">
		            <thead>
		              <tr>
		                <th style="border: 1px solid #000; padding: 5px 10px; text-align: center; white-space: nowrap;">Sr No.</th>
		                <th style="border: 1px solid #000; padding: 5px 10px; text-align: center; white-space: nowrap;">First Name</th>
		                <th style="border: 1px solid #000; padding: 5px 10px; text-align: center; white-space: nowrap;">Last Name</th>
		                <th style="border: 1px solid #000; padding: 5px 10px; text-align: center; white-space: nowrap;">Reg Number</th>
		                <th style="border: 1px solid #000; padding: 5px 10px; text-align: center; white-space: nowrap;">Photo</th>
		                <th style="border: 1px solid #000; padding: 5px 10px; text-align: center; white-space: nowrap;">Signature</th>
		              </tr>
		            </thead>
			...
			{ 
			    ...
			    $regnumber = $worksheet->getCellByColumnAndRow(3, $row)->getValue(); 

			    // Extract images from the worksheet
			    foreach ($worksheet->getDrawingCollection() as $drawing) {
			    	//print_r($drawing->getCoordinates());die;
			        //if ($drawing instanceof PHPExcel_Worksheet_Drawing) {

			    		$coordinates = $drawing->getCoordinates(); // E.g., "E2" or "F2"
				        $imageExtension = 'jpg'; // Default extension 

				        // Get the image type
				        if ($drawing->getMimeType() == 'image/png') {
				            $imageExtension = 'png';
				        } elseif ($drawing->getMimeType() == 'image/gif') {
				            $imageExtension = 'gif';
				        }

				        if ($coordinates == 'E' . $row) {
				        	$photoPath = "p_".$regnumber.".".$imageExtension; 
			                $imageFilePath = "uploads/Automation/test_demo_csv_file/store_image/{$photoPath}";
			                // Get the image resource and save it
					        $imageResource = $drawing->getImageResource();
					        if ($imageResource) {
					            if ($imageExtension == 'png') {
					                imagepng($imageResource, $imageFilePath);
					            } elseif ($imageExtension == 'gif') {
					                imagegif($imageResource, $imageFilePath);
					            } else {
					                imagejpeg($imageResource, $imageFilePath);
					            }
					        }
			            } 
			            if ($coordinates == 'F' . $row) {
			            	$signPath = "s_".$regnumber.".".$imageExtension; 
			                $imageFilePath = "uploads/Automation/test_demo_csv_file/store_image/{$signPath}";
			                // Get the image resource and save it
					        $imageResource = $drawing->getImageResource();
					        if ($imageResource) {
					            if ($imageExtension == 'png') {
				        ...
				        // Define file path
				        //$imageFile = "uploads/Automation/test_demo_csv_file/{$coordinates}.{$imageExtension}";
			    ...
			    //echo $row.")".$regnumber."==".$photoPath."==".$signPath."<br>";

			    if($regnumber > 0){
			    	$this->db->select('id');
					$where = array('regnumber' => $regnumber);
	    			$check_exists = $this->master_model->getRecords('test_demo_csv_file_with_img', $where);

	    			if(count($check_exists) > 0){ 
	    				//$this->db->query('DELETE FROM test_demo_csv_file_with_img WHERE id ='.$check_exists[0]['id']); 
	    			}else{
	    				$query = "INSERT INTO test_demo_csv_file_with_img(firstname, lastname, regnumber, photo, sign) VALUES ('" . $firstname . "','" . $lastname . "','" . $regnumber . "','" . $photoPath . "','" . $signPath . "')";
						...
						$excel_data .='
		                  <tr>
		                    <td style="border: 1px solid #000; padding: 5px 10px; text-align: center;">'.$sr_no.'</td>
		                    <td style="border: 1px solid #000; padding: 5px 10px; text-align: center;">'.$firstname.'</td>
		                    <td style="border: 1px solid #000; padding: 5px 10px; text-align: center;">'.$lastname.'</td>
		                    <td style="border: 1px solid #000; padding: 5px 10px; text-align: center;">'.$regnumber.'</td>
		                    <td style="border: 1px solid #000; padding: 5px 10px; text-align: center;"><img src="'.base_url('uploads/Automation/test_demo_csv_file/store_image/'.$photoPath).'" width="100" height="100" /></td>
		                    <td style="border: 1px solid #000; padding: 5px 10px; text-align: left;"><img src="'.base_url('uploads/Automation/test_demo_csv_file/store_image/'.$signPath).'" style="max-width:200px;" /></td>
		                  </tr>'; 

			    //echo $row.")".$regnumber."==".$photoPath."==".$signPath."<br>";
			    // Insert into database
			    //$stmt->execute([$id, $firstname, $lastname, $regnumber, $photoPath, $signPath]);
			}
		    ...
		    echo "<br><br><br>File data successfully imported to database. Total imported count is ".$cnt." and total fail count is ".$err_cnt;
