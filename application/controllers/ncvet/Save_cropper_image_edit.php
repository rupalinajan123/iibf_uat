<?php 
  ...
  ********************************************************************************************************************/
  ...
  class Save_cropper_image_edit extends CI_Controller 
  {
    ...
    { 
      ...
      { /* _pa($_POST); _pa($_FILES); */
        ...
        if($_FILES['selected_image']['name'] != "")
        {
          ...
          $min_size_in_kb = '40'; //IN KB
          $max_size_in_kb = '5120'; //IN KB

          $minFileSize = 40 * 1024;      // 40 KB
          $maxFileSize = 5 * 1024 * 1024; // 5 MB

          $resize_width = $resize_height = '';
          $min_height= $min_width = '50';//IN PIXEL

          if($db_tbl_name == 'ncvet_candidates')
          {
            $id_proof_file_path = 'uploads/ncvet/id_proof';
            $aadhar_file_path   = 'uploads/ncvet/aadhar_file';
            ...
            $declaration_path = 'uploads/ncvet/declaration';
            $exp_certificate_path = 'uploads/ncvet/experience';
            $institute_idproof_path = 'uploads/ncvet/institute_idproof';
            $scanned_vis_imp_cert_path = $scanned_orth_han_cert_path = $scanned_cer_palsy_cert_path = 'uploads/ncvet/disability';
            ...
            {
              $upload_path = $aadhar_file_path."_cropper_temp";
              $new_file_name = "aadhar_".$file_name_str;
            ...
            {
              $upload_path = $scanned_vis_imp_cert_path."_cropper_temp";
              if($current_image_id == 'vis_imp_cert_img')
                $new_file_name = "vis_imp_".$file_name_str;
              if($current_image_id == 'orth_han_cert_img')
                $new_file_name = "orth_han_".$file_name_str;
              if($current_image_id == 'cer_palsy_cert_img')
                $new_file_name = "cer_palsy_".$file_name_str;
          ...
          if($upload_path != "" && $new_file_name != "")
          {
            ...
            $error=0;
            $fileType = $_FILES['selected_image']['type'];
            ...
            if($error==1) {
              $flag = "error2";
              $response_msg = 'Invalid File Extension';
            }
            else 
            {
              
              $upload_data = $this->Ncvet_model->upload_file("selected_image", array('jpg','jpeg','png','pdf'), $new_file_name, "./".$upload_path.'/'.date('Ymd'), "jpg|jpeg|png|pdf", '', '', '', '', '',$resize_width,$resize_height,$new_file_name,$minFileSize,$maxFileSize);
    ...
    function delete_old_folders()
    {
      ...
      $folder_arr = array('uploads/ncvet/photo_cropper_temp', 'uploads/ncvet/sign_cropper_temp', 'uploads/ncvet/id_proof_cropper_temp', 'uploads/ncvet/qualification_certificate_cropper_temp', 'uploads/ncvet/declaration_cropper_temp', 'uploads/ncvet/qualification_certificate_file_cropper_temp', 'uploads/ncvet/exp_certificate_cropper_temp','uploads/ncvet/experience_cropper_temp', 'uploads/ncvet/institute_idproof_cropper_temp','uploads/ncvet/aadhar_file_cropper_temp','uploads/ncvet/disability_cropper_temp');
