...
class Save_cropper_image extends CI_Controller
{
  ...
  function save_image()
  {
    ...
    $response_msg = "";
    $form_data = array();
    if (isset($_POST) && count($_POST) > 0) { /* _pa($_POST); _pa($_FILES); */
      ...
      if ($_FILES['selected_image']['name'] != "") {
        ...
        if ($db_tbl_name == 'ncvet_candidates') {
          ...
          if ($current_image_id == 'candidate_photo') {
            ...
            if ($current_image_id == 'scanned_vis_imp_cert')
              $new_file_name = "vis_imp_" . $file_name_str;
            if ($current_image_id == 'scanned_orth_han_cert')
              $new_file_name = "orth_han_" . $file_name_str;
            if ($current_image_id == 'scanned_cer_palsy_cert')
        ...
        if ($upload_path != "" && $new_file_name != "") {
          ...
          if ($error == 1) {
            ...
            $upload_data = $this->Ncvet_model->upload_file("selected_image", array('jpg', 'jpeg', 'png', 'pdf'), $new_file_name, "./" . $upload_path . '/' . date('Ymd'), "jpg|jpeg|png|pdf", '', '', '', '', '', $resize_width, $resize_height, $new_file_name);
  ...
  function compressImage($source_img = '', $destination_img = '', $min_height = 0, $min_width = 0, $max_size_in_kb = 0, $min_size_in_kb = 0)
  {
    ...
    switch ($source_info['mime']) {
      ...
      case 'image/gif':
        ...
        break;
      default:
        $result['message'] = 'Unsupported image type.';
        return $result;
    }
  ...
  function delete_old_folders()
  {
    ...
    foreach ($folder_arr as $dir_name) {
      if (strpos($dir_name, '_cropper_temp') !== false) {
      } else {
        $dir_name = $dir_name . '_cropper_temp';
      }
