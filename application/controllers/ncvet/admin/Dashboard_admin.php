<?php 
  /********************************************************************************************************************
  ** Description: Controller for NCVET Dashboard Page (Global : Admin) 
  ...
  ********************************************************************************************************************/
    ...
    public function index()
		{   
			...
			$data['sub_act_id'] = "";
      $data['page_title'] = 'IIBF - NCVET Admin Dashboard';

      //START : DASHBOARD COUNT DATA
      $all_exam_registration_data = $this->Ncvet_model->get_total_candidate_data();
     
      //START : FOR ALL REGISTRATION COUNT
      $data['total_candidate_cnt']                    = $all_exam_registration_data['total_candidate_cnt'];
      $data['total_kyc_completed_candidate_cnt']      = $all_exam_registration_data['total_kyc_completed_candidate_cnt'];
      
      $data['total_bfsi_candidate_cnt']               = $all_exam_registration_data['total_bfsi_candidate_cnt'];
      $data['total_kyc_completed_bfsi_candidate_cnt'] = $all_exam_registration_data['total_kyc_completed_bfsi_candidate_cnt'];

      $data['total_iibf_candidate_cnt']               = $all_exam_registration_data['total_candidate_cnt'] - $all_exam_registration_data['total_bfsi_candidate_cnt'];
      $data['total_kyc_completed_iibf_candidate_cnt'] = $all_exam_registration_data['total_kyc_completed_candidate_cnt'] - $all_exam_registration_data['total_kyc_completed_bfsi_candidate_cnt'];
      
      $data['total_advanced_exam_reg_cnt']         = 0;
      $data['total_basic_re_attempt_exam_reg_cnt'] = 0;
      //END : FOR ALL REGISTRATION COUNT

      $this->load->view('ncvet/admin/dashboard_admin', $data);
    }
    ...
    function change_password()
      ...
      { 
        $data['page_title'] = 'IIBF - NCVET Admin Change Password'; 

        $data['form_data'] = $form_data = $this->master_model->getRecords('ncvet_admin', array('admin_id' => $this->login_admin_id), 'admin_id, is_active, is_deleted');
        ...
        $log_slug = 'admin_self_password_action';
          ...
          $admin_name = $this->Ncvet_model->getLoggedInUserDetails($this->login_admin_id, $this->login_user_type);

          if($this->login_user_type == "admin")
          {
            $up_data['admin_password'] = $this->Ncvet_model->password_encryption($this->input->post('new_pass_admin'));
            $up_data['updated_on'] = date("Y-m-d H:i:s");
            $up_data['updated_by'] = $this->login_admin_id;
            $this->master_model->updateRecord('ncvet_admin', $up_data, array('admin_id' => $this->login_admin_id));
            
            $this->Ncvet_model->insert_common_log('Admin : Profile password updated', 'ncvet_admin', $this->db->last_query(), $this->login_admin_id,'admin_self_password_action','The '.$admin_name['disp_name'].' has successfully updated the password.', $posted_arr); 
          }  
					$this->session->set_flashdata('success','Password successfully updated');
					
          redirect(site_url('ncvet/admin/dashboard_admin/change_password'));
				}
			}
			
      $data["enc_login_admin_id"] = url_encode($this->login_admin_id);
			$this->load->view('ncvet/admin/change_password_admin', $data);
    ...
    /******** START : VALIDATION FUNCTION TO CHECK THE OLD PASSWORD ********/
      ...
      $return_val_ajax = 'false';
        ...
        if($type == '1') { $current_pass_admin = $this->security->xss_clean($this->input->post('current_pass_admin')); }
				...
				$enc_password = $this->Ncvet_model->password_encryption($current_pass_admin);

        if($this->login_user_type == "admin") 
        { 
          if(count($this->master_model->getRecords('ncvet_admin', array('admin_password' => $enc_password, 'admin_id' => $this->login_admin_id, 'is_active' => '1', 'is_deleted' => '0'), 'admin_id, is_active, is_deleted')) > 0)
          {
