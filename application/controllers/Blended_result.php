...
//Result for Blended login 
			...
			$config = array(
				...
				if ($this->form_validation->run() == TRUE){
					...
					if($this->db->table_exists('blended_marks') ){
						...
						if(count($meminfo) > 0){    
							//marks table
						 	$user_info= $this->master_model->getRecords('blended_marks', array(
							...
							if(count($user_info) > 0){ 
							    ...
							    //echo "iibf"; die;
        						// member detils 
					 			$mem_info= $this->master_model->getRecords('blended_members', array(
						    ...
						    //consolidatedmarks table
						 	$user_info= $this->master_model->getRecords('blended_cons_mrk', array(
        									'exam_code' =>$this->input->post('exam'),
	...
	public function blended_result_advice(){
		...
		if($this->db->table_exists('blended_members')){
			//member detils 
 			$mem_info= $this->master_model->getRecords('blended_members', array('exam_code'=>$exam_code, 'exam_period' => $exam_period, 'member_number' => $member_id) );

			//marks table
			$this->db->group_by('exam_id,exam_period,subject_id');
			$marks_info= $this->master_model->getRecords('blended_marks', array('exam_id' =>$exam_code,
						'regnumber' => $member_id) );
