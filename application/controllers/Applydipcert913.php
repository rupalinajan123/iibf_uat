...
class Applydipcert913 extends CI_Controller {
...
public function login(){ 
	...
	if(!isset($_COOKIE["dipcert913"]) || $_COOKIE["dipcert913"]==0)
	{
		redirect('https://iibf.esdsconnect.com/instructoinLoginexam913.php');
	}
	...
	if(isset($_POST['submit'])){  
			...
			if ($this->form_validation->run() == TRUE){
				...
				$chk_old_app = $this->master_model->getRecords('admit_card_details_913','','admitcard_id');
				...
				$chk_app = $this->master_model->getRecords('admit_card_details','','admitcard_id,exm_prd,exm_cd,app_update');
				 ...
				 if(count($chk_app) > 0 && count($chk_old_app) > 0){
					...
					if($chk_app[0]['app_update'] != 0){
						 ...
						 redirect(base_url().'Applydipcert913/comApplication');
					}
				 }else{
	...
	$this->load->view('examlogin913',$data);
}
public function comApplication(){
	
	$data = array();
	if($this->session->userdata('memexcode')==''){
		redirect(base_url().'Applydipcert913/login');
	}
	...
	if(isset($_POST['btnPreviewSubmit'])){
		...
		if($this->form_validation->run()==TRUE){
			...
			$log_title ="Dipcert 913 free Member exam apply details";
			...
			redirect(base_url().'Applydipcert913/preview');
	...
	$data=array('middle_content'=>'dipcert913free_comApplication','user_info'=>$user_info,'examinfo'=>$examinfo,'compulsory_subjects'=>$compulsory_subjects,'center'=>$center,'old_center'=>$old_center);
...
public function preview(){
	...
	$data=array('middle_content'=>'dipcert913free_preview','user_info'=>$user_info,'examinfo'=>$examinfo,'compulsory_subjects'=>$this->session->userdata['examinfo']['subject_arr'],'center'=>$center);
	$this->load->view('memapplyexam/mem_apply_exam_common_view',$data);
}
	...
	if(!empty($this->session->userdata['examinfo']['subject_arr'])){
		...
		if($admitcard_pdf != ''){
			...
			if($this->Emailsending->mailsend_attch($info_arr,$files)){
				...
				redirect(base_url().'Applydipcert913/mail_success');
			}else{
				...
				redirect(base_url().'Applydipcert913/mail_fail');
			}
		}
	}
}
...
public function mail_success(){
	...
	$data=array('middle_content'=>'dipcert913free_success','user_info'=>$user_info,'examinfo'=>$examinfo,'center'=>$center);
	$this->load->view('memapplyexam/mem_apply_exam_common_view',$data);
}
public function mail_fail(){
	...
	$data=array('middle_content'=>'dipcert913free_fail','user_info'=>$user_info,'examinfo'=>$examinfo,'center'=>$center);
	$this->load->view('memapplyexam/mem_apply_exam_common_view',$data);
}
