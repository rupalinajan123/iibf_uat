...
class Dbftojaiib extends CI_Controller
{
    ...
    public function login()
    {
        ...
        $data = array('middle_content' => 'dbftojaiib/login_search', 'aCandidate' => $aCandidate);
      
        $this->load->view('dbftojaiib/common_view_fullwidth', $data);
    }
	...
	public function get_alredy_applied_examname($regnumber=NULL)
	{
		...
		$this->db->select('dbftojaiib_registrations.*');
		$applied_exam_date=$this->master_model->getRecords('dbftojaiib_registrations',array('regnumber'=>($regnumber),'pay_status '=>1));
		if(count($applied_exam_date) > 0)
		{
			$msg='You have already submitted application for DBF to JAIIB Certificate conversion. ';
	...
	//	error_reporting(E_ALL);ini_set('display_errors', '1');
        ...
        $regnumber = base64_decode($regnumber);
		//$regnumber = '1017843';
        $otp_data = $this->db->query("SELECT * FROM verify_otp_dbftojaiib 
        ...
        if (empty($otp_data)) {
            ...
            redirect(base_url() . 'Dbftojaiib/login');
            exit;
        }
		...
		if ($regnumber) {
			...
			if (count($aCandidate) > 0) {
					...
					if( isset($aCandidate['regnumber'])){
						...
						{
							$regnumber= $valcookie;
							$checkpayment=$this->master_model->getRecords('payment_transaction',array('member_regnumber'=>$regnumber,'status'=>'2','pay_type'=>$this->config->item('dbftojaiib_pay_type')),'',array('id'=>'DESC'));
						...
						if($cookieflag==0)
						{
							$data=array('middle_content'=>'dbftojaiib/exam_apply_cms_msg');
							return $this->load->view('dbftojaiib/nm_common_view',$data);
						}
						if($flag==0 && $cookieflag==1)
						{
							$data=array('middle_content'=>'dbftojaiib/not_eligible','check_eligibility'=>$message);
							return $this->load->view('dbftojaiib/nm_common_view',$data);
						}
		...
		$fee_details = $this->master_model->getRecords('dbftojaiib_fee_master', array('fee_delete' => '0'));
			$fee = $fee_details[0];

		if(isset($_POST['btnSubmit']))  	
		{

            	$this->form_validation->set_rules('certificate_number','Certificate Number','trim|max_length[30]|required|alpha_numeric|xss_clean|callback_cross_check_certificate');
				...
				$this->form_validation->set_rules('certificate_date','Certificate Date','trim|required|xss_clean');
				...
				$this->form_validation->set_rules('bank_joining_date','Date of Bank Joining','trim|required|xss_clean|callback_valid_date');

				if(!isset($_POST['hiddenoffer_letter']) || !file_exists('./uploads/offer_letter/'.$_POST['hiddenoffer_letter']) ) {
					//$this->form_validation->set_rules('offer_letter','Offer Letter Document','file_required|file_allowed_type[pdf]|file_size_max[1000]|callback_offer_letter_upload');
				}
				...
				if($this->form_validation->run()==TRUE)
				{
					...
					if($errFlag==1) {
						...
						redirect(base_url().'Dbftojaiib/candidate_details/'.base64_encode($regnumber).'');
						exit;
					}
					...
					$user_data=array(	'display_name'				=>$_POST["display_name"],
										'mobile'				=>$_POST["mobile"],
										'dateofbirth'			=>$_POST["dateofbirth"],
										'email'					=>$_POST["email"],
										'dbf_regnumber'			=>$_POST["dbf_regnumber"],
					...
					$log_title ="DBFTOJAIIB session enduserinfo:".$regnumber;
					$log_message = serialize($user_data);
					$rId = $regnumber;
					$regNo = $regnumber;
					storedUserActivity($log_title, $log_message, $rId, $regNo);

					redirect(base_url().'Dbftojaiib/preview');
        ...
        $data = array(
			'middle_content' => 'dbftojaiib/dbftojaiib_form','aCandidate' => $aCandidate,'regnumber'=>$regnumber,'fee'=>$fee
		);
        $this->load->view('dbftojaiib/common_view_fullwidth', $data);
    }
	...
	public function cross_check_certificate_func(){
		$url = "http://10.10.233.76:8091/dbfToJAIIB/".$this->input->post('dbf_regnumber')."/".$this->input->post('certificate_number'); //certificate verification
		...
		else 
		{
			...
			//	echo'<pre>==';print_r($result);exit;
				if(count($result) > 0 && isset($result['certificate_date']))
				{ 
					return $result;
				}
				else
	...
	public function ajax_cross_check_dob() //priyanka d >> 11-july-23 added below condition
	{
		...
		if($dbfmemberdetails && $omemberdetails ) {
			...
			if(date('Y-m-d',strtotime($dbfmemberdetails[0]['dateofbirth'])) != date('Y-m-d',strtotime($omemberdetails[0]['dateofbirth'])))
			{
				...
				echo json_encode($dbfmemberdetails[0]);
			}
		}
		...
		//echo "IPPB Service is not available due to maintenance activity from 5 PM to 07 PM."; exit;
         ...
         if(!$this->session->userdata('enduserinfo'))
         {
            redirect(base_url());
					 	...
					 	$log_title ="dbftojaiib session enduserinfo on preview page:".$this->session->userdata['enduserinfo']['regnumber'];
         ...
         $data=array('middle_content'=>'dbftojaiib/preview_form','enduserinfo'=>$this->session->userdata['enduserinfo']);
         $this->load->view('dbftojaiib/common_view_fullwidth',$data);
    }
    ...
    public function accessdenied()
	{
        ...
        $data=array('middle_content'=>'dbftojaiib/access-denied-registration','check_eligibility'=>$message);
        $this->load->view('dbftojaiib/common_view_fullwidth',$data);
	}
	...
	public function send_otp()
	{
		if (isset($_POST['regnumber']) && !empty($_POST['regnumber'])) {
			...
			$this->db->delete('verify_otp_dbftojaiib', array('regnumber' => $_POST['regnumber']));
			$data = array(
			...
			$this->db->insert('verify_otp_dbftojaiib', $data);
	...
	function resend_otp()
	{
		if (isset($_POST['otp_id']) && !empty($_POST['otp_id'])) {
			...
			$this->db->update('verify_otp_dbftojaiib', $data, $where);
			echo json_encode(array(
				'status' => 'success',
				'sec' => $this->otptime,
				'user_otp_send_on' => $user_otp_send_on,
	...
	function verify_otp()
	{
		if (isset($_POST['submitted_otp']) && !empty($_POST['submitted_otp']) && isset($_POST['otp_id']) && !empty($_POST['otp_id'])) {
			...
			$otp_id = $_POST['otp_id'];
			$otp_data = $this->db->query("Select * from verify_otp_dbftojaiib where otp_id=" . $otp_id)->row();
	...
	public function send_otp_via_api($otp, $stud_arr)
	{
		...
		$emailerstr1 = $this->master_model->getRecords('emailer', array('emailer_name' => "dbftojaiib_email"));
		...
		$ippb_otp_mail_arr = array(
			'to' => $stud_arr['email'],
			//'cc'=>'priyanka.dhikale@esds.co.in',
			'subject' => 'Login OTP of DBf to JAIIB conversion From IIBF',
    ...
    public function addmember()
    {
        ...
        $check_duplication = $this->master_model->getRecords('dbftojaiib_registrations', array('regnumber' => $this->session->userdata['enduserinfo']['regnumber'],'pay_status' => '1' ));
       ...
       // echo $this->db->last_query();exit;
        ...
        
        {
            ...
            $certificate_number = ($this->session->userdata['enduserinfo']['certificate_number']);
            $certificate_date = date('Y-m-d',strtotime($this->session->userdata['enduserinfo']['certificate_date']));
            ...
            if(count($check_duplication) <= 0)
           	{
                ...
                if($last_id =$this->master_model->insertRecord('dbftojaiib_registrations',$insert_info,true))
                {
								$this->session->userdata['enduserinfo']['dbftojaiib_registration_id'] = $last_id;
                                $this->session->set_userdata('non_memberdata', $this->session->userdata['enduserinfo']);
								
                                if($this->config->item('exam_apply_gateway')=='sbi')
                                {
                                    redirect(base_url().'Dbftojaiib/sbi_make_payment/');
                                }
	...
	//	error_reporting(E_ALL);ini_set('display_errors', '1');
      ...
      //  $this->db->join('exam_invoice','exam_invoice.receipt_no=payment_transaction.receipt_no');
        $this->db->join('payment_transaction','payment_transaction.ref_id=dbftojaiib_registrations.id');
		$this->db->where('payment_transaction.pay_type',$this->config->item('dbftojaiib_pay_type'));
        $result=$this->master_model->getRecords('dbftojaiib_registrations',array('dbftojaiib_registrations.regnumber'=>$payment_info[0]['member_regnumber']));
		//echo $this->db->last_query(); exit;
        ...
        $this->load->view('dbftojaiib/profile_thankyou',$data);
    }
	...
	public function sbi_make_payment()
	{
      ...
      $cs_total=$igst_total='';
        ...
        if ($valcookie) {
            ...
            if (count($checkuser) > 0) {
                ...
                $checkpayment = $this->master_model->getRecords('payment_transaction', array(
                    'pay_type' => $this->config->item('dbftojaiib_pay_type'),
					'member_regnumber'=>$this->session->userdata('enduserinfo')['regnumber'],
                    'status' => '2'
                ));
                if (count($checkpayment) > 0) {
...

		{
			...
			$log_title ="Dbftojaiib - Clicked on processPayment ".$this->session->userdata['enduserinfo']['regnumber'];
						$log_message = serialize($this->session->userdata('enduserinfo'));
						$rId = $this->session->userdata['enduserinfo']['regnumber'];
						$regNo = $this->session->userdata['enduserinfo']['regnumber'];
						storedUserActivity($log_title, $log_message, $rId, $regNo);
			...
			$regno = $this->session->userdata['non_memberdata']['regnumber'];
...

			{
				$today_date=date('Y-m-d');
				$this->db->where("('$today_date' BETWEEN fr_date AND `to_date`)");
				$fee_details = $this->master_model->getRecords('dbftojaiib_fee_master', array(
			...
			$insert_data = array(
				...
				'ref_id'           => $this->session->userdata['enduserinfo']['dbftojaiib_registration_id'],

				'description'      => 'DBF to JAIIB Certificate Conversion',
			...
			if($getstate[0]['exempt']=='E')
			{
				...
				$center_details = $this->master_model->getRecords('center_master', array(
                    ...
                    'exam_name' => $this->config->item('examCodeJaiib') ,'exam_period'=>'999'
                ));
			...
			$log_title ="DBFTOJAIIB exam invoice insertion :".$inser_id;
			...
			if($inser_id==''){
				...
				$log_title ="DBFTOJAIIB exam invoice insertion again :".$inser_id;
		...
		
			{
                ...
                $custom_field_billdesk    = $regnumber . "-" . $pg_flag . "-" . $MerchantOrderNo . "-" . $regnumber;
                $billdesk_res = $this->billdesk_pg_model->init_payment_request($MerchantOrderNo, $amount, $regnumber, $regnumber, '', 'Dbftojaiib/handle_billdesk_response', '', '', '', $custom_field_billdesk);
    ...
    public function handle_billdesk_response()
    {
        ...
        if (isset($_REQUEST['transaction_response'])) 
			...
			{				
                ...
                $this->master_model->updateRecord('payment_transaction', $update_data, array('receipt_no' => $MerchantOrderNo,'pay_type'=>$this->config->item('dbftojaiib_pay_type')));
				
				$log_title   = "DBFTOJAIIB payment paymentupdate";
				...
				if($this->db->affected_rows())
				{
					if (count($user_payment_txn_info) > 0) {
                        ...
                        $this->master_model->updateRecord('dbftojaiib_registrations', $update_mem_data, array('id' => $user_payment_txn_info[0]['ref_id']));
                    }
					$dbftojaiib_registrations_details = $this->master_model->getRecords('dbftojaiib_registrations', array('id' => $user_payment_txn_info[0]['ref_id']));
					...
					$log_title   = "DBFTOJAIIB get invoice details :" . $reg_id;
                    ...
                    if (count($getinvoice_number) > 0) {
                        ...
                        if ($invoiceNumber) {
                            $invoiceNumber = 'DJ/24-25/'. $invoiceNumber;
                        }
                        ...
                        $attachpath = genarate_dbftojaiib_invoice($getinvoice_number[0]['invoice_id'],$dbftojaiib_registrations_details[0]['dbf_regnumber']);
                        $log_title   = "DBFTOJAIIB Invoice log update  :" . $reg_id;
                        $log_message = serialize($this->db->last_query());
                        storedUserActivity($log_title, $log_message, $reg_id, $reg_id);
                    }

                   $emailerstr=$this->master_model->getRecords('emailer',array('emailer_name'=>'dbftojaiib_email'));
                    if(count($emailerstr) > 0)
                    {  
						$log_title   = "DBFTOJAIIB email to send  :" . $reg_id;
                        $log_message = serialize($this->db->last_query());
                        storedUserActivity($log_title, $log_message, $reg_id, $reg_id);
                      /*'to'=>$email,*/
                      $final_str = $emailerstr[0]['emailer_text'];
                                ...
                                $info_arr = array(
                                'to'=>$dbftojaiib_registrations_details[0]['email'],
                          ...
                          if($attachpath!='')
                          {
                            ...
                            if($this->Emailsending->mailsend_attch($info_arr,$attachpath)){
								$log_title   = "DBFTOJAIIB email to send  attachpath :" . $attachpath;
				...
				{
					$log_title ="DBFTOJAIIB B2B Update fail:".$user_payment_txn_info[0]['member_regnumber'];
					$log_message = serialize($update_data);
					$rId = $MerchantOrderNo;
				...
				redirect(base_url().'Dbftojaiib/acknowledge/'.base64_encode($MerchantOrderNo));
				exit();
                //-----------------------------------------------------------
		...
		{
			redirect(base_url('/dbftojaiib/trans_under_process'));
		}
    ...
    {     
		...
		$this->load->view('dbftojaiib/trans_under_process');
	}
	...
	public function already_apply($exam_period_date=null)
	{
        ...
        $data=array('middle_content'=>'dbftojaiib/already_apply','check_eligibility'=>$message);
        $this->load->view('dbftojaiib/mem_apply_exam_common_view',$data);
	}
