  ...
  ********************************************************************************************************************/
    ...
    public function index()
     ...
     /* if($this->login_user_type == "candidate") 
      ...
      if(isset($_POST) && !empty($_POST) && isset($_POST['exam_code_period'])) {
        ...
        $this->db->where("id",$this->login_candidate_or_centre_id);
        $this->db->update("supervision_candidates",$update_data); 

        $this->session->set_flashdata('success','Details are saved successfully');
          redirect(site_url('/supervision/candidate/dashboard_candidate')); 
      ...
      $this->load->view('supervision/candidate/setting_candidate', $data);
    }
        ...
        if($form_id !=0 && count($form_data)>0 &&  $form_data[0]['pay_status']==1) { 
          ...
          {     
            ...
            if($data['candidate_data'][0]['role_id']!='' && $data['candidate_data'][0]['role_id']!=0) {
              $role_fee = $this->master_model->getRecords('supervision_role_fee am', array('am.is_deleted' => '0','am.role_id' => $data['candidate_data'][0]['role_id'],'exm_period' => $exam_code_period[1]));
            }
