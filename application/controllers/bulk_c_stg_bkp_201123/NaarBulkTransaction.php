...
class NaarBulkTransaction extends CI_Controller {
	...
	public function naar_getadmitcardpdfsp()
	{
		...
		$this->db->select('mem_mem_no, mam_nam_1, mem_adr_1, mem_adr_2, mem_adr_3, mem_adr_4, mem_adr_5, mem_pin_cd, mode, pwd, center_code, m_1,vendor_code,center_code');
	...
	public function transactions()
	{
		...
		$data['middle_content']	= 'bulk/transaction/naartransactions';
	...
	public function getTransactions()
	{
		...
		if($res)
		{
			...
			foreach($result as $row)
			{
				...
				// action -
				$action = '<a href="'.base_url().'bulk/NaarBulkTransaction/view_inst_receipt/'.base64_encode($row['id']).'" target="_blank">Receipt</a>';
				...
				if($invoice_img_path != '')
				{
					if($date_invoice < '2021-01-01' && $date_invoice !=''){
						...
						if($exam_invoice[0]['gstin_no'] == ''){
							...
							$action .= ' <br> | <a href="'.base_url().'bulk/NaarBulkTransaction/getInvoice/'.base64_encode($str_invoice_no).'">E-Invoice</a>';
						}
					}
				}
				...
				{
					$action .= ' <br> | <a href="'.base_url().'bulk/NaarBulkTransaction/mem_admit_card_list/'.base64_encode($row['id']).'" target="_blank">Admit Card</a>';
				}
			...
			$url = base_url()."bulk/NaarBulkTransaction/getTransactions/";
	...
	public function mem_admit_card_list($txn_id)
	{
		...
		// get list of all members for this payment transaction -
		$select = 'member_registration.regid,member_registration.regnumber,member_registration.firstname,member_registration.middlename,member_registration.lastname,member_exam.exam_code,member_exam.exam_period';	
		$this->db->join('member_exam','member_exam.id = bulk_member_payment_transaction.memexamid','LEFT');
		$this->db->join('member_registration','member_registration.regnumber = member_exam.regnumber','LEFT');
		$this->db->where('member_registration.isactive = "1"');
		$this->db->where('bulk_member_payment_transaction.ptid = '.$txn_id);
	...
	public function neft_transactions()
	{
		...
		$data['middle_content']	= 'bulk/transaction/neft_naartransactions';
       	$this->load->view('bulk/bulk_naarcommon_view', $data); 
	}
	...
	public function getNeftTransactions()
	{
		...
		$this->db->where("bulk_payment_transaction.inst_code",$this->session->userdata('institute_id'));
		...
		if($res)
		{
			...
			foreach($result as $row)
			{
				...
				if($row['status'] == 3)	// status = 3 for Applied
				{
						...
						$action = '<a href="'.base_url().'bulk/NaarBulkTransaction/view_inst_pr_invoice/'.$row['id'].'" target="_blank">Proforma Invoice</a></span> |  <br> <span data-id="'.$row['id'].'"><a href="javascript:void(0)" onclick="confirmVerify('.$row['id'].');">Update UTR </a></span>  |<br> <span data-id="'.$row['id'].'"><a href="'.base_url().'bulk/NaarBulkTransaction/performa_invoice_details/'.base64_encode($row['id']).'" target="_blank">View more </a></span>';
			...
			if(count($result))
			{
				...
				$url = base_url()."bulk/NaarBulkTransaction/getNeftTransactions/";
	...
	public function performa_invoice_details(){
		...
		$this->load->view('bulk/admin/transaction/perdorma_details',$data);
	}
