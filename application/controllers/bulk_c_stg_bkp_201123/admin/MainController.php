...
class MainController extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('bulk/admin/home/home', $data);	
	}
	public function getUserInfo(){
		$data['BulkUser'] = $this->UserModel->getBulkUserInfo( $this->UserID );
		return $data;
	}
	...
	/*public function total_reg_dra_exam($current_date = "")
	{
		...
		$this->db->where($where);
		$res = $this->UserModel->getRecords("bulk_member_exam ", $select, '', '', '', '', '', '');
	...
	/*public function total_reg_reattempt($current_date = "")
	{
		...
		$sql = "SELECT regid, COUNT(*) - 1 AS total FROM bulk_member_exam WHERE " . $where . " GROUP BY regid, exam_code Having COUNT(*) > 1"; 
