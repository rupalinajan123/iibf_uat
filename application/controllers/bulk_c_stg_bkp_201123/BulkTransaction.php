...
class BulkTransaction extends CI_Controller {
	public function __construct(){
        ...
        $this->elearning_course_code =  [528,529,530];
	}
	...
	public function naar_getadmitcardpdfsp()
	{
		...
		if($exam_code == 1002 || $exam_code == 1003 || $exam_code == 1004 || $exam_code == 1005 || $exam_code == 1006 || $exam_code == 1007 || $exam_code == 1008 || $exam_code == 1009 || $exam_code == 1010 || $exam_code == 1011 || $exam_code == 1012 || $exam_code == 1013 || $exam_code == 1014 || $exam_code == 1015 || $exam_code == 1017 || $exam_code == 1018 || $exam_code == 1026 || $exam_code == 1027 || $exam_code == 1029  || $exam_code == 1028 || $exam_code == 1030){
	...
	public function getTransactions()
	{
		...
		if($res)
		{
			...
			foreach($result as $row)
			{
			  ...
			  ;
				// admit card -
				if($pay_status == 1 && ! in_array($row['exam_code'], $this->elearning_course_code)) // if payment status is Success
				{
	...
	public function mem_admit_card_list($txn_id)
	{
		...
		// get list of all members for this payment transaction -
		$select = 'member_registration.regid,member_registration.regnumber,member_registration.firstname,member_registration.middlename,member_registration.lastname,member_exam.exam_code,member_exam.exam_period,member_registration.bank_emp_id';	
		$this->db->join('member_exam','member_exam.id = bulk_member_payment_transaction.memexamid','LEFT');
		$this->db->join('member_registration','member_registration.regnumber = member_exam.regnumber','LEFT');
		$this->db->where('member_registration.isactive = "1"');
		$this->db->where('bulk_member_payment_transaction.ptid = '.$txn_id);
