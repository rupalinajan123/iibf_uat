...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Citap extends CI_Controller 
	{		
	public function __construct()
    {
      ...
      $this->load->model('billdesk_pg_model');
			...
			$this->load->helper('dbftojaiib_invoice_helper');
  ...
  public function login()
  {
    ...
    if ($this->input->post('form_type') == 'search_form') 
    {
      ...
      if ($this->form_validation->run() == TRUE) 
      {
        ...
        $aCandidate_ar = $this->master_model->getRecords('member_registration', array('regnumber' => $searchStr,'isdeleted' => '0','registrationtype'=>'O'));
				...
				if(!empty($aCandidate_ar)) 
				{
					...
					$aes->require_pkcs5();
					$decpass = $aes->decrypt(trim($aCandidate[0]['usrpassword']));

					$this->session->set_userdata('aCandidate_ar',$aCandidate);

					$mysqltime=date("H:i:s");
					$user_data=array('mregid_applyexam'=>$aCandidate_ar[0]['regid'],
												'mregnumber_applyexam'=>$aCandidate_ar[0]['regnumber'],
												'mfirstname_applyexam'=>$aCandidate_ar[0]['firstname'],
												'mmiddlename_applyexam'=>$aCandidate_ar[0]['middlename'],
												'mlastname_applyexam'=>$aCandidate_ar[0]['lastname'],
												'mtimer_applyexam'=>base64_encode($mysqltime),
												'memtype'=>$aCandidate_ar[0]['registrationtype'],
												'verify_otp'=>'No',
												'mpassword_applyexam'=>base64_encode($decpass));
					$this->session->set_userdata($user_data);
					
					$sess  = $this->session->userdata();
					$flag  = $this->checkusers($this->exam_code);
					$check = $this->examapplied($this->security->xss_clean($this->input->post('searchStr')),base64_encode($this->exam_code));
				
					if($flag==0) 
					{
						$data['error'] = '<span style="">You are not eligible to register for Certified Information Technology Audit Professional (CITAP)</span>';
					}
					else if($check)
					{
						$data['error'] = '<span style="">You are already applied for this exam.</span>';
					}
					else
					{
						$data['aCandidate'] = $aCandidate; 
					}
				}
			  else if($aCandidate_ar[0]['isactive']==0)
			  {
    ...
    $data['middle_content'] = 'CITAP/login_search';
		// $data = array('middle_content' => 'CITAP/login_search','aCandidate' => $aCandidate);  
    $this->load->view('CITAP/common_view_fullwidth', $data);
	}
  ...
  public function send_otp()
	{
		if (isset($_POST['regnumber']) && !empty($_POST['regnumber'])) {
			...
			$user_otp_expired_on = date('Y-m-d H:i:s', strtotime('+10 minutes', strtotime($user_otp_send_on)));
			...
			$this->db->delete('verify_otp_citap', array('regnumber' => $_POST['regnumber']));
			$data = array(
				'regnumber' => $_POST['regnumber'],
				'email' => $_POST['email'],
				'mobile' => $_POST['mobile'],
			...
			$this->db->insert('verify_otp_citap', $data);
  ...
  public function generate_otp()
	...
	public function send_otp_via_api($otp, $stud_arr)
	{
		...
		$emailerstr1 = $this->master_model->getRecords('emailer', array('emailer_name' => "citap_email"));
		
		$sms_final_str1 = str_replace("#OTP#",$otp,  $emailerstr1[0]['sms_text']);
		$email_final_str1 = str_replace("#OTP#",$otp,  $emailerstr1[0]['emailer_text']);
		...
		$ippb_otp_mail_arr = array(
			'to' => $stud_arr['email'],
			'cc'=>'gaurav.shewale@esds.co.in',
			'subject' => 'Login OTP of CITAP From IIBF',
			'message' => $email_final_str1,
		);
	...
	function resend_otp()
	{
		if (isset($_POST['otp_id']) && !empty($_POST['otp_id'])) {
			...
			$this->db->update('verify_otp_citap', $data, $where);
			echo json_encode(array(
				'status' => 'success',
				'sec' => $this->otptime,
				'user_otp_send_on' => $user_otp_send_on,
	...
	function verify_otp()
	{
		if (isset($_POST['submitted_otp']) && !empty($_POST['submitted_otp']) && isset($_POST['otp_id']) && !empty($_POST['otp_id'])) {
			...
			$otp_data = $this->db->query("Select * from verify_otp_citap where otp_id=" . $otp_id)->row();
			...
			if (!empty($otp_data)) {
				...
				if (strtotime($otp_data->user_otp_expired_on) < strtotime(date('Y-m-d H:i:s'))) {
					...
					{
						$flag  = $this->checkusers($this->exam_code);
						$check = $this->examapplied($this->session->userdata('mregnumber_applyexam'),base64_encode($this->exam_code));
					
						if($flag==0) 
						{
							echo json_encode(array(
								'status' => 'failed',
								'msg' => 'You are not eligible to register for Certified Information Technology Audit Professional (CITAP)'
							));
							exit;
						}
						else if($check)
						{
							echo json_encode(array(
								'status' => 'failed',
								'msg' => 'You are already applied for this exam.'
							));
							exit;
						}
						else
						{
							...
							$this->session->set_userdata('verify_otp', 'Yes');
  ...
  function logout()
  {
    $user_data=array('aCandidate_ar'=>'','mregid_applyexam'=>'', 'mregnumber_applyexam'=>'', 'mfirstname_applyexam'=>'', 'mmiddlename_applyexam'=>'', 'mlastname_applyexam'=>'', 'mtimer_applyexam'=>'', 'memtype'=>'', 'mpassword_applyexam'=>'');
    $this->session->set_userdata($user_data);
    redirect(site_url('Citap/login'));
  }
	...
	{  
    	$this->check_admin_session_all_pages();
		$dataarr = array("regnumber"=>$this->session->userdata('mregnumber_applyexam'));
		
		$verify_otp_flag = $this->session->userdata('verify_otp');
		
		if($verify_otp_flag == "No")
		{
			redirect(site_url('Citap/login'));
		}
		...
		$this->load->view('CITAP/CITAP_DASHBOARD',$data);			
	}
	...
	{
    $this->check_admin_session_all_pages();
		$message='<div style="color:#F00">You are not eligible to register for Certified Information Technology Audit Professional (CITAP)</div>';
		$data=array('middle_content'=>'CITAP/not_eligible','check_eligibility'=>$message);
  	    $this->load->view('CITAP/mem_apply_exam_common_view',$data);
	}
	
	public function alreadyapplied()
	{
    $this->check_admin_session_all_pages();
		 $message='<div style="color:#F00">You have already applied for Certified Information Technology Audit Professional (CITAP) </div>';
		 $data=array('middle_content'=>'CITAP/not_eligible','check_eligibility'=>$message);
		 $this->load->view('CITAP/mem_apply_exam_common_view',$data);
	}
	...
	{		
			...
			if($flag==0)
			{
				redirect(base_url().'Citap/accessdenied/');
			}
			...
			//{
              ...
              $check_qualify_exam_eligibility = $this->master_model->getRecords('exam_activation_master',array('exam_code'=>$examcode),'exam_code');
              $eligible_response = array();
              if(count($check_qualify_exam_eligibility) > 0)
              {
                ...
                $eligible_response = $this->master_model->disa_api_curl($exam_code, $exam_period, $member_number);
              }

              //if(count($check_eligibility_for_applied_exam) > 0)
							if(isset($eligible_response['flag']) && $eligible_response['flag'] == 'success')
							{
                ...
                if(isset($eligible_response['eligible_flag']) && ($eligible_response['eligible_flag'] == 'N' || $eligible_response['eligible_flag'] == ''))
                {
                  $flag=0;
									$message= 'You are not eligible';
                }
                else if(isset($eligible_response['eligible_flag']) && $eligible_response['eligible_flag'] == 'Y')
								{
			 ...
			 if($is_transaction_doone >0)
			 {
				...
				{
					$data=array('middle_content'=>'DISA/exam_apply_cms_msg');
					$this->load->view('DISA/mem_apply_exam_common_view',$data);
				}
				else if($flag==0 && $cookieflag==1)
				{
					if($profile_flag==0)
					{
					 ...
					 $data=array('middle_content'=>'DISA/not_eligible','check_eligibility'=>$message);
					 $this->load->view('DISA/mem_apply_exam_common_view',$data);
				}
				...
				else if(count($applied_exam_info) > 0)
				{
					...
					if($profile_flag==0)
					{
					 ...
					 $data=array('middle_content'=>'DISA/already_apply','check_eligibility'=>$message);
					 $this->load->view('DISA/mem_apply_exam_common_view',$data);
				}
				else if($cookieflag==1 && $profile_flag==1)
				{
					...
					$data=array('middle_content'=>'DISA/cms_page','exam_info'=>$exam_info);
					$this->load->view('DISA/mem_apply_exam_common_view',$data);
				}
    ...
    //START : CODE BEFORE ELIGIBLE MASTER API : COMMENTED BY SAGAR ON 2024-06-13
		...
		else
		{
			...
			$check_qualify=array('flag'=>$flag,'message'=>$message);
			return $check_qualify;
		} */ //END : CODE BEFORE ELIGIBLE MASTER API : COMMENTED BY SAGAR ON 2024-06-13

    //START : CODE AFTER ELIGIBLE MASTER API : ADDED BY SAGAR ON 2024-06-13
    $check_qualify_exam_eligibility = $this->master_model->getRecords('exam_activation_master',array('exam_code'=>$qualify_id),'exam_code');
    if(count($check_qualify_exam_eligibility) > 0)
    {
      ...
      $member_number = $this->session->userdata('mregnumber_applyexam');
      $eligible_response = $this->master_model->disa_api_curl($exam_code, $exam_period, $member_number);
      //echo '<pre>'; print_r($eligible_response); echo '</pre>'; //exit;      
      
      if(isset($eligible_response['flag']) && $eligible_response['flag'] == 'success')
      {
        if(isset($eligible_response['eligible_flag']) && $eligible_response['eligible_flag'] == 'Y')
        {
          $flag = 1;
        }
        else
    ...
    $check_qualify = array('flag'=>$flag,'message'=>$message);
    return $check_qualify;
    //END : CODE AFTER ELIGIBLE MASTER API : ADDED BY SAGAR ON 2024-06-13
	}
	...
	public function Msuccess()
	{
		...
		if(isset($_POST['btnPreview']))
		{
			...
			if($inser_id=$this->master_model->insertRecord('member_exam',$inser_array,true))
			{
				...
				{
					redirect(base_url().'Disa/sbi_make_payment/');
				}
				else
				{
					redirect(base_url().'Disa/make_payment/');
				}
			}
		}
		else
	...
	public function make_payment()
	{
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			$pg_return_url = base_url()."Disa/pg_response";
...
requestparameter=IIBF|2138759|NA|500.00|NA|NA|NA|INR|NA|R|iibf|NA|NA|F|iibfexam|500081141|148201701|NA|NA|NA|NA|http://abc.somedomain.com|2387462372
			...
			if ($res_checksum == $checksum)
			{
				if($pg_res[16] == "iibfexam")
				{
					...
					$exam_info=$this->master_model->getRecords('member_exam',array('regnumber'=>$this->session->userdata('mregnumber_applyexam'),'member_exam.id'=>$get_user_regnum[0]['ref_id']),'member_exam.exam_mode,member_exam.exam_medium,member_exam.exam_period,center_master.center_name,member_exam.exam_center_code,exam_master.description,misc_master.exam_month');
						...
						{
							redirect(base_url().'Disa/details/'.base64_encode($MerchantOrderNo).'/'.$this->session->userdata['examinfo']['excd']);
						}
						}
						else if($payment_status==0)
						{
							...
							if($this->Emailsending->mailsend($info_arr))
							{
								redirect(base_url().'Disa/fail/'.base64_encode($MerchantOrderNo));
							}
	...
	public function sbi_make_payment()
	{
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			$amount =$this->config->item('CITAP_cs_total');
			$inser_array=array(	'regnumber'=>$this->session->userdata('mregnumber_applyexam'),
		  ...
		  if($this->session->userdata('memtype')=='O' || $this->session->userdata('memtype')=='A' || $this->session->userdata('memtype')=='F')
			...
			else if($this->session->userdata('memtype')=='NM' || $this->session->userdata('memtype')=='DB')
			{
			...
			$insert_data = array(
									...
									'pay_type'         => '2',
									'ref_id'           => $inser_id,
									'description'      =>'Certified Information Technology Audit Professional(CITAP)',
									'status'           => '2',
									'exam_code'    	   => $this->exam_code,
			...
			else
			{
				...
				$invoice_insert_array=array('pay_txn_id'=>$pt_id,
											...
											'app_type'=>'O',
											'exam_period'=>$this->exam_period,
											'service_code'=>$this->config->item('CITAP_service_code'),
											'qty'=>'1',
											'state_code'=>'27',
											'state_name'=>'MAHARASHTRA',
											'tax_type'=>'Intra',
											'fee_amt'=>$this->config->item('CITAP_apply_fee'),
											'cgst_rate'=>$this->config->item('CITAP_cgst_rate'),
											'cgst_amt'=>$this->config->item('CITAP_cgst_amt'),
											'sgst_rate'=>$this->config->item('CITAP_sgst_rate'),
											'sgst_amt'=>$this->config->item('CITAP_sgst_amt'),
											'igst_rate'=>'0.00',
											'igst_amt'=>'0.00',
											'cs_total'=>$this->config->item('CITAP_cs_total'),
											'igst_total'=>'0.00',
											'exempt'=>'NE',
											'created_on'=>date('Y-m-d H:i:s'));
					...
					if ($pg_name == 'sbi')
					{
						...
						$pg_success_url = base_url()."Disa/sbitranssuccess";
						$pg_fail_url    = base_url()."Disa/sbitransfail";
					...
					elseif ($pg_name == 'billdesk')
					{
						...
						$billdesk_res = $this->billdesk_pg_model->init_payment_request($MerchantOrderNo, $amount, $member_exam_id, $member_exam_id, '', 'Citap/handle_billdesk_response', '', '', '', $custom_field_billdesk);
			...
			if (isset($_REQUEST['transaction_response'])) 
			{
				...
				if($auth_status == "0300" && $qry_api_response['auth_status'] == '0300')
				{
					...
					if($get_user_regnum[0]['status']==2)
					{
						...
						else if($exam_info[0]['exam_code'] != $this->exam_code ) // Add condition gaurav replace exam code only 990 to 1234
						{
							//$month = date('Y')."-".substr($exam_info[0]['exam_month'],4)."-".date('d');
							$month = date('Y')."-".substr($exam_info[0]['exam_month'],4);
							$exam_period_date = date('F',strtotime($month))."-".substr($exam_info[0]['exam_month'],0,-2);
						...
						{
							// get Elective Subject name for CAIIB Exam	
						...
						{
							if($exam_info[0]['exam_code'] == $this->exam_code) // Add condition gaurav replace exam code only 990 to 1234
							{
								$emailerstr=$this->master_model->getRecords('emailer',array('emailer_name'=>'CITAP_emailer'));
						...
						{
							$invoiceNumber = generate_CITAP_invoice_number($getinvoice_number[0]['invoice_id']);
							if($invoiceNumber)
							{
								$invoiceNumber = $this->config->item('CITAP_invoice_no_prefix').$invoiceNumber;
							}
							...
							$attachpath=genarate_CITAP_invoice($getinvoice_number[0]['invoice_id']);
						}
								
						if($exam_info[0]['exam_code'] == $this->exam_code) { $sms_final_str = $emailerstr[0]['sms_text']; } // Add condition gaurav replace exam code only 990 to 1234
						else
						{
						...
						if($attachpath!='')
						{	
							/* XXX$this->master_model->send_sms($result[0]['mobile'],$sms_final_str);	*/
					...
					redirect(base_url().'Citap/details/'.base64_encode($MerchantOrderNo));
				}
				elseif ($auth_status=='0002') {
					...
					$pg_response = "encData=".$encData."&merchIdVal=".$merchIdVal."&Bank_Code=".$Bank_Code;
					$this->log_model->logtransaction("sbiepay", $pg_response,$responsedata['transaction_error_type']);
					redirect(base_url().'Citap/pending/'.base64_encode($MerchantOrderNo));
				}
				else
				{
					...
					if($get_user_regnum[0]['status']!=0 && $get_user_regnum[0]['status']==2)
					{
						...
						if($get_user_regnum[0]['exam_code'] != $this->exam_code) // Add condition gaurav replace exam code only 990 to 1234
						{
							//Query to get user details
							$this->db->join('state_master','state_master.state_code=member_registration.state');
							$this->db->join('institution_master','institution_master.institude_id=member_registration.associatedinstitute');
					...
					//End Of SBICALL Back	
					redirect(base_url().'Citap/fail/'.base64_encode($MerchantOrderNo));
				}
			}
			else 
			{
	...
	///End of SBICALL Back	
		...
		//$this->session->set_userdata($user_data);		
		$this->session->unset_userdata($user_data);
		//redirect(base_url().'Disa/details/'.base64_encode($MerchantOrderNo).'/'.base64_encode($exam_info[0]['exam_code']));
		redirect(base_url().'Disa/details/'.base64_encode($MerchantOrderNo));
	}
	...
	public function details($order_no=NULL,$excd=NULL)
	{
		if($order_no!=NULL)
		{
			$data=array('middle_content'=>'CITAP/exam_applied_success');
			$this->load->view('CITAP/mem_apply_exam_common_view',$data);
		}
		else
		{
	...
	public function fail($order_no=NULL)
	{
		if($order_no!=NULL)
		{
			$data = array('middle_content'=>'CITAP/exam_applied_fail');
			$this->load->view('CITAP/mem_apply_exam_common_view',$data);
		}
		else
		{
	...
	public function pending($order_no=NULL)
	{
		if($order_no!=NULL)
		{
			$data=array('middle_content'=>'CITAP/exam_applied_pending');
			$this->load->view('CITAP/mem_apply_exam_common_view',$data);
		}
		else
		{
	...
	{		
      ...
      $eligible_response = $this->master_model->citap_api_curl($exam_code, $exam_period, $member_number);

      if(isset($eligible_response['flag']) && $eligible_response['flag'] == 'success' && isset($eligible_response['eligible_flag']) && $eligible_response['eligible_flag'] == 'Y')
      {
        $flag = '1';
			}
		}
  ...
  function disa_api_curl_qa($exam_code=0, $exam_period=0, $member_number=0)
  {
    $res = $this->master_model->disa_api_curl($exam_code, $exam_period, $member_number);
    echo json_encode($res);
  }

  /******** START : CHECK SESSION AFTER LOGIN ********/
  function check_admin_session_all_pages($login_as='') // login_as = admin or candidate
  {
    $mregid_applyexam = $this->session->userdata('mregid_applyexam');
    $mregnumber_applyexam = $this->session->userdata('mregnumber_applyexam');

    if (!isset($mregid_applyexam) || $mregid_applyexam == "" || !isset($mregnumber_applyexam) || $mregnumber_applyexam == "")
    {
      redirect(site_url('citap/logout'),'refresh');
    }
  }/******** END : CHECK SESSION AFTER LOGIN ********/


  function check_disa_api($exam_code="990", $exam_period= "904", $member_number="510398270")
  {
    $res = $this->master_model->disa_api_curl($exam_code, $exam_period, $member_number);
