...
class PaymentS2S_clone extends CI_Controller {
	...
	if ($crnt_day_txn_qry->num_rows())
	{
		...
		foreach ($crnt_day_txn_qry->result_array() as $c_row)
		{
			...
			if($data_count == 0)
			{
				...
				$fp = @fopen($dir."/logs_new_".date("dmY").".txt", "a") or die("Unable to open file logs!");
				$str = "\n $receipt_no=$encData\n";
				fwrite($fp, $str);
				...
				//$fp = @fopen("/usr/local/bin/php/home/supp0rttest/cs2s_log/detail_logs_new_".date("dmY").".txt", "a") or die("Unable to open file!");
			...
			//log responce in txt
			//$fp = @fopen("/usr/local/bin/php/home/supp0rttest/cs2s_log/logs_".date("dmY").".txt", "a") or die("Unable to open file!");
