...
class Naarlogin extends CI_Controller {
...
public function __construct()
		...
		if($this->session->userdata('institute_id')=='')
		{
			...
			$dataarr=array(
				...
				if ($this->form_validation->run() == TRUE)
				{
					...
					if($this->input->post('bankname') == '10004NAR' && $encpass = '87ACU5')
					{ 
						 if($bank_info[0]['accerdited_delete']==0)
						  {
								...
								$user_data=array('institute_id'=>'10004NAR',
															'institute_name'=>$bank_info[0]['institute_name'],
															'timer'=>base64_encode($mysqltime),
															'exmCrdPrd'=>$act_info[0]['exam_period']
															);
								$this->session->set_userdata($user_data);
								
								redirect(base_url().'bulk/Naardashboard/');	
						  }
			...
			$this->load->view('bulk/login',$data);
		}
		else
		{
			redirect(base_url().'bulk/Naardashboard/');
	...
	public function check_captcha_userlogin($code) 
	{
		...
		{
			redirect(base_url().'bulk/Naarlogin/');
		}
			...
			if($this->form_validation->run())
			{
				...
				if($query)
				{
							...
							if($this->Emailsending->mailsend($info_arr))
							{
								//$this->session->set_flashdata('success','Your Application has been Submitted Successfully !!');
								 redirect(base_url().'bulk/Naarlogin/forgetack/');
							}
							else
							{
								$this->session->set_flashdata('error','Error while sending email !!');
								 redirect(base_url().'bulk/Naarlogin/mylogin');
							}					
					}
					}
				else
				{
							$this->session->set_flashdata('error_message','Invalid Membership/Registration No!');
						 	redirect(base_url().'bulk/Naarlogin/forgotpassword/');
				}
			}
		}
