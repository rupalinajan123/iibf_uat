...
class BulkTransaction extends CI_Controller {
	...
	public function naar_getadmitcardpdfsp()
	{
		...
		// print_r($data);die;
		if($exam_code == 1002 || $exam_code == 1003 || $exam_code == 1004 || $exam_code == 1005 || $exam_code == 1006 || $exam_code == 1007 || $exam_code == 1008 || $exam_code == 1009 || $exam_code == 1010 || $exam_code == 1011 || $exam_code == 1012 || $exam_code == 1013 || $exam_code == 1014 || $exam_code == 1015 || $exam_code == 1017 || $exam_code == 1018 || $exam_code == 1026 || $exam_code == 1027 || $exam_code == 1029 || $exam_code == 1028 ||$exam_code == 1030 || $exam_code == 1033 || $exam_code == 1051 || $exam_code == 1034 || $exam_code == 1043 || $exam_code == 1048 || $exam_code == 1059 || $exam_code == 1061){
			// ECHO '***';die;
	...
	public function getTransactions()
	{
		...
		if($res)
		{
			...
			foreach($result as $row)
			{
				...
				// admit card -
				if(isset($row['exam_code']) && $row['exam_code'] != "" && isset($this->elearning_course_code) && count($this->elearning_course_code) > 0 && $row['exam_code']!=420){ // Priyanka D >>   DBFMOUPAYMENTCHANGE >> 10-july-25
	...
	public function mem_admit_card_list($txn_id)
	{
		...
		// get list of all members for this payment transaction -
		$select = 'member_registration.regid,member_registration.regnumber,member_registration.usrpassword,member_registration.firstname,member_registration.middlename,member_registration.lastname,member_registration.email,member_registration.mobile,member_exam.exam_code,member_exam.exam_period,member_registration.bank_emp_id,member_exam.institute_id';	
		$this->db->join('member_exam','member_exam.id = bulk_member_payment_transaction.memexamid','LEFT');
		$this->db->join('member_registration','member_registration.regnumber = member_exam.regnumber','LEFT');
		$this->db->where('member_registration.isactive = "1"');
		$this->db->where('bulk_member_payment_transaction.ptid = '.$txn_id);
	...
	public function proforma_invoice_payment() { // Priyanka D >>  start DBFMOUPAYMENTCHANGE >> 10-july-25
		$this->session->set_userdata('field','');
		$this->session->set_userdata('value','');
		$this->session->set_userdata('per_page','');
		$this->session->set_userdata('sortkey','');
		...
		if($this->session->userdata('mou_flg')!=1) { 
			redirect(base_url() . 'bulk/BulkTransaction/neft_transactions');
		...
		$data['middle_content']	= 'bulk/transaction/proforma_invoice_payment';
		
       	$this->load->view('bulk/bulk_common_view', $data);
	} // Priyanka D >>  end DBFMOUPAYMENTCHANGE >> 10-july-25
	// function to get list of NEFT transactions-
	public function getProformaTransactions() // Priyanka D >>  start DBFMOUPAYMENTCHANGE >> 10-july-25
	{
		$data['result'] = array();
		$data['action'] = array();
		$data['links'] = '';
		...
		$select = 'bulk_payment_transaction.id,bulk_payment_transaction.exam_code,bulk_payment_transaction.exam_period,bulk_payment_transaction.receipt_no,proformo_invoice_no,bulk_payment_transaction.status,bulk_payment_transaction.transaction_no,exam_invoice.created_on as exam_inv_date,pay_count AS member_count,amount,'; // "Bulk" is Application in NEFT Transactions table in Bulk Admin (Hard coded)
		$this->db->join('exam_invoice','exam_invoice.pay_txn_id = bulk_payment_transaction.id','LEFT');
		$this->db->join('bulk_accerdited_master','bulk_accerdited_master.institute_code = bulk_payment_transaction.inst_code','LEFT');
		$this->db->join('exam_master','exam_master.exam_code = bulk_payment_transaction.exam_code','LEFT');
		$this->db->where('gateway = "1"');
		$this->db->where('bulk_payment_transaction.exam_code = "420"');
		$this->db->where('exam_invoice.app_type = "Z"');
		...
		if($res)
		{
			...
			$setReult = $result;
			foreach($setReult as $key=> $row2)
			{
				if ($row2['state']!='') {
					$setReult[$key]['transaction_no']=$row2['transaction_no']."<br>".$row2['state'];
				}
				unset($setReult[$key]['status']);
				
			}

			//end : code added by vishal for 994 exam
			
			$data['result'] = $setReult;
			$current_date  = date('Y-m-d');
			//print_r($data);exit;
			foreach($result as $row)
			{
				// check if approved by Bulk Admin -
				//if($row['status'] == 3 || $row['status'] == 5 || $row['status'] == 0 )	// status = 3 for Applied
				{

					$active_exam_info = $this->master_model->getRecords('bulk_exam_activation_master', array(
					...
					$active_exam_to_date   = isset($active_exam_info[0]['exam_to_date']) ? $active_exam_info[0]['exam_to_date'] : 0;
						...
						$action = '<a href="'.base_url().'bulk/BulkTransaction/view_inst_pr_invoice/'.$row['id'].'" target="_blank">Proforma Invoice</a></span> |  <br> <span data-id="'.$row['id'].'"> <span data-id="'.$row['id'].'"><a href="'.base_url().'bulk/BulkTransaction/performa_invoice_details/'.base64_encode($row['id']).'" target="_blank">View more </a></span>';
						if ($current_date >= $active_exam_from_date && $current_date <= $active_exam_to_date && ( $row['status'] == 3 || $row['status'] == 0 )  ) 
						{
							$action .= '| <br><a class="btn btn-sm btn-warning"  href="'.base_url().'bulk/BulkTransaction/getPayment/'.base64_encode($row['id']).'">Make Payment</a></span>  ';
						}
				...
				if($row['status'] == 1) // status = 1 for Success
				{
					$action .= '<span class="label label-success">Success</span>';	
				}
				else if($row['status'] == 6) // status = 0 for Rejected
				{
					$action .= '<span class="label label-danger">Failed</span>';	
				}
				$data['action'][] = $action;
				if($row['exam_code']!='')
				{
			...
			if(count($result))
			{
				...
				$url = base_url()."bulk/BulkTransaction/getProformaTransactions/";
	...
	public function getPayment($payTransId=0) //Priyanka D >> start DBFMOUPAYMENTCHANGE >> 10-july-25
	{
			$id = base64_decode($payTransId);

			$this->db->where('status !=',7);
			$paymentTransInfo = $this->master_model->getRecords('bulk_payment_transaction', array(
				'id' => $id
			));
			...
			if ($current_date >= $active_exam_from_date && $current_date <= $active_exam_to_date) 
			{
				...
				if (count($paymentTransInfo) <= 0) {
					$this->session->set_flashdata('error', 'Payment/Proformo details are not found, or a Proforma invoice is invoked.');
						redirect(base_url() . 'bulk/BulkTransaction/proforma_invoice_payment');
				} 	 
				...
				$successTransInfo = $this->master_model->getRecords('bulk_payment_transaction');
				...
				//echo'<pre>';print_r($paymentTransInfo);
				//echo $this->db->last_query();exit;
			...
			$this->db->select('bulk_accerdited_master.*');
			$this->db->where('bulk_accerdited_master.institute_code',$this->session->userdata('institute_id'));
			$this->db->where('bulk_accerdited_master.accerdited_delete',0);
			$inst_registration_info = $this->master_model->getRecords('bulk_accerdited_master');
			...
			if (count($arr_payment_transaction_data) > 0) 
			{
				...
				$institude_dt = $this->master_model->getRecords('bulk_accerdited_master', array(
						'institute_code' => $this->session->userdata('institute_id') ,
						'accerdited_delete' => 0
					) );
					$data['instdata']= $institude_dt[0];

				$this->db->select('description');
				$examDetails = $this->master_model->getRecords('exam_master', array('exam_code' => $exam_code));
				//echo'<pre>';print_r($examDetails);exit;
				$data["tot_fee"]        = base64_encode($tot_fee);
				$data["exam_code"]      = base64_encode($exam_code);
				$data["exam_name"]      = $examDetails[0]['description'];
				$data["exam_period"]    = base64_encode($exam_period);
				$data["payTransId"]     = $payTransId;
				$data["middle_content"] = 'bulk/transaction/bulk_make_online_payment_page';

				
				$log_title ="bulk payment online - bulk_make_online_payment_page";
				...
				$this->db->join('bulk_exam_activation_master b','a.exam_code = b.exam_code', 'right');
				$res = $this->master_model->getRecords("exam_master a");
	...
	public function make_payment()  //Priyanka D >> start DBFMOUPAYMENTCHANGE >> 10-july-25
	{	
			...
			{
				$exam_code = base64_decode($_POST['exam_code']);
				...
				if ($current_date >= $active_exam_from_date && $current_date <= $active_exam_to_date) 
				{
					...
					$this->db->where('status !=',7);
					$paymentTransInfo = $this->master_model->getRecords('bulk_payment_transaction');
				...
				{	
					...
					$bulk_mem_list    = base64_decode($_POST['regNosToPay']);
					$bulk_mem_array   = explode("|",$bulk_mem_list);
					$examcode        = base64_decode($_POST['exam_code']);
					$examperiod      = base64_decode($_POST['exam_period']);

					$bulk_exam_invoice_data=$this->master_model->getRecords('exam_invoice',array('pay_txn_id'=>$payTransId,'exam_code'=>$examcode,'exam_period'=>$examperiod,'app_type'=>'Z'));
					...
					if ( isset($paymentTransInfo) && count($paymentTransInfo)>0 ) 
					{
						if( $paymentTransInfo[0]['status'] == 0 || $paymentTransInfo[0]['status'] == 6 ) 
						{
							...
							$InprocessPayment = $this->master_model->getRecords('bulk_payment_transaction',array('proformo_invoice_no' => $proformo_invoice_no,'status' => 5));
							...
							$this->master_model->updateRecord('bulk_payment_transaction',$update_trans_data,array('id'=>$payTransId));

							$proformoMembers = $this->master_model->getRecords('bulk_member_payment_transaction',array('ptid' => $payTransId)); 
							...
							$arr_insert_data['created_date']   = date('Y-m-d H:i:s');
							$arr_insert_data['status'] = 5;

							$payTransId = $this->master_model->insertRecord('bulk_payment_transaction',$arr_insert_data, true);

							$MerchantOrderNo = $payTransId;//sbi_exam_order_id($payTransId);
							$ref4 = $exam_code.date('Y').date('m');
				      		$custom_field          = $MerchantOrderNo."^iibfexam^iibfbulkexam^".$ref4;
							$custom_field_billdesk = $payTransId . "-iibfexam-IIBF_BULK-" . $MerchantOrderNo;
							...
							$log_title ="bulk new payment transaction entry";
							$log_message = serialize($arr_insert_data);
							$rId = $pt_id;
							$regNo = $pt_id;
							$inst_id = $this->session->userdata('institute_id');
							...
							foreach ($proformoMembers as $key => $pfMember) 
							{
								...
								$memPayId = $this->master_model->insertRecord('bulk_member_payment_transaction',$arr_insert_pfmember, true);
							}
					...
					$update_data = array(
						...
						$update_query = $this->master_model->updateRecord('bulk_payment_transaction', $update_data, array('id' => $payTransId));
					...
					$instdata= $institude_dt[0];

					//echo'<pre>';print_r($instdata);exit;
					$inst_code       = $instdata['institute_code']; 				
					
					$examcode = isset($active_exam_info[0]['exam_code']) ? $active_exam_info[0]['exam_code'] : 0;	
					$exam_month_year = $this->master_model->getRecords('misc_master',array('exam_code'=>$examcode,'exam_period'=>$active_exam_period));
					...
					elseif($center_data == "Institute")
					{
				  	...
				  	$regnumber_arr = array();
					$total_member_cnt = count($bulk_mem_array);
					$free_member_cnt = 0;
					foreach($bulk_mem_array as $rid)
					{
						$this->db->join('member_exam','member_exam.regnumber = member_registration.regnumber');
						$bulk_member_exam = $this->master_model->getRecords('member_registration',array('member_exam.regnumber'=>$rid,'member_exam.exam_code'=>$examcode,'member_exam.exam_period'=>$examperiod),'member_registration.regnumber, free_paid_flg');
						
						//echo $this->db->last_query();exit;
						if(isset($bulk_member_exam[0]['free_paid_flg']) && $bulk_member_exam[0]['free_paid_flg'] != 'F') //CONDITION ADDED BY SAGAR ON 07-10-2020
						{ 
							$regnumber_arr[] = $bulk_member_exam[0]['regnumber'];
						}
					...
					if(count($regnumber_arr) > 0)
					{
						...
						{						
							if($regnumber_arr != '')
							{
								$bulk_eligible_master = $this->master_model->getRecords('eligible_master',array('member_no'=>$regnumber_arr,'exam_code'=>$examcode,'eligible_period'=>$examperiod),'app_category');
								
								if(count($bulk_eligible_master) > 0)
								{								
									
									if($bulk_eligible_master[0]['app_category'] != 'R' && $bulk_eligible_master[0]['app_category'] != 'B1_1')
									{
										$unit_S1=$unit_S1+1	;
									}
									else
							...
							if(isset($bulk_eligible_master[0]['app_category']))
							{
								$app_category[] = $bulk_eligible_master[0]['app_category'];
							}
							else
							{
								$app_category[] = 'B1_1';
				  ...
				  //end swati=============
					...
					$chk_exam_period = $this->master_model->getRecords('bulk_exam_activation_master', array(
						'exam_code' => $examcode,
					...
					$custom_field_billdesk = $pt_id . "-IIBF_BULK_EXAM-" . $MerchantOrderNo;
					...
					$bulkInstState = $this->master_model->getRecords('state_master',array('state_code'=>$instStateCode,'state_delete'=>'0'));
					//echo $this->db->last_query();exit;
					...
					{
						/* Get reg id   */
						$this->db->where('regnumber', $bulk_mem_id);
						$this->db->where('exam_code', $examcode);
						$this->db->where('exam_period', $examperiod);
						$getRegId = $this->master_model->getRecords('member_exam', '', 'regnumber,free_paid_flg');

						// $RegId = $getRegId[0]['regid'];
				  		$RegId = $bulk_mem_id;
						$free_paid_flag = $getRegId[0]['free_paid_flg'];			

						/* Get Member Number  */
						//$registrationtype = "NM";
						$this->db->where('regnumber', $RegId);
						$getMemberNo = $this->master_model->getRecords('member_registration', '', 'regnumber,registrationtype');
						...
						if($member_no != 0 && $member_no != "")
						{
							...
							$this->db->where('exam_code', $examcode);
							$getGrpCd = $this->master_model->getRecords('eligible_master', '', 'app_category');
							if(count($getGrpCd)>0)
							{
					...
					if($amount > 0) 
					{
						...
						if($newPaymentTransactionEntry==1)
						 {
							...
							$log_title ="bulk invoice new entry";
						 ...
						 $invoice_update_array = array(
							...
							$log_title ="bulk invoice update entry";
							$log_message = $this->db->last_query();
							$rId = $pt_id;
							$regNo = $pt_id;
							$inst_id = $this->session->userdata('institute_id');
						...
						$bulk_exam_invoice_data=$this->master_model->getRecords('exam_invoice',array('pay_txn_id'=>$pt_id,'exam_code'=>$examcode,'exam_period'=>$examperiod,'app_type'=>'Z'));

						$inv_id = $bulk_exam_invoice_data[0]['invoice_id '] ;
						
						
						$log_title ="bulk before init_payment_request";
						...
						if ($pg_name == 'billdesk')
							{ 
								$billdesk_res = $this->billdesk_pg_model->init_payment_request($MerchantOrderNo, $amount, $inv_id, $inv_id, '', 'bulk/BulkTransaction/handle_billdesk_response', '', '', '', $custom_field_billdesk);
								...
								if (count($billdesk_res) > 0 && $billdesk_res['status'] == 'ACTIVE')
								{
									...
									$data['bulk_payment_flag'] = 'IIBF_BULK_EXAM';////
	...
	{   //Priyanka D >> start DBFMOUPAYMENTCHANGE >> 10-july-25
	    ...
	    if (isset($_REQUEST['transaction_response']))
	    {
	      ...
	      $get_user_regnum_info = $this->master_model->getRecords('bulk_payment_transaction', array('receipt_no' => $MerchantOrderNo), 'id,exam_code,exam_period,gateway,amount,date,inst_code,pay_count,status,created_date,callback');
	      ...
	      else if ($auth_status == '0399' && $qry_api_response['auth_status'] == '0399') 
	      {
        	...
        	$update_query = $this->master_model->updateRecord('bulk_payment_transaction', $update_data, array('receipt_no' => $MerchantOrderNo));      
			...
			{ 
				...
				if (count($get_user_regnum_info) > 0)
				{
					...
					if($MerchantOrderNo)
					{
						$_SESSION['session_ptid'] = $pt_id = $id = $get_user_regnum_info[0]['id'];	
						...
						if($payment_action == "Approved")
						{
							...
							$log_title ="bulk exam payment success";
							$log_message = serialize($update_data);
							$rId = $get_user_regnum_info[0]['id'];
							$regNo = $get_user_regnum_info[0]['id'];
							$inst_id = $this->session->userdata('institute_id');
							//echo '<pre>',print_r($user_data),'</pre>';exit;
							bulk_storedUserActivity($log_title, $log_message,$inst_id ,$rId, $regNo);
						...
						else if($payment_action == "Rejected")
						{
							...
							$log_title ="bulk exam payment Failed";
							$log_message = serialize($update_data);
							$rId = $get_user_regnum_info[0]['id'];
							$regNo = $get_user_regnum_info[0]['id'];
							$inst_id = $this->session->userdata('institute_id');
						...
						if($this->master_model->updateRecord('bulk_payment_transaction',$update_data,  array('id' => $id)))
							{
								...
								if( $status == 1 ) //WHEN PAYMENT IS APPROVED
								{
									...
									foreach ($member_array as $member_record) {
										...
										if ($capacity != 0) {
											...
											$log_title ="bulk exam After onlie payment - Capacity available";
											$log_message = serialize($update_data);
											$rId = $pt_id;
											$regNo = $pt_id;
											$inst_id = $this->session->userdata('institute_id');
											...
											$log_title ="bulk exam After onlie payment - Capacity not available";
											$log_message = serialize($update_data_one);
											$rId = $pt_id;
											$regNo = $pt_id;
											$inst_id = $this->session->userdata('institute_id');
											//echo '<pre>',print_r($user_data),'</pre>';exit;
									...
									if (count($member_array) > 0 && $flag == 0 && count($member_array) == count($sub_arr)) {
										...
										foreach ($sub_arr as $sub_details) {
											...
											if ($seat_allocation != '') {
												...
												$log_title ="bulk exam After onlie payment - Seat allocate successfully";
												$log_message = serialize($update_data_two);
												$rId = $pt_id;
												$regNo = $pt_id;
												$inst_id = $this->session->userdata('institute_id');
												//echo '<pre>',print_r($user_data),'</pre>';exit;
												...
												$log_title ="bulk exam After onlie payment - Seat not allocate";
												$log_message = serialize($update_data_three);
												$rId = $pt_id;
												$regNo = $pt_id;
												$inst_id = $this->session->userdata('institute_id');
												//echo '<pre>',print_r($user_data),'</pre>';exit;
									...
									if (count($sub_arr) == count($member_array) && $flag == 0 && count($member_array) > 0) {
										...
										foreach ($sub_arr as $record) {
											...
											$chk_mem_exist = $this->master_model->getRecords('dra_members', '', 'regid'); //this is checking in dra_members refer from bulk/admin/Transaction.php
											if (count($chk_mem_exist) > 0) {
												$user_stat = 1;
											} else {
											...
											if ($user_stat == 0) {
												...
												if ($member_exam_info[0]['member_type'] != 'O' && $member_exam_info[0]['member_type'] != 'o') {
													...
													$log_title ="bulk exam After onlie payment - regnumber generated";
													$log_message = $this->db->last_query();//serialize($update_data_member_tbl);
													$rId = $pt_id;
													$regNo = $pt_id;
													$inst_id = $this->session->userdata('institute_id');
													//echo '<pre>',print_r($user_data),'</pre>';exit;
												...
												$log_title ="bulk exam Online Payment - Bulk admitcard updated successfully";
												$log_message = $this->db->last_query();
												$rId = $pt_id;
												$regNo = $pt_id;
												$inst_id = $this->session->userdata('institute_id');
												...
												if ($member_exam_info[0]['member_type'] != 'O' && $member_exam_info[0]['member_type'] != 'o') {
													...
													if($member_exam_info[0]['exam_code']==1055 || $member_exam_info[0]['exam_code']==1056 || $member_exam_info[0]['exam_code']==1046){
													...
													if (count($upd_files) > 0) {
														...
														$log_title ="bulk exam Online Payment - Bulk Member Images Updated successfully ";
											...
											if($member_exam_info[0]['exam_code']==1009 && $member_exam_info[0]['member_type'] != 'O' && $member_exam_info[0]['member_type'] != 'o'){
												...
												if (count($upd_files) > 0) {
													...
													$log_title ="bulk exam Online Payment - Bulk Fedai Member Images Updated successfully ";
														$log_message = serialize($upd_files);
														$rId = $pt_id;
														$regNo = $pt_id;
														$inst_id = $this->session->userdata('institute_id');
	...
	public function neft_transactions()
	{
		...
		if($this->session->userdata('mou_flg')==1) { // Priyanka D >>  DBFMOUPAYMENTCHANGE >> 10-july-25
			redirect(base_url() . 'bulk/BulkTransaction/proforma_invoice_payment');
		...
		$data['middle_content']	= 'bulk/transaction/neft_transactions';
	...
	public function Get_password($number='')
	{ 		
		if($number != "")
		{
			include_once APPPATH . 'third_party/SBI_ePay/CryptAES.php';
