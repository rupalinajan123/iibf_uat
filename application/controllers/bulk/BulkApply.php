...
class BulkApply extends CI_Controller
{
    ...
    public function examlist()
    {
        ...
        $this->db->select('exam_master.exam_code,exam_master.exam_type,exam_master.description,exam_master.description,subject_master.exam_period,subject_master.exam_period,exam_date,exam_from_date,exam_to_date');
        ...
        $data['elearning_course_code']=['528','529','530','531','534'];

        // if($exam_list[0]['exam_code'] =='1033' && $this->get_client_ip() =='115.124.115.75' )
        // { 
        //     //echo die;&& $this->get_client_ip() !='182.73.101.70'
        // }
    ...
    ##------------------ Exam list for mou  collage logged in user(---------------##
    public function mouexamlist()
    {
        ...
        //Below query has been modified by pooja mane to dispaly all MOU collage exams -- 2024-10-10
        $this->db->select('exam_master.exam_code,exam_master.exam_type,exam_master.description,exam_master.description,subject_master.exam_period,subject_master.exam_period,exam_date,exam_from_date,exam_to_date');
        ...
        $this->db->where("bulk_exam_activation_master.exam_activation_delete", "0");
        $this->db->where("bulk_exam_activation_master.mou_flg", "1");
        ...
        $data = array('middle_content' => 'bulk/mou_exam_list', 'exam_list' => $exam_list);
        $data['elearning_course_code']=['528','529','530','531','534'];
       ...
       // echo $this->session->userdata('examcode');exit;
        if (!$this->session->userdata('examcode')){
            if($this->session->userdata('mou_flg') == '1'){
                redirect(base_url() . 'bulk/BulkApply/mouexamlist/');//added by pooja mane for mou clg - 2024-10-11
            }else{
        ...
        if($this->session->userdata('examcode')==1009) {
            $check_fedai_eligible = $this->master_model->check_fedai_eligible($this->session->userdata('institute_id'),0,0);
            if($check_fedai_eligible==false) {
                $this->session->set_flashdata('error', 'You are not eligible for Fedai Exam');
        ...
        if ($this->session->userdata('examcode') != '420' && $this->session->userdata('examcode') != '101' && $this->session->userdata('examcode') != '1046' && $this->session->userdata('is_elearning_course') == 'n') {
        ...
        if (isset($_POST['getdata'])) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                if (isset($_POST['regnumber'])) {
                    ...
                    //priyanka d >> 26-july-24
                    if($this->session->userdata('examcode')==1009) {
                        $arr_fedai_eligible = $this->master_model->check_fedai_eligible($this->session->userdata('examcode'),$ex_prd,$_POST['regnumber'],$this->session->userdata('institute_id'));
                        //
                    ...
                    $chk_eligible_member = $this->master_model->getRecords('eligible_master', '', 'discount_flag,institute_id,fee_paid_flag,exam_status,app_category');
                    ...
                    // Condition added to allow only S1 category members present in the eligible master if not in eligible master do not allow by (Pooja mane - 2024-03-18)
                    else
                    {
                        ...
                        if(in_array($this->session->userdata('examcode'), array()))//1033,1005
                        {
                            $this->session->set_flashdata('error','You are not Eligible !!');
                                //exit;
                            redirect(base_url('bulk/BulkApply/add_member/'));
                    ...
                    // Condition end added to allow only S1 category member present in the eligible master
                   ...
                   //  echo $this->db->last_query();exit;
                    if (count($is_member_valid) <= 0) {
                        ...
                        if($this->session->userdata('examcode') == 1033 && !in_array($member_inst, array(46,10059)))
                        {
                        ...
                        if ($member_inst != $login_inst) {
                            ...
                            if ($this->session->userdata('examcode') != 1015 && $this->session->userdata('examcode') != 1026 && $this->session->userdata('examcode') != 1027 && $this->session->userdata('examcode') != 1029 && $this->session->userdata('examcode') != 1028 && $this->session->userdata('examcode') != 1030 && $this->session->userdata('examcode') != 1033 && $this->session->userdata('is_elearning_course') == 'n') {
            ...
            if (!empty($mem_info)) {
                ...
                if ($profile_flag == 1) {
                    ...
                    if (count($check_qualify_exam) > 0) {
                        ...
                        if ($check_qualify_exam[0]['qualifying_exam3'] != '' && $check_qualify_exam[0]['qualifying_exam3'] != '0') {
                            ...
                            $this->db->where("eligible_master.institute_id", $this->session->userdata('institute_id'));
                            ...
                            if (count($check_eligibility_for_applied_exam) > 0) {
                                ...
                                if (!$check) {
                                    ...
                                    $get_period_info = $this->master_model->getRecords('misc_master', array('misc_master.exam_code' => $this->session->userdata('examcode'), 'misc_master.misc_delete' => '0'), 'exam_month', array('id' => 'DESC'),'0','1'); 
                ...
                if ($profile_flag == 0) {
                    ...
                    if ($check_qualify_exam[0]['exam_category'] == 1) {
                        ...
                        if (count($examinfo) <= 0) {
                            ...
                            if (count($special_exam_dates) > 0) {
                                ...
                                $compulsory_subjects = $this->master_model->getRecords('subject_master', array('exam_code' => $this->session->userdata('examcode'), 'subject_delete' => '0', 'group_code' => 'C', 'exam_period' => $ex_prd), '', array('subject_code' => 'ASC'));
                    ...
                    //Normal exam
                        ...
                        // if (COUNT($examinfo) == 0 && $this->session->userdata('examcode') == '530') 

                        // gaurav comment if condition and add the new condition (2-3-2024)
                        // if (COUNT($examinfo) == 0 && $this->session->userdata('examcode') == '530')
                        if (COUNT($examinfo) == 0 ) 
                        {
                            ...
                            $this->db->join('bulk_exam_activation_master', 'bulk_exam_activation_master.exam_code=misc_master.exam_code AND misc_master.exam_period=bulk_exam_activation_master.exam_period ');
                            ...
                            $examinfo = $this->master_model->getRecords('exam_master');
                            // echo '<pre>here',print_r($examinfo),'</pre>';exit;
                        }
                        ...
                        $data = array('states' => $states, 'undergraduate' => $undergraduate, 'graduate' => $graduate, 'postgraduate' => $postgraduate, 'institution_master' => $institution_master, 'designation' => $designation, 'idtype_master' => $idtype_master, 'examinfo' => $examinfo, 'medium' => $medium, 'center' => $center, 'caiib_subjects' => $caiib_subjects, 'compulsory_subjects' => $compulsory_subjects, 'mem_info' => $mem_info, 'middle_content' => 'bulk/bulk_add_member', 'bulk_zone_master' => $bulk_zone_master, 'bulk_payment_scale_master' => $bulk_payment_scale_master,'old_bcbf_institute_data' => $old_bcbf_institute_data);
                ...
                if (count($get_exam) > 0) {
                    ...
                    if ($Extype == '3') {
                        ...
                        if ($qualification == 91 && $examcode == 19) {
                            ...
                            if ($examcode == 19) {$message = 'You are not eligible to apply for this exam, you should either be CAIIB passed or should have CS qualification.';} 
                            else {
        ...
        if (count($applied_exam_date) > 0) {
            ...
            if (count($getapplied_exam_code) > 0) {
                ...
                foreach ($getapplied_exam_code as $exist_ex_code) {
                    ...
                    $getapplied_exam_date = $this->master_model->getRecords('subject_master', array('exam_code' => $exist_ex_code['exam_code'],'exam_period' => $exist_ex_code['exam_period'], 'exam_date >=' => $today_date, 'subject_delete' => '0'));
    ...
    public function get_alredy_applied_examname($regnumber = null, $exam_code = null)
    {
        ...
        if($exam_code == '1005'){
            // $this->db->where('subject_master.exam_period','855');
        }
        ...
        if (count($applied_exam_date) > 0) {
            ...
            $getapplied_exam_code = $this->master_model->getRecords('member_exam', array('regnumber' => $regnumber, 'bulk_isdelete' => '0'), 'member_exam.exam_code,exam_master.description,member_exam.exam_period'); //'pay_status'=>'1'
            // echo $this->db->last_query();
