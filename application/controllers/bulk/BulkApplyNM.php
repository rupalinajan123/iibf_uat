...
class BulkApplyNM extends CI_Controller
{
    ...
    public function add_member()
    {
        ...
        $old_bcbf_institute_data = $data['old_bcbf_institute_data'] = array();
        $ExamCode = $this->session->userdata('examcode');
        if( $ExamCode == 994 || $ExamCode == 996 || $ExamCode == 1055 || $ExamCode == 1056 || $ExamCode == 1046 ) 
        { 
            $this->db->where('is_deleted', '0');
            $old_bcbf_institute_data = $data['old_bcbf_institute_data'] = $this->master_model->getRecords('bcbf_old_exam_institute_master', '', '', array('institute_name' => 'asc')); 
            //print_r($old_bcbf_institute_data);
        }
        /*OLD BCBF Inst Mater Dropdown*/
            ...
            if ($this->form_validation->run() == true) {
                ...
                if(isset($_POST['regnumber'])) {
                    ...
                    if($this->session->userdata('examcode')==1009) {
                        $arr_fedai_eligible = $this->master_model->check_fedai_eligible($exam_code,$ex_prd,$_POST['regnumber'],$this->session->userdata('institute_id'));
                        if($arr_fedai_eligible==false) {
                            $this->session->set_flashdata('error', 'Selected Member is not eligible for Fedai Exam');
                            redirect(base_url() . 'bulk/BulkApply/examlist/');
                        }
                        $arr_response = [];
                        if ($arr_fedai_eligible['api_res_flag'] == 'success') {
                            $arr_response = json_decode($arr_fedai_eligible['api_res_response'],true); 
                            //echo'<pre>'.$this->session->userdata('institute_id'); print_r($arr_response); exit; 
                            if (count($arr_response)>0) {
                                ...
                                if($AssociateInstituteId!=$this->session->userdata('institute_id')) {
                                   
                                    $this->session->set_flashdata('error', 'Selected Member is from another institute');
                                    redirect(base_url() . 'bulk/BulkApply/examlist/');
                                }
                               // echo $AssociateInstituteId.'='.$this->session->userdata('institute_id');exit;
                            }
                        }
                    }
                ...
                if($this->session->userdata('examcode')==531 ) {
                    if( isset($_POST['regnumber'])) {

                        
                        $this->db->where('member_no', $_POST['regnumber']);

                        $this->db->where('exam_code', 530);

                    //    $this->db->where('institute_id !=', 0);//Added by pooja mane for fetching bulk eligible only

                        $this->db->where('exam_status', 'P');

                        $check_member_elg1 = $this->master_model->getRecords('eligible_master', '');

                        $this->db->where('member_no', $_POST['regnumber']);

                        $this->db->where('exam_code', 531);

                     //   $this->db->where('institute_id !=', 0);//Added by pooja mane for fetching bulk eligible only

                        //$this->db->where('exam_status', 'F');

                        $check_member_elg2 = $this->master_model->getRecords('eligible_master', '');

                        if(count($check_member_elg1)<=0 || $check_member_elg1 <= 0) {

                            $this->session->set_flashdata('error', 'You are not eligible to apply for exam.');
                            redirect(base_url('bulk/BulkApplyNM/add_member/'));
                        }

                        if(count($check_member_elg2) > 0) {
                            if($check_member_elg2[0]['exam_status']!='F') {
                ...
                if (isset($_POST['regnumber'])) {
                    ...
                    if ($exam_code == 1017 || $exam_code == 1018) {
                            ...
                            if ($profile_flag == 1) {
                                ...
                                if (count($check_qualify_exam) > 0) {
                                    ...
                                    if ($check_qualify_exam[0]['qualifying_exam3'] != '' && $check_qualify_exam[0]['qualifying_exam3'] != '0') {
                                        ...
                                        $this->db->where("eligible_master.institute_id", $this->session->userdata('institute_id'));//Added by pooja mane for fetching bulk eligible only
                        ...
                        if ($profile_flag == 0) {
                            ...
                            $this->db->where("eligible_master.institute_id !=", 0);//Added by pooja mane for fetching bulk eligible only
                    ...
                    // code to check same exam date application exist by Pooja Mane(23-02-2024)
                    $exam_date = $this->master_model->getRecords('subject_master', array('exam_code' => $this->session->userdata('examcode'), 'subject_delete' => '0', 'group_code' => 'C', 'exam_period' => $ex_prd),'exam_date');
                    $exam_date = $exam_date[0]['exam_date'];

                    $this->db->where('mem_mem_no', $_POST['regnumber']);
                    $this->db->where('exam_date', $exam_date);
                    $this->db->where('remark', 1);
                    $exam_date_exist = $this->master_model->getRecords('admit_card_details', '', 'sub_dsc');

                    if($exam_date_exist[0]['sub_dsc'])
                    {
                        $this->session->set_flashdata('error', 'Another application exist with same exam date - ('.$exam_date_exist[0]['sub_dsc'].' '.$exam_date.')');
                        redirect(base_url('bulk/BulkApply/add_member/'));
                    }
                    // code to check same exam date application exist end by Pooja Mane(23-02-2024)

                        // code to validate user
                        
                            $this->db->where('member_no', $_POST['regnumber']);
                            $this->db->where('exam_code', $this->session->userdata('examcode'));
                            if($this->session->userdata('examcode')!=531 && $this->session->userdata('examcode')!=1055 && $this->session->userdata('examcode')!=1056 && $this->session->userdata('examcode')!=1046) {
                                $this->db->where('institute_id !=', 0);//Added by pooja mane for fetching bulk eligible only
                            }
                            $this->db->where('eligible_period', $this->session->userdata['exmCrdPrd']['exam_prd']);
                            $chk_eligible_member = $this->master_model->getRecords('eligible_master', '', 'discount_flag,institute_id,fee_paid_flag,reapeter_flag,exam_status,app_category');
                        
                            // echo $this->db->last_query();exit; 
                            if (count($chk_eligible_member) > 0) {

                                // if($this->session->userdata('examcode')!=531) {
                                //     //Added by pooja mane to allow only S1 Candidate
                                    
                                //     // if ($chk_eligible_member[0]['app_category'] != 'S1') {
                                //     //     $this->session->set_flashdata('error', 'You are not eligible!!');
                                //     //     redirect(base_url('bulk/BulkApplyNM/add_member/'));
                                //     // }//Added by pooja mane to allow only S1 Candidate END
                                // }

                                if (in_array($this->session->userdata('examcode'), array(1055,1056,1046))) {

                                    if($chk_eligible_member[0]['exam_status'] == 'D'){
                                        $this->session->set_flashdata('error', 'You Are Debarred For this Exam!!');
                                        redirect(base_url('bulk/BulkApplyNM/add_member/'));
                                    }
                                    if($chk_eligible_member[0]['exam_status'] == 'V'){
                                        $this->session->set_flashdata('error', 'Valid application exist, Attempt remaining!!');
                                        redirect(base_url('bulk/BulkApplyNM/add_member/'));
                                    }
                                    if($chk_eligible_member[0]['exam_status'] == 'P'){
                                        $this->session->set_flashdata('error', 'You have already passed this exam!!');
                                        redirect(base_url('bulk/BulkApplyNM/add_member/'));
                                    }  
                                }
                                ...
                                if ($this->session->userdata('is_elearning_course')=='n' && $this->session->userdata['institute_id'] != $chk_eligible_member[0]['institute_id']) {
                                    ...
                                    if (in_array($this->session->userdata('examcode'), array(1055,1056,1046)) && $chk_eligible_member[0]['institute_id'] == "0")
                                    {
                                ...
                                $exmarr = array(1026, 1005, 1006, 1007, 1008, 1009, 531, 1055, 1056, 1046, 1059, 1034, 1061,1002);
                                //added by pooja mane to restrict 1030 passed member to 1048,1049,1050 on 2024-08-26
                                if(in_array($this->session->userdata('examcode'), array(1048,1049,1050)))
                                {
                                    if($this->master_model->getRecords('eligible_master',array('exam_code'=>'1030', 'member_no'=> $_POST['regnumber'], 'exam_status'=>'P'))){
                                ...
                                else if(!in_array($this->session->userdata('examcode'), $exmarr)){
                                    //echo $this->session->userdata('examcode');exit;
                                    if ($this->session->userdata('is_elearning_course') == 'n' || $this->session->userdata('examcode') != "1026") {
                                        //echo $this->session->userdata('is_elearning_course');die;
                                            $this->session->set_flashdata('error', 'Wrong member number!!!');
                                        redirect(base_url('bulk/BulkApplyNM/add_member/'));
                                    }
                                }
                        ...
                        if (!empty($mem_info)) {
                            ...
                            if ($profile_flag == 1) {
                                ...
                                if (count($check_qualify_exam) > 0) {
                                    ...
                                    //echo "qualifying_part1==".$flag;die;
                                    ...
                                    //echo "qualifying_part2==".$flag;die;
                                    //if($check_qualify_exam[0]['qualifying_exam3']!='' && $check_qualify_exam[0]['qualifying_exam3']!='0' && $checkqualifyflag==0)
                                    ...
                                    if ($check_qualify_exam[0]['qualifying_exam3'] != '' && $check_qualify_exam[0]['qualifying_exam3'] != '0') {
                                        ...
                                        $flag = $qaulifyarry['flag'];
                                        //echo "qualifying_part3==".$flag;die;
                                        $message = $qaulifyarry['message'];
                                        ...
                                        //echo "qualifying_part4==".$flag;die;
                                        //echo 'in';exit;
                                        ...
                                        //echo $this->db->last_query();die;
                                        if (count($check_eligibility_for_applied_exam) > 0) {
                                            //echo "check_eligibility_for_applied_exam==".$flag;die;
                                            foreach ($check_eligibility_for_applied_exam as $check_exam_status) {
                                            ...
                                            if ($exam_status == 1) {
                                                ...
                                                $message = $check_eligibility_for_applied_exam[0]['remark'];
                                                //echo "remark==".$flag;die;
                                            } else if ($exam_status == 0) {
                                                //echo "exam_status==".$flag;die;
                                                // print_r($this->session->userdata('examcode'));exit;
                                ...
                                //$this->db->where('member_exam.pay_status','1');
                                if($this->session->userdata('mregnumber_applyexam')=='802518643' && $examcode==531) {
                                 //   $this->db->where('member_exam.pay_status','1');
                                }
                                $this->db->where('member_exam.bulk_isdelete', '0');
                        ...
                        if ($profile_flag == 0) {
                            ...
                            $message = '<div style="color:#F00" class="col-md-4">Please update your profile!!<a target="_blank" href=' . base_url() ."nonmem". '> Click here </a>to login</div>';
        ...
        if (empty($examinfo)) {
            ...
            $this->db->where('exam_master.exam_code', $this->session->userdata('examcode'));
            $this->db->order_by('bulk_exam_activation_master.exam_period','DESC');
        ...
        if($institute_id != "17171"){
            $this->db->like('name', 'Any ID card issued by Government');
        }

        if($institute_id == "17171"){
            //
            $this->db->not_like('name', 'Pan card');
            $this->db->not_like('name', 'Passport');
            $this->db->not_like('name', 'Driving License');
        }
...
public function declarationform_upload()
{
    ...
    public function comApplication()
    {
        ...
        if (isset($_POST['btnPreviewSubmit'])) {
            ...
            if ($this->session->userdata('examcode') == 1009 )  // priyanka d >> 30-july 24 bulk fedai && isset($_POST['new_registertaion_nm']) && $_POST['new_registertaion_nm']==1
			{
            ...
            /*Exam Code 994,996,1055,1056,1046*/
            ...
            if($this->session->userdata('examcode') == 994 || $this->session->userdata('examcode') == 996 || $this->session->userdata('examcode') == 1055 || $this->session->userdata('examcode') == 1056 || $this->session->userdata('examcode') == 1046){ 
                $this->form_validation->set_rules('name_of_bank_bc', 'Name of Bank where working as BC', 'trim|required|xss_clean');
                ...
                //$this->form_validation->set_rules('empidproofphoto', 'Upload Bank BC ID Card', 'file_required|file_allowed_type[jpg,jpeg]|file_size_max[300]|callback_empidproofphoto_upload'); //|file_size_min[100]
                ...
                $error_bank_bc = '';
                if($this->input->post('doj1') != ''){  
                    //$this->form_validation->set_rules('doj1', 'Date of Joining', 'trim|required|xss_clean|callback_check_date_of_joining_bc_validation');
                    $date_of_commenc_bc = $this->input->post('doj1');
                    if ($date_of_commenc_bc != "") { 
                          ...
                          {
                            $error_bank_bc = 'Only Agents who joined the Bank as BC on or before March 31, 2024, are eligible.';  
                          }
                        ...
                        $error_bank_bc = 'Date of joining field is required.'; 
                    } 
                }    
                ...
                if ($ippb_emp_id != "" && $ippb_emp_id != "0" && $name_of_bank_bc != '') {
                    ...
                    //$this->db->where("regid != ",$this->session->userdata('regid')); 
                    ...
                    //echo $this->db->last_query();
                    if ($prev_count > 0) {
                        $error_bank_bc = 'Bank BC ID No Already Exists for selected Name of Bank.'; 
                    } 
                } else if ($ippb_emp_id == "0") {
                    $error_bank_bc = 'Bank BC ID No field is required.'; 
                } else if ($name_of_bank_bc == "") {
                    $error_bank_bc = 'Name of Bank field is required.'; 
                }

                if($error_bank_bc != ""){
                    $this->session->set_flashdata('error', $error_bank_bc);
                    redirect(base_url() . 'bulk/BulkApplyNM/add_member');
                } 
            ...
            if ($this->form_validation->run() == true) {
                ...
                $date = date('Y-m-d');
                $empidproof_file =$declaration_file='';
					...
					if (isset($_FILES['empidproofphoto']['name']) && ($_FILES['empidproofphoto']['name'] != '')) {
						...
						if ($size) {
							if ($this->upload->do_upload($img)) {
                              ...
                              //  echo $outputempidproof1.'<br>';exit;
							} else {
                            ...
                            //echo'<pre>'.$outputempidproof1;exit;
						} else {
                ...
                $bank_bc_id_card_file_path = $bank_bc_id_card_filename = $date_of_commenc_bc = '';
                if ($this->session->userdata('examcode') == 994 || $this->session->userdata('examcode') == 996 || $this->session->userdata('examcode') == 1055 || $this->session->userdata('examcode') == 1056 || $this->session->userdata('examcode') == 1046) 
                {
                    ...
                    if (isset($_FILES['empidproofphoto']['name']) && ($_FILES['empidproofphoto']['name'] != '')) 
                    {
                        ...
                        if ($size) 
                        {
                            ...
                            {
                                $dt = $this->upload->data(); 
                                if ($dt['file_size'] < 100) { // Minimum 100 KB
                                    $var_errors .= 'The uploaded Bank BC ID Card file size must be at least 100 KB.'; 
                                    $this->session->set_flashdata('error', 'The uploaded Bank BC ID Card file size must be at least 100 KB.');
                                    redirect(base_url() . 'bulk/BulkApplyNM/add_member');
                                }else{
                            ...
                            {
                                $this->session->set_flashdata('error', 'Bank BC ID Card :' . $this->upload->display_errors());
                                redirect(base_url() . 'bulk/BulkApplyNM/add_member');
                            }
                        } 
                        else 
                ...
                $user_data = array('photo' => '',
                    ...
                    'empidproofphotoname'	=> $empidproof_file, //priyanka d -30-july
					'declarationphotoname'              => $declaration_file,//priyanka d -30-july
    ...
    public function comApplication_reg()
    {
        ...
        if (isset($_POST['btnSubmit'])) {
            ...
            if($this->session->userdata('examcode') == 994 || $this->session->userdata('examcode') == 996 || $this->session->userdata('examcode') == 1055 || $this->session->userdata('examcode') == 1056 || $this->session->userdata('examcode') == 1046){ 
                ...
                $this->form_validation->set_rules('ippb_emp_id', 'Bank BC ID No', 'required|alpha_numeric_spaces|max_length[20]|xss_clean');
                //$this->form_validation->set_rules('empidproofphoto', 'Upload Bank BC ID Card', 'file_required|file_allowed_type[jpg,jpeg]|file_size_min[100]|file_size_max[300]|callback_empidproofphoto_upload'); //|file_size_min[100]
                if ($this->input->post('name_of_bank_bc') != '') {
                    $name_of_bank_bc = $this->input->post('name_of_bank_bc');
                    $this->form_validation->set_rules('ippb_emp_id', 'Bank BC ID No', 'trim|required|alpha_numeric_spaces|xss_clean|callback_check_bank_bc_id_no_duplication[' . $name_of_bank_bc . ']');
                }
            ...
            if ($this->form_validation->run() == true) {
              ...
              if ($scannedphoto_file != '' && $idproof_file != '' && $scannedsignaturephoto_file != '') {
                    ...
                    $user_data = array(
                        ...
                        'empidproofphotoname'	       => $empidproof_file, //priyanka d -30-july
						'declarationphotoname'         => $declaration_file,//priyanka d -30-july
                        'institutionworking'          => $_POST['institutionworking'],//Pooja M - 17-02-2025
    ...
    public function Msuccess()
    {
        ...
        if (isset($_POST['btnPreview'])) {
            ...
            $update_array1 = array('bank_branch' => $this->session->userdata['examinfo']['bank_branch'],
                ...
                if( $this->session->userdata['examinfo']['excd'] == 1009 ) { // priyanka d - 20-8-24 >> fedai
                    $update_array1['empidproofphoto'] = $this->session->userdata['examinfo']['empidproofphotoname'];
                    $update_array1['declaration']         = $this->session->userdata['examinfo']['declarationphotoname'];
                }
            ...
            if ($inser_id) {
                ...
                //skippedadmitcard so inserting vendor details in new table on 12 Dec 2025 added by Anil S
                if(in_array($this->session->userdata['examinfo']['excd'],$this->config->item('skippedAdmitCardForExams'))) 
                {
                    $curr_center_info = $this->master_model->getRecords('center_master', array('center_code'=>$this->session->userdata['examinfo']['txtCenterCode'],'exam_name' =>$this->session->userdata['examinfo']['excd']));
                ...
                //Exam Code 994,996,1055,1056,1046
                $sess_exam_code = $this->session->userdata['examinfo']['excd'];
                if($sess_exam_code == 994 || $sess_exam_code == 996 || $sess_exam_code == 1055 || $sess_exam_code == 1056 || $sess_exam_code == 1046){
                ...
                if (!empty($this->session->userdata['examinfo']['subject_arr'])) {
                    ...
                    foreach ($this->session->userdata['examinfo']['subject_arr'] as $k => $v) {
                        ...
                        bulk_logactivity($log_title = "bulk_member_admitcard data ", $log_message = $this->db->last_query(), $this->session->userdata['institute_id']);
                    ...
                    if ($this->session->userdata['examinfo']['excd'] != 101
                        ...
                        && $this->session->userdata['examinfo']['excd'] != 1046
    ...
    public function Msuccess_reg()
    {
        ...
        // echo '<pre>',print_r($this->session->userdata['enduserinfo']),'</pre>';die;
        ...
        if (isset($_POST['btnSubmit'])) {
            ...
            $institutionworking = $empidproofphotoname = $declarationphotoname='';
			
			// priyanka d
			$declaration = '';
			if( $this->session->userdata['enduserinfo']['excd'] == 1009 ) {
				$empidproofphotoname = $this->session->userdata['enduserinfo']['empidproofphotoname'];
				$declarationphotoname         = $this->session->userdata['enduserinfo']['declarationphotoname'];
                $institutionworking = $this->session->userdata['enduserinfo']['institutionworking'];
			}

            $name_of_bank_bc = $ippb_emp_id = $date_of_commenc_bc = $bank_bc_id_card = ''; 
              if ($this->session->userdata['enduserinfo']['excd'] == 994 || $this->session->userdata['enduserinfo']['excd'] == 996 || $this->session->userdata['enduserinfo']['excd'] == 1055 || $this->session->userdata['enduserinfo']['excd'] == 1056 || $this->session->userdata['enduserinfo']['excd'] == 1046)
              {
            ...
            $insert_info = array(
                ...
                'associatedinstitute'   =>$institutionworking, // priyanka d >> 26-july-24

                'empidproofphoto' => $empidproofphotoname,// priyanka d >> 26-july-24
                'declaration' => $declarationphotoname,// priyanka d >> 26-july-24
            ...
            //echo "<pre>".print_r($insert_info)."</pre>";die;
                ...
                if ($exam_last_id = $this->master_model->insertRecord('member_exam', $inser_exam_array, true)) {
                    ...
                    if (!empty($this->session->userdata['enduserinfo']['subject_arr'])) {
                        ...
                        if ($this->session->userdata['enduserinfo']['excd'] != 101
                            ...
                            && $this->session->userdata['enduserinfo']['excd'] != 1046
...
//echo $examcode;exit;
    ...
    public function examdate($regnumber = null, $exam_code = null)
    {
        ...
        //echo "Period==".$this->session->userdata['exmCrdPrd']['exam_prd'];die;

        if(in_array($exam_code,array(1055,1056,1046))){

            $subject_master_exam_date = $this->master_model->getRecords('subject_master', array('exam_code' => $exam_code, 'exam_date >=' => $today_date, 'subject_delete' => '0'));
            if (count($subject_master_exam_date) > 0){

                $this->db->join('bulk_exam_activation_master', 'bulk_exam_activation_master.exam_code=member_exam.exam_code AND bulk_exam_activation_master.exam_period=member_exam.exam_period');
                $this->db->where("bulk_exam_activation_master.institute_code", $this->session->userdata('institute_id'));
                $this->db->where('institute_id!=', '');

                $getapplied_exam_code = $this->master_model->getRecords('member_exam', array('regnumber' => $regnumber, 'bulk_isdelete' => '0'), 'member_exam.exam_code,member_exam.exam_period'); //'pay_status'=>'1'
                ...
                if (count($getapplied_exam_code) <= 0) {
                    ...
                    $getapplied_exam_code = $this->master_model->getRecords('member_exam', array('regnumber' => $regnumber, 'pay_status' => '1'), 'member_exam.exam_code,member_exam.exam_period');
                ...
                if (count($getapplied_exam_code) > 0) {
                    ...
                    foreach ($getapplied_exam_code as $exist_ex_code) {
                        //'exm_cd' => $exist_ex_code['exam_code'], 'exm_prd' => $exist_ex_code['exam_period'], 
                        $getapplied_exam_date = $this->master_model->getRecords('admit_card_details', array('mem_mem_no' => $regnumber));
                        if (count($getapplied_exam_date) > 0) {

                            foreach ($getapplied_exam_date as $exist_ex_date) {

                                foreach ($subject_master_exam_date as $sub_ex_date) {

                                    if ($exist_ex_date['exam_date'] == $sub_ex_date['exam_date']) {
            ...
            return $flag; // For Exam Code 1055 & 1056 & 1046
    ...
    public function get_alredy_applied_examname($regnumber = null, $exam_code = null)
    {
        ...
        if(in_array($exam_code,array(1055,1056,1046))){
            ...
            $exam_info = $this->session->userdata('exmCrdPrd'); //exam_prd //echo $exam_info['exam_prd'];

            $this->db->select('subject_master.*,exam_master.description'); 

            $this->db->where('admit_card_details.remark','1');

            $this->db->join('admit_card_details', 'admit_card_details.exm_cd=subject_master.exam_code AND admit_card_details.exm_prd=subject_master.exam_period');
            $this->db->join('exam_master', 'exam_master.exam_code=subject_master.exam_code'); 

            $applied_exam_date = $this->master_model->getRecords('subject_master', array('subject_master.exam_code' => $exam_code,'subject_master.exam_period' => $exam_info['exam_prd'], 'subject_master.exam_date >=' => $today_date, 'subject_master.subject_delete' => '0'));             
            
            if (count($applied_exam_date) > 0)
            {
                $msg = "You have already applied for <strong>" . $applied_exam_date[0]['description'] . "</strong> falling on same day, So you can not apply for <strong>" . $applied_exam_date[0]['description'] . "</strong> examination.";
                //break;
            }

        }else if (count($applied_exam_date) > 0) {
            ...
            if (count($getapplied_exam_code) > 0) {
                ...
                foreach ($getapplied_exam_code as $exist_ex_code) {
                    ...
                    /*if(in_array($exam_code,array(1055,1056,1046))){
                        $exam_info = $this->session->userdata('exmCrdPrd'); //exam_prd //echo $exam_info['exam_prd'];
                        $this->db->where('exam_period',$exam_info['exam_prd']);
                    }*/
                    $getapplied_exam_date = $this->master_model->getRecords('subject_master', array('exam_code' => $exist_ex_code['exam_code'], 'exam_date >=' => $today_date, 'subject_delete' => '0'));
                    //echo $this->db->last_query();
                    if (count($getapplied_exam_date) > 0) {
...
<tr>
    ...
    public function check_emailduplication($email)
    {
        ...
        if ($email != "") {
            ...
            if ($this->session->userdata('examcode') == 101 && $this->session->userdata('examcode') == 1046 && $this->session->userdata['institute_id'] == 9994) {
