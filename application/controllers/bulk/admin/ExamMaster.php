...
class ExamMaster extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('iibfdra/admin/masters/exam_list',$data);
	}
	...
	public function getList(){
		...
		$url = base_url()."iibfdra/admin/ExamMaster/getList/";
		...
		if($res)
		{
			...
			foreach($result as $row)
			{
				...
				$action = '<a href="'.base_url().'iibfdra/admin/ExamMaster/edit/'.$row['id'].'">Edit |</a><a href="'.base_url().'iibfdra/admin/ExamMaster/delete/'.$row['id'].'" onclick="'.$confirm.'">Delete </a>';
				$data['action'][] = $action;
			}
	...
	public function add()
	{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				if($upload_flg == 1 || $upload_flg == 2)
				{
					...
					if($this->master_model->insertRecord('dra_exam_master',$insert_data))
					{
						...
						redirect(base_url().'iibfdra/admin/ExamMaster');
					}
					else
					{
						...
						redirect(base_url().'iibfdra/admin/ExamMaster/add');
					}
				}
		...
		$data['breadcrumb'] = '<ol class="breadcrumb"><li><a href="'.base_url().'iibfdra/admin/MainController"><i class="fa fa-dashboard"></i> Home</a></li>
					<li><a href="'.base_url().'iibfdra/admin/'.$this->router->fetch_class().'">Manage Exam Master</a></li>
					<li class="active">Add</li>
				</ol>';
		
		$data["exam_type_list"] = $this->Master_model->getRecords("exam_type");
		$this->load->view('iibfdra/admin/masters/exam_add',$data);
	}
	...
	public function edit()
	{
		...
		if(isset($_POST['btnSubmit']))
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				else
				{
					...
					redirect(base_url().'iibfdra/admin/ExamMaster/edit/'.$id);
				}
			}
			else
			{
		...
		$data["exam_type_list"] = $this->Master_model->getRecords("exam_type");
		$this->load->view('iibfdra/admin/masters/exam_edit',$data);
	}
	...
	public function import(){
							...
							while (($line = fgets($handle)) !== false) {
								...
								else
								{
									...
									else
									{
										...
										redirect(base_url().'iibfdra/admin/ExamMaster/import');
									}
								}
							}
		...
		$this->load->view('iibfdra/admin/masters/exam_import', $data);
	}
