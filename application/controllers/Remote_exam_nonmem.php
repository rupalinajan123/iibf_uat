...
class Remote_exam_nonmem extends CI_Controller {
	...
	public function examdetails(){ 
		...
		$this->db->where('regnumber',$this->session->userdata('nmregnumber'));
		...
		$this->db->where('mem_mem_no',$this->session->userdata('nmregnumber'));
		...
		if(isset($_POST['btnPreviewSubmit']))  	
		{
			...
			$this->form_validation->set_rules('scribe_flag','Scribe Services');
				...
				if($this->form_validation->run()==TRUE)
				{
					...
					if(count($venue) >0 && count($date) >0 && count($time) >0)	
					{
						...
						{	
							...
							redirect(base_url().'Remote_exam_nonmem/examdetails/?ExId='.$this->input->get('excode2').'&Extype='.$this->input->get('Extype').'&Exprd='.base64_encode($exam_period)); 
								}
							}
						}
						if($sub_flag==0)
		...
		if($member_number == '' ){ 
			redirect(base_url().'Nonmember/index/');
		}else{
			...
			//user information
			$user_info=$this->master_model->getRecords('member_registration',array('isactive'=>'1','regnumber'=>$this->session->userdata('nmregnumber')));
			...
			else
			{
				...
				{
					$data=array('scribe_disability' => $scribe_disability,'middle_content'=>'remote_exam/comApplication_nonmem','states'=>$states,'undergraduate'=>$undergraduate,'graduate'=>$graduate,'postgraduate'=>$postgraduate,'institution_master'=>$institution_master,'designation'=>$designation,'user_info'=>$user_info,'idtype_master'=>$idtype_master,'examinfo'=>$examinfo,'medium'=>$medium,'center'=>$center,'caiib_subjects'=>$caiib_subjects,'compulsory_subjects'=>$compulsory_subjects,'benchmark_disability_info' => $benchmark_disability_info,'exam_period'=>$exam_period);
					$this->load->view('nonmember/nm_common_view',$data);
				}
	 ...
	 foreach($subject_arr1 as $k => $v)
      {
		$flag=allowed_examdate($this->session->userdata('nmregnumber'),$this->session->userdata('examcode'),$v['date']);
		 if ($flag == 1)
			  {
					redirect(base_url().'Remote_exam_nonmem/info');
			 }
	  }
	 }
		...
		{
			redirect(base_url().'Remote_exam_nonmem/accessdenied/');
		}
		...
		{		
			...
			foreach($subject_arr as $k=>$v)
			{
				...
				{
					$this->session->set_flashdata('error',$msg);
					redirect(base_url()."Remote_exam_nonmem/examdetails/?ExId=".$this->session->userdata['examinfo']['excd']."&Extype=".$this->session->userdata['examinfo']['extype']."&Exprd=".$this->session->userdata['examinfo']['eprid']);
				}
			}
		}
		if($sub_flag==0)
		...
		{		
			...
			else
			{
			        ...
			        'scribe_sub_disability' => $scribe_sub_disability,'middle_content'=>'remote_exam/exam_preview_nonmem','user_info'=>$user_info,'medium'=>$medium,'center'=>$center,'misc'=>$misc,'states'=>$states,'compulsory_subjects'=>$this->session->userdata['examinfo']['subject_arr'],'benchmark_disability_info' => $benchmark_disability_info);
			}
			$this->load->view('nonmember/nm_common_view',$data);
		}
		else
				...
				foreach($this->session->userdata['examinfo']['subject_arr'] as $k=>$v)
				{
					...
					if(count($exam_admicard_details) > 0){
						foreach($exam_admicard_details as $row){
							...
							if($capacity==0){
								$log_title =" phase-3 Remote exam nonmem Capacity full id:".$this->session->userdata('mregnumber_elapplyexam');
								...
								storedUserActivity($log_title, $log_message, $rId, $regNo);
								redirect(base_url().'Remote_exam_nonmem/exapplylogin/');
							}
						}
					}
						...
						foreach($exam_admicard_details as $row){
							...
							if($seat_number!=''){
								...
								if(count($admit_card_details) > 0){
									...
									$log_title ="phase-3 Remote exam nonmem Seat number already allocated id:".$this->session->userdata('mregnumber_elapplyexam');
									$log_message = serialize($exam_admicard_details);
									$rId = $admit_card_details[0]['admitcard_id'];
									$regNo = $this->session->userdata('mregnumber_elapplyexam');
									storedUserActivity($log_title, $log_message, $rId, $regNo);
								}else{
									$log_title ="phase-3 Remote exam nonmem Fail user seat allocation id:".$this->session->userdata('mregnumber_elapplyexam');
									$log_message = serialize($exam_admicard_details);
									$rId = $this->session->userdata('mregnumber_elapplyexam');
									$regNo = $this->session->userdata('mregnumber_elapplyexam');
									storedUserActivity($log_title, $log_message, $rId, $regNo);
					...
					$admitcard_pdf=remote_genarate_admitcard($this->session->userdata('nmregnumber'),base64_decode($this->session->userdata['examinfo']['excd']),$this->session->userdata['examinfo']['eprid']);
					...
					if($admitcard_pdf!=''){
						...
						$admitcard_image = base64_decode($this->session->userdata['examinfo']['excd']).'_'.$this->session->userdata['examinfo']['eprid'].'_'.$this->session->userdata('nmregnumber').'.pdf'; 
		...
		if(isset($_POST['btnPreview']))
		{
			...
			if($inser_id=$this->master_model->insertRecord('member_exam',$inser_array,true))
			{
				...
				if($this->session->userdata['examinfo']['photo']!='')
				{
					...
					$photo_name=$this->master_model->getRecords('member_registration',array('regid'=>$this->session->userdata('mregid_applyexam'),'regnumber'=>$this->session->userdata('nmregnumber')),'scannedphoto');
				...
				if($this->session->userdata['examinfo']['signname']!='')
				{
					...
					$sing_name=$this->master_model->getRecords('member_registration',array('regid'=>$this->session->userdata('mregid_applyexam'),'regnumber'=>$this->session->userdata('nmregnumber')),'scannedsignaturephoto');
				...
				if(count($update_array) > 0)
				{
					...
					$prevData = array();
					$user_info=$this->master_model->getRecords('member_registration',array('regid'=>$this->session->userdata('mregid_applyexam'),'regnumber'=>$this->session->userdata('nmregnumber'),'isactive'=>'1'));
					if(count($user_info))
					{
					...
					$this->master_model->updateRecord('member_registration',$update_array,array('regid'=>$this->session->userdata('mregid_applyexam'),'regnumber'=>$this->session->userdata('nmregnumber')));
					
					log_profile_user($log_title = "phase-3 Profile updated successfully", $edited,'data',$this->session->userdata('mregid_applyexam'),$this->session->userdata('nmregnumber'));
				...
				{
					redirect(base_url().'Remote_exam_nonmem/sbi_make_payment/');
				}
				else
				{
					redirect(base_url().'Remote_exam_nonmem/make_payment/');
				}
			}
		}
		else
	...
	public function sbi_make_payment()
	{
		...
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			$pg_success_url = base_url()."Remote_exam_nonmem/sbitranssuccess";
			$pg_fail_url    = base_url()."Remote_exam_nonmem/sbitransfail";
			...
			$log_title = "phase-3 Remote exam nonmem Exam invoice data from applyexam cntrlr inser_id = '".$inser_id."'";
		 ...
		 //############check Gender########
			...
			if(!empty($this->session->userdata['examinfo']['subject_arr']))
			{
					foreach($this->session->userdata['examinfo']['subject_arr'] as $k=>$v)
					{
						...
						$log_title ="phase-3 Remote exam nonmem  Admit card data from Applyexam cntrlr";
					...
					$check_eligibility_for_applied_exam=$this->master_model->getRecords('eligible_master',array('eligible_master.exam_code'=>base64_decode($this->session->userdata['examinfo']['excd']),'member_no'=>$this->session->userdata('nmregnumber'),'eligible_period'=>$this->session->userdata['examinfo']['eprid']));
					...
					if(count($check_eligibility_for_applied_exam) <= 0 || $check_eligibility_for_applied_exam[0]['app_category']=='R')
					{
						if(!empty($this->session->userdata['examinfo']['subject_arr']))
						{
							...
							foreach($this->session->userdata['examinfo']['subject_arr'] as $k=>$v)
							{
								$check_admit_card_details=$this->master_model->getRecords('admit_card_details',array('mem_mem_no'=>$this->session->userdata('nmregnumber'),'exm_cd'=>base64_decode($this->session->userdata['examinfo']['excd']),'sub_cd'=>$k,'venueid'=>$v['venue'],'exam_date'=>$v['date'],'time'=>$v['session_time'],'center_code'=>$this->session->userdata['examinfo']['selCenterName']));
						...
						{
								$log_title = "phase-3 Remote exam nonmem Fresh Member subject missing applyexam cntrlr";
					...
					else
					{
							...
							{
								$log_title = "phase-3 Remote exam nonmem Existing Member subject missing  applyexam cntrlr";
								$log_message = serialize($this->session->userdata['examinfo']['subject_arr']);
								$rId = $this->session->userdata('nmregnumber');
								$regNo = $this->session->userdata('nmregnumber');
								storedUserActivity($log_title, $log_message, $rId, $regNo);
...
=1000003|DOM|IN|INR|2|Other|https://test.sbiepay.coom/secure/fail.jsp|SBIEPAY|2|2|NB|ONLINE|ONLINE
	...
	if ($q_details)
	{
		if ($q_details[2] == "SUCCESS")
		{
			...
			if($get_user_regnum[0]['status']==2)
			{
				...
				$log_title = "phase-3 Remote exam nonmem update payment table after success  = '".$MerchantOrderNo."'";
				...
				if(count($exam_admicard_details) > 0){
					...
					foreach($exam_admicard_details as $row){
						...
						if($seat_number!=''){
							...
							$log_title ="phase-3 Remote exam nonmem Seat number allocated id:".$this->session->userdata('mregnumber_elapplyexam');
							$log_message = serialize($exam_admicard_details);
							$rId = $admit_card_details[0]['admitcard_id'];
							$regNo = $this->session->userdata('mregnumber_elapplyexam');
							storedUserActivity($log_title, $log_message, $rId, $regNo);
							...
							if(count($admit_card_details) > 0){
								...
								$log_title ="phase-3  Remote exam nonmem nonmem Fail user seat allocation id:".$this->session->userdata('mregnumber_elapplyexam');
								$log_message = serialize($exam_admicard_details);
								$rId = $this->session->userdata('mregnumber_elapplyexam');
								$regNo = $this->session->userdata('mregnumber_elapplyexam');
								storedUserActivity($log_title, $log_message, $rId, $regNo);
				...
				$log_title = "phase-3 Remote exam nonmem update member exam pay status  = '".$get_user_regnum[0]['ref_id']."'";
				...
				if(count($getinvoice_number) > 0)
				{
					...
					if($invoiceNumber)
					{
						...
						$log_title = "phase-3 Remote exam nonmem invoice number generation done  = '".$invoiceNumber."'";
				...
				{	
					...
					$log_title = "phase-3 Remote exam nonmem invoice image generation done  = '".$invoiceNumber."'";
				...
				{
					$log_title ="phase-3 Remote exam nonmem B2B Update fail:".$get_user_regnum[0]['member_regnumber'];
					$log_message = serialize($update_data);
					$rId = $MerchantOrderNo;
	...
	///End of SBICALL Back	
		...
		$this->load->view('nonmember/nm_common_view',$data_arr);
	...
	public function sbitransfail()
	{
		if (isset($_REQUEST['encData']))
		{
			...
			//End Of SBICALL Back	
			redirect(base_url().'Remote_exam_nonmem/fail/'.base64_encode($MerchantOrderNo));
		}
		else
		{
...
<a href="' . base_url() . 'GstRecovery/" target="new">click here</a> </div>';
		$data=array('middle_content'=>'nonmember/nmmember_notification','check_eligibility'=>$message);
  	    $this->load->view('nonmember/nm_common_view',$data);
	}
	
	public function info()
	{
		$message = $this->get_alredy_applied_examname($this->session->userdata('nmregnumber'),$this->session->userdata['examinfo']['excd']);
