...
class Applyjaiib_centerchange_special extends CI_Controller {
...
public function login(){ 
	...
	if(isset($_POST['submit'])){  
			...
			if ($this->form_validation->run() == TRUE){
				...
				$chk_old_app = $this->master_model->getRecords('admit_card_details_jaiibcenter_change_scpl','','mem_mem_no');  
				//echo $this->db->last_query(); die;
				if(count($chk_old_app) <= 0)
				{
					...
					$this->db->where('app_update','0');
					//$this->db->where('created_on LIKE','%2022-02-%');
					...
					if(count($user_info) > 0 && count($chk_app) > 0 ){
						...
						if($chk_app[0]['app_update'] != 0){ 
							 ...
							 redirect(base_url().'Applyjaiib_centerchange_special/comApplication');
						}
...
public function comApplication(){  
	...
	if($this->session->userdata('memexcode')==''){
		redirect(base_url().'Applyjaiib_centerchange_special/login');
	}
		...
		if($this->form_validation->run()==TRUE){
			...
			if(count($venue) >0 && count($date) >0 && count($time) >0){
				foreach($venue as $k=>$v){
						...
						$subject_arr[$k]=array('venue_code'=>$v,'date'=>$date[$k],'session_time'=>$time[$k],'subject_name'=>$compulsory_subjects_name[0]['sub_dsc']);
				}
			...
			redirect(base_url().'Applyjaiib_centerchange_special/preview');
		}
	}
	$this->db->where('mem_mem_no',$this->session->userdata('mregnumber_applyexam'));
	$this->db->where('exm_cd',$this->session->userdata('memexcode'));
	$this->db->where('exm_prd',$this->session->userdata('memexprd'));
	$this->db->where('remark',1);
	$compulsory_subjects = $this->master_model->getRecords('admit_card_details','','sub_cd,sub_dsc,center_code,venueid,exam_date,time,center_name',array('sub_cd'=>'ASC'));
	//echo $this->db->last_query();
	...
	$this->db->where('exm_prd',$this->session->userdata('memexprd'));
	//$this->db->where('remark',0);
	$new_exam_dates = $this->master_model->getRecords('admit_card_details_jaiibcenter_change_scpl','','exam_date',array('sub_cd'=>'ASC'));
	//echo $this->db->last_query(); //die;
	foreach ($new_exam_dates as $row){
		//$data[] = array('name' => $row->name);
		$new_date[] = $row['exam_date']; //die;
	...
	//echo $this->db->last_query();
	$data=array('middle_content'=>'jaiibcenterchange_comApplication','user_info'=>$user_info,'examinfo'=>$examinfo,'compulsory_subjects'=>$compulsory_subjects,'center'=>$center,'old_center'=>$old_center,'new_date'=>$new_date);
...
public function preview(){
	...
	$this->db->where("center_delete",'0');
	$center=$this->master_model->getRecords('center_master','','',array('center_name'=>'ASC'));
	//print_r($this->session->userdata['examinfo']['subject_arr']);//echo $this->db->last_query(); 
	$data=array('middle_content'=>'jaiibcenterchange_preview','user_info'=>$user_info,'examinfo'=>$examinfo,'compulsory_subjects'=>$this->session->userdata['examinfo']['subject_arr'],'center'=>$center);
	$this->load->view('memapplyexam/mem_apply_exam_common_view',$data);
}
public function add_record(){ 
	...
	$this->db->where('mem_mem_no',$this->session->userdata('mregnumber_applyexam'));
	$this->db->where('exm_cd',$this->session->userdata('memexcode'));
	$this->db->where('exm_prd','121');//$this->session->userdata('memexprd')
	$this->db->where('remark',1);
	$this->db->where('app_update','1');
	$chk_already_apppy = $this->master_model->getRecords('admit_card_details','','admitcard_id,app_update');
	if(isset($chk_already_apppy) && count($chk_already_apppy) > 0 ){
	...
	if(!empty($this->session->userdata['examinfo']['subject_arr'])){ 
		...
		foreach($this->session->userdata['examinfo']['subject_arr'] as $k=>$v){
			...
			$get_subject_details=$this->master_model->getRecords('venue_master',array('venue_code'=>$v['venue_code'],'exam_date'=>$v['date'],'session_time'=>$v['session_time'],'center_code'=>$this->session->userdata['examinfo']['selCenterName']));
			//echo $this->db->last_query(); exit;
							...
							$admitcard_insert_array=array(
													'mem_exam_id'=>$admit_card_data[0]['mem_exam_id'],//$this->session->userdata['examinfo']['insdet_id'],
		...
		//exit; 
		$this->db->where('mem_mem_no',$this->session->userdata('mregnumber_applyexam'));
			$this->db->where('mem_exam_id',$admit_card_data[0]['mem_exam_id']);
			$this->db->where('exm_cd',$this->session->userdata('memexcode'));
			$this->db->where('exm_prd','221');
			$this->db->where('remark','2');
			//$this->db->where('exam_date LIKE','%2022-02-%');
			$this->db->where('created_on LIKE','%2022-02-%');
			$exam_admicard_details = $this->master_model->getRecords('admit_card_details','');
			//echo $this->db->last_query(); //die;
			//echo '<pre>'; print_r($exam_admicard_details); echo '</pre>'; //die;
			$this->db->where('mem_mem_no',$this->session->userdata('mregnumber_applyexam'));
			$this->db->where('exm_cd',$this->session->userdata('memexcode'));
			$this->db->where('exm_prd','221');
			$this->db->where('remark',0);
			...
			foreach($exam_admicard_details as $row)
					...
					$admit_card_details=$this->master_model->getRecords('admit_card_details',array('venueid'=>$row['venueid'],'exam_date'=>$date,'time'=>$row['time'],'mem_exam_id'=>$admit_card_data[0]['mem_exam_id'],'sub_cd'=>$row['sub_cd'] , 'admitcard_id'=>$row['admitcard_id']));
					//echo $this->db->last_query().'<br>'; //die;
					//seat allocation
					$seat_number=getseat($this->session->userdata('memexcode'),$this->session->userdata['examinfo']['selCenterName'],$get_subject_details[0]['venue_code'],$date,$get_subject_details[0]['session_time'],'221',$row['sub_cd'],$get_subject_details[0]['session_capacity'],$row['admitcard_id']);
					//print_r($seat_number); //die; 
					if($seat_number!='')
					{ 
						$final_seat_number = $seat_number;
						$update_data = array('pwd' => $password,'seat_identification' => $final_seat_number,'remark'=>1,'app_update'=>1,'modified_on'=>date('Y-m-d H:i:s'));
						$this->master_model->updateRecord('admit_card_details',$update_data,array('admitcard_id'=>$row['admitcard_id']));
						//echo $this->db->last_query().'<br>';
						 $update_mem = array('app_update'=>1,'modified_on'=>date('Y-m-d H:i:s'));
						$this->master_model->updateRecord('member_exam',$update_mem,array('id'=>$admit_card_data[0]['mem_exam_id'])); 
						//echo $this->db->last_query().'<br>';
						$update_mem = array('remark'=>1,'modified_on'=>date('Y-m-d H:i:s'),'app_update'=>1);
						$this->master_model->updateRecord('admit_card_details_jaiibcenter_change_scpl',$update_mem,array('mem_mem_no'=>$this->session->userdata('mregnumber_applyexam'))); 
						//echo $this->db->last_query().'<br>';
						
						$update_adinfo = array('center_code'=>$row['center_code'],
											'venueid'=>$get_subject_details[0]['venue_code'],
											'venueadd1'=>$get_subject_details[0]['venue_name'],
											'venueadd2'=>$get_subject_details[0]['venue_addr1'],
											'venueadd3'=>$get_subject_details[0]['venue_addr2'],
											'venueadd4'=>$get_subject_details[0]['venue_addr3'],
											'venueadd5'=>$get_subject_details[0]['venue_addr4'],
											'venpin'=>$get_subject_details[0]['venue_pincode'],
						...
						$this->master_model->updateRecord('admitcard_info',$update_adinfo,array('mem_mem_no'=>$this->session->userdata('mregnumber_applyexam'),'exm_cd'=>$this->session->userdata('memexcode'),'remark'=>'0'));
						
						 //echo $this->db->last_query().'<br>';
					} 
					else
					{ 
						$admit_card_details=$this->master_model->getRecords('admit_card_details',array('mem_exam_id'=>$admit_card_details[0]['mem_exam_id'],'remark'=>1));
						//echo $this->db->last_query();
		...
		$admitcard_pdf=genarate_admitcard_centerchange_custom($this->session->userdata('mregnumber_applyexam'),$this->session->userdata('memexcode'),'221',$admitcard_id_arr);
		if($admitcard_pdf != ''){
			$this->db->where('exam_code',$this->session->userdata('memexcode'));
			...
			if($this->Emailsending->mailsend_attch($info_arr,$files)){
				...
				$regNo = $this->session->userdata('mregnumber_applyexam');
				storedUserActivity($log_title, $log_message, $rId, $regNo);
				redirect(base_url().'Applyjaiib_centerchange_special/mail_success');
			}else{
				$log_title ="JAIIB free Member exam apply details";
				...
				$regNo = $this->session->userdata('mregnumber_applyexam');
				storedUserActivity($log_title, $log_message, $rId, $regNo);
				redirect(base_url().'Applyjaiib_centerchange_special/mail_fail');
			}
		}
	}
}
...
public function mail_success(){
	...
	$this->db->where("center_delete",'0');
	$center=$this->master_model->getRecords('center_master','','',array('center_name'=>'ASC'));
	$data=array('middle_content'=>'jaiibcenterchange_success','user_info'=>$user_info,'examinfo'=>$examinfo,'center'=>$center);
	$this->load->view('memapplyexam/mem_apply_exam_common_view',$data);
}
