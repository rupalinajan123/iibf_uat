<?php
/*
 * Controller Name	:	Cron Member Invoice Settlement
 * Created By		:	Padmashri Joshi
 * Created Date		:	24-09-2019
 * Last Update 		:   24-09-2019
*/
defined('BASEPATH') OR exit('No direct script access allowed');
class Cron_member_invoice_settlement_hit  extends CI_Controller {
	...
	*/
	public function index()
	{
			...
			/*	echo "==>".$exam_invoice.'<br/>';*/
				$is_insert_in_settlement = 0;
 			  	...
 			  	if($is_insert_in_settlement == 1 )
 				{
 					...
 					$this->exam_settlement($res,$arr_member_exam,$val_is_new_record,$str_reason,'member_invoice');
 				}
 			}
		...
		{
			$arr_update = array('created_at' => '');
			$this->master_model->updateRecord('cron_limit',$arr_update,array('created_at' => $current_date));
		}
		echo "Done..!!";
	}
 	...
 	{
		//////die();
 	 	$this->db->where('record_type','member_invoice');
 		...
 		if(!empty($isExists) && count($isExists) > 0)
 		{
 			foreach ($isExists as $key => $res)
 			{
 				...
 				else
 				{
 					...
 					$data_result = dynamic_member_invoice_generation($res['member_regnumber'],$res['ref_id'],$res);
 					//echo "<br/> Dynamic invoice creation ".$data_result."<br/>";
 				  	/* update the status for why its here */
 	...
 	public function check_config($invoice_id)
 	{
 		if(isset($invoice_id) && $invoice_id!='')
 		{
			...
			$is_exists_in_config = $this->master_model->getRecords('config_reg_invoice');
			...
			{
				//$config_auto =  $this->master_model->insertRecord('config_reg_invoice',array('invoice_id'=>$invoice_id),'true');
				$insert_info1 = array('invoice_id'=>$invoice_id);
				$config_auto = str_pad($this->master_model->insertRecord('config_reg_invoice',$insert_info1,true), 6, "0", STR_PAD_LEFT); 
 	...
 	public function generate_invoice_no_image($config_auto_id,$type,$member_no='')
 	{
		...
		$str_year = $cal_year.'-'.$next_year; 
 		...
 		{
 			$str_return =$this->config->item('mem_invoice_no_prefix').$config_auto_id;
 		}
 		return $str_return;
 	}

 
 	public function exam_settlement($res,$arr_member_exam,$is_new_record,$str_reason='',$record_type)
 	{
		...
		//echo "<br/>".print_r($insert_data);echo "<br/><hr/>";
		$where = array(
						"exam_code"        => $exam_code,
 			...
 			foreach ($isExists as $key => $res)
 			{
 				...
 				if(!empty($arr_invoice))
 				{
 					...
 					if($invoice_id!='' && is_numeric($invoice_id))
 					{
 						 ...
 						 $path = custom_genarate_reg_invoice_new($invoice_id);
		 		 		//echo "<br/>";
		 		 		if(isset($path) && $path!='')
		 		 		{
