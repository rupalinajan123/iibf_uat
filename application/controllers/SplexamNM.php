...
class SplexamNM extends CI_Controller {
	...
	public function memlogin()
	{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				{ 
					 if($user_info[0]['isactive']==1)
					 {
						...
						redirect(base_url().'SpecialExamNm/showexam/?ExId='.$this->input->get('ExId').'&Extype='.$Extype);
						 }
		...
		$this->load->view('specialexamNM/nonmember_login',$data);
	...
	public function accessdenied()
	{
		...
		$data=array('middle_content'=>'specialexamNM/access-denied-registration','check_eligibility'=>$message);
		$this->load->view('specialexamNM/common_view_fullwidth',$data);
	}
	...
	{	
		...
		{
			redirect(base_url().'SplexamNM/accessdenied/');
		}
		 ...
		 if(isset($_POST['btnSubmit']))  	
		 {
				...
				if($this->form_validation->run()==TRUE)
				{
						...
						{	
							...
							foreach($subject_arr as $k=>$v)
							{
								...
								if($msg!='')
								{
									...
									redirect(base_url().'SplexamNM/member/?Mtype='.$this->input->get('Mtype').'=&ExId='.$this->input->get('ExId').'');
								}
							}
						}
						if($sub_flag==0)
					...
					{
						$user_data=array('firstname'		=>trim($_POST["firstname"]),
													'sel_namesub'	=>trim($_POST["sel_namesub"]),
													...
													'optedu'			=>trim($_POST["optedu"]),	
													'optnletter'		=>trim($_POST["optnletter"]),	
													...
													'idname'			=>$idproof_file,
													'selCenterName'		=>trim($_POST["selCenterName"]),
													'txtCenterCode'=>	trim($_POST["txtCenterCode"]),
													'optmode'			=>trim($_POST["optmode"]),
													'exid'				=>trim($_POST["exid"]),
													'mtype'				=>trim($_POST["mtype"]),
													'memtype'			=>trim($_POST["memtype"]),
													'eprid'				=>trim($_POST["eprid"]),
													'rrsub'				=>trim($_POST["rrsub"]),
													'excd'				=>trim($_POST["excd"]),
					...
					logactivity($log_title ="Non-Member user registration ", $log_message = serialize($user_data));			
					redirect(base_url().'SplexamNM/preview');
					}
					else
					{
		...
		/**/
			...
			####### get compulsory subject list##########
												 $this->db->group_by('subject_code');
			$compulsory_subjects=$this->master_model->getRecords('subject_master',array('exam_code'=>$ExId,'subject_delete'=>'0','group_code'=>'C','exam_period'=>$examinfo[0]['exam_period']));
			...
			if($last_id =$this->master_model->insertRecord('member_registration',$insert_info,true))
			{
					...
					$inser_exam_array=array(	'regnumber'=>$last_id,
						...
						if($exam_last_id=$this->master_model->insertRecord('member_exam',$inser_exam_array,true))
						{
							...
							{
									redirect(base_url().'SplexamNM/sbi_make_payment/');
							}
							else
							{
										 redirect(base_url()."SplexamNM/make_payment");
							}
						}
					else
					{
	...
	public function sbi_make_payment()
	{
		...
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			$pg_success_url = base_url()."SplexamNM/sbitranssuccess";
			$pg_fail_url    = base_url()."SplexamNM/sbitransfail";
			...
			$exam_period = '';
			$yearmonth=$this->master_model->getRecords('member_exam',array('id'=>$this->session->userdata['non_memberdata']['member_exam_id']),'examination_date');
			...
			$exam_code=$this->session->userdata['enduserinfo']['excd'];
			if($this->session->userdata['enduserinfo']['excd']==1770)
			{
				$exam_code=177;
			}
			...
			$log_title = "Exam invoice data from splexamNM cntrlr inser_id = '".$inser_id."'";
...
=1000003|DOM|IN|INR|2|Other|https://test.sbiepay.coom/secure/fail.jsp|SBIEPAY|2|2|NB|ONLINE|ONLINE
	...
	public function sbitranssuccess()
	{
		...
		if ($q_details)
		{
			...
			if($get_user_regnum[0]['status']==2)
			{
				...
				{			
					...
					foreach($exam_admicard_details as $row)
					{
						...
						if($capacity==0)
						{
							...
							redirect(base_url().'SplexamNM/refund/'.base64_encode($MerchantOrderNo));
						}
					}
				}
				...
				else
				{
					...
					$this->master_model->updateRecord('member_registration',$upd_files,array('regid'=>$reg_id));
					$log_title ="splexamNM member PICS MANUAL PICS Update :".$reg_id;
		...
		$get_user_regnum=$this->master_model->getRecords('payment_transaction',array('receipt_no'=>$MerchantOrderNo),'member_regnumber,,exam_code,status');
		...
		$insert_data = array(
			'examination_date' => $this->session->userdata['enduserinfo']['special_exam_date'],
			'center'          		 => $this->session->userdata['enduserinfo']['selCenterName'],
		...
		redirect(base_url().'SplexamNM/acknowledge/'.base64_encode($MerchantOrderNo));
	}
	...
	public function sbitransfail()
	{
		...
		if (isset($_REQUEST['encData']))
		{
			...
			$get_user_regnum_info=$this->master_model->getRecords('payment_transaction',array('receipt_no'=>$MerchantOrderNo),'member_regnumber,ref_id,status');
	...
	public function make_payment()
	{
			...
			$pg_return_url = base_url()."SplexamNM/pg_response";
			$member_exam_id=$this->session->userdata['non_memberdata']['member_exam_id'];
...
requestparameter=IIBF|2138759|NA|500.00|NA|NA|NA|INR|NA|R|iibf|NA|NA|F|iibfexam|500081141|148201701|NA|NA|NA|NA|http://abc.somedomain.com|2387462372
		...
		if (isset($_REQUEST['msg']))
		{
			...
			$final_str = str_replace("#TRANSACTION_DATE#", "".date('Y-m-d H:i:sA',strtotime($payment_info[0]['date']))."",$newstring20);
				#-----------------------------------------E-learning msg end ----------------------------------------------------------#	
						...
						{
							redirect(base_url().'SplexamNM/acknowledge/'.base64_encode($applicationNo).'/'.base64_encode($MerchantOrderNo));
	...
	public function acknowledge($MerchantOrderNo=NULL)
	{
		...
		$this->load->view('specialexamNM/profile_thankyou',$data);
	}
	 ...
	 public function preview()
    {
			...
			{		
				...
				foreach($subject_arr as $k=>$v)
				{
					...
					{
						$this->session->set_flashdata('error',$msg);
						redirect(base_url().'SplexamNM/member/?Mtype='.$this->session->userdata['enduserinfo']['mtype'].'=&ExId='.$this->session->userdata['enduserinfo']['excd'].'');
					}
				}
			}
			if($sub_flag==0)
...
/*change  by pooj@ godse*/
		...
		// $examination_total_cnt_as_per_date=$this->master_model->getRecordCount('special_exam_apply',array('examination_date'=>$this->session->userdata['enduserinfo']['special_exam_date']));
		 ...
		 //  $examination_total_cnt_as_per_center=$this->master_model->getRecordCount('special_exam_apply',array('examination_date'=>$this->session->userdata['enduserinfo']['special_exam_date'],'center'=>$this->session->userdata['enduserinfo']['selCenterName']));
	 ...
	 $examinfo=$this->master_model->getRecords('exam_master',array('exam_code'=>$this->session->userdata['enduserinfo']['excd']),'description');
				
		/* if($examination_total_cnt_as_per_date==108)
		 {
			  $message='Registration for '.$examinfo[0]['description'].' Exam on '.date('d-M-Y',strtotime($this->session->userdata['enduserinfo']['special_exam_date'])).' has been closed!';
			 $data=array('middle_content'=>'specialexamNM/registration_for_special_exam_closed_preview','check_eligibility'=>$message);
			 $this->load->view('specialexamNM/common_view_fullwidth',$data);	
		  ...
		  else if($examination_total_cnt_as_per_center==54)
		 {
			 ...
			 $this->db->join('exam_activation_master','exam_activation_master.exam_code=center_master.exam_name AND exam_activation_master.exam_period=center_master.exam_period');
			 $centerinfo=$this->master_model->getRecords('center_master',array('exam_name'=>$this->session->userdata['enduserinfo']['excd'],'center_code'=>$this->session->userdata['enduserinfo']['selCenterName']),'center_name');
			 $message='Registration for '.$examinfo[0]['description'].' Exam on '.date('d-M-Y',strtotime($this->session->userdata['enduserinfo']['special_exam_date'])).' for '.$centerinfo[0]['center_name'].' center has been closed!';
	 ...
	 public function mobileduplication()
	{
		...
		{ 
			...
			else
			{
				...
				//$username=$user_info[0]['firstname'].' '.$user_info[0]['middlename'].' '.$user_info[0]['lastname'];
				//$userfinalstrname= preg_replace('#[\s]+#', ' ', $username);
				$str='The  mobile no already exist';
				$data_arr=array('ans'=>'exists','output'=>$str);		
				echo json_encode($data_arr);
			}
		}
	...
	public function Logout(){
		...
		//redirect(base_url().'SplexamNM/memlogin/?Extype=MQ==&Mtype=Tk0='); 
	...
	public function refund($order_no=NULL)
	{
		...
		$data=array('middle_content'=>'specialexamNM/external_special_examNM_refund','payment_info'=>$payment_info,'exam_name'=>$exam_name);
		$this->load->view('specialexamNM/common_view_fullwidth',$data);	
	}
}
