...
class ApplyexamMK909 extends CI_Controller {
	...
	{	
		...
		{
			redirect(base_url().'ApplyexamMK909/accessdenied/');
		}*/
		...
		$this->form_validation->set_rules($config);
			...
			{
				$where="(registrationtype='O' OR registrationtype='A' OR registrationtype='F' OR registrationtype='NM')";
				$this->db->where($where);
				...
				{ 
					 if($user_info[0]['isactive']==1)
					 {
						...
						redirect(base_url().'ApplyexamMK909/examdetails/?ExId='.$this->input->get('ExId').'&Extype='.$Extype);
						 }
	...
	public function accessdenied()
	{
		...
		$data=array('middle_content'=>'memapplyexam/not_eligible','check_eligibility'=>$message);
  	    $this->load->view('memapplyexam/mem_apply_exam_common_viewMK',$data);
	}
	...
	{		
			...
			/*if((base64_decode($this->input->get('ExId'))!=21 && ($this->session->userdata('mregnumber_applyexam')!='510226859' )))*/
			if($this->session->userdata('mregnumber_applyexam')!='510233554')
			{
					echo 'in123';exit;
					redirect(base_url().'ApplyexamMK909/accessdenied/');
			}
			...
			//{
				...
				if(count($check_qualify_exam) > 0)
				{
					...
					else if($flag==1 && $checkqualifyflag==0)
					{
							...
							$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=spl_eligible_master.exam_code AND spl_exam_activation_master.exam_period=spl_eligible_master.eligible_period');
							$check_eligibility_for_applied_exam=$this->master_model->getRecords('spl_eligible_master',array('spl_eligible_master.exam_code'=>$examcode,'member_type'=>$this->session->userdata('memtype'),'member_no'=>$this->session->userdata('mregnumber_applyexam')));
							if(count($check_eligibility_for_applied_exam) > 0)
							{
								...
								else if($exam_status==0)
								{
									...
									{
										$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=spl_misc_master.exam_code AND spl_exam_activation_master.exam_period=spl_misc_master.exam_period');
										$get_period_info=$this->master_model->getRecords('spl_misc_master',array('spl_misc_master.exam_code'=>base64_decode($this->input->get('ExId')),'spl_misc_master.misc_delete'=>'0'),'exam_month');
			 ...
			 if($is_transaction_doone >0)
			 {
				...
				//$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=member_exam.exam_code');
				$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=member_exam.exam_code AND spl_exam_activation_master.exam_period=member_exam.exam_period');
				$this->db->where("'$today_date' BETWEEN spl_exam_activation_master.exam_from_date AND spl_exam_activation_master.exam_to_date");
				...
				else if($flag==0 && $cookieflag==1)
				{
					if($profile_flag==0)
					{
						...
						{
							$message='<div style="color:#F00" class="col-md-4">Please update your profile!!<a href='.base_url().'nonmem> Click here </a>to login</div>';
						}
				...
				{
					$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=spl_misc_master.exam_code AND spl_exam_activation_master.exam_period=spl_misc_master.exam_period');
					$get_period_info=$this->master_model->getRecords('spl_misc_master',array('spl_misc_master.exam_code'=>$examcode,'spl_misc_master.misc_delete'=>'0'),'exam_month');
					//$month = date('Y')."-".substr($get_period_info[0]['exam_month'],4)."-".date('d');
					$month = date('Y')."-".substr($get_period_info[0]['exam_month'],4);
					$exam_period_date=date('F',strtotime($month))."-".substr($get_period_info[0]['exam_month'],0,-2);
					$message='Application for this examination is already registered by you and is valid for <strong>'.$exam_period_date.'</strong>. period. Hence you need not apply for the same.';
				...
				else if($cookieflag==1 && $profile_flag==1)
				{
					...
					$data=array('middle_content'=>'memapplyexam/cms_pageMK909','exam_info'=>$exam_info);
					$this->load->view('memapplyexam/mem_apply_exam_common_viewMK',$data);
				}
	...
	public function examdate($regnumber=NULL,$exam_code=NULL)
	{
		$flag=0;
		$today_date=date('Y-m-d');
		$applied_exam_date=$this->master_model->getRecords('spl_subject_master',array('exam_code'=>base64_decode($exam_code),'exam_date >='=>$today_date,'subject_delete'=>'0'));
		if(count($applied_exam_date) > 0)
		{
			...
			if(count($getapplied_exam_code) >0)
			{
				...
				{	
					$getapplied_exam_date=$this->master_model->getRecords('spl_subject_master',array('exam_code'=>$exist_ex_code['exam_code'],'exam_date >='=>$today_date,'subject_delete'=>'0'));
	...
	public function get_alredy_applied_examname($regnumber=NULL,$exam_code=NULL)
	{
		...
		$this->db->select('spl_subject_master.*,exam_master.description');
		$this->db->join('exam_master','exam_master.exam_code=spl_subject_master.exam_code');
		$applied_exam_date=$this->master_model->getRecords('spl_subject_master',array('spl_subject_master.exam_code'=>base64_decode($exam_code),'exam_date >='=>$today_date,'subject_delete'=>'0'));
		if(count($applied_exam_date) > 0)
		{
			$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=member_exam.exam_code AND spl_exam_activation_master.exam_period=member_exam.exam_period');
			$this->db->join('exam_master','exam_master.exam_code=member_exam.exam_code');
	...
	{
		//echo '>>>'. $this->session->userdata('mregnumber_applyexam');
		//510226859
		//echo '===>>'.$this->session->userdata('mregnumber_applyexam');
		if($this->session->userdata('mregnumber_applyexam')!='510233554')
		{
			...
			if($this->session->userdata('examcode')==$this->config->item('examCodeCaiib'))
			{
				...
				if($this->session->userdata('mregnumber_applyexam') == '510233554'){
					...
					$user_data=array('email'=>$_POST["email"],	
												...
												'optmode'=>'ON',
												'extype'=>2,
												'exname'=>'CERTIFIED TREASURY PROFESSIONALS',
												'excd'=>59,
												'eprid'=>909,
												'fee'=>$fee_tmp,
												'txtCenterCode'=>$center_tmp,
												'insdet_id'=>'',
												'selected_elect_subcode'=>'',
												'selected_elect_subname'=>'',
												'placeofwork'=>'',
												'state_place_of_work'=>$state_tmp,
												'pincode_place_of_work'=>$pincode_place_of_work_tmp,
												'elected_exam_mode'=>'',
												'grp_code'=>$grp_code_tmp,
												'subject_arr'=>'',
												'scribe_flag'=>'N'
												);
					...
					redirect(base_url().'ApplyexamMK909/Msuccess');
				}
				else
				{
		...
		if($this->session->userdata('examcode')=='')
		{
			redirect(base_url().'ApplyexamMK909/examlist/');	
		}
	
		$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=exam_master.exam_code');
		$this->db->join("spl_eligible_master",'spl_eligible_master.exam_code=spl_exam_activation_master.exam_code AND spl_exam_activation_master.exam_period=spl_eligible_master.eligible_period','left');
		$this->db->join('spl_misc_master','spl_misc_master.exam_code=exam_master.exam_code AND spl_exam_activation_master.exam_period=spl_misc_master.exam_period');
		$this->db->where("spl_misc_master.misc_delete",'0');
		$this->db->where("spl_eligible_master.member_no",$this->session->userdata('mregnumber_applyexam'));
		$this->db->where("spl_eligible_master.app_category !=",'R');
		...
		if(count($examinfo) > 0)
		{
			foreach($examinfo as $rowdata)
			{
					...
					{
						$compulsory_subjects[]=$this->master_model->getRecords('spl_subject_master',array('exam_code'=>$this->session->userdata('examcode'),'subject_delete'=>'0','exam_period'=>$rowdata['exam_period'],'subject_code'=>$rowdata['subject_code']));	
					}
				}	
		...
		$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=spl_center_master.exam_name AND spl_exam_activation_master.exam_period=spl_center_master.exam_period');
		$this->db->where('spl_center_master.exam_name',$this->session->userdata('examcode'));
		$this->db->where("center_delete",'0');
		$center=$this->master_model->getRecords('spl_center_master','','',array('center_name'=>'ASC'));
		//Below code, if member is new member
		if(count($examinfo) <=0)
		{
			$this->db->select('exam_master.*,spl_misc_master.*');
			$this->db->join('spl_misc_master','spl_misc_master.exam_code=exam_master.exam_code');
			$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=spl_misc_master.exam_code AND spl_misc_master.exam_period=spl_exam_activation_master.exam_period');//added on 5/6/2017
			$this->db->where("spl_misc_master.misc_delete",'0');
			$this->db->where('exam_master.exam_code',$this->session->userdata('examcode'));
 			$examinfo = $this->master_model->getRecords('exam_master');
			//echo $this->db->last_query();exit;
			...
			$this->db->group_by('spl_center_master.center_name');
			$center=$this->master_model->getRecords('spl_center_master');
			####### get compulsory subject list##########
			$compulsory_subjects=$this->master_model->getRecords('spl_subject_master',array('exam_code'=>$this->session->userdata('examcode'),'subject_delete'=>'0','group_code'=>'C','exam_period'=>$examinfo[0]['exam_period']),'',array('subject_code'=>'ASC'));
		}
		...
		$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=spl_medium_master.exam_code AND spl_exam_activation_master.exam_period=spl_medium_master.exam_period');
		$this->db->where('spl_medium_master.exam_code',$this->session->userdata('examcode'));
		$this->db->where('medium_delete','0');
		$medium=$this->master_model->getRecords('spl_medium_master');
		...
		//subject information
		$caiib_subjects=$this->master_model->getRecords('spl_subject_master',array('exam_code'=>$this->session->userdata('examcode'),'subject_delete'=>'0','group_code'=>'E','exam_period'=>$examinfo[0]['exam_period']));
		...
		$getfees=$this->master_model->getRecords('spl_fee_master',array('exam_code'=>$this->session->userdata('examcode'),'member_category'=>$this->session->userdata('memtype'),'exam_period'=>$examinfo[0]['exam_period'],'group_code'=>$grp_code,'exempt'=>'NE'));
		
		$data=array('middle_content'=>'memapplyexam/comApplicationMK909','states'=>$states,'undergraduate'=>$undergraduate,'graduate'=>$graduate,'postgraduate'=>$postgraduate,'institution_master'=>$institution_master,'designation'=>$designation,'user_info'=>$user_info,'idtype_master'=>$idtype_master,'examinfo'=>$examinfo,'medium'=>$medium,'center'=>$center,'caiib_subjects'=>$caiib_subjects,'compulsory_subjects'=>$compulsory_subjects,'getfees'=>$getfees);
		$this->load->view('memapplyexam/mem_apply_exam_common_viewMK',$data);
...
$user_data=array('email'=>$_POST["email"],	
	...
	/*	$check_exam_activation=check_exam_activate(base64_decode($this->session->userdata['examinfo']['excd']));
		...
		{		
			...
			foreach($subject_arr as $k=>$v)
			{
				...
				if($msg!='')
				{
					...
					redirect(base_url().'ApplyexamMK909/comApplication');
				}
			}
		}
		if($sub_flag==0)
		...
		{		
			$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=spl_medium_master.exam_code AND spl_exam_activation_master.exam_period=spl_medium_master.exam_period');
			$this->db->where('spl_medium_master.exam_code',base64_decode($this->session->userdata['examinfo']['excd']));
			$this->db->where('medium_delete','0');
			$medium=$this->master_model->getRecords('spl_medium_master');
			...
			$center=$this->master_model->getRecords('spl_center_master','','center_name');
			//echo $this->db->last_query();exit;
			$user_info=$this->master_model->getRecords('member_registration',array('regid'=>$this->session->userdata('mregid_applyexam'),'regnumber'=>$this->session->userdata('mregnumber_applyexam')));
			if(count($user_info) <=0)
			{
			...
			$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=spl_misc_master.exam_code AND spl_exam_activation_master.exam_period=spl_misc_master.exam_period');
			$misc=$this->master_model->getRecords('spl_misc_master',array('spl_misc_master.exam_code'=>base64_decode($this->session->userdata['examinfo']['excd']),'misc_delete'=>'0'));
			...
			{
					$data=array('middle_content'=>'memapplyexam/exam_previewMK','user_info'=>$user_info,'medium'=>$medium,'center'=>$center,'misc'=>$misc,'states'=>$states,'compulsory_subjects'=>$this->session->userdata['examinfo']['subject_arr']);
			}
			$this->load->view('memapplyexam/mem_apply_exam_common_viewMK',$data);
		}
		else
		{
			$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=spl_misc_master.exam_code AND spl_exam_activation_master.exam_period=spl_misc_master.exam_period');
			$get_period_info=$this->master_model->getRecords('spl_misc_master',array('spl_misc_master.exam_code'=>base64_decode($this->session->userdata['examinfo']['excd']),'spl_misc_master.misc_delete'=>'0'),'exam_month');
			//$month = date('Y')."-".substr($get_period_info[0]['exam_month'],4)."-".date('d');
			$month = date('Y')."-".substr($get_period_info[0]['exam_month'],4);
			$exam_period_date=date('F',strtotime($month))."-".substr($get_period_info[0]['exam_month'],0,-2);
			$message='Application for this examination is already registered by you and is valid for <strong>'.$exam_period_date.'</strong>. period. Hence you need not apply for the same.';
	...
	/*public function savedetails()
	{
		...
		$today_date=date('Y-m-d');
		$this->db->select('member_exam.id,member_exam.regnumber,member_exam.exam_code,member_exam.exam_mode,member_exam.exam_medium,member_exam.exam_period,member_exam.exam_center_code,member_exam.exam_fee,member_exam.note,member_exam.pay_status,exam_master.description,spl_misc_master.exam_month');
		$this->db->where('elg_mem_o','Y');	
		$this->db->join('spl_misc_master','spl_misc_master.exam_code=member_exam.exam_code AND spl_misc_master.exam_period=member_exam.exam_period');
		$this->db->where("spl_misc_master.misc_delete",'0');
		$this->db->join('exam_master','exam_master.exam_code=member_exam.exam_code');
		$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=member_exam.exam_code');
		$this->db->where("'$today_date' BETWEEN spl_exam_activation_master.exam_from_date AND spl_exam_activation_master.exam_to_date");
		...
		$medium=$this->master_model->getRecords('spl_medium_master','','medium_description');
   ...
   public function checkpin()
	...
	public function Msuccess()
	{
		...
		if(($this->session->userdata('examinfo')==''))
		{
			...
			$inser_array=array(	'regnumber'=>$this->session->userdata('mregnumber_applyexam'),
											...
											'exam_medium'=>'E',
											'exam_period'=>909,
											'exam_center_code'=>$center_tmp,
											'exam_fee'=>$fee_tmp,
											'elected_sub_code'=>0,
											'place_of_work'=>'',
											'state_place_of_work'=>$state_tmp,
											'pin_code_place_of_work'=>$pincode_place_of_work_tmp,
											'scribe_flag'=>'N',
			...
			if($inser_id=$this->master_model->insertRecord('member_exam',$inser_array,true))
			{
				...
				{
					redirect(base_url().'ApplyexamMK909/sbi_make_payment/');
				}
				else
				{
					redirect(base_url().'ApplyexamMK909/make_payment/');
				}
			}
	...
	public function make_payment()
	{
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			$pg_return_url = base_url()."ApplyexamMK909/pg_response";
...
requestparameter=IIBF|2138759|NA|500.00|NA|NA|NA|INR|NA|R|iibf|NA|NA|F|iibfexam|500081141|148201701|NA|NA|NA|NA|http://abc.somedomain.com|2387462372
	...
	{
		//$_REQUEST['msg'] = "IIBF|2138196|HYBK4895974090|39740|00000002.00|YBK|NA|01|INR|DIRECT|NA|NA|NA|15-11-2016 13:23:02|0300|NA|iibfexam|510296983|32201701|NA|NA|NA|NA|NA|Merchant transaction successfull|2915503922";
		...
		if (isset($_REQUEST['msg']))
		{
			...
			if ($res_checksum == $checksum)
			{
				if($pg_res[16] == "iibfexam")
				{
					...
					$this->db->join('spl_center_master','spl_center_master.center_code=member_exam.exam_center_code AND spl_center_master.exam_name=member_exam.exam_code');
					$this->db->join('exam_master','exam_master.exam_code=member_exam.exam_code');
					$this->db->join('spl_misc_master','spl_misc_master.exam_code=member_exam.exam_code');
					$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=member_exam.exam_code AND spl_exam_activation_master.exam_period=member_exam.exam_period');
					$exam_info=$this->master_model->getRecords('member_exam',array('regnumber'=>$this->session->userdata('mregnumber_applyexam'),'member_exam.id'=>$get_user_regnum[0]['ref_id']),'member_exam.exam_mode,member_exam.exam_medium,member_exam.exam_period,spl_center_master.center_name,member_exam.exam_center_code,exam_master.description,spl_misc_master.exam_month');
						...
						{
							redirect(base_url().'ApplyexamMK909/details/'.base64_encode($MerchantOrderNo).'/'.$this->session->userdata['examinfo']['excd']);
						}
						}
						else if($payment_status==0)
						{
							...
							if($this->Emailsending->mailsend($info_arr))
							{
								redirect(base_url().'ApplyexamMK909/fail/'.base64_encode($MerchantOrderNo));
							}
	...
	public function sbi_make_payment()
	{
		...
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			$pg_success_url = base_url()."ApplyexamMK909/sbitranssuccess";
			$pg_fail_url    = base_url()."ApplyexamMK909/sbitransfail";
		...
		//	{
				...
				//$amount=spl_getExamFee($this->session->userdata['examinfo']['txtCenterCode'],$this->session->userdata['examinfo']['eprid'],$this->session->userdata['examinfo']['excd'],$this->session->userdata['examinfo']['grp_code'],$this->session->userdata('memtype'));
			//}
			if($this->session->userdata('mregnumber_applyexam') == '510233554'){
					$fee_tmp = '7080';
					$center_tmp = '468';
			...
			$yearmonth=$this->master_model->getRecords('spl_misc_master',array('exam_code'=>base64_decode($this->session->userdata['examinfo']['excd']),'exam_period'=> $this->session->userdata['examinfo']['eprid']),'exam_month');
			...
			else if(base64_decode($this->session->userdata['examinfo']['excd'])==1600)
			{
				$exam_code=160;	
			}
			...
			{
				$exam_code=20;	
			}else if(base64_decode($this->session->userdata['examinfo']['excd'])==1770)  
			{
				$exam_code=177;	
			}
			else
			...
			$exam_code =59;
			 ...
			 ####END Code added by pooja ######
			$ref4=($exam_code).'1201912';
			...
			$insert_data = array(
				...
				'pay_type'         => '2',
				'ref_id'           => $this->session->userdata['examinfo']['insdet_id'],
				'description'      => 'CERTIFIED TREASURY PROFESSIONALS',
				'status'           => '2',
				'exam_code'    	   => 59,
			...
			$getcenter=$this->master_model->getRecords('spl_center_master',array('exam_name'=>($this->session->userdata['examinfo']['excd']),'center_code'=>$this->session->userdata['examinfo']['txtCenterCode'],'exam_period'=>$this->session->userdata['examinfo']['eprid'],'center_delete'=>'0'));
			
			$getstate=$this->master_model->getRecords('state_master',array('state_code'=>'21','state_delete'=>'0'));
			...
			if(count($getcenter) > 0)
			{
				...
				$getfees=spl_getExamFeedetails($this->session->userdata['examinfo']['txtCenterCode'],$this->session->userdata['examinfo']['eprid'],$this->session->userdata['examinfo']['excd'],$this->session->userdata['examinfo']['grp_code'],$this->session->userdata('memtype'));
				
			}
			if(count($getcenter)> 0 && $getcenter[0]['state_code']=='MAH')
			{
			...
			if(count($getstate)> 0  &&  $getstate[0]['exempt']=='E')
			{
				 $cgst_rate=$sgst_rate=$igst_rate='';	
				 $cgst_amt=$sgst_amt=$igst_amt='';	
			}	
			...
			if($this->session->userdata('mregnumber_applyexam') == '510233554'){
					...
					$grp_code_tmp = 'B1_1';
					$amount=7080;
					$center_name_tmp = 'MADURAI';
					$state_name_tmp = 'TAMILNADU';
					$base_amount = '6000';
					$state_code_tmp = '33';
				}
			...
			$invoice_insert_array=array('pay_txn_id'=>$pt_id,
													'receipt_no'=>$MerchantOrderNo,
													'exam_code'=>59,
													'center_code'=>$center_tmp,
													'center_name'=>$center_name_tmp,
													'state_of_center'=>$state_tmp,
													'member_no'=>$this->session->userdata('mregnumber_applyexam'),
													'app_type'=>'O',
													'exam_period'=>909,
													'service_code'=>$this->config->item('exam_service_code'),
													'qty'=>'1',
													'state_code'=>$state_code_tmp,
													'state_name'=>$state_name_tmp,
													'tax_type'=>$tax_type,
													'fee_amt'=>$base_amount,
													'cgst_rate'=>$cgst_rate,
													'cgst_amt'=>$cgst_amt,
			...
			$log_title = "Exam invoice data from ApplyexamMK909 cntrlr inser_id = '".$inser_id."'";
...
=1000003|DOM|IN|INR|2|Other|https://test.sbiepay.coom/secure/fail.jsp|SBIEPAY|2|2|NB|ONLINE|ONLINE
	...
	if ($q_details)
	{
		if ($q_details[2] == "SUCCESS")
		{
			...
			if($get_user_regnum[0]['status']==2)
			{
				...
				//Query to get exam details	
				$this->db->join('spl_center_master','spl_center_master.center_code=member_exam.exam_center_code AND spl_center_master.exam_name=member_exam.exam_code AND spl_center_master.exam_period=member_exam.exam_period');
				$this->db->join('exam_master','exam_master.exam_code=member_exam.exam_code');
				$this->db->join('spl_misc_master','spl_misc_master.exam_code=member_exam.exam_code AND spl_misc_master.exam_period=member_exam.exam_period');
				$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=member_exam.exam_code AND spl_exam_activation_master.exam_period=member_exam.exam_period');
				$exam_info=$this->master_model->getRecords('member_exam',array('regnumber'=>$get_user_regnum[0]['member_regnumber'],'member_exam.id'=>$get_user_regnum[0]['ref_id']),'member_exam.exam_code,member_exam.exam_mode,member_exam.exam_medium,member_exam.exam_period,spl_center_master.center_name,member_exam.exam_center_code,exam_master.description,spl_misc_master.exam_month,member_exam.state_place_of_work,member_exam.place_of_work,member_exam.pin_code_place_of_work,member_exam.examination_date,member_exam.elected_sub_code');
				//echo $this->db->last_query();exit;
				...
				/*if($exam_info[0]['exam_code']!=101)
				{
					...
					foreach($exam_admicard_details as $row)
					{
						...
						if($capacity==0)
						{
							...
							storedUserActivity($log_title, $log_message, $rId, $regNo);
							redirect(base_url().'ApplyexamMK909/refund/'.base64_encode($MerchantOrderNo));
						}
					}
				}
				if(count($exam_admicard_details) > 0)
			...
			#--chnages done by pooja --#
				...
				if($exam_info[0]['place_of_work']!='' && $exam_info[0]['state_place_of_work']!='' && $exam_info[0]['pin_code_place_of_work']!='')
				{
				   ...
				   {
					   $elective_sub_name_arr=$this->master_model->getRecords('spl_subject_master',array('subject_code'=>$exam_info[0]['elected_sub_code'],'subject_delete'=>0),'subject_description');
	...
	///End of SBICALL Back	
		...
		$user_data=array(	'regid'=>$this->session->userdata('mregid_applyexam'),
										...
										'password'=>$this->session->userdata('mpassword_applyexam'));
		$this->session->set_userdata($user_data);		
		redirect(base_url().'ApplyexamMK909/details/'.base64_encode($MerchantOrderNo).'/'.base64_encode($exam_info[0]['exam_code']));
	...
	public function sbitransfail()
	{
		if (isset($_REQUEST['encData']))
		{
			...
			$this->db->join('spl_misc_master','spl_misc_master.exam_code=member_exam.exam_code AND spl_misc_master.exam_period=member_exam.exam_period');
			$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=member_exam.exam_code AND spl_exam_activation_master.exam_period=member_exam.exam_period');
			$exam_info=$this->master_model->getRecords('member_exam',array('regnumber'=>$get_user_regnum[0]['member_regnumber'],'member_exam.id'=>$get_user_regnum[0]['ref_id']),'member_exam.exam_mode,member_exam.exam_medium,member_exam.exam_period,spl_center_master.center_name,member_exam.exam_center_code,exam_master.description,spl_misc_master.exam_month');
	...
	public function details($order_no=NULL,$excd=NULL)
	{
		...
		$this->db->join('spl_exam_activation_master','spl_exam_activation_master.exam_code=member_exam.exam_code');
		$this->db->where("'$today_date' BETWEEN spl_exam_activation_master.exam_from_date AND spl_exam_activation_master.exam_to_date");
		$applied_exam_info=$this->master_model->getRecords('member_exam',array('member_exam.exam_code'=>base64_decode($excd),'regnumber'=>$this->session->userdata('mregnumber_applyexam'),'pay_status'=>'1'));
		if(count($applied_exam_info)<=0)
		{
	...
	public function checkusers($examcode=NULL)
	{
		...
		if($examcode!=NULL)
		{
			...
			{ 
				 $this->db->where_in('spl_eligible_master.exam_code', $exam_code);
				 $valid_member_list=$this->master_model->getRecords('spl_eligible_master',array('eligible_period'=>'417','member_type'=>'O'),'member_no');
