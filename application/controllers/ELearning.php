...
class ELearning extends CI_Controller {
	...
	public function __construct()
	{
		...
		// if($system_date > '2022-12-23')
		// { 
		// 	echo '<br><br><h3><center>This page is under maintenance for upgradation activities. <br><br><center></h3>';
		// 	exit;
		// } 
	}
	...
	public function elearning_app(){
		exit;
		$var_errors = '';
		
		$exam = $this->master_model->getRecords('elearning_exam_master','','exam_code,description');
		...
		$data = array('middle_content' => 'e_learning','var_errors' => $var_errors,'image' => $captcha_img,'exam'=>$exam);
        $this->load->view('renewal_common_view', $data);
		
		
	}
	
	public function add_elearning_app(){
		exit;
			$exam_code = $this->input->post('exam_code');
			$this->db->where('exam_code',$exam_code); 
			$exam = $this->master_model->getRecords('elearning_exam_master','','description');
			
			$subject_code = $this->input->post('subject_code');
			$this->db->where('subject_code',$subject_code);
			$subject = $this->master_model->getRecords('elearning_subject_master','','subject_description');
			...
			$insert_array = array(
									...
									'exam_code'=>$this->input->post('exam_code'),
									'exam_name'=>$exam[0]['description'],
									'subject_code'=>$this->input->post('subject_code'),
									'subject_name'=>$subject[0]['subject_description'],
									'created_on '=>date('Y-m-d')
								);
			if($last_id = $this->master_model->insertRecord('tbl_sify_elearning',$insert_array,true)){
				$this->session->set_flashdata('success','Record added successfully');
				redirect(base_url().'ELearning/elearning_app');
			}
	...
	public function getsubject(){
		...
		$this->db->where('exam_code',$exam_code);
		$subject = $this->master_model->getRecords('elearning_subject_master','','subject_code,subject_description');
		$val = '<option value="" >Select</option>';
		
		foreach($subject as $res){
			$val.='<option value="'.$res['subject_code'].'" >'.$res['subject_description'].'</option>';
		}
		echo $val;
	}
	...
	public function emailduplication() {
        ...
        if ($email != "") {
			...
			$result = $this->master_model->getRecords('tbl_sify_elearning',array('email_id'=>$email)); 
            ...
            if (count($result) == 0) {
				...
				$data_arr = array('ans' => 'exists','str'=>'Enter email id already exist');
                echo json_encode($data_arr);
	...
	public function mobileduplication() {
        ...
        if ($mobile != "") {
			...
			$result = $this->master_model->getRecords('tbl_sify_elearning',array('contact_no'=>$mobile)); 
			
            if (count($result) == 0) {
                $data_arr = array('ans' => 'ok','str'=>'');
                echo json_encode($data_arr);
            } else {
				$data_arr = array('ans' => 'exists','str'=>'Enter mobile number already exist');
                echo json_encode($data_arr);
            }
        } else {
            echo 'error';
	...
	public function elearning_mail(){
		
		$exam_arr = array(20,$this->config->item('examCodeJaiib'),$this->config->item('examCodeCaiib'));
		$date =  $date =  date('Y-m-d', strtotime(' -1 day'));
		//$date =  '2020-04-24'; 
		
		$html = 'Hello Sir/Madam <br/><br/>'; 
		$html.= 'Elearning registration records of '.$date;
		$html.= '<br/><br/>';
		$html.= '<table border = 1>';
		$html.= '<tr>';
		$html.= '<td>Exam code</td>';
		$html.= '<td>Exam name</td>';
		$html.= '<td>Subject code</td>';
		$html.= '<td>Subject name</td>';
		$html.= '<td>Count</td>';
		$html.= '<td>Date</td>';
		$html.= '<td>Total count till date</td>';
		$html.= '</tr>';
		
		for($i=0; $i < count($exam_arr); $i++){
			
			$this->db->distinct('subject_code');
			$this->db->where('exam_code',$exam_arr[$i]);
			$this->db->where('created_on',$date);
			$get_subject = $this->master_model->getRecords('tbl_sify_elearning','','exam_code,subject_code,exam_name');
			
			if(count($get_subject) > 0){
			
				foreach($get_subject as $rec){
				
				$this->db->where('subject_code',$rec['subject_code']);
				$this->db->where('created_on',$date);
				$get_subject_cnt = $this->master_model->getRecordCount('tbl_sify_elearning','','exam_code,subject_code,subject_name');
				
				$this->db->where('subject_code',$rec['subject_code']);
				$this->db->where('created_on < ',date('Y-m-d'));
				$get_total_subject_cnt = $this->master_model->getRecordCount('tbl_sify_elearning','','exam_code,subject_code,subject_name');
				
				$this->db->where('subject_code',$rec['subject_code']);
				$this->db->limit(1);
				$subject_master = $this->master_model->getRecords('tbl_sify_elearning','','subject_name');
				
				
				
				$html.= '<tr>';
				$html.= '<td>'.$get_subject[0]['exam_code'].'</td>';
				$html.= '<td>'.$get_subject[0]['exam_name'].'</td>';
				$html.= '<td>'.$rec['subject_code'].'</td>';
				$html.= '<td>'.$subject_master[0]['subject_name'].'</td>';
				$html.= '<td>'.$get_subject_cnt.'</td>';
				$html.= '<td>'.$date.'</td>';
				$html.= '<td>'.$get_total_subject_cnt.'</td>';
				$html.= '</tr>';
		...
		//$arr_to = array('pawansing.pardeshi@esds.co.in'); 
		
		$arr_to = array('smuralidaran@iibf.org.in','kalpanashetty@iibf.org.in','iibfdevp@esds.co.in');
	...
	{
		/*echo '<br/><br/><h1><center>Due to technical upgradation, the online registration is temporarily unavailable. We regret the inconvenience caused.<center></h1>';
		exit;		*/
		...
		{
			redirect(base_url().'ELearning/accessdenied/');
		}
		...
		$this->form_validation->set_rules($config);
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				{ 
					 ...
					 {
						//Adding checking for qualify exam - priyanka d >> 08-jan-24
						$exam_status=$this->check_qualify_exam_eligibility(base64_decode($this->input->get('ExId')),$user_info[0]['regnumber']);
						
						if($exam_status==0) {
							redirect(base_url().'ELearning/accessdenied/');
						} 
						//end - Adding checking for qualify exam - priyanka d >> 08-jan-24
						...
						redirect(base_url().'ELearning/examdetails/?ExId='.$this->input->get('ExId').'&Extype='.$Extype);
						 }
	...
	public function check_qualify_exam_eligibility($exam_code,$regnumber) {
		$exam_status = 1;
			$checkQualifyExam = $this->master_model->getRecords('exam_master', array('exam_code' => $exam_code));
			
			if ($checkQualifyExam[0]['qualifying_exam1'] != '' && $checkQualifyExam[0]['qualifying_exam1'] != '0') {
				
				$check_qualify_exam_eligibility = $this->master_model->getRecords('eligible_master', array('eligible_master.exam_code' => $checkQualifyExam[0]['qualifying_exam1'], 'part_no' => $checkQualifyExam[0]['qualifying_part1'], 'member_no' => $regnumber), 'exam_status,remark');
				//echo $this->db->last_query();
				//echo'<pre>';print_r($check_qualify_exam_eligibility);exit;
	...
	{
		$message='<div style="color:#F00"><strong>You have not completed Climate Risk & Sustainable Finance (Foundation).</strong></div>';
...
<a href="' . base_url() . 'GstRecovery/" target="new">click here</a> </div>';
	...
	{		
			...
			if($GST_val==2)
			{
				redirect(base_url() . 'ELearning/GST');
			}
			...
			{
				redirect(base_url().'ELearning/examdetails/?ExId='.$this->input->get('ExId').'&'.'Extype='.$this->input->get('Extype'));
			}
			...
			//{
			 ...
			 if($is_transaction_doone >0)
			 {
				...
				else if($cookieflag==1 && $profile_flag==1)
				{
					...
					redirect(base_url().'ELearning/comApplication');
					//$data=array('middle_content'=>'ELearning/cms_page','exam_info'=>$exam_info);
					//$this->load->view('ELearning/mem_apply_exam_common_view',$data);
				}
	...
	public function examapplied($regnumber=NULL,$exam_code=NULL)
	{
		...
		$this->db->limit('1');
		$get_eligible_date=$this->master_model->getRecords('eligible_master',array('eligible_master.exam_code'=>base64_decode($exam_code)),'eligible_master.created_on');
	...
	public function comApplication()
	{
		...
		if(isset($_POST['btnPreviewSubmit']))  	
		{
			...
			/*  code commented on 7 jul 2021 as per client mail */
			...
			if($this->form_validation->run()==TRUE)
			{
                ...
                if (isset($_POST['scribe_flag']))
					...
					redirect(base_url().'ELearning/preview');
				}
				else
				{
		...
		$exam_status=$this->check_qualify_exam_eligibility($this->session->userdata('examcode'),$this->session->userdata('eregnumber'));
		...
		if($this->session->userdata('examcode')=='')
		{
			redirect(base_url().'ELearning/examlist/');	
		}
   ...
   public function checkpin()
	...
	public function Msuccess()
	{
		...
		if(isset($_POST['btnPreview']))
		{
			...
			if($inser_id=$this->master_model->insertRecord('member_exam',$inser_array,true))
			{
				...
				{
					redirect(base_url().'ELearning/sbi_make_payment/');
				}
				else
				{
					redirect(base_url().'ELearning/make_payment/');
				}
			}
		}
		else
	...
	public function make_payment()
	{
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			$pg_return_url = base_url()."ELearning/pg_response";
...
requestparameter=IIBF|2138759|NA|500.00|NA|NA|NA|INR|NA|R|iibf|NA|NA|F|iibfexam|500081141|148201701|NA|NA|NA|NA|http://abc.somedomain.com|2387462372
			...
			if ($res_checksum == $checksum)
			{
				if($pg_res[16] == "iibfexam")
				{
					...
					$exam_info=$this->master_model->getRecords('member_exam',array('regnumber'=>$this->session->userdata('eregnumber'),'member_exam.id'=>$get_user_regnum[0]['ref_id']),'member_exam.exam_mode,member_exam.exam_medium,member_exam.exam_period,center_master.center_name,member_exam.exam_center_code,exam_master.description,misc_master.exam_month');
						...
						if($this->Emailsending->mailsend($info_arr))
						{
							redirect(base_url().'ELearning/details/'.base64_encode($MerchantOrderNo).'/'.$this->session->userdata['examinfo']['excd']);
						}
						}
						else if($payment_status==0)
						{
							...
							if($this->Emailsending->mailsend($info_arr))
							{
								redirect(base_url().'ELearning/fail/'.base64_encode($MerchantOrderNo));
							}
	...
	public function sbi_make_payment_old_before_billdesk_july_2022()
	{
		exit();
		$this->chk_session->ELearn_Mem_checklogin_external_user();
		...
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			$pg_success_url = base_url()."ELearning/sbitranssuccess";
			$pg_fail_url    = base_url()."ELearning/sbitransfail";
...
=1000003|DOM|IN|INR|2|Other|https://test.sbiepay.coom/secure/fail.jsp|SBIEPAY|2|2|NB|ONLINE|ONLINE
	...
	public function sbi_make_payment()
	{
		...
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			// payment gateway custom fields -
			$custom_field = $MerchantOrderNo."^iibfexam^"."IIBF_EL"."^".$ref4;
			$custom_field_billdesk = $MerchantOrderNo."-iibfexam-IIBF_EL-".$ref4;
		 ...
		 //############check Gender########
			...
			if ($pg_name == 'sbi')
			{
				...
				$merchIdVal = $this->config->item('sbi_merchIdVal');
				$AggregatorId = $this->config->item('sbi_AggregatorId');
				$pg_success_url = site_url("ELearning/sbitranssuccess");
				$pg_fail_url    = site_url("ELearning/sbitransfail");
			
				/* Payment Process Code */
				$MerchantCustomerID = $regno; //$regno;
				$data["pg_form_url"] = $this->config->item('sbi_pg_form_url'); // SBI ePay form URL
				$data["merchIdVal"]  = $merchIdVal;
				$EncryptTrans = $merchIdVal."|DOM|IN|INR|".$amount."|".$custom_field."|".$pg_success_url."|".$pg_fail_url."|".$AggregatorId."|".$MerchantOrderNo."|".$MerchantCustomerID."|NB|ONLINE|ONLINE";
				$aes = new CryptAES();
			...
			elseif ($pg_name == 'billdesk') 
			{
				...
				$billdesk_res = $this->billdesk_pg_model->init_payment_request($MerchantOrderNo, $amount, $regno, $regno, '', 'ELearning/handle_billdesk_response', '', '', '', $custom_field_billdesk);
          ...
          $this->load->view('pg_billdesk/pg_billdesk_form', $data);
				...
				else
				{
					...
					redirect(base_url().'ELearning/logout');
				}
			}
		...
		if (isset($_REQUEST['transaction_response'])) 
		{
			...
			if($auth_status == '0300' && $qry_api_response['auth_status'] == '0300')
    		{
	            ...
	            if($get_user_regnum[0]['status']==2)
	            {
		            ...
		            if($this->db->affected_rows())	
		            {
						...
						$sms_emailerstr=$this->master_model->getRecords('emailer',array('emailer_name'=>'elearning_exam_acknowledgement'));
						...
						if($attachpath!='')
						{
							...
							$this->master_model->send_sms_common_all($result[0]['mobile'], $sms_final_str, $sms_emailerstr[0]['sms_template_id'], $sms_emailerstr[0]['sms_sender'], $exam_info[0]['exam_code']);
    		...
    		elseif ($auth_status == '0002') {
    			...
    			$update_data = array('transaction_no' => $transaction_no, 'status' => 2, 'transaction_details' => $responsedata['transaction_error_type']. " - " .$responsedata['transaction_error_desc'], 'auth_code' => '0002', 'bankcode' => $responsedata['bankid'], 'paymode' =>  $responsedata['txn_process_type'], 'callback'=>'B2B');
					...
					$pg_response = "encData=".$encData."&merchIdVal=".$merchIdVal."&Bank_Code=".$Bank_Code;
					$this->log_model->logtransaction("billdesk", $pg_response,$responsedata['transaction_error_type']);	
					redirect(base_url().'ELearning/pending/'.base64_encode($MerchantOrderNo));
    		}
    		else{
			...
			redirect(base_url().'ELearning/details/'.base64_encode($MerchantOrderNo).'/'.base64_encode($exam_info[0]['exam_code']));
		}
		else 
		{
			die("Please try again...");
	...
	public function pending($order_no=NULL)
	{
		...
		$data=array('middle_content'=>'ELearning/exam_applied_pending','payment_info'=>$payment_info);
		$this->load->view('ELearning/mem_apply_exam_common_view',$data);
