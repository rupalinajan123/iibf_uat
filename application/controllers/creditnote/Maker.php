...
;
	...
	{		
		...
		$this->db->join('credit_note_list cl','m.req_id=cl.req_id','LEFT');
		$this->db->group_by('m.req_id');
		$this->db->where_in('m.req_status !=',array('3','10'));	
		$this->db->order_by('m.id','DESC');
		$res_arr = $this->master_model->getRecords("maker_checker m");	
		...
		$get_transaction_no = $this->master_model->getRecords('maker_checker',array('req_status'=>1),'transaction_no');
			...
			if($payment_transaction){ 
				if($payment_transaction[0]['status'] == 1){
					...
					$member_register = $this->master_model->getRecords('member_registration',array('regnumber'=>$payment_transaction[0]['member_regnumber']),'excode,exam_period,registrationtype');
					
					if($payment_transaction[0]['pay_type'] == 2){
						// Check on member exam
						$member_exam = $this->master_model->getRecords('member_exam',array('id'=>$payment_transaction[0]['ref_id']),'pay_status,exam_period,exam_code');
						
						// NM Cancel Check and Cancel
					...
					if($payment_transaction[0]['exam_code'] != 101 && $payment_transaction[0]['exam_code'] != 1010 && $payment_transaction[0]['exam_code'] != 10100){
						if($payment_transaction[0]['pay_type'] == 2){
					...
					if($flag_invoice == 1  && $flag_payment == 1){
						...
						// Update All modules exam_invoice
						...
						// Update blended regisytration table
						if($payment_transaction[0]['pay_type'] == 10){
								...
								if($program_code!='' && $member_no!=''){
									...
									//echo $this->db->last_query();exit;
									if($blended_eligible[0]['attempt']== 1){
										$update_eligible = array('attempt' => 0);
										$this->master_model->updateRecord('blended_eligible_master',$update_eligible,array('member_no'=>$member_no , 'program_code'=>$program_code));
									}
								}
							}	
	...
	{		
		...
		$end_date = $this->input->post('to_date');//'2019-09-15';
			...
			$this->db->join('pay_type_master p','m.req_module=p.pay_type','LEFT');		
			$this->db->join('credit_note_list cl','m.req_id=cl.req_id','LEFT');
			$this->db->where('(DATE(req_created_on) BETWEEN "'.$from_date.'" AND "'.$end_date.'")');
			$this->db->group_by('m.req_id');
			$this->db->order_by('m.id','DESC');
			$res_arr = $this->master_model->getRecords("maker_checker m");	
	...
	public function report_by_date()
	{
		...
		if(strpos($value, '~') !== false)
		{
			...
			$this->db->join('pay_type_master p','m.req_module=p.pay_type','LEFT');		
			$this->db->join('credit_note_list cl','m.req_id=cl.req_id','LEFT');
			$this->db->where('(DATE(req_created_on) BETWEEN "'.$date1.'" AND "'.$date2.'")');
			$this->db->group_by('m.req_id');
			$this->db->order_by('m.id','DESC');
