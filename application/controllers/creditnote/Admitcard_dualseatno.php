...
");
	...
	public function capacity_incrase(){
		...
		foreach($venue_details as $venue_details_rec){
			...
			$this->master_model->updateRecord('venue_master_pawan',$update_data_new,array('center_code'=>$venue_details_rec['center_code'],'venue_code'=>$venue_details_rec['venue_code']));	
...
'; 
	...
	public function custom_admitcard_pdf_single(){
		$exam_code = $this->config->item('examCodeJaiib');     
		$exam_period = 219;  
		/*echo $path = genarate_admitcard_custom_new('700021488',$exam_code,$exam_period); */   
		
		$member_array = array('510177823');        
	...
	public function custome_bulk_invoice_new_design(){
		echo $attach = custome_generate_bulk_examinvoice_new_design(141); 	
	}
    ...
    public function custom_dra_acc_invoice_newdesign(){ 
		...
		//echo $path = custom_genarate_reg_invoice_new($invoice_id);
			...
			$arr = array(1614940,1622499,1625457,1625903,1625909,1641380,1643675,1664150,1665301,1667052);   
		for($i=0;$i<=9;$i++){
	...
	public function new_blended_invoice_custom(){ 
		...
		echo $path = genarate_blended_invoice_custom_new($invoice_no,$zone_code,$program_name,$mem_gstin_no);
	}
  ...
  //By swati for update transection no of DRA exam invoice 
			...
			if($eligible[0]['app_category']!=''){
				if($exam_invoice[0]['invoice_id'] == ''){
					...
					if($last_id > 0){
						...
						$invoice_image = $payment[0]['member_regnumber'].'_EX_19-20_'.$config_last_id.'.jpg';
						$update_arr = array(
	...
	public function swati_data_insert(){
