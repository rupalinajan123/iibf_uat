<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Duplicate_icard extends CI_Controller 
	{
  	...
  	{ 		
		...
		public function index()
		{
			...
			if(isset($_POST['submit']))
			{
				...
				if($from_date != "") { $this->db->where('duplicate_icard.added_date >=', $from_date." 00:00:00"); }
				if($to_date != "") { $this->db->where('duplicate_icard.added_date <=', $to_date." 23:59:59"); }
				$count = $this->master_model->getRecordCount('duplicate_icard');				
			}
			
			$data['count'] = $count;
			$data['from_date'] = $from_date;
			$data['to_date'] = $to_date;
			$this->load->view('webmanager/duplicate_icard',$data);
		}
	}		