<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Garp extends CI_Controller 
	{
  	...
  	{ 		
        ...
        $this->db->join('payment_transaction dpt', 'dpt.receipt_no = ei.receipt_no', 'INNER');
				//$this->db->where('ei.exam_period','199');
				$this->db->where('ei.exam_code','1018');
				$this->db->where('ei.invoice_no !=','');
				$this->db->where('ei.app_type','O');				
			...
			$this->load->view('webmanager/Garp',$data);
		}
	}		