...
class CmsComplaint extends CI_Controller {
	...
	{	    
		if($this->get_ip_address() == '115.124.115.71' /* || (isset($_SESSION['direct_access'])) && $_SESSION['direct_access'] == 'On' */) { }
		else 
		{
			$current_date = date('Y-m-d H:i');
			if($current_date >= '2022-01-01 21:00' && $current_date <= '2022-02-03 10:00')
			{
				// Start for Website Under Maintenance
				//echo "<h1 align='center'><img src='https://iibf.esdsconnect.com/under-maintenance.gif' alt='Website Under Maintenance'> </h1>";
				echo "<br><br><br><br><h1 align='center'>Technical Upgradation activity is in process, service will resume shortly</h1>";
				exit;
				// END for Website Under Maintenance
			}
		}
				...
				if($this->master_model->insertRecord('cms_master',$insert_data)) {
					...
					$emailerstr=$this->master_model->getRecords('emailer',array('emailer_name'=>'complaint_email'));
				 	if(count($emailerstr) > 0) {
						...
						$newstring = str_replace("*email*", $this->replace_dot_in_email($this->input->post('email')), $newstring);
						if( $country_code == 0 ) $cntrycd = '';
						else $cntrycd = $country_code; 
						$newstring = str_replace("*country_code*", $cntrycd, $newstring);
		...
		$query_cats = $this->master_model->getRecords("cms_query_category", array('is_delete'=>'0'));
		...
		$this->load->model('Captcha_model');
		 $captcha_image = $this->Captcha_model->generate_captcha_img('cmscaptcha');
		$data = array("middle_content"=>'cms_complaint', 'query_cats'=> $query_cats, 'image' => $captcha_image, 'validation_errors' => $valerrors );
		$this->load->view('common_view_fullwidth',$data);	
	}


  function replace_dot_in_email($email='')
  {
	...
	public function generatecaptchaajax()
	{
		...
		echo $data; } */
			$this->load->model('Captcha_model');
			echo $captcha_image = $this->Captcha_model->generate_captcha_img('cmscaptcha');
	...
	//get sub-categories ajax callback function
		...
		if( !empty( $catcode ) ) {
			...
			$subcats = $this->master_model->getRecords("cms_query_subcategory", array('is_delete'=>'0','category_code'=>$catcode,'subcatcode!='=>21,'subcatcode!='=>80));	
			if( count( $subcats ) > 0 ) {
			...
			if( $catcode == 'EXM' ) {
				$exams = array();
				if( empty( $subcatcode ) ) {
					$exams = $this->master_model->getRecords("cms_exams", array('is_delete'=>'0'));
				} else {
					$exams = $this->db->query("select * from cms_exams where (belong_subcat LIKE '%,".$subcatcode.",%' OR belong_subcat LIKE '".$subcatcode.",%' OR belong_subcat LIKE '%,".$subcatcode."' OR  belong_subcat = '".$subcatcode."') AND is_delete = '0'");
