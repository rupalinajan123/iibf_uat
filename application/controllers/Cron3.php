<?php
	...
	class Cron3 extends CI_Controller {
		...
		{
			$log_title ="sbicallback Function call Cron3";
			$log_message = 'sbicallback Function call Cron3';
			$rId = '';
			$regNo = '';
			$this->member_regn_log($log_title, $log_message, $rId, $regNo);
			$filehandle = fopen("cs2s_log/cron3/lock.txt", "c+");
			if (flock($filehandle, LOCK_EX | LOCK_NB)) {
				...
				AND date > NOW() - INTERVAL 80 MINUTE AND gateway = "sbiepay" AND status = 2';
				...
				if ($crnt_day_txn_qry->num_rows())
				{
					...
					$dir = 'cs2s_log/cron3/'.$todays_date;
					if(!is_dir($dir)){
						mkdir($dir, 0755);
					}
					$cell = 1;
					...
					foreach ($crnt_day_txn_qry->result_array() as $c_row)
					{
						...
						if (isset($responsedata) && count($responsedata) > 0)
						{
							...
							/*if($responsedata[12]=='iibfregn')
								...
								## Add query log
								$log_title ="Payment status update in payment transaction for member registration Cron3 C_S2S  :".$reg_id;
								...
								########get Old image Name############
								$log_title ="Ordinory member OLD Image Cron3 C_S2S :".$reg_id;
								...
								$log_title ="Ordinory member PIC update Cron3 C_S2S :".$reg_id;
								...
								$upd_files['idproofphoto'] = $proof_file;
								$this->master_model->updateRecord('member_registration',$upd_files,array('regid'=>$reg_id));
								$log_title ="Member MANUAL PICS Update Cron3 C_S2S :".$reg_id;
								$log_message = serialize($upd_files);
								$rId = $reg_id;
								$regNo = $reg_id;
								//storedUserActivity($log_title, $log_message, $rId, $regNo);	
								...
								$log_title ="Ordinory member invoice update C_S2S Cron3 Invoice number:".$invoiceNumber;
