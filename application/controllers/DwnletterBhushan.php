...
class DwnletterBhushan extends CI_Controller {
	...
	public function index(){
        ...
        $encData = $aes->decrypt('7lOBZip3jJu2u5JeRoyMzVvNHX7pxBNVCJZBupyrvc0=');
...
");
	...
	public function custom_examinvoice_send_mail(){  
		$receipt_array = array(901609028,901609291,901609297,901613709,901613716,901613717,901613719,901617160,901623197,901623202,901623284,901626108,901626322,901626323,901630997,901633743);   
		...
		$final_str.= "<br/><br/>";
		$final_str.= 'Please check your invoice receipt for CAIIB exam registration.'; 
		$final_str.= "<br/><br/>";
		$final_str.= "Regards,";
		$final_str.= "<br/>";
		$final_str.= "IIBF TEAM";
...
'; 
	...
	public function custom_admitcard_pdf(){  
		...
		$exam_period = 119;             
		//echo $path = genarate_admitcard_custom('510420494',$exam_code,$exam_period);   
		$member_array = array(510089333,
510139771,
510379278);      
...
510379278);           
	...
	public function subject_missing(){ 
		$member_array = array();
		$exarr = array($this->config->item('examCodeCaiib'),62,63,64,65,66,67,68,69,70,71,72);
		$this->db->select('mem_mem_no'); 
		$this->db->distinct('mem_mem_no');
		...
		$this->db->where('exm_prd',119);
		$this->db->where('remark','1');
		$this->db->where('created_on >= ','2019-04-01 00:00:00'); 
		$this->db->where('created_on <= ','2019-04-02 23:59:59');
		...
		foreach($admit_card as $member_no){
			...
			//$this->db->where('member_no',510137740);
	...
	public function custom_exam_invoice_new_design(){
		...
		$arr = array(1594049,1596927,1597141,1597142,1601884,1604670);   
		for($i=0;$i<=5;$i++){
	...
	public function custome_bulk_invoice_new_design(){
		echo $attach = custome_generate_bulk_examinvoice_new_design(136);	
	}

    public function custom_dra_acc_invoice_newdesign(){ 
		$invoice_id = 1625903;
		echo $path = custom_genarate_dra_acc_invoice_newdesign($invoice_id);
	}	
