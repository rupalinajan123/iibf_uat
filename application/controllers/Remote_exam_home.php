...
class Remote_exam_home extends CI_Controller {
	...
	public function examdetails(){ 
		...
		$this->db->where('mem_mem_no',$this->session->userdata('regnumber'));
	  	...
	  	else
		{
			...
			//user information
			$user_info=$this->master_model->getRecords('member_registration',array('isactive'=>'1','regnumber'=>$this->session->userdata('regnumber')));
	 ...
	 public function preview()
{
	 ...
	 foreach($subject_arr1 as $k => $v)
      {
		$flag=allowed_examdate($this->session->userdata('regnumber'),base64_decode($this->session->userdata['examinfo']['excd']),$v['date']);
		 if ($flag == 1)
			  {
					redirect(base_url().'Remote_exam_home/info');
			 }
	  }
	 }
        $sub_flag = 1;
        ...
        // echo $this->session->userdata['examinfo']['selCenterName'];exit;
            ...
            {
            redirect(base_url() . 'Remote_exam_home/accessdenied/');
            }
        ...
        if (count($subject_arr) > 0)
            ...
            foreach($subject_arr as $k => $v)
                ...
                if ($msg != '')
                    {
                    $this->session->set_flashdata('error', $msg);
                    redirect(base_url().'Remote_exam_home/examdetails/?ExId='.$this->session->userdata['examinfo']['excd'].'&Extype='.$this->session->userdata['examinfo']['extype'].'&Exprd='.$this->session->userdata['examinfo']['eprid']); 
                    }
                }
            }
        if ($sub_flag == 0)
        ...
        if (!$check)
            ...
            if ($cookieflag == 0)
                ...
                $data = array(
                    'middle_content' => 'remote_exam/exam_preview_home',
                    'user_info' => $user_info,
                    'medium' => $medium,
                    'center' => $center,
                    'misc' => $misc,
				...
				foreach($this->session->userdata['examinfo']['subject_arr'] as $k=>$v)
				{
					...
					if(count($exam_admicard_details) > 0){
						foreach($exam_admicard_details as $row){
							...
							if($capacity==0){
								$log_title =" phase-3 Remote exam home Capacity full id:".$this->session->userdata('mregnumber_elapplyexam');
								$log_message = serialize($exam_admicard_details);
								$rId = $this->session->userdata('regnumber');
								$regNo = $this->session->userdata('regnumber');
								storedUserActivity($log_title, $log_message, $rId, $regNo);
								redirect(base_url().'Remote_exam_home/exapplylogin/');
							}
						}
					}
						...
						foreach($exam_admicard_details as $row){
							...
							if($seat_number!=''){
								...
								if(count($admit_card_details) > 0){
									...
									$log_title ="phase-3 Remote exam home Seat number already allocated id:".$this->session->userdata('mregnumber_elapplyexam');
									$log_message = serialize($exam_admicard_details);
									$rId = $admit_card_details[0]['admitcard_id'];
									$regNo = $this->session->userdata('regnumber');
									storedUserActivity($log_title, $log_message, $rId, $regNo);
								}else{
									$log_title ="phase-3 Remote exam home Fail user seat allocation id:".$this->session->userdata('mregnumber_elapplyexam');
									$log_message = serialize($exam_admicard_details);
									$rId = $this->session->userdata('regnumber');
									$regNo = $this->session->userdata('regnumber');
									storedUserActivity($log_title, $log_message, $rId, $regNo);
					...
					$admitcard_pdf=remote_genarate_admitcard($this->session->userdata('regnumber'),base64_decode($this->session->userdata['examinfo']['excd']),$this->session->userdata['examinfo']['eprid']);
					...
					if($admitcard_pdf!=''){
						...
						$admitcard_image = base64_decode($this->session->userdata['examinfo']['excd']).'_'.$this->session->userdata['examinfo']['eprid'].'_'.$this->session->userdata('regnumber').'.pdf'; 
		...
		if(isset($_POST['btnPreview']))
		{
			...
			if($inser_id=$this->master_model->insertRecord('member_exam',$inser_array,true))
			{
				...
				if($this->session->userdata['examinfo']['photo']!='')
				{
					...
					$photo_name=$this->master_model->getRecords('member_registration',array('regid'=>$this->session->userdata('mregid_applyexam'),'regnumber'=>$this->session->userdata('regnumber')),'scannedphoto');
				...
				if($this->session->userdata['examinfo']['signname']!='')
				{
					...
					$sing_name=$this->master_model->getRecords('member_registration',array('regid'=>$this->session->userdata('mregid_applyexam'),'regnumber'=>$this->session->userdata('regnumber')),'scannedsignaturephoto');
				...
				if(count($update_array) > 0)
				{
					...
					$prevData = array();
					$user_info=$this->master_model->getRecords('member_registration',array('regid'=>$this->session->userdata('mregid_applyexam'),'regnumber'=>$this->session->userdata('regnumber'),'isactive'=>'1'));
					if(count($user_info))
					{
					...
					$this->master_model->updateRecord('member_registration',$update_array,array('regid'=>$this->session->userdata('mregid_applyexam'),'regnumber'=>$this->session->userdata('regnumber')));
					
					log_profile_user($log_title = "phase-3 Profile updated successfully", $edited,'data',$this->session->userdata('mregid_applyexam'),$this->session->userdata('regnumber'));
				...
				{
					redirect(base_url().'Remote_exam_home/sbi_make_payment/');
				}
				else
				{
					redirect(base_url().'Remote_exam_home/make_payment/');
				}
			}
		}
		else
	...
	public function sbi_make_payment()
	{
		...
		if(isset($_POST['processPayment']) && $_POST['processPayment'])
		{
			...
			$pg_success_url = base_url()."Remote_exam_home/sbitranssuccess";
			$pg_fail_url    = base_url()."Remote_exam_home/sbitransfail";
			...
			$log_title = "phase-3 Remote exam home invoice data from applyexam cntrlr inser_id = '".$inser_id."'";
		 ...
		 //############check Gender########
			...
			if(!empty($this->session->userdata['examinfo']['subject_arr']))
			{
					foreach($this->session->userdata['examinfo']['subject_arr'] as $k=>$v)
					{
						...
						$log_title ="phase-3 Remote exam home  Admit card data from Applyexam cntrlr";
					...
					if(count($check_eligibility_for_applied_exam) <= 0 || $check_eligibility_for_applied_exam[0]['app_category']=='R')
					{
						...
						{
								$log_title = "phase-3 Remote exam home Fresh Member subject missing applyexam cntrlr";
					...
					else
					{
							...
							{
								$log_title = "phase-3 Remote exam home Existing Member subject missing  applyexam cntrlr";
								$log_message = serialize($this->session->userdata['examinfo']['subject_arr']);
								$rId = $this->session->userdata('regnumber');
								$regNo = $this->session->userdata('regnumber');
								storedUserActivity($log_title, $log_message, $rId, $regNo);
...
=1000003|DOM|IN|INR|2|Other|https://test.sbiepay.coom/secure/fail.jsp|SBIEPAY|2|2|NB|ONLINE|ONLINE
	...
	if ($q_details)
	{
		if ($q_details[2] == "SUCCESS")
		{
			...
			if($get_user_regnum[0]['status']==2)
			{
				...
				$log_title = "phase-3 Remote exam home update payment table after success  = '".$MerchantOrderNo."'";
				...
				if(count($exam_admicard_details) > 0){
					...
					foreach($exam_admicard_details as $row){
						...
						if($seat_number!=''){
							...
							$log_title ="phase-3 Remote exam home Seat number allocated id:".$this->session->userdata('mregnumber_elapplyexam');
							$log_message = serialize($exam_admicard_details);
							$rId = $admit_card_details[0]['admitcard_id'];
							$regNo = $this->session->userdata('mregnumber_elapplyexam');
							storedUserActivity($log_title, $log_message, $rId, $regNo);
							...
							if(count($admit_card_details) > 0){
								...
								$log_title ="phase-3  Remote exam home home Fail user seat allocation id:".$this->session->userdata('mregnumber_elapplyexam');
								$log_message = serialize($exam_admicard_details);
								$rId = $this->session->userdata('mregnumber_elapplyexam');
								$regNo = $this->session->userdata('mregnumber_elapplyexam');
								storedUserActivity($log_title, $log_message, $rId, $regNo);
				...
				$log_title = "phase-3 Remote exam home update member exam pay status  = '".$get_user_regnum[0]['ref_id']."'";
				...
				if(count($getinvoice_number) > 0)
				{
					...
					if($invoiceNumber)
					{
						...
						$log_title = "phase-3 Remote exam home invoice number generation done  = '".$invoiceNumber."'";
				...
				{	
					...
					$log_title = "phase-3 Remote exam home invoice image generation done  = '".$invoiceNumber."'";
				...
				{
					$log_title ="phase-3 Remote exam home B2B Update fail:".$get_user_regnum[0]['member_regnumber'];
					$log_message = serialize($update_data);
					$rId = $MerchantOrderNo;
	...
	///End of SBICALL Back	
		...
		$this->load->view('common_view',$data_arr);
	...
	public function sbitransfail()
	{
		if (isset($_REQUEST['encData']))
		{
			...
			//End Of SBICALL Back	
			redirect(base_url().'Remote_exam_home/fail/'.base64_encode($MerchantOrderNo));
		}
		else
		{
...
<a href="' . base_url() . 'GstRecovery/" target="new">click here</a> </div>';
		$data=array('middle_content'=>'member_notification','check_eligibility'=>$message);
  	    $this->load->view('common_view',$data);
	}
