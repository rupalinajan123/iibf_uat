<?php
	...
	class Cron1_billdesk extends CI_Controller 
	{
        ...
        //START : PAYMENT TRANSACTION QUERY
				$query='SELECT receipt_no, pg_flag FROM payment_transaction WHERE date <= NOW() - INTERVAL 20 MINUTE AND date > NOW() - INTERVAL 40 MINUTE AND gateway = "billdesk" AND status = 2';
        ...
        $query_bcbf = 'SELECT receipt_no, pg_flag FROM iibfbcbf_payment_transaction WHERE date <= NOW() - INTERVAL 20 MINUTE AND date > NOW() - INTERVAL 40 MINUTE AND gateway = "2" AND status = "2" AND payment_mode = "Individual"';
        //$query_bcbf = 'SELECT receipt_no, pg_flag FROM iibfbcbf_payment_transaction WHERE ((date <= NOW() - INTERVAL 20 MINUTE AND date > NOW() - INTERVAL 40 MINUTE) OR receipt_no IN (904607924)) AND gateway = "2" AND status = "2" AND payment_mode = "Individual"';
        ...
        $query_amp = 'SELECT receipt_no, pg_flag FROM amp_payment_transaction WHERE date <= NOW() - INTERVAL 20 MINUTE AND date > NOW() - INTERVAL 40 MINUTE AND gateway = "billdesk" AND status = "2"';
        ...
        $query_dra = 'SELECT id,receipt_no, pg_flag FROM dra_payment_transaction WHERE date <= NOW() - INTERVAL 20 MINUTE AND date > NOW() - INTERVAL 40 MINUTE AND gateway = "2" AND (status = "5" OR status = "3")';
        // echo $query_dra; exit;
        //$query_bcbf = 'SELECT receipt_no, pg_flag FROM iibfbcbf_payment_transaction WHERE ((date <= NOW() - INTERVAL 20 MINUTE AND date > NOW() - INTERVAL 40 MINUTE) OR receipt_no IN (904607924)) AND gateway = "2" AND status = "2" AND payment_mode = "Individual"';
        ...
        //echo $this->db->last_query();exit;
              ...
              if($pg_flag == 'BC')
              {
                ...
                $iibfbcbf_payment_data = $this->master_model->getRecords('iibfbcbf_payment_transaction',array('receipt_no'=>$receipt_no));
              ...
              else
              {
                  ...
                  else if($pg_flag == 'iibfbulkdra')
                  {
                  	...
                  	$update_data = array(
					          	...
					          	'date'			          => $updated_date,
					          	'auth_code'           => $responsedata['auth_status'],
					          	'callback'            => 'C_S2S',
				            ...
				            $this->db->group_end();
										...
										if ($this->db->affected_rows())
									  {
									  	log_dra_admin($log_title = "Transaction status is updated when Online Payment is failed :  C_S2S", $log_message = serialize($update_data));
	                    $iibfdra_payment_data = $this->master_model->getRecords('dra_payment_transaction',array('receipt_no'=>$receipt_no));
	                    if(count($iibfdra_payment_data) > 0)
	                    {
