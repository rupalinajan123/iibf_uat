<?php
	...
	class Cron3_billdesk extends CI_Controller 
	{
		...
		public function billdesk_callback()
		{
			...
			$log_title ="billdesk_callback Function call Cron3";
			$log_message = 'billdesk_callback Function call Cron3';
			$rId = $regNo = '';
			$this->member_regn_log($log_title, $log_message, $rId, $regNo);
			
			$filehandle = fopen("cs2s_log/billdesk_cron3/lock.txt", "c+");
        ...
        //START : PAYMENT TRANSACTION QUERY
				$query='SELECT receipt_no, pg_flag FROM payment_transaction WHERE date <= NOW() - INTERVAL 20 MINUTE AND date > NOW() - INTERVAL 80 MINUTE AND gateway = "billdesk" AND status = 2';
        ...
        $query_bcbf = 'SELECT receipt_no, pg_flag FROM iibfbcbf_payment_transaction WHERE date <= NOW() - INTERVAL 20 MINUTE AND date > NOW() - INTERVAL 80 MINUTE AND gateway = "Billdesk" AND status = "2" AND payment_mode = "Individual"';
        ...
        //echo '<br>'.$this->db->last_query();
				...
				AND date > NOW() - INTERVAL 80 MINUTE AND gateway = "billdesk" AND status = 2';

				// SELECT receipt_no,date FROM payment_transaction WHERE date <= NOW() - INTERVAL 60 MINUTE AND date > NOW() - INTERVAL 960 MINUTE AND gateway = "billdesk" AND status = 2 
				$crnt_day_txn_qry = $this->db->query($query);
				...
				if ($crnt_day_txn_qry->num_rows())
				{
					...
					$dir = 'cs2s_log/billdesk_cron3/'.$todays_date;
					...
					foreach ($crnt_day_txn_qry->result_array() as $c_row)
					{
						...
						{
							$refundInitiated=1;
							$update_data = array( 'status' => 3, 'transaction_details' =>  " Refunded from billdesk- ", 
							 'callback' => 'c_S2S');
						...
						$data_count = $this->master_model->getRecordCount('payment_transaction',array('receipt_no'=>$receipt_no,'status'=>1));
							...
							'txn_data' 		=> $encData.'&CALLBACK=C_S2S3',
							...
							{
								$this->billdesk_pg_module_cron->billdesk_cron_settlement_common($responsedata, $encData);
							}
							else
							{
