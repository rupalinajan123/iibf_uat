...
class CSCNonMemberDemo extends CI_Controller {
	...
	{ 
		...
		{*/
		/*if($this->router->fetch_method()!='comApplication' && $this->router->fetch_method()!='preview' && $this->router->fetch_method()!='Msuccess' && $this->router->fetch_method()!='editmobile' && $this->router->fetch_method()!='editemailduplication' && $this->router->fetch_method()!='setExamSession' && $this->router->fetch_method()!='details' && $this->router->fetch_method()!='applydetails' && $this->router->fetch_method()!='saveexam' && $this->router->fetch_method()!='exampdf' && $this->router->fetch_method()!='printexamdetails' && $this->router->fetch_method()!='sbi_make_payment' && $this->router->fetch_method()!='sbitranssuccess' && $this->router->fetch_method()!='sbitransfail'  && $this->router->fetch_method()!='accessdenied' && $this->router->fetch_method()!='check_emailduplication' && $this->router->fetch_method()!='check_mobileduplication' && $this->router->fetch_method()!='check_checkpin' && $this->router->fetch_method()!='refund'&& $this->router->fetch_method()!='memlogin')
		{
			if($this->session->userdata('cscexaminfo'))
			{ echo '1';exit;
				$this->session->unset_userdata('cscexaminfo');
			}
			if($this->session->userdata('cscexamcode'))
			{echo '2';exit;
				$this->session->unset_userdata('cscexamcode');
			}
		}*/
	...
	public function memlogin(){
		...
		if($check_exam_activation==0 || !in_array(base64_decode($this->input->get('ExId')),$exam_code_array)){
			echo '1';exit;
			redirect(base_url().'CSCNonMemberDemo/accessdenied/');
		}
			...
			if($this->form_validation->run() == TRUE){
				...
				if(count($user_info)){ 
					if($user_info[0]['isactive']==1){
						...
						redirect(base_url().'CSCNonMemberDemo/showexam/?ExId='.$this->input->get('ExId').'&Extype='.$Extype);
	...
	public function showexam(){
		   ...
		   if($check_qualify_exam[0]['qualifying_exam1']!='' && $check_qualify_exam[0]['qualifying_exam1']!='0'){
			...
			if($checkqualifyflag==0){
				...
				if($check){
					...
					$message='You have already applied for the exam, please wait till the result is declared1. Hence you need not apply for the same.';
					$flag=0;
		...
		if($flag==1 && $checkqualifyflag==0){
			redirect(base_url().'CSCNonMemberDemo/examdetails/?excode2='.$examcode.'&Extype='.$Extype); 
		}else{
  ...
  // ##---------End user profile (prafull)-----------##
   ...
   public function acknowledge()
   {
					...
					if($flag==1)
					{
						...
						/* Close User Log Actitives */
							...
							if(!is_file(get_img_name($this->session->userdata('cscnmregnumber'),'pr')) || !is_file(get_img_name($this->session->userdata('cscnmregnumber'),'s')) || !is_file(get_img_name($this->session->userdata('cscnmregnumber'),'p')) || validate_edit_nonmemdata($this->session->userdata('cscnmregnumber')))
							{
								...
								redirect(base_url().'CSCNonMemberDemo/profile/');
							}
		...
		if(isset($_POST['btn_password']))
		{
			...
			if($this->form_validation->run())
			{
				...
				if($row==0)
				{
					...
					redirect(base_url().'CSCNonMemberDemo/changepass/');
				}
				else
				{
	...
	public function examlist()
	{
			...
			{
				redirect(base_url().'CSCNonMemberDemo/notification');
			}
			
		 $today_date=date('Y-m-d');
		 $flag=1;
		 $examcodes=array('526','527');
		 $exam_list=array();
	...
	{		$this->chk_session->chk_cscnon_member_session();
			...
			{
				redirect(base_url().'CSCNonMemberDemo/examdetails/?excode2='.$this->input->get('excode2').'&'.'Extype='.$this->input->get('Extype'));
			}
			...
			//End Of ask user to wait for 5 min, until the payment transaction process complete by (PRAFULL)
						...
						$this->db->join('exam_activation_master','exam_activation_master.exam_code=eligible_master.exam_code AND exam_activation_master.exam_period=eligible_master.eligible_period');
							...
							if(count($check_eligibility_for_applied_exam) > 0)
							{
								...
								else if($exam_status==0)
								{
									...
									else
									{
										...
										$message='You have already applied for the exam, please wait till the result is declared2. Hence you need not apply for the same.';
							...
							else
							{
								...
								else
								{
										...
										$message='You have already applied for the exam, please wait till the result is declared3. Hence you need not apply for the same.';
			 ...
			 if($is_transaction_doone >0)
			 {
				...
				else if($eligiblecnt)
				{
						...
						$message='You have already applied for the exam, please wait till the result is declared4. Hence you need not apply for the same.';
	...
	public function checkqualify($qualify_id=NULL,$examcode=NULL,$part_no=NULL)
	{
		...
		{
			redirect(base_url().'CSCNonMemberDemo/dashboard');
		}
	...
	public function comApplication ()
	{
		...
		if(isset($_POST['btnPreviewSubmit']))  	
		{
			...
			$this->form_validation->set_rules('scannedsignaturephoto','Scanned Signature Specimen','file_required|file_allowed_type[jpg,jpeg]|file_size_max[20]|callback_scannedsignaturephoto_upload');
				...
				if($this->form_validation->run()==TRUE)
				{
						...
						{	
							...
							foreach($subject_arr as $k=>$v)
							{
								...
								if($msg!='')
								{
									...
									redirect(base_url().'CSCNonMemberDemo/comApplication');
								}
							}
						}
						if($sub_flag==0)
					...
					storedUserActivity($log_title, $log_message, $rId, $regNo);
					/* Close User Log Actitives */
					redirect(base_url().'CSCNonMemberDemo/preview');
				}
				else
				{
		...
		{
			redirect(base_url().'CSCNonMemberDemo/examlist/');	
		}
		//check exam acivation
	...
	public function preview()
	{
		...
		else
		{
			 ...
			 $message='You have already applied for the exam, please wait till the result is declared5. Hence you need not apply for the same.';
	...
	public function examapplied($cscnmregnumber=NULL,$exam_code=NULL)
	{
		...
		/*echo count($applied_exam_info);
		echo '<br/>';
		echo count($get_eligible_date);
		exit;*/
						...
						foreach($getapplied_exam_date as $exist_ex_date)
						{
							foreach($applied_exam_date as $sel_ex_date)
							{
									if($sel_ex_date['exam_date']==$exist_ex_date['exam_date'])
									{
										...
										$msg='You have already applied for the exam, please wait till the result is declared6. period. Hence you need not apply for the same.';
								...
								if($flag==1)
								{
										...
										$msg='You have already applied for the exam, please wait till the result is declared7. Hence you need not apply for the same.';
		...
		if(isset($_POST['btnPreview']))
		{
			...
			if($insert_id=$this->master_model->insertRecord('member_exam',$inser_array,true))
			{
				...
				/* Close User Log Actitives */
				 ...
				 //redirect(base_url().'CSCNonMemberDemo/connect');
	...
	public function csc_transsuccess()
	{
		...
		if ($params['txn_status'] == '100')
		{
					...
					{		
						...
						foreach($exam_admicard_details as $row)
						{
							...
							if($capacity==0)
							{
								...
								storedUserActivity($log_title, $log_message, $rId, $regNo);
								redirect(base_url().'CSCNonMemberDemo/refund/'.base64_encode($MerchantOrderNo));
							}
						}
					}
			...
			redirect(base_url().'CSCNonMemberDemo/acknowledge/'.base64_encode($MerchantOrderNo));
			}
		}else{
		...
		$this->db->join('exam_activation_master','exam_activation_master.exam_code=member_exam.exam_code AND exam_activation_master.exam_period=member_exam.exam_period');
		$exam_info=$this->master_model->getRecords('member_exam',array('regnumber'=>$get_user_regnum[0]['member_regnumber'],'member_exam.id'=>$get_user_regnum[0]['ref_id']),'member_exam.exam_code,member_exam.exam_mode,member_exam.exam_medium,member_exam.exam_period,center_master.center_name,member_exam.exam_center_code,exam_master.description,misc_master.exam_month');
		redirect(base_url().'CSCNonMemberDemo/details/'.base64_encode($MerchantOrderNo).'/'.base64_encode($exam_info[0]['exam_code']));
	}
	...
	{	
		...
		$customer_id=$params['merchant_id']; 
			...
			//END OF SBI CALLBACK
			//Main Code
			redirect(base_url().'CSCNonMemberDemo/fail/'.base64_encode($MerchantOrderNo));
