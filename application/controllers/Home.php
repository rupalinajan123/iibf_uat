...
class Home extends CI_Controller
{
    ...
    public function profile()
    {
        ...
        $member_info     = $this->master_model->getRecords('member_registration', array(
            'regid' => $this->session->userdata('regid'),
        ), 'scannedphoto,scannedsignaturephoto,idproofphoto,declaration,registrationtype');
        ...
        if (isset($_POST['btnSubmit'])) {
            ...
            // Conditional check added for email and mobile OTP verification on 2024-12-02 //
            if (isset($_POST['email_verify_status']) && $_POST['email_verify_status'] == 'yes') {
                ...
                $this->form_validation->set_rules('email', 'Email', 'trim|callback_check_email_mobile_otp_verification[email]|required|valid_email|is_validuniqueO[member_registration.email.regid.' . $this->session->userdata('regid') . '.isactive.1.registrationtype.' . $this->session->userdata('memtype') . ']|xss_clean');
            }

            if (isset($_POST['mobile_verify_status']) && $_POST['mobile_verify_status'] == 'yes') {
                        ...
                        if ($this->master_model->updateRecord('member_registration', $update_data_image, array(
                            ...
                            if ($kyc_edit_flag == 1) {
                                ...
                                if (count($kycmemdetails) > 0) {
                                    ...
                                    if ($kycmemdetails[0]['kyc_status'] == '1' || $kycmemdetails[0]['kyc_state'] == '3') { //priyanka d >> 06-jan-25 >> just adding one more record in mem_kyc so it will go to recommender
                                        $kyc_insert_data = array(
                                            'regnumber' => $this->session->userdata('regnumber'),
                                            'mem_type' => $member_info[0]['registrationtype'],
                                            'mem_name' => '1',
                                            'mem_dob' => '1',
                                            'mem_associate_inst' => '1',
                                            'mem_photo' => '0',
                                            'mem_sign' => '0',
                                            'mem_proof' => '0',
                                            'employee_proof' => '0',
                                            'mem_declaration' => '0',
                                            'mem_bank_bc_id_card' => '0',
                                            'field_count' => 3,
                                            'old_data' => serialize($member_info[0]),
                                            'kyc_status' => '0',
                                            'kyc_state' => '2',
                                            'recommended_by' => '0',
                                            'user_type' => 'recommender',
                                            'recommended_date' => '0000-00-00 00:00:00',
                                            'user_edited_date' => date('Y-m-d H:i:s'),
                                            'record_source' => 'Edit'
                                        );
                                        $last_insterid = $this->master_model->insertRecord('member_kyc', $kyc_insert_data, true);
                                    }
    ...
    public function editimages()
    {
        ...
        $kyc_edit_flag = 0;
        $flag = 1;
        $member_info = $this->master_model->getRecords('member_registration', array('regid' => $this->session->userdata('regid')), 'scannedphoto,scannedsignaturephoto,idproofphoto,declaration, namesub, firstname, middlename, lastname,registrationtype');
        ...
        if (isset($_POST) && count($_POST) > 0) {
            ...
            if (isset($_POST['scannedphoto_cropper']) && $_POST['scannedphoto_cropper'] != "") {
                $scannedphoto_req_flg = '';
            } else if ($member_info[0]['scannedphoto'] != "") {
                $scannedphoto_req_flg = '';
            } else if (isset($_POST['scannedphoto1_hidd']) && $_POST['scannedphoto1_hidd'] != "") {
                $scannedphoto_req_flg = '';
            }

            if (isset($_POST['scannedsignaturephoto_cropper']) && $_POST['scannedsignaturephoto_cropper'] != "") {
                $scannedsignaturephoto_req_flg = '';
            } else if ($member_info[0]['scannedsignaturephoto'] != "") {
                $scannedsignaturephoto_req_flg = '';
            } else if (isset($_POST['scannedsignaturephoto1_hidd']) && $_POST['scannedsignaturephoto1_hidd'] != "") {
                $scannedsignaturephoto_req_flg = '';
            }

            if (isset($_POST['idproofphoto_cropper']) && $_POST['idproofphoto_cropper'] != "") {
                $idproofphoto_req_flg = '';
            } else if ($member_info[0]['idproofphoto'] != "") {
                $idproofphoto_req_flg = '';
            } else if (isset($_POST['idproofphoto1_hidd']) && $_POST['idproofphoto1_hidd'] != "") {
                $idproofphoto_req_flg = '';
            }
            ...
            if (isset($_POST['declaration_cropper']) && $_POST['declaration_cropper'] != "") {
                ...
                $declaration_req_flg = '';
            } else if (isset($_POST['declaration_hidd']) && $_POST['declaration_hidd'] != "") {
                $declaration_req_flg = '';
            }
                ...
                if ($flag == 1) {
                    ...
                    if (count($update_info) > 0) {
                        ...
                        if (isset($ex_mem_info) && count($ex_mem_info) > 0) {
                            //echo "<pre>";print_r($ex_mem_info);die;
                            ...
                            if ($kyc_edit_flag == 1) {
                                ...
                                if (count($kycmemdetails) > 0) {
                                    ...
                                    if ($kycmemdetails[0]['kyc_status'] == '1' || $kycmemdetails[0]['kyc_state'] == '3') { //priyanka d >> 06-jan-25 >> just adding one more record in mem_kyc so it will go to recommender
                                        ...
                                        $last_insterid = $this->master_model->insertRecord('member_kyc', $kyc_insert_data, true);
                                        // echo $this->db->last_query();
                                        //echo'<pre>=';print_r($kycmemdetails);exit;
                                    }
                                    // check membership count
    ...
    public function benchmark_disability_check()
    {
        ...
        $user_images = $this->master_model->getRecords('member_registration', array(
            ...
            'regnumber' => $this->session->userdata('regnumber'),
            'isactive'  => '1',
        ), 'scannedphoto,scannedsignaturephoto,idproofphoto,declaration,mobile,email,benchmark_disability,vis_imp_cert_img,orth_han_cert_img,cer_palsy_cert_img');
...

    {
        ...
        } //else if added by pooja mane to force to update disablity if flag is Y - 2024-09-30
        elseif ($user_images[0]['benchmark_disability'] == 'Y' && $user_images[0]['vis_imp_cert_img'] == '' && $user_images[0]['orth_han_cert_img'] == '' && $user_images[0]['cer_palsy_cert_img'] == '') {
            $this->session->set_flashdata('error', "Kindly Update the, Benchmark Disability and then apply for exam. For any queries contact zonal office.");
            redirect(base_url() . 'Home/profile');
        } //added by pooja mane to force to update disablity - 2024-09-30
    ...
    public function examlist()
    {
        ...
        // if($this->get_client_ip1()=='115.124.115.72') {
        //     echo 'Under maintenance';exit;
        // }
        // accedd denied due to GST
        $GST_val = check_GST($this->session->userdata('regnumber'));
        ...
        $user_images = $this->master_model->getRecords('member_registration', array(
            ...
            'regnumber' => $this->session->userdata('regnumber'),
            'isactive'  => '1',
        ), 'scannedphoto,scannedsignaturephoto,idproofphoto,declaration,mobile,email,benchmark_disability,vis_imp_cert_img,orth_han_cert_img,cer_palsy_cert_img,associatedinstitute');
        /* if(!file_exists('./uploads/photograph/'.$user_images[0]['scannedphoto']) || !file_exists('./uploads/scansignature/'.$user_images[0]['scannedsignaturephoto']) || !file_exists('./uploads/idproof/'.$user_images[0]['idproofphoto'])
        ||$user_images[0]['scannedphoto']=='' ||$user_images[0]['scannedsignaturephoto']=='' || $user_images[0]['idproofphoto']==''
        ||$user_images[0]['mobile']=='' ||$user_images[0]['email']=='')
        {*/
        ...
        if (in_array($this->session->userdata('regnumber'), $this->jaiib_reschedule_arr)) {
            ...
            /*if (in_array($this->session->userdata('regnumber'), $this->rpe_reschedule_arr)){
                $examcodes = array('528', '529', '530', '531', '534', '991', '997', '1052', '1054', '1053');
            }else{
                $examcodes = array('528', '529', '530', '531', '534', '991', '997', '1052', '1054', '1053', '1002', '1003', '1004', '1005', '1009', '1013', '1014');
            }*/
        ...
        if ( $this->session->userdata('special_login') == 'Yes' ) {
            $data = array(
                'middle_content' => 'member_special_exam_list',
                'exam_list'      => $exam_list,
    ...
    public function examdetails()
    {
        ...
        if ($examcode == 19  || $examcode == 156 || $examcode == 153) {
            $check_eligible_fresh_old_member = $this->master_model->getRecords('eligible_master', array('
                exam_code' => $examcode, 'member_no' => $this->session->userdata('regnumber')));
        ...
        $excd = base64_decode($this->input->get('excode2'));
        if ($excd == 210 || $excd == 420) {
        ...
        if (in_array(base64_decode($this->input->get('excode2')), $dipcert_exam_codes_chk) && !in_array($get_ip_address, $dipcert_set_client_ip_address)) {
            // echo "<br>".$get_ip_address;
            // echo "Site Under Maintenance. Please try again later."; exit;
        }
        ...
        if ($check_valid_exam_flag == 1) {
            if (in_array($decode_exam_code, array(1031, 1032))) {
                ...
                $this->session->set_flashdata('error_invalide_exam_selection', "This certificate course is applicable for Karur Vysya Bank staff only. In case you have changed your organisation to Karur Vysya Bank, kindly update the bank name in your membership profile.");
            }
    ...
    public function comApplication()
    {
       // IP whitelist condition
        ...
        if ($this->get_client_ip1() != '152.59.57.254'    &&  $this->get_client_ip1() != '115.124.115.72'    && $this->get_client_ip1() != '115.124.115.75'    && $this->get_client_ip1() != '182.73.101.70' && $this->get_client_ip1() != '192.168.11.253') {

            $excodes = array(505, 506, 507, 508, 509, 600);

            if (in_array($this->session->userdata('examcode'), $excodes)) {
                // echo'CAIIB exam will be live soon';exit; 
            }
        }
        ...
        if (in_array($this->session->userdata('regnumber'), $restrictNumbers) && in_array($this->session->userdata('examcode'), $restrictNumbersexcodes)) {
            echo 'You are debarred for this exam';
            exit;
        }
        if ($this->session->userdata('examcode') == 220) {
            if ($this->session->userdata('regnumber') == 510179030 || $this->session->userdata('regnumber') == 510453825 || $this->session->userdata('regnumber') == 510152055) {
            } else {
        ...
        if (isset($_POST['btnPreviewSubmit'])) {
            ...
            $oldmemberdetails =  $this->master_model->getRecords('member_registration', array(

                'regnumber' => $this->session->userdata('regnumber'),
            ));

            $log_title   = "Old member details - 1";
            $log_message = serialize($oldmemberdetails);
            ...
            if ($this->form_validation->run() == true) {
                ...
                if (count($venue) > 0 && count($date) > 0 && count($time) > 0) {
                    ...
                    if (count($subject_arr) > 0) {
                        ...
                        foreach ($subject_arr as $k => $v) {
                            ...
                            $capacity = get_capacity($v['venue'], $v['date'], $v['session_time'], $_POST['selCenterName'], base64_decode($_POST['excd']));  //priyanka d >>27-dec-24 >> by default selecting venue for jaiib/caiiib as we don't have to create admitcard from filled form now >> exam_cd
                            if ($capacity <= 1) {
                ...
                if (($this->session->userdata('examcode') == 101 || $this->session->userdata('examcode') == 1046 || $this->session->userdata('examcode') == 1047)) {
                    ...
                    if (isset($_FILES['empidproofphoto']['name']) && ($_FILES['empidproofphoto']['name'] != '')) 
                    {
                        ...
                        $img = "empidproofphoto";
                        $tmp_inputempidproof = strtotime($new_date) . rand(0, 100);
                    ...
                    if (isset($_POST['bank_bc_id_card_cropper']) && $_POST['bank_bc_id_card_cropper'] != "") {
                        ...
                        if ($this->session->userdata('regnumber') != "") {
                            $bank_bc_id_card_new_file_name = "bank_bc_id_card_" . $this->session->userdata('regnumber') . '.' . strtolower(pathinfo($bank_bc_id_card_cropper, PATHINFO_EXTENSION));
                        } else {
    ...
    public function preview()
    {
        ...
        if (count($subject_arr) > 0) {
            ...
            foreach ($subject_arr as $k => $v) {
                ...
                $capacity = get_capacity($v['venue'], $v['date'], $v['session_time'], $this->session->userdata['examinfo']['selCenterName'], $ExamCode);  //priyanka d >>27-dec-24 >> by default selecting venue for jaiib/caiiib as we don't have to create admitcard from filled form now >> exam_cd
                if ($capacity <= 1) {
    ...
    public function Msuccess()
    {
        ...
        if (in_array($exCode, $examCd_Arr)) {
            ...
            $duplicate_applied_exam_chk = $this->master_model->getRecords('member_exam', array('member_exam.exam_period' => $this->session->userdata['examinfo']['eprid'], 'member_exam.regnumber' => $this->session->userdata('regnumber')));
            ...
            $insert_dup_oldbcbf_application_data['member_no'] = $this->session->userdata('regnumber');
        ...
        if (isset($_POST['btnPreview'])) {
            ...
            if ($inser_id = $this->master_model->insertRecord('member_exam', $inser_array, true)) {
                ...
                $log_title   = "Old member details - 4";
                $log_message = serialize($oldmemberdetails);
                $rId         = $this->session->userdata('regnumber');
                $regNo       = $this->session->userdata('regnumber');
                storedUserActivity($log_title, $log_message, $rId, $regNo);
        ...
        if (isset($_POST['processPayment']) && $_POST['processPayment']) {
            ...
            if (count($subject_arr) > 0) {
                ...
                foreach ($subject_arr as $k => $v) {
                    ...
                    $capacity = get_capacity($v['venue'], $v['date'], $v['session_time'], $this->session->userdata['examinfo']['selCenterName'], base64_decode($this->session->userdata['examinfo']['excd']));   //priyanka d >>27-dec-24 >> by default selecting venue for jaiib/caiiib as we don't have to create admitcard from filled form now >> exam_cd
                    if ($capacity <= 1) {
                        $total_admit_count = getLastseat(base64_decode($this->session->userdata['examinfo']['excd']), $this->session->userdata['examinfo']['selCenterName'], $v['venue'], $v['date'], $v['session_time']);
                        if ($total_admit_count > 0) {
                            $msg = getVenueDetails($v['venue'], $v['date'], $v['session_time'], $this->session->userdata['examinfo']['selCenterName']);
        ...
        if (isset($_REQUEST['transaction_response'])) {
            ...
            if ($auth_status == '0300' && $qry_api_response['auth_status'] == '0300') {
                ...
                if ($get_user_regnum[0]['status'] == 2) {
                    ...
                    $log_title   = "Old member details - 2";
                    $log_message = serialize($oldmemberdetails);
                    ...
                    if ($exam_info[0]['exam_code'] != 101 && $exam_info[0]['exam_code'] != 1046 && $exam_info[0]['exam_code'] != 1047) {
                        ...
                        $log_title   = "Old member details - 3";
                        $log_message = serialize($oldmemberdetails);
                        $rId         = $get_user_regnum[0]['member_regnumber'];
                        $regNo       = $get_user_regnum[0]['member_regnumber'];
                        storedUserActivity($log_title, $log_message, $rId, $regNo);
    ...
    public function details($order_no = null, $excd = null)
    {
        ...
        if ($this->get_client_ip1() == '115.124.115.72') {
            ...
            // echo base64_encode(904636311).'/'.base64_encode(600);exit;
        }
    ...
    public function examapplied($regnumber = null, $exam_code = null, $exam_date_arr = array())
    {
        ...
        if (count($applied_exam_info) <= 0 && count($exam_date_arr) > 0) {
            ...
            if (count($applied_exam_info) > 0) //START : LOG ADDED BY SAGAR M ON 2024-10-02
            {
                ...
                if (isset($_POST) && count($_POST) > 0) {
                    $add_duplicate_log['posted_data'] = json_encode($_POST);
                }
                if (isset($_SESSION)) {
                    $add_duplicate_log['session_data'] = json_encode($_SESSION);
                }
    ...
    public function change_center()
    {
        ...
        //$examcodes = array(600, 505, 506, 507, 508, 509);
        $examcodes = array(210);
        $this->db->where('elg_mem_o', 'Y');
        $this->db->join('subject_master', 'subject_master.exam_code=exam_master.exam_code');
        $this->db->join('center_master', 'center_master.exam_name=exam_master.exam_code');
        ...
        foreach ($exam_list as $kk => $curr_exam) {
            $this->db->where('exam_date > ', $today_date);
            $this->db->where('exm_prd', $curr_exam['exam_period']);
            $this->db->where('admitcard_image != ', '');
            $admitcard_record = $this->master_model->getRecords('admit_card_details', array('mem_mem_no' => $this->session->userdata('regnumber'), 'exm_cd' => $curr_exam['exam_code'], 'remark' => 1));
            $exam_list[$kk]['center_code'] = $admitcard_record[0]['center_code'];
            $exam_list[$kk]['center_name'] = $admitcard_record[0]['center_name'];
        }
        $registrations_till_date = strtotime('2024-10-28'); //strtotime($exam_list[0]['exam_to_date']);
        $centerchange_last_date = strtotime("+9 day", $registrations_till_date);
        $centerchange_last_date = date('Y-m-d', $centerchange_last_date);
        $registrations_till_date = date('Y-m-d', strtotime($exam_list[0]['exam_to_date']));
        //echo $centerchange_last_date;exit;
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $this->form_validation->set_rules('transfer_letter', 'Transfer order letter', 'file_required|file_allowed_type[jpg,jpeg,pdf]|file_size_max[300]|callback_transfer_letter_upload');
            if ($this->form_validation->run() == TRUE) {
                ...
                if (count($record_exist) > 0) {
                    ...
                    $emailerstr = $this->master_model->getRecords('emailer', array(
                        'emailer_name' => 'center_change_request_sent',
                    ));
                    ...
                    if ($record_exist['status'] != 1) {
                        if ($record_exist['status'] == 2) {
                            ...
                            $user_data['current_center'] = $_POST['current_center'];
                        ...
                        if (count($emailerstr) > 0) {
                            ...
                            $info_arr  = array(
                                'to'      => 'iibfdevp@esds.co.in', //$member_info[0]['email'],
                                'from'    => $emailerstr[0]['from'],
                                'subject' => $emailerstr[0]['subject'] . ' ' . $this->session->userdata('regnumber'),
                                'message' => $final_str,
                            );
                            if ($this->Emailsending->mailsend($info_arr)) {
                                // echo $member_info[0]['email'];exit;
                            }
                        }
    ...
    public function getCentersByExamCode()
    {
        ...
        $this->db->where('center_master.exam_period', '225');
        $this->db->where("center_delete", '0');
    ...
    /* Added by Pooja Mane for mobile & email OTP verification 2024-11-15*/
    ...
    /* Added by Pooja Mane for mobile & email OTP verification 2024-11-29*/
}
