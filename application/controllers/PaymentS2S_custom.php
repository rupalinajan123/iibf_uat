...
class PaymentS2S_custom extends CI_Controller {
...
echo "MerchantOrderNo = ".$MerchantOrderNo; print_r($update_data);
...

	{
		...
		$filehandle = fopen("cs2s_log/lock_custom.txt", "c+");
		...
		if (flock($filehandle, LOCK_EX | LOCK_NB)) {
			...
			//$query="SELECT *  FROM `payment_transaction` WHERE `exam_code` IN (21,42,992) AND `date` BETWEEN '2021-03-09 04:00:00.000000' AND '2021-03-09 13:00:00.000000' AND `pay_type` = 2 AND `status` = 2 ORDER BY `id` DESC LIMIT 200";

			$query="SELECT *  FROM `payment_transaction` WHERE `receipt_no`= 812250024 ";
...
explode('|','1000169|3331894563539|SUCCESS|IN|INR|2554532^iibfregn^2554532^812250024|812250024|1770|Payment In Clearing|sbiepay|202108900970963|2021-03-30 16:27:46|DC|10001692021033001422|1000169|0.00^0.00||||||||||&CALLBACK=C_S2S');
...

				{
...

					{
...

						{
							...
							$log_title ="Payment status update in payment transaction C_S2S :".$reg_id;
	...
	$this->Emailsending->mailsend_attch($info_arr,$attachpath);
			  ...
			  if (!empty($applicationNo)) {
				...
				## Code commented on 8-Mar-2021  Uncommented on 9 Mar 2021
