...
class CSCAdmitcard_imagename_missing extends CI_Controller {
	...
	public function settle(){
		...
		$this->db->where("module_type","CSCAdmitcard_imagename_missing");
		...
		if(count($eligible) > 0){
			...
			if($start_limit[0]['start_point'] >= count($cnt)){
				...
				$this->master_model->updateRecord('cron_limit',$arr_update,array('created_at' => $current_date,'module_type'=>'CSCAdmitcard_imagename_missing'));
			}  
		}
	...
	public function cron_add($start_point,$end_point,$current_date){
		$insert_limit = array(
								...
								'end_point'   => $end_point,
								'module_type' => 'CSCAdmitcard_imagename_missing',
