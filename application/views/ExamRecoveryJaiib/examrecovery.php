...
<div class="container">
  <section class="content-header box-header with-border" style="height: 45px; background-color: #1287C0; ">
    <h1 class="register">JAIIB Exam Recovery Form</h1>
    <br />
  </section>
  <section class="">
    <div class="row">
      <div class="col-md-12 col-sm-12" style="">
        ...
        <?php }?>
        <form name="getDetailsForm" id="getDetailsForm" method="post" action="<?php echo base_url(); ?>ExamRecoveryJaiib">
          <br />
          <div class="">
...
?>" <?php if (isset($row['regnumber'])) { echo "readonly='readonly'";} elseif (set_value('regnumber')) { echo "readonly='readonly'"; } ?> style="border-color:#000;" title="Membership No.">
            ...
            <div class="col-sm-12 col-md-3">
              <?php 
				...
				?>
              <a href="<?php echo base_url();?>ExamRecoveryJaiib" class="btn btn-warning" id="modify" style="">Reset</a>
  ...
  <?php if(!empty($MemberArray)) { 
	/* Payphi code start */
	$url = "https://qa.phicommerce.com/pg/api/sale?v=2";
  ...
  ?>
  <form class="form-horizontal" name="GstRecoveryAddForm" id="GstRecoveryAddForm"  method="post" action='<?php echo $url; ?>' enctype="multipart/form-data">
    <section class="content">
      <div class="row">
        <div class="col-md-12">
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title"><strong>Candidate Details</strong></h3>
            </div>
            <br/>
            <div class="table-responsive">
			  ...
			  foreach($MemberArray as $value)
			  {
				...
				/* Payphi code start */
				$amount = $value['fee_amt'];
				//$amount = 1;
				$currencyCode = "356";
				$payType = "0";
				$transactionType = "SALE";

				//$returnURL = "https://phpsamurai.esdsdev.com/payphi/return.php";
				$returnURL =  base_url().'ExamRecoveryJaiib/payphisuccess';
	
				$txnDate = date("YmdHis");

				$customerEmailId = $value['email'];
				$customerMobileNo = $value['mobile'];

				$hashString = $amount."".$currencyCode."".$customerEmailId."".$customerMobileNo."".$merchantID."".$merchantTxnNo."".$payType."".$returnURL."".$transactionType."".$txnDate;

				$secureHash = hash_hmac('SHA256', $hashString, $key);
				$_SESSION['merchantTxnNo'] = $value['merchantTxnNo'];
			  ?>
                <tr>
                  <td><?php echo $sr; ?></td>
                  ...
                  <td><?php 
					if($value['pay_type'] == 2){
						echo $payStatusArr[$value['pay_type']].' ('.$value['description'].')';
					}else{
						echo $payStatusArr[$value['pay_type']]; 
					}?></td>
                  <td><span>&#8377;</span>&nbsp;<?php echo $value['fee_amt'].'+GST'; ?></td>
                  <td><?php  
                    ...
                    <?php
                        ...
                        ?>
                    <a href="<?php echo base_url();?>ExamRecoveryJaiib/stored_details/<?php echo base64_encode($value['invoice_id']); ?>" class="btn btn-info" style="width: 75px;">Pay Now</a>
					<!--  Payphi code -->
					<input type="hidden" name="merchantID" value="<?php echo $merchantID; ?>" />
					<input type="hidden" name="merchantTxnNo" value="<?php echo $merchantTxnNo; ?>" />
					<input type="hidden" name="amount" value="<?php echo $amount; ?>" />
					<input type="hidden" name="currencyCode" value="<?php echo $currencyCode; ?>" />
					<input type="hidden" name="payType" value="<?php echo $payType; ?>" />
					<input type="hidden" name="customerEmailId" value="<?php echo $customerEmailId; ?>" />
					...
					<input type="hidden" name="customerMobileNo" value="<?php echo $customerMobileNo; ?>" />
					<input type="hidden" name="secureHash" value="<?php echo $secureHash; ?>" />
					<!-- <input type="submit" name="pay_now" value="Payphi" onclick="storeData('<?php echo base64_encode($value['invoice_id']); ?>','<?php echo $value['merchantTxnNo'];?>');" /> -->
					<?php }?></td>
                </tr>
                <?php 
			  $sr++;
...
<script>
function storeData(inv_id,merchantTxnNo)
{
	//alert(inv_id);
	if(inv_id != "")
	{
		 $.ajax({url: "<?php echo base_url();?>ExamRecoveryJaiib/payphi_stored_details/"+inv_id+"/"+merchantTxnNo, success: function(result){
...
window.addEventListener('popstate', function(event) {
    window.location.assign(site_url+"ExamRecoveryJaiib/");
});
