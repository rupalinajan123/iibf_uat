...
<body>
 <table border="0" cellpadding="0" cellspacing="0" width="900px" align="center" style=" margin-bottom:10px;">
   ...
   <td rowspan="2" width="75%">
  	<ul>
    	<?php 
			if($this->session->userdata('result_examcode') != '101'){
				if(in_array($this->session->userdata('result_examcode'),array('11',$this->config->item('examCodeJaiib'),'32','33',$this->config->item('examCodeDBF'),'47','51','52',$this->config->item('examCodeCaiib')))){
		?>
    	<li class="resultlink"><a href="<?php echo base_url()?>examresult/consolidatedresult" target="_blank">Consolidated Marksheet</a></li>
        <?php } }?>
        <?php //echo "###". $this->session->userdata('result_examcode');?>
        <?php if($this->session->userdata('result_examcode') != ''){?>
        <li class="resultlink"><a href="<?php echo base_url()?>examresult/adviceresultjcd" target="_blank">Result Advice</a></li>
        <?php }?>
    </ul>
   </td>
   </tr>
