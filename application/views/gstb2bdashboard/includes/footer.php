...
<script> 	
	...
	$(document).ready(function() 
	{			
		$("#from_date_cn").attr('autocomplete', 'off');
		$("#to_date_cn").attr('autocomplete', 'off');
				
		$('#from_date_cn').datepicker({ format: 'yyyy-mm-dd', endDate: '-2d', autoclose: true, forceParse: true }).on('changeDate', function()
		{
			$('#to_date_cn').datepicker('setStartDate', new Date($(this).val()));
		}); 
		
		$('#to_date_cn').datepicker({ format: 'yyyy-mm-dd', endDate: '-2d', autoclose: true, forceParse: true }).on('changeDate', function()
		{
			$('#from_date_cn').datepicker('setEndDate', new Date($(this).val()));
		});
	});
	</script>
...
<?php if(isset($from_date_cn) && $from_date_cn != "") {	?> 
	<script>$('#to_date_cn').datepicker({ format: 'yyyy-mm-dd', startDate:'<?php echo $from_date_cn; ?>', endDate: '+0d', autoclose: true });</script>	
<?php }

if(isset($to_date_cn) && $to_date_cn != "") {	?> 
	<script>$('#from_date_cn').datepicker({ format: 'yyyy-mm-dd', endDate: '<?php echo $to_date_cn; ?>', autoclose: true });</script>	
<?php } ?>
