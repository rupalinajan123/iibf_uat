...
<html>
	...
	<body class="fixed-sidebar">
		...
		$bcbf_agency_code = (isset($_SESSION['IIBF_BCBF_AGENCY_CODE']) ? $_SESSION['IIBF_BCBF_AGENCY_CODE'] : '');
		?>
    ...
    <?php /****** START : Modal for display the form preview data  ******/ ?>
		...
		<?php $this->load->view('iibfbcbf/common/inc_cropper_script', array('page_name'=>'bcbf_agency_add_candidate')); ?>
      ...
      /********** START : Function for applying validation to 'Id Proof Number' input field  ***********/
            ...
            last_name:{ allow_only_alphabets_and_space:true,maxlength:20 },
						...
						mobile_no:{ required: true, allow_only_numbers:true, first_zero_not_allowed:true, maxlength:10, minlength:10, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_mobile_exist/0/1'); ?>", type: "post", data: { "enc_candidate_id": function() { return "<?php echo $enc_candidate_id; ?>"; }, "enc_batch_id":"<?php echo $enc_batch_id; ?>" } } },            
						alt_mobile_no:{ allow_only_numbers:true, first_zero_not_allowed:true, maxlength:10, minlength:10 },
						email_id:{ required: true, maxlength:80, valid_email:true, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_email_exist/0/1'); ?>", type: "post", data: { "enc_candidate_id": function() { return "<?php echo $enc_candidate_id; ?>"; }, "enc_batch_id":"<?php echo $enc_batch_id; ?>" } } },
            ...
            { 
              ...
              allow_only_alphabets_and_space:true, maxlength:90
						...
						id_proof_number:{ validate_id_proof_number: true, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_id_proof_number_exist/0/1'); ?>", type: "post", data: { "enc_candidate_id": function() { return "<?php echo $enc_candidate_id; ?>"; }, "enc_batch_id":"<?php echo $enc_batch_id; ?>" } } },
            ...
            candidate_sign:{ <?php if($mode == 'Add' || ($mode == 'Update' && $form_data[0]['candidate_sign'] == "")) { ?>required: true,<?php } ?> check_valid_file:true, valid_file_format:'.jpg,.jpeg,.png', filesize_min:'14000', filesize_max:'20000' }, //use size in bytes //filesize_max: 1MB : 1000000 
						aadhar_no:{ allow_only_numbers:true, maxlength:12, minlength:12, CheckAadharNumberWithIdProof: true, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_aadhar_no_exist/0/1'); ?>", type: "post", data: { "enc_candidate_id": function() { return "<?php echo $enc_candidate_id; ?>"; }, "enc_batch_id":"<?php echo $enc_batch_id; ?>" } } },
					},
					messages:
					{
        ...
        { 
          ...
          allow_only_alphabets_and_space:true, maxlength:90 
				...
				$("#id_proof_number").rules("add", { validate_id_proof_number: true, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_id_proof_number_exist/0/1'); ?>", type: "post", data: { "enc_candidate_id": function() { return "<?php echo $enc_candidate_id; ?>"; }, "enc_batch_id":"<?php echo $enc_batch_id; ?>" } } });
      ...
      /***** START : Function to get the candidate details when 'Training ID or Registration Number' entered. Only applicable for centre  ***/
			...
			{      
					...
					$.ajax(
					{
						...
						success:function(data)
						{
							if(data.flag == "success")
							{
								...
								$("#bank_emp_id").val(data.response.bank_emp_id);

                $("input[name='associated_with_any_bank'][value='2']").prop("checked", true);

                var selected_cnt = 0;
                if(data.response.bank_associated != "")
                {
                  <?php if(count($bank_associated_master_data) > 0) 
                  {
                    foreach($bank_associated_master_data as $res) 
                    { ?>
                      if(data.response.bank_associated == "<?php echo $res['bank_code']; ?>")
                      {
                  ...
                  if(selected_cnt == 0)
                  {
                    $("#bank_associated").val('Other');
                    $("#bank_associated_other").val(data.response.bank_associated);
                  }
                  show_hide_other_section();
                }
								...
								if($.trim(data.response.id_proof_file) != "")
								{
									...
									$('#id_proof_file_preview').html('<a href="'+id_proof_file_url+'" class="example-image-link" data-lightbox="candidate_images" data-title="Proof of Identity - '+data.response.first_name+' ('+data.response.training_id+')"><img src="'+id_proof_file_url+'"></a>');   
								...
								if($.trim(data.response.qualification_certificate_file) != "")
								{
									...
									$('#qualification_certificate_file_preview').html('<a href="'+qualification_certificate_file_url+'" class="example-image-link" data-lightbox="candidate_images" data-title="Qualification Certificate - '+data.response.first_name+' ('+data.response.training_id+')"><img src="'+qualification_certificate_file_url+'"></a>');
								...
								if($.trim(data.response.candidate_photo) != "")
								{
									...
									$('#candidate_photo_preview').html('<a href="'+candidate_photo_url+'" class="example-image-link" data-lightbox="candidate_images" data-title="Passport-size Photo of the Candidate - '+data.response.first_name+' ('+data.response.training_id+')"><img src="'+candidate_photo_url+'"></a>');
								...
								if($.trim(data.response.candidate_sign) != "")
								{
									...
									$('#candidate_sign_preview').html('<a href="'+candidate_sign_url+'" class="example-image-link" data-lightbox="candidate_images" data-title="Signature of the Candidate - '+data.response.first_name+' ('+data.response.training_id+')"><img src="'+candidate_sign_url+'"></a>');   
                ...
                $("input[name='associated_with_any_bank'][value='1']").prop("checked", true);
								$("#bank_associated").val("");
								$("#bank_associated_other").val("");
