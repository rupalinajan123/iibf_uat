<div class="modal-header">
  ...
  <h4 class="modal-title">Change Password : <?php echo $form_data[0]['agency_name']; ?></h4>
</div>

<form class="modal_form_all" method="post" action="<?php echo site_url('iibfbcbf/admin/agency/change_password/'.$enc_agency_id); ?>" id="change_pass_form" enctype="multipart/form-data" autocomplete="off">
	<div class="modal-body">
		<div class="form-group row">
      <label class="col-xl-3 col-lg-3" for="agency_password" class="form_label">New Password <sup class="text-danger">*</sup></label>
      <div class="col-xl-9 col-lg-9">
        <div class="form-group login_password_common">
          <input type="password" class="form-control custom_input" name="agency_password" id="agency_password" value="<?php echo set_value('agency_password'); ?>" placeholder="New Password *" required minlength="8" maxlength="20">
          ...
          <span class="show-password" onclick="show_hide_password(this,'show', 'agency_password')" style="top:8px;"><i class="fa fa-eye" aria-hidden="true"></i></span>
          <span class="hide-password" onclick="show_hide_password(this,'hide', 'agency_password')" style="display:none;top:8px;"><i class="fa fa-eye-slash" aria-hidden="true"></i></span>
        </div>
      </div>
    </div>
	...
	$(document ).ready( function() 
	{
      ...
      onkeyup: function(element) 
      {
        $(element).valid();
			...
			rules:
			{
				agency_password: { required: true, minlength: 8, maxlength:20, pwcheck: true, remote: { url: "<?php echo site_url('iibfbcbf/admin/agency/check_old_password'); ?>", type: "post", data: { "enc_agency_id": function() { return "<?php echo $enc_agency_id; ?>"; } } } },
        confirm_password: { required: true, equalTo: "#agency_password" },
			},
			messages:
			{
				agency_password: { required: "Please enter the new password", minlength: "Please enter minimum 8 character", pwcheck:"Password must contain at least one upper-case character, one lower-case character, one digit and one special character", remote:"New password can not be same as current password" },					
      ...
      {
        if (element.attr("name") == "agency_password") { error.insertAfter("#agency_password_err"); }
