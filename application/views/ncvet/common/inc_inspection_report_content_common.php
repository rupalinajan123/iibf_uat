  ...
  <div class="ibox-content">
    <div class="table-responsive">
      <table class="table table-bordered custom_inner_tbl" style="width:100%">
        <tbody>
          <?php $this->load->view('ncvet/common/inc_training_batch_details_common'); ?> 
          ...
          <tr>
            ...
            { ?>
              <td>
                ...
                <a href="<?php echo site_url('ncvet/download_file_common/index/'.url_encode($batch_data[0]['batch_id']).'/inspection_report_by_admin'); ?>" class="example-image-link btn btn-success btn-sm">Download Inspection Report</a>
              </td>
            <?php } ?>
          </tr>
      ...
      <table class="table table-bordered table-striped batch_inspection_form_tbl" style="width:100%">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>Whether the faculty (in case of new faculty only) and all the candidates have attended preparatory / briefing session on the etiquettes of the upcoming NCVET training (Yes / No).</td>
            <?php if(count($inspection_data) > 0)
            {
              foreach($inspection_data as $res)
              { ?>
          ...
          <tr>
            <td class="text-center"></td>
            <td>d) Quality of learning by NCVET:<br>i.  Interaction with Faculty</td>
            <?php if(count($inspection_data) > 0)
            {
              foreach($inspection_data as $res)
              { ?>
          ...
          <tr>
            ...
            <td>Any other observations with respect to non-adherence to the conditions stipulated by IIBF for conducting on-line NCVET Training.</td>
            <?php if(count($inspection_data) > 0)
            {
              foreach($inspection_data as $res)
              { ?>
          ...
          <tr>
            ...
            <td>Overall Observation of the Inspector on the training of the NCVET Batch.</td>
            <?php if(count($inspection_data) > 0)
            {
              foreach($inspection_data as $res)
              { ?>
          ...
          <tr>
            ...
            <td>Over all compliance on imparting of NCVET Training</td>
            <?php if(count($inspection_data) > 0)
            {
              foreach($inspection_data as $res)
              { ?>
            ...
            <?php if(count($inspection_data) > 0)
            {
              ...
              { ?>
                <td>
                  ...
                  { ?>
                    <a href="<?php echo site_url('ncvet/download_file_common/index/'.url_encode($res['inspection_id']).'/attachment'); ?>" class="example-image-link btn btn-success btn-sm" target="_blank">Download</a>
                  <?php } ?>
                </td>
              <?php }
            } ?>
        ...
        <table class="table table-bordered table-hover table-striped batch_inspection_form_tbl dataTables-example toggle_btn_tbl_outer" style="width:100%">
          <thead>
            <tr> 
              ...
              <?php if($this->session->userdata('NCVET_USER_TYPE') == 'admin') { ?>
            ...
            { ?>
              <tr>
                ...
                <?php if($this->session->userdata('NCVET_USER_TYPE') == 'admin') { ?>
                  ...
                  <?php if(isset($inspection_report_pdf_flag) && $inspection_report_pdf_flag == '1') { }else { ?>
                    <td class="text-center">
                      <a href="<?php echo site_url('ncvet/admin/inspection_summary_admin/candidates_inspection_details/'.url_encode($batch_data[0]['batch_id']).'/'.url_encode($batch_candidate_res['candidate_id'])); ?>" class="btn btn-success btn-xs" title="View Candidate Inspection Details" target="_blank"><i class="fa fa-eye" aria-hidden="true"></i></a>
                    </td>
                  <?php }
                } ?>                             
              </tr>
