  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body text-centerr">
        ...
        <div class="guidelines_style1">Before applying online, a candidate will be required to have a scanned (digital) image of his/her photograph, signature, Aadhar card & declaration as per the specifications given below. Your online application will not be registered unless you upload your photograph, signature, Aadhar card & declaration as specified</div> -->
        ...
        <label class="mt-5 error" id="img_pop_up_error" style="margin: 0px 0px 0 219px !important;"></label>
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  (() => 
  {
    ...
    fileChooser.on("change", (e) => 
    {
      ...
      var ImageExtensionError = 'Please upload only .png, .jpg, .jpeg, .pdf image.';  
      if(current_image_id=='candidate_photo' || current_image_id=='candidate_sign') 
      {
        valid_extensions_arr = valid_extensions_arr.filter(function(item) {
            return item !== 'pdf' && item !== 'application/pdf';
        });
         console.log('my array',valid_extensions_arr);

        if(img_type_str=='jpg' || img_type_str=='jpeg' || img_type_str=='png') {
        }
        else 
        {
          show_error_flag = 1;
          ImageExtensionError = "Please upload only .png, .jpg, .jpeg image.";  
        }
      }
      else 
      {  
        ...
        else if( (img_type_str=='application/pdf' || img_type_str=='pdf') && show_error_flag == 0)  
        {
          valid_extensions_arr.push('pdf','application/pdf');
      ...
      { 
        ...
        sweet_alert_error('Please upload valid image.'); 
      } 
      else if ($.inArray(img_type_str, valid_extensions_arr) === -1) 
      { 
        show_error_flag = 1;
        // $("#img_pop_up_error").html(ImageExtensionError);
        sweet_alert_error(ImageExtensionError); 
      }
      else if (img_size_in_kb > 5120 || img_size_in_kb < 40) 
      {
        show_error_flag = 1;         
        // $("#img_pop_up_error").html("Please upload image having size less than 5MB.")
        sweet_alert_error("Please upload image having size minimum 40KB and maximum 5MB."); 
      }
      
      $("#img_pop_up_error").html("");
       
      if (img_type_str != 'application/pdf' && show_error_flag == 0) 
      {
        show_error_flag = 0;
      ...
      if(show_error_flag == '1' && img_type_str != 'application/pdf')
      { 
        if($("#current_image_id").val() == 'candidate_photo' || $("#current_image_id").val() == 'candidate_sign' || $("#current_image_id").val() == 'id_proof_file' || $("#current_image_id").val() == 'aadhar_file' || $("#current_image_id").val() == 'declaration' || $("#current_image_id").val() == 'empidproofphoto' || $("#current_image_id").val() == 'bank_bc_id_card' || $("#current_image_id").val() == 'declarationform' || $("#current_image_id").val() == 'institute_idproof' || $("#current_image_id").val() == 'qualification_certificate_file' || $("#current_image_id").val() == 'exp_certificate' || $("#current_image_id").val() == 'institute_idproof' || $("#current_image_id").val() == 'vis_imp_cert_img' || $("#current_image_id").val() == 'orth_han_cert_img' || $("#current_image_id").val() == 'cer_palsy_cert_img') 
        { 
    ...
    function cropImage() 
    {
      ...
      if(db_tbl_name == 'ncvet_candidates') //FOR NCVET CANDIDATES
      {
    ...
    function saveImage(t,progress,progressBar,blob='') {
      ...
      if(blob!=''){
        ...
        $("#page_loader").show();
        $.ajax('<?php echo site_url('ncvet/save_cropper_image_edit/save_image'); ?>', 
        {
          method: 'POST',
          data: formData,
          processData: false,
          ...
          success: function(data) 
          {
            try 
            {
              if ($.trim(data.flag) == 'success') 
              {
                ...
                else if(current_image_id == 'aadhar_file') { lightbox_data_title = 'Aadhar File - '; }
                ...
                else if(current_image_id == 'exp_certificate') { lightbox_data_title = 'Experience Certificate - '; }
                else if(current_image_id == 'institute_idproof') { lightbox_data_title = 'Institutional IDproof - '; }
                // else if(current_image_id == 'scanned_vis_imp_cert') { lightbox_data_title = 'PWD certificate - '; }
                // else if(current_image_id == 'scanned_orth_han_cert') { lightbox_data_title = 'PWD certificate - '; }
                // else if(current_image_id == 'scanned_cer_palsy_cert') { lightbox_data_title = 'PWD certificate - '; }
                else if(current_image_id == 'vis_imp_cert_img') { lightbox_data_title = 'Visually Impaired Certificate - '; }
                else if(current_image_id == 'orth_han_cert_img') { lightbox_data_title = 'Orthopedically Handicapped Certificate - '; }
                else if(current_image_id == 'cer_palsy_cert_img') { lightbox_data_title = 'Cerebral Palsy Certificate - '; }
                ...
                if(blob!=''){
                  ...
                  var mozilaURL = "https://mozilla.github.io/pdf.js/web/viewer.html?file=";
                  var iframeSRC = mozilaURL+data.response_msg+'?'+ currentTime;
                  var PDFURL    = "<?php echo base_url('assets/images/pdf.png'); ?>";
                  
                  $("#" + preview_id).html('<a data-caption="'+lightbox_data_title+data_lightbox_title_hidden+'" data-fancybox data-type="iframe" data-src="'+iframeSRC+'" href="javascript:;" title="'+lightbox_data_title+data_lightbox_title_hidden+'"><img src="'+PDFURL+'" width="60" height="60" alt="PDF"></a>');
                }
  ...
  function open_guidelines_modal() 
  {
    ...
    {
      $("#GuidelinesModalTitle").html('Guidelines for Accepting APAAR ID/ABC ID.');  
      
      var content = '';
      // content +='<div class="guidelines_style2 mt-5">';
      // content +='<ul>';
      // content +='<li>APAAR ID/ABC ID uploaded should be of clearly visible.</li>';    
      // content +='<li>Incorrect size or cropped APAAR ID/ABC ID shall not be accepted.</li>';    
      // content +='</ul>';

      // content +='<h4 class="guidelines_header mt-5 mb-3 pb-2 text-center">Acceptable APAAR ID/ABC ID Guide</h4>';
      // content +='<ul>';
      // content +='<li>APAAR ID/ABC ID to be submitted should be clear / readable and verifiable.</li>';    
      // content +='<li> Card image should be clearly visible.</li>';    
      // content +='</ul>';
      // content +='</div>'; 
    ...
    {
      $("#GuidelinesModalTitle").html('Guidelines for Accepting Aadhar Card.');  
      
      var content = '';
      // content +='<div class="guidelines_style2 mt-5">';
      // content +='<ul>';
      // content +='<li>Aadhar Card uploaded should be of clearly visible.</li>';    
      // content +='<li>Incorrect size or cropped Aadhar Card shall not be accepted.</li>';    
      // content +='</ul>';

      // content +='<h4 class="guidelines_header mt-5 mb-3 pb-2 text-center">Acceptable Aadhar Card Guide</h4>';
      // content +='<ul>';
      // content +='<li>Aadhar Card to be submitted should be clear / readable and verifiable.</li>';    
      // content +='<li> Card image should be clearly visible.</li>';    
      // content +='</ul>';
      // content +='</div>'; 
    ...
    {
      $("#GuidelinesModalTitle").html('Qualification Certificate Upload Guidelines');  
      
      var content = '';
      // content +='<div class="guidelines_style2 mt-5">';
      // content +='<ul>';
      // content +='<li>Qualification Certificate uploaded should be of clearly visible.</li>';    
      // content +='<li>Incorrect size or cropped Qualification certificate shall not be accepted.</li>';    
      // content +='</ul>';

      // content +='<h4 class="guidelines_header mt-5 mb-3 pb-2 text-center">Acceptable Qualification Certificate Guide</h4>';
      // content +='<ul>';
      // content +='<li>Qualification Certificate to be submitted should be clear / readable and verifiable.</li>';    
      // content +='<li> Qualification Certificate image should be clearly visible.</li>';    
      // content +='</ul>';
      // content +='</div>';
    ...
    {
      $("#GuidelinesModalTitle").html('Experience Certificate Upload Guidelines');    
      
      var content = '';
      // content +='<div class="guidelines_style2 mt-5">';
      // content +='<ul>';
      // content +='<li>Experience Certificate uploaded should be clearly visible.</li>';    
      // content +='<li>Incorrect size or cropped Experience Certificate shall not be accepted.</li>';    
      // content +='</ul>';
      // content +='</div>';  
    ...
    {
      $("#GuidelinesModalTitle").html('Institution ID Proof Upload Guidelines');    
      
      var content = '';
      // content +='<div class="guidelines_style2 mt-5">';
      // content +='<ul>';
      // content +='<li>Institution ID Proof uploaded should be clearly visible.</li>';    
      // content +='<li>Incorrect size or cropped Institution Proof shall not be accepted.</li>';    
      // content +='</ul>';
      // content +='</div>';  
    ...
    else if(current_image_id == 'vis_imp_cert_img' || current_image_id == 'orth_han_cert_img'  || current_image_id == 'cer_palsy_cert_img')
    {
      $("#GuidelinesModalTitle").html('Disability certificates Upload Guidelines');    
  ...
  { 
    if (db_tbl_name == 'ncvet_candidates') //FOR NCVET MEMBER
    {
      if (current_image_id == 'candidate_photo') 
      {
        $("#img_pop_up_title").html('Upload your Passport-size Photo');
        $("#cropModal .modal-title").html('Upload your Passport-size Photo');
        $("#img_pop_up_note").html('Note: Please select only .jpg, .jpeg file upto 5MB');
      } 
      else if (current_image_id == 'candidate_sign') 
      {
      ...
      {
        $("#img_pop_up_title").html('Upload your APAAR ID/ABC ID');
        $("#cropModal .modal-title").html('Upload your APAAR ID/ABC ID');
        $("#img_pop_up_note").html('Note: Please select only .jpg, .jpeg, .PDF file upto 5MB');
      }
      else if (current_image_id == 'aadhar_file') 
      {
        $("#img_pop_up_title").html('Upload your Aadhar Card');
        $("#cropModal .modal-title").html('Upload your Aadhar Card');
        $("#img_pop_up_note").html('Note: Please select only .jpg, .jpeg, .PDF file upto 5MB');
      }
      else if (current_image_id == 'institute_idproof') 
      {
        $("#img_pop_up_title").html('Upload your Institution ID');
        $("#cropModal .modal-title").html('Upload your Institution ID');
        $("#img_pop_up_note").html('Note: Please select only .jpg, .jpeg, .PDF file upto 5MB');
      }
      else if (current_image_id == 'declaration' || current_image_id == 'declarationform') 
      {
      ...
      {
        $("#img_pop_up_title").html('Upload your Qualification certificate');
        $("#cropModal .modal-title").html('Upload your Qualification certificate');
        $("#img_pop_up_note").html('Note: Please select only .jpg, .jpeg, .PDF file upto 5MB');
      }
      else if (current_image_id == 'exp_certificate') 
      {
        $("#img_pop_up_title").html('Upload your Experience certificate');
        $("#cropModal .modal-title").html('Upload your Experience certificate');
        $("#img_pop_up_note").html('Note: Please select only .jpg, .jpeg, .PDF file upto 5MB');
      }
      else if (current_image_id == 'vis_imp_cert_img') 
      {
        $("#img_pop_up_title").html('Upload your Visually Impaired Certificate');
        $("#cropModal .modal-title").html('Upload your Visually Impaired Certificate');
        $("#img_pop_up_note").html('Note: Please select only .jpg, .jpeg, .PDF file upto 5MB');
      }
      else if (current_image_id == 'orth_han_cert_img') 
      {
        $("#img_pop_up_title").html('Upload your Orthopedically Handicapped Certificate');
        $("#cropModal .modal-title").html('Upload your Orthopedically Handicapped Certificate');
        $("#img_pop_up_note").html('Note: Please select only .jpg, .jpeg, .PDF file upto 5MB');
      }
      else if (current_image_id == 'cer_palsy_cert_img') 
      {
        $("#img_pop_up_title").html('Upload your Cerebral Palsy Certificate');
        $("#cropModal .modal-title").html('Upload your Cerebral Palsy Certificate');
        $("#img_pop_up_note").html('Note: Please select only .jpg, .jpeg, .PDF file upto 5MB');
      }
    }
  }
