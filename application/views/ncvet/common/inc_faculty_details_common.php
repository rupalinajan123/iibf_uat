...
<tr>
  <td <?php if($faculty_data[0]['created_by_type'] == '2' && $this->session->userdata('NCVET_USER_TYPE') != 'centre') { echo 'colspan="2"'; } ?>>
    <b>Agency Name</b> : 
...
{ ?>
  <tr>
    <td><b>Period of Association with the agency in providing NCVET training (Year)</b> : <?php echo $faculty_data[0]['training_faculty_exp_year']; ?></td>
    <td><b>Period of Association with the agency in providing NCVET training (Month)</b> : <?php echo $faculty_data[0]['training_faculty_exp_month']; ?></td>
  </tr>
<?php } */ ?>
...
<tr>
  <?php if($this->session->userdata('NCVET_USER_TYPE') != 'centre')
  { ?>
    <td><b>Added By</b> : <?php if($faculty_data[0]['created_by_type'] == '1') { echo 'Centre'; } else { echo 'Agency'; } ?></td>
  <?php } ?>

  <td <?php if($this->session->userdata('NCVET_USER_TYPE') == 'centre') { echo 'colspan="2"'; } ?>><b style="vertical-align:top">Status</b> : <span class="disp_status_details badge <?php echo show_faculty_status($faculty_data[0]['status']); ?>" style="min-width:90px;"><?php echo $faculty_data[0]['DispStatus']; ?></span></td>
</tr>