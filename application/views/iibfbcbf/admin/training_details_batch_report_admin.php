      ...
      <?php $this->load->view('iibfbcbf/admin/inc_sidebar_admin'); ?>
        ...
        <?php $this->load->view('iibfbcbf/admin/inc_topbar_admin'); ?>
				...
				<div class="row wrapper border-bottom white-bg page-heading">
					<div class="col-lg-12">
						<h2>Training Batch Details Report</h2>
						<ol class="breadcrumb">
							<li class="breadcrumb-item"><a href="<?php echo site_url('iibfbcbf/admin/dashboard_admin'); ?>">Dashboard</a></li>
							<li class="breadcrumb-item">Reports</li>
							<li class="breadcrumb-item active"> <strong>Training Batch Details Report</strong></li>
						</ol>
					</div>
				</div>
                ...
                <div class="ibox-content">
                  <form method="POST" action="<?php echo base_url("iibfbcbf/admin/reports/get_training_batch_details_data_ajax"); ?>" id="search_form" class="search_form_common_all" action="javascript:void(0)" autocomplete="off">
                  	<input type="hidden" name="selected_batch_ids" id="selected_batch_ids">
                  ...
                  <div class="table-responsive">
                            ...
                            <label class="css_checkbox_radio"><input type="checkbox" id="checkbox_training_details_btch_all" name="checkbox_training_details_btch_all" onclick="select_all_batches()"><span class="checkmark"></span></label>
                          </th>
                          <th class="no-sort text-center nowrap" style="width:60px;">Sr. No.</th>
													...
													<th class="text-center">Total Candiadtes registered for Training</th> 
													<th class="text-center">No. of Candidates Eligible for Exam</th> 
													<th class="text-center">No. of Candidates Applied for Exam</th> 
													<!-- <th class="text-center">No. of candidates passed</th> --> 
    ...
    <script language="javascript">
      ...
      $(document).ready(function()
          ...
          searching: true,
					...
					"ajax": 
          {
						"url": '<?php echo site_url("iibfbcbf/admin/reports/get_training_batch_details_data_ajax"); ?>',
						"type": "POST",	
            "data": function ( d ) 
						{
              ...
              d.selected_batch_ids = $("#selected_batch_ids").val();
          ...
          rowReorder: false,
					...
					[
						...
						{"targets": [0, 1, 4, 5, 6, 7, 8], "className": "text-center"}, 
					],
					"aaSorting": [],
					"stateSave": false,		          			
				});
	          ...
	          $.ajax(
	          {
				...
				$('#DataTables_Table_0').on('draw.dt', function() {
				    //alert();
						if($("#checkbox_training_details_btch_all").prop('checked') == true){
							$('#checkbox_training_details_btch_all').prop('checked', false); 
						}
				});
      ...
      { 	
        ...
        {
          type: "POST",
          url: "<?php echo site_url('iibfbcbf/admin/reports/export_to_excel/'); ?>",
      ...
      function select_all_batches(){  
        if($("#checkbox_training_details_btch_all").prop('checked') == true){
          $('.checkbox_training_details_btch_common').each(function(){
              //this.checked = true;
              $('.checkbox_training_details_btch_common').prop('checked',true);
          });
          set_all_batch_id_list();
        }else{
          $('.checkbox_training_details_btch_common').each(function(){
              //this.checked = false;
           		$('.checkbox_training_details_btch_common').prop('checked',false);
          });
          $("#selected_batch_ids").val('');
          set_all_batch_id_list(); 
        } 
	  	}

	  	function set_all_batch_id_list(){

	    	var batch_id_list = [];
	    	var uncheck_batch_id_list = [];
	    	var is_checked = 0;
	    	var exist_batch_ids = $('#selected_batch_ids').val();
	    	if(exist_batch_ids != ""){
      		batch_id_list = $('#selected_batch_ids').val().split(','); // Get existing selected_batch_ids from hidden field
      	}

	    	
	    	$("input:checkbox[class=checkbox_training_details_btch_common]:checked").each(function () {
	          //alert("Id: " + $(this).attr("id") + " Value: " + $(this).val());
	    		//batch_id_list.push($(this).val());
	    		if (!batch_id_list.includes($(this).val())) { // Check if the ID is already in the list
              batch_id_list.push($(this).val()); // Add new ID to the list
          }
	      });
	      batch_id_list = batch_id_list.join(",");

	      /*$("input:checkbox[class=checkbox_training_details_btch_common]:not(:checked)").each(function () {
	          uncheck_batch_id_list.push($(this).val());
	      });
	      uncheck_batch_id_list = uncheck_batch_id_list.join(",");

	      if(uncheck_batch_id_list){
	      	alert(batch_id_list);
	      }*/
	       

	      if(batch_id_list){ 
	      	$("#selected_batch_ids").val(batch_id_list);
	      }  
	    } 

	    function only_checked_id(val) {
	    	//alert(val);
	    	var updated_list = '';
				var list = $('#selected_batch_ids').val();
	    	if($("#checkboxlist_batch_id_"+val).prop('checked') == false){ 
				   //var updated_list = removeExistingValue('1,2,3', '1'); // 2,3
				   updated_list = removeExistingValue(list, val);
				   $('#selected_batch_ids').val(updated_list);
				} 
	    }

	    function removeExistingValue(list, value) {
