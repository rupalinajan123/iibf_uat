<table class="table table-bordered custom_inner_tbl custom_inner_tbl_dark" style="max-width:600px; background:#FFFFCC; margin:0 auto;">
  <tbody>
    <tr>
      <td colspan='2'>
        <p>Dear Sir/Madam, <br /><br /> We acknowledge with thanks the receipt of the payment for BCBF Application as per the details given below:</p>
      </td>
    </tr>
    ...
    <?php 
      $candidate_receipt_url = site_url('iibfbcbf/agency/transaction_details_agency/payment_receipt_candidate_listing_agency/'.url_encode($payment_data[0]['id']));
      
      if($this->session->userdata('IIBF_BCBF_USER_TYPE') == 'admin')
      {
        $candidate_receipt_url = site_url('iibfbcbf/admin/transaction/transactions_details/'.url_encode($payment_data[0]['id']));
      } 
    ?>
    ...
    <tr>
      <td colspan='2'><b>Kindly note,<b><br />
        <ul>
          ...
          <li>IIBF reserves the right to accept or reject your Enrollment in BCBF Application without assigning any reason what so ever.</li>
          <li>IIBF will process  application based on the information provided and in case, any of the information found false or incorrect your Enrollment in BCBF Application is liable for cancellation.</li>
        </ul>
      </td>
    </tr>
  </tbody>
