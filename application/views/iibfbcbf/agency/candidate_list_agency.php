        ...
        if($enc_batch_id != '0') { $disp_title = 'Candidate List'; } ?>
				...
				<div class="row wrapper border-bottom white-bg page-heading">
					<div class="col-lg-12">
						...
						<ol class="breadcrumb">
							...
							<?php if($enc_batch_id != '0' || (isset($batch_data) && count($batch_data) > 0))  { ?><li class="breadcrumb-item"><a href="<?php echo site_url('iibfbcbf/agency/training_batches_agency'); ?>">Training Batches (<?php echo $batch_data[0]['batch_code']; ?>)</a></li><?php } ?>
                ...
                <?php if($enc_batch_id != '0' || (isset($batch_data) && count($batch_data) > 0))  { ?>
                  <div class="ibox-title">
                    <div class="ibox-tools">
                      ...
                      { /* only centre can add candidates */
                        if($batch_data[0]['batch_status'] == 3 && $chk_batch_start_date >= date('Y-m-d') && $batch_candidate_count < $batch_data[0]['total_candidates']) 
                        { ?>
                          <a href="<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/add_candidates_agency/'.$enc_batch_id); ?>" class="btn btn-primary custom_right_add_new_btn">Add Candidate</a>                    
                        <?php } 
                      } ?>
                ...
                <div class="ibox-content">
                  <form method="POST" id="search_form" class="search_form_common_all" action="javascript:void(0)" autocomplete="off">
                    ...
                    { ?>
                      <?php if($this->login_user_type == 'agency') 
                      { ?>
                    ...
                    <div class="form-group text-left">
                      <select class="form-control search_opt" name="s_status" id="s_status" >
                        ...
                        <option value="4">Auto Hold + Manual Hold</option>                       
                      </select>
                    </div>

                    <div class="form-group text-left" style="width:auto;">
                      <select class="form-control search_opt" name="s_filter_type" id="s_filter_type" >
                        <option value="">Select</option>                       
                        <option value="1" <?php if($s_filter_type == '1') { echo 'selected'; } ?>>Total Training Completed Candidates</option>                       
                        <option value="2" <?php if($s_filter_type == '2') { echo 'selected'; } ?>>Total Hold Candidates</option>                       
                        <option value="3" <?php if($s_filter_type == '3') { echo 'selected'; } ?>>Total Exam Applied Candidates</option>                       
                        <option value="4" <?php if($s_filter_type == '4') { echo 'selected'; } ?>>Total Exam Not Applied Candidates</option>                       
                      </select>
                    </div>
                  ...
                  <div class="table-responsive">
                          ...
                          <th class="no-sort text-center nowrap" style="width:60px;">Sr. No.</th>
													...
													<th class="text-center nowrap <?php if($this->session->userdata('IIBF_BCBF_USER_TYPE') == 'agency') { echo 'all'; } ?>" style="width:80px;">Status</th>
    ...
    <script language="javascript">
      $(document).ready(function()
          ...
          searching: true,
					...
					"ajax": 
          {
						"url": '<?php echo site_url("iibfbcbf/agency/batch_candidates_agency/get_batch_candidates_agency_data_ajax"); ?>',
						"type": "POST",	
            "data": function ( d ) 
						{
              d.enc_batch_id = "<?php echo $enc_batch_id; ?>";
              <?php if($this->login_user_type == 'agency') { ?>d.s_centre = $("#s_centre").val();<?php } ?>
              <?php if($enc_batch_id == '0') { ?>d.s_batch_code = $("#s_batch_code").val();<?php } ?>
							d.s_name = $("#s_name").val();
							d.s_regnumber = $("#s_regnumber").val();
							d.s_status = $("#s_status").val();              
							d.s_filter_type = $("#s_filter_type").val();              
            },
          ...
          rowReorder: false,
            ...
            {"targets": [9], "className": "text-center nowrap"},
      ...
      <?php if($this->session->userdata('IIBF_BCBF_USER_TYPE') == 'agency') { ?>
        function change_hold_release_status(cand_id, current_status)
        {
          ...
          {
            type: "POST",
            url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/change_hold_release_status_agency'); ?>",
            data: data,
