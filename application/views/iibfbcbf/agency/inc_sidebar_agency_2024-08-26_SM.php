          ...
          <div class="sidebar-collapse">
            <ul class="nav metismenu side-bg-color" id="side-menu">
              ...
              <?php if($this->session->userdata('IIBF_BCBF_USER_TYPE') == 'centre') 
              {
                ...
                $this->db->where(" (CASE WHEN em.exam_code IN (1037,1038,1041,1042) THEN sm.exam_date > '".date("Y-m-d")."' ELSE 1 END) ","",FALSE);
                ...
                if(count($get_active_exam_data) > 0)
                {
                  ...
                  { 
                    ...
                    <li <?php if($act_id == "Exam ".$exam_res['exam_code']) { ?>class="active" <?php } ?>><a href="<?php echo $apply_link; ?>"><i class="fa fa-book"></i> <span class="nav-label">Apply For <?php echo display_exam_name($exam_res['description'], $exam_res['exam_code'], $exam_res['exam_type']).' - '.$exam_res['exam_period']; /* helpers/iibfbcbf/iibf_bcbf_helper.php */ ?></span></a></li>
                  <?php }
                }
                ?>
              <?php } ?>
