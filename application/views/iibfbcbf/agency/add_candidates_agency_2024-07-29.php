...
<html>
  <head>
				...
				<div class="row wrapper border-bottom white-bg page-heading">
					<div class="col-lg-10">
						...
						<ol class="breadcrumb">
							...
							<li class="breadcrumb-item"><a href="<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/index/'.url_encode($batch_data[0]['batch_id'])); ?>">Candidate List (<?php echo $batch_data[0]['batch_code']; ?>)</a></li>
							...
							<div class="ibox">
								<div class="ibox-title">
									<div class="ibox-tools">
										<a href="<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/index/'.url_encode($batch_data[0]['batch_id'])); ?>" class="btn btn-danger custom_right_add_new_btn">Back</a>
                  </div>
                </div>
								<div class="ibox-content">
                  <form method="post" action="<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/add_candidates_agency/'.url_encode($batch_data[0]['batch_id']).'/'.$enc_candidate_id); ?>" id="add_candidate_form" enctype="multipart/form-data" autocomplete="off">
                    ...
                    <?php 
                      ...
                      <?php if($chk_batch_start_date >= date("Y-m-d"))  { ?><?php /* Centre can update the candidate's basic details till batch first day. */ ?>
                      <div class="row">
                        <?php if($mode == 'Add') { ?>
                          <div class="col-xl-12 col-lg-12">
                            <div class="get_details_outer">
                              <div>
                                <label for="training_id_regnumber" class="form_label">Training ID or Registration Number</label>
                                <input type="text" name="training_id_regnumber" id="training_id_regnumber" class="form-control" placeholder="Training ID or Registration Number" autocomplete="off" value="<?php echo set_value('training_id_regnumber'); ?>" />
                                <span id="training_id_regnumber_err" class="custom_err_msg"></span>
                              </div>
                              <button type="button" onclick="get_candidate_details()" class="btn btn-primary">Get Details</button>
                              <a href="<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/add_candidates_agency/'.url_encode($batch_data[0]['batch_id']).'/'.$enc_candidate_id); ?>" class="btn btn-danger" id="reset_btn" style="display:none;">Reset</a>
                            </div>
                          </div>
                          
                          <input type="hidden" class="form-control" readonly name="old_candidate_id" id="old_candidate_id" value="<?php echo set_value('old_candidate_id'); ?>" />
                          <input type="hidden" class="form-control" readonly name="old_training_id" id="old_training_id" value="<?php echo set_value('old_training_id'); ?>" />
                          <input type="hidden" class="form-control" readonly name="old_regnumber" id="old_regnumber" value="<?php echo set_value('old_regnumber'); ?>" />
                          <input type="hidden" class="form-control" readonly name="old_registration_type" id="old_registration_type" value="<?php echo set_value('old_registration_type'); ?>" />
                          <input type="hidden" class="form-control" readonly name="parent_table" id="parent_table" value="<?php echo set_value('parent_table'); ?>" />
                        <?php } ?>
                        
                        <div class="col-xl-12 col-lg-12">
                          <div class="alert alert-primary"><b>Below mentioned fields (i.e. Basic Details) need to be filled or edited carefully with appropriate contents till end of Day - 1 of the Training Period (i.e. by 11.59.59 PM of <?php echo date("d.m.Y",strtotime($chk_batch_start_date)); ?>).<br>No contents can be changed or edited after the Day - 1 (i.e. <?php echo date("d.m.Y",strtotime($chk_batch_start_date)); ?>) of the Training Period.</b></div> 
                        </div>
                      ...
                      <div class="table-responsive">
                          ...
                          <tbody>
                            <tr>
                              <td>
                                <b>Candidate Name : </b>
                                <?php echo $form_data[0]['salutation'].' '.$form_data[0]['first_name']; 
                                  if($form_data[0]['middle_name'] != "") { echo " ".$form_data[0]['middle_name']; } 
                                if($form_data[0]['last_name'] != "") { echo " ".$form_data[0]['last_name']; } ?>
                              </td>
                              <td><b>Training ID : </b><?php echo $form_data[0]['training_id']; ?></td>
                            </tr>
                            <tr>
                              <td><b>Date of Birth : </b><?php echo $form_data[0]['dob']; ?></td>
                              <td><b>Gender : </b><?php echo $form_data[0]['DispGender']; ?></td>
                            </tr>
                            <tr>
                              <td><b>Mobile Number : </b><?php echo $form_data[0]['mobile_no']; ?></td>
                              <td><b>Alternate Mobile Number : </b><?php echo $form_data[0]['alt_mobile_no']; ?></td>
                            </tr>
                            <tr>
                              <td><b>Email id : </b><?php echo $form_data[0]['email_id']; ?></td>
                              <td><b>Alternate Email id : </b><?php echo $form_data[0]['alt_email_id']; ?></td>
                            </tr>
                            <tr>
                              <td colspan="2"><b>Qualification : </b><?php echo $form_data[0]['DispQualification']; ?></td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                    ...
                    <div class="row">
                      <div class="col-xl-12 col-lg-12">
                        <div class="alert alert-primary"><b>Below mentioned fields (i.e. Other Details) need to be filled / uploaded / edited carefully with appropriate contents till end of Day - 3 of the Training Period (i.e. by 11.59.59 PM of <?php echo date("d.m.Y", strtotime($chk_candidate_edit_date)); ?>).<br>No contents can be changed or edited after the Day - 3 (i.e. <?php echo date("d.m.Y", strtotime($chk_candidate_edit_date)); ?>) of the Training Period.</b></div>
                      </div>
                      ...
                      <div class="col-xl-6 col-lg-6"><?php /* Bank associated with */ ?>
                        <div class="form-group">
                          <label for="bank_associated" class="form_label">Bank associated with <sup class="text-danger"></sup></label>
                          <input type="text" name="bank_associated" id="bank_associated" value="<?php if($mode == "Add") { echo set_value('bank_associated'); } else { echo $form_data[0]['bank_associated']; } ?>" placeholder="Bank associated with" class="form-control custom_input allow_only_alphabets_and_space" maxlength="90"/>
                          <note class="form_note" id="bank_associated_err">Note: Please enter only 90 characters</note>
                      ...
                      <div class="col-xl-6 col-lg-6"><?php // Upload Proof of Identity ?>
                        <div class="form-group">
                          <div class="img_preview_input_outer pull-left">
                            ...
                            <?php /* <input type="file" name="id_proof_file" id="id_proof_file" class="form-control" accept=".png,.jpeg,.jpg" data-accept=".jpg,.jpeg,.pdf" onchange="file_validation_modify('id_proof_file'); show_preview(event, 'id_proof_file_preview'); validate_input('id_proof_file');" <?php if($mode == 'Add' || ($mode == 'Update' && $form_data[0]['id_proof_file'] == "")) { echo 'required'; } ?> />
                          ...
                          <div id="id_proof_file_preview" class="upload_img_preview pull-right">
                            <?php 
                              ...
                              { 
                                ...
                                $preview_training_id = set_value('hidden');
                              }
                              else if($mode == 'Update') 
                              { 
                            ...
                            { 
                              ...
                              { ?>
                              <a href="<?php echo base_url($id_proof_file_path.'/'.$preview_id_proof_file)."?".time(); ?>" class="example-image-link" data-lightbox="candidate_file" data-title="<?php echo $preview_first_name." (".$preview_training_id.")"; ?>">
                                <img src="<?php echo base_url($id_proof_file_path.'/'.$preview_id_proof_file)."?".time(); ?>">
                              </a>
                      ...
                      <div class="col-xl-6 col-lg-6"><?php // Upload Qualification Certificate ?>
                        <div class="form-group">
                          <div class="img_preview_input_outer pull-left">
                            ...
                            <?php /* <input type="file" name="qualification_certificate_file" id="qualification_certificate_file" class="form-control" accept=".png,.jpeg,.jpg" data-accept=".jpg,.jpeg,.pdf" onchange="file_validation_modify('qualification_certificate_file'); show_preview(event, 'qualification_certificate_file_preview'); validate_input('qualification_certificate_file');" <?php if($mode == 'Add' || ($mode == 'Update' && $form_data[0]['qualification_certificate_file'] == "")) { echo 'required'; } ?> />
                          ...
                          <div id="qualification_certificate_file_preview" class="upload_img_preview pull-right">
                            ...
                            { 
                              ...
                              { ?>
                              <a href="<?php echo base_url($qualification_certificate_file_path.'/'.$preview_qualification_certificate_file)."?".time(); ?>" class="example-image-link" data-lightbox="candidate_file" data-title="<?php echo $preview_first_name." (".$preview_training_id.")"; ?>">
                                <img src="<?php echo base_url($qualification_certificate_file_path.'/'.$preview_qualification_certificate_file)."?".time(); ?>">
                              </a>
                      ...
                      <div class="col-xl-6 col-lg-6"><?php // Upload Passport-size Photo ?>
                        <div class="form-group">
                          <div class="img_preview_input_outer pull-left">
                            ...
                            <?php /*<input type="file" name="candidate_photo" id="candidate_photo" class="form-control" accept=".png,.jpeg,.jpg" data-accept=".jpg,.jpeg,.pdf" onchange="file_validation_modify('candidate_photo'); show_preview(event, 'candidate_photo_preview'); validate_input('candidate_photo');" <?php if($mode == 'Add' || ($mode == 'Update' && $form_data[0]['candidate_photo'] == "")) { echo 'required'; } ?> />
                          ...
                          <div id="candidate_photo_preview" class="upload_img_preview pull-right">
                            ...
                            { 
                              ...
                              { ?>
                              <a href="<?php echo base_url($candidate_photo_path.'/'.$preview_candidate_photo)."?".time(); ?>" class="example-image-link" data-lightbox="candidate_file" data-title="<?php echo $preview_first_name." (".$preview_training_id.")"; ?>">
                                <img src="<?php echo base_url($candidate_photo_path.'/'.$preview_candidate_photo)."?".time(); ?>">
                              </a>
                      ...
                      <div class="col-xl-6 col-lg-6"><?php // Upload Signature of the Candidate ?>
                        <div class="form-group">
                          <div class="img_preview_input_outer pull-left">
                            ...
                            <?php /* <input type="file" name="candidate_sign" id="candidate_sign" class="form-control ignore_required" accept=".png,.jpeg,.jpg" data-accept=".jpg,.jpeg,.pdf" onchange="file_validation_modify('candidate_sign'); show_preview(event, 'candidate_sign_preview'); validate_input('candidate_sign');" <?php if($mode == 'Add' || ($mode == 'Update' && $form_data[0]['candidate_sign'] == "")) { echo 'required'; } ?> />
                          ...
                          { 
                            $preview_candidate_sign = set_value('candidate_sign_cropper'); 
                              ...
                              { ?>
                              <a href="<?php echo base_url($candidate_sign_path.'/'.$preview_candidate_sign)."?".time(); ?>" class="example-image-link" data-lightbox="candidate_file" data-title="<?php echo $preview_first_name." (".$preview_training_id.")"; ?>">
                                <img src="<?php echo base_url($candidate_sign_path.'/'.$preview_candidate_sign)."?".time(); ?>">
                              </a>
                    ...
                    <div class="row">
                      <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center" id="submit_btn_outer2">
                        ...
                        <a class="btn btn-danger" href="<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/index/'.url_encode($batch_data[0]['batch_id'])); ?>">Back</a>
                      </div>
                    </div>
                  </form>
                </div>
    ...
    <?php $this->load->view('iibfbcbf/common/inc_common_validation_all'); ?>
  	<?php $this->load->view('iibfbcbf/common/inc_cropper_script'); ?>
    ...
    <script type="text/javascript">
      ...
      function show_preview_modal()
      {
        ...
        $("#preview_bank_associated").html($("#bank_associated").val());
      ...
      function get_city_ajax(state_id)
      {
        ...
        {
          type: "POST",
          url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/get_city_ajax'); ?>",
          data: parameters,
          cache: false,
          dataType: 'JSON',
          async:false,
      ...
      $(document ).ready( function() 
      {
        ...
        $.validator.addMethod("validate_qualification_candidates", function(value, element)
        {
          ...
          else
          {
            ...
            {
              type: "POST",
              url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_qualification_candidates/0/1'); ?>",
              data: parameter,
              async: false,
              cache : false,
              dataType: 'JSON',
        ...
        var form = $("#add_candidate_form").validate( 
        {
          ...
          rules:
          {
            ...
            mobile_no:{ required: true, allow_only_numbers:true, first_zero_not_allowed:true, maxlength:10, minlength:10, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_mobile_exist/0/1'); ?>", type: "post", data: { "enc_candidate_id": function() { return "<?php echo $enc_candidate_id; ?>"; } } } },            
            alt_mobile_no:{ allow_only_numbers:true, first_zero_not_allowed:true, maxlength:10, minlength:10 },
            email_id:{ required: true, maxlength:80, valid_email:true, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_email_exist/0/1'); ?>", type: "post", data: { "enc_candidate_id": function() { return "<?php echo $enc_candidate_id; ?>"; } } } },
            ...
            pincode:{ required: true, allow_only_numbers:true, minlength:6, maxlength: 6, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_valid_pincode/0/1'); ?>", type: "post", data: { "selected_state_code": function() { return $("#state").val(); } } } },  //check validation for pincode as per selected state
            bank_associated:{ allow_only_alphabets_and_space:true, maxlength:90 }, 
            corporate_bc_associated:{ allow_only_alphabets_and_space:true, maxlength:90 }, 
            id_proof_type:{ required: true },  
            id_proof_number:{ validate_id_proof_number: true, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_id_proof_number_exist/0/1'); ?>", type: "post", data: { "enc_candidate_id": function() { return "<?php echo $enc_candidate_id; ?>"; } } } },
            ...
            aadhar_no:{ allow_only_numbers:true, maxlength:12, minlength:12, CheckAadharNumberWithIdProof: true, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_aadhar_no_exist/0/1'); ?>", type: "post", data: { "enc_candidate_id": function() { return "<?php echo $enc_candidate_id; ?>"; } } } },
          },
          messages:
          {
          ...
          submitHandler: function(form) 
          {
            ...
            else
            {
              ...
              { 
                ...
                {
                  $("#submit_btn_outer2").html('<input type="button" class="btn btn-primary" id="submitAll" name="submitAll" value="<?php if($mode == 'Add') { echo "Submit II"; } else { echo "Update Candidate"; } ?>" style="cursor:wait"> <a class="btn btn-danger" href="<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/index/'.url_encode($batch_data[0]['batch_id'])); ?>">Back</a>');
                }
                form.submit();
              });   
            }
      ...
      <?php if($mode == 'Add') { ?> if($("#id_proof_file_cropper").val() == "" && $("#id_proof_file_old").val() == "") { $("#id_proof_file").prop('required',true); }<?php }
        else if($mode == 'Update' && $form_data[0]['id_proof_file'] != "") { ?>$("#id_proof_file").prop('required',false);<?php } ?>
        
      <?php if($mode == 'Add') { ?> if($("#qualification_certificate_file_cropper").val() == "" && $("#qualification_certificate_file_old").val() == "") { $("#qualification_certificate_file").prop('required',true); }<?php }
        else if($mode == 'Update' && $form_data[0]['qualification_certificate_file'] != "") { ?>$("#qualification_certificate_file").prop('required',false);<?php } ?>
        
      <?php if($mode == 'Add') { ?> if($("#candidate_photo_cropper").val() == "" && $("#candidate_photo_old").val() == "") { $("#candidate_photo").prop('required',true); }
      <?php }
      else if($mode == 'Update' && $form_data[0]['candidate_photo'] != "") { ?> $("#candidate_photo").prop('required',false); <?php } ?>
        
      <?php if($mode == 'Add') { ?> if($("#candidate_sign_cropper").val() == "" && $("#candidate_sign_old").val() == "") { $("#candidate_sign").prop('required',true); }<?php }
        ...
        $("#pincode").rules("add", { required: true, allow_only_numbers:true, minlength:6, maxlength: 6, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_valid_pincode/0/1'); ?>", type: "post", data: { "selected_state_code": function() { return $("#state").val(); } } } });
        $("#bank_associated").rules("add", { allow_only_alphabets_and_space:true, maxlength:90 });
        ...
        $("#id_proof_number").rules("add", { validate_id_proof_number: true, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_id_proof_number_exist/0/1'); ?>", type: "post", data: { "enc_candidate_id": function() { return "<?php echo $enc_candidate_id; ?>"; } } } });
        
        var id_proof_file_required_flag = true;
      if($("#id_proof_file_cropper").val() != "" || $("#id_proof_file_old").val() != "") { id_proof_file_required_flag = false; }
        
        $("#id_proof_file").rules("add", 
        { 
          <?php if($mode == 'Add') { ?> required: id_proof_file_required_flag, <?php }
          else if($mode == 'Update' && $form_data[0]['id_proof_file'] == "") { ?>required: true,<?php } ?> 
        ...
        if($("#qualification_certificate_file_cropper").val() != "" || $("#qualification_certificate_file_old").val() != "") { qualification_certificate_file_required_flag = false; }
        $("#qualification_certificate_file").rules("add", 
        { 
          <?php if($mode == 'Add') { ?> required: qualification_certificate_file_required_flag, <?php }
          else if($mode == 'Update' && $form_data[0]['qualification_certificate_file'] == "") { ?>required: true,<?php } ?> 
          check_valid_file:true, 
          valid_file_format:'.jpg,.jpeg,.png',
          filesize_min:'75000', 
          filesize_max:'100000' 
        ...
        if($("#candidate_photo_cropper").val() != "" || $("#candidate_photo_old").val() != "") { candidate_photo_required_flag = false; }
        $("#candidate_photo").rules("add", 
        { 
          <?php if($mode == 'Add') { ?> required: candidate_photo_required_flag, <?php }
          else if($mode == 'Update' && $form_data[0]['candidate_photo'] == "") { ?>required: true,<?php } ?> 
          check_valid_file:true, 
          valid_file_format:'.jpg,.jpeg,.png', 
          filesize_min:'14000',
          filesize_max:'20000' 
        ...
        if($("#candidate_sign_cropper").val() != "" || $("#candidate_sign_old").val() != "") { candidate_sign_required_flag = false; }
        $("#candidate_sign").rules("add", 
        { 
          <?php if($mode == 'Add') { ?> required: candidate_sign_required_flag, <?php }
          else if($mode == 'Update' && $form_data[0]['candidate_sign'] == "") { ?>required: true,<?php } ?> 
          check_valid_file:true, 
          valid_file_format:'.jpg,.jpeg,.png', 
          filesize_min:'14000',
          filesize_max:'20000' 
        ...
        $("#aadhar_no").rules("add", { allow_only_numbers:true, maxlength:12, minlength:12, CheckAadharNumberWithIdProof: true, remote: { url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/validation_check_aadhar_no_exist/0/1'); ?>", type: "post", data: { "enc_candidate_id": function() { return "<?php echo $enc_candidate_id; ?>"; } } } });
      ...
      function file_validation_modify(file_input_name='') { $("#"+file_input_name+"_old").val(""); }
      
      <?php if(set_value('training_id_regnumber')!= "") { ?> show_hide_reset_btn(1); <?php } ?>
      
      function show_hide_reset_btn(show_hide_flag='')//1=>show, 2=>hide
      {
        if(show_hide_flag == '1')
        {
          $("#training_id_regnumber" ).prop("readonly", true);
          $("#reset_btn" ).show();
        }
        else if(show_hide_flag == '2')
        {
          $("#training_id_regnumber" ).prop("readonly", false);
          $("#reset_btn" ).hide();
        }
      }
      
      /***** START : Function to get the candidate details when 'Training ID or Registration Number' entered. Only applicable for centre  ***/
      function get_candidate_details()
      {      
        $("#add_candidate_form" ).validate().resetForm();
        var training_id_regnumber = $.trim($("#training_id_regnumber").val());
        if(training_id_regnumber =='')
        {
          $("#training_id_regnumber_err").html("Please enter the Training ID or Registration Number");
        }
        else
        {
          $("#training_id_regnumber_err").html("");
          $("#page_loader").show();
          parameters={ 'training_id_regnumber':training_id_regnumber, 'enc_batch_id':'<?php echo url_encode($batch_data[0]['batch_id']); ?>' } ;
          
          $.ajax(
          {
            type: "POST",
            url: "<?php echo site_url('iibfbcbf/agency/batch_candidates_agency/get_candidate_details_ajax'); ?>",
            data: parameters,
            cache: false,            
            dataType: 'JSON',
            //async:false,
            success:function(data)
            {
              if(data.flag == "success")
              {
                $("#old_candidate_id").val(data.response.candidate_id);
                $("#old_training_id").val(data.response.training_id);
                $("#old_regnumber").val(data.response.regnumber);
                $("#old_registration_type").val(data.response.registration_type);
                $("#parent_table").val(data.response.parent_table);
                
                if(data.response.salutation == 'Mr.' || data.response.salutation == 'Mrs.' || data.response.salutation == 'Ms.')
                {
                  $("#salutation").val(data.response.salutation);              
                }
                
                $("#first_name").val(data.response.first_name);
                $("#middle_name").val(data.response.middle_name);
                $("#last_name").val(data.response.last_name);
                $("#dob").val(data.response.dob);
                $('#dob').val(data.response.dob).datepicker("update");
                $("input[name='gender'][value='"+data.response.gender+"']").prop("checked", true);
                $("#mobile_no").val(data.response.mobile_no);
                $("#alt_mobile_no").val(data.response.alt_mobile_no);
                $("#email_id").val(data.response.email_id);
                $("#alt_email_id").val(data.response.alt_email_id);
                $("input[name='qualification'][value='"+data.response.qualification+"']").prop("checked", true);
                
                $("#address1").val(data.response.address1);
                $("#address2").val(data.response.address2);
                $("#address3").val(data.response.address3);
                $("#address4").val(data.response.address4);
                
                $(".chosen-select").chosen();
                $("#state").val(data.response.state);
                $("#state").trigger("chosen:updated");
                get_city_ajax(data.response.state)
                ...
                $("#bank_associated").val(data.response.bank_associated);
                $("#corporate_bc_associated").val(data.response.corporate_bc_associated);
                
                $("input[name='id_proof_type'][value='"+data.response.id_proof_type+"']").prop("checked", true);
                id_proof_number_validation('0');
                $("#id_proof_number").val(data.response.id_proof_number);
                $("input[name='qualification_certificate_type'][value='"+data.response.qualification_certificate_type+"']").prop("checked", true);
                $("#aadhar_no").val(data.response.aadhar_no);
                
                if($.trim(data.response.id_proof_file) != "")
                {
                  var id_proof_file_url = data.response.id_proof_file+"<?php echo "?".time(); ?>";
                  $('#id_proof_file_preview').html('<a href="'+id_proof_file_url+'" class="example-image-link" data-lightbox="candidate_file" data-title="'+data.response.first_name+' ('+data.response.training_id+')"><img src="'+id_proof_file_url+'"></a>');   
                  
                  $("#id_proof_file_old").val(data.response.id_proof_file); 
                $("#id_proof_file_cropper").val(''); 
                  $("#id_proof_file").rules("remove"); 
                  $("#id_proof_file").prop('required',false);
                }
                
                if($.trim(data.response.qualification_certificate_file) != "")
                {
                  var qualification_certificate_file_url = data.response.qualification_certificate_file+"<?php echo "?".time(); ?>";
                  $('#qualification_certificate_file_preview').html('<a href="'+qualification_certificate_file_url+'" class="example-image-link" data-lightbox="candidate_file" data-title="'+data.response.first_name+' ('+data.response.training_id+')"><img src="'+qualification_certificate_file_url+'"></a>');
                  
                  $("#qualification_certificate_file_old").val(data.response.qualification_certificate_file); 
                $("#qualification_certificate_file_cropper").val(''); 
                  $("#qualification_certificate_file").rules("remove"); 
                  $("#qualification_certificate_file").prop('required',false);
                }
                
                if($.trim(data.response.candidate_photo) != "")
                {
                  var candidate_photo_url = data.response.candidate_photo+"<?php echo "?".time(); ?>";
                  $('#candidate_photo_preview').html('<a href="'+candidate_photo_url+'" class="example-image-link" data-lightbox="candidate_file" data-title="'+data.response.first_name+' ('+data.response.training_id+')"><img src="'+candidate_photo_url+'"></a>');
                  
                  $("#candidate_photo_old").val(data.response.candidate_photo); 
                $("#candidate_photo_cropper").val(''); 
                  $("#candidate_photo").rules("remove"); 
                  $("#candidate_photo").prop('required',false);
                }
                
                if($.trim(data.response.candidate_sign) != "")
                {
                  var candidate_sign_url = data.response.candidate_sign+"<?php echo "?".time(); ?>";
                  $('#candidate_sign_preview').html('<a href="'+candidate_sign_url+'" class="example-image-link" data-lightbox="candidate_file" data-title="'+data.response.first_name+' ('+data.response.training_id+')"><img src="'+candidate_sign_url+'"></a>');   
                  
                  $("#candidate_sign_old").val(data.response.candidate_sign); 
                $("#candidate_sign_cropper").val('');  
                  $("#candidate_sign").rules("remove"); 
                  $("#candidate_sign").prop('required',false);
                }
                
                show_hide_reset_btn(1)
                $("#page_loader").hide();
              }
              else
              {
                $("#old_candidate_id").val("");
                $("#old_training_id").val("");
                $("#old_regnumber").val("");
                $("#old_registration_type").val("");
                $("#salutation").val("");
                $("#first_name").val("");
                $("#middle_name").val("");
                $("#last_name").val("");
                ...
                $("#email_id").val("");
                $("#alt_email_id").val("");
                $("input[name='qualification']").prop("checked", false);
                ...
                $("input[name='id_proof_type']").prop("checked", false);
                id_proof_number_validation('0');
                $("#id_proof_number").val("");
                $("input[name='qualification_certificate_type']").prop("checked", false);
                $("#aadhar_no").val("");
                
                $('#id_proof_file_preview').html('<i class="fa fa-picture-o" aria-hidden="true"></i>');
                $('#qualification_certificate_file_preview').html('<i class="fa fa-picture-o" aria-hidden="true"></i>');
                $('#candidate_photo_preview').html('<i class="fa fa-picture-o" aria-hidden="true"></i>');
                $('#candidate_sign_preview').html('<i class="fa fa-picture-o" aria-hidden="true"></i>');
                
                $("#id_proof_file_old").val(''); 
              $("#id_proof_file_cropper").val(''); 
                $("#id_proof_file").prop('required',true);
                
                $("#qualification_certificate_file_old").val(''); 
              $("#qualification_certificate_file_cropper").val(''); 
                $("#qualification_certificate_file").prop('required',true);
                
                $("#candidate_photo_old").val(''); 
              $("#candidate_photo_cropper").val(''); 
                $("#candidate_photo").prop('required',true);
                
                $("#candidate_sign_old").val(''); 
              $("#candidate_sign_cropper").val(''); 
                $("#candidate_sign").prop('required',true);
                
                show_hide_reset_btn(2);
      ...
      }/**** END : Function to get the candidate details when 'Training ID or Registration Number' entered. Only applicable for centre  ***/
    </script>
