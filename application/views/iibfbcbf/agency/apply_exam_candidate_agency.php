  ...
  <head>
        ...
        <?php $this->load->view('iibfbcbf/agency/inc_topbar_agency'); ?>
              ...
              <li class="breadcrumb-item active"> <strong>Candidate Details</strong></li>
				...
				<div class="wrapper wrapper-content animated fadeInRight">
                    ...
                    <a href="<?php echo site_url('iibfbcbf/agency/apply_exam_agency/candidate_listing/'.$enc_exam_code); ?>" class="btn btn-danger custom_right_add_new_btn">Back</a>
                  </div>
                </div>
                    ...
                    <div class="table-responsive">
                      <table class="table table-bordered custom_inner_tbl" style="width:100%">
                        <tbody>
                          ...
                          <?php 
                          $exam_date_eligible_exam_codes_arr = array(1037,1038);
                          if(count($subject_master) > 0 && in_array($exam_code,$exam_date_eligible_exam_codes_arr)) 
                          { ?>
                            <tr>
                              <td><strong>Examination Date</strong></td>
                              <td><?php echo date("d-M-Y", strtotime($subject_master[0]['exam_date'])); ?></td>
                            </tr>
                          <?php } ?>
                            ...
                            <td>                              
                              <select required class="form-control" name="exam_medium" id="exam_medium">
                                ...
                                if(count($medium_master) > 0)
                                {
                                  ...
                                  { ?>
                                    <option value="<?php echo $mediums['medium_code'];?>" <?php if($chk_medium_code == $mediums['medium_code']) { echo  'selected="selected"'; } ?>><?php echo $mediums['medium_description'];?>
                                      </option>
                                  <?php } 
                                } ?>
                              </select>
                          ...
                          <tr>
                            ...
                            <td>
                              <select required class="form-control" name="exam_centre" id="exam_centre" <?php if(in_array($exam_code,$venue_master_eligible_exam_codes_arr)) { ?> onchange="get_venue_details(this.value)" <?php } ?>>
                                ...
                                {      
                                  ...
                                  { ?>   
                                    <option value="<?php echo $centres['centre_code'];?>" <?php if($chk_centre_code == $centres['centre_code']){ echo 'selected="selected"'; } ?>><?php echo $centres['centre_name'];?>
                                    </option>
                                  <?php } 
                                } ?>
                              </select>
                          ...
                          <?php if(in_array($exam_code,$venue_master_eligible_exam_codes_arr)) 
                          { ?>
                            <tr>
                            ...
                            <tr>
                              ...
                              <td>
                                <div id="exam_date_outer">
                                  <select required class="form-control" name="exam_date" id="exam_date" onchange="get_venue_time_details(this.value)">
                            ...
                            <tr>
                              ...
                              <td>
                                <div id="exam_time_outer">
                                  <select required class="form-control" name="exam_time" id="exam_time" onchange="get_capacity_details(this.value)">
                      ...
                      <div class="text-center">                      
                        <a href="<?php echo site_url('iibfbcbf/agency/apply_exam_agency/candidate_listing/'.$enc_exam_code); ?>" class="btn btn-danger">Back</a>
                      </div>
                    </div>  
                  </form>
                </div>
    ...
    <script type="text/javascript">
      ...
      $(document ).ready( function() 
        ...
        $("#apply_exam_form").validate( 
          ...
          rules:
            ...
            if(submit_form_flag == 1)
            {
              let exam_fees = "<?php echo $exam_fees; ?>";
              if(exam_fees !="" && $.isNumeric(exam_fees) && exam_fees > 0)
              {
              ...
              {
                sweet_alert_error("Exam fee can not be zero(0)");
              } 
            }
            else
            {
      ...
      { ?> 
        ...
        if(selected_exam_centre != '') { get_venue_details(selected_exam_centre); }
        
        function get_venue_details(centre_code)
        {
          <?php 
            $selected_venue_name = '';
          ...
          {
            type: "POST",
            url: "<?php echo site_url('iibfbcbf/agency/apply_exam_agency/get_venue_details_ajax'); ?>",
            data: parameters,
            cache: false,
            dataType: 'JSON',
            success:function(data)
            {
              if(data.flag == "success")
              {
                ...
                if(selected_venue_code != '') { get_venue_date_details(selected_venue_code); }
          ...
          get_venue_date_details();
          get_venue_time_details();
          get_capacity_details();
        }
        
        function get_venue_date_details(venue_code)
        {
          <?php 
            $selected_venue_date = '';
          ...
          $("#page_loader").show();
          parameters= { "centre_code":$.trim($("#exam_centre").val()), "venue_code":venue_code, "exam_code":"<?php echo $exam_code; ?>", "exam_period":"<?php echo $exam_period; ?>", "selected_venue_date":"<?php echo $selected_venue_date; ?>" };

          $.ajax(
          {
            type: "POST",
            url: "<?php echo site_url('iibfbcbf/agency/apply_exam_agency/get_venue_date_details_ajax'); ?>",
            data: parameters,
            cache: false,
            dataType: 'JSON',
            success:function(data)
            {
              if(data.flag == "success")
              {
                ...
                if(selected_exam_date != '') { get_venue_time_details(selected_exam_date); }
        ...
        function get_venue_time_details(exam_date)
        {
          <?php 
            $selected_venue_time = '';
          ...
          $("#page_loader").show();
          parameters= { "centre_code":$.trim($("#exam_centre").val()), "venue_code":$.trim($("#venue_name").val()), "exam_date":exam_date, "exam_code":"<?php echo $exam_code; ?>", "exam_period":"<?php echo $exam_period; ?>", "selected_venue_time":"<?php echo $selected_venue_time; ?>" };

          $.ajax(
          {
            type: "POST",
            url: "<?php echo site_url('iibfbcbf/agency/apply_exam_agency/get_venue_time_details_ajax'); ?>",
            data: parameters,
            cache: false,
            dataType: 'JSON',
            success:function(data)
            {
              if(data.flag == "success")
              {
                ...
                if(selected_exam_time != '') { get_capacity_details(selected_exam_time); }
        ...
        function get_capacity_details(exam_time)
        {
          <?php 
            $chk_member_exam_id = '';
          ...
          {
            type: "POST",
            url: "<?php echo site_url('iibfbcbf/agency/apply_exam_agency/get_capacity_details_ajax'); ?>",
            data: parameters,
            cache: false,
            dataType: 'JSON',
            success:function(data)
