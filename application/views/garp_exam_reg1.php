...
if ($this->session->flashdata('error') != '') {?>
        ...
        <?php } ?>
        <form name="getDetailsForm" autocomplete="off" id="getDetailsForm" method="post" action="<?php echo base_url(); ?>Jaiib_centerchange/new_link">
          <br />
          <?php
...
?>" <?php if (isset($row['mem_mem_no'])) { echo "readonly='readonly'";} elseif (set_value('regnumber')) { echo "readonly='readonly'"; } ?> style="border-color:#000;" title="Membership No.">
        ...
        ?>
              <a href="<?php echo base_url();?>Jaiib_centerchange/new_link" class="btn btn-info" id="modify" style="height: 32px; width: 150px">Modify</a>
...
The Change of request If accepted, a revised Admit letter will be sent to the registered email id as well as will be available under the candidate’s login profile one week before the exam date.
            ...
            <div class="box-footer">
              ...
              <div class="col-sm-6 col-sm-offset-4">
                ...
                <a href="<?php echo base_url();?>Jaiib_centerchange/new_link" class="btn btn-default" >Reset</a> </div>
...
window.addEventListener('popstate', function(event) {
    window.location.assign(site_url+"Jaiib_centerchange/new_link");
});
...
$('#reload_captcha').click(function(event){
  ...
  $.ajax({
    type: 'POST',
    url: site_url+'Jaiib_centerchange/generatecaptchaajax/',
 ...
 if(exam_code != '0')
				{
					$.ajax({
						type: 'GET',
						url: site_url+'Jaiib_centerchange/center_data/',
						data: {exam_code:exam_code},
						success: function(data){
						//	$("#loader").hide();
						$('#new_center_code').html(data);
                    }
				   ...
				   $('#new_center_code').html('<option value="">- Select Center -</option>');
				}
