<!DOCTYPE html>
<html>
  <head> <?php $this->load->view('scribe_dashboard/includes/header');?> <style type="text/css">
      .box-header {
        background: #eaeaea;
        ;
      }
  ...
  <body class="hold-transition skin-blue sidebar-mini">
    <div class="wrapper"> <?php $this->load->view('scribe_dashboard/includes/topbar'); ?> <?php $this->load->view('scribe_dashboard/includes/sidebar'); ?> <div class="content-wrapper" style="min-kash: 946px;">
        <section class="content">
          <div id="custom_msg_outer"></div>
          <div class="hide d-none"> <?php /* echo @$paid_count_qry; */ ?> </div>
          <h4 class="title_common">Scribe Application List</h4>
          <!-- Flash Data : POOJA MANE : 24/08/2022 --> <?php if($this->session->flashdata('error')!=''){?> <div class="alert alert-danger alert-dismissible" id="error_id">
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> <?php echo $this->session->flashdata('error'); ?>
          </div> <?php } 
								
								if($this->session->flashdata('success')!=''){ ?> <div class="alert alert-success alert-dismissible" id="success_id">
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> <?php echo $this->session->flashdata('success'); ?>
          </div> <?php } ?>
                ...
                <form class="form-control" name="searchScribeDetails" id="searchScribeDetails" action="
                  ...
                  <div class="row">
                    <div class="col-md-2">
                      <div class="form-group">
                        ...
                        <input type="text" class="form-control custom_filter" name="from_date" id="from_date" value=''
													<?php if(set_value('from_date')) { echo set_value('from_date'); } else { echo $from_date; } ?>">
                      </div>
                    </div>
                    <div class="col-md-2">
                      <div class="form-group">
                        ...
                        <input type="text" class="form-control custom_filter" name="to_date" id="to_date" value=''
														<?php if(set_value('to_date')) { echo set_value('to_date'); } else { echo $to_date; } ?>">
                      </div>
                    </div>
                    <div class="col-md-2">
                      <div class="form-group">
                        <label>Applied Date</label>
                        <input type="text" class="form-control custom_filter" name="created_on" id="created_on" value=''
															<?php if(set_value('created_on')) { echo set_value('created_on'); }?>">
                      </div>
                    </div>
                    <div class="col-md-2">
                      <div class="form-group">
                        <label>Exam Name</label>
                        <input type="text" class="form-control custom_filter" name="exam_name" id="exam_name" value=''
																<?php if(set_value('exam_name')) { echo set_value('exam_name'); }?>">
                      </div>
                    </div>
                    <div class="col-md-1">
                      <div class="form-group">
                    ...
                    <div class="col-md-1">
                      <div class="form-group">
                        <label>Clear</label>
                        <input type="button" class="mb-2 float-right btn btn-primary" name="clear" id="clear" value="clear" onclick="location.href='
																		<?php echo base_url('scribe_dashboard/Scribe_list/scribe'); ?>';">
                      </div>
                    </div>
                    <div class="col-md-1">
                      <div class="form-group">
                      ...
                      <tbody class="no-bd-y" id="list2"> <?php 
                            ...
                            echo '
                                        <td>'.$res['mobile_scribe'].' </td>';
														...
														}elseif($res['scribe_approve'] == 3)
														{
															if($res['remark'] == 3)
															{
																$reuest_status = '
																				...
																				<span class="reuest_status" style="color: blue;">CANCELED</span>' ;
															}else{
															$reuest_status = '
														...
														/*echo '
																				<td>
																					<a class="btn btn-info btn-xs vbtn" href="'.base_url().'scribe_dashboard/Scribe_list/view'.$res['id'].'">View</a>';*/
														echo '
																					<td>
																						<a class="btn btn-primary btn-xs vbtn" href="'.base_url().'scribe_dashboard/Scribe_list/view/'.$res['id'].'">View</a>';
														echo '
																					</tr>';
														$k++;   
														}
      ...
      </div> <?php $this->load->view('scribe_dashboard/includes/footer');?>
      <!-- Data Tables -->
      <link href="
      ...
      <!----- FOR DATEPICKER ----->
      <script src="
																<?php echo base_url('assets/admin/plugins/datepicker/bootstrap-datepicker.js'); ?>">
      </script>
      <link rel="stylesheet" href="
																<?php echo base_url('assets/admin/plugins/datepicker/datepicker3.css'); ?>">
      <!-- FONT-AWESOME -->
...
</script> <?php if(isset($from_date) && $from_date != "") {   ?> <script>
  $('#to_date').datepicker({
        format: 'yyyy-mm-dd',
        startDate: ' < ? php echo $from_date; ? > ', endDate: ' + 0 d ', autoclose: true });
</script> <?php }

if(isset($to_date) && $to_date != "") { ?> <script>
  $('#from_date').datepicker({
        format: 'yyyy-mm-dd',
        endDate: ' < ? php echo $to_date; ? > ', autoclose: true });
</script> <?php } ?> <script src="
															<?php echo base_url('assets/js/jquery.validate.js'); ?>">
