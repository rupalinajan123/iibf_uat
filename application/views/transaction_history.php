                ...
                <?php if(count($transData)){
                    ...
                    <tr>
						...
						<?php if($row['transaction_no']) {?>
							<td>
                             <?php
							 	$explode_ename = explode("-",$row['description']);
								$arr = array('JAN','DEC','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','2019','2020','2021');
								
								foreach ($arr as $char) {
									$pos = 0;
									while ($pos = strpos($row['description'], $char, $pos)) {
										$positions[$char][] = $pos;
										$pos += strlen($char);
									}
								}
								
								$ex_name =  str_replace($arr, '', $row['description']);
								//echo preg_replace("/\([^)]+\)/","",$ex_name);
							  
							 	echo str_replace("\\'","",html_entity_decode($ex_name));
							 ?> </td>
						<?php } else{ ?>
							<td> <?php $exam_name = $this->master_model->getRecords('exam_master',array('exam_code'=>$row['exam_code']),'description');
						if(count($exam_name)){ 
						
							$explode_ename = explode("-",$exam_name[0]['description']);
							$arr = array('JAN','DEC','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','2019','2020','2021');
							
							foreach ($arr as $char) {
								$pos = 0;
								while ($pos = strpos($exam_name[0]['description'], $char, $pos)) {
									$positions[$char][] = $pos;
									$pos += strlen($char);
								}
							}
							
							$ex_name =  str_replace($arr, '', $exam_name[0]['description']);
							echo preg_replace("/\([^)]+\)/","",$ex_name);
