                ...
                <div class="table-responsive">
                  <form action="<?php echo base_url() ?>bulk/Bulk_exam_payment/make_neft/" method="post" class="form-horizontal" name="neft_pay_form" id="neft_pay_form">
					...
					?>
                          ...
                          <td width="55%">
						  	<?php
								...
								if(isset($row['member_type'])){
									...
									}elseif(($row['app_category']=='B1_1'|| $row['app_category']=='B3_1' || $row['app_category']=='B2_1') && $row['member_type']=='DB' && $row['bulk_discount_flg']=='N' && $row['elearning_flag']=='N'){
										echo 'DB member' ;
									}elseif(($row['app_category']=='B1_1'|| $row['app_category']=='B3_1' || $row['app_category']=='B2_1') && $row['member_type']=='DB' && $row['bulk_discount_flg']=='Y' && $row['elearning_flag']=='N'){
										echo 'DB member with discount' ;
									}elseif(($row['app_category']=='B1_1'|| $row['app_category']=='B3_1' || $row['app_category']=='B2_1') && $row['member_type']=='DB' && $row['bulk_discount_flg']=='N' && $row['elearning_flag']=='Y'){
										echo 'DB member with Elearning' ;
									}elseif(($row['app_category']=='B1_1'|| $row['app_category']=='B3_1' || $row['app_category']=='B2_1') && $row['member_type']=='DB' && $row['bulk_discount_flg']=='Y' && $row['elearning_flag']=='Y'){
                      ...
                      <?php }else if($this->session->userdata('institute_id') == "17171"){
                      	?>
                      	<tr>
                          <td width="10%"></td>
                          <td width="55%" style="text-align:right; font-weight:bold;"><?php
																				echo ' TDS Deduction (%) (if applicable)';
                      ...
                      <!-- Priyanka D >> start DBFMOUPAYMENTCHANGE >> 10-july-25 -->
                       <?php
                        if($this->session->userdata('mou_flg')==1 && $examinfo['exam_code']==420) { ?>  
                          ...
                          <a href="#"  class="w3-btn w3-black btn btn-warning previewproformainv" >Preview Proforma Invoice</a>
                          <input type="submit" value="Generate Proforma Invoice" name="Submit" id="Submit" style="margin-left: 1%;" class="w3-btn w3-black btn btn-success" >
                          <?php } else { ?>
...
//Priyanka D >> start DBFMOUPAYMENTCHANGE >> 10-july-25
   ...
   $('.previewproformainv').on('click', function() {
        var subtotal_after_tds = $('input[name="final_subtotal_after_tds"]').val(); // get input field value
        var tds_amt = $('input[name="tds_amt"]').val(); // get input field value
        var gst_rate_amt = $('input[name="gst_rate_amt"]').val(); // get input field value
        var base_fee_amt = $('input[name="base_amt_tot"]').val(); // get input field value
        var tax_type = $('input[name="tax_type"]').val(); // get input field value
        var dsct_rate_amt = $('input[name="base_amt_after_dsct"]').val(); // get input field value

        $.ajax({
            url: '<?php echo base_url(); ?>/bulk/Bulk_exam_payment/before_preview_profoma', // replace with your server endpoint
            type: 'POST',
            data: { subtotal_after_tds: subtotal_after_tds,tds_amt:tds_amt ,gst_rate_amt:gst_rate_amt,base_fee_amt:base_fee_amt,tax_type:tax_type,dsct_rate_amt:dsct_rate_amt},
            success: function(response) {
                if(response=='ok') {
                    window.open('<?php echo base_url() ?>bulk/Bulk_exam_payment/view_inst_pr_invoice', '_blank', 'noopener');
                }
            },
