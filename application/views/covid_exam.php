...
<div class="container">
  <section class="content-header box-header with-border" style="background-color: #1287C0; padding:16px 10px;">
    <h1 class="register" style="padding:0;">Request registration form.<span style='display: block;font-size: 16px;line-height: 18px; margin: 6px 0 0 0; '>Candidates affected with COVID/Election Duty on the exam dates during the exam time and could not take the exam(JAIIB/DBF/SOB Feb-Mar 2022 Examination)</span></h1>    
	</section>
  <div> 
    <!-- Start Get Details -->
		<?php
    ...
    <div class="row">
      <div class="col-md-12" style="">
        ...
        <?php
				...
				<?php } ?>
        <form name="getDetailsForm" id="getDetailsForm" method="post" action="<?php echo base_url(); ?>Covid">
          <br />
          <?php if(validation_errors() != ''){ ?>
          ...
          <div class="">
            ...
            <div class="col-sm-3" style="padding-bottom: 10px">
              <?php if (isset($row['regnumber']) || set_value('regnumber')) { ?>
								<a href="<?php echo base_url();?>Covid" class="btn btn-info" id="modify" style="height: 32px; width: 150px">Modify</a>
					...
					<div class="col-md-12">
						...
						<div class="box box-info">
                    		...
                    		<option value="">Select</option>
									...
									<div class="form-group">
										<label for="roleid" class="col-sm-4 control-label">Covid Certificate/Election Deputation Letter:<span style="color:#F00">*</span></label>
										<div class="col-sm-8">
											...
											<p style="line-height: 16px;margin: 3px 0 0 0;color: red;font-weight: 600;"><small>Upload the relevant COVID certificate duly attested by Bank Manager with Bank seal/stamp affixed on it, other than DB&F Candidates.<span style="display:block; margin-top:10px;">Note : Upload only pdf file having size less than 1MB</span></small></p>
										</div>
									</div>
							</div>
...
<script>
    ...
    $("#regnumber").focus();
		...
		window.Parsley.addValidator('bnqmobilecheck', function (value, requirement) {
			...
			$.ajax({
				url:site_url+'Covid/mobileduplication/',
				data: datastring,
				type:'POST',
				dataType:'json',
				async: false,
		...
		window.Parsley.addValidator('bnqemailcheck', function (value, requirement) {
			...
			$.ajax({
				url:site_url+'Covid/emailduplication/',
				data: datastring,
				type:'POST',
				dataType:'json',
				async: false,
