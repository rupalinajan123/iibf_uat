         ...
         if(count($exam_list) > 0)
          ...
          if($examrow['exam_code'] == "101" || $examrow['exam_code'] == "1046" || $examrow['exam_code'] == "1047"){
           ...
           if($examrow["exam_code"] != 75){
            ...
            //if($examrow['exam_code'] == 530 && $associatedinstitute != "" && in_array($associatedinstitute,$this->config->item('VALID_EXAMCODE_INSTITUTECODE_ARR'))){

            //}

            //print_r($examrow);//
            //echo $examrow["exam_period"];
            if($hide_exam_flag == 1){ 

                /*$get_ip_address = '';
                $get_ip_address = get_ip_address();
                //115.124.115.69
                if($examrow['exam_code'] == '1031'){  
                 $set_client_ip_address = $this->config->item('set_client_ip_address');
                 if(in_array($get_ip_address,$set_client_ip_address) && $examrow['exam_period'] == '323'){*/  
           ?> 
                  	<!-- <div class="form-group">
                    	   <img src="<?php echo base_url()?>assets/images/bullet2.gif"> <a href="<?php echo base_url();?>Home/examdetails/?excode2=<?php echo base64_encode($examrow['exam_code']);?>&Extype=<?php echo base64_encode($examrow['exam_type']);?>"> <?php echo $examrow['description'];//."==".get_ip_address();//."==".$associatedinstitute."==".$examrow['exam_code'];?></a>
           				 </div> -->
             	 <?php
                  /*}
                }else{*/
                  ?>
                  <div class="form-group">
                         <img src="<?php echo base_url()?>assets/images/bullet2.gif"> <a href="<?php echo base_url();?>Home/examdetails/?excode2=<?php echo base64_encode($examrow['exam_code']);?>&Extype=<?php echo base64_encode($examrow['exam_type']);?>"> <?php echo $examrow['description']." ".$exam_name_append." - ".$examrow['exam_code'];//."==".get_ip_address();//."==".$associatedinstitute."==".$examrow['exam_code'];?></a>
                   </div>
                  <?php
                //} 
