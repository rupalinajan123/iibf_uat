...
<body> 
 <table border="0" cellpadding="0" cellspacing="0" width="900px" align="center" style=" margin-bottom:10px;">
   ...
   <?php }else{?>
   <tr>
   <td style="text-align:center; padding:10px 0; font-size:16px; font-weight:bold; color:#b1b1b1;">
   <?php 
   	if($this->session->userdata('result_examcode') != 52){
   ?>
   Consolidate Mark Sheet - <?php echo preg_replace("/\([^)]+\)/","",$exam_name);?> Examination
   <?php }else{
	   echo "Consolidate Mark Sheet - Advanced Diploma in Urban Co-op Banking Examination";
   }?>
   </td>
   </tr>
   <tr>
...
Pin :<?php echo $user_info->pin_code?>
   ...
   <td style="border-bottom:1px solid #619fda; font-family: Tahoma, Geneva, sans-serif; font-size:12px;">
     <table border="0" cellpadding="0" cellspacing="0" width="100%" align="center">
       ...
       <tr>
       <td style="border-right:1px solid #28aae2; padding:5px 10px;">
	   <?php
	    if($this->session->userdata('result_examcode') != 52){
	    	echo preg_replace("/\([^)]+\)/","",$exam_name);
		}else{
			echo "Advanced Diploma in Urban Co-op Banking";	
		}
	   ?>
       </td>
        ...
        <td style="border: 1px #6699CC solid;" class="text21" align="center">
			<?php 
				if($result->exam_result_date == ''){
					...
					$hold2 = explode("-",$result->exam_result_date);
				if(count($hold2) == 3 && !is_numeric($hold2[1]))
				{
					$dateval= $hold2[0]."-".$hold2[1]."-".$hold2[2];
					echo date('d-M-Y',strtotime($dateval));
				}

				if(count($hold2) == 3 && is_numeric($hold2[1]))
				{
					$dateval= $hold2[2]."-".$hold2[1]."-".$hold2[0];
					echo date('d-M-Y',strtotime($dateval));
				}
				}
			?>
...
unfair practices being established.
   ...
   <table width="10%" align="center" border="0" cellpadding="0" cellspacing="0" style="padding:30px 0 0;">
                    <tr>
                      ...
                      <td>
                      <a href="<?php echo base_url();?>result/diplomacertification" style="background-color:#ddd; color:#000; text-align:center; padding:10px 15px; text-decoration:none;">Back</a>
                      </td>
                    </tr>
                  </table>
   </table>
   ...
   <table style="border-collapse:collapse;" width="98%" align="center">
		...
		<?php 
			if($result->exam_result_date == ''){
				...
				$hold1 = explode("-",$result->exam_result_date);
				if(count($hold1) == 3 && !is_numeric($hold1[1]))
				{
					$dateval= $hold1[0]."-".$hold1[1]."-".$hold1[2];
					echo date('d-M-Y',strtotime($dateval));
				}

				if(count($hold1) == 3 && is_numeric($hold1[1]))
				{
					$dateval= $hold1[2]."-".$hold1[1]."-".$hold1[0];
					echo date('d-M-Y',strtotime($dateval));
				}
			}
		?>
