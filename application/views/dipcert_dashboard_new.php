...
<body>
 <table border="0" cellpadding="0" cellspacing="0" width="900px" align="center" style=" margin-bottom:10px;">
  ...
  <!-- && $this->session->userdata('result_period') == 808-->
  	<ul>
    	<?php if($this->session->userdata('result_examcode') == 8 || $this->session->userdata('result_examcode') == 11 ||  $this->session->userdata('result_examcode') == 151 || $this->session->userdata('result_examcode') == 153 || $this->session->userdata('result_examcode') == 156 || $this->session->userdata('result_examcode') == 158 || $this->session->userdata('result_examcode') == $this->config->item('examCodeSOB') )  {?>
    	<li class="resultlink"><a href="<?php echo base_url()?>marksheet/dipcert_consolidated" target="_blank">Consolidated Marksheet</a></li>
        <?php }?>

        <li class="resultlink"><a href="<?php echo base_url()?>marksheet/dipcert_adviceresult" target="_blank">Result Advice</a></li>
