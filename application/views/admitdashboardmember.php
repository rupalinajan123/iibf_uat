            ...
            <td style="border:1px solid #1287c0; padding:5px;">
              <table cellpadding="0" cellspacing="0" width="100%" border="0" align="center">
                ...
                <tr>
                  ...
                  <?php
                  	foreach($exam_name as $exam_name){
						...
						$isvalid=$this->master_model->checkadmitcarddate($exam_name->exm_cd);
							if($isvalid)
							{
								$dates=$this->master_model->getRecords('admitcard_info',array('exm_cd'=>$exam_name->exm_cd,"mem_mem_no"=>$this->session->userdata('regnumber')),'date');
				  ...
				  ?>
                        <a href="<?php echo base_url()?>admitcard/getadmitcard/<?php echo base64_encode($exam_name->exm_cd)?>">
                        <?php
