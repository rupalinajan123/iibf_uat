...
<div class="container">
	<section class="content-header">
		<h1 class="register">DB&F to JAIIB Credit Transfer</h1><br />
	</section>
	<span class="error">
		<?php
						...
						<table class="table table-bordered">
							<thead>
								<tr>
									...
									<?php if(isset($oCandidate['regnumber'])){echo '<th>Registration No</th>';} ?>
									<th>Name</th>
									<th>Email Id</th>
									<th>Mobile No</th>
								</tr>
								...
								<tbody>
									<tr>
										...
										<?php if(isset($oCandidate['regnumber'])){echo '<td>'.$oCandidate['regnumber'].'</td>';} ?>
										<td><?php echo $oCandidate['firstname']; ?></td>
										<td><?php echo $oCandidate['email']; ?></td>
										<td><?php echo $oCandidate['mobile']; ?></td>
									</tr>
								</tbody>
					...
					<div class="box box-info">
						...
						<div class="box-body">
							...
							<div class="form-group">
								<label class="col-sm-4 control-label">Enter <?php if (!empty($oCandidate)) echo'DB&F'; ?> Membership Number<span style="color:#F00">*</span></label>
								<div class="col-sm-5">
									<input type="text" class="form-control" id="searchStr" name="searchStr" value="<?php echo set_value('searchStr'); ?>" required maxlength="15">
									<?php
									if($detailsNotMatched==1) {
										echo'<span class="error" style="color:red;">Details are not matched</span>';
									}
									?>
									<?php
									if($not_eligible==1) {
										echo'<span class="error" style="color:red;">You are not eligible for Credit Transfer</span>';
									}
									?>
								...
								<div class="col-sm-3">
									...
									&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?php echo base_url(); ?>/Dbftojaiibcredit/login/" >Reset Details</a>
								</div>
							</div>
						</div>
					</div>
...
<script>
	$(document).ready(function() {
		...
		$('.send_otp').on('click', function() {
			...
			$.ajax({
				url: '<?php echo base_url(); ?>Dbftojaiibcredit/send_otp',
				type: 'post',
				dataType: "json",
				data: {
					regnumber: regnumber,
		...
		$('.resend_otp').on('click', function() {
			...
			$.ajax({
				url: '<?php echo base_url(); ?>Dbftojaiibcredit/resend_otp',
				type: 'post',
				dataType: "json",
				data: {
					otp_id: otp_id,
		...
		$('.verify_otp_btn').on('click', function() {
			...
			var otp_id = $('.otp_id').val();
			$.ajax({
				url: '<?php echo base_url(); ?>Dbftojaiibcredit/verify_otp',
				type: 'post',
				dataType: "json",
				data: {
				...
				success: function(data) {
					if (data.status == 'success') {
						...
						setTimeout(function() {
							if (data.registerType == 'O') {
								window.location.href = "<?php echo base_url(); ?>Dbftojaiibcredit/login/<?php echo base64_encode($aCandidate['regnumber']); ?>";
							}
							else {
								window.location.href = "<?php echo base_url(); ?>Dbftojaiibcredit/candidate_details/";
							}
						}, 1000);
					} else {
