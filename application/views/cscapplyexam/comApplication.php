...
?>
  ...
  <div class="content-wrapper">
	...
	<form class="form-horizontal" name="member_conApplication" id="member_conApplication"  method="post"  enctype="multipart/form-data" action="<?php echo base_url();?>CSCApplyexam/comApplication/">
...
<?php }
 ...
 ?>
               ...
               <?php 
			    if(is_file(get_img_name($this->session->userdata('cscregnumber_applyexam'),'p')))
				{?>
                     <img src="<?php echo base_url();?><?php echo get_img_name($this->session->userdata('cscregnumber_applyexam'),'p');?><?php echo '?'.time(); ?>" height="100" width="100" >
				<?php 
                }
                else
                {?>
                ...
                <label for="roleid" class="col-sm-3 control-label">Signature</label>
                	<div class="col-sm-2">
                     <label for="roleid" class="col-sm-3 control-label">
                 <?php 
			   	if(is_file(get_img_name($this->session->userdata('cscregnumber_applyexam'),'s')))
				{?>
                     <img src="<?php echo base_url();?><?php echo get_img_name($this->session->userdata('cscregnumber_applyexam'),'s');?><?php echo '?'.time(); ?>" height="100" width="100">
                   <?php 
				}
				else
				{?>
			...
			{*/
			if(!is_file(get_img_name($this->session->userdata('cscregnumber_applyexam'),'s')) || !is_file(get_img_name($this->session->userdata('cscregnumber_applyexam'),'p')))
			{?>
            	   <!--<div class="form-group">
				...
				<option value="">--Select--</option>				
                    ...
                    <label for="roleid" class="col-sm-3 control-label">Do you intend to use <br />the services of a scribe ?<span style="color:#F00">*</span> </label> 
                        <div class="col-sm-3">
                           ...
                           <input type="radio" name="scribe_flag" id="scribe_flag" value="Y" onclick="showSelect_scribe_flagY();" disabled="disabled">YES
             ...
             <div class="box-footer">
                  <div class="col-sm-4 col-xs-offset-3">
                   ...
                   <!-- <input type="submit" class="btn btn-info" name="btnSubmit" id="btnSubmit" value="Save">-->
                     ...
                     <a href="<?php echo base_url();?>CSCApplyexam/examdetails/?ExId=<?php echo base64_encode($this->session->userdata('examcode'));?>" class="btn btn-info" id="preview">Back</a>
                    </div>
              </div>
             </div>
     </div>
...
//$('#Sub_menue').removeAttr("required")
 ...
 var base_url = '<?php echo base_url();?>'
   function getsub_menue(deptid)
   {
   			$.ajax({
   		type:"POST",
   		url: base_url+"CSCApplyexam/getsub_menue",
   		data:{deptid:deptid},
