...
<html>
  <head>
                  ...
                  <form method="post" style="display: inline-block;	vertical-align: top; margin: -5px 0 0 10px; width:99%;">  
                        ...
                        <tr>
                          ...
                          <?php foreach($center_master_data as $center) {
                            ?>
                              <option style="display:none;" exam_period="<?php echo $center['exam_period'] ?>"  exam_code="<?php echo $center['exam_name'] ?>" <?php if($form_data[0]['center_code'] == $center['center_code'] && $form_data[0]['exam_period'] == $center['exam_period']) echo'selected' ?> value="<?php echo $center['center_code'] ?>"><?php echo $center['center_name'] ?></option>
                            <?php
                          } ?>
                          </select>
                        </td>
    ...
    function exam_change_func() {
      var exam_code_period = $('select.select_exam option:selected').attr('value').split("-");
      $(".select_center option").each(function() {
        if($(this).attr('exam_code')==exam_code_period[0] && $(this).attr('exam_period')==exam_code_period[1])
          $(this).show();
        else
        $(this).hide();
    ...
    $('.submit_candidate_details').click( function() {
      ...
      {
        var exam_code_period = $("select[name='exam_code_period']").val();
        ...
        if(status_value=='' || role_id=='' || exam_code_period=='' || center_code=='') {  // 
        ...
        if(enc_id != "")
        {
          ...
          { 
            ...
            $.ajax(
            {
              ...
              data: {enc_id:enc_id,status_value:status_value,role_id:role_id,exam_code_period:exam_code_period,center_code:center_code},
              /*async: false,
              cache : false,*/
              dataType: 'JSON',
