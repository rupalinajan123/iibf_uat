	 ...
	 if(count($exam_list) > 0)
	 {
	  ...
	  {?>
              <div class="form-group has-feedback">
				...
				{?>
                    <a title="<?php echo $row['description']." - ".$row['exam_code']; ?>" href="<?php echo base_url();?>nonreg/memlogin/?Extype=<?php echo $Extype;?>&Mtype=<?php echo $Mtype;?>&ExId=<?php echo base64_encode($row['exam_code']);?>"><?php echo $row['description'];?>
                    </a>
              <?php 
				}?>
              </div>
<?php 
  ...
  foreach($exam_list_bcbf as $row)
	  { ?>
              <div class="form-group has-feedback">
    		    <?php echo $i?>:  <a title="<?php echo $row['description']." - ".$row['exam_code']; ?>" href="<?php echo site_url('iibfbcbf/apply_exam_individual'); ?>"><?php echo $row['description'];?></a>
              </div>
<?php 
	$i++;}
