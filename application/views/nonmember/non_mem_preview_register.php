...
<div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <a  href="<?php echo base_url('nonreg/member/?Mtype='.base64_encode($member_type).'&ExId='.base64_encode($exam_code).'&is_verified='.base64_encode('1')) ?>?>">Modify</a>
      </h1>
      <br>
      <!--<ol class="breadcrumb">
    ...
    action="<?php echo base_url()?>Nonreg/addmember/">
    <section class="content">
      <div class="row">
        ...
        <?php if( $exam_code == 1009 && $member_type == 'NM' ) { ?>

        <!-- Add field by gaurav 8th may -->
        <?php }if( $exam_code == 1009 && $member_type == 'NM' ) { ?>
        <div class="form-group">
            ...
            <?php if(count($institution_master))
                {
                    ...
                    {   
                        if($this->session->userdata['enduserinfo']['institution']==$institution_row['institude_id'])
                        {
        ...
        <!-- Add field by gaurav 8th may -->
        <?php }
            if(($exam_code == 101 || $exam_code == 1046 || $exam_code == 1047 || $exam_code == 991 || $exam_code == 997) && $member_type == 'NM' ) { ?> 
        <div class="form-group">
            <label for="roleid" class="col-sm-3 control-label">Date of commencement of operations/joining as BC</label>
            <div class="col-sm-3">
                <?php echo date("d-m-Y",strtotime($this->session->userdata['enduserinfo']['doj'])); ?>
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <label for="roleid" class="col-sm-3 control-label">Uploaded Bank BC ID Card</label>
        ...
        <div class="form-group">
        <label for="roleid" class="col-sm-3 control-label"><?php echo (($exam_code == 101 || $exam_code == 1046 || $exam_code == 1047) ? 'Bank BC ID No' : 'Bank Employee Id'); ?></label>
        <div class="col-sm-5">
        ...
        <label for="roleid" class="col-sm-2 control-label"><img src="<?php echo $this->session->userdata['enduserinfo']['scannedsignaturephoto'];?>" height="100" width="100"></label>
        
        <label for="roleid" class="col-sm-2 control-label"><img src="<?php echo $this->session->userdata['enduserinfo']['idproofphoto'];?>"  height="100" width="100"></label>
        <?php if( ($exam_code == 1009) && $member_type == 'NM' ) { ?>
            <label for="roleid" class="col-sm-2 control-label"><img src="<?php echo $this->session->userdata['enduserinfo']['empidproofphoto'];?>"  height="100" width="100"></label>
        <?php }
            if( $exam_code == 1009 && $member_type == 'NM' ) { ?> 
            <label for="roleid" class="col-sm-2 control-label"><img src="<?php echo $this->session->userdata['enduserinfo']['declarationform'];?>"  height="100" width="100"></label>     
        <?php } ?>
       </div>  
                
        <div class="form-group">
            <label for="roleid" class="col-sm-3 control-label">Uploaded Photo</label>
            <label for="roleid" class="col-sm-2 control-label">uploaded Signature</label>
            <label for="roleid" class="col-sm-2 control-label">Uploaded ID Proof</label>
            <?php if( ($exam_code == 1009) && $member_type == 'NM' ) { ?>
                <label for="roleid" class="col-sm-2 control-label">Uploaded Employee ID Proof</label> 
            <?php } ?>
            <?php if( $exam_code == 1009 && $member_type == 'NM' ) { ?> 
                <label for="roleid" class="col-sm-2 control-label">Uploaded Declaration Form</label>
            <?php } ?>
        </div>
        ...
        <label for="roleid" class="col-sm-10 control-label">I agree to receive the Annual report from the Institute in a softcopy, at my registered email ID, in place of physical copy</label>
            <div class="col-sm-2">
               <input value="Y" name="optnletter" id="optnletter"  type="radio"  <?php //if($this->session->userdata['enduserinfo']['optnletter']=='Y'){echo  'checked="checked"';}else{echo 'disabled="disabled"';}?> >Yes
                <input value="N" name="optnletter" id="optnletter" type="radio"  <?php //if($this->session->userdata['enduserinfo']['optnletter']=='N'){echo  'checked="checked"';}else{echo 'disabled="disabled"';}?>>No
       ...
       <!---->
        ...
        <div class="box-body">
           ...
           <?php
              ...
              <?php if(!in_array($exam_code,$this->config->item('skippedAdmitCardForExams'))) { ?>
              <label for="roleid" class="col-sm-5 text-center"><strong class="black_clr">Venue</strong></label>
              <?php } ?>
              <label for="roleid" class="col-sm-2 text-center"><strong class="black_clr">Date</strong></label>
              <?php if(!in_array($exam_code,$this->config->item('skippedAdmitCardForExams'))) { ?>
              <label for="roleid" class="col-sm-2 text-center"><strong class="black_clr">Time</strong></label>
              <?php } ?>  
            </div>
            <?php }?>
			<br />
            <?php 
				   if(count($compulsory_subjects) > 0 && $this->session->userdata['enduserinfo']['excd']!=101 && $this->session->userdata['enduserinfo']['excd']!=1046 && $this->session->userdata['enduserinfo']['excd']!=1047) 
				   {
					   $i=1;
					   foreach($compulsory_subjects as $k=>$v)
					   {
						  $venue_add_finalstring='';
						 $get_venue_details=$this->master_model->getRecords('venue_master',array('venue_code'=>$v['venue'],'exam_date'=>$v['date'],'session_time'=>$v['session_time'],'center_code'=>$this->session->userdata['enduserinfo']['selCenterName']));
            ...
            <div class="form-group">
              ...
              <?php if(!in_array($exam_code,$this->config->item('skippedAdmitCardForExams'))) { ?>
              <span for="roleid" class="col-sm-5"><?php echo $venue_add_finalstring; ?></span>
              <?php } ?> 
              <span for="roleid" class="col-sm-2" style="text-align: right"><?php echo date('d-M-Y',strtotime($v['date']));?></span>
              <?php if(!in_array($exam_code,$this->config->item('skippedAdmitCardForExams'))) { ?>
              <span for="roleid" class="col-sm-2" style="text-align: right"><?php echo $v['session_time']; ?></span>
              <?php } ?> 
            </div>
          ...
          <?php 
            ...
            <!--<div class="form-group">
                <label for="roleid" class="col-sm-3 control-label">GSTIN No.</label>
                  ...
                  <?php echo $this->session->userdata['enduserinfo']['gstin_no'];?>
                    </div>
                </div>-->
