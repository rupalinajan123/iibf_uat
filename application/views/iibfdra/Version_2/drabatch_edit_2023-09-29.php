...
.datepicker table tr td.disabled, .datepicker table tr td.disabled:hover { cursor: not-allowed; }
						...
						<div class="box-body">
										...
										foreach($agency_center as $val)
										{
											...
											<?php if($val['center_validity_from'] == '' && $val['center_validity_to'] == '' && $val['pay_status']==1){
													...
													<?php }elseif($val['center_status'] == 'A' && $val['pay_status'] == 1 && $val['center_validity_to'] > date('Y-m-d') && $val['center_validity_from'] <= date('Y-m-d'))
													{
														...
														<?php }elseif($val['center_validity_from'] > date('Y-m-d'))
														{?>
														<option value="<?php echo $val['center_id']?>" disabled="disabled"><?php echo $val['location_name'].' ( Your Accreditation period is not started. )'; ?></option><?php } ?>
											<?php } 
							...
							<div class="form-group">
								...
								<div class="col-sm-3">
									...
									<span class="note">Note: Please Select From Date greater than <?php echo $date_check; ?></span></br>
                        ...
                        <div class="table-responsive">
                          <table id="listitems_logs" class="table table-bordered table-striped">
                            ...
                            <tbody class="no-bd-y" id="list222">
                              ...
                              <tr>
                                <td><?php echo $k; ?></td>
                                <td><?php echo str_replace("DRA Admin","",$res_log['title']).' '.$Accridation_text.' -  '.$update_by; ?></td>
...
<script type="text/javascript">	
    ...
    function calc_candidates(){
     ...
     function form_submit(){
       ...
       // alert(e);
        ...
        if($("#draExamAddFrm").parsley().isValid() && grossTime_err == 0 && validateDoc_err == 0 && totalBrk_err == 0 && netTime_err == 0 && totalnetTime_err == 0 && grossDays_err == 0){
            //console.log('if');
            //$( "#draExamAddFrm" )[0].submit();
             return true;
        }
	...
	$(document).ready(function() {
		...
		$('#batch_from').datepicker({
			format: "yyyy-mm-dd",
      startDate: '<?php echo date('Y-m-d', strtotime("+1days", strtotime($date_check))); ?>',
      ...
      //clearBtn: true    
		...
		$('#batch_from').change(function() {
			...
			var date2 = $('#batch_from').datepicker('getDate', '+3d'); 
