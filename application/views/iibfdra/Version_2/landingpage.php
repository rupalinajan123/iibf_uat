...
<?php
$this->load->view('iibfdra/Version_2/front-header'); ?>
                                ...
                                if(count($exams) > 0) {
                                    ...
                                    foreach($exams as $exam) { 
                                        ...
                                        if( strtotime($comp_currdate) >= strtotime($comp_frmdate) && strtotime($comp_currdate) <= strtotime($comp_todate) ) { //display dra exam link only when any one exam is active
												...
												if( $count == 0 || $flg != 1 ) { ?>
													<h3 style="margin-top:0px;"><a href="<?php echo base_url().'iibfdra/Version_2/InstituteLogin'?>"><span style="color:#F00">DRA Exam - <span style="font-size:18px;">Click here for registration</span> </span></a></h3>
												<?php } $flg = 1; 
										} //Display exam details irrespective of it is active or not
										//exam date added on 01-02-2017
										
										elseif($count == 0){ ?>
										<h3 style="margin-top:0px;"><a href="<?php echo base_url().'iibfdra/Version_2/InstituteLogin'?>"><span style="color:#F00">DRA Exam - <span style="font-size:18px;">Click here for login</span> </span></a></h3>
...
<?php $this->load->view('iibfdra/Version_2/front-footer');?>