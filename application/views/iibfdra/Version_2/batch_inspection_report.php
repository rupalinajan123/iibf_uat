<?php //$this->load->view('iibfdra/header');?>
<?php //$this->load->view('iibfdra/sidebar');?>
...
<div class="content-wrapper"> 
  ...
  <section class="content">
    ...
    <form method="POST" action="<?php echo base_url('iibfdra/Version_2/InspectorHome/export_to_pdf'); ?>">
    <div class="row">
      <div class="col-md-12">
        <div class="box box-info box-solid disabled">
          <div class="box-header with-border">
            <h3 class="box-title">Select Batch for Inspections</h3>
            <div class="box-body">
              <div class="row">
                <div class="col-md-10">
                  <select class="form-control" name="batch_id" id="batch_id">
                    ...
                    ?>
                        <option value="<?php echo $value['id']; ?>" agency-id-attr="<?php echo $value['agency_id']; ?>" agency-name-attr="<?php echo $value['institute_name']; ?>" batch-code-attr="<?php echo $value['batch_code']; ?>"  batch-duration-attr="<?php echo $value['batch_from_date'].' To '.$value['batch_to_date']; ?>" batch_online_offline_flag-attr="<?php echo $value['batch_online_offline_flag']; ?>"  batch-platform-attr="<?php echo $value['online_training_platform']; ?>" batch-time-attr="<?php echo $value['hours'].' Hours'; ?>" daily-batch-timing="<?php echo $value['timing_from'].' To '.$value['timing_to']; ?>" candidate-count-attr="<?php echo $value['total_candidates']; ?>" batch-medium-attr="<?php echo $value['training_medium']; ?>" assigned-facultyid-attr1="<?php echo $value['first_faculty_id']; ?>" assigned-facultyphoto-attr1="<?php echo $first_faculty_photo_url; ?>" assigned-faculty-attr1="<?php echo $value['first_faculty_code'].'_'.$value['first_faculty_name']; ?>" assigned-facultyid-attr2="<?php echo $value['sec_faculty_id']; ?>" assigned-facultyphoto-attr2="<?php echo $sec_faculty_photo_url; ?>" assigned-faculty-attr2="<?php echo $value['sec_faculty_code'].'_'.$value['sec_faculty_name']; ?>" additional-facultyid-attr1="<?php echo $value['add_first_faculty_id']; ?>" additional-facultyphoto-attr1="<?php echo $add_first_faculty_photo_url; ?>" additional-faculty-attr1="<?php echo $value['add_first_faculty_code'].'_'.$value['add_first_faculty_name']; ?>" additional-facultyid-attr2="<?php echo $value['add_sec_faculty_id']; ?>" additional-facultyphoto-attr2="<?php echo $add_sec_faculty_photo_url; ?>" additional-faculty-attr2="<?php echo $value['add_sec_faculty_code'].'_'.$value['add_sec_faculty_name']; ?>" contact-person-attr="<?php echo $value['contact_person_name'].' ('. $value['contact_person_phone'].')';?>" alt-contact-person-attr="<?php echo $value['alt_contact_person_name'].' ('. $value['alt_contact_person_phone'].')';?>" platform-attr="<?php echo $value['platform_link'] ?>" document-attr="<?php echo $value['training_schedule'] ?>"><?php echo $value['batch_code'].", (".$value['hours']." Hours, ".date("d/m/Y", strtotime($value['batch_from_date']))." To ".date("d/m/Y", strtotime($value['batch_to_date'])).", ".$value['timing_from']." To ".$value['timing_to'].", ".$value['training_medium'].")".' ('.$value['short_inst_name'].')'.' ('.$value['inspector_name'].')'. ' Reported Count ('.$value['reported'].') '.$overall_compliance_str; ?></option>
                      <?php }
                    ?>
                  </select>
                </div>
      ...
      <table class="table table-bordered table-striped" >
        <tbody>
          ...
          <tr>
            ...
            <td width="2%"></td>
            <td width="29%"><strong>Total No. of Candidates (Estimated):</strong></td>
          ...
          <?php /*?><tr>
            ...
            <td width="20%" ><?php echo $inspector_name.'/'.$inspector_id; ?>
          </tr></td><?php */?>
              ...
              <table id="listitems" class="table table-bordered table-striped">
                <thead>
                  <tr>
                    ...
                    <th width="20">Candidate Status</th>
                    <th width="5">DOB</th>
...
$(function () {
  ...
  $('#batch_id').on('change',function () {
    ...
    if(batch_id != '') 
    {
      ...
      if (assigned_faculty1 != '_') {
        $('#assigned_faculty1').attr('href',assigned_facultyphoto1);  
      } 

      if (assigned_faculty2 != '_') {
        $('#assigned_faculty2').attr('href',assigned_facultyphoto2);  
      } 

      if (additional_faculty1 != '_') {
        $('#additional_faculty1').attr('href',additional_facultyphoto1);  
      } 

      if (additional_faculty2 != '_') {
        $('#additional_faculty2').attr('href',additional_facultyphoto2);  
      }  
      ...
      //alert(batch_online_offline_flag);
      ...
      $.ajax({
          url:site_url+'iibfdra/Version_2/InspectorHome/get_data',
          ...
          success: function(res) {
            ...
            if(data != '')
            {
              ...
              $.each(JSON.parse(data), function(idx, obj) {
                ...
                //var view = '<a href="'+site_url+"iibfdra/admin/inspectionSummary/details/"+btoa(obj.regid)+'" >details </a>'
                ...
                console.log(obj.absent_cnt);
                values = [[j, obj.training_id, obj.name, obj.hold_release, obj.dateofbirth, idproof, photo, obj.photo_verify, obj.photo_remark, obj.present_cnt, obj.absent_cnt, obj.remark,quali,obj.qualification_verify,obj.qualification_remark]];
