...
<style type="text/css">
 ...
 <?php 
 				$enc_batch_id = base64_encode($result['id']);
      ...
      <div class="col-md-12">
        <div class="box box-info box-solid disabled">
          <div class="box-header with-border">
            ...
            <div class="pull-right">
            	<a href="<?php echo base_url();?>iibfdra/Version_2/batch/" class="btn btn-warning">Back</a>
          ...
          <div class="box-body" style="display: block;">
	   			...
	   			if($result['batch_status'] == 'Approved' ){ 
					...
					if(isset($is_applied)){
						if( (date('Y-m-d',strtotime("+6 day", strtotime($result['batch_to_date']))) > date('Y-m-d'))){
						    ...
						    if($is_applied == 0 ){
								$str_btn = '<textarea name="rejection" maxlength="1000" class="reason" rows="5" cols="40" placeholder="Describe cancel reason here"></textarea></br><a class="cancel_batch btn btn-danger " dataid ="'.$result['id'].'" href="javascript:void(0);">Cancel Batch</a>';	
							}else{
							$str_btn = '<a class="btn btn-danger" disabled="disabled" dataid ="'.$result['id'].'" href="javascript:void(0);">Cancel Batch</a> <br> <span class="act_msg"> Some Candidate already applied for exam </span>';	
								//$str
							}
					...
					$div_class2 = '#9900cc';
						...
						$str_btn = '<textarea name="rejection" maxlength="1000" class="reason" rows="5" cols="40" placeholder="Describe rejection reason here"></textarea> <a class="approve_batch btn btn-success " dataid ="'.$result['id'].'" href="javascript:void(0);">Go Ahead</a>&nbsp; &nbsp; &nbsp;<a class="reject_batch btn btn-danger " dataid ="'.$result['id'].'" href="javascript:void(0);">Reject</a> &nbsp; &nbsp;';
						$div_class = '#f8d7da';
						$div_class2 = '#cca300';
			   		}*/
			   		...
			   		elseif($result['batch_status'] == 'UnHold' ){ 
				   		...
				   		$str_btn = '<textarea name="rejection" maxlength="1000" class="reason" rows="5" cols="40" placeholder="Describe Cancellation reason here"></textarea> <a class="cancel_batch btn btn-danger " dataid ="'.$result['id'].'" href="javascript:void(0);">Cancel Batch</a> &nbsp; &nbsp;';
						...
						$str_btn = '<textarea name="rejection" maxlength="1000" class="reason" rows="5" cols="40" placeholder="Describe rejection reason here"></textarea> <a class="approve_batch btn btn-success " dataid ="'.$result['id'].'" href="javascript:void(0);">Go Ahead</a>&nbsp; &nbsp; &nbsp;<a class="batch_error btn" style="background-color:#FF8C00" dataid ="'.$result['id'].'" href="javascript:void(0);">Batch Error</a>&nbsp; &nbsp; &nbsp;<a class="reject_batch btn btn-danger " dataid ="'.$result['id'].'" href="javascript:void(0);">Reject</a> &nbsp; &nbsp;';
			...
			?>
             <form method="post" name="appfrom" id="approve_from"  enctype="multipart/form-data">
              ...
              <div class="table-responsive ">
                <table class="table table-bordered table-striped" style="word-wrap:anywhere; background:<?php echo $div_class; ?>;">
                  <tbody>
                    ...
                    <tr>
                      <td width="50%"><strong>1st Faculty :</strong></td>
                      <td width="50%">
												<?php  if($result['first_faculty_name'] != ''){ ?>
						   					<a href="<?php echo base_url();?>iibfdra/Version_2/admin/faculty_master/faculty_view/<?php echo base64_encode($result['first_faculty']); ?>" target="_blank">	<?php echo $result['first_faculty_code'].'_'.$result['first_faculty_name']; ?> </a>
	                      <?php } else { echo '--'; } ?>
                       </td>
                    </tr>
                    
                     <tr>
                      <td width="50%"><strong>2nd Faculty :</strong></td>
                      <td width="50%">
												<?php  if($result['sec_faculty_name'] != ''){ ?>
												<a href="<?php echo base_url();?>iibfdra/Version_2/admin/faculty_master/faculty_view/<?php echo base64_encode($result['sec_faculty']); ?>" target="_blank"> <?php echo $result['sec_faculty_code'].'_'. $result['sec_faculty_name']; ?>
	                      <?php } else { echo '--'; } ?>
                      </td>
                    </tr>

                    <tr>
                      <td width="50%"><strong>Additional Faculty I :</strong></td>
                      <td width="50%">
												<?php  if($result['add_first_faculty_name'] != '') { ?>
												<a href="<?php echo base_url();?>iibfdra/Version_2/admin/faculty_master/faculty_view/<?php echo base64_encode($result['additional_first_faculty']); ?>" target="_blank"> <?php echo $result['add_first_faculty_code'].'_'.$result['add_first_faculty_name']; ?> </a>
	                      <?php } else { echo '--'; } ?>
                      </td>
                    </tr>
                    
                     <tr>
                      <td width="50%"><strong>Additional Faculty II :</strong></td>
                      <td width="50%">
												<?php  if($result['add_sec_faculty_name'] != '') { ?>
					   						<a href="<?php echo base_url();?>iibfdra/Version_2/admin/faculty_master/faculty_view/<?php echo base64_encode($result['additional_sec_faculty']); ?>" target="_blank"> <?php echo $result['add_sec_faculty_code'].'_'.$result['add_sec_faculty_name']; ?> </a>
                       	<?php } else { echo '--';} ?>
                      </td>
                    </tr>
              ...
              <?php
			          $breakTime1 = '';
                if ($result['brk_from_time1'] != '' && $result['brk_to_time1'] != '') {
                  $breakTime1 = $result['brk_from_time1'].' to '.$result['brk_to_time1'];
                }
			        ?>
            	<td width="50%"><strong>Break Time 1:</strong></td>
            	<td width="50%"><?php echo $breakTime1; ?></td>
            </tr>

            <tr>
              <?php
			          $breakTime2 = '';
                if ($result['brk_from_time2'] != '' && $result['brk_to_time2'] != '') {
                  $breakTime2 = $result['brk_from_time2'].' to '.$result['brk_to_time2'];
                }
			        ?>
            	<td width="50%"><strong>Break Time 2:</strong></td>
            	<td width="50%"><?php echo $breakTime2; ?></td>
            </tr>

            <tr>
              <?php
			          $breakTime3 = '';
                if ($result['brk_from_time3'] != '' && $result['brk_to_time3'] != '') {
                  $breakTime3 = $result['brk_from_time3'].' to '.$result['brk_to_time3'];
                }
			        ?>
            	<td width="50%"><strong>Break Time 3:</strong></td>
            	<td width="50%"><?php echo $breakTime3; ?></td>
            </tr>
            ...
            <tr>
              <?php
            	...
            	<td width="50%"><?php echo $result['total_break_time']; ?></td>
				    ...
				    <tr>
				    	<?php $total_net_time = explode(':', $result['total_net_time']);?>
				      	<td width="50%"><strong>Total Net Training Time :</strong></td>
				      	<td width="50%"><?php if ($total_net_time[0] != "" && $total_net_time[1] != "") {
                            echo $total_net_time[0] . ' hr : ' . $total_net_time[1] . 'min';
                          } ?></td>
				    </tr>

                    <tr>
                      	<td width="50%"><strong>Total No. of Candidates (Estimated) :</strong></td>
                        <td>
            	...
            	<tr>
              	<td width="50%"><strong>No. of Registered Candidates (Actual) :</strong></td>
                <td>
	                <table class="table table-bordered" style="border-color:#ccc; margin:0;">
										<tbody>
											<tr>
          ...
          <!-- <tr>
            <td width="50%"><strong>No. of Registered Candidates :</strong></td>
            <td width="50%"><?php echo $total_registered_candidates; ?></td>
          </tr> -->
          ...
          <?php if($result['batch_online_offline_flag'] != 1) { ?>
						...
						<tr>
							<td><strong>CCTV Link</strong></td>
							<td><?php echo $result['cctv_link']; ?></td>
						</tr>

						<?php if ($result['offline_city_name'] != "") {
              $location = $result['offline_city_name'];
            } else {
              $location = $result['offline_location_name'];
            } ?>

					<tr>
						<td><strong>Batch Center</strong></td>
						<td><?php echo $location; ?></td>
					</tr>

					<tr>
						<td><strong>Address of Concern Center</strong></td>
						<td><?php echo $result['center_address']; ?></td>
					</tr>
          <?php } ?>
                    ...
                    <?php if( $result['batch_status'] == 'Approved') { ?>
	                    <tr>
	                      <td>
	                      	<strong>Edit Training Period:</strong>
	                      </td>
	                      
	                      <td>
	                        <form method="post" action="<?php echo base_url('iibfdra/Version_2/batch/edit_training_period'); ?>" id="edit_training_period_form" enctype="multipart/form-data" autocomplete="off">
	                          <input type="hidden" id="batch_id" name="batch_id" value="<?php echo $result['id']; ?>">
	                          <input type="hidden" id="old_from_date" name="old_from_date" value="<?php echo $result['batch_from_date']; ?>">
	                          <input type="hidden" id="old_to_date" name="old_to_date" value="<?php echo $result['batch_to_date']; ?>">
	                          
	                          <div class="row">
	                          	<div class="col-md-6">
	                          		<div class="mt-3"></div>
			                          <input type="text" name="batch_from_date" id="batch_from_date" placeholder="Batch From Date *" class="form-control custom_input" value="<?php echo $result['batch_from_date']; ?>" required>

			                          <?php if(form_error('batch_from_date')!=""){ ?> <div class="clearfix"></div><label class="error"><?php echo form_error('batch_from_date'); ?></label> <?php } ?>
			                        </div>
			                        <div class="col-md-6">  
			                          <div class="mt-3"></div>
			                          <input type="text" name="batch_to_date" id="batch_to_date" placeholder="Batch To Date *" class="form-control custom_input" value="<?php echo $result['batch_to_date']; ?>" required>

			                          <?php if(form_error('batch_to_date')!="") { ?> <div class="clearfix"></div><label class="error"><?php echo form_error('batch_to_date'); ?></label> <?php } ?>
	                          	</div>
	                          </div>
	                          <div class="row" id="submit_btn_outer" style="margin-top: 10px;">
	                          	<div class="col-md-6 mt-3">
	                            	<button class="btn btn-primary mt-2" type="submit">Update Training Period</button>
	                          	</div>
	                          </div>
	                        </form>
	                      </td>
                    ...
                    <?php if( date('Y-m-d') <= $result['batch_to_date'] && $result['batch_status'] == 'Approved') { ?>
	                    <tr>
	                      <td width="50%">
	                      	<strong>Extend Date for Add/Update candidates:</strong>
	                      </td>
	                      
	                      <td>
	                        <form method="post" action="<?php echo base_url('iibfdra/Version_2/batch/extend_edit_date'); ?>" id="extend_date_form" enctype="multipart/form-data" autocomplete="off">
	                          ...
	                          <div id="batch_extend_type_err">
	                            <label class="css_checkbox_radio radio_only"> Add/Update Candidate
	                              <input type="radio" value="1" name="batch_extend_type" id="batch_extend_type" required <?php if($result['batch_extend_type'] != '2') { echo "checked"; } ?>>
	                              <span class="radiobtn"></span>
	                            </label>&nbsp;&nbsp; 
	                            <label class="css_checkbox_radio radio_only"> Only Update Candidate
	                              <input type="radio" value="2" name="batch_extend_type" id="batch_extend_type" required <?php if($result['batch_extend_type'] == '2') { echo "checked"; } ?>>
	                              <span class="radiobtn"></span>
	                            </label>
	                          ...
	                          <div class="mt-3"></div>
	                          <input type="text" name="batch_extend_date" id="batch_extend_date" placeholder="Extend Date for Add/Update candidates *" class="form-control custom_input" value="<?php echo $result['batch_extend_date']; ?>" onchange="validate_input('batch_extend_date');" required>
	                          <note class="form_note" id="batch_extend_date_err">Note: Please select the date between <?php echo $result['batch_from_date']; ?> and <?php echo $result['batch_to_date']; ?></note>
	                          ...
	                          <div id="submit_btn_outer">
	                            ...
	                            <?php if($result['batch_extend_date'] != "") { ?>
	    ...
	    <div class="col-xs-12" >
	        <div class="box">
	          ...
	          <div class="box-body ">
	            <div class="table-responsive">
	              <table id="batch_attendance" class="table table-bordered table-striped">
	                <thead>
	                  <tr>
	                  	<th>Sr.No.</th>
	                    <th>Action</th>
...
function fun_clear_extended_date()
  {
    ...
    text: 'Please confirm to clear the extended date',
    icon: 'warning',
    ...
    if (result.value) {
        window.location.href = "<?php echo base_url('iibfdra/Version_2/batch/clear_extended_date/'.$enc_batch_id); ?>";
    }
});
...
$(function () {
   ...
   $('#batch_extend_date').datepicker({ 
   	keyboardNavigation: true, 
   	forceParse: true, 
   	autoclose: true, 
   	format: "yyyy-mm-dd", 
   	clearBtn: true, 
   	startDate:"<?php echo $result['batch_from_date']; ?>", 
   	endDate:"<?php echo $result['batch_to_date']; ?>" 
   });

   const fromDateInput = $('#batch_from_date');
   const toDateInput = $('#batch_to_date');

    // Default dates if already available
    const initialFromDate = fromDateInput.val() ? new Date(fromDateInput.val()) : null;
    const initialToDate = toDateInput.val() ? new Date(toDateInput.val()) : null;

    // Set limits for From Date if initialToDate exists
    if (initialFromDate && initialToDate) {
        const fromMinDate = new Date(initialFromDate);
        const fromMaxDate = new Date(initialFromDate);
        fromMinDate.setDate(fromMinDate.getDate() - 3); // From Date Lower limit
        fromMaxDate.setDate(fromMaxDate.getDate() + 3); // From Date Upper limit
        ...
        const toMinDate = new Date(initialToDate);
        const toMaxDate = new Date(initialToDate);
        toMinDate.setDate(toMinDate.getDate() - 3); // To Date Lower limit
        toMaxDate.setDate(toMaxDate.getDate() + 3); // To Date Upper limit
	...
	$('.update_batch_communication').click(function(){ 
		...
		if(batch_communication == ''){
			...
			$.ajax({
	            url:site_url+'iibfdra/Version_2/Batch/agency_update_batch_communication',
...
var table = jQuery('#listitems_logs,#batch_attendance').DataTable( {  
	buttons: [
        'print'
    ],
    "paging": true,	
