          ...
          <div class="box-body">
            <form class="form-horizontal" autocomplete="off" name="frmDrACenter" id="faculty_form" method="post" enctype="multipart/form-data" data-parsley-validate>
              ...
              <div class="col-sm-12">
                  ...
                  <div class="col-sm-4">
                    <select name="instituteId" id="instituteId" class="form-control instituteId">
                      <!-- <option value="">Select Institute</option> -->
                      <?php foreach ($faculty_code_data as $key => $fac_institute) { ?>
                       <option value="<?php echo $fac_institute['institute_id']; ?>" attr-faculty="<?php echo base64_encode($fac_institute['faculty_id']); ?>" <?php if ($faculty_data[0]['institute_id'] == $fac_institute['institute_id']) { echo 'selected'; } ?>> <?php echo $fac_institute['inst_name']; ?> </option> 
                      <?php } ?> 
                    </select>
                  </div>
              </div>
              ...
              <?php 
                ...
                $current_batch_qry = $this->db->query("SELECT batch_from_date, batch_to_date, batch_code, batch_name
                      FROM agency_batch 
                      WHERE is_deleted = 0
                      AND   (first_faculty = ".$faculty_data[0]['faculty_id']." OR sec_faculty = ".$faculty_data[0]['faculty_id']." OR additional_first_faculty = ".$faculty_data[0]['faculty_id']." OR additional_sec_faculty = ".$faculty_data[0]['faculty_id'].") AND CURDATE() BETWEEN batch_from_date AND batch_to_date");

                $current_batch_data = $current_batch_qry->result_array();
                
                if(count($current_batch_data) > 0)
                {
                  foreach ($current_batch_data as $current_key => $value) 
                  {
                    $current_srno = $current_key+1;
                    $current_str.= $current_srno.".".$value['batch_code'].' ('.$value['batch_from_date'].' to '.$value['batch_to_date'].') &nbsp';
                  }
                }

                $upcoming_str = '';
                $upcoming_batch_qry = $this->db->query("SELECT batch_from_date, batch_to_date, batch_code, batch_name
                      FROM agency_batch 
                      WHERE is_deleted = 0
                      AND   (first_faculty = ".$faculty_data[0]['faculty_id']." OR sec_faculty = ".$faculty_data[0]['faculty_id']." OR additional_first_faculty = ".$faculty_data[0]['faculty_id']." OR additional_sec_faculty = ".$faculty_data[0]['faculty_id'].") AND batch_from_date > CURDATE()");

                $upcoming_batch_data = $upcoming_batch_qry->result_array();
                
                if(count($upcoming_batch_data) > 0) 
                {
                  foreach ($upcoming_batch_data as $upcoming_key => $upcoming_value) 
                  {
                    $upcoming_srno = $upcoming_key+1;
                    $upcoming_str.= $upcoming_srno.".".$upcoming_value['batch_code'].' ('.$upcoming_value['batch_from_date'].' to '.$upcoming_value['batch_to_date'].') &nbsp';
                  }
                }

                $complted_str = '';
                $complted_batch_qry = $this->db->query("SELECT batch_code, batch_name
                      FROM agency_batch 
                      WHERE is_deleted = 0
                      AND   (first_faculty = ".$faculty_data[0]['faculty_id']." OR sec_faculty = ".$faculty_data[0]['faculty_id']." OR additional_first_faculty = ".$faculty_data[0]['faculty_id']." OR additional_sec_faculty = ".$faculty_data[0]['faculty_id'].") AND batch_to_date < CURDATE()");

                $complted_batch_data = $complted_batch_qry->result_array();
                
                if(count($complted_batch_data) > 0) 
                {
                  foreach ($complted_batch_data as $complted_key => $complted_value) 
                  {
                    $complted_srno = $complted_key+1;
                    $complted_str.= $complted_srno.".".$complted_value['batch_code'].' &nbsp';
                  }
                }
              ...
              <div class="col-sm-12">
                <div class="form-group">
                  <label class="col-sm-3 control-label">Live Batches</label>
                  <div class="col-sm-6">
                    <?php echo $current_str; ?>
                  </div>
                </div>
              </div>
              ...
              <div class="col-sm-12">
                <div class="form-group">
                  <label class="col-sm-3 control-label">Upcoming Batches</label>
                  <div class="col-sm-6">
                    <?php echo $upcoming_str; ?>
                  </div>
                </div>
              </div>
              ...
              <div class="col-sm-12">
                <div class="form-group">
                  ...
                  <div class="col-sm-3">
                    <input type="text" class="form-control" id="faculty_name" name="faculty_number" placeholder="Faculty Number" value="<?php echo isset($faculty_data[0]['faculty_code']) ? $faculty_data[0]['faculty_code'] : ''; ?>" readonly="readonly">
                  </div>
                </div>
              </div>
              ...
              if (!empty($faculty_data[0]['faculty_photo'])) { ?>
                <div class="col-sm-12" id="faculty_photo_show">
                  <div class="form-group">
                    ...
                    <div class="col-sm-5">
                      <a href="<?php echo base_url(); ?>uploads/faculty_photo/<?php echo $faculty_data[0]['faculty_photo']; ?>" target="_blank"><img height="100" width="100" src="<?php echo base_url(); ?>uploads/faculty_photo/<?php echo $faculty_data[0]['faculty_photo']; ?>" /></a>
                    </div>
                  </div>
                </div>
              <?php } ?>
              ...
              <?php if (!empty($faculty_data[0]['pan_photo'])) { ?>
                <div class="col-sm-12" id="pan_photo_show">
                  <div class="form-group">
                    ...
                    <div class="col-sm-5">
                      <a href="<?php echo base_url(); ?>uploads/pan_photo/<?php echo $faculty_data[0]['pan_photo']; ?>" target="_blank"><img height="100" width="100" src="<?php echo base_url(); ?>uploads/pan_photo/<?php echo $faculty_data[0]['pan_photo']; ?>" /></a>
                    </div>
                  </div>
                </div>
              <?php } ?>
              ...
              <div class="col-sm-12">
                <div class="form-group">
                  <label class="col-sm-3 control-label">Completed Batches</label>
                  <div class="col-sm-6">
                    <?php echo $complted_str; ?>
                  </div>
                </div>
              </div>  
...
<script type="text/javascript">
  ...
  {
    var selInstituteId = $(this).val();
    var selFacultyId   = $(this).find(':selected').attr('attr-faculty');
    // alert(selFacultyId);
    if ( selInstituteId != '' && selFacultyId != '') 
    {
      window.location.href = base_url+"iibfdra/Version_2/admin/faculty_master/faculty_view/"+selFacultyId;
    }
  })
