        ...
        <div class="box box-info">
          ...
          <div class="box-body">
            <form class="form-horizontal" autocomplete="off" name="frmDrACenter" id="faculty_form" method="post" enctype="multipart/form-data" data-parsley-validate>
              ...
              <div class="col-sm-12">
                <label class="col-sm-3 control-label">In Review Count </label>
                <label class="col-sm-2" style="margin-top:3px;"><?php echo $inreview_count; ?></label>
              </div>
              
              <div class="col-sm-12">
                <label class="col-sm-3 control-label">Active Count </label>
                <label class="col-sm-2" style="margin-top:3px;"><?php echo $active_count; ?></label>
              </div>

              <div class="col-sm-12">
                <label class="col-sm-3 control-label">Inactive Count </label>
                <label class="col-sm-2" style="margin-top:3px;"><?php echo $inactive_count; ?></label>
              </div>
                <!-- <div class="col-sm-12">
                  <label class="col-sm-3 control-label">Faculty Use Status</label>
                  <div class="col-sm-2">
                    <div class="form-check">
                      <?php if( $faculty_data[0]['faculty_use_status'] == 'InUse' ) { ?>
                        <label class="btn btn-warning" style="cursor: auto;">In Use</label>
                      <?php } else { ?>  
                        <label class="btn btn-warning" style="cursor: auto;">Not In Use</label>
                      <?php } ?>  
                    </div>
              ...
              <?php 
                if( $request_from != 'Inspector' ) 
                {
                  foreach ($faculty_code_data as $key => $fac_data) 
                  { 
                    $mul_faculty_id = $fac_data['faculty_id'];
              ?>
                <div class="col-sm-12">
                  <label class="col-sm-3 control-label"> <?php echo $fac_data['inst_name']; ?> Change Status</label>
                  <div class="col-sm-4">
                    <div class="form-check">
                      <?php if ($fac_data['status'] == 'In Review' || $fac_data['status'] == 'Inactive' || $fac_data['status'] == 'Re-Submitted') { ?>
                        <label><input type="radio" class="form-check-input radiobtn" data-facId="<?php echo $mul_faculty_id; ?>" id="active_<?php echo $mul_faculty_id; ?>" name="radiobt_<?php echo $mul_faculty_id; ?>" value="Active">Active</label>
                      <?php } ?>

                      <?php if ($fac_data['status'] == 'In Review' || $fac_data['status'] == 'Active' || $fac_data['status'] == 'Re-Submitted') { ?>
                        <label><input type="radio" class="form-check-input radiobtn" data-facId="<?php echo $mul_faculty_id; ?>" id="inactive_<?php echo $mul_faculty_id; ?>" name="radiobt_<?php echo $mul_faculty_id; ?>" value="Inactive" onclick="return confirm_action(event,'<?php echo $mul_faculty_id; ?>')">Inactive</label>
                      <?php } ?>
                    </div>
                    <span class="error" id="status_error_<?php echo $mul_faculty_id; ?>"></span>
                  </div>
                  <input type="hidden" name="inst_id_<?php echo $mul_faculty_id; ?>" id="inst_id_<?php echo $mul_faculty_id; ?>" value="<?php echo $fac_data['institute_id']; ?>">
                  <div class="col-sm-5">
                    <input type="button" name="btn_status" id="btn_status" value="Change Status" onclick="change_status('<?php echo $mul_faculty_id; ?>')">
                  </div>
                </div>
              
                <div class="col-sm-12" id="reason_div_<?php echo $mul_faculty_id; ?>" style="display: none;">
                  <label class="col-sm-3 control-label">Reason</label>
                  <div class="col-sm-3" style="margin-top:10px">
                    <textarea rows="2" id="reason_<?php echo $mul_faculty_id; ?>"></textarea>
                    <br><span class="error" id="reason_error_<?php echo $mul_faculty_id; ?>"></span>
                  </div>
                </div>
              <?php 
                  }
              ...
              {
                if ($faculty_code_data[0]['institute_id'] != '') 
                {
                  $where = array('dra_inst_registration_id' => $value['institute_id']);
                  $institute_data = $this->master_model->getRecords('dra_accerdited_master', $where);
                  $institute_name .= $institute_data[0]['institute_name'].'<br>';
                } 
              ...
              <div class="col-sm-12">
                <div class="form-group">
                  <label class="col-sm-3 control-label">All Institute</label>
                  <div class="col-sm-6">
              ...
              <div class="col-sm-12">
                <div class="form-group">
                  <label class="col-sm-3 control-label">All Batches</label>
                  <div class="col-sm-6">
                    <label class=""><b><?php echo $str; ?></b></label>
                  </div>
                </div>
              </div>
              ...
              <div class="col-sm-12">
                <div class="form-group">
                  <label class="col-sm-3 control-label">Faculty Code.<span style="color:#F00">*</span></label>
                  <div class="col-sm-3">
                    <input type="text" class="form-control" id="faculty_name" name="faculty_number" placeholder="Faculty Code" value="<?php echo isset($faculty_data[0]['faculty_code']) ? $faculty_data[0]['faculty_code'] : ''; ?>" readonly="readonly">
                  </div>
                </div>
              </div>
              ...
              <?php 
                ...
                <div class="col-sm-12">
                  <div class="form-group">
                    ...
                    <div class="col-sm-9">
                      <select class="form-control" data-placeholder="Select Language" name="language[]" id="language"  multiple data-parsley-required disabled>
                        <option value=""></option>
                ...
                <div class="col-sm-12">
                  <center>
                    ...
                    <?php if ($request_from != 'Inspector' && ($action == 'add' || $action == 'edit' || $action == 'view')) { ?>
                      <a href="<?php echo base_url('iibfdra/admin/faculty_master'); ?>" class="btn btn-warning mr-2">Back</a>
                    <?php }?>
                    <?php if($request_from == 'Inspector') {?>
...
<script type="text/javascript">
  $(document).ready(function() { $('#language').chosen(); });

  var base_url = '<?php echo base_url(); ?>';
  var action = '<?php echo $action; ?>';
  var institute_id = '<?php echo $faculty_data[0]['institute_id'] ?>';
  ...
  $('.radiobtn').click(function() {
    var status = $(this).val();
    var fac_attr = $(this).attr('data-facId');
    console.log(status);
    if (status == 'Active') {
      //$('#reason').text('');
      $('#reason_div_'+fac_attr).css('display', 'none');
    }
    /*else{
  ...
  function confirm_action(evt,faculty_id) {
    ...
    swal({
      ...
      if (OK.value) {
        $.ajax({
          url: base_url + "iibfdra/admin/faculty_master/check_faculty_status",
          type: 'POST',
          data: {
            'ci_csrf_token': '',
            prim_id: faculty_id
          ...
          success: function(response) {
            ...
            if (response != '') {
              ...
              $('#reason_div_'+faculty_id).css('display', 'block');
              $('#reason_error_'+faculty_id).text('');
            }
          }
        })
      } else {
        $("#inactive_"+faculty_id).prop('checked', false);
      }
    });
  ...
  $(".radiobtn").on("change", function() {
    var faculty_attr = $(this).attr('data-facId');
    $("#status_error_"+faculty_attr).text("");
  });

  function change_status(faculty_id) {
    var err_cnt = 0;
    // var status = $("input[name='radiobt']:checked").val();
    var status = $("input[name='radiobt_"+faculty_id+"']:checked").val();
    var institute_id = $('#inst_id_'+faculty_id).val();
    var reason = $('#reason_'+faculty_id).val();
    var pan_no_hidden = $('#pan_no').val();
    console.log('++'+pan_no_hidden);
    // alert(status);
    // alert(reason);
    if (status == 'Inactive' && (reason == '' || reason == undefined)) {
      $('#reason_error_'+faculty_id).text('Please enter Reason');
      err_cnt = 1;
    ...
    if (err_cnt == 0) {
      $("#status_error_"+faculty_id).text("")
      if (status == 'Inactive' || status == 'Active') {
        $("#status_error_"+faculty_id).text("")
        $.ajax({
          type: 'POST',
          url: base_url + "iibfdra/admin/faculty_master/change_status",
          data: {
        ...
        $("#status_error_"+faculty_id).text("Please select the status")
      }
    }
  }
</script>
