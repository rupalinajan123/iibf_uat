                  ...
                  <select class="form-control" name="batch_id" id="batch_id">
                    ...
                    <?php 
                      foreach ($batch as $key => $value) {?>
                        <option value="<?php echo $value['id']; ?>" agency-id-attr="<?php echo $value['agency_id']; ?>" agency-name-attr="<?php echo $value['institute_name']; ?>" batch-code-attr="<?php echo $value['batch_code']; ?>"  batch-duration-attr="<?php echo $value['batch_from_date'].' To '.$value['batch_to_date']; ?>"  batch-platform-attr="<?php echo $value['online_training_platform']; ?>" batch-time-attr="<?php echo $value['hours'].' Hours'; ?>" daily-batch-timing="<?php echo $value['timing_from'].' To '.$value['timing_to']; ?>" candidate-count-attr="<?php echo $value['total_candidates']; ?>" batch-medium-attr="<?php echo $value['training_medium']; ?>" assigned-faculty-attr1="<?php echo $value['first_faculty_name']; ?>" assigned-faculty-attr2="<?php echo $value['sec_faculty_name']; ?>" additional-faculty-attr1="<?php echo $value['add_first_faculty_name']; ?>" additional-faculty-attr2="<?php echo $value['add_sec_faculty_name']; ?>" contact-person-attr="<?php echo $value['contact_person_name'].' ('. $value['contact_person_phone'].')';?>" alt-contact-person-attr="<?php echo $value['alt_contact_person_name'].' ('. $value['alt_contact_person_phone'].')';?>" platform-attr="<?php echo $value['platform_link'] ?>"><?php echo $value['batch_code']; ?></option>
                      <?php }
                    ?>
                  </select>
                </div>
      ...
      <table class="table table-bordered table-striped" style="word-wrap:anywhere; background:<?php echo $div_class; ?>;">
        <tbody>
          ...
          <tr>
            ...
            <td width="2%"></td>
            <td width="29%"><strong>Duration:</strong></td>
...
$(function () {
  ...
  $('#batch_id').on('change',function () {
    ...
    var additional_faculty1 = $(this).children(":selected").attr('additional-faculty-attr2');
    var assigned_faculty2 = $(this).children(":selected").attr('assigned-faculty-attr2');
    var additional_faculty2 = $(this).children(":selected").attr('additional-faculty-attr');
    ...
    if(batch_id != ''){
      ...
      $.ajax({
          url:site_url+'iibfdra/admin/InspectionSummary/get_candidate_data',
          ...
          success: function(res) {
            ...
            if(data != '')
            {
              ...
              $.each(JSON.parse(data), function(idx, obj) {
                ...
                values = [[j, obj.training_id, obj.name, obj.dateofbirth, obj.mobile_no, photo, obj.present_cnt, obj.absent_cnt, obj.remark]];
