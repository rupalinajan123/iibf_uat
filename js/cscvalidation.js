// JavaScript Document
	...
	$('#selSubName').change(function(){
		...
		$.ajax({
								url:site_url+'CSCVenue/getElectiveVenue/',
...
/*changes done by pooj@ godse*/		
	...
	function valCentre(cCode)
	{
		...
		var Eval = 'N';
		var selection_csc_nseit='csc';//priyanka d- 31 may 23 >> added selection_csc_nseit
		$(".loading").show();
		if($('.selection_csc_nseit').length > 0) { //priyanka d- 31 may 23 >> added selection_csc_nseit
			selection_csc_nseit = $('input[name="selection_csc_nseit"]:checked').val();
			console.log(selection_csc_nseit);
		}
		if(cCode!='')
		{
				...
				var datastring_exam='centerCode='+cCode+'&examCode='+examCode+'&subject_array='+subject_array+'&selection_csc_nseit='+selection_csc_nseit; //priyanka d- 31 may 23 >> added selection_csc_nseit
				$.ajax({
								url:site_url+'CSCVenue/getVenue/',
								data: datastring_exam,
								type:'POST',
								async: false,
								dataType: 'json',
	...
	function venue(venue_code,date_id,time_id,subject_code,seat_capacity_id)
	{
		...
		var selection_csc_nseit ='csc'; //priyanka d- 31 may 23
		//	if(venue_code!='' && selCenterCode!='')
		//{
			$(".loading").show();

			//var datastring='centerCode='+selCenterCode+'&venue_code='+venue_code;
			if($('input[name="ippbSelectedVenue_'+subject_code+'"]').length > 0) {
				$('input[name="ippbSelectedVenue_'+subject_code+'"]').val(venue_code);
			}  //priyanka d- 31 may 23
			if($('.selection_csc_nseit').length > 0) {
				selection_csc_nseit = $('input[name="selection_csc_nseit"]:checked').val();
			}  //priyanka d- 31 may 23


				//var datastring='centerCode='+selCenterCode+'&venue_code='+venue_code;
				var datastring='eprid='+eprid+'&examcode='+examcode+'&subject_code='+subject_code+'&venue_code='+venue_code+'&selection_csc_nseit='+selection_csc_nseit;  //priyanka d- 31 may 23 >> added 
				$.ajax({
								url:site_url+'CSCVenue/getDate/',
								data: datastring,
								type:'POST',
								async: false,
								dataType: 'json',
	...
	function date(date_code,venue_id,time_id,subject_code='')
	{
		var selCenterCode= document.getElementById('selCenterName').value;
		var venue_code= document.getElementById(venue_id).value;
		var selection_csc_nseit='csc';
		//if(date_code!='' && venue_code!='' && selCenterCode!='')
		//{
		$(".loading").show();
		if($('input[name="ippbSelectedDate_'+subject_code+'"]').length > 0) {
			$('input[name="ippbSelectedDate_'+subject_code+'"]').val(date_code);
	   	}  //priyanka d- 31 may 23
		if($('.selection_csc_nseit').length > 0) {
			selection_csc_nseit = $('input[name="selection_csc_nseit"]:checked').val();
		}  //priyanka d- 31 may 23

			var datastring='centerCode='+selCenterCode+'&venue_code='+venue_code+'&date_code='+date_code+'&selection_csc_nseit='+selection_csc_nseit;
			$.ajax({
								url:site_url+'CSCVenue/getTime/',
								data: datastring,
								type:'POST',
								async: false,
								dataType: 'json',
	...
	function time(time,venue_id,date_id,seat_capcity_id,subject_code='')
	{
		var selCenterCode= document.getElementById('selCenterName').value;
		var venue_code= document.getElementById(venue_id).value;
		...
		//{
		$(".loading").show();
		if($('input[name="ippbSelectedTime_'+subject_code+'"]').length > 0) {
			$('input[name="ippbSelectedTime_'+subject_code+'"]').val(time);
	   	}  //priyanka d- 31 may 23

		if($('.selection_csc_nseit').length > 0) {
			selection_csc_nseit = $('input[name="selection_csc_nseit"]:checked').val();
		}  //priyanka d- 31 may 23
			var datastring='centerCode='+selCenterCode+'&venue_code='+venue_code+'&date_code='+date_id+'&time='+time+'&selection_csc_nseit='+selection_csc_nseit;
			$.ajax({
							url:site_url+'CSCVenue/getCapacity/',
							data: datastring,
							type:'POST',
							async: false,
							dataType: 'json',
