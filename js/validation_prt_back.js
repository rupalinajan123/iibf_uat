...
{	
	...
	public function member()
	{
		...
		$data['validation_errors'] = '';
		 ...
		 {
				$scannedphoto_file=$scannedsignaturephoto_file=$idproof_file=$outputphoto1=$outputsign1=$outputidproof1=$declaration_form=$state='';
				...
				$this->form_validation->set_rules('idproof','Declaration Form','trim|required|xss_clean');
...
public function address1($addressline1) 
{
	...
	public function addmember()
	{
		...
		// $decData = $aes->decrypt($encPass);
		 ...
		 //{
			...
			$declaration_form = $this->session->userdata['enduserinfo']['declaration'];
			$img_response = check_files_exist('./uploads/declaration/'.$declaration_form); //update_image_name_helper.php
			if($img_response['flag'] != 'success') { $image_error_flag = 1; }
			...
			if($last_id = $this->master_model->insertRecord('member_registration',$insert_info,true))
			{
				...
				$add_img_data['declaration'] = convert_img_into_base64(base_url().'uploads/declaration/'.$declaration_form);
	...
	*/
		$data=array('middle_content'=>'thankyou','application_number','application_number'=>$user_info[0]['regnumber'],'password'=>$this->session->userdata['memberdata']['password']);
...
//		file_put_contents($outputidproof, file_get_contents($inputidproofphoto));
	...
	public function examlist()
	{
		...
		$flag=1;
		$exam_list=array();$exam_list=array();
	...
	public function examlist_test()
	{
		$today_date=date('Y-m-d');
		$flag=1;
		$exam_list=array();$exam_list=array();$examcodes=array('526','527','991');
		$Extype = base64_decode($this->input->get('Extype'));
		$Mtype = base64_decode($this->input->get('Mtype'));
		if($Mtype!='O' && $Mtype!='A' && $Mtype!='F' && $Mtype!='DB' && $Mtype!='NM')
		{
