...
// JavaScript Document
	...
	//###---------------------------Non-Member validation------------------------####
		...
		window.Parsley.addValidator('nonmemcheckpin', function (value, requirement) {
			...
			$.ajax({
				...
				url:site_url+'bulk/BulkApplyNM/checkpin/',
		...
		window.Parsley.addValidator('nonmememailcheck', function (value, requirement) {
			...
			$.ajax({
				...
				url:site_url+'bulk/BulkApplyNM/emailduplication/',
		...
		window.Parsley.addValidator('nonmobilecheck', function (value, requirement) {
			...
			var msg='';
				...
				url:site_url+'bulk/BulkApplyNM/mobileduplication/',
...

	{
		...
		document.getElementById('error_photo_size').innerHTML="File size should be maximum 50KB.";
	...
	$( "#empidproofphoto" ).change(function() {
	...
	var empidproofphoto=document.getElementById('empidproofphoto');

	//fileUpload[appKey]['photo'] = empidproofphoto;

	var empidproofphoto_im=empidproofphoto.value;

	var ext1=empidproofphoto_im.substring(empidproofphoto_im.lastIndexOf('.')+1);

	if(empidproofphoto.value!=""&&  ext1!='jpg' && ext1!='JPG' && ext1!='jpeg' && ext1!='JPEG')

	{

			$('#error_empidproofphoto').show();

			$('#error_empidproofphoto').fadeIn(3000);	

			document.getElementById('error_empidproofphoto').innerHTML="Upload JPG or jpg file only.";

			setTimeout(function(){

			$('#error_empidproofphoto_size').css('color','#B94A48');

			document.getElementById("empidproofphoto").value = "";

			$('#hiddenempidproofphoto').val('');
...

	{
		...
		$('#error_empidproofphoto_size').show();

		$('#error_empidproofphoto_size').fadeIn(3000);	

		document.getElementById('error_empidproofphoto_size').innerHTML="File size should be maximum 20KB.";
...

		{
			...
			$('#error_empidproofphoto').html('');

			$('#error_empidproofphoto_size').html('');
			...
			if (/^image/.test( files[0].type)){ // only image file
				...
				reader.onloadend = function(){ // set image data as background of div
					...
					$('#hiddenempidproofphoto').val(this.result);

				}

			}

			 readURL(this,'image_upload_empidproof_preview');
	...
	var declarationform=document.getElementById('declarationform');

	//fileUpload[appKey]['photo'] = declarationform;

	var declarationform_im=declarationform.value;

	var ext1=declarationform_im.substring(declarationform_im.lastIndexOf('.')+1);

	if(declarationform.value!=""&&  ext1!='jpg' && ext1!='JPG' && ext1!='jpeg' && ext1!='JPEG')

	{

			$('#error_declarationform').show();

			$('#error_declarationform').fadeIn(3000);	

			document.getElementById('error_declarationform').innerHTML="Upload JPG or jpg file only.";

			setTimeout(function(){

			$('#error_declarationform_size').css('color','#B94A48');
...

	{
		...
		$('#error_declarationform_size').show();

		$('#error_declarationform_size').fadeIn(3000);	

		document.getElementById('error_declarationform_size').innerHTML="File size should be maximum 20KB.";
...

		{
			...
			$('#error_declarationform').html('');

			$('#error_declarationform_size').html('');
	...
	$( "#empidproofphoto_bulk" ).change(function() {
	...
	var empidproofphoto=document.getElementById('empidproofphoto_bulk');
...

	{
			...
			document.getElementById("empidproofphoto_bulk").value = "";
...

	{
		...
		document.getElementById('error_empidproofphoto_size').innerHTML="File size should be maximum 300KB.";
...

		{
...

		 {
			...
			$('#error_signature_size').show();
				...
				document.getElementById('error_signature_size').innerHTML="File size should be maximum 50KB.";
...

			{
...

				{
					...
					$.ajax({
						...
						url: site_url+'bulk/BulkApplyNM/ajax_check_captcha',
...
/*changes done by pooj@ godse*/		
...

	{
...

		{
			...
			if($('.bulk_age_validate_input').length >0) {
				var date_max = new Date($('.bulk_age_validate_input').attr('maxagedate'));
				var date_min = new Date($('.bulk_age_validate_input').attr('minagedate'));
			}
			else {
				var date_max = new Date(max_year, '08', '01');

				var date_min = new Date(min_year, '08', '01');
			}
			...
			//console.log(date_max+' > '+dob_date+' || '+date_min+' < '+dob_date);
	...
	*/
...

	{
...

		{
				...
				$.ajax({
								...
								url:site_url+'Bulk_venue/getVenue/',
		...
		//{
			...
			$(".loading").show();
				...
				$.ajax({
								...
								url:site_url+'Bulk_venue/getDate/',
...

	{
		...
		$(".loading").show();
			...
			$.ajax({
								...
								url:site_url+'Bulk_venue/getTime/',
...

	{
		...
		$(".loading").show();
			...
			var datastring='centerCode='+selCenterCode+'&venue_code='+venue_code+'&date_code='+date_id+'&time='+time+'&examcode='+examcode;

			$.ajax({

							url:site_url+'Bulk_venue/getCapacity/',
