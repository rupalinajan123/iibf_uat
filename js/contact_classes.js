// JavaScript Document
	...
	var site_url = 'https://'+window.location.hostname+'/staging/';
	...
	var noncheckval=0;
	   ...
	   // check valid pin for member user	
		...
		window.Parsley.addValidator('emailcheck', function (value, requirement) {
			...
			{
				$.ajax({
				url:site_url+'FinQuest/emailduplication/',
				data: datastring,
				type:'POST',
				dataType:'json',
				async: false,
		...
		window.Parsley.addValidator('mobilecheck', function (value, requirement) {
			...
			var msg='';
				...
				$.ajax({
				url:site_url+'FinQuest/mobileduplication/',
				data: datastring,
				type:'POST',
				dataType:'json',
				async: false,
	...
	*/
	function valCentre(cCode)
	{
		...
		if(cCode!='')
		{
				...
				var datastring_exam='centerCode='+cCode+'&excd='+excd;
				$.ajax({
...
function getcenter(courcecode,memno)
{

	if(courcecode!='')
		{
			var datastring='courcecode='+courcecode+'&memno='+memno;
			var htm = '';
		
						$.ajax({
							type:'POST',
								url: site_url+'ContactClasses/getcenter',
								data: datastring,
								dataType: 'json',
 								success: function(data) //we're calling the response json array 'cities'
								{
									
									if(data.redirect_url=='Yes')
									{
									...
									}else if(data.redirect_url=='No')
									{
										
									 htm+= '<option value = "">--SELECT--</option>';
									
									$.each( data.cource, function( key, value ) {
									 htm+= '<option value = "'+value.center_code+'">'+value.center_name+'</option>';
									});
	...
	if(centercode!='')
		{
			...
			var htm = '';
						...
						$.ajax({
							type:'POST',
								url: site_url+'ContactClasses/getsub',
			data: { centercode : centercode, course_id : course_id },
     		success: function(data) //we're calling the response json array 'cities'
								{
								//	alert(data);
								
								//	 htm+= '<label  value = "">Subjects</label>';
								
								
									/*$.each( data.subject, function( key, value ) {
									 htm+= '<label  value= "'+value.Scode+'">'+value.Sname+'</label>';
								  ...
								  $('#submsg, #submsg_label').hide();
											$("#submsg").html();
								} //end success
         }); //end AJAX
    } else {
		        $('#subject, #subject_label').hide();
				$("#subject").html();
