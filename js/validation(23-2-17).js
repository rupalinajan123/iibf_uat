	...
	$( "#idproofphoto" ).change(function() {
		...
		if(dob_proof_image.value!=""&&  ext3!='jpg' && ext3!='JPG' && ext3!='jpeg' && ext3!='JPEG')
		{
		...
		else  if(filesize2)
		 {
			...
			document.getElementById('error_dob_size').innerHTML="File size should be maximum 25KB.";
			setTimeout(function(){
...
//for member non-mem  preview
	...
	{	
		...
		if(flag)
		{
			...
			$('#email').val() == $('#email_hidd').val() && $('#stdcode').val() == $('#stdcode_hidd').val() && $('#phone').val() == $('#phone_hidd').val() && $('#office').val() == $('#office_hidd').val() && $('#mobile').val() == $('#mobile_hidd').val() && idproof == $('#idproof_hidd').val() && $('#idNo').val() == $('#idNo_hidd').val() 
			&& optnletter == $('#optnletter_hidd').val() && $('#designation').val().trim() == $('#designation_hidd').val().trim() && $('#doj1').val()==$('#doj_hidd_validate').val() && (eduqual1==specify_q || eduqual2==specify_q || eduqual3==specify_q)  && $('#dob_hidd').val()==$("#dob1").val() && $('#hidd_nameoncard').val()==$('#nameoncard').val() && $('#firstname').val()==$('#hidd_firstname').val() && $('#sel_namesub').val()==$('#hidd_sel_namesub').val() && $('#gender').val()==$('#hidd_gender').val())
			{
	...
	//for logged in member  checkform
 	function loginusercheckform()
 	{
		...
		{
				$.ajax({
				url: site_url+'Home/setExamSession',
				type: 'post',
				data:$("form").serialize(),
				success: function(result) {
				//alert(result);
				window.open(site_url+'Home/preview', '_self')
				//	return false
				}/*,
				error: function(e) {
					alert('Error occured: ' + JSON.stringify(e));
	...
	//for logged in member  checkform during special exam apply
 	function splexamapply()
 	{
		...
		{
				$.ajax({
				url: site_url+'SplexamM/setExamSession',
				type: 'post',
				data:$("form").serialize(),
				success: function(result) {
				//alert(result);
				window.open(site_url+'SplexamM/preview', '_self')
				//	return false
				}/*,
				error: function(e) {
					alert('Error occured: ' + JSON.stringify(e));
	...
	//Member can apply exam directly
 	function member_apply_exam()
 	{
		...
		{
				$.ajax({
				url: site_url+'Applyexam/setExamSession',
				type: 'post',
				data:$("form").serialize(),
				success: function(result) {
				window.open(site_url+'Applyexam/preview', '_self')
				//	return false
				}/*,
				error: function(e) {
					alert('Error occured: ' + JSON.stringify(e));
	...
	//Member can apply for special exam directly 
 	function spl_member_apply_exam()
 	{
		...
		{
				$.ajax({
				url: site_url+'ApplySplexamM/setExamSession',
				type: 'post',
				data:$("form").serialize(),
				success: function(result) {
				window.open(site_url+'ApplySplexamM/preview', '_self')
				//	return false
				}/*,
				error: function(e) {
					alert('Error occured: ' + JSON.stringify(e));
	...
	//for logged in non-member  checkform
 	function login_nm_checkform()
 	{
		...
		{
				$.ajax({
				url: site_url+'NonMember/setExamSession',
				type: 'post',
				data:$("form").serialize(),
				success: function(result) {
				//alert(result);
				window.open(site_url+'NonMember/preview', '_self')
				//	return false
				}/*,
				error: function(e) {
					alert('Error occured: ' + JSON.stringify(e));
	...
	//loggedin non-member  Special Exam
 	function login_specialexam_checkform()
 	{
		...
		{
				$.ajax({
				url: site_url+'SpecialExamNm/setExamSession',
				type: 'post',
				data:$("form").serialize(),
				success: function(result) {
				//alert(result);
				window.open(site_url+'SpecialExamNm/preview', '_self')
				//	return false
				}/*,
				error: function(e) {
					alert('Error occured: ' + JSON.stringify(e));
	...
	//for logged in dbf checkform
 	function login_dbf_checkform()
 	{
		...
		{
				$.ajax({
				url: site_url+'Dbf/setExamSession',
				type: 'post',
				data:$("form").serialize(),
				success: function(result) {
				//alert(result);
				window.open(site_url+'Dbf/preview', '_self')
				//	return false
				}/*,
				error: function(e) {
					alert('Error occured: ' + JSON.stringify(e));
