...
$(document).ready(function() {
	...
	$("#exam_code").change(function(){
		var datastring='exam_code='+$('#exam_code').val();
		$.ajax({
			url:site_url+'ELearning/getsubject/',
			data: datastring,
			type:'POST',
			async: false,
	...
	window.Parsley.addValidator('elremailcheck', function (value, requirement) {
		//.focus()
		//$("#email_id").focusout();
		var response = false;
		var filter = /^[a-zA-Z0-9._-]+@([a-zA-Z0-9.-]+\.)+[a-zA-Z0-9.-]{2,4}$/;
		var datastring='email='+value;
			$.ajax({
			url:site_url+'ELearning/emailduplication/',
			data: datastring,
			type:'POST',
			dataType:'json',
			async: false,
	...
	.addMessage('en', 'elremailcheck', 'The email id already exists');
	...
	window.Parsley.addValidator('elrmobilecheck', function (value, requirement) {
		var response = false;
		var msg='';
			var datastring='mobile='+value;
			$.ajax({
			url:site_url+'ELearning/mobileduplication/',
			data: datastring,
			type:'POST',
			dataType:'json',
			async: false,
	...
	.addMessage('en', 'elrmobilecheck', 'The mobile number already exists.');
...
function elrcheckform(){
	...
	$('#captchaid').html('');
	var code=$('#code').val();
	var form_flag=$('#elrForm').parsley().validate();
	//alert('>>'+form_flag);
	if(code != '' && form_flag){
		$.ajax({
			url: site_url+'ELearning/ajax_check_captcha',
			type: 'post',
			data:'code='+code+'&random='+Math.random(),
			success: function(result) {
				if(result=='true'){
