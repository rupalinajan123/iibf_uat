<?php
## Refund paymentsCron - Created on 3-Apr-2021
try
{	
	/* MySQL DB Connection */
	...
	$get_details = mysqli_query($connect,"SELECT receipt_no FROM `total_refund` WHERE refund = 0");
	while($get_details_row = mysqli_fetch_array($get_details))
	{
		...
		if($q_result[2] == 'SUCCESS')
		{
			...
			echo "SELECT member_regnumber,receipt_no,amount,pay_type,status,gateway,date,transaction_no FROM `payment_transaction` WHERE receipt_no = '".$receipt_no."' AND gateway = 'sbiepay' LIMIT 1";
			
			$get_receipt_no = mysqli_query($connect,"SELECT member_regnumber,receipt_no,amount,pay_type,status,gateway,date,transaction_no FROM `payment_transaction` WHERE receipt_no = '".$receipt_no."' AND gateway = 'sbiepay' LIMIT 1");
			...
			while($row = mysqli_fetch_array($get_receipt_no))
			{
				...
				$transaction_no = $q_result[1];
					...
					if($inv[0]['invoice_no'] == '' || $inv[0]['invoice_image'] == '' || $inv[0]['transaction_no'] == '')
					{
						...
						if($pt_status == "3")
						{
							...
							echo $update_pt_sql = "UPDATE `payment_transaction` SET `status` = '".$pt_status."',transaction_no = '".$transaction_no."' WHERE receipt_no = '".$MerchantOrderNo."' AND gateway = 'sbiepay'";
						
							mysqli_query($connect,$update_pt_sql);
							
							
							##Update log
							echo $log = "UPDATE total_refund SET response = '".$pr_result_str."',refund = '1' WHERE receipt_no = '".$receipt_no."'";
							mysqli_query($connect,$log);
						
							echo "<br />Refund successful<br/>End at: ".date("Y-m-d H:i:s").'<br/>';
						}
					}
					else{
						
						echo $log = "UPDATE total_refund SET response = 'Invoice generated in exam_invoice',refund = '0' WHERE receipt_no = '".$receipt_no."'";
						mysqli_query($connect,$log);
					}
				}
				...
				echo $log = "UPDATE total_refund SET response = 'Refund Completed',refund = '0' WHERE receipt_no = '".$receipt_no."'";
				mysqli_query($connect,$log);
		}	
	}	
