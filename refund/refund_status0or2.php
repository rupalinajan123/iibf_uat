...
{	
	...
	$get_details = mysqli_query($connect,"SELECT receipt_no FROM `refund_status0or2` WHERE refund = 0 order by id desc LIMIT 36");
	...
	while($get_details_row = mysqli_fetch_array($get_details))
	{
		...
		if($q_result[2] == 'SUCCESS')
		{
			...
			##check invoice
			echo "<br/>SELECT transaction_no,invoice_no,invoice_image FROM `exam_invoice` WHERE receipt_no = '".$receipt_no."' LIMIT 1";
			...
			if($inv[0]['invoice_no'] == '' || $inv[0]['invoice_image'] == '' || $inv[0]['transaction_no'] == '')
			{
				...
				if($pt_status == "3")
				{
					...
					##Update log
					echo $log = "UPDATE refund_status0or2 SET response = '".$pr_result_str."',refund = '1' WHERE receipt_no = '".$receipt_no."'";
				...
				echo $log = "UPDATE refund_status0or2 SET response = 'Invoice generated in exam_invoice',refund = '0' WHERE receipt_no = '".$receipt_no."'";mysqli_query($connect,$log);
			}
		}
		}
	}
