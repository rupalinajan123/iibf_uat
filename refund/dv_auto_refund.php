<?php
/*
 * Module Name	:	SBIEPAY DV Auto Refund
 * Author Name	:	Bhushan Amrutkar
 * Created Date	:	08-09-2020
*/

// https://iibf.esdsconnect.com/refund/dv_auto_refund.php
// /usr/local/bin/php /home/supp0rttest/public_html/refund/dv_auto_refund.php
...
try
{
	...
	$get_details = mysql_query("SELECT order_id,trn_no  FROM `dv_payment_refunds` WHERE  refund_status  = ''");
	...
	while($get_details_row = mysql_fetch_array($get_details))
	{
		...
		$order_id = $get_details_row['order_id'];
		
		/* Get Details from `payment_transaction` DB Table */
		$get_receipt_no = mysql_query("SELECT receipt_no,pay_type,status FROM `payment_transaction` WHERE receipt_no = '".$order_id."' AND status = 0 ");
		...
		while($receipt_no_row = mysql_fetch_array($get_receipt_no))
		{
			...
			/* Get Details from `payment_transaction` DB Table */
			$sql = mysql_query("SELECT * FROM `payment_transaction` WHERE gateway = 'sbiepay' AND pay_type = '".$pay_type."' AND status = 0 AND receipt_no = '".$receipt_no."'");
			...
			{ 
				...
				$date = $row['date'];
				$atrn = $transaction_no = $transaction_no;
				...
				if ($q_result[8] == "Transaction Paid Out") 
				{
					...
					$insert_sql = "INSERT INTO `payment_refund`(`member_regnumber`, `gateway`, `amount`, `date`, `refund_request_id`, `transaction_no`, `receipt_no`, `ARRN`, `description`, `refund_details`, `status`, `admin_user_id`) VALUES ('".$member_regnumber."','".$gateway."','".$amount."','".date('Y-m-d H:i:s')."','".$refund_request_id."','".$transaction_no."','".$receipt_no."','','Found in DV Payment Refund Table  (".$member_regnumber.")','',2,'')";
					...
					$update_pt_sql = "UPDATE `payment_transaction` SET `status` = '".$pt_status."', transaction_no = '".$transaction_no."' WHERE receipt_no = '".$MerchantOrderNo."' AND gateway = 'sbiepay' AND pay_type = '".$pay_type."' AND status = 0";
					...
					$update_Maker_Checker_sql = "UPDATE `dv_payment_refunds` SET `refund_status` = 'Done' , refund_date = '".date('Y-m-d')."' WHERE order_id = '".$MerchantOrderNo."' ";
					mysql_query($update_Maker_Checker_sql);	
				}
			}
		}
	}
