 ...
 * Module Name	:	SBIEPAY Member Exam Refund
...
try{
	...
	$connect = mysqli_connect('10.11.38.120','supp0rttest_iifbportal','Hdz&BRo00PL1','supp0rttest_iibf_portal');
	...
	//$cid = mysqli_select_db($connect,'supp0rttest_iibf_portal');	//exit;

	

	 /*$sql = mysqli_query($connect,"SELECT * FROM `payment_transaction` WHERE gateway = 'sbiepay' AND pay_type = 2 AND status = 1 AND receipt_no IN ('902624652','902411439')");*/
	 
	 echo $sql = "SELECT * FROM `payment_transaction` WHERE gateway = 'sbiepay' AND pay_type = 2 AND status = 2 AND receipt_no IN ('902624652')";
	...
	while($row = mysqli_fetch_array($sql)){
		...
		echo "<BR> === Txn AMT : ".$refundAmount = $amount = $row['amount'];

		echo "<BR> === Txn Date : ".$date = $row['date'];

		echo "<BR> === Txn No. : ".$atrn = $transaction_no = $row['transaction_no'];

		echo "<BR> === Receipt No. : ".$MerchantOrderNo = $receipt_no = $row['receipt_no'];

		echo "<BR> === Gateway : ".$gateway = $row['gateway'];
		...
		if ($q_result[8] == "Transaction Paid Out") {
			...
			echo "<BR>Insert PR : ".$insert_sql = "INSERT INTO `payment_refund`(`member_regnumber`, `gateway`, `amount`, `date`, `refund_request_id`, `transaction_no`, `receipt_no`, `ARRN`, `description`, `refund_details`, `status`, `admin_user_id`) VALUES ('".$member_regnumber."','".$gateway."','".$amount."','".date('Y-m-d H:i:s')."','".$refund_request_id."','".$transaction_no."','".$receipt_no."','','Found in Monitoring No invoice Gen  - No response received from sbi to esds (".$member_regnumber.")','',2,'')";
			...
			echo "<BR>UPDATE PT : ".$update_pt_sql = "UPDATE `payment_transaction` SET `status` = '".$pt_status."' WHERE receipt_no = '".$MerchantOrderNo."' AND gateway = 'sbiepay' AND pay_type = 2 AND status = 1";
