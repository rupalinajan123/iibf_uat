<?php
/*
 * Module Name	:	SBIEPAY DRA Member Exam Refund
...
try{
	...
	//$sql = mysql_query("SELECT * FROM `dra_payment_transaction` WHERE gateway = 2 AND amount = '33630.00' AND status = 1 AND receipt_no IN (707903)");
	$sql = mysql_query("SELECT * FROM `dra_payment_transaction` WHERE gateway = 2 AND status = 1 AND receipt_no IN (709979)");
	while($row = mysql_fetch_array($sql)){
		echo "<BR> DRA Inst Code = ".$member_regnumber = "DRA_inst_code: ".$row['inst_code'];
		...
		echo "<br> === Gateway : ".$gateway = "sbiepay"; //$row['gateway'];
		...
		if ($q_result[8] == "Transaction Paid Out") {
			...
			echo "<br><BR>Insert PR : ".$insert_sql = "INSERT INTO `payment_refund`(`member_regnumber`, `gateway`, `amount`, `date`, `refund_request_id`, `transaction_no`, `receipt_no`, `ARRN`, `description`, `refund_details`, `status`, `admin_user_id`) VALUES ('".$member_regnumber."','".$gateway."','".$amount."','".date('Y-m-d H:i:s')."','".$refund_request_id."','".$transaction_no."','".$receipt_no."','','Found In monitoring Dual Application - double entries found in our data base - Tejasvi Email','',2,'')";
			...
			if ($pr_result[3] == "SUCCESS")
			{
				$pr_status = "1";
				//$pt_status = "3";
				$pt_status = "0"; // FOR ONLY DRA Refund = 0
			}
			else
			{
				$pr_status = "0";
			...
			echo "<br><BR>UPDATE PT : ".$update_pt_sql = "UPDATE `dra_payment_transaction` SET `status` = '".$pt_status."' WHERE receipt_no = '".$MerchantOrderNo."' AND status = 1";
...
function sbirefundapi($MerchantOrderNo, $refundRequestId, $atrn, $refundAmount)
{
	...
	//echo "<BR>Refund Response = ".$result;
