<?php
/*
 * Module Name	:	SBIEPAY Blended Refunds
 * Author Name	:	Bhushan Amrutkar
 * Created Date	:	09-10-2019
*/

// https://iibf.esdsconnect.com/refund/blended_refunds.php
// /usr/local/bin/php /home/supp0rttest/public_html/refund/blended_refunds.php
...
try
{
	...
	/* Get Details from `blended_refunds` DB Table */
	$get_details = mysql_query("SELECT transaction_no,receipt_no FROM `blended_refunds` WHERE refund_status = '0'");
	...
	while($get_details_row = mysql_fetch_array($get_details))
	{
		...
		$get_receipt_no = mysql_query("SELECT receipt_no,pay_type,status FROM `payment_transaction` WHERE receipt_no = '".$receipt_no."' AND transaction_no = '".$transaction_no."' AND pay_type = '".$module."' AND status = 1 ");
		...
		while($receipt_no_row = mysql_fetch_array($get_receipt_no))
		{
			...
			/* Get Details from `payment_transaction` DB Table */
			$sql = mysql_query("SELECT * FROM `payment_transaction` WHERE gateway = 'sbiepay' AND pay_type = '".$pay_type."' AND status = 1 AND receipt_no = '".$receipt_no."'");
			...
			{ 
				...
				if ($q_result[8] == "Transaction Paid Out") 
				{
					...
					$insert_sql = "INSERT INTO `payment_refund`(`member_regnumber`, `gateway`, `amount`, `date`, `refund_request_id`, `transaction_no`, `receipt_no`, `ARRN`, `description`, `refund_details`, `status`, `admin_user_id`) VALUES ('".$member_regnumber."','".$gateway."','".$amount."','".date('Y-m-d H:i:s')."','".$refund_request_id."','".$transaction_no."','".$receipt_no."','','Found in blended_refunds Table (".$member_regnumber.")','',2,'')";
					...
					/* Update Details in `exam_invoice_settlement` DB Table */
					$update_blended_refunds_sql = "UPDATE `blended_refunds` SET `refund_status` = 1, refund_date = '".date('Y-m-d')."' WHERE transaction_no = '".$transaction_no."' ";
					mysql_query($update_blended_refunds_sql);	
				}
			}
		}
	}
