<?php
/*
 * Module Name	:	SBIEPAY Auto Refund
 * Author Name	:	Bhushan Amrutkar
 * Created Date	:	11-09-2019
*/

// https://iibf.esdsconnect.com/refund/auto_refund.php
// /usr/local/bin/php /home/supp0rttest/public_html/refund/auto_refund.php
...
{	echo 'in try'; 
	...
	echo "<br/>SELECT transaction_no,req_module,req_status FROM `maker_checker` WHERE  req_status  = '4' AND transaction_no = '9957306720335' <br/>";
	$get_details = mysqli_query($connect,"SELECT transaction_no,req_module,req_status FROM `maker_checker` WHERE  req_status  = '4' AND transaction_no = '9957306720335' ");
	print_r($get_details);
	...
	while($get_details_row = mysqli_fetch_array($get_details))
	{
		...
		$get_receipt_no = mysqli_query($connect,"SELECT receipt_no,pay_type,status FROM `payment_transaction` WHERE transaction_no = '".$transaction_no."' AND pay_type = '".$req_module."' AND status = 1 ");
	
		while($receipt_no_row = mysqli_fetch_array($get_receipt_no))
		{
			$receipt_no = $receipt_no_row['receipt_no'];
			$pay_type = $receipt_no_row['pay_type'];
			
			/* Get Details from `payment_transaction` DB Table */
			$sql = mysqli_query($connect,"SELECT * FROM `payment_transaction` WHERE gateway = 'sbiepay' AND pay_type = '".$pay_type."' AND status = 1 AND receipt_no = '".$receipt_no."'");
			...
			{ 
				...
				$gateway = $row['gateway'];
				echo "<br/>Request send to sbi at: ".date("Y-m-d H:i:s");
				...
				if ($q_result[8] == "Transaction Paid Out") 
				{
					...
					$update_pt_sql = "UPDATE `payment_transaction` SET `status` = '".$pt_status."' WHERE receipt_no = '".$MerchantOrderNo."' AND gateway = 'sbiepay' AND pay_type = '".$pay_type."' AND status = 1";
					...
					$update_Maker_Checker_sql = "UPDATE `maker_checker` SET `req_status` = 5 , refund_date = '".date('Y-m-d')."' WHERE transaction_no = '".$transaction_no."' AND req_status  = 4 ";
					mysqli_query($connect,$update_Maker_Checker_sql);	
					
					echo "<br />End at: ".date("Y-m-d H:i:s");
				}
			}
		}
	}
...
function generate_refund_request_id($field_name)
{
	...
	$sc_query = mysqli_query($connect,"SELECT value FROM site_config WHERE name = '".$field_name."'");
	...
	if (mysqli_num_rows($sc_query) > 0)
	{
		...
		mysqli_query($connect,$update_sc_sql);
