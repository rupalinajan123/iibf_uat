...
// https://iibf.esdsconnect.com/refund/seat_full_refund.php
// /usr/local/bin/php /home/supp0rttest/public_html/refund/seat_full_refund.php
...
try
{
	...
	$get_details = mysql_query("SELECT transaction_no,receipt_no FROM `exam_invoice_settlement` WHERE refund_case = '1' AND refund_status = '0' ");
	exit;
		...
		while($receipt_no_row = mysql_fetch_array($get_receipt_no))
		{
			...
			{ 
				...
				if ($q_result[8] == "Transaction Paid Out") 
				{
					...
					$insert_sql = "INSERT INTO `payment_refund`(`member_regnumber`, `gateway`, `amount`, `date`, `refund_request_id`, `transaction_no`, `receipt_no`, `ARRN`, `description`, `refund_details`, `status`, `admin_user_id`) VALUES ('".$member_regnumber."','".$gateway."','".$amount."','".date('Y-m-d H:i:s')."','".$refund_request_id."','".$transaction_no."','".$receipt_no."','','Found in exam_invoice_settlement Table - seat full case (".$member_regnumber.")','',2,'')";
					...
					$update_exam_invoice_settlement_sql = "UPDATE `exam_invoice_settlement` SET `refund_status` = 1, refund_date = '".date('Y-m-d')."' WHERE transaction_no = '".$transaction_no."' ";
