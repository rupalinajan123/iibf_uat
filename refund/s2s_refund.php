<?php
## Cron - Created on 06-Apr-2021
## Daily cron to auto refund for s2s
try
{
	/* MySQL DB Connection */
	...
	$get_details = mysqli_query($connect,'SELECT receipt_no FROM S2S_direcrt_refund WHERE status = 0  ORDER By id DESC LIMIT 1');
			...
			while($row = mysqli_fetch_array($get_receipt_no))
			{
				...
				$transaction_no = $q_result[1];
					...
					if($inv[0]['invoice_no'] == '' || $inv[0]['invoice_image'] == '' || $inv[0]['transaction_no'] == '')
					{
						...
						echo $log = "INSERT INTO cron_auto_refund_log (receipt_no,transaction_no,response,refund) VALUES ('".$receipt_no."','".$transaction_no."','".$pr_result_str."','0')";
						mysqli_query($connect,$log);
					}
				}
