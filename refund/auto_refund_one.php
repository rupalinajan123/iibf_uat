...
{	
	...
	while($get_details_row = mysqli_fetch_array($get_details))
	{
		...
		echo "SELECT member_regnumber,receipt_no,amount,pay_type,status,gateway,date FROM `payment_transaction` WHERE receipt_no = '".$receipt_no."' AND pay_type = '2' AND status = '2' AND gateway = 'sbiepay' LIMIT 1";
		
		$get_receipt_no = mysqli_query($connect,"SELECT member_regnumber,receipt_no,amount,pay_type,status,gateway,date FROM `payment_transaction` WHERE receipt_no = '".$receipt_no."' AND pay_type = '2' AND status = '2' AND gateway = 'sbiepay' LIMIT 1");
		...
		while($row = mysqli_fetch_array($get_receipt_no))
		{
			...
			if($pt_status == "3")
			{
				...
				echo $update_pt_sql = "UPDATE `payment_transaction` SET `status` = '".$pt_status."',transaction_no = '".$transaction_no."' WHERE receipt_no = '".$MerchantOrderNo."' AND gateway = 'sbiepay' AND pay_type = '2' AND status = 2";
			
				mysqli_query($connect,$update_pt_sql);
				
				
				##Update log
				echo $log = "UPDATE refund_one SET response = '".$pr_result_str."' WHERE receipt_no = '".$receipt_no."'";
				...
				echo $log = "UPDATE refund_one SET response = '".$pr_result_str."' WHERE receipt_no = '".$receipt_no."'";mysqli_query($connect,$log);
			}
		}
	}
} catch(Exception $e){ 
