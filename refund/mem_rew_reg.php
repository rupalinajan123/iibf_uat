<?php
/*
 * Module Name	:	SBIEPAY Member Renewal Registration Refund
 * Author Name	:	Bhushan
...
try{
	...
	$sql = mysql_query("SELECT * FROM `payment_transaction` WHERE gateway = 'sbiepay' AND amount = '1770.00' AND pay_type = 5 AND status = 1 AND receipt_no IN ('812162947')");
	while($row = mysql_fetch_array($sql)){
		...
		if ($q_result[8] == "Transaction Paid Out") {
			...
			echo "<BR>Insert PR : ".$insert_sql = "INSERT INTO `payment_refund`(`member_regnumber`, `gateway`, `amount`, `date`, `refund_request_id`, `transaction_no`, `receipt_no`, `ARRN`, `description`, `refund_details`, `status`, `admin_user_id`) VALUES ('".$member_regnumber."','".$gateway."','".$amount."','".date('Y-m-d H:i:s')."','".$refund_request_id."','".$transaction_no."','".$receipt_no."','','Found In SBI MIS Report (Rs.1770.00)-Member Renewal module','',2,'')";
			...
			echo "<BR>UPDATE PT : ".$update_pt_sql = "UPDATE `payment_transaction` SET `status` = '".$pt_status."' WHERE receipt_no = '".$MerchantOrderNo."' AND gateway = 'sbiepay' AND amount = '1770.00' AND pay_type = 5 AND status = 1";
