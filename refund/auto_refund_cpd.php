...
{	
	...
	/* Get Details from `maker_checker` DB Table */
	 $get_details = mysqli_query($connect,"SELECT * FROM `maker_checker` WHERE  `req_module` = 9 AND `req_status` = 4 "); 	
	...
	while($get_details_row = mysqli_fetch_array($get_details))
	{
		...
		echo "SELECT member_regnumber,receipt_no,amount,pay_type,status,transaction_no,gateway,date FROM `payment_transaction` WHERE transaction_no = '".$transaction_no."' AND pay_type = '9' AND status = 1 AND gateway = 'sbiepay' LIMIT 1";
		
		$get_receipt_no = mysqli_query($connect,"SELECT member_regnumber,receipt_no,amount,pay_type,status,transaction_no,gateway,date FROM `payment_transaction` WHERE transaction_no = '".$transaction_no."' AND pay_type = '9' AND status = 1 AND gateway = 'sbiepay' LIMIT 1");
		...
		while($row = mysqli_fetch_array($get_receipt_no))
		{
			...
			if($pt_status == "3")
			{
				...
				echo $update_pt_sql = "UPDATE `payment_transaction` SET `status` = '".$pt_status."' WHERE receipt_no = '".$MerchantOrderNo."' AND gateway = 'sbiepay' AND pay_type = '9' AND status = 1";
