<?php
## Cron - Created on 03-Apr-2021
## Daily cron to auto refund for transactions with status 0 or 2
try
{
	/* MySQL DB Connection */
	...
	/* $get_details = mysqli_query($connect,'SELECT receipt_no FROM payment_transaction WHERE date <= NOW() - INTERVAL 120 MINUTE 
			AND date > NOW() - INTERVAL 180 MINUTE AND gateway = "sbiepay" AND (status = 2 OR status = 0)'); */
	         $get_details = mysqli_query($connect,'SELECT receipt_no FROM payment_transaction WHERE date BETWEEN \'2022-04-09\' AND \'2022-04-27\' AND gateway = "sbiepay" AND (status = 2 OR status = 0)');
	...
	while($get_details_row = mysqli_fetch_array($get_details))
	{
		...
		//Check transaction status at sbi end
		...
		if($q_result[2] == 'SUCCESS')
		{
			...
			echo "SELECT member_regnumber,receipt_no,amount,pay_type,status,gateway,date,transaction_no FROM `payment_transaction` WHERE receipt_no = '".$receipt_no."' AND (status = '0' OR status = '2') AND gateway = 'sbiepay' LIMIT 1";
			
			$get_receipt_no = mysqli_query($connect,"SELECT member_regnumber,receipt_no,amount,pay_type,status,gateway,date,transaction_no FROM `payment_transaction` WHERE receipt_no = '".$receipt_no."' AND (status = '0' OR status = '2') AND gateway = 'sbiepay' LIMIT 1");
				...
				$transaction_no = $q_result[1];
					...
					if($inv[0]['invoice_no'] == '' || $inv[0]['invoice_image'] == '' || $inv[0]['transaction_no'] == '')
					{
						...
						if($pt_status == "3")
						{
							...
							echo $update_pt_sql = "UPDATE `payment_transaction` SET `status` = '".$pt_status."',transaction_no = '".$transaction_no."' WHERE receipt_no = '".$MerchantOrderNo."' AND gateway = 'sbiepay' AND (status = '0' OR status = '2')";
						
							mysqli_query($connect,$update_pt_sql);
							
							
							##Update log
							echo $log = "INSERT INTO cron_auto_refund_log (receipt_no,transaction_no,response,refund) VALUES ('".$receipt_no."','".$transaction_no."','".$pr_result_str."','1')";
						...
						echo $log = "INSERT INTO cron_auto_refund_log (receipt_no,transaction_no,response,refund) VALUES ('".$receipt_no."','".$transaction_no."','".implode("|",$q_result)."','0')";
						mysqli_query($connect,$log);
					}
				}
