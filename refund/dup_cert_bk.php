...
try{
	...
	$pay_type = '9';
	$why_refund = 'client mail - CPD registration for member:510360852 - No Invoice,no Application';
	//$why_refund = 'Found in SBI MIS Report-Only payment debited-No Invoice,no Application,also 0 response from gateway';
	$sql = mysqli_query($connect,"SELECT * FROM `payment_transaction` WHERE gateway = 'sbiepay' AND pay_type = '".$pay_type."' AND status = 1 AND receipt_no IN ('812193360')");
	...
	while($row = mysqli_fetch_array($connect,$sql))
	{
		...
		if ($q_result[8] == "Transaction Paid Out") 
		{
			...
			echo "<BR><BR> UPDATE PT : ".$update_pt_sql = "UPDATE `payment_transaction` SET `status` = '".$pt_status."' WHERE receipt_no = '".$MerchantOrderNo."' AND gateway = 'sbiepay' AND pay_type = '".$pay_type."' AND status = 1";
...
function generate_refund_request_id($field_name)
{
	...
	if (mysqli_num_rows($connect,$sc_query) > 0)
	{
		$sc_row = mysqli_fetch_array($connect,$sc_query);
