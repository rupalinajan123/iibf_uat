...
function callback()
{
  ...
  $curl_opts = array(
    ...
    CURLOPT_URL => RESOURCE_URL . '?access_token=' . $token_resp_data['access_token']
  ...
  {
    $produrl = 'https://' . $_SERVER['HTTP_HOST'];
  }
  else
  {
    $produrl = 'http://' . $_SERVER['HTTP_HOST'];
  }
  ...
  print_r($_SESSION);
  echo '</pre>';
  echo '<br>'.$produrl . '/staging/WalletRedirect/wallet/' . $csc_id; exit; */

  header('location:' . $produrl . '/staging/WalletRedirect/wallet/' . $csc_id);
}
...
function fetch_data($url, $post, $heads)
{
  ...
  {
    $api_res_msg = curl_error($curl);
    echo '<br> Curl Error<br>';
    echo '<pre>'; print_r($api_res_msg); echo '</pre>';
    echo '<br>'.$api_res_msg; 
    exit;
  }
  ...
  if (!$result)
  {
    ...
    echo '<pre>'; print_r(array('Error code' => $httpcode, 'URL' => $url, 'post' => $post, 'LOG' => "")); echo '</pre>';
    exit("Error: 378972");
  }
  curl_close($curl);
